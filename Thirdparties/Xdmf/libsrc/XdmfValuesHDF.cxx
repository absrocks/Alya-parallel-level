...
/*                                                                 */
/*  Id : $Id: XdmfValuesHDF.cxx,v 1.18 2010-01-06 16:34:07 kwleiter Exp $  */
/*  Date : $Date: 2010-01-06 16:34:07 $ */
...
XdmfValuesHDF::Write(XdmfArray *anArray, XdmfConstString aHeavyDataSetName){
    ...
    H5.SetDsmBuffer(this->DsmBuffer);
    if(H5.Open(hds, "rw") == XDMF_FAIL){ // Original
    //if(H5.Open(hds, "W+") == XDMF_FAIL){ // Modification Raul
...
/* Future improvement: return a XdmfXmlNode instead a XdmfString */
XdmfString XdmfValuesHDF::DataItemFromHDF(XdmfConstString H5DataSet, XdmfInt32 serialFile) {
    XdmfHDF         H5;
    ostrstream      StringOutput;
    char            *Ptr;
    static XdmfString ReturnString = NULL;

    H5.SetUseSerialFile( serialFile );
    if(H5.Open(H5DataSet, "r") == XDMF_FAIL){
        XdmfErrorMessage("Can't open H5 Dataset " << H5DataSet << " for reading");
        return(NULL);
    }
    StringOutput << "<DataItem Format=\"HDF";
    StringOutput << "\" NumberType=\"";
