...
public:
    ...
    //! Check to see if Element references another XML node (which will then be returned) or if Element is a valid reference object
    /*!
    \param Element is the head XML node of a potential reference chain for Read mode and the reference object to verify in Write mode
    \param refElement ( NULL = Read | Reference Element = Write ) specifies how to verify Reference (using DOM->Get("Reference", Element) or DOM->GetPath(refElement))
    */
    virtual XdmfXmlNode CheckForReference(XdmfXmlNode Element, XdmfXmlNode refElement = NULL);
...
//! Get the XML Node
    ...
    */
    ...
    XdmfInt32 BuildFromDataXml(XdmfInt32 AllowCData = 0, XdmfInt32 force = 0);
    ...
    XdmfXmlNode FollowReference(XdmfXmlNode Element, XdmfXmlNode refElement = NULL);
    //! Get DSM Buffer
    ...
    //! Returns the number of parents (count ancestors)
    XdmfInt32 GetNumberOfParents(XdmfXmlNode anElement);
