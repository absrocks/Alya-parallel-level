...
**************************************************************************/
void ReadGraph(GraphType *graph, char *filename, my_int *wgtflag)
{
  my_int i, j, k, l, fmt, readew, readvw, ncon, edge, ewgt;
  ...
  *wgtflag = 0;
  ...
  for (xadj[0]=0, k=0, i=0; i<graph->nvtxs; i++) {
    ...
    if (readvw) {
      for (l=0; l<ncon; l++) {
        vwgt[i*ncon+l] = (my_int)strtol(oldstr, &newstr, 10);
        oldstr = newstr;
      }
    }

    for (;;) {
      edge = (my_int)strtol(oldstr, &newstr, 10) -1;
      oldstr = newstr;

      if (readew) {
        ewgt = (my_int)strtol(oldstr, &newstr, 10);
        oldstr = newstr;
      }
...
**************************************************************************/
void WritePartition(char *fname, idxtype *part, my_int n, my_int nparts)
{
  FILE *fpout;
  my_int i;
  char filename[256];
...
**************************************************************************/
void WriteMeshPartition(char *fname, my_int nparts, my_int ne, idxtype *epart, my_int nn, idxtype *npart)
{
  FILE *fpout;
  my_int i;
  char filename[256];
...
* This function writes out the partition vector
**************************************************************************/
void WritePermutation(char *fname, idxtype *iperm, my_int n)
{
  FILE *fpout;
  my_int i;
  char filename[256];
...
**************************************************************************/
my_int CheckGraph(GraphType *graph)
{
  my_int i, j, k, l, nvtxs, err=0;
...
**************************************************************************/
idxtype *ReadMesh(char *filename, my_int *ne, my_int *nn, my_int *etype)
{
  my_int i, j, k, esize;
  idxtype *elmnts;
  FILE *fpin;
...
**************************************************************************/
void WriteGraph(char *filename, my_int nvtxs, idxtype *xadj, idxtype *adjncy)
{
  my_int i, j;
  FILE *fpout;
...
void WriteMocGraph(GraphType *graph)
{
  ...
  for (i=0; i<nvtxs; i++) {
    fprintf(fpout, "\n");
    for (j=0; j<ncon; j++)
      fprintf(fpout, "%d ", (my_int)((float)10e6*nvwgt[i*ncon+j]));
