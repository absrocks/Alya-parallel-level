module CommonArgs
  def self.init(pgaus,pnode,ndime,vector_length,nb_instances,seed = nil)
    ...
    @@kfl_p1ve2_nsi = 0
    @@kfl_linea_nsi = 2
    @@pabdf_nsi     = ANArray.float(8*vector_length,10).random!
    @@pabdf_nsi.fill(1.0)
    @@kfl_confi_nsi = 0
    ...
    @@gpgve     = ANArray.float(8*vector_length,vector_length,@@ndime,@@pgaus,10).random!
    ...
    @@densi = ANArray.float(8*vector_length,vector_length,@@pgaus,@@nbdfp_nsi).random!
    ...
    nb_instances.times{|i|
      ...
      when 0
        ...
        @@gpgrp.push ANArray.float(8*vector_length, *(@@gpgrp[0].shape))[] = @@gpgrp[0][] 
        @@gprhs.push ANArray.float(8*vector_length, *(@@gprhs[0].shape))[] = @@gprhs[0][]
        @@gpvep.push ANArray.float(8*vector_length, *(@@gpvep[0].shape))[] = @@gpvep[0][]
        @@gprhc.push ANArray.float(8*vector_length, *(@@gprhc[0].shape))[] = @@gprhc[0][]
        @@gprhs_sgs.push ANArray.float(8*vector_length, *(@@gprhs_sgs[0].shape))[] = @@gprhs_sgs[0][]
      end
    }
  end
end
