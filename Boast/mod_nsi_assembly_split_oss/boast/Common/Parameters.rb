...
module Parameters
  ...
  $kfl_convection_type_nsi = Int("kfl_convection_type_nsi",  :dir => :in) ###
  $nsi_galerkin = Int("NSI_GALERKIN",    :dir => :in) ###
  $nsi_algebraic_split_oss = Int("NSI_ALGEBRAIC_SPLIT_OSS",    :dir => :in) ###
  $nsi_fractional_step_int = Int("NSI_FRACTIONAL_STEP_int", :dir => :in) ###
  $nsi_convection_conservative = Int("NSI_CONVECTION_CONSERVATIVE", :dir => :in) ###
  $nsi_convection_skew = Int("NSI_CONVECTION_SKEW", :dir => :in) ###
  $nsi_convection_emac = Int("NSI_CONVECTION_EMAC", :dir => :in) ###
...
#  $kfl_bubbl_nsi = Int("kfl_bubbl_nsi", :dir => :in)
  ...
  def self.initialize(vector_length)
    ...
    $elbub     = Real("elbub",     :dir => :in,     :vector_length => vector_length)
...
#    $rcont     = Real("rcont",     :dir => :in,     :vector_length => vector_length, :dim => [Dim($ndime),Dim($pnode),Dim($pgaus)])
    $pabdf_nsi     = Real("pabdf_nsi",    :dir => :inout, :dim => [Dim(10)])
...
#    $rmom2 = Real("rmom2", :dir => :in,  :vector_length => vector_length, :dim => [Dim($ndime),Dim($ndime),Dim($pnode),Dim($pgaus)])
    ...
    $gpgve = Real("gpgve", :dir => :in,  :vector_length => vector_length, :dim => [Dim($ndime),Dim($pgaus),Dim()])
...
#    $gppre = Real("gppre", :dir => :in,  :vector_length => vector_length, :dim => [Dim($pgaus)]) 
    ...
    # Others
    $dtinv_loc = Real("dtinv_loc", :dir => :in, :vector_length => vector_length)
    $dtsgs     = Real("dtsgs",     :dir => :in, :vector_length => vector_length)
    $pbubl     = Int("pbubl",      :dir => :in, :vector_length => vector_length, :dim => 1)
    ...
    $gpcar_bub = Real("gpcar_bub", :dir => :in, :vector_length => vector_length, :dim => [Dim($ndime),Dim($pgaus)])
		$densi		 = Real("densi",		 :dir => :in, :vector_length => vector_length,  :dim => [Dim($pgaus), Dim($nbdfp_nsi)])
    ...
    $agrau     = Real("agrau",:vector_length => vector_length, :dim => [Dim($pnode),Dim($pgaus)],:allocate => allocate)
    $ivect  = Int("ivect")
    $kdime  = Int("kdime")
    $xvis2  = Real("xvis2")
    $xvisc  = Real("xvisc")
    ...
    $c1        = Real("c1",       :vector_length => vector_length,      :allocate => allocate)
    $c2        = Real("c2",        :vector_length => vector_length,      :allocate => allocate)
    $c3        = Real("c3",        :vector_length => vector_length,      :allocate => allocate)
    $c4        = Real("c4",        :vector_length => vector_length,      :allocate => allocate)
    ...
    $alpha     = Real("alpha",     :vector_length => vector_length,      :allocate => allocate)
    $beta      = Real("beta",      :vector_length => vector_length,      :allocate => allocate)
    ...
    $dtinv_mod = Real("dtinv_mod", :vector_length => vector_length,      :allocate => allocate)
    ...
    $idofv      = Int("idofv", :dim => [Dim(3)], :allocate => allocate)
    $jdofv      = Int("jdofv", :dim => [Dim(3)], :allocate => allocate)
  end
