...
class Example
  ...
  def self.run(vector_size,unroll,inline,oflags,lang)

    # Specify the kernel options
		nests = (1..12).to_a


		if inline == ":included" then
			inline = :included
		elsif inline == ":call" then
			inline = :call
		elsif inline == ":inlined" then
			inline = :inlined 
		end
    ...
    k_boast_params = {:vector_length => vector_size, :nests => nests, :unroll => unroll, :inline => inline, :OFLAGS => oflags}
  

  	filename="nsi_element_assembly_split_oss_boast.f90"
	
		if lang == "C" then
  		filename="nsi_element_assembly_split_oss_boast.c"
			lang=C
		else
			lang=FORTRAN
		end
    ...
    k.generate
    puts k.kernel   
    #k.kernel.build(:FCFLAGS => "-fimplicit-none ", :LDFLAGS => "-lgfortran")
		#k.kernel.build(:FCFLAGS => "-fbounds-check -fimplicit-none", :LDFLAGS => "-lgfortran")
		k.kernel.build(:FCFLAGS => oflags, :LDFLAGS => "-lgfortran")
...
opt_parser = OptionParser.new { |opts|
  ...
  opts.on("-uVAL","--unroll=VAL", "Specify unroll value (expected true or false)") { |n|
    options[:unroll] = n
  }
  opts.on("-inVAL","--inline=VAL", "Specify inline value (expected call, included or inlined)") { |n|
    options[:inline] = n
  }
  opts.on("-oVAL","--oflags=VAL", "Specify OFLAGS value (ex: -02)") { |n|
    options[:OFLAGS] = n
  }
...
k = Example.run(options[:vector_size], options[:unroll], options[:inline], options[:OFLAGS], options[:lang])
