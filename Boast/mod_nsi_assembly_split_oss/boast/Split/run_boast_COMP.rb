...
class Experiment
  ...
  def self.run(output_info,vector_size, lang)
    nests = (1..12).to_a
    ...
    nests.each{|n|
      opts1 = {:vector_length => vector_size, :preprocessor => false, :nests => [n], :unroll => true, :inline => :included, :CFLAGS => "-O3", :FCFLAGS => "-fimplicit-none"}
      #SVG: opts1 = {:vector_length => vector_size, :preprocessor => false, :nests => [n], :unroll => true, :inline => :included, :CFLAGS => "-O3 -fbounds-check", :FCFLAGS => "-fbounds-check -fimplicit-none"}
      opts2 = {:vector_length => vector_size, :preprocessor => false, :nests => [n], :unroll => true, :inline => :inlined, :CFLAGS => "-O3", :FCFLAGS => "-fimplicit-none"}
      opts3 = {:vector_length => vector_size, :preprocessor => false, :nests => [n], :unroll => true, :inline => :call, :CFLAGS => "-O3", :FCFLAGS => "-fimplicit-none"}
...
stats, k = Experiment.run(options[:kernel_path],options[:vector_size], options[:lang])
