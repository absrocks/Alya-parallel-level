...
class Example
  ...
  def self.run(vector_size)
    ...
    k_ref_params = {:vector_length => vector_size,:nests => nests, :unroll => false, :inline => :included}
    
    set_lang(FORTRAN)
    #set_lang(C)

    # Create the kernel
    k = KSplitOssRef::new(k_ref_params)
    # Generate the will create a CKernel instance available using the variable kernel.
    # And can be used directly used as usuall 
    k.generate
    puts k.kernel   
    #k.kernel.build(:FCFLAGS => "-fbounds-check", :LDFLAGS => "-lgfortran")
    k.kernel.build
