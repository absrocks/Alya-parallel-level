...
contains
  ...
  subroutine nsi_assembly(iassembly,mnode,pnode,pgaus,ntens,ncomp_nsi,amatr,rhsid)
    ...
    real(rp)                 :: densi(VECTOR_SIZE,pgaus,3)                        ! Density at previous time-steps
    !
    ! Enrichement
    !
    ...
    integer(ip)    :: kfl_convection_type_nsi


    integer(ip)    :: NSI_FRACTIONAL_STEP_int=0
