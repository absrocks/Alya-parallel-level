...
int VersionList[] = {2400, 2300, 2200, 2100, 2000,
                     1270, 1200, 1100, 1050};
...
/* end KMW */
        ...
        (*nuser_data) = ((cgns_rotating *)posit)->nuser_data;
         ...
         (*nuser_data) = ((cgns_user_data *)posit)->nuser_data;
    else if (strcmp(posit_label,"GridConnectivityProperty_t")==0)
...
int cg_ptset_info(PointSetType_t *ptset_type, int *npnts)
{
    ...
    if (cgi_check_mode(cg->filename, cg->mode, MODE_READ))
	return CG_ERROR;

    ptset = cgi_ptset_address(MODE_READ, &ier);
    if (ptset == 0)
	return ier;
...
/* begin KMW */
	strcmp(posit_label,"EMElectricFieldModel_t") &&
	strcmp(posit_label,"EMMagneticFieldModel_t") &&
	strcmp(posit_label,"EMConductivityModel_t") &&
/* end KMW */
...
int cg_ptset_write(PointSetType_t ptset_type, int npnts, int const * pnts)
{
    ...
    /* verify input */
    if (cgi_check_mode(cg->filename, cg->mode, MODE_WRITE))
	return CG_ERROR;

    ptset = cgi_ptset_address(MODE_WRITE, &ier);
    if (ptset==0)
	return ier;
			   ...
			   int *rmax, void const *field_ptr, int *F)
{
    ...
    for (index=0; index<sol->nfields; index++) {
        if (strcmp(fieldname, sol->field[index].name)==0) {
            field = &(sol->field[index]);
             ...
             /* in MODE_WRITE, children names must be unique */
            if (cg->mode==MODE_WRITE) {
		int m_start[12], m_end[12], m_dim[12], stride[12], i;

		for (i = 0; i < field->data_dim; ++i)
		{
		    if(rmin[i] >= 1 && rmin[i] <= rmax[i] &&
		       rmax[i] <= field->dim_vals[i])
		    {
			m_start[i] = 1;
		...
		return CG_OK;
             ...
             /* overwrite an existing solution */
                ...
                if (type==cgi_datatype(field->data_type)) {
		    /* delete the existing base from file */
		    if (cgi_delete_node(sol->id, field->id))
			return CG_ERROR;

		    /* free memory */
		    cgi_free_array(field);
		}
            }
            break;
        }
    ...
    for (n=0; n<field->data_dim; n++) {
        if (sol->location == CellCenter)
            field->dim_vals[n] = zone->nijk[n+zone->index_dim] + sol->rind_planes[2*n]
                               + sol->rind_planes[2*n+1];
        else if (sol->location == Vertex)
            field->dim_vals[n] = zone->nijk[n] + sol->rind_planes[2*n]
                               + sol->rind_planes[2*n+1];
        else if (sol->location == IFaceCenter || sol->location == JFaceCenter ||
             sol->location == KFaceCenter) {
            field->dim_vals[n] = zone->nijk[n] + sol->rind_planes[2*n]
                               + sol->rind_planes[2*n+1];
            if ((sol->location == IFaceCenter && n!=0) ||
                (sol->location == JFaceCenter && n!=1) ||
                (sol->location == KFaceCenter && n!=2)) field->dim_vals[n]--;
        } else {
            cgi_error("Location not yet supported");
            return CG_ERROR;
        }

	if( !(rmin[n] >= 1 && rmin[n] <= rmax[n] &&
    ...
    for (index=0; index<zcoor->ncoords; index++) {
        if (strcmp(coordname, zcoor->coord[index].name)==0) {
            coord = &(zcoor->coord[index]);
             ...
             /* in MODE_WRITE, append the new data */
            if (cg->mode==MODE_WRITE) {
		int m_start[12], m_end[12], m_dim[12], stride[12], i;

		for (i = 0; i < coord->data_dim; ++i)
		{
		    if(rmin[i] >= 1 && rmin[i] <= rmax[i] &&
		       rmax[i] <= coord->dim_vals[i])
		    {
			m_start[i] = 1;
			m_end[i] = rmax[i] - rmin[i] + 1;
			m_dim[i] = m_end[i];
		...
		return CG_OK;
             ...
             /* overwrite an existing coordinate vector */
                ...
                if (type==cgi_datatype(coord->data_type)) {
		    /* delete the existing base from file */
		    if (cgi_delete_node(zcoor->id, coord->id ))
			return CG_ERROR;

		    /* free memory */
		    cgi_free_array(coord);
		}
            }
            break;
        }
			     ...
			     int const * elements, int *S)
{
    ...
    int index, num, npe;
    int ElementDataSize=0;
    int *combined_elems = 0;
    cgns_array *parent_save = 0;
    ...
    for (index=0; index<zone->nsections; index++) {
	    ...
	    /* in MODE_WRITE, extract needed data from existing node */
            if (cg->mode==MODE_WRITE) {
		int *el_conn_ptr[2];
		int elems[3], elemdatasize[3], i, num;

		/* must be able to create a continuous combined elems array*/
		if(start > section->range[1] + 1 || end < section->range[0] - 1)
		{
		    cgi_error("Invalid element range defined for section '%s'",SectionName);
		    return CG_ERROR;
		}

		/* must have the existing and new element lists of the
		 * same type
		 */
		if(type != section->el_type)
		{
		    cgi_error("Invalid element types for combined element lists for section '%s'",SectionName);
		    return CG_ERROR;
		}

		if(start <= section->range[0])
		{
		    /* new elements are written first */
		    el_conn_ptr[0] = (int *)elements;
		    el_conn_ptr[1] = (int *)section->connect->data;

		    elems[0] = end - start + 1;
		    elems[1] = end - section->range[0] + 1;
		    elems[2] = section->range[1] - end;

		    /* set the new values for the range */
		    start = start;
		    end = (end > section->range[1]) ? end : section->range[1];
		}
		else
		{
		    /* old elements are written first */
		    el_conn_ptr[0] = (int *)section->connect->data;
		    el_conn_ptr[1] = (int *)elements;

		    elems[0] = start - section->range[0];
		    elems[1] = 0;
		    elems[2] = end - start + 1;
		...
		/* Loop over the 3 distinct ranges (existing, overlap and
		 * new) of the combined element lists, collecting data sizes
		 * of each.
		 */
		for(i = 0; i < 3; ++i)
		{
		    ...
		    {
			if (cg_npe(type, &npe)) return CG_ERROR;
			elemdatasize[i] = elems[i] * npe;

		    } else {
			int n;
			ElementType_t el_type;
			for (n=0; n < elems[i]; n++) {
			    el_type = (ElementType_t)
				el_conn_ptr[k][elemdatasize[i]];
			    if (cg_npe(el_type, &npe)) return CG_ERROR;
			    if (npe<=0) {
				cgi_error("Error in MIXED elements connectivity vector...\n");
				return CG_ERROR;
			    }
			    elemdatasize[i] += (npe+1);
			}
		    }

		    if(i == 1)/* advance the ptr */
			el_conn_ptr[1] += elemdatasize[i];
		}

		/* create the combined element lists */
		ElementDataSize = elemdatasize[0] + elemdatasize[2];

		if (!(combined_elems = (int *)malloc(ElementDataSize*
						     sizeof(int))))
		{
		    cgi_error("Error allocating combined connectivity data");
		    return CG_ERROR;
		}

		memcpy(combined_elems, el_conn_ptr[0],
		       elemdatasize[0]*sizeof(int));
		memcpy(combined_elems + elemdatasize[0], el_conn_ptr[1],
		       elemdatasize[2]*sizeof(int));

		/* don't want to lose the parent data so save it
		* for writing a new node
		*/
		if(section->parent)
		{
		    parent_save = CGNS_NEW(cgns_array, 1);

		    num = section->parent->dim_vals[0];
		    if (!(parent_save->data = (void *)malloc(num*4*sizeof(int))))
		    {
			cgi_error("Error allocating parent data");
			return CG_ERROR;
		    }
		    memcpy(parent_save->data, section->parent->data, num*4*sizeof(int));

		    strcpy(parent_save->data_type, "I4");
		    strcpy(parent_save->name, "ParentData");
		    parent_save->data_dim =2;
		    parent_save->dim_vals[0] = section->parent->dim_vals[0];
		    parent_save->dim_vals[1] = 4;
		    parent_save->range[0] = section->parent->range[0];
		    parent_save->range[1] = section->parent->range[1];

		    /* initialize other fields */
		    parent_save->id=0;
		    parent_save->link=0;
		    parent_save->ndescr=0;
		    parent_save->data_class=DataClassNull;
		    parent_save->units=0;
		    parent_save->exponents=0;
		    parent_save->convert=0;
		}
    ...
    /* Write element connectivity in internal data structure */
    if(combined_elems == 0) /* true for the first call */
    {
	/* Compute ElementDataSize */
	num = end - start +1;
	if (type!= MIXED) {
	...
	section->connect->data = (void *)combined_elems;
                                 ...
                                 int const *parent_data)
{
    ...
    if (section->parent) {
        if (cg->mode==MODE_WRITE) {
	    int *par_ptr[2];
	    int parlen[3], pardatasize[3], i, int_cnt;

	    parent = section->parent;

	    /* must be able to create a continuous combined parent data array*/
	    if(start > parent->range[1] + 1 || end < parent->range[0] - 1)
	    {
		cgi_error("Invalid parent data range defined.");
		return CG_ERROR;
	    }

	    if(start <= parent->range[0])
	    {
		/* new elements are written first */
		par_ptr[0] = (int *)parent_data;
		par_ptr[1] = (int *)parent->data;

		parlen[0] = end - start + 1;
		parlen[1] = end - parent->range[0] + 1;
		parlen[2] = parent->range[1] - end;

		/* set the new values for the range */
		start = start;
		end = (end > parent->range[1]) ? end : parent->range[1];
	    }
	    else
	    {
		/* old elements are written first */
		par_ptr[0] = (int *)parent->data;
		par_ptr[1] = (int *)parent_data;

		parlen[0] = start - parent->range[0];
		parlen[1] = 0;
		parlen[2] = end - start + 1;
	    ...
	    for(i = 0; i < 3; ++i)
	    {
		pardatasize[i] = parlen[i] * 4;

		if(i == 1)/* advance the ptr */
		    par_ptr[1] += pardatasize[i];
	    }

	    int_cnt = pardatasize[0] + pardatasize[2];

	    combined_par = (int *)malloc(int_cnt*sizeof(int));

	    memcpy(combined_par, par_ptr[0], pardatasize[0]*sizeof(int));
	    memcpy(combined_par + pardatasize[0], par_ptr[1],
		   pardatasize[2]*sizeof(int));
    ...
    } else section->parent = CGNS_NEW(cgns_array, 1);
    ...
    if(combined_par == 0)
    {
	if (!(parent->data = (void *)malloc(num*4*sizeof(int)))) {
	    cgi_error("Error allocating parent->data");
	    return CG_ERROR;
	}
	memcpy(parent->data, parent_data, num*4*sizeof(int));
...
int cg_section_read_ext(int file_number, int B, int Z, int S,
                        ...
                        int *rmin_parent, int *rmax_parent)
{
    ...
    *nbndry = section->el_bound;
    ...
    {
        *parent_flag = 1;
        *rmin_parent = section->parent->range[0];
        *rmax_parent = section->parent->range[1];
    }
    else
    {
	*parent_flag=0;
...
int cg_elements_read_ext(int file_number, int B, int Z, int S,
			 ...
			 int *parent_data, int rmin_parent, int rmax_parent)
{
    ...
    ElementType_t el_type;
    int *el_data_start;
    int int_cnt=0, el_len=0, el_cnt=0, el_st;
    ...
    */
    if(rmin_elems > rmax_elems || rmin_elems < section->range[0] ||
       rmax_elems > section->range[1])
    {
	cgi_error("Error in requested element data range.");
        return CG_ERROR;
    }

    el_st = rmin_elems - section->range[0];

    /* get a count of the int's before the first element requested */
    if(section->el_type != MIXED) {
	if (cg_npe(section->el_type, &npe)) return CG_ERROR;
	int_cnt += npe*el_st;
    }
    ...
    {
	for (n=0; n < el_st; n++) {
	    el_type = (ElementType_t) *((int *)section->connect->data+int_cnt);
	    if (cg_npe(el_type, &npe)) return CG_ERROR;
	    int_cnt += (npe+1);
	}
    }

    /* set the pointer for the beginning of the first element requested */
    el_data_start = (int *)section->connect->data + int_cnt;

    /* get the length of the int's for the elements requested */
    el_cnt = rmax_elems - rmin_elems + 1;
    if(section->el_type != MIXED) {
	if (cg_npe(section->el_type, &npe)) return CG_ERROR;
    ...
    {
	for (n=el_st; n < rmax_elems - section->range[0] + 1; n++) {
	    el_type = (ElementType_t) *((int *)section->connect->data+int_cnt);
	    if (cg_npe(el_type, &npe)) return CG_ERROR;
	    int_cnt += (npe+1);
	    el_len += (npe+1);
    ...
    memcpy(elements, el_data_start, el_len*sizeof(int));

    if (section->parent)
    {
        cgns_array *parent = section->parent;
	int *sec_par_data = (int *)parent->data;

	/* If there wasn't subsetted parent data written or the parent
	 * data range requested is null, return the range of data based
	 * on the provided element data range.
	 */
        if(parent->range[0] == 0 || parent->range[1] == 0 ||
	   rmax_parent == 0 || rmin_parent == 0)
	{
	    sec_par_data += el_st * 4;
            memcpy(parent_data, sec_par_data, el_cnt*4*sizeof(int));
	}
	else if(rmax_parent >= rmin_parent &&
	        rmin_parent >= parent->range[0] &&
                rmax_parent <= parent->range[1])
        {
	    int len;

	    /* move to beginning of parent data to be read */
	    len = rmin_parent - parent->range[0];
	    sec_par_data += len * 4;

	    /* get length of data to extract, and extract it */
	    len = rmax_parent - rmin_parent + 1;
	    memcpy(parent_data, sec_par_data, len*4*sizeof(int));

        }else
        {
            cgi_error("Error in requested parent data range.");
            return CG_ERROR;
        }
    }
    return CG_OK;
...
int cg_bcdataset_write(char *name, BCType_t BCType, BCDataType_t BCDataType)
{
    ...
    {
	cgns_fambc *fambc = (cgns_fambc *)posit;
	...
	dataset = cgi_bcdataset_address(MODE_WRITE, 0, name, &ierr);
...
int cg_bcdataset_info(int *n_dataset)
{
	...
	*n_dataset = fambc->ndataset;
        ...
        cgi_error("FamilyBC_t node not supported under '%s' type node",posit_label);
		      ...
		      int *DirichletFlag, int *NeumannFlag)
{
    ...
    dataset = cgi_bcdataset_address(MODE_READ, index, NULL, &ier);
    if (dataset==0) return CG_ERROR;
