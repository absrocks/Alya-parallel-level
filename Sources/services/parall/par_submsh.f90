subroutine par_submsh()
  ...
  if( INOTMASTER ) then
     ...
     call memory_alloca(mem_servi(1:2,servi),'BPERM','par_submsh',bperm,commd % nneig)
  ...
  if( INOTMASTER ) then
     ...
     if( 1 == 1 .and. kpoin > 0 ) then
        ...
        !call graphs_poipoi(&
        !     kpoin,nelem,mnode,lnods,lnnod,ltype,ia,ja,'SQUARE REMOVE DIAGONAL')
  ...
  if( nboun_total > 0 ) then
     ...
     if( INOTMASTER ) then
        ...
        call memory_deallo(mem_servi(1:2,servi),'LBINV','par_submsh',lbinv_loc)
        call memory_alloca(mem_servi(1:2,servi),'LBINV','par_submsh',lbinv_loc,max(1_ip,nboun))
...
subroutine par_ownbou()
  ...
  if( INOTMASTER ) then
     ...
     !
     allocate( lnown(commd % nneig) , stat = istat )
     do ineig = 1, commd % nneig
