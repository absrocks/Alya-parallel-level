subroutine par_cgschu(nbvar,bi,bb,xi,xb,aii,aib,abi,abb)
  ...
  use def_master, only    :  modul,npoi1,npoi2,npoi3,kfl_paral,gisca
  use def_master, only    :  lninv_loc,parr1,kfl_timin,intost
  use def_master, only    :  IMASTER,INOTMASTER,ISLAVE,INOTSLAVE,NPOIN_TYPE
  use def_master, only    :  nnren_par,iaren_par,jaren_par 
  use def_master, only    :  permr_par,invpr_par 
  use def_solver, only    :  resi1,iters,solve_sol,memit
  use def_solver, only    :  SOL_DIAGONAL,SOL_APPROXIMATE_SCHUR,SOL_ABB
  use def_solver, only    :  SOL_MOD_DIAGONAL
  use def_solver, only    :  nzdom_aii,nzdom_aib,nzdom_abi,nzdom_abb
  ...
  use def_domain, only    :  permr_aii,invpr_aii
  ...
  use mod_csrdir
  use mod_graphs
  use mod_parall, only    :  PAR_COMM_MY_CODE4,PAR_INTEGER,commd
  implicit none
  ...
  !
  ! => CONJUGATE GRADIENT  ----------------------------------------------
  integer(ip)               :: nrows,ierro,maxiter,dummi
  ...
  real(rp)                  :: invb2,rnorm,xnorm,resi2,resfi
  ...
  if( kfl_timin == 1 ) call par_barrie() 
  call cputim(time1)
  ...
  call par_pararr('SUM',0_ip,1_ip,bnorm)
  bnorm = sqrt(bnorm)
  !
  ...
  call par_pararr('SUM',0_ip,1_ip,xnorm)
  ...
  call par_pararr('SUM',0_ip,1_ip,rnorm)
  call par_pararr('SUM',0_ip,1_ip,invb2)
  rnorm = sqrt(rnorm)
  invb2 = sqrt(invb2)
  ...
  call par_pararr('SUM',0_ip,1_ip,newrho)
  resid = sqrt(newrho)     
  resin = resid * invnb
  resi1 = resin
  ...
  do while( iters < maxiter .and. resid > stopcri )
     ...
     call par_pararr('SUM',0_ip,1_ip,alpha)
