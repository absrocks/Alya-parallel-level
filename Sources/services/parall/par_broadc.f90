subroutine par_broadc()
...
#else
  include  'mpif.h'
  integer(4)             :: istat,kfl_desti_par4,npari4
  ...
  if( PART_AND_RUN() ) then
     ...
     if( npari > 0 ) then
        npari4 = int(npari,4)
        call MPI_Bcast( parin(1:npari), npari4, PAR_INTEGER, 0_4, PAR_COMM_MY_CODE4, istat )
        npari = 0
     end if

     if( nparr > 0 ) then
        nparr4 = int(nparr,4)
        call MPI_Bcast( parre(1:nparr), nparr4, MPI_DOUBLE_PRECISION, 0_4, PAR_COMM_MY_CODE4, istat )
        nparr = 0
     end if

     if( nparc > 0 ) then
        nparc4 = int(nparc,4)
        call MPI_Bcast( parch, nparc4, MPI_CHARACTER, 0_4, PAR_COMM_MY_CODE4, istat )
        nparc = 0
     end if

     if( nparx > 0 ) then
        nparx4 = int(nparx,4)
        call MPI_Bcast( parcx(1:nparx), nparx4, MPI_DOUBLE_COMPLEX, 0_4, PAR_COMM_MY_CODE4, istat )
        nparx = 0
     end if

     if( nparl > 0 ) then
        nparl4 = int(nparl,4)
        call MPI_Bcast( parlo(1:nparl), nparl4, MPI_LOGICAL, 0_4, PAR_COMM_MY_CODE4, istat )
        nparl = 0
     end if

#endif

  else if( IMASTER .and. PART_AND_WRITE() ) then

     !-------------------------------------------------------------------
     !
     ! Master parts and writes restart files
     !
     !-------------------------------------------------------------------
