subroutine par_partit()
  ...
  integer(ip)             :: isign,ielty,idime,pnode,npart_sfc
  integer(ip)             :: dummi,ipart,ielem,kelem,inode,ipoin
  integer(ip)             :: nedge_tmp,kzone,izone,nelem_tmp
  integer(ip)             :: kpart,npart_cur,kfl_weigh_metis_par
  ...
  if( kfl_partition_par == PAR_METIS4 ) then
     ...
     ! Oriented bin
     !
     call memory_alloca(mem_servi(1:2,servi),'COORD_ELEM','par_prepro',coord_elem,ndime,nelem)
     ...
     call memory_deallo(mem_servi(1:2,servi),'COORD_ELEM','par_prepro',coord_elem)
     ...
     !call partition_sfc(lepar_par,npart_sfc,lenti_,lweig_,nenti_,ndime,PAR_COMM_MY_CODE_WM4)
