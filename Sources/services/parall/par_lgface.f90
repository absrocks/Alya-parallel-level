subroutine par_lgface(itask)
  ...
  integer(ip)              :: ii,jj,bsize,ji,poin,ini,dom_i
  ...
  if( INOTMASTER ) then
     ...
     case ( 1_ip ) 
        ...
        !
        allocate( commd % bface_perm(commd % bface_dim) , stat = istat )        
        call memchk(zero,istat,mem_servi(1:2,servi),'commd % bface_perm','par_lgface',commd % bface_perm)
        allocate( commd % bface_size(nneig+1)           , stat = istat )        
        call memchk(zero,istat,mem_servi(1:2,servi),'commd % bface_size','par_lgface',commd % bface_size)
        ...
        call memchk(zero,istat,mem_servi(1:2,servi),'LORF1','par_lgface',lorf1)
        allocate( lorf2(commd % bface_dim) , stat = istat )
        call memchk(zero,istat,mem_servi(1:2,servi),'LORF2','par_lgface',lorf2)
        !
        ! List of faces
        !
        kk = 0
        ...
        ! Deallocate memory
        !
        call memchk(two,istat,mem_servi(1:2,servi),'LORF1','par_lgface',lorf1)
        deallocate( lorf1 , stat = istat )
        if(istat/=0) call memerr(two,'LORF1','par_lgface',0_ip)
        call memchk(two,istat,mem_servi(1:2,servi),'LORF2','par_lgface',lorf2)
