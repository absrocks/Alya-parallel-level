subroutine par_gatgro()
  ...
  call memchk(zero,istat,mem_servi(1:2,servi),'ngloc','par_domgro',ngloc)
  allocate(domli(ngrou,npart_par),stat=istat)
  call memchk(zero,istat,mem_servi(1:2,servi),'domli','par_memgro',domli)
  ...
  call par_parari('SUM',0_ip,npart_par,      ngloc)
  call par_parari('SUM',0_ip,npart_par*ngrou,domli)
  !
  ...
  call memchk(two,istat,mem_servi(1:2,servi),'ngloc','par_gatgro',ngloc)
  deallocate(ngloc,stat=istat)
  if(istat/=0) call memerr(two,'ngloc','par_gatgro',0_ip) 

  call memchk(two,istat,mem_servi(1:2,servi),'domli','par_gatgro',domli)
...
#else
  solve_sol(1) % disp4 => solve_sol(1) % displ
  solve_sol(1) % lcou4 => solve_sol(1) % lcoun
#endif
