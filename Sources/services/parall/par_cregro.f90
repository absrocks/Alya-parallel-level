subroutine par_cregro()
  ...
  if( IPARALL ) then
     ...
     !
     if( IMASTER ) then
        call memchk( two, istat, mem_servi(1:2,servi), 'ISKYL','par_sengeo', solve_sol(1)%iskyl)
  ...
  if( IMASTER .and. .not. READ_AND_RUN() ) then
     ...
     call memchk( zero, istat, mem_servi(1:2,servi), 'lgrou_loc', 'par_sengeo', lgrou_loc )
     ...
     ! Deallocate memory
     !
     call memchk( two, istat, mem_servi(1:2,servi), 'LGROU_LOC','par_sengeo', lgrou_loc)
     deallocate( lgrou_loc, stat=istat ) 
     if(istat/=0) call memerr( two, 'LGROU_LOC', 'par_sengeo',0_ip)
     call memchk( two, istat, mem_servi(1:2,servi), 'LGROU','par_sengeo', solve_sol(1)%lgrou)
