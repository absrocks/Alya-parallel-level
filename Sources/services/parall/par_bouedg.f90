subroutine par_bouedg()
...
#endif
  integer(ip)          :: ipoin,ii,jj,bsize,dom_i,iown1,iown2
  integer(4)           :: istat,bsize4,bsizes,bsizer
  integer(ip)          :: iedgg,iedgb,knode,ineig,kk,lsize
  integer(ip)          :: kowne,jpoin,node1,node2
  integer(ip)          :: ll,ipoi1,ipoi2,dom_m,qpoin
  type(i1p),   pointer :: lnown(:)
  integer(ip), pointer :: nneis(:),nneir(:)
  type(i1p),   pointer :: lneis(:),lneir(:),ledgs(:),lowns(:)
  ...
  if( INOTMASTER ) then
     ...
     allocate( ledgs(nneig) , stat = istat )
     ...
     do ineig = 1,nneig
        ...
        allocate( ledgs(ineig) % l(max(1_ip,nneis(ineig)))   , stat = istat )
     ...
     do iedgg = 1,nedgg
        if( ledgg(4,iedgg) > 0 ) then
           ...
           do ineig = 1,nneig
              ...
              if( knode == 2 ) then
                 nneis(ineig) = nneis(ineig) + 1
                 ledgs(ineig) % l(nneis(ineig)) = iedgg
                 if( node1 > node2 ) then
     ...
     !
     allocate( ledgb(4+nneig,nedgb), stat=istat )
     call memchk(zero,istat,mem_servi(1:2,servi),'LEDGB','par_submsh',ledgb) 
     !
     ! 1. LEDGB: Fill in edge table. List of neighbors sharing this edge
     ...
     do ineig = 1,nneig
        ...
        do ii = 1,nneis(ineig)
           ...
           if( knode == 2 ) then
              ...
              iedgb = ledgg(4,iedgg)
              kk    = ledgb(4,iedgb) + 1
              ledgb(4,   iedgb) = kk                     ! A2: # subd. that share this edge
              ledgb(4+kk,iedgb) = ineig                  ! A3: List of subd. that share this edge
              !
     ...
     do iedgg = 1,nedgg
        ...
        if( ledgg(4,iedgg) > 0 ) then
           ...
           iedgb          = ledgg(4,iedgg)
           ledgb(1,iedgb) = iedgg
           ledgb(2,iedgb) = lninv_loc(ipoi1)          ! A1: global edge node
           ledgb(3,iedgb) = lninv_loc(ipoi2)          ! A1: global edge node
           iown1          = 0
           iown2          = 0
           ...
           if( iown1 == 1 .and. iown2 == 1 ) then
              ...
              ll    = 0
              dom_m = kfl_paral
              do kk = 1,ledgb(4,iedgb)
                 ineig = ledgb(4+kk,iedgb)
                 dom_i = commd%neights(ineig)
                 ii    = 0
                 do while( ii < nneis(ineig) )
                    ii = ii + 1
                    if( iedgg == ledgs(ineig) % l(ii) ) then
              ...
              if( iown1 == 1 .or. iown2 == 1 ) then
                 ...
                 if( ll == 0 ) then
                    ...
                    if( lninv_loc(ipoin) <= ledgb(2,iedgb) .and. lninv_loc(ipoin) <= ledgb(3,iedgb) ) then
                       kowne = -1_ip
                    else
                       kowne =  1_ip
                    end if
