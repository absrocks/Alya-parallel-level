...
subroutine solope(&
  ...
  use def_master,         only : IMASTER,zeror,INOTMASTER,npoi1,INOTSLAVE
  ...
  use def_domain,         only : lnoch,meshe,npoin
...
!!$     end if
     ...
     !do ii = 1,nrows
     !   invdiag(ii) = solver_invdiag(ii)        ! OJO
     !end do
     !call diagon(nbnodes,nbvar,solve_sol(1) % kfl_symme,solve_sol(1) % kfl_full_rows,ia,ja,an,invdiag)
        ...
        if( idprecon == SOL_LINELET .or. ( idprecon == SOL_MULTIGRID .and. solve_sol(1) % kfl_defpr == SOL_LINELET ) ) then
          ...
          call direct_solver_factorization(solve_sol(1) % direct_solver_Block_LU)
           ...
           if( solve_sol(1) % kfl_block_ras == 0 ) then
              call direct_allocate_temporary_matrix(solve_sol(1) % direct_solver_RAS(1))
              call matrix_copy_matrix(&
                   solve_sol(1) % direct_solver_RAS(1) % nn , solve_sol(1) % direct_solver_RAS(1) % ndof, &
                   solve_sol(1) % direct_solver_RAS(1) % ia , solve_sol(1) % direct_solver_RAS(1) % ja,   &
                   an                                       , solve_sol(1) % direct_solver_RAS(1) % aa    )
              call PAR_INTERFACE_MATRIX_EXCHANGE(nbvar,solve_sol(1) % direct_solver_RAS(1) % aa,PAR_COMM_MY_CODE_ARRAY(1))
              !
              ! Sparsification: symbolic factorization must be done
              !
              !if( 1 == 1 ) then
              !   dummi = solve_sol(1) % direct_solver_RAS(1) % nz
              !   call direct_solver_cleaning(solve_sol(1) % direct_solver_RAS(1))
              !   call matrix_sparsify(1.0e-3_rp,solve_sol(1) % direct_solver_RAS(1) % nn,nbvar,&
              !        solve_sol(1) % direct_solver_RAS(1) % nz,solve_sol(1) % direct_solver_RAS(1) % ia,&
              !        solve_sol(1) % direct_solver_RAS(1) % ja,solve_sol(1) % direct_solver_RAS(1) % aa,&
              !        memor_opt = solve_sol(1) % direct_solver_RAS(1) % memor )
              !   call direct_solver_initialization(solve_sol(1) % direct_solver_RAS(1))
              !   print*,dummi,solve_sol(1) % direct_solver_RAS(1) % nz,xsymm
              !   !call runend('O.K.!')
              !end if
              call direct_solver_factorization(solve_sol(1) % direct_solver_RAS(1))
           else
              do ii = 1,nbvar
                 call direct_allocate_temporary_matrix(solve_sol(1) % direct_solver_RAS(ii))
                 call matrix_copy_matrix_block(&
                      solve_sol(1) % direct_solver_RAS(ii) % nn , nbvar,                                     &
                      ii                                        , solve_sol(1) % direct_solver_RAS(ii) % ia,     &
                      solve_sol(1) % direct_solver_RAS(ii) % ja , an, solve_sol(1) % direct_solver_RAS(ii) % aa)
                 call PAR_INTERFACE_MATRIX_EXCHANGE(1_ip,solve_sol(1) % direct_solver_RAS(ii) % aa,PAR_COMM_MY_CODE_ARRAY(1))
                 call direct_solver_factorization(solve_sol(1) % direct_solver_RAS(ii))
              end do
           end if
     ...
     if( solve_sol(1) % kfl_clean_precond == 1 ) then
        ...
        if (idprecon == SOL_BIDIAGONAL .and. solve_sol(1) % kfl_renumbered_gs == 1 .and. INOTMASTER ) then
           ...
           call graphs_number_along_vector(&
                ...
                solve_sol(1)%ngrou_gs,'DEALLOCATE')
