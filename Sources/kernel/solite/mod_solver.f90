...
!!$          end do
  ...
  subroutine solver_solve_system_s(solve,A,b,u,M)
    type(soltyp),      intent(in),    pointer  :: solve
    real(rp),          intent(inout)           :: A(*)
    real(rp),          intent(inout)           :: b(*)
    real(rp),          intent(inout)           :: u(*)
  ...
  end subroutine solver_solve_system_s
  ...
  subroutine solver_postprocess(solve,A,b,u)
    ...
    if( INOTMASTER ) then
       ...
       if( num_blocks == 1 ) then
          ...
          ! 2. Rotate first
          !
          call solver_rotate_1by1(solve,ndof1,A,b,u,LOCAL_TO_GLOBAL=.true.)
          !
...
!!$  end subroutine solver_preprocess_dirichlet_2by2
  ...
  subroutine solver_rotate_1by1(solve,ndof1,A11,b1,u1,GLOBAL_TO_LOCAL,LOCAL_TO_GLOBAL)
    ...
    if( solve(1) % kfl_iffix /= 0 .and. associated(solve(1) % kfl_fixrs) ) then
  ...
  subroutine solver_allocate_system_memory(solve,an,bb,MEMORY_COUNTER)
    ...
    if( solve % kfl_format == SOL_CSR_FORMAT ) then
       call memory_alloca(memor_loc,'AN','solver_allocate_system_memory',an,nnz)
    else
       call runend('MOD_SOLVER: ALLOCATION OF A SYSTEM ONLY IN CSR FORMAT')
    end if
    
    if( present(bb) ) then
       call memory_alloca(memor_loc,'AN','solver_allocate_system_memory',bb,nn)
    end if
  ...
  subroutine solver_parallel_SpMV(&
       solve,an,xx,yy,&
       MPI,OPENMP,INITIALIZATION,TIMING,MY_TIMING,ASYNCHRONISM,OPENMP_INTERFACE,BLOCK_MATRIX)
       ...
       if( .not. do_mpi ) then
          ...
          if( solve % kfl_format == SOL_CSR_FORMAT ) then
             ...
             call matrix_CSR_SpMV(&
                  1_ip,nn,ndof,ndof,ndof_an,ndof_an,solve % ia,solve % ja,an,xx,yy,&
          ...
          if( solve % kfl_format == SOL_CSR_FORMAT ) then
             ...
             if( do_asynchronism ) then
                ...
                if( solve % kfl_full_rows == 1 ) then
                   ...
                   call cputim(time2)
                   call matrix_CSR_SpMV(&
                        1_ip,nn,ndof,ndof,ndof_an,ndof_an,solve % ia_full,&
                   ...
                   call cputim(time4)
                   call matrix_CSR_SpMV(&
                        1_ip,nn,ndof,ndof,ndof_an,ndof_an,solve % ia_full_ini,&
                   ...
                   call cputim(time1)
                   call matrix_CSR_SpMV(&
                        npoi1+1_ip,nn,ndof,ndof,ndof_an,ndof_an,solve % ia,solve % ja,an,xx,yy,&
                   ...
                   call cputim(time3)
                   call matrix_CSR_SpMV(&
                        1_ip,npoi1,ndof,ndof,ndof_an,ndof_an,solve % ia,solve % ja,an,xx,yy,&
                        OPENMP=do_openmp,INITIALIZATION=do_init,CHUNK=CHUNK,SCHEDULE=SCHEDULE)
                ...
                if( solve % kfl_full_rows == 1 ) then
                   ...
                   call cputim(time2)
                   call matrix_CSR_SpMV(1_ip,nn,ndof,ndof,ndof_an,ndof_an,solve % ia_full,solve % ja_full,an,xx,yy,&
                        OPENMP=do_openmp,INITIALIZATION=do_init,CHUNK=CHUNK,SCHEDULE=SCHEDULE)
                   ...
                   !
                   call cputim(time1)
                   call matrix_CSR_SpMV(1_ip,nn,ndof,ndof,ndof_an,ndof_an,solve % ia,solve % ja,an,xx,yy,&
                        OPENMP=do_openmp,INITIALIZATION=do_init,CHUNK=CHUNK,SCHEDULE=SCHEDULE)
                   !
...
use def_master, only : kfl_paral
  ...
  subroutine solver_copy_matrix(solve,ain,aout)
