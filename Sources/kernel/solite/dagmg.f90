...
!-----------------------------------------------------------------------
  MODULE dagmg_PARDISSO
    TYPE PARDISO_SEQ_HANDLE
       INTEGER(KIND=8) DUMMY
    END TYPE PARDISO_SEQ_HANDLE
  END MODULE dagmg_PARDISSO
  MODULE dagmg_PARDISO_SEQ
    USE dagmg_PARDISSO
    INTERFACE
       SUBROUTINE PARDISO_SEQ( PT, MAXFCT, MNUM, MTYPE, PHASE, N, A, IA, JA  &
            , PERM, NRHS, IPARM, MSGLVL, B, X, ERROR )
         USE dagmg_PARDISSO
         TYPE(PARDISO_SEQ_HANDLE), INTENT(INOUT) :: PT(*)
       ...
       END SUBROUTINE PARDISO_SEQ
    END INTERFACE
  END MODULE dagmg_PARDISO_SEQ
!-----------------------------------------------------------------------
    SUBROUTINE PARDISO_SEQ( PT, MAXFCT, MNUM, MTYPE, PHASE, N, A, IA, JA    &
         , PERM, NRHS, IPARM, MSGLVL, B, X, ERROR )
      USE dagmg_PARDISSO
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_setind(nc,ndd,ldd,lcg,m,ind)
    INTEGER :: nc,m,lcg(m,*),nll,ldd(ndd),ind(*),i,k,l,ndd
    DO i=1,ndd
       ind(ldd(i))=0
    END DO
    DO i=1,nc
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_DIRseq(n,f,ijob,a,ja,ia)
    ...
    INTEGER, OPTIONAL, TARGET :: ja(*)
    REAL(kind(0.0d0)) , SAVE :: iflop
    TYPE(PARDISO_SEQ_HANDLE), ALLOCATABLE, SAVE :: PT(:)
...
!!!!!!!
    ...
    LOGICAL, PARAMETER :: mklpardiso_seq=.FALSE.
    ...
    IF (ijob == -2) THEN
       !
       IF (mklpardiso_seq) THEN
          PHASE=-1
          CALL PARDISO_SEQ( PT, MAXFCT, MNUM, MTYPE, PHASE, N      &
               , dum, idum, idum                               &
    ...
    !
    RETURN
999 FORMAT('Error detected in PARDISO_SEQ; error code:', i2,' ; Phase:',i2)
