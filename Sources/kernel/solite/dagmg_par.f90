...
!          WRITE(*,*) 'hhh',IRANK                          !hhh
!          flush(6)
!          WRITE(*,*) 'hhh',prtint(1:12)
!          flush(6)
!          WRITE(*,*) 'hhh',dble(nlc(2))/dble(n)
!          flush(6)
!          WRITE(*,*) 'hhh', &
!               dble(nlcp(2))/dble(nlc(2))
!          flush(6)

!          WRITE(*,*) 'hhh',IRANK,prtint(1:12),dble(nlc(2))/dble(n),  &
!               dble(nlcp(2))/dble(nlc(2))
!          flush(6)
!          WRITE(*,933) IRANK,prtint(1:12),dble(nlc(2))/dble(n),  &
!               dble(nlcp(2))/dble(nlc(2))
!          flush(6)
          WRITE(iout,*) IRANK,'*','     Nonzeros in local rows:',prtint(1:12),' (per row:',dble(nlc(2))/dble(n),  &
               '; red. ratio:',dble(nlcp(2))/dble(nlc(2)),')'          
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_setextnum(nc,l,listrank,iw)
    ...
    DO i=1,nneigh
       ...
       kfin=dt(l)%ilstin(i+1)-1
       ii=0
       IF (kfin.GE.kdeb .AND. dt(l+1)%ilstout(i+1).GT.ip) THEN
          DO j=kdeb,kfin
             jj=NINT(buffi(j))
             IF (jj .GT. 0) THEN
                ic=max(ic,jj)
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_globnum(n,ja,ia,iext,irg,jcg,lstout         &
    ...
    !
    ier=0 !meti hhh  --porque sino check se quejaba al pasar algo sin valor asignado
!    write(*,*) 'n,MPI_INTEGER,ICOMM,ier',n,MPI_INTEGER,ICOMM,ier
!    write(*,*) 'nloc(1:n)',nloc(1:n)
...
!
    ...
    IF (wfo) THEN
       ...
       WRITE(iout,'()')
       IF (ITER .NE. 0) THEN
         flop=flop*log(1.0d0/10)/log(RESID+1e-15)
