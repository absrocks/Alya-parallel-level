...
contains 
  ...
  subroutine alya2agmg_solution(solve,amatr,rhsid,unkno)
    ...
    integer(4)   :: ifirstlistrank_aux,niter_aux,iprint,nz_aux,ndtnp_aux,nkryl_aux



    integer(ip)  :: nd_aux,lowerb_lr(1),upperb_lr(1),i
    ...
    if( ISLAVE ) then
       ...
       PAR_COMM_TO_USE = PAR_COMM_MY_CODE_WM4   ! communicator without master
       ...
       neqn = nbrows*nd_aux
       nz_aux  = meshe(ndivi) % r_dom(nbrows+1)-1
    ...
    call parari('MAX',0_ip,1_ip,solve % iters)
  ...
  subroutine calc_lexplode_ndofn(ndofn,npoin_own,nz_own,ia_in,ja_in,ia_ndofn,ja_ndofn, &
    ...
    integer(ip) :: nz,ii_ndofn,iz_ndofn,kk1,kk2,ii,jj,iz,idof,irow,i,j
