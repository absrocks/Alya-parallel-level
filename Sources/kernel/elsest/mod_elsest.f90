...
4   format(i7,50(1x,i7))
  ...
  subroutine elsest_tree_deallocate(imesh)
    ...
    if( tree_struc(imesh) % iallo /= 0 ) then
       ...
       call memory_deallo(memor8,'TREE_STRUC % PELPO'     ,'elsest_tree_deallocate',tree_struc(imesh) % pelpo)
       call memory_deallo(memor8,'TREE_STRUC % LELPO'     ,'elsest_tree_deallocate',tree_struc(imesh) % lelpo)
       call memory_deallo(memor8,'TREE_STRUC % ELEMENT_BB','elsest_tree_deallocate',tree_struc(imesh) % element_bb)
  ...
  subroutine elsest_kdtree_preprocess(&
    ...
    if( ipara(16) == 1 ) then
       call memory_alloca(memor8,'TREE_STRUC % ELEMENT_BB','elsest_bin_preprocess',tree_struc(imesh) % element_bb,ndime,2_ip,nelem)
       call elsest_element_bounding_boxes(&
