...
contains
  ...
  subroutine postpr_mesh_mpio_IP1(mesh,xx,wopos,itste,ttime,TAG1,TAG2)
    ...
    integer(ip)                                  :: PAR_COMM,fh,nsize,ierr
    ...
    if( mesh % comm % rank4 >= 0_4 ) then
       ...
       if(      wopos(3) == 'NELEM' ) then
          nsize  = mesh % nelem
          offset = mesh % comm % offset_nelem
       else if( wopos(3) == 'NPOIN' ) then
          nsize  = mesh % npoin
          offset = mesh % comm % offset_npoin
       end if
  ...
  subroutine postpr_mesh_mpio_IP2(mesh,xx,wopos,itste,ttime,TAG1,TAG2)
    ...
    if( mesh % comm % rank4 >= 0_4 ) then
       ...
       wopos_loc(5)       = 'INT'
       header             =  postpr_mesh_header_mpio(mesh,wopos_loc,1_ip)       
       header % columns   =  size(xx,1)

       call postpr_mesh_permutation(mesh,xx,wopos,yy) 
       if(      wopos(3) == 'NELEM' ) then
          nsize  = mesh % nelem * size(xx,1)
          offset = mesh % comm % offset_nelem * size(xx,1)
       else if( wopos(3) == 'NPOIN' ) then
          nsize  = mesh % npoin * size(xx,1)
          offset = mesh % comm % offset_npoin * size(xx,1)
       end if
  ...
  function postpr_mesh_header_mpio(mesh,wopos,columns) result(header)
    ...
    call PAR_SUM(nlines,WHO='INCLUDE MASTER',PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))

    header % lines = int(nlines,KIND=header_ip)
    if( wopos(5) == 'REAL' ) then
       header % size(1:7) = '8BYTE00'//char(0)
       header % type      = 'REAL000'//char(0)       
    else if( wopos(5) == 'INT' ) then
       header % type='INTEG00'//char(0)
       if( ip == 4 ) then
          header % size(1:7) = '4BYTE00'//char(0)
       else
          header % size(1:7) = '8BYTE00'//char(0)       
       end if
    end if
  ...
  subroutine postpr_mesh_alya_IP1(mesh,xx,wopos,itste,ttime,TAG1,TAG2)
    ...
    if( mesh % comm % RANK4 >= 0 ) then
       ...
       nenti_tot = nenti
       call PAR_SUM(nenti_tot,WHO='INCLUDE MASTER',PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))

       if( mesh % comm % RANK4 == 0 ) then
          
          call iofile_open_unit(nunit,filename,formo='unformatted')       
          call postpr_mesh_header_alya(mesh,wopos_loc,nunit,itste,ttime,nenti_tot,1_ip,TAG1,TAG2)
          call postpr_mesh_permutation(mesh,xx,wopos,yy)           
          ...
          do ipart = 1,int(comm_size4,ip)-1
             call PAR_RECEIVE(pleng,DOM_I=ipart,PAR_COMM_IN4=PAR_COMM4)
             allocate(yy(pleng))
             call PAR_RECEIVE(pleng,yy,DOM_I=ipart,PAR_COMM_IN4=PAR_COMM4)             
             write(nunit) pleng
          ...
          call postpr_mesh_permutation(mesh,xx,wopos,yy) 
          call PAR_SEND(nenti,DOM_I=0_ip,PAR_COMM_IN4=PAR_COMM4)
          call PAR_SEND(nenti,yy,DOM_I=0_ip,PAR_COMM_IN4=PAR_COMM4)
  ...
  subroutine postpr_mesh_alya_IP2(mesh,xx,wopos,itste,ttime,TAG1,TAG2)
    class(mesh_type_basic),           intent(in) :: mesh
    character(5),                     intent(in) :: wopos(*)
    integer(ip),            pointer,  intent(in) :: xx(:,:)
    integer(ip),                      intent(in) :: itste
    ...
    integer(ip),            optional, intent(in) :: TAG1
    integer(ip),            optional, intent(in) :: TAG2
    integer(4)                                   :: comm_size4,my_rank4,PAR_COMM4
    integer(ip)                                  :: pleng,pdime,nenti
    ...
    if( mesh % comm % RANK4 >= 0 ) then
       ...
       pdime = size(xx,1)
       nenti_tot = nenti
       call PAR_MAX(pdime,PAR_COMM_IN4=PAR_COMM4)
       call PAR_SUM(nenti_tot,WHO='INCLUDE MASTER',PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))
       
       if( mesh % comm % RANK4 == 0 ) then

          call iofile_open_unit(nunit,filename,formo='unformatted')       
          call postpr_mesh_header_alya(mesh,wopos_loc,nunit,itste,ttime,nenti_tot,pdime,TAG1,TAG2)
          call postpr_mesh_permutation(mesh,xx,wopos,yy)
          ...
          do ipart = 1,int(comm_size4,ip)-1
             ...
             call PAR_RECEIVE(yy,DOM_I=ipart,PAR_COMM_IN4=PAR_COMM4)
          ...
          call postpr_mesh_permutation(mesh,xx,wopos,yy) 
          call PAR_SEND(nenti,DOM_I=0_ip,PAR_COMM_IN4=PAR_COMM4)
          call PAR_SEND(yy,   DOM_I=0_ip,PAR_COMM_IN4=PAR_COMM4)
  ...
  subroutine postpr_mesh_alya_RP1(mesh,xx,wopos,itste,ttime,TAG1,TAG2)
    class(mesh_type_basic),           intent(in) :: mesh
    character(5),                     intent(in) :: wopos(*)
    real(rp),               pointer,  intent(in) :: xx(:)
    integer(ip),                      intent(in) :: itste
  ...
  subroutine postpr_mesh_alya_RP2(mesh,xx,wopos,itste,ttime,TAG1,TAG2)
    class(mesh_type_basic),           intent(in) :: mesh
    character(5),                     intent(in) :: wopos(*)
    real(rp),               pointer,  intent(in) :: xx(:,:)
    integer(ip),                      intent(in) :: itste
  ...
  function mesh_file_name(mesh,wopos,itste,TAG1,TAG2) result(filename)
    ...
    dir      = trim(mesh % name)//'/'
    prb      = trim(namda)//'-'//trim(mesh % name)
    tag      = '-'//wopos//trim(wtags)//adjustl(trim(tim))//trim(ext)
  ...
  subroutine postpr_mesh_header_alya(mesh,wopos,nunit,itste,ttime,pleng,pdime,TAG1,TAG2)
