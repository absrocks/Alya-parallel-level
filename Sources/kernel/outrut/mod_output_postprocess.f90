...
module mod_output_postprocess
  ...
  public :: output_postprocess_allocate_sets_and_witness      !  3
...
contains
  ...
  subroutine output_postprocess_allocate(imodu)
    ...
    integer(ip)           :: kmodu,ivarp,ivars,ivarw,ivart,ivari
  ...
  subroutine output_postprocess_read()
    ...
    integer(ip)                :: ipost,nvabs,nvaes,nvans,ii,iposi,imesh
    integer(4)                 :: istat
  ...
  subroutine output_postprocess_allocate_sets_and_witness(postp_in)

    type(typos),           intent(inout) :: postp_in

    if(maxval(postp_in % npp_setse)>0) then
       call memory_alloca(mem_modul(1:2,modul),'VESET','mod_output_postprocess',postp_in % veset,postp_in % nvaes+1,neset)
    end if
    if(maxval(postp_in % npp_setsb)>0) then
       call memory_alloca(mem_modul(1:2,modul),'VBSET','mod_output_postprocess',postp_in % vbset,postp_in % nvabs+1,nbset)
    end if
    if(maxval(postp_in % npp_setsn)>0) then
       call memory_alloca(mem_modul(1:2,modul),'VNSET','mod_output_postprocess',postp_in % vnset,postp_in % nvans,nnset)
    end if
    if(maxval(postp_in % npp_witne)>0) then
       call memory_alloca(mem_modul(1:2,modul),'WITNE','mod_output_postprocess',postp_in % witne,postp_in % nvawi,nwitn)
    end if
    if(maxval(postp_in % npp_witng)>0) then
       call memory_alloca(mem_modul(1:2,modul),'WITNG',            'mod_output_postprocess',postp_in % witng,            postp_in % nvawg,nwitg)
       call memory_alloca(mem_modul(1:2,modul),'WITNG_PERIO_COUNT','mod_output_postprocess',postp_in % witng_perio_count,postp_in % nvawg,nwitg)
       call memory_alloca(mem_modul(1:2,modul),'WITNG_DENOM',      'mod_output_postprocess',postp_in % witng_denom,      postp_in % nvawg,nwitg)
       call memory_alloca(mem_modul(1:2,modul),'WITNG_DELDENOM',   'mod_output_postprocess',postp_in % witng_deldenom,   postp_in % nvawg,nwitg)
       call memory_alloca(mem_modul(1:2,modul),'WITNG_DELPERIO',   'mod_output_postprocess',postp_in % witng_delperio,   postp_in % nvawg,nwitg)
    end if
  ...
  logical(lg) function output_postprocess_check_variable_postprocess_now(variable_number,FILTER,FORCE_POSTPROCESS,MESH_ID)
    ...
    integer(ip)                       :: ivari,imodu,ivarp,ivarw,ivart,ivars,itime
    integer(ip)                       :: ipost,nvabs,nvaes,nvans,ivara,imesh
          ...
          if( postp(1) % pos_alrea(ivari,imesh) /= 2 ) then
             ...
             if( ittyp == ITASK_INITIA .and. kfl_rstar < 2 ) then  ! outputs if INITI or no restart
                ...
                if( postp(1) % npp_iniso == 1 .and. postp(1) % npp_stepi(ivari,imesh) > 0 ) then  
                   ivara = ivari
                   if( .not. if_filter ) postp(1) % pos_alrea(ivari,imesh) = 1 ! To avoid repostprocess when no time step is required
                end if
                ...
                if( cutim >= postp(1) % pos_tinit ) then
                   if( .not. if_filter ) postp(1) % pos_alrea(ivari,imesh) = 0
                   !
                   ...
                   if( &
                      ...
                      if( mod(ittim, postp(1) % npp_stepi(ivari,imesh)) == 0 ) then
                         ivara = ivari
                         if( .not. if_filter ) postp(1) % pos_alrea(ivari,imesh) = 1
                      end if
                   end if
                   !
                   ! At a given time
       ...
       if( postp(1) % vox_alrea(ivari,imesh) /= 2 ) then
          if( ittyp == ITASK_INITIA .and. kfl_rstar == 0 ) then
             ...
             if( postp(1) % npp_iniso == 1 .and. postp(1) % vox_stepi(ivari,imesh) > 0 ) then  
                ivara = ivari
                if( .not. if_filter ) postp(1) % vox_alrea(ivari,imesh) = 1 ! To avoid repostprocess when no time step is required
             end if
             ...
             !
             if( cutim >= postp(1) % pos_tinit ) then
                if( .not. if_filter ) postp(1) % vox_alrea(ivari,imesh) = 0
                !
                ! At a given time step
                !           
                if( &
                   ...
                   if( mod(ittim, postp(1) % vox_stepi(ivari,imesh)) == 0 ) then
                      ivara = ivari
                      if( .not. if_filter ) postp(1) % vox_alrea(ivari,imesh) = 1
                   end if
                end if
                !
                ! At a given time
  ...
  logical(lg) function output_postprocess_check_variable_postprocess(ivara,VARIABLE_NAME)
    ...
    if( present(ivara) ) then
       ...
       do while( trim(postp(1) % wopos(1,jvara)) /= trim(VARIABLE_NAME) )
          jvara = jvara + 1
          if( jvara > size(postp(1) % npp_stepi) ) then
