...
module mod_postpr_tools
  ...
  use def_mpio,         only : kfl_mpio_post
  use def_mpio,         only : kfl_mpio_rst 
  use def_mpio,         only : IO_CLASSIC
...
contains
  ...
  subroutine postpr_components(icomp,wopos,ncomp)
    ...
    character(len=*),           intent(inout) :: wopos
  ...
  subroutine postpr_file_name(wfile,wopos,TAG1,TAG2)
    ...
    character(len=*),         intent(in),   optional :: wopos(*)
  ...
  function postpr_is_mpio(wopos,itste,ttime,pdime,pleng_opt,TAG1,TAG2) result(ret)
    ...
    character(*), intent(in), optional  :: wopos(*)
