...
module mod_timings
  ...
  use mod_alya2talp,      only : dlb_module
#endif  
  ...
  subroutine timings_doiter(CURRENT_MODULE)
    ...
    do imodu = imod1,imod2
       if( kfl_modul(imodu) /= 0 ) then
          cpu_solvers  = cpu_modul(CPU_SOLVER,imodu) - cpu_solvers(imodu)
       end if
    end do
    !
...
10  format(1x,i9,1x,i9,1x,50(1x,es13.6))
  ...
  subroutine timings_assembly(time1,time2,CURRENT_MODULE,TYPE_OF_ASSEMBLY)
    ...
    if( if_element ) then
       ...
       cpu_modul(CPU_ASSEMBLY,imodu)       = cpu_modul(CPU_ASSEMBLY,imodu)       + time1    ! sum_k ti
       cpu_element(imodu)                  = cpu_element(imodu)                  + time1
       count_element(imodu)                = count_element(imodu)                + 1.0_rp
       cpu_modul(CPU_MINI_ASSEMBLY,imodu)  = min(cpu_modul(CPU_MINI_ASSEMBLY,imodu),time1)  ! min_k ti
       cpu_modul(CPU_MAXI_ASSEMBLY,imodu)  = max(cpu_modul(CPU_MAXI_ASSEMBLY,imodu),time1)  ! max_k ti
    end if
    !
    ! Boundary assembly
    !
  ...
  subroutine timings_output(&
    ...
    ! Postprocess
    !
    cpu_ave_post = cpu_modul(ITASK_OUTPUT,imodu)
    cpu_max_post = cpu_modul(ITASK_OUTPUT,imodu)
    call PAR_MAX(    cpu_max_post,'IN MY CODE','INCLUDE MASTER')
