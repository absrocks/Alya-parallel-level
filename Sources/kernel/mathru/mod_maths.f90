...
module mod_maths
  ...
  public :: maths_inverse_permutation           ! Inverse of a permutation array
...
contains
  ...
  pure function maths_volume_tetrahedron(x1,x2,x3,x4) result(V)
    ...
    V       = dot_product(v1(1:3),v4(1:3))/6.0_rp
  ...
  !> @details Compute the inverse of a permutation. Can include zeros
  !>          For example:
  !>          INPUT  = [ 4 6 0 0 3 0 ]
  !>          OUTPUT = [ 0 0 5 1 0 2 ] 
  !> 
  ...
  subroutine maths_inverse_permutation(permr,invpr,memor)

    integer(ip), pointer          , intent(inout) :: permr(:)
    integer(ip), pointer, optional, intent(inout) :: invpr(:)
    integer(8),           optional, intent(inout) :: memor(2)
    integer(ip), pointer                          :: invpr_loc(:)
    integer(ip)                                   :: nn,ii,jj
    ...
    nn = memory_size(permr)
    if( nn > 0 ) then
       nullify(invpr_loc)

       if( present(invpr) ) then
          if( .not. associated(invpr) ) &
               call memory_alloca(memor_loc,'INVPR','clusters_elements',invpr,nn)
          invpr_loc => invpr
       else
          call memory_alloca(memor_loc,'INVPR_LOC','clusters_elements',invpr_loc,nn)
       end if
       
       do ii = 1,nn
          jj = permr(ii)
          if( jj /= 0 ) invpr_loc(jj) = ii
       end do
       
       if( .not. present(invpr) ) &
            call memory_copy(memor_loc,'INVPR_LOC','clusters_elements',invpr_loc,permr)
    end if
