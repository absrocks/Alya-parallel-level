  ...
  subroutine clusters(lmask,legro,nclus,meshe,ia,ja,ON_ELEMENTS,ON_NODES,GRAPH_CRITERION)
    ...
    if( present(ia) .and. present(ja) ) then
       ...
       if( wherein == CLUSTERS_ON_ELEMENTS ) then
          ...
          if( INOTEMPTY ) & 
               call graphs_element_element_graph(meshe,'SHARING FACES',trim(w_GRAPH_CRITERION),'INCLUDING DIAGONAL',ia_loc,ja_loc)
       else
          ...
          if( INOTEMPTY ) & 
               call graphs_poipoi(meshe,ia_loc,ja_loc,IA_NAME='IA_LOC',JA_NAME='JA_LOC',INCLUDE_HALOS=.true.)
       end if
    else
