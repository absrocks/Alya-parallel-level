MODULE mod_nrutil
  ! This I downloaded it directly from the internet I assume it is the same as 
  ! what is found on page 1364 of "Numerical Recipes F90"
  ! changed nrtype -> def_kintyp  ; I4B -> ip ; SP & DP & SPC -> rp ; 
  USE def_kintyp
  use def_parame, only     :  pi
  implicit none
  INTEGER(ip), PARAMETER :: NPAR_ARTH=16,NPAR2_ARTH=8
  INTEGER(ip), PARAMETER :: NPAR_GEOP=4,NPAR2_GEOP=2
  INTEGER(ip), PARAMETER :: NPAR_CUMSUM=16
  INTEGER(ip), PARAMETER :: NPAR_CUMPROD=8
  INTEGER(ip), PARAMETER :: NPAR_POLY=8
  INTEGER(ip), PARAMETER :: NPAR_POLYTERM=8
...
!    REAL(rp), DIMENSION(:), INTENT(IN) :: src
!    REAL(rp), DIMENSION(:), INTENT(OUT) :: dest
!    INTEGER(ip), INTENT(OUT) :: n_copied, n_not_copied
...
!  END SUBROUTINE array_copy_r
  ...
  SUBROUTINE array_copy_i(src,dest,n_copied,n_not_copied)
    INTEGER(ip), DIMENSION(:), INTENT(IN) :: src
    INTEGER(ip), DIMENSION(:), INTENT(OUT) :: dest
    INTEGER(ip), INTENT(OUT) :: n_copied, n_not_copied
    n_copied=min(size(src),size(dest))
    n_not_copied=size(src)-n_copied
    dest(1:n_copied)=src(1:n_copied)
  ...
  SUBROUTINE masked_swap_rv(a,b,mask)
    REAL(rp), DIMENSION(:), INTENT(INOUT) :: a,b
    LOGICAL(lg), DIMENSION(:), INTENT(IN) :: mask
  ...
  FUNCTION reallocate_rv(p,n)
    REAL(rp), DIMENSION(:), POINTER :: p, reallocate_rv
    INTEGER(ip), INTENT(IN) :: n
    INTEGER(ip) :: nold,ierr
  ...
  FUNCTION reallocate_iv(p,n)
    INTEGER(ip), DIMENSION(:), POINTER :: p, reallocate_iv
    INTEGER(ip), INTENT(IN) :: n
    INTEGER(ip) :: nold,ierr
  ...
  FUNCTION reallocate_rm(p,n,m)
    REAL(rp), DIMENSION(:,:), POINTER :: p, reallocate_rm
    INTEGER(ip), INTENT(IN) :: n,m
    INTEGER(ip) :: nold,mold,ierr
  ...
  FUNCTION reallocate_im(p,n,m)
    INTEGER(ip), DIMENSION(:,:), POINTER :: p, reallocate_im
    INTEGER(ip), INTENT(IN) :: n,m
    INTEGER(ip) :: nold,mold,ierr
  ...
  FUNCTION ifirstloc(mask)
    LOGICAL(lg), DIMENSION(:), INTENT(IN) :: mask
    INTEGER(ip) :: ifirstloc
    INTEGER(ip), DIMENSION(1) :: loci
    loci=maxloc(merge(1,0,mask))
    ifirstloc=loci(1)
  ...
  FUNCTION imaxloc_r(arr)
    ...
    INTEGER(ip) :: imaxloc_r
    INTEGER(ip), DIMENSION(1) :: imax
    imax=maxloc(arr(:))
    imaxloc_r=imax(1)
  END FUNCTION imaxloc_r
  !BL
  FUNCTION imaxloc_i(iarr)
    INTEGER(ip), DIMENSION(:), INTENT(IN) :: iarr
    INTEGER(ip), DIMENSION(1) :: imax
    INTEGER(ip) :: imaxloc_i
    imax=maxloc(iarr(:))
    imaxloc_i=imax(1)
...
!  END FUNCTION arth_r
  ...
  FUNCTION arth_i(first,increment,n)
    INTEGER(ip), INTENT(IN) :: first,increment,n
    INTEGER(ip), DIMENSION(n) :: arth_i
    INTEGER(ip) :: k,k2,temp
...
!    REAL(rp), INTENT(IN) :: first,factor
!    INTEGER(ip), INTENT(IN) :: n
!    REAL(rp), DIMENSION(n) :: geop_r
!    INTEGER(ip) :: k,k2
!    REAL(rp) :: temp
...
!  END FUNCTION geop_r
  ...
  FUNCTION geop_i(first,factor,n)
    INTEGER(ip), INTENT(IN) :: first,factor,n
    INTEGER(ip), DIMENSION(n) :: geop_i
    INTEGER(ip) :: k,k2,temp
  ...
  FUNCTION geop_c(first,factor,n)
    COMPLEX(rp), INTENT(IN) :: first,factor
    INTEGER(ip), INTENT(IN) :: n
    COMPLEX(rp), DIMENSION(n) :: geop_c
    INTEGER(ip) :: k,k2
    COMPLEX(rp) :: temp
  ...
  FUNCTION geop_dv(first,factor,n)
    REAL(rp), DIMENSION(:), INTENT(IN) :: first,factor
    INTEGER(ip), INTENT(IN) :: n
    REAL(rp), DIMENSION(size(first),n) :: geop_dv
    INTEGER(ip) :: k,k2
    REAL(rp), DIMENSION(size(first)) :: temp
  ...
  RECURSIVE FUNCTION cumsum_r(arr,seed) RESULT(ans)
    ...
    REAL(rp), DIMENSION(size(arr)) :: ans
    INTEGER(ip) :: n,j
    REAL(rp) :: sd
    n=size(arr)
    if (n == 0_ip) RETURN
  ...
  RECURSIVE FUNCTION cumsum_i(arr,seed) RESULT(ans)
    ...
    INTEGER(ip), OPTIONAL, INTENT(IN) :: seed
    INTEGER(ip), DIMENSION(size(arr)) :: ans
    INTEGER(ip) :: n,j,sd
    n=size(arr)
    if (n == 0_ip) RETURN
    sd=0_ip
...
!    REAL(rp), INTENT(IN) :: x
!    REAL(rp), DIMENSION(:), INTENT(IN) :: coeffs
!    REAL(rp) :: poly_rr
!    REAL(rp) :: pow
!    REAL(rp), DIMENSION(:), ALLOCATABLE :: vec
!    INTEGER(ip) :: i,n,nn
!    n=size(coeffs)
!    if (n <= 0) then
!       poly_rr=0.0_rp
...
!  END FUNCTION poly_rr
  ...
  FUNCTION poly_rc(x,coeffs)
    ...
    COMPLEX(rp) :: poly_rc
    COMPLEX(rp) :: pow
    COMPLEX(rp), DIMENSION(:), ALLOCATABLE :: vec
    INTEGER(ip) :: i,n,nn
    n=size(coeffs)
    if (n <= 0) then
       poly_rc=0.0_rp
  ...
  FUNCTION poly_cc(x,coeffs)
    COMPLEX(rp), INTENT(IN) :: x
    COMPLEX(rp), DIMENSION(:), INTENT(IN) :: coeffs
    COMPLEX(rp) :: poly_cc
    COMPLEX(rp) :: pow
    COMPLEX(rp), DIMENSION(:), ALLOCATABLE :: vec
    INTEGER(ip) :: i,n,nn
...
!    REAL(rp), DIMENSION(:), INTENT(IN) :: coeffs,x
!    REAL(rp), DIMENSION(size(x)) :: poly_rrv
!    INTEGER(ip) :: i,n,m
!    m=size(coeffs)
!    n=size(x)
!    if (m <= 0) then
...
!  END FUNCTION poly_rrv
  ...
  FUNCTION poly_ddv(x,coeffs)
    REAL(rp), DIMENSION(:), INTENT(IN) :: coeffs,x
    REAL(rp), DIMENSION(size(x)) :: poly_ddv
    INTEGER(ip) :: i,n,m
    m=size(coeffs)
    n=size(x)
    if (m <= 0) then
...
!    REAL(rp), DIMENSION(:), INTENT(IN) :: coeffs,x
!    LOGICAL(lg), DIMENSION(:), INTENT(IN) :: mask
!    REAL(rp), DIMENSION(size(x)) :: poly_msk_rrv
!    poly_msk_rrv=unpack(poly_rrv(pack(x,mask),coeffs),mask,0.0_rp)
!  END FUNCTION poly_msk_rrv
  !BL
  FUNCTION poly_msk_ddv(x,coeffs,mask)
    REAL(rp), DIMENSION(:), INTENT(IN) :: coeffs,x
    LOGICAL(lg), DIMENSION(:), INTENT(IN) :: mask
    REAL(rp), DIMENSION(size(x)) :: poly_msk_ddv
    poly_msk_ddv=unpack(poly_ddv(pack(x,mask),coeffs),mask,0.0_rp)
  ...
  RECURSIVE FUNCTION poly_term_cc(a,b) RESULT(u)
    ...
    COMPLEX(rp), INTENT(IN) :: b
    COMPLEX(rp), DIMENSION(size(a)) :: u
    INTEGER(ip) :: n,j
    n=size(a)
    if (n <= 0) RETURN
    u(1)=a(1)
  ...
  FUNCTION zroots_unity(n,nn)
    INTEGER(ip), INTENT(IN) :: n,nn
    COMPLEX(rp), DIMENSION(nn) :: zroots_unity
    INTEGER(ip) :: k
    REAL(rp) :: theta
    zroots_unity(1)=1.0
    theta=2.0_rp*pi/n
    ...
    do
       if (k >= nn) exit
       zroots_unity(k+1)=cmplx(cos(k*theta),sin(k*theta),rp)
...
!  END FUNCTION outerprod_r
  ...
  FUNCTION outerprod_d(a,b)
    REAL(rp), DIMENSION(:), INTENT(IN) :: a,b
    REAL(rp), DIMENSION(size(a),size(b)) :: outerprod_d
  ...
  FUNCTION outerdiv(a,b)
    REAL(rp), DIMENSION(:), INTENT(IN) :: a,b
    REAL(rp), DIMENSION(size(a),size(b)) :: outerdiv
  ...
  FUNCTION outersum(a,b)
    REAL(rp), DIMENSION(:), INTENT(IN) :: a,b
    REAL(rp), DIMENSION(size(a),size(b)) :: outersum
...
!  END FUNCTION outerdiff_r
  ...
  FUNCTION outerdiff_d(a,b)
    REAL(rp), DIMENSION(:), INTENT(IN) :: a,b
    REAL(rp), DIMENSION(size(a),size(b)) :: outerdiff_d
    outerdiff_d = spread(a,dim=2,ncopies=size(b)) - &
         spread(b,dim=1,ncopies=size(a))
  END FUNCTION outerdiff_d
  !BL
  FUNCTION outerdiff_i(a,b)
    INTEGER(ip), DIMENSION(:), INTENT(IN) :: a,b
    INTEGER(ip), DIMENSION(size(a),size(b)) :: outerdiff_i
    outerdiff_i = spread(a,dim=2,ncopies=size(b)) - &
         spread(b,dim=1,ncopies=size(a))
  END FUNCTION outerdiff_i
  !BL
  FUNCTION outerand(a,b)
    LOGICAL(lg), DIMENSION(:), INTENT(IN) :: a,b
    LOGICAL(lg), DIMENSION(size(a),size(b)) :: outerand
...
!    REAL(rp), DIMENSION(:), INTENT(OUT) :: dest
!    REAL(rp), DIMENSION(:), INTENT(IN) :: source
!    INTEGER(ip), DIMENSION(:), INTENT(IN) :: dest_index
!    INTEGER(ip) :: m,n,j,i
...
!    REAL(rp), DIMENSION(size(mat,1)) :: get_diag_rv
!    INTEGER(ip) :: j
...
!  END FUNCTION get_diag_rv
  ...
  FUNCTION get_diag_dv(mat)
    REAL(rp), DIMENSION(:,:), INTENT(IN) :: mat
    REAL(rp), DIMENSION(size(mat,1)) :: get_diag_dv
    INTEGER(ip) :: j
  ...
  SUBROUTINE put_diag_rv(diagv,mat)
    REAL(rp), DIMENSION(:), INTENT(IN) :: diagv
    REAL(rp), DIMENSION(:,:), INTENT(INOUT) :: mat
    INTEGER(ip) :: j,n
  ...
  FUNCTION upper_triangle(j,k,extra)
    INTEGER(ip), INTENT(IN) :: j,k
    INTEGER(ip), OPTIONAL, INTENT(IN) :: extra
    LOGICAL(lg), DIMENSION(j,k) :: upper_triangle
    INTEGER(ip) :: n
    n=0
    if (present(extra)) n=extra
    upper_triangle=(outerdiff(arth_i(1_ip,1_ip,j),arth_i(1_ip,1_ip,k)) < n)
  END FUNCTION upper_triangle
  !BL
  FUNCTION lower_triangle(j,k,extra)
    INTEGER(ip), INTENT(IN) :: j,k
    INTEGER(ip), OPTIONAL, INTENT(IN) :: extra
    LOGICAL(lg), DIMENSION(j,k) :: lower_triangle
    INTEGER(ip) :: n
    n=0
    if (present(extra)) n=extra
    lower_triangle=(outerdiff(arth_i(1_ip,1_ip,j),arth_i(1_ip,1_ip,k)) > -n)
