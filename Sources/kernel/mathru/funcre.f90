function funcre(param,npara,ifuge,timev)
  ...
  real(rp)                 :: f,g,H1,T1,H1_prim,T1_prim,timev_prim,H2,T2
  ...
  if( ifuge == 0 ) then
    ...
    !
    f      = (0.5229_rp * sin(1.899_rp *timev))*0.01164_rp
    g      = -(0.3674_rp * sin(1.429_rp *(timev -1.65_rp))+0.0351_rp& 
         *sin(6.223_rp*(timev-1.65_rp))+0.08071_rp*sin(3.572_rp*(timev-1.65_rp)))*0.01164_rp
    !
    ! mixing function for meeting point (1.65s and 4s) T1 for the first and T2 both
    !
    H1     = 0.5_rp*(tanh(slmix1*(timev-whmix1))+1.0_rp)
    T1     = g*H1+(1.0_rp-H1)*f
    !
    ! new timev and dependance
    !
    timev_prim  = timev-4_rp
    H1_prim     = 0.5_rp*(tanh(slmix1*(timev_prim-whmix1))+1.0_rp)
    T1_prim     = g*H1_prim+(1.0_rp-H1)*f
    H2          = 0.5_rp*(tanh(slmix2*(timev-whmix2))+1.0_rp)
    !
    funcre = T1_prim*H2+(1.0_rp-H2)*T1
 end if
