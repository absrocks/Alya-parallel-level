subroutine elmsh2(&
  ...
  if( nddif == 1 ) then
     ...
     if( pgaus == 1 .and. pelty == TRI03 ) then
        ...
        do igaus=1,pgaus
           gpres    =  0.0_rp                                 ! Residual
           F2       = -gpden(igaus) * gpvel(1,igaus) 
           F3       = -gpden(igaus) * gpvel(2,igaus) 
           gpgru(1) =  0.0_rp
           gpgru(2) =  0.0_rp
           do inode = 1,pnode
              gpres   = gpres + elunk(inode)*(     &
     ...
     if( pgaus == 1 .and. pelty == TRI03 ) then
        ...
        if( grnor>umbra ) then
           vepar = gpres/grnor
           CD(1) = factt*shock*chale*vepar-sqrt(gpdif(1,1)**2+gpdif(2,1)**2)
           if( CD(1)>0.0_rp .and. gpve2>umbra ) then
              cdv2  = CD(1)/gpve2
              CF(1) = max(0.0_rp,cdv2-gpden(1)*gpstp(1)*gpden(1)) - cdv2
              CF(1) = facta*CF(1)
        ...
        do igaus=1,pgaus
           ...
           if( grnor > umbra ) then
              vepar     = gpres / grnor
              CD(igaus) = factt * shock * chale * vepar - sqrt(gpdif(1,igaus)**2+gpdif(2,igaus)**2)
              if( CD(igaus) > 0.0_rp .and. gpve2 > umbra ) then
                 cdv2      = CD(igaus) / gpve2
                 CF(igaus) = max(0.0_rp,cdv2-gpden(igaus)*gpstp(igaus)*gpden(igaus)) - cdv2
                 CF(igaus) = facta * CF(igaus)
        ...
        do igaus=1,pgaus
           ...
           if( grnor>umbra ) then
              ...
              CD(igaus) = factt*0.5_rp*shock*chale*vepar&
                   &      -sqrt(gpdif(1,igaus)**2+gpdif(2,igaus)**2+gpdif(3,igaus)**2)
              if( CD(igaus)>0.0_rp .and. gpve2>umbra ) then
                 cdv2      = CD(igaus)/gpve2
                 CF(igaus) = max(0.0_rp,cdv2-gpden(igaus)*gpstp(igaus)*gpden(igaus)) - cdv2
                 CF(igaus) = facta*CF(igaus)
