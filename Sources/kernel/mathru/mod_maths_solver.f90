...
10  if( present(deter) ) deter = det
  ...
  !-----------------------------------------------------------------------
    ...
    subroutine maths_conjugate_gradient(n,A,b,x,TOLERANCE,ITERATIONS,RELATIVE_TOLERANCE,ierr)
    ...
    integer(ip)                         :: nsize,maxiter,iiter,ii,ierr_loc
...
10  continue
  ...
  subroutine maths_eigen_3x3_symmetric_matrix(Ain,eigVal,eigVec,SOLUTION_METHOD,QUAD_REAL)
    ...
    integer(ip)                         :: ii,iimin,nsign,jj,ierr
    character(10)                       :: method
    ...
    if( .not. if_quad_real ) then
       ...
       block
         ...
         real(rp) :: A(3,3),D(3,3),Rpsi1(2,2),Rpsi2(2,2)
         real(rp) :: rr,phi,k,delta
         real(rp) :: bb,ee,vv,ww,alpha,p1,pp,qq
         real(rp) :: R1(3,3),R2(3,3),R3(3,3),phi1,phi2,phi3
         real(rp) :: f1(2),f2(2),g1(2),g2(2),Rg(2)
         real(rp) :: g1x,g1y,g2x,g2y,psi1,psi2,phi11,phi12
         real(rp) :: f1n,f2n,l12,l23,l13,l31,lambda,lambda3
         real(rp) :: diff_min,phi11_min,phi12_min,phi1_min
         real(rp) :: phi2_min,phi3_min,traceA,extraA,strangA
       ...
       block
         ...
         real(qp)            :: eigVal_qp(3),eigVec_qp(3,3)
         real(qp)            :: A_qp(3,3),D_qp(3,3),R_qp(2,2),Ain_qp(3,3)
         real(qp)            :: rr_qp,phi_qp,k_qp,delta_qp
         real(qp)            :: bb_qp,ee_qp,vv_qp,ww_qp,alpha_qp,p1_qp,pp_qp,qq_qp
         real(qp)            :: R1_qp(3,3),R2_qp(3,3),R3_qp(3,3),phi1_qp,phi2_qp,phi3_qp
         ...
         real(qp)            :: diff_min_qp,phi11_min_qp,phi12_min_qp,phi1_min_qp
            ...
            if( p1_qp == 0.0_qp ) then 
               ...
               if( present(eigVec) ) then
                  ...
                  case ( 'cg' ) 
                     ...
                     call runend('MOD_MATHS_SOLVER: CG ALGORITHM NOT AVAILABLE FOR QP')
  ...
  subroutine maths_eigen_symmetric_matrix(n,A,eigVal,eigVec)
    ...
    if( n == 2 ) then
       ...
       call maths_eigen_3x3_symmetric_matrix(A,eigVal,eigVec)
    else
  ...
  subroutine maths_eigen_3x3_symmetric_matrix_vect(itask,A,eigVal,eigVec)
    ...
    real(rp) :: bb,ee,vv,ww,alpha,p1,pp,qq
    real(rp) :: R1(3,3),R2(3,3),R3(3,3),phi1
