...
!     .. Parameters ..
      ...
      real(rp), parameter :: SQRT3 = 1.73205080756887729352744634151D0
      real(rp), parameter :: EPS = 2.2204460492503131D-16 

!     .. Local Variables ..
      real(rp) :: A(4)
      real(rp) :: DELTA
      real(rp) :: DD(3)
      real(rp) :: XL, XH, X, X0(3)
      real(rp) :: DX, DXOLD
      real(rp) :: F, DF
      real(rp) :: ERROR
      real(rp) :: T(3)
      real(rp) :: ALPHA, BETA, GAMMAX
      real(rp) :: P, SQRTP, Q, C, S, PHI
      INTEGER(ip) ::          I, J, NITER
...
!     Calculate roots of f(x) = 0 analytically (analogous to ZHEEVC3)
      ...
      T(2)  = D(1) * D(3)
      T(3)  = D(1) * D(2)
      GAMMAX = T(1) * D(1) + ( Z(1)*T(1) + Z(2)*T(2) + Z(3)*T(3) )
     ...
     &            + ( T(1) + T(2) + T(3) )
      ...
      Q     = ALPHA * (P - (3.0D0/2.0D0) * BETA) + (27.0D0/2.0D0)*GAMMAX
      SQRTP = SQRT(ABS(P))

      PHI   = 27.0D0 * ( 0.25D0 * BETA**2 * (P - BETA)&
     &                       - GAMMAX * (Q - (27.0D0/4.0D0) * GAMMAX) )
