...
!> @date    06/11/2013
!> @author  Herbert Owen
!> @brief   Idem Smoot2 but VECTOR
!> @details Idem Smoot2 but VECTOR
!> @} 
!------------------------------------------------------------------------
subroutine smoot3(velem,vpoin,nsize)
  ...
  real(rp),intent(in)               :: velem(nsize,nelem)
        ...
        if( pelty > 0 ) then
           ...
           gauss_points: do igaus = 1,pgaus
              ...
              do inode = 1,knode
                 ...
                 do idime=1,nsize
                    vpoin(idime,ipoin) = vpoin(idime,ipoin) + xfact * velem(idime,ielem)
                 end do
                 do jnode = 1,pnode
