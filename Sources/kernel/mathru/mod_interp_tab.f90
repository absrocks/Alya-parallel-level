module mod_interp_tab
  ...
  type typ_tab_coord
      ...
      integer(ip)               :: i_alya           ! Index in the structure of alya [C, Cvar/chiSt, Z, Zvar, I]
...
contains
    ...
    subroutine tab_allocate(itask, coords, tab, order)
        ...
        select case(itask)
            ...
            case(2_ip)
                ...
                call memory_alloca(mem_modul(1:2,modul),'TAB'//trim(intost(itab))//' % TAB','tab_allocate', tab % tab, nrow, tab % nvar)
                call memory_alloca(mem_modul(1:2,modul),'TAB'//trim(intost(itab))//' % VARNAME','tab_allocate', tab % varname, tab % nvar)
        end select
    ...
    subroutine fw_allocate(itask, fw, iscal)
        ...
        select case(itask)
            case(1_ip)
                ...
                !
                call memory_alloca(mem_modul(1:2,modul),'FW % KFL_SCALE','fw_allocate', fw % kfl_scale,fw % main_table % ndim)
                        ...
                        if (fw % kfl_scale(ii) == 1) then
                            ...
                            do kk = 1_ip, fw % scaling(ii) % tab % ndim
                                do jj = 1_ip, fw % main_table % ndim
                                    if ( fw % scaling(ii) % tab % coords(kk) % i_alya == fw % main_table  % coords(jj) % i_alya)   fw % scaling(ii) % i_main(kk) = jj
                                enddo
                            enddo
    ...
    subroutine tab_par_exchange(coords, tab)
        ...
        do parii=1,2 
           ...
           call PAR_EXCHANGE(tab % ndim, parin,npari,parii)           
           call PAR_EXCHANGE(tab % nvar, parin,npari,parii)           
           call PAR_EXCHANGE(tab % order, parin,npari,parii)           
           do ii = 1_ip, size(coords, KIND=ip)
               call PAR_EXCHANGE(coords(ii) % leng,   parin,npari,parii)           
               call PAR_EXCHANGE(coords(ii) % name,   parch,nparc,parii)           
               call PAR_EXCHANGE(coords(ii) % i_alya, parin,npari,parii)           
           enddo
        ...
        do parii=1,2 
           ...
           do ii = 1_ip, tab % ndim
              ...
              do jj = 1_ip, coords(ii) % leng
                 call PAR_EXCHANGE(coords(ii) % x(jj), parre,nparr,parii) 
              enddo
           enddo
           ...
           do jj = 1_ip, tab%nvar
              call PAR_EXCHANGE(tab % varname(jj), parch,nparc,parii) 
           enddo
        ...
        do jj = 1_ip, tab%nvar
           do parii=1,2 
              ...
              do ii = 1_ip, kk
                 call PAR_EXCHANGE(tab % tab(ii,jj), parre,nparr,parii) 
              enddo
    ...
    subroutine fw_par_exchange(itask,fw)
        ...
        case(1_ip)
            ...
            do parii=1,2 
               ...
               call PAR_EXCHANGE(fw % kfl_tab_main,    parin,npari,parii) 
               
               if( parii == 1 ) then
                  call memory_alloca(mem_modul(1:2,modul),'PARIN','fw_par_exchange',parin,npari)
                  !call memory_alloca(mem_modul(1:2,modul),'PARRE','fw_par_exchange',parre,nparr)
                  if( ISLAVE  ) call PAR_BROADCAST(parin,      'IN MY CODE')
                  !if( ISLAVE  ) call PAR_BROADCAST(parre,      'IN MY CODE')
                  !if( ISLAVE  ) call PAR_BROADCAST(nparc,parch,'IN MY CODE')
               else
            ...
            enddo
            call memory_deallo(mem_modul(1:2,modul),'PARIN','fw_par_exchange',parin)
            !call memory_deallo(mem_modul(1:2,modul),'PARRE','fw_par_exchange',parre)
        case(2_ip)
            !
            ...
            do parii=1,2 
               ...
               do ii = 1_ip, fw % main_table % ndim
                  call PAR_EXCHANGE(fw % kfl_scale(ii),    parin,npari,parii) 
               enddo
            ...
            do parii=1,2 
               ...
               do ii = 1_ip, fw % main_table % ndim
                  call PAR_EXCHANGE(fw % scaling(ii) % lims(1), parre,nparr,parii) 
                  call PAR_EXCHANGE(fw % scaling(ii) % lims(2), parre,nparr,parii) 
               enddo
               do ii = 1_ip, fw % main_table % ndim
                  call PAR_EXCHANGE(fw % scaling(ii) % kfl_tab, parin,npari,parii) 
               enddo
    ...
    subroutine tab_load_file(coords, tab, write_table, dont_read_data, order)
      ...
      integer(ip)      :: ii, jj, kk, nn, nrow, inds(max_lookup_dim), shap(max_lookup_dim), ioerror
      ...
      call messages_live('shap=                          ',INT_ARRAY  = shap(1:tab % ndim),FMT='(a,300(1x,i5))')
      ...
      if (read_data) then
         do ii = 1, tab % ndim
            !
            ! If there was a dummy coordinate, read it's name:
            !
            if ( coords(ii) % name  == 'DUMMY' ) then  
               coords(ii) % name  = words( ii )
            endif

            !
            ! Check index of coordinate in inner Alya customs:
            ! 1: C
            ! 2: Cvar / chiSt
            ! 3: Z
            ! 4: Zvar
         ...
         do ii = 1, nrow
            ...
            tab % tab(ii, 1:tab %nvar) = real_read(tab % ndim+1 : tab % ndim+tab % nvar) 
         enddo
      ...
      !
      do ii = 1, tab % ndim
         call messages_live('tab % coords('//trim(intost(ii))//'): '//tab % coords(ii) % name//' i_alya= ',INT_NUMBER=tab % coords(ii) % i_alya,FMT='(a,1x,i5)')
      enddo
      if (read_data) then               
      ...
      if (present(write_table)) then
         if (write_table) then
            if (read_data) then               
               do ii = 1, nrow
                  call i2inds(ii, tab % ndim ,shap,inds)
                  do jj = 1, tab%ndim
                     test_output(jj) = tab % coords(jj) % x(inds(jj))
                  enddo
                  write(660+tab%ndim,'(500(2X,E20.8))') test_output(1:tab%ndim),  tab % tab(ii,1:tab%nvar)
               enddo
            endif
         endif
      endif
    ...
    subroutine seek_ind_seq(vec,val,leng,i,w)
        ...
        integer(ip),    intent(inout)  :: i(2)
        real(rp),       intent(out),optional :: w(2)

        integer(ip)                    :: ii, i_clipped
        real(rp)                       :: val_clip
        ...
        if (leng == 1) then
            ...
            i(1)        = 1_ip
            i(2)        = 2_ip
            val_clip    = min(val,vec(leng))   ! upper bound (lower is treated by
                                               ! the initialization of i and w)

            do ii = 1,leng-1
              i_clipped = mod( i(1) -1_ip + ii, leng)
              
              if (vec(i_clipped + 1_ip ) >= val_clip .and. &
                  vec(i_clipped        ) <  val_clip) then
      
                i(1)    = i_clipped
                i(2)    = i_clipped + 1
                if (present(w)) then
                   w(1)    = (vec(i(1) + 1_ip ) - val_clip ) / (vec(i(1) + 1_ip ) - vec(i(1)) )
                   w(2)    = 1.0_rp-w(1)
                endif
                exit
   ...
   subroutine tab_ho_interp(tab,val,res,ind)
      ...
      do jj = 1,nst ! any combination of the individual indeces 
         ...
         ! Get index in table
         !
         call inds2i(itab,tab%ndim,tab%coords(:)%leng,kk)
         hyper(jj,:) = tab%tab(kk,:)
      enddo
    ...
    subroutine tab_interp(tab,val,res,ind,snap_on_nearest,irow)
      ...
      integer(ip)                         :: inear(tab%ndim,2), ii, jj, kk, itab(tab%ndim), &
                                             icube(tab%ndim), nres
      real(rp)                            :: wnear(tab%ndim,2), hyper(2_ip**tab%ndim,tab%nvar),&
      ...
      if ( (tab%order /= 1) .and. (.not. snap)) then !
         ...
         weight      = 1.0_rp
         if (present(ind)) then
             inear(:,1)  = ind(1:tab%ndim)
             inear(:,2)  = ind(1:tab%ndim)+1_ip
         else
             inear(:,1)  = 1_ip 
             inear(:,2)  = 2_ip 
         endif
         ...
         !
         do ii = 1,2_ip**tab%ndim
            !
            ! convert to multidimensional indeces
            ! e.g.: 3D cube -> 8 vertices -> ii=1..8 icube=(1,1,1)...(2,2,2)
            !
            icube(1:tab%ndim) = precomp_i2inds(tab%ndim,ii,1:tab%ndim)

            !
            ! loop through dimensions: construct weights and 
            ! find index along coordinates
            !
            do jj = 1,tab%ndim
               weight(ii) = weight(ii) * wnear(jj,icube(jj))
               itab(jj)   = inear(jj,icube(jj))
            enddo

            !
            ! find index in table 
            !
            call inds2i(itab,tab%ndim,tab%coords(:)%leng,jj)
         ...
         if (snap) then
             ...
             do jj = 1,tab%ndim
                 itab(jj)   = inear(jj,icube(jj))
             enddo
             call inds2i(itab,tab%ndim,tab%coords(:)%leng,irow)
    ...
    !
    subroutine fw_scale_cont_var( control, scale_control, lim_control, fw, xtab )
            ...
            if ( fw % kfl_scale(ii) >= 0_ip .and. fw % kfl_scale(ii) <= 100_ip ) then
                 ...
                 select case( fw % kfl_scale(ii) )
                     ...
                     case (1_ip)
                         ...
                         enddo
                         call tab_interp(fw % scaling(ii) % tab, xtemp(1:fw % scaling(ii) % tab % ndim), res_tab(1:fw % scaling(ii) % tab % nvar,ii))
        ...
        if (present(xtab)) xtab(1:fw % main_table % ndim) = xtab_loc(1:fw % main_table % ndim)

    end subroutine fw_scale_cont_var



    subroutine fw_lookup( control, scale_control, fw, retva )
        real(rp),                         intent(in)  :: control(:)
        real(rp),                         intent(out) :: scale_control(:)
        type(typ_lookup_framework),       intent(in)  :: fw
        ...
        real(rp)    :: xtab(fw % main_table % ndim)

        !
        ! Sale control variables
        !
        call fw_scale_cont_var( control, scale_control, lim_control, fw, xtab )

        !
        ! Interpolate properties from database
        !
        call tab_interp(fw % main_table,scale_control,retva) 
