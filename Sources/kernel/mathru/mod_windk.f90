...
module mod_windk
  ...
  use def_kermod,         only : max_windk_systems
  use def_kermod,         only : number_windk_systems
  use def_kermod,         only : windk_systems
  ...
  contains
   ...
   endsubroutine mod_windk_create_system
...
!     end do
   ...
   endsubroutine mod_windk_systems_par_exchange
   ...
   !>
   !-----------------------------------------------------------------------
   subroutine mod_windk_interact(x_in, y_out, mod_in, tag_in, OPT_mod_out, OPT_tag_out, OPT_dx_in, OPT_ddx_in, OPT_direction)
        implicit none
        ...
        case(1_ip) ! BIDIR. Read and write in the same call, in the same module.
           !if(INOTMASTER) call compute_function(sysid,x_in,y_out)
           !need to have it on master for restart            
           call compute_function(sysid,x_in,y_out)
   ...
   endsubroutine mod_windk_interact
   ...
   !>
   !-----------------------------------------------------------------------
   subroutine compute_function(f_sysid, x, y)
        implicit none
       ...
       findex=sysid2index(f_sysid)
        ...
        if( abs( windk_systems(findex) % stored_time_step ) < epsilon(1.0_rp) ) then
            r1 = windk_systems(findex) % params(1)
       ...
       !
        ...
        !md-TODO
            ...
            r1 = windk_systems(findex) % params(1)
            c1 = windk_systems(findex) % params(2)
        ...
        !md-TODO
            r1 = windk_systems(findex) % params(1)
            c1 = windk_systems(findex) % params(2)
            r2 = windk_systems(findex) % params(3)
        ...
        !md-TODO
            ...
            c1 = windk_systems(findex) % params(2)
            r2 = windk_systems(findex) % params(3)
            l1 = windk_systems(findex) % params(4)
   ...
   endsubroutine compute_function
