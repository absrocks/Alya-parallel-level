...
contains
  ...
  subroutine couplings_search(&
    ...
    integer(4),                intent(in)            :: PAR_COMM_CURRENT4
  ...
  subroutine couplings_communication(&
    ...
    if( INOTMASTER ) then
       ...
       if( itype == ELEMENT_INTERPOLATION ) then
          ...
          call memory_alloca(memor_cou,'COUPLING % GEOME % LELEM'            ,vacal,coupling % geome % lelem_source,coupling % geome % nelem_source)
