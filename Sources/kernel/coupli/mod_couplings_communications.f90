  ...
  subroutine COU_GENERATE_LOCAL_TRANSMISSION_MATRICES(coupling,all_interp_)
    ...
    if(I_AM_TARGET) then
       ...
       do ineig = 1,nneig
          ...
          call memory_alloca(memor_cou,'LTRANSMAT_TARGET',vacal,coupling % ltransmat_target(ineig),1_ip,nentr_ineig)
          ...
          if(coupling % wet % npoin_wet > 0) then 
             ...
             do ineig = 1,nneig
                if(associated(coupling % ltransmat_target(ineig) % iA)) then
                   ...
                   call memory_deallo(memor_cou,'LTRANSMAT_TARGET',vacal,coupling % ltransmat_target(ineig))
                   call memory_alloca(memor_cou,'LTRANSMAT_TARGET',vacal,coupling % ltransmat_target(ineig), 1_ip, int(size(M_aux % iA),ip))
