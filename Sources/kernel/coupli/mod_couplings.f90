...
!!!!  endif
    ...
    if( coupling % scheme == RELAXATION_SCHEME ) then
       ...
       if( (ittim.eq.1).and.(current_in_iter .eq. 1_ip)) then
          ...
          !
          if ( (coupling % history_iqnls.gt.0) .and. (current_in_iter.eq.1) )then
...
!            ENDIF
       ...
       !
        ...
        if(filled_columns_inner_iqnls .ge. coupling % ranku_iqnls) filled_columns_inner_iqnls = coupling % ranku_iqnls

        filled_columns_history_iqnls=ittim-1
        if(filled_columns_history_iqnls.ge.coupling % history_iqnls) filled_columns_history_iqnls = coupling % history_iqnls

        max_columns_increment_iqnls = coupling % ranku_iqnls-1

       filled_columns_increment_iqnls=filled_columns_inner_iqnls-1

       if(coupling % history_iqnls .gt. 0_ip) then
         ...
         if(filled_columns_increment_iqnls .ge. max_columns_increment_iqnls) filled_columns_increment_iqnls = max_columns_increment_iqnls
       endif
...
!write(6,*) 'IM RANK', PAR_MY_CODE_RANK, 'filled_increment', filled_columns_increment_iqnls
       ...
       if(inner_iter_counter .gt. coupling % ranku_iqnls) then
       ...
       do ipoin=1, npoin_wet
          do idofn=1, ndofn
             ...
             !
             if (current_in_iter .eq. 1_ip .and. associated(coupling % values_converged) ) then
...
!ENDIF
  ...
  subroutine couplings_impose_dirichlet(solve,unkno)
   ...
   function  couplings_are_exhaustive() result(istat)
      ...
      integer(ip)                 :: icoup, kpoin, ipoin, ii
      ...
      call memory_alloca(memor_cou,'XSOURCE',vacal,xsource,1_ip,max(1_ip,npoin))
      ...
      do icoup = 1,mcoup
         ...
         if( I_AM_IN_COUPLING(icoup) .and. INOTMASTER ) then
            ...
            call COU_INTERPOLATE_NODAL_VALUES(icoup,1_ip,xsource)
