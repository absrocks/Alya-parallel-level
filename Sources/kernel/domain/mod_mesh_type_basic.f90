...
module mod_mesh_type_basic
  ...
  use def_maths_octree,   only : maths_octree
  ...
  use mod_parall,         only : PAR_ALLOCATE_COMMUNICATION_ARRAY
  ...
  public :: mesh_type_basic_collapse_nodes     ! Collapse nodes with same coordinates
...
contains
  ...
  subroutine mesh_type_basic_copy_boundary_mesh(mesh_bou,mesh,MEMORY_COUNTER)
  ...
  subroutine mesh_type_basic_output(mesh)
    ...
    if( mesh % comm % rank4 >= 0_4 ) then
       ...
       if( nsubd > 0 ) then
          ...
          call PAR_GATHER(mesh % nelem,nelem_gat,PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))
          call PAR_GATHER(mesh % npoin,npoin_gat,PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))
       end if

       if( mesh % comm % rank4 == 0_4 ) then
 
          call iofile_create_directory(mesh % name) 
...
2   format(10(1x,i9))
  ...
  subroutine mesh_type_basic_parallel(mesh,PAR_COMM_INITIAL,PAR_RANK_INITIAL,COMM_INITIAL)
    ...
    class(mesh_type_basic),                      intent(inout) :: mesh
    integer(ip),               optional,         intent(in)    :: PAR_COMM_INITIAL
    ...
    if( PAR_RANK_PARENT >= 0 ) then
       ...
       if( my_rank >= 0 ) then
          ...
          call PAR_ALLGATHER(mesh % npoin,npoin_gat,PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))
       ...
       if( iparent == 1 ) then
          ...
          if( my_rank >= 0 ) then        
             call memory_alloca(memor_dom,'NPOIN_GAT','par_submsh',npoin_gat,comm_size,lboun=0_ip)
             call PAR_ALLGATHER(my_npoin,npoin_gat,PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))
       ...
       if( my_rank >= 0 ) then
          ...
          call PAR_ALLGATHER(mesh % nelem,nelem_gat,PAR_COMM_IN4=int(mesh % comm % PAR_COMM_WORLD,4))
  ...
  subroutine mesh_type_basic_valid_mesh(mesh,mesh_not)
    ...
    integer(ip)                                      :: iclus,kclus,iclus_max
    ...
    if( nn > 0 ) then
       ...
       if( nclus > 1 ) then
          ...
          iclus_max = 0
          do iclus = 1,nclus
             kclus = count(lclus==iclus)
             if( kclus > iclus_max ) iclus_max = iclus
          end do
          allocate(lmask(nn))
          do ii = 1,nn
  ...
  pure function mesh_type_basic_identical_nodes(mesh,ipoin,bin,TOLERANCE) result(res)
    ...
    integer(ip)                                          :: xx(3)
    integer(ip)                                          :: isize,ii,num_nodes,idime
    integer(ip)                                          :: jpoin
    integer(ip),           allocatable                   :: list_tmp(:)
    real(rp)                                             :: xcoor(3),epsil
    ...
    xcoor(1:mesh % ndime) = mesh % coord(1:mesh % ndime,ipoin)
    call bin % find(xcoor,xx)

    if( associated(bin % list(xx(1),xx(2),xx(3)) % l) ) then

       isize = size(bin % list(xx(1),xx(2),xx(3)) % l)
       num_nodes = 0
       allocate(list_tmp(isize))
       do ii = 1,isize
          jpoin = bin % list(xx(1),xx(2),xx(3)) % l(ii)
          ifoun = .true.
          idime_loop: do idime = 1,mesh % ndime
             if( abs(mesh % coord(idime,jpoin)-xcoor(idime)) > epsil ) then
                ifoun = .false.
                exit idime_loop
             end if
          end do idime_loop
  ...
  end function mesh_type_basic_identical_nodes
  ...
  !> @details Collpase nodes of a mesh
  !> 
  !-----------------------------------------------------------------------

  
  subroutine mesh_type_basic_collapse_nodes(mesh)

    type(mesh_type_basic),  intent(inout) :: mesh
    type(maths_bin)                       :: bin
    integer(ip)                           :: ipoin,boxes(3)
    integer(ip),            allocatable   :: list_nodes(:)
    integer(ip)                           :: ii,jpoin,lninv_max
    integer(ip)                           :: imast,npoin_new,kpoin
    integer(ip)                           :: pelty,inode,ielem
    ...
    if( mesh % npoin > 0 ) then
       ...
       boxes = 100    
       call bin % init()
       call bin % fill(mesh % coord,boxes)
       !
       ! Keep nodes with maximum LNINV_LOC
       !
       call memory_alloca(memor_dom,'LMAST','mesh_type_basic_valid_mesh',lmast,mesh % npoin)
       do ipoin = 1,mesh % npoin
          if( lmast(ipoin) == 0 ) then
             list_nodes = mesh_type_basic_identical_nodes(mesh,ipoin,bin)
             if( size(list_nodes) > 1 ) then
                ...
                imast = 0
                ii    = 0
                do while( imast == 0 .and. ii < size(list_nodes) )
                   ii    = ii + 1
                   jpoin = list_nodes(ii)
       ...
       if( npoin_new < npoin_old ) then
          ...
          if( associated(mesh % comm % bound_perm) ) then
             do ii = 1,size(mesh % comm % bound_perm)
          ...
          call memory_copy  (memor_dom,'COORD_TMP'       ,'mesh_type_basic_valid_mesh',mesh % coord    ,coord_tmp)
          call memory_copy  (memor_dom,'PERMN_TMP'       ,'mesh_type_basic_valid_mesh',mesh % permn    ,permn_tmp)
          call memory_copy  (memor_dom,'LNINV_TMP'       ,'mesh_type_basic_valid_mesh',mesh % lninv_loc,lninv_tmp)          
          call memory_alloca(memor_dom,'MESH % COORD'    ,'mesh_type_basic_valid_mesh',mesh % coord    ,mesh % ndime,mesh % npoin)
          call memory_alloca(memor_dom,'MESH % PERMN'    ,'mesh_type_basic_valid_mesh',mesh % permn    ,mesh % npoin)
          call memory_alloca(memor_dom,'MESH % LNINV_LOC','mesh_type_basic_valid_mesh',mesh % lninv_loc,mesh % npoin)
       ...
       call memory_deallo(memor_dom,'LMAST','mesh_type_basic_valid_mesh',lmast)

    end if

  end subroutine mesh_type_basic_collapse_nodes
  ...
  subroutine mesh_type_basic_parallel_interface(mesh)
    ...
    npoin     =  mesh % npoin
    COMM_SIZE =  int(mesh % comm % SIZE4,ip)
    PAR_COMM4 =  int(mesh % comm % PAR_COMM_WORLD,4)
    my_rank   =  kfl_paral
    !
    ! Bounding boxes
    !
    ...
    call PAR_ALLGATHER(comin,par_comin,3_4,PAR_COMM_IN4=PAR_COMM4)
    call PAR_ALLGATHER(comax,par_comax,3_4,PAR_COMM_IN4=PAR_COMM4)
    !
    ...
    !
    do ipart = 1,COMM_SIZE-1
       call PAR_SEND_RECEIVE(lninv_send(ipart) % l,lninv_recv(ipart) % l,DOM_I=ipart,PAR_COMM_IN4=PAR_COMM4)       
    end do
    !
    ...
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(mesh % comm)
    ...
    do ipart = 1,COMM_SIZE-1
       if( ipart /= my_rank ) then
          ...
          if( num_inte(ipart) > 0 ) then
             ...
             call maths_heap_sort(2_ip,kpoin,bound_perm(ipart) % l,PERMUTATION=mesh % lninv_loc)         
          end if
       end if
    end do
    !
    ...
    call PAR_ALLOCATE_COMMUNICATION_ARRAY(mesh % comm,memor_dom)
    kk = 0
  ...
  subroutine mesh_type_basic_send_recv(mesh_send,mesh_recv,ipart,PAR_COMM4)
    ...
    call PAR_SEND_RECEIVE(mesh_send % coord    ,mesh_recv % coord,    'IN MY CODE',ipart,PAR_COMM_IN4=PAR_COMM4)                   
    call PAR_SEND_RECEIVE(mesh_send % lninv_loc,mesh_recv % lninv_loc,'IN MY CODE',ipart,PAR_COMM_IN4=PAR_COMM4)                            
    call PAR_SEND_RECEIVE(mesh_send % lnods    ,mesh_recv % lnods,    'IN MY CODE',ipart,PAR_COMM_IN4=PAR_COMM4)                   
    call PAR_SEND_RECEIVE(mesh_send % ltype    ,mesh_recv % ltype,    'IN MY CODE',ipart,PAR_COMM_IN4=PAR_COMM4)                   
    call PAR_SEND_RECEIVE(mesh_send % leinv_loc,mesh_recv % leinv_loc,'IN MY CODE',ipart,PAR_COMM_IN4=PAR_COMM4)
  ...
  subroutine mesh_type_basic_octree_mesh(mesh_oct,mesh,limit,OFFSET,CENTROID)
    ...
    call octree   % init      ()
    call octree   % fill      (mesh % coord,limit,OFFSET=OFFSET)
  ...
  subroutine mesh_type_basic_bin_mesh(mesh_bin,mesh,boxes,OFFSET,CENTROID)
    ...
    call bin      % init      ()
    call bin      % fill      (mesh % coord,boxes,OFFSET=OFFSET)
  ...
  subroutine mesh_type_basic_octbin_mesh(mesh_octbin,mesh,boxes,limit,OFFSET,CENTROID)
    ...
    call octbin      % init      ()
    call octbin      % fill      (mesh % coord,boxes,limit,OFFSET=OFFSET,ENABLE_GAP=.false.)
