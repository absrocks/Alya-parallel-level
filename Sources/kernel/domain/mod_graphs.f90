...
module mod_graphs
  ...
  use mod_alya_direct_solver, only : alya_Symbolical_CSR_LU_Deallocate
  ...
  interface graphs_list_faces
     module procedure graphs_list_faces_int,&
          &           graphs_list_faces_typ
  end interface graphs_list_faces
...
contains
  ...
  subroutine graphs_compress(nn_in,ia_in,ja_in,lmask,nn_out,ia_out,ja_out,invpr)
    ...
    nullify(permr)
    call memory_alloca(memor_dom,'PERMR','graphs_compress',permr,nn_in)
    ...
    if( present(invpr) ) then
       call memory_alloca(memor_dom,'INVPR','graphs_compress',invpr,nn_out)
       nn_out = 0
       do ii = 1,nn_in
          if( lmask(ii) >= 0 ) then
             nn_out = nn_out + 1
    ...
    call memory_alloca(memor_dom,'IA_OUT','graphs_compress',ia_out,nn_out+1_ip)
    call memory_alloca(memor_dom,'JA_OUT','graphs_compress',ja_out,nz_out)
    ...
    call memory_deallo(memor_dom,'PERMR','graphs_compress',permr)
  ...
  subroutine graphs_poipoi_deallocate(ia,ja)
    ...
    if( present(ia) ) then
       call memory_deallo(memor_dom,'IA','graphs_poipoi_deallocate',ia)
    end if
    if( present(ja) ) then
       call memory_deallo(memor_dom,'JA','graphs_poipoi_deallocate',ja)
    end if
    
  end subroutine graphs_poipoi_deallocate
  
  subroutine graphs_poipoi_0(&
       meshe,ia,ja,message,IA_NAME,JA_NAME,INCLUDE_HALOS)
    ...
    if( .not. associated(meshe % lnnod) ) then
       call memory_alloca(memor_dom,'LNNOD_LOC','graphs_poipoi',lnnod_loc,nelem_loc)
    ...
    call memory_deallo(memor_dom,'LELPO','graphs_poipoi',lelpo)
    call memory_deallo(memor_dom,'PELPO','graphs_poipoi',pelpo)
    
    if( .not. associated(meshe % lnnod) ) then
       call memory_deallo(memor_dom,'LNNOD_LOC','graphs_poipoi',lnnod_loc)
    end if
  ...
  subroutine graphs_poipoi_1(&
    ...
    if( .not. associated(lnnod) ) then
       call memory_alloca(memor_dom,'LNNOD_LOC','graphs_poipoi',lnnod_loc,nelem)
       do ielem = 1,nelem
  ...
  subroutine graphs_poipoi_go(&
    ...
    !
    call memory_alloca(memor_dom,trim(my_ia_name),'graphs_poipoi',ia,npoin+1)
    ...
    if( kfl_lotme == 0 ) then
       ...
       if( which_graph == 0 ) then
          ...
          ! 
          call memory_alloca(memor_dom,trim(my_ja_name),'graphs_poipoi',ja,nz)
          izdom = 1
          do ipoin = 1,npoin
             nlelp = pelp2(ipoin+1) - pelp2(ipoin)
             ncoef = nlelp * mnode
    ...
    if( present(pelpo) .and. present(lelpo) ) then
       ...
       call memory_deallo(memor_dom,'LELP2','graphs_poipoi',lelp2)
       call memory_deallo(memor_dom,'PELP2','graphs_poipoi',pelp2)
    end if
  ...
  subroutine graphs_elepoi_deallocate(pelpo,lelpo,PELPO_NAME,LELPO_NAME)
    ...
    if( present(PELPO) ) then
       if( present(PELPO_NAME) ) then
          call memory_deallo(memor_dom,trim(PELPO_NAME),'graphs_elepoi_deallocate',pelpo)
       else
          call memory_deallo(memor_dom,'PELPO','graphs_elepoi_deallocate',pelpo)
       end if
    end if
    if( present(LELPO) ) then
       if( present(LELPO_NAME) ) then
          call memory_deallo(memor_dom,trim(LELPO_NAME),'graphs_elepoi_deallocate',lelpo)
       else
          call memory_deallo(memor_dom,'LELPO','graphs_elepoi_deallocate',lelpo)
       end if
    end if
    
  end subroutine graphs_elepoi_deallocate
  
  subroutine graphs_elepoi_0(meshe,mepoi,pelpo,lelpo,PELPO_NAME,LELPO_NAME)
    ...
    if( meshe % nelem > 0 ) then
       nullify(lnnod_loc)
       call memory_alloca(memor_dom,'LNNOD_LOC','graphs_poipoi',lnnod_loc,meshe % nelem)
       ...
       call graphs_elepoi_1(&
            ...
            pelpo,lelpo,PELPO_NAME=PELPO_NAME,LELPO_NAME=LELPO_NAME)       
       call memory_deallo(memor_dom,'LNNOD_LOC','graphs_poipoi',lnnod_loc)
    end if
  ...
  subroutine graphs_elepoi_1(&
    ...
    !
    if( present(PELPO_NAME) ) then
       call memory_alloca(memor_dom,trim(PELPO_NAME),'elepoi',pelpo,npoin+1_ip)
    else
       call memory_alloca(memor_dom,'PELPO','elepoi',pelpo,npoin+1_ip)
    end if
    pelpo(1) = 1
    do ipoin = 1,npoin
    ...
    nlelp = pelpo(npoin+1)
    if( present(LELPO_NAME) ) then
       call memory_alloca(memor_dom,trim(LELPO_NAME),'elepoi',lelpo,nlelp)
    else
  ...
  subroutine graphs_eleele_deallocate(pelel,lelel,pelpo,lelpo)
    ...
    if( present(PELEL) ) then
       call memory_deallo(memor_dom,'PELEL','graphs_eleele_deallocate',pelel)
    end if
    if( present(LELEL) ) then
       call memory_deallo(memor_dom,'LELEL','graphs_eleele_deallocate',lelel)
    end if
    if( present(PELPO) ) then
       call memory_deallo(memor_dom,'PELPO','graphs_eleele_deallocate',pelpo)
    end if
    if( present(LELPO) ) then
       call memory_deallo(memor_dom,'LELPO','graphs_eleele_deallocate',lelpo)
    end if
  ...
  subroutine graphs_deallocate_ip14(xarray,ARRAY_NAME)
    implicit none  
    integer(4),  pointer, intent(inout)          :: xarray(:)
    character(len=*),     intent(in),   optional :: ARRAY_NAME
    if( present(ARRAY_NAME) ) then
       call memory_deallo(memor_dom,trim(ARRAY_NAME),'graphs_deallocate',xarray)
    else
       call memory_deallo(memor_dom,'XARRAY','graphs_deallocate',xarray)
    end if
  end subroutine graphs_deallocate_ip14
  subroutine graphs_deallocate_ip18(xarray,ARRAY_NAME)
    implicit none  
  ...
  subroutine graphs_deallocate_rp1(xarray,ARRAY_NAME)
    implicit none  
  ...
  subroutine graphs_dealep(ia,ja,IA_NAME,JA_NAME)
    ...
    if( present(IA_NAME) .and. present(JA_NAME) ) then
       call memory_deallo(memor_dom,trim(IA_NAME),'graphs_dealep',ia)
       call memory_deallo(memor_dom,trim(jA_NAME),'graphs_dealep',ja)
    else
       call memory_deallo(memor_dom,'IA','graphs_dealep',ia)
       call memory_deallo(memor_dom,'JA','graphs_dealep',ja)
    end if
  ...
  subroutine graphs_deagra(ia,ja)
    ...
    call memory_deallo(memor_dom,'IA','graphs_deagra',ia)
    call memory_deallo(memor_dom,'JA','graphs_deagra',ja)
  ...
  subroutine graphs_rengra(nn,nnz,permr,invpr,ia,ja)
    ...
    !
    call memory_alloca(memor_dom,'IACPY_RENGRA','graphs_rengra',iacpy_rengra,nn+1_ip)
    call memory_alloca(memor_dom,'JACPY_RENGRA','graphs_rengra',jacpy_rengra,nnz2)
    !
    ...
    !
    call memory_deallo(memor_dom,'JACPY_RENGRA','graphs_rengra',jacpy_rengra)
    call memory_deallo(memor_dom,'IACPY_RENGRA','graphs_rengra',iacpy_rengra)
  ...
  subroutine graphs_subgra(nn,nnz,permr,invpr,ia,ja)
    ...
    !
    call memory_alloca(memor_dom,'IACPY_SUBGRA','graphs_subgra',iacpy_subgra,nn+1_ip)
    call memory_alloca(memor_dom,'JACPY_SUBGRA','graphs_subgra',jacpy_subgra,nnz)
    !
    ...
    !
    call memory_deallo(memor_dom,'JACPY_SUBGRA','graphs_subgra',jacpy_subgra)
    call memory_deallo(memor_dom,'IACPY_SUBGRA','graphs_subgra',iacpy_subgra)
  ...
  subroutine graphs_permut(nn,nnz,ia,ja,permr,invpr,PERMR_NAME,INVPR_NAME)
    ...
    if( present(PERMR_NAME) ) then
       call memory_alloca(memor_dom,trim(PERMR_NAME),'graphs_permut',permr,nn)
    else
       call memory_alloca(memor_dom,'PERMR','graphs_permut',permr,nn)
    end if
    if( present(INVPR_NAME) ) then
       call memory_alloca(memor_dom,trim(INVPR_NAME),'graphs_permut',invpr,nn)
    else
    ...
    !
    call graphs_offdia(1_ip,nn,nnz,ia,ja,iadiag,jadiag)
    !
  ...
  subroutine graphs_renmat(ndof,nn,invpr,ia,ja,ianew,janew,an)
    ...
    call memory_alloca(memor_dom,'ANCPY','graphs_renmat',ancpy,ndof,ndof,nnz)
    ...
    call memory_deallo(memor_dom,'ANCPY','graphs_renmat',ancpy)
  ...
  subroutine graphs_copyij_deallocate(iacpy,jacpy,memor_opt,IACPY_NAME,JACPY_NAME)
    ...
    if( present(memor_opt) ) then
       ...
       if( present(IACPY_NAME) ) then
          call memory_deallo(memor_dom,trim(IACPY_NAME),'graphs_copyij',iacpy)
       else
          call memory_deallo(memor_dom,'IACPY','graphs_copyij',iacpy)
       end if
       if( present(JACPY_NAME) ) then
          call memory_deallo(memor_dom,trim(JACPY_NAME),'graphs_copyij',jacpy)
       else
          call memory_deallo(memor_dom,'JACPY','graphs_copyij',jacpy)
       end if
    end if
  ...
  subroutine graphs_copyij(nn,ia,ja,iacpy,jacpy,memor_opt,IACPY_NAME,JACPY_NAME)
    ...
    if( present(memor_opt) ) then
       ...
       if( present(IACPY_NAME) ) then
          call memory_alloca(memor_dom,trim(IACPY_NAME),'graphs_copyij',iacpy,nn+1_ip)
       else
          call memory_alloca(memor_dom,'IACPY','graphs_copyij',iacpy,nn+1_ip)
       end if
       if( present(JACPY_NAME) ) then
          call memory_alloca(memor_dom,trim(JACPY_NAME),'graphs_copyij',jacpy,nnz)
       else
          call memory_alloca(memor_dom,'JACPY','graphs_copyij',jacpy,nnz)
       end if
    end if
  ...
  subroutine graphs_iniper(nn,permr,invpr)
    ...
    ! Permutation arrays
    !
    call memory_alloca(memor_dom,'PERMR','graphs_iniper',permr,nn)
    call memory_alloca(memor_dom,'INVPR','graphs_iniper',invpr,nn)
    !
    ! Initialize permutation: useful if Parall is off
    !    
    do ii = 1,nn
  ...
  subroutine graphs_postorder(nn,ia,ja,permr,invpr)
    ...
    !
    nullify(iL,jL,iU,jU)
    call alya_Symbolical_CSR_LU(nn,iA,jA,iL,jL,iU,jU,MEMORY_COUNTER=memor_dom)
    !
    ...
    ! Deallocate memory
    !
    call alya_Symbolical_CSR_LU_Deallocate(iL,jL,iU,jU,MEMORY_COUNTER=memor_dom)
  ...
  subroutine graphs_BuildEliminationTree(nn,iL,jL,tree)
    ...
    call memory_alloca(memor_dom,'TREE % DESCENDIENTE','graphs_BuildEliminationTree',tree % descendiente,nn)
    call memory_alloca(memor_dom,'TREE % NANT'        ,'graphs_BuildEliminationTree',tree % nant        ,nn)
    call memory_alloca(memor_dom,'TREE % TNANT'       ,'graphs_BuildEliminationTree',tree % Tnant       ,nn)
    call memory_alloca(memor_dom,'TREE % ANTECESORES' ,'graphs_BuildEliminationTree',tree % antecesores ,nn)
    !allocate(tree % descendiente(nn))
    !allocate(tree % nant        (nn))
    !allocate(tree % Tnant       (nn))
    !allocate(tree % antecesores (nn))
    !tree % descendiente = 0
    !tree % nant = 0
    !tree % Tnant = 0
    !do ii = 1,nn
    !   nullify(tree % antecesores(ii)%l)
    !end do
    ...
    do ii = 1,nn
       kk = nant(ii)
       if( kk > 0 ) then
          call memory_alloca(memor_dom,'TREE % ANTECESORES % L','graphs_BuildEliminationTree', antecesores(ii) % l,kk)
  ...
  subroutine graphs_generatePostOrder1(tree,node,invpr,last)
    ...
    call memory_alloca(memor_dom,'iwa1','graphs_generatePostOrder1',iwa1,nn)
    call memory_alloca(memor_dom,'mask','graphs_generatePostOrder1',mask,nn)       
    ...
    call memory_deallo(memor_dom,'MASK','graphs_generatePostOrder1',mask)
    call memory_deallo(memor_dom,'IWA1','graphs_generatePostOrder1',iwa1)
  ...
  subroutine graphs_deallocatetree(tree)
    ...
    call memory_deallo(memor_dom,'TREE % DESCENDIENTE','graphs_deallocatetree',tree % descendiente)
    call memory_deallo(memor_dom,'TREE % NANT'        ,'graphs_deallocatetree',tree % nant)
    call memory_deallo(memor_dom,'TREE % TNANT'       ,'graphs_deallocatetree',tree % Tnant)
    call memory_deallo(memor_dom,'TREE % ANTECESORES' ,'graphs_deallocatetree',tree % antecesores)
  ...
  subroutine graphs_groups(npoi1,ngrou,npoin,ia,ja,lgrou,kfl_fixno) 
    ...
    integer(ip), intent(in)                     :: npoin          !< Number of nodes
    integer(ip), intent(in),    pointer         :: ia(:)          !< Graph linked list
    integer(ip), intent(in),    pointer         :: ja(:)          !< Graph linked list
    integer(ip), intent(inout), pointer           :: lgrou(:)       !< List of groups
    integer(ip), intent(inout), pointer, optional :: kfl_fixno(:,:) !< Prescribed nodes
    ...
    !     
    call memory_alloca(memor_dom,'LGROU','graphs_groups',lgrou,npoin)
    !
    ...
    !
    call memory_alloca(memor_dom,'LQUEU','graphs_groups',lqueu,npoin)
    call memory_alloca(memor_dom,'LMARK','graphs_groups',lmark,npoin)
    call memory_alloca(memor_dom,'LFRON','graphs_groups',lfron,npoin)
    do ipoin=1,npoin
       lmark(ipoin)=.false.
    end do
    !
...
999 continue
    ...
    call memory_deallo(memor_dom,'LFRON','graphs_groups',lfron)
    call memory_deallo(memor_dom,'LMARK','graphs_groups',lmark)
    call memory_deallo(memor_dom,'LQUEU','graphs_groups',lqueu)
  ...
  subroutine graphs_element_element_graph(&
    ...
    integer(ip)                                               :: mepoi_2
    integer(ip)                                               :: dummi,medge,nelem_2
    ...
    if( .not. use_r_dom_c_dom ) then
       call memory_deallo(memor_dom,'LELPO_2','graphs_element_element_graph',lelpo_2)
       call memory_deallo(memor_dom,'PELPO_2','graphs_element_element_graph',pelpo_2)
    end if
  ...
  subroutine graphs_eleele_faces_0(&    
       meshe,pelpo,lelpo,pelel,lelel,wdiag)
    ...
    if( present(lelpo) .and. present(pelpo) ) then
       ...
       call graphs_elepoi_0(meshe,mepoi,pelpo_loc,lelpo_loc)
    end if
    
    call graphs_eleele_faces_1(& 
         meshe % nelem,meshe % nelem,meshe % mnode,meshe % lnods,meshe % ltype,&
         pelpo_loc,lelpo_loc,nedge,medge,pelel,lelel,wdiag)
  ...
  subroutine graphs_eleele_faces_1(& 
       ...
       lelpo,nedge,medge,pelel,lelel,wdiag)
    ...
    integer(ip)                                :: pepoi,jelty,mface,pnodb
    integer(ip)                                :: pnodj,pnodi,mnodb
    integer(ip), pointer                       :: faces(:,:,:)
    logical(lg)                                :: equal_faces,include_diag
    ...
    !
    nullify(faces)       
    call memory_alloca(memor_dom,'FACES','graphs_eleele_faces',faces,mnodb+1_ip,mface,nelem_2)
  ...
  subroutine graphs_permut_metis_postordering_deallocate(permr,invpr,memor_opt)
    ...
    if( present(permr) ) then
       if( present(memor_opt) ) then
          call memory_deallo(memor_opt,'PERMR','graphs_permut_metis_postordering',permr)
       else
          call memory_deallo(memor_dom,'PERMR','graphs_permut_metis_postordering',permr)
       end if
    end if
    if( present(invpr) ) then
       if( present(memor_opt) ) then
          call memory_deallo(memor_opt,'INVPR','graphs_permut_metis_postordering',invpr)
       else
          call memory_deallo(memor_dom,'INVPR','graphs_permut_metis_postordering',invpr)
       end if
    end if
    
  end subroutine graphs_permut_metis_postordering_deallocate

  subroutine graphs_permut_metis_postordering(nn,nnz,ia,ja,permr,invpr,PERMR_NAME,INVPR_NAME)
    integer(ip),               intent(in)             :: nn         !< Number of rows
    integer(ip),               intent(in)             :: nnz        !< Size of graph
    ...
    ! Permute using METIS
    !
    call graphs_permut(nn,nnz,ia,ja,permr,invpr)
    !
    ! Postordering 
    !
    ...
    call graphs_iniper(nn,permr_2,invpr_2)
    call graphs_postorder(nn,ia,ja,permr_2,invpr_2)
    call graphs_rengra(nn,nnz,permr_2,invpr_2,ia,ja)
    !
    ! Compose permutation PERMR, INVPR and PERMR_2, INVPR_2
    !
    call graphs_comper(nn,permr,invpr,permr_2,invpr_2)
    call graphs_deaper(permr_2,invpr_2)
  ...
  subroutine graphs_list_faces_int(&
    ...
    do ielem = 1,nelem  
       ...
       if( ichek ) then
          ielty = abs(ltype(ielem))       
          call memory_alloca(memor_dom,'LELFA % L','lgface',lelfa(ielem)%l,nface(ielty))
  ...
  end subroutine graphs_list_faces_int

  subroutine graphs_list_faces_typ(&
       nelem,mnode,mnodb,nelty,mface,lnods,ltype,&
       nnodf,cfael,nface,pelpo,lelpo,nfacg,lface,lelfa,&
       lchek)
    implicit none  
    ...
    integer(ip),          intent(in)           :: lnods(mnode,*)                  !< Connectivity array
    integer(ip),          intent(in)           :: ltype(*)                        !< Array of element types
    type(i1p),            intent(in)           :: nnodf(nelty)                    !< Number of face of element types
    type(i2p),            intent(in)           :: cfael(nelty)                    !< Number of face of element types
    integer(ip),          intent(in)           :: nface(*)                        !< Number of face of element types
    integer(ip), pointer, intent(in)           :: pelpo(:)                        !< Linked list of (element-node) pointer
    integer(ip), pointer, intent(in)           :: lelpo(:)                        !< Linked list of (element-node) elements
    integer(ip),          intent(out)          :: nfacg                           !< Number of faces
    ...
    do ielem = 1,nelem  
       ...
       if( ichek ) then
          ...
          call memory_alloca(memor_dom,'LELFA % L','lgface',lelfa(ielem)%l,nface(ielty))
          do iface = 1,nface(ielty) 
             do inodb = 1,nnodf(ielty) %l(iface)
                inode = cfael(ielty) % l(inodb,iface)
                faces(inodb,iface,ielem) = lnods(inode,ielem)
             end do
             faces(mnodb+1,iface,ielem) = 1
             call sortin(nnodf(ielty) % l(iface),faces(1,iface,ielem))
          end do
       end if
    end do
    !
  ...
  end subroutine graphs_list_faces_typ
  ...
  subroutine graphs_coloring_deallocate(lcolo,ia_color,ja_color)
    ...
    if( present(lcolo) ) then
       call memory_deallo(memor_dom,'LCOLO','graphs_coloring_deallocate',lcolo)
    end if
    if( present(ia_color) ) then
       call memory_deallo(memor_dom,'IA_COLOR','graphs_coloring_deallocate',ia_color)
    end if
    if( present(ja_color) ) then
       call memory_deallo(memor_dom,'jA_COLOR','graphs_coloring_deallocate',ja_color)
    end if
    
  end subroutine graphs_coloring_deallocate


  subroutine graphs_coloring(nvert,ia,ja,lcolo,ncolo,ia_color,ja_color,COLOR_NAME,IA_NAME,JA_NAME)
    ...
    if( .not. associated(lcolo) ) then
       if( present(COLOR_NAME) ) then
          call memory_alloca(memor_dom,trim(COLOR_NAME),'graphs_coloring',lcolo,nvert)
       else
          call memory_alloca(memor_dom,'LCOLO','graphs_coloring',lcolo,nvert)
       end if
    end if
    call memory_alloca(memor_dom,'LSTACK','graphs_coloring',lstack,nvert)
    ...
    if( present(ncolo) .and. present(ia_color) .and. present(ja_color) ) then
       if( present(IA_NAME) ) then
          call memory_alloca(memor_dom,trim(IA_NAME),'graphs_coloring',ia_color,ncolo+1)
       else
          call memory_alloca(memor_dom,'IA_COLOR','graphs_coloring',ia_color,ncolo+1)
       end if
       if( present(JA_NAME) ) then
          call memory_alloca(memor_dom,trim(JA_NAME),'graphs_coloring',ja_color,nvert)
       else
  ...
  !
  !------------------------------------------------------------------------
  subroutine graphs_coloring_greedy(nvert,ia,ja,lcolo,ncolo,ia_color,ja_color,COLOR_NAME,IA_NAME,JA_NAME)
    integer(ip), intent(in)                     :: nvert
    integer(ip), intent(in),  pointer           :: ia(:)
    integer(ip), intent(in),  pointer           :: ja(:)
    integer(ip), intent(inout), pointer           :: lcolo(:)
    ...
    integer(ip),              pointer           :: next(:)
    integer(ip)                                 :: maxIndex, maxVertex, clr, maxClr, k, clrindex, Location, aux 
    integer(ip)                                 :: minColorCnt, ii, jj, kk
    integer(ip)                                 :: maxAdjacent, LocationAux
    ...
    call memory_alloca(memor_dom,'VERTEXORDER','graphs_coloring',vertexOrder,nvert)
    call memory_alloca(memor_dom,'VERTEXRANK','graphs_coloring',vertexRank,nvert)
    ...
    call memory_deallo(memor_dom,'LSTACK','graphs_coloring',vertexRank)
    call memory_alloca(memor_dom,'CLRORDER','graphs_coloring',clrOrder,nvert)
    call memory_alloca(memor_dom,'CLRCNT','graphs_coloring',clrCnt,nvert)
    call memory_alloca(memor_dom,'ADJACENTCOLORS','graphs_coloring',adjacentColors,maxAdjacent)
    ...
    call memory_deallo(memor_dom,'clrOrder','graphs_coloring',clrOrder)
    call memory_deallo(memor_dom,'clrCnt','graphs_coloring',clrCnt)
    call memory_deallo(memor_dom,'vertexOrder','graphs_coloring',vertexOrder)
    call memory_deallo(memor_dom,'adjacentColors','graphs_coloring',adjacentColors)
  ...
  subroutine reorderColorOrder(clrCnt, clrOrder, clr, idx, nvert)
    ...
    integer(ip), intent(in)                     :: nvert
    integer(ip)                                 :: countVer1, countVer2, countVer3, ordAux, idxAux, i
  ...
  !> @details Partition a graph
  !> @author  Guillaume Houzeaux
  !
  !------------------------------------------------------------------------

  subroutine graphs_partitioning(&
       nvert,nedge,npart,ia,ja,lpart,weight_vertices,weight_edges)
    ...
    integer(ip), pointer, intent(in), optional :: weight_vertices(:)
    integer(ip), pointer, intent(in), optional :: weight_edges(:)
    integer(ip)                                :: dummi,kfl_weight
    integer(ip), pointer                       :: weight_vertices_loc(:)
    integer(ip), pointer                       :: weight_edges_loc(:)
    !
    ! Check and nullify
    !
    call runend('NOT CODED')
    if( size(ia,KIND=ip) /= nvert+1 ) call runend('GRAPHS_PARTITIONING: WRONG IA LIST')
    if( size(ja,KIND=ip) /= nedge )   call runend('GRAPHS_PARTITIONING: WRONG JA LIST')
    nullify(weight_vertices_loc)
    nullify(weight_edges_loc)

    kfl_weight = 0

    if( present(weight_vertices) ) then
       weight_vertices_loc => weight_vertices
    else
       call memory_alloca(memor_dom,'WEIGHT_VERTICES_LOC','graphs_partitioning',weight_vertices_loc,nvert)
       weight_vertices = 1_ip
    end if
    if( present(weight_edges) ) then
       weight_edges_loc => weight_edges
    else
       call memory_alloca(memor_dom,'WEIGHT_EDGES_LOC','graphs_partitioning',weight_edges_loc,1_ip)
       weight_edges = 0_ip
    end if

    !call par_metis(                                         &
    !     1_ip , nvert, nedge , ia, ja, weight_vertices_loc, &
    !     weight_edges_loc, kfl_weight, npart,               &
    !     lpart ,dummi , dummi , dummi , dummi ,             & 
    !     dummi, dummi , nvert , 1_ip , memor_dom            )

    if( .not. present(weight_vertices) ) call memory_deallo(memor_dom,'WEIGHT_VERTICES_LOC','graphs_partitioning',weight_vertices_loc)
    if( .not. present(weight_edges) )    call memory_deallo(memor_dom,'WEIGHT_EDGES_LOC'   ,'graphs_partitioning',weight_edges_loc)
    !
  ...
  end subroutine graphs_partitioning
  ...
  subroutine graphs_deallocate_list_faces(lface,lelfa,lchek)
    integer(ip), pointer, intent(inout)          :: lface(:,:)                      !< List of faces
    type(i1p),   pointer, intent(inout)          :: lelfa(:)                        !< List of alement faces
    logical(lg), pointer, intent(in), optional :: lchek(:)                        !< Linked list of (element-node) elements
    integer(ip)                                :: ielem
    ...
    if( present(lchek) ) then
       do ielem = 1,size(lelfa,KIND=ip)
          ichek = lchek(ielem) 
          if( ichek ) then
             call memory_deallo(memor_dom,'LELFA % L','lgface',lelfa(ielem) % l)
          end if
       end do
    else
       do ielem = 1,size(lelfa,KIND=ip)
  ...
  subroutine graphs_number_along_vector(meshe,vecto,angle_stream,permr,invpr,lgrou,kfl_fixno,ngrou_gs,message)
    ...
    integer(ip)                                       :: ipoin,nqueu 
    integer(ip)                                       :: idime,igrou
    integer(ip)                                       :: iqueu,ipinl
    integer(ip)                                       :: izdom,jpoin
    integer(ip)                                       :: kpoin,kqueu
    integer(ip)                                       :: mpoin,ibopo
    integer(ip)                                       :: nfron,ifron 
    integer(ip)                                       :: nneg,npinl
    integer(ip)                                       :: nauxi,kbopo
    integer(ip)                                       :: nmarkt,nmark
    integer(ip)                                       :: counter,ngrou
    ...
    if( present(message) ) then
       if( trim(message) == 'DEALLOCATE' ) then
          call memory_deallo(memor_dom,'PERMR','graphs_numbering_along_vector',permr)
          call memory_deallo(memor_dom,'INVPR','graphs_numbering_along_vector',invpr)
          call memory_deallo(memor_dom,'LGROU','graphs_numbering_along_vector',lgrou)
          return
       end if
    end if
    ...
    !
    if( .not. associated(permr) ) call memory_alloca(memor_dom,'PERMR','graphs_numbering_along_vector',permr,npoin)
    if( .not. associated(invpr) ) call memory_alloca(memor_dom,'INVPR','graphs_numbering_along_vector',invpr,npoin)
    if( .not. associated(lgrou) ) call memory_alloca(memor_dom,'LGROU','graphs_numbering_along_vector',lgrou,npoin)
    permr(1:npoin) = 0
    invpr(1:npoin) = 0
    lgrou(1:npoin) = 0
    !do ipoin = 1,npoin
    ...
    ! Local arrays
    ! 
    call memory_alloca(memor_dom,'LORDE','graphs_numbering_along_vector',lorde,npoin)
    call memory_alloca(memor_dom,'LMARK','graphs_numbering_along_vector',lmark,npoin)
    call memory_alloca(memor_dom,'LINLE','graphs_numbering_along_vector',linle,npoin)
    call memory_alloca(memor_dom,'VMODU','graphs_numbering_along_vector',vmodu,npoin)
    !
...
10  continue
    ...
    call memory_deallo(memor_dom,'LORDE','graphs_numbering_along_vector',lorde)
    call memory_deallo(memor_dom,'LMARK','graphs_numbering_along_vector',lmark)
    call memory_deallo(memor_dom,'LINLE','graphs_numbering_along_vector',linle)
    call memory_deallo(memor_dom,'VMODU','graphs_numbering_along_vector',vmodu)
  ...
  !
  !> @author  Guillaume Houzeaux
  !> @brief   BCSR to CSR format
  !> @details Explode a graph from BCSR to CSR format
  !
  !------------------------------------------------------------------------

  subroutine graphs_bcsr_to_csr(ndof,ia,ja,ia_ndof,ja_ndof)

    integer(ip), intent(in)                     :: ndof
    integer(ip), intent(in),  pointer           :: ia(:)
    integer(ip), intent(in),  pointer           :: ja(:)
    integer(ip), intent(inout), pointer           :: ia_ndof(:)
    integer(ip), intent(inout), pointer           :: ja_ndof(:)
    integer(ip)                                 :: nn,nnz,neqn
    integer(ip)                                 :: ii,JJ,iz,kk1,kk2
    integer(ip)                                 :: ii_ndof,iz_ndof

    if( associated(ia_ndof) ) call runend('graphs_bcsr_to_csr: graph IA_NDOF should not be associated')
    if( associated(ja_ndof) ) call runend('graphs_bcsr_to_csr: graph JA_NDOF should not be associated')
    nullify(ia_ndof)
    nullify(ja_ndof)
    ...
    if( ndof == 1 ) then
       ...
       nn   = size(ia,KIND=ip)-1
       neqn = nn*ndof
       nnz  = ia(nn+1)-1 
       call memory_alloca(memor_dom,'IA_NDOF','graphs_bcsr_to_csr',ia_ndof,neqn+1)
       call memory_alloca(memor_dom,'JA_NDOF','graphs_bcsr_to_csr',ja_ndof,nnz*ndof*ndof)

       !allocate( amatr_ndof(nnz*ndof*ndof) )
  ...
  end subroutine graphs_bcsr_to_csr
  ...
  subroutine graphs_edges(meshe)
    ...
    ! Allocate memory for NEPOI and compute it
    ! 
    call memory_alloca(memor_dom,'NEPOI','graphs_edges',nepoi,npoin)
    mpop2 = 0
    do ielem = 1,nelem
    ...
    ! Allocate memory for PELPO and compute it
    !
    call memory_alloca(memor_dom,'PELPO','graphs_edges',pelpo,npoin+1_ip)
    pelpo(1) = 1
    do ipoin = 1,npoin
       pelpo(ipoin+1) = pelpo(ipoin) + nepoi(ipoin)
    end do
    ...
    !
    nlelp = pelpo(npoin+1)
    call memory_alloca(memor_dom,'LELPO','graphs_edges',lelpo,nlelp)
    do ielem = 1,nelem
       do inode = 1,lnnod(ielem)
          ipoin = lnods(inode,ielem)
          lelpo(pelpo(ipoin)) = ielem
    ...
    ! Allocate memory
    !
    call memory_alloca(memor_dom,'LEDGP','graphs_edges',ledgp,mpop2)
    call memory_alloca(memor_dom,'PEDGP','graphs_edges',pedgp,npoin+1_ip)
    !
    ! Construct the array of indexes
    !     
    pedgp(1) = 1
    ...
    meshe % nedge = pedgp(npoin+1) - 1       
    !
    ! EDGE_TO_NODE(1:2,:) ... Fill in edge table
    !
    call memory_alloca(memor_dom,'MESHE % EDGE_TO_NODE','graphs_edges',meshe % edge_to_node,2_ip,meshe % nedge)
    iedgg = 0
    do ipoin = 1,npoin
       do ilisn = 1,pedgp(ipoin+1)-pedgp(ipoin)
          ...
          jpoin                         = ledgp(iedgg)
          if( ipoin < jpoin ) then
             meshe % edge_to_node(1,iedgg) = ipoin
             meshe % edge_to_node(2,iedgg) = jpoin
          else
             meshe % edge_to_node(1,iedgg) = jpoin
             meshe % edge_to_node(2,iedgg) = ipoin
          end if
       end do
    end do
    !
    ...
    do ielem = 1,nelem
       pelty = ltype(ielem)
       meshe % medge = max(meshe % medge,element_type(pelty) % number_edges)
    end do
    !
    ! LEDGS(1:PEDGE,1:NELEM) ... Fill in element to edge connectivity
    ! LLNED(IELEM) ............. Number of edges
    !
    call memory_alloca(memor_dom,'MESHE % LEDGS','graphs_edges',meshe % ledgs,meshe % medge,nelem)
    call memory_alloca(memor_dom,'MESHE % LNNED','graphs_edges',meshe % lnned,nelem)
    do ielem = 1,nelem
       pelty = ltype(ielem)
       pedge = element_type(pelty) % number_edges
       meshe % lnned(ielem) = pedge
       do iedge = 1,pedge
          ipoin_1 = lnods(element_type(pelty) % list_edges(1,iedge),ielem)
          ipoin_2 = lnods(element_type(pelty) % list_edges(2,iedge),ielem)
    ...
    ! LLNED(IELEM) ............. Number of edges
    !
    call memory_alloca(memor_dom,'MESHE % LEDGB','graphs_edges',meshe % ledgb,meshe % medge,nboun)
    call memory_alloca(memor_dom,'MESHE % LNNEB','graphs_edges',meshe % lnneb,nboun)
    do iboun = 1,nboun
       pelty = meshe % ltypb(iboun)
       pedge = element_type(pelty) % number_edges
       meshe % lnneb(iboun) = pedge
       do iedge = 1,pedge
    ...
    !
    call graphs_poipoi(&
         meshe % nedge,nelem,meshe % medge,meshe % ledgs,meshe % lnned,ltype,&
         meshe % r_edg,meshe % c_edg)
    !
    ! Deallocate memory
    !
    call memory_deallo(memor_dom,'LEDGP','graphs_edges',ledgp)
    call memory_deallo(memor_dom,'PEDGP','graphs_edges',pedgp)
    call memory_deallo(memor_dom,'LELPO','graphs_edges',lelpo)
    call memory_deallo(memor_dom,'PELPO','graphs_edges',pelpo)
    call memory_deallo(memor_dom,'NEPOI','graphs_edges',nepoi)

  end subroutine graphs_edges
  ...
  subroutine graphs_number_to_linked_list(nn,ia)
    integer(ip), intent(in)    :: nn
  ...
  subroutine graphs_csr_to_coo(nn,ndofn,ia,ja,nz,row,column,message)
    integer(ip),  intent(in)              :: nn
    ...
    if( .not. associated(row) ) then
       call memory_alloca(memor_dom,'ROW'   ,'graphs_csr_to_coo',row,nz)
    ...
    if( .not. associated(column) ) then
       call memory_alloca(memor_dom,'COLUMN','graphs_csr_to_coo',column,nz)
  ...
  subroutine graphs_csr_to_ell(nn,ndofn,ia,ja,nz_ell,column,message)
    integer(ip),  intent(in)              :: nn
    integer(ip),  intent(in)              :: ndofn
    integer(ip),  intent(in)              :: ia(nn+1)
    integer(ip),  intent(in)              :: ja(*)
    ...
    !
    if( .not. associated(column) ) then
       call memory_alloca(memor_dom,'COLUMN','graphs_csr_to_ell',column,nz_ell,nn)
    else
  ...
  subroutine graphs_dual_graph_deallocate(iaDual,jaDual,translDual,memor)
    ...
    if( present(iadual) ) then
       call memory_deallo(memor_loc,'IADUAL'    ,'graphs_dual_graph',iadual)
    end if
    if( present(jadual) ) then
       call memory_deallo(memor_loc,'jADUAL'    ,'graphs_dual_graph',jadual)
    end if
    if( present(translDual) ) then
       call memory_deallo(memor_loc,'TRANSLDUAL','graphs_dual_graph',translDual)
    end if
  ...
  subroutine graphs_dual_graph(nn,ia,ja,nnDual,iaDual,jaDual,translDual,memor)
    ...
    !
    call memory_alloca(memor_loc,'translDual','graphs_dual_graph',translDual,ia(nn+1)-1)
    ...
    !
    call memory_alloca(memor_loc,'iwa','graphs_dual_graph',iwa,nnDual)
    ...
    !
    call memory_alloca(memor_loc,'iaDual','graphs_dual_graph',iaDual,nnDual+1_ip)
    ...
    !
    call memory_alloca(memor_loc,'jaDual','graphs_dual_graph',jaDual,iaDual(nnDual+1)-1)
    ...
    call memory_deallo(memor_loc,'iwa'       ,'graphs_dual_graph',iwa)
  ...
  subroutine graphs_color_graph_deallocate(colors,memor)
    ...
    call memory_deallo(memor_loc,'colors','graphs_color_graph_deallocate',colors)
  ...
  subroutine graphs_color_graph(nn,ia,ja,nncolors,colors,memor)
    ...
    integer(8),  intent(out),   optional    :: memor(2)
    integer(ip),                parameter   :: maxColors = 1000_ip
    integer(ip)                             :: colNode, ii, vv, ww, node,nn0,ifact
    integer(8)                              :: memor_loc(2)
    ...
    if( nn == 0 ) then
       ...
       call memory_alloca(memor_loc,'colors'       ,'graphs_color_graph',colors,nn)
       call memory_alloca(memor_loc,'SortVec_node' ,'graphs_color_graph',SortVec_node, nn)
       call memory_alloca(memor_loc,'SortVec_nbAdj','graphs_color_graph',SortVec_nbAdj,nn)

1      continue
       call memory_alloca(memor_loc,'mask'         ,'graphs_color_graph',mask,ifact*maxColors+1_ip,lboun=0_ip)
       ...
       do vv = 2,nn
          ...
          do ii = ia(node),ia(node+1)-1
             ...
             if( colors(ja(ii)) > umask ) then
                ...
                call memory_deallo(memor_loc,'mask'         ,'graphs_color_graph',mask)
                ifact = 2
                goto 1
             end if
       ...
       call memory_deallo(memor_loc,'mask'         ,'graphs_color_graph',mask)
       call memory_deallo(memor_loc,'SortVec_node' ,'graphs_color_graph',SortVec_node)
       call memory_deallo(memor_loc,'SortVec_nbadj','graphs_color_graph',SortVec_nbadj)
