subroutine cderda()
  ...
  do ielty = iesta_dom,iesto_dom
     ...
     if( lexis(ielty) == 1 .and. pface /= 0 ) then
        call memory_alloca(memor_dom,'LTYPF % L','cderda',ltypf(ielty) % l,pface)
        call memory_alloca(memor_dom,'NNODF % L','cderda',nnodf(ielty) % l,pface)
        call memory_alloca(memor_dom,'LFACE % L','cderda',lface(ielty) % l,mnodb,pface)
        call domfac(&
