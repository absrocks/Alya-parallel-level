...
module mod_meshes
  ...
  public :: meshes_check_mesh_connectivity
...
contains 
  ...
  subroutine meshes_list_boundary_nodes(&
    ...
    if( INOTMASTER .and. nelem > 0 ) then
       ...
       call graphs_elepoi(&
            ...
            PELPO_NAME='PELPO_LOC',LELPO_NAME='LELPO_LOC')
  ...
  subroutine meshes_list_boundary_elements(&
    ...
    integer(ip)                                        :: ielty,ielem,iface,inodf,ilist,ifacg
    integer(ip)                                        :: inode,jelem,jface,jelty,ipoin,pnodf
    integer(ip)                                        :: pepoi,ielpo,knode,mface,mnodf,ii
    ...
    integer(ip),      pointer                          :: facel(:,:,:)
    type(i1p),        pointer                          :: lelfa(:)
    character(30)                                      :: my_lelbo_name
    character(30)                                      :: my_lnodb_name
    character(30)                                      :: my_ltypb_name
    ...
    if( present(LELBO_NAME) ) then
       my_lelbo_name = LELBO_NAME
    else
       my_lelbo_name = 'BOUNDARY_ELEMENT'
    end if
    if( present(LNODB_NAME) ) then
       my_lnodb_name = LNODB_NAME
    else
       my_lnodb_name = 'BOUNDARY_CONNECTIVITY'
    end if
    if( present(LTYPB_NAME) ) then
       my_ltypb_name = LTYPB_NAME
    else
       my_ltypb_name = 'BOUNDARY_TYPE'
    end if
    ...
    if( INOTMASTER ) then
       ...
       if( present(ltypb) ) then
          call memory_alloca(memor_loc,trim(my_lnodb_name),'meshes_list_boundary_elements',ltypb,nboun)
       end if
  ...
  subroutine meshes_check_mesh_connectivity(&
    ...
    integer(ip), intent(in)    :: lnodb(:,:)
    integer(ip), intent(in)    :: lnnob(:)
    integer(ip), intent(in)    :: lelbo(:)
    integer(ip), intent(inout) :: ierro_element_wrong_node
    integer(ip), intent(inout) :: ierro_floating_node
    integer(ip), intent(inout) :: ierro_boundary_wrong_node
    integer(ip), intent(inout) :: ierro_local_node
    integer(ip)                :: ielem,ipoin,inode,pelty,pnode
    integer(ip)                :: iboun,inodb,isize
    integer(ip), pointer       :: touch_nodes(:)
    integer(ip)                :: ierro_all(4)
    logical(lg)                :: ifoun
    ...
    do ipoin = 1,npoin
       if( touch_nodes(ipoin) == 0 ) then
          write(*,*) 'NODE '//trim(intost(ipoin))//' IS NOT IN ANY ELEMENT'
          ierro_floating_node = 1
       end if
    end do
  ...
  end subroutine meshes_check_mesh_connectivity
  ...
  subroutine meshes_glue_two_meshes(meshe_i,meshe_o,COLLAPSE,array_i,array_o)
    ...
    integer(ip)                                       :: kpoin,inode,mnode_i,ndime_i
    integer(ip)                                       :: ndima,ndim1,ndim3,ifiel
    logical(lg)                                       :: isin,if_collapse
