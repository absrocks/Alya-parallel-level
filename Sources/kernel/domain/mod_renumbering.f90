...
module mod_renumbering
  ...
  use def_AMR,            only : coupling_AMR_npoin
  use def_AMR,            only : coupling_AMR_npoin_nearest
  use def_AMR,            only : coupling_AMR_nelem
...
contains
  ...
  subroutine renumbering_elements(itask,meshe,permr,list_elements,offset_opt)
    ...
    call memory_alloca(memor_dom,'PERMR','renumber_elements',permr,meshe % nelem)
    !end if
    ...
    if( itask == 0 ) then
       ...
       call graphs_elepoi(meshe % npoin,meshe % nelem,meshe % mnode,meshe % lnods,meshe % lnnod,mepoi,pelpo,lelpo)
...
!!$       end do
  ...
  subroutine renumbering_element_arrays(permr)
    ...
    use def_domain,         only : nnode,ltypb
    ...
    !----------------------------------------------------------------------
     ...
     if( associated(coupling_AMR_nelem % commd % lrecv_perm) ) then
       do ii = 1,size(coupling_AMR_nelem % commd % lrecv_perm)
          ipoin = coupling_AMR_nelem % commd % lrecv_perm(ii)
          if(ipoin==0) print*,'cannot be zero'
          coupling_AMR_nelem % commd % lrecv_perm(ii) = permr(ipoin)
       end do
    end if
  ...
  subroutine renumbering_node_arrays(permr,npoin_opt,nelem_opt,nboun_opt,PERMUTE_LMAST)
    ...
    if( associated(coord) ) then
       ...
       call memory_alloca(memor_dom,'COORD_TMP','par_renumber_nodes',coord_tmp,ndime,npoin_end)
       ...
       call memory_deallo(memor_dom,'COORD_TMP','par_renumber_nodes',coord_tmp)
    ...
    if( associated(lnset) ) then
       ...
       call memory_alloca(memor_dom,'LLLLL_TMP','par_renumber_nodes',lllll_tmp,npoin_min)
       lllll_tmp(1:npoin_min) = lnset(1:npoin_min)
       do ipoin = 1,npoin_min
          jpoin = permr(ipoin)
          lnset(jpoin) = lllll_tmp(ipoin)
       end do
       call memory_deallo(memor_dom,'LLLLL_TMP','par_renumber_nodes',lllll_tmp)
    ...
    if( associated(lnoch) ) then
       ...
       call memory_alloca(memor_dom,'LNOCH_TMP','par_renumber_nodes',lnoch_tmp,npoin_min)
       lnoch_tmp(1:npoin_min) = lnoch(1:npoin_min)
       do ipoin = 1,npoin_min
          jpoin = permr(ipoin)
          lnoch(jpoin) = lnoch_tmp(ipoin)
       end do
       call memory_deallo(memor_dom,'LNOCH_TMP','par_renumber_nodes',lnoch_tmp)
    ...
    if( nfiel > 0 ) then
       ...
       call memory_alloca(memor_dom,'XFIEL_TMP','par_renumber_nodes',xfiel_tmp,nfiel)
       do ifiel = 1,nfiel
          if( kfl_field(1,ifiel) > 0 ) then
             ...
             if( kfl_field(2,ifiel) == NPOIN_TYPE ) then
                ...
                call memory_alloca(memor_dom,'XFIEL_TMP % A','par_renumber_nodes',xfiel_tmp(ifiel)%a,kfl_field(1,ifiel),npoin_min,kfl_field(4,ifiel))
                do istep = 1,nsteps
                   do ipoin = 1,npoin_min
       ...
       call memory_deallo(memor_dom,'XFIEL_TMP','par_renumber_nodes',xfiel_tmp)
    ...
    if( ngrou_dom > 0 .and. associated(lgrou_dom) ) then
       ...
       call memory_alloca(memor_dom,'LGROU_DOM_TMP','par_renumber_nodes',lgrou_dom_tmp,npoin_min)       
       do ipoin = 1,npoin_min
       ...
       call memory_deallo(memor_dom,'LGROU_DOM_TMP','par_renumber_nodes',lgrou_dom_tmp)
    ...
    if( kfl_icodn > 0 .and. associated(kfl_codno) ) then
       ...
       call memory_alloca(memor_dom,'KFL_CODNO_TMP','par_renumber_nodes',kfl_codno_tmp,mcono,npoin_min)
       do ipoin = 1,npoin_min
          do icono = 1,mcono
       ...
       call memory_deallo(memor_dom,'KFL_CODNO_TMP','par_renumber_nodes',kfl_codno_tmp)
    ...
    if( associated(commd) .and. ISLAVE ) then
       if( associated(commd % bound_perm) ) then
          ...
          call memory_alloca(memor_dom,'BOUND_PERM','par_renumber_nodes',bound_perm,commd % bound_dim)
          ...
          call memory_deallo(memor_dom,'BOUND_PERM','par_renumber_nodes',bound_perm)

       end if
       if( associated(commd % bound_invp) ) then

          call memory_alloca(memor_dom,'BOUND_INVP','par_renumber_nodes',bound_invp,commd % bound_dim)
          do ii = 1,commd % bound_dim         
          ...
          call memory_deallo(memor_dom,'BOUND_INVP','par_renumber_nodes',bound_invp)
    ...
    if( associated(coupling_AMR_npoin % commd % lrecv_perm) ) then
       do ii = 1,size(coupling_AMR_npoin % commd % lrecv_perm)
          ipoin = coupling_AMR_npoin % commd % lrecv_perm(ii)
          coupling_AMR_npoin % commd % lrecv_perm(ii) = permr(ipoin)
       end do
    end if

    if( associated(coupling_AMR_npoin_nearest % commd % lrecv_perm) ) then
       do ii = 1,size(coupling_AMR_npoin_nearest % commd % lrecv_perm)
          ipoin = coupling_AMR_npoin_nearest % commd % lrecv_perm(ii)
          coupling_AMR_npoin_nearest % commd % lrecv_perm(ii) = permr(ipoin)
       end do
    end if
    ...
    if( associated(lninv_loc) ) then
       ...
       call memory_alloca(memor_dom,'LNINV_LOC_TMP','par_renumber_nodes',lninv_loc_tmp,npoin_min)
       ...
       call memory_deallo(memor_dom,'LNINV_LOC_TMP','par_renumber_nodes',lninv_loc_tmp)
  ...
  subroutine renumbering_nodes(what,kpoin,ia,ja,permr)
    ...
    use def_master, only : iaren_par
    use def_master, only : jaren_par
    use def_master, only : nnren_par
    use def_master, only : permr_par
    use def_master, only : invpr_par
...
!!!!!!!!!!!!!!!! todas las lineas de abaj son al PEDO!!!!!  -- las borro
  ...
  subroutine renumbering_temporal_locality(meshe)
    ...
    integer(ip)                         :: jnode,jelem,izdom
    integer(ip)                         :: jpoin,jelpo,i1,i2
  ...
  subroutine renumbering_sfc_recursive(nsfc,coord,perm,NUMBER_OF_NODES)
    ...
    integer(ip)                         :: i,ielem,counter
    integer(ip)                         :: divmax,limit,nn,npoin
    integer(ip)                         :: ndime,kpoin,d,jpoin
    integer(ip)                         :: ipoin,ii,jj,kk,ll,idime,nd
    integer(ip)                         :: boxes(3),dfather
    integer(2)                          :: orientation
    integer(ip), pointer                :: i2dto1d(:,:,:)

    real(rp)                            :: toler,xsize
    real(rp)                            :: time1,time2,time4,time5      
    real(rp)                            :: comin(3),x,y,z
    real(rp)                            :: comax(3)
    ...
    do while( conti )
       ...
       if( current_o % npoinbox > limit ) then
          ...
          if( current_o % childid < divmax .and. current_o % id /= 0 ) then
             ...
             ! Wrong child ID
             !
             call elsest_runend('WRONG CHILD ID: '//trim(intost(current_o % childid)))    
          end if
    ...
    do ifiel = 1,nfiel
       if( kfl_field(2,ifiel) == NBOUN_TYPE ) then
          ...
          do istep = 1,nsteps
             ...
             do iboun_old = 1,nboun
                iboun_new        = permr(iboun_old)
                if( iboun_new > 0 ) xfiel(ifiel) % a(:,iboun_new,istep) = xfiel_tmp(:,iboun)
             end do
             call memory_deallo(memor_dom,'XFIEL_TMP','renumbering_element_arrays',xfiel_tmp) 
          end do
       end if
