...
module mod_domain
  ...
  use def_mpio,           only : mpio_flag_geometry_export
  ...
  subroutine domain_memory_allocate_deallocate(itask,wvari,NUMBER1,NUMBER2)
    ...
    case ( 'XFIEL % A' , 'FIELDS' )
       ...
       do ifiel = ifiel1,ifiel2                
          if( itask == 0 ) then                
             if ( (kfl_field(6,ifiel) == 1) .AND. (mpio_flag_geometry_export /= PAR_MPIO_ON) ) then !ondemand 
    ...
    case ( 'KFL_CODNO' )
       ...
       if( itask == 0 ) then
          if( kfl_icodn > 0 ) then 
             call memory_alloca(memor_dom,'KFL_CODNO','mod_domain_memory',kfl_codno,mcono,npoin)
          end if
       else
          if( kfl_icodn > 0 ) then
    ...
    case ( 'WITNESS_MESH' )
       ...
       if( itask == 0 ) then
          if( nwith > 0 ) then
             ...
             do ii = 1,nwith
                witness_mesh(ii) % geom % param = 0.0_rp
                nullify(witness_mesh(ii) % inte % lelem)
                nullify(witness_mesh(ii) % inte % shapf)
             end do
          end if
       else
          if( nwith > 0 ) then
