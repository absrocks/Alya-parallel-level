...
module mod_elmgeo
  ...
  use mod_maths,  only : maths_circumradius_tetrahedron     
  use mod_maths,  only : maths_circumradius_triangle        
  use mod_maths,  only : maths_circumradius_quadrilateral        
  ...
  integer(ip), parameter :: lnode(3,4)   = reshape ( (/1,2,3,1,3,4,2,3,4,1,2,4/),(/3,4/) ) ! Triangles of a bilinear surface
  ...
  !
  ! Element subdivision into tetrahedra TET04
  !
  integer(ip), target    :: HEX08_TO_TET04(4,6) = reshape ( (/1,3,4,5  ,  3,4,5,8  ,  1,2,3,5 , 2,3,5,6 , 3,5,6,7  , 3,5,7,8 /),(/4,6/) ) 
  integer(ip), target    :: PEN06_TO_TET04(4,3) = reshape ( (/1,2,3,4  ,  2,3,4,5  ,  3,4,5,6 /),(/4,3/) ) 
  integer(ip), target    :: PYR05_TO_TET04(4,2) = reshape ( (/1,2,3,5  ,  1,3,4,5  /),(/4,2/) )   
  integer(ip), target    :: TET04_TO_TET04(4,1) = reshape ( (/1,2,3,4  /),(/4,1/) )   
  !
  ...
  integer(ip), parameter :: list_edges_BAR03(2,1)  = reshape ( (/ 1,       2                                                 /), (/2,1 /) )
  ...
  integer(ip), parameter :: list_edges_BAR04(2,1)  = reshape ( (/ 1,       2                                                 /), (/2,1 /) )
  ...
  integer(ip), parameter :: list_edges_TRI06(2,3)  = reshape ( (/ 1,2,     2,3,     3,1                                      /), (/2,3 /) )
  ...
  integer(ip), parameter :: list_edges_TRI10(2,3)  = reshape ( (/ 1,2,     2,3,     3,1                                      /), (/2,3 /) )
  ...
  integer(ip), parameter :: list_edges_QUA08(2,4)  = reshape ( (/ 1,2,     2,3,     3,4,     4,1                             /), (/2,4 /) )
  ...
  integer(ip), parameter :: list_edges_QUA09(2,4)  = reshape ( (/ 1,2,     2,3,     3,4,     4,1                             /), (/2,4 /) )
  ...
  integer(ip), parameter :: list_edges_TET10(2,6)  = reshape ( (/ 1,2, 1,3, 1,4, 2,3, 4,2,   3,4                             /), (/2,6 /) )
  integer(ip), parameter :: list_faces_TET10(6,4)  = reshape ( (/ 1,2,4,5,9,8, 2,3,4,6,10,9, 3,1,4,7,8,10, 1,2,3,5,6,7       /), (/6,4 /) )
  ! What guillaume has put is identically what he had in domfac
  ! Wouldn't it be nicer as I propose in the line bellow so that there is a better corresponndance with TET04
  ! integer(ip), parameter :: list_faces_TET10(6,4)  = reshape ( (/ 1,2,3,5,6,7, 2,3,4,6,10,9, 1,2,4,5,9,8, 3,1,4,7,8,10       /), (/6,4 /) )
  ! Moreover, is there any rule on how the nodes are ordered within the face. It is strange to me that for faces 2-4 the first 3 nodes match those of TET04
  ! But in face 1 it is 1,3,2  instead of 1,2,3 as in TET04? are tet04 or tet10 wrong?
  ...
  integer(ip), parameter :: list_faces_TET20(10,4)  = reshape &
     ( (/ 1,3,2,10,9,8,7,6,5,17, 1,2,4,5,6,16,15,11,12,18, 1,3,4,10,9,14,13,11,12,19, 2,3,4,7,8,14,13,15,16,20                /), (/10,4 /) )
  ...
  integer(ip), parameter :: list_edges_HEX27(2,12) = reshape ( (/ 1,2, 1,4, 1,5, 2,3, 2,6, 3,4, 3,7, 4,8, 5,6, 5,8, 6,7, 7,8 /), (/2,12/) )
  ! list_edges is Idem HEX08 - the same happens for TET10. When I talked with Abel he said that he includes all three nodes in th edge
  ! we should think a bit more about this.
  !  integer(ip), parameter :: list_faces_HEX27(9,6)  = reshape ( (/ 1,4,3,2,12,11,10,9,21,  1,2,6,5,9,14,17,13,22,  2,3,7,6,10,15,18,14,23, & ! not totally sure if I can change line here
  !       3,4,8,7,11,16,19,15,24,  1,5,8,4,13,20,16,12,25,  5,6,7,8,17,18,19,20,26       /), (/9,6 /) )
  ! As I have mentioned for the TET10 case we could reorder so that it resembles more the case of HEX08
  integer(ip), parameter :: list_faces_HEX27(9,6)  = &
       reshape ( (/ 1,4,3,2,12,11,10,9,21,  2,3,7,6,10,15,18,14,23,  5,6,7,8,17,18,19,20,26, 1,5,8,4,13,20,16,12,25, 1,2,6,5,9,14,17,13,22, 3,4,8,7,11,16,19,15,24 /), (/9,6/) )
  ...
  type elem_typ
     ...
     !
     integer(ip)          :: number_corner_nodes  !< Number of corner nodes
     integer(ip)          :: number_edge_nodes    !< Number of edge nodes
     integer(ip)          :: number_face_nodes    !< Number of face nodes
     integer(ip)          :: number_volume_nodes  !< Number of volume nodes 
     integer(ip), pointer :: list_corner_nodes(:) !< List of corner nodes 
     integer(ip), pointer :: list_edge_nodes(:)   !< List of edge nodes   
     integer(ip), pointer :: list_face_nodes(:)   !< List of face nodes   
     integer(ip), pointer :: list_volume_nodes(:) !< List of volume nodes 
  end type elem_typ
  ...
  ! Timings
  !
  integer(ip)             :: numb_natural_coordinates_bb(nelty)
  integer(ip)             :: numb_natural_coordinates_ray(nelty)
  integer(ip)             :: numb_natural_coordinates_nr(nelty)
  real(rp)                :: time_natural_coordinates_bb(nelty)
  real(rp)                :: time_natural_coordinates_ray(nelty)
  real(rp)                :: time_natural_coordinates_nr(nelty)
  !
  ...
  public :: elmgeo_element_type_retrocompatibility
  !
  ! Timings
  !
  public :: numb_natural_coordinates_bb      
  public :: numb_natural_coordinates_ray      
  public :: numb_natural_coordinates_nr     
  public :: time_natural_coordinates_bb  
  public :: time_natural_coordinates_ray 
  public :: time_natural_coordinates_nr  
...
contains
  ...
  subroutine elmgeo_element_type_initialization()
    ...
    !
    !                                                          dim    ord    top    nnode    hess     npf    nfac   edge   iso_len
    element_type(POINT) = elem_typ( 'POINT' , 'Point'         , 0_ip , 1_ip ,-2_ip , 1_ip , .false. , 0_ip , 0_ip , 0_ip , 1.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                          null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(BAR02) = elem_typ( 'BAR02' , 'Linear'        , 1_ip , 1_ip ,-1_ip , 2_ip , .false. , 1_ip , 2_ip , 1_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(BAR03) = elem_typ( 'BAR03' , 'Linear'        , 1_ip , 2_ip ,-1_ip , 3_ip , .true.  , 1_ip , 2_ip , 1_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(BAR04) = elem_typ( 'BAR04' , 'Linear'        , 1_ip , 3_ip ,-1_ip , 4_ip , .false.  ,1_ip , 2_ip , 1_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(TRI03) = elem_typ( 'TRI03' , 'Triangle'      , 2_ip , 1_ip , 1_ip , 3_ip , .false. , 2_ip , 3_ip , 3_ip , 1.0_rp, (/ x1ove3 , x1ove3 , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(TRI06) = elem_typ( 'TRI06' , 'Triangle'      , 2_ip , 2_ip , 1_ip , 6_ip , .true.  , 3_ip , 3_ip , 3_ip , 1.0_rp, (/ x1ove3 , x1ove3 , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(TRI10) = elem_typ( 'TRI10' , 'Triangle'      , 2_ip , 3_ip , 1_ip , 10_ip ,.false.  ,4_ip , 3_ip , 3_ip , 1.0_rp, (/ x1ove3 , x1ove3 , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(QUA04) = elem_typ( 'QUA04' , 'Quadrilateral' , 2_ip , 1_ip , 0_ip , 4_ip , .true.  , 2_ip , 4_ip , 4_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(QUA08) = elem_typ( 'QUA08' , 'Quadrilateral' , 2_ip , 2_ip , 0_ip , 8_ip , .true.  , 3_ip , 4_ip , 4_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(QUA09) = elem_typ( 'QUA09' , 'Quadrilateral' , 2_ip , 2_ip , 0_ip , 9_ip , .true.  , 3_ip , 4_ip , 4_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(QUA16) = elem_typ( 'QUA16' , 'Quadrilateral' , 2_ip , 3_ip , 0_ip ,16_ip , .true.  , 4_ip , 4_ip , 0_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(TET04) = elem_typ( 'TET04' , 'Tetrahedra'    , 3_ip , 1_ip , 1_ip , 4_ip , .false. , 3_ip , 4_ip , 6_ip , 1.0_rp, (/ x1ove3 , x1ove3 , x1ove3 /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(TET10) = elem_typ( 'TET10' , 'Tetrahedra'    , 3_ip , 2_ip , 1_ip ,10_ip , .true.  , 6_ip , 4_ip , 6_ip , 1.0_rp, (/ x1ove3 , x1ove3 , x1ove3 /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(TET20) = elem_typ( 'TET20' , 'Tetrahedra'    , 3_ip , 3_ip , 1_ip ,20_ip , .false.  ,10_ip ,4_ip , 6_ip , 1.0_rp, (/ x1ove3 , x1ove3 , x1ove3 /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(PYR05) = elem_typ( 'PYR05' , 'Pyramid'       , 3_ip , 1_ip , 3_ip , 5_ip , .false. , 4_ip , 5_ip , 8_ip , 1.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(PYR14) = elem_typ( 'PYR14' , 'Pyramid'       , 3_ip , 2_ip , 3_ip ,14_ip , .true.  , 0_ip , 0_ip , 0_ip , 1.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(PEN06) = elem_typ( 'PEN06' , 'Prism'         , 3_ip , 1_ip , 2_ip , 6_ip , .false. , 4_ip , 5_ip , 9_ip , 1.0_rp, (/ x1ove3 , x1ove3 , 0.5_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(PEN15) = elem_typ( 'PEN15' , 'Prism'         , 3_ip , 2_ip , 2_ip ,15_ip , .false.  , 8_ip , 5_ip , 9_ip , 1.0_rp, (/ x1ove3 , x1ove3 , 0.5_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(PEN18) = elem_typ( 'PEN18' , 'Prism'         , 3_ip , 2_ip , 2_ip ,18_ip , .false.  , 9_ip , 5_ip , 9_ip , 1.0_rp, (/ x1ove3 , x1ove3 , 0.5_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(HEX08) = elem_typ( 'HEX08' , 'Hexahedra'     , 3_ip , 1_ip , 0_ip , 8_ip , .true.  , 4_ip , 6_ip ,12_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )

    !element_type(HEX20) = elem_typ( 'HEX20' , 'Hexahedra'     , 3_ip , 2_ip , 0_ip ,20_ip , .false. , 0_ip , 0_ip , 0_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
    !     &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )

    element_type(HEX27) = elem_typ( 'HEX27' , 'Hexahedra'     , 3_ip , 2_ip , 0_ip ,27_ip , .true.  , 9_ip , 6_ip ,12_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(SHELL) = elem_typ( 'SHELL' , 'Triangle'      , 2_ip , 1_ip , 1_ip , 3_ip , .false. , 2_ip , 3_ip , 3_ip , 1.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    element_type(BAR3D) = elem_typ( 'BAR3D' , 'Linear'        , 1_ip , 1_ip ,-1_ip , 2_ip , .false. , 1_ip , 2_ip , 1_ip , 2.0_rp, (/ 0.0_rp , 0.0_rp , 0.0_rp /), &
         &                           null(), null(), null(), null() , 0_ip, 0_ip, 0_ip, 0_ip, null(), null(), null(), null() )
    ...
    do ielty = 1,nelty                                                                                                                                                                           
       ...
       if( .not. associated(element_type(ielty) % type_faces) ) allocate( element_type(ielty) % type_faces(max(1_ip,pface)) )
       if( .not. associated(element_type(ielty) % node_faces) ) allocate( element_type(ielty) % node_faces(max(1_ip,pface)) )
       if( .not. associated(element_type(ielty) % list_faces) ) allocate( element_type(ielty) % list_faces(max(1_ip,pnodf),max(1_ip,pface)) )
       if( .not. associated(element_type(ielty) % list_edges) ) allocate( element_type(ielty) % list_edges(2,pedge) )
    ...
    ! Timings
    !
    numb_natural_coordinates_bb  = 0
    numb_natural_coordinates_ray = 0
    numb_natural_coordinates_nr  = 0
    time_natural_coordinates_bb  = 0.0_rp
    time_natural_coordinates_ray = 0.0_rp
    time_natural_coordinates_nr  = 0.0_rp
    
  end subroutine elmgeo_element_type_initialization

  subroutine elmgeo_element_type_retrocompatibility(&
       iesta_dom,iesto_dom,nface,lexis,needg,leedg,&
       ldime,lorde,ltopo,nnode,nnodf,ltypf,lface)

    integer(ip), intent(in) :: iesta_dom
    integer(ip), intent(in) :: iesto_dom
    integer(ip), intent(in) :: nface(*)
    integer(ip), intent(in) :: lexis(*)
    integer(ip), intent(in) :: needg(*)
    integer(ip), intent(in) :: leedg(2,20,*)
    integer(ip), intent(in) :: ldime(*)
    integer(ip), intent(in) :: lorde(*)
    integer(ip), intent(in) :: ltopo(*)
    integer(ip), intent(in) :: nnode(*)
    type(i1p),   intent(in) :: nnodf(*)
    type(i1p),   intent(in) :: ltypf(*)
    type(i2p),   intent(in) :: lface(*)
    integer(ip)             :: iface,inodf,pnodf,pflty,iedge
    integer(ip)             :: inode,ielty,pface,pedge
    real(rp),    parameter  :: x1ove3 = 1.0_rp/3.0_rp
    ...
    do ielty = iesta_dom,iesto_dom
       if( trim(element_type(ielty) % name) /= 'NULL' ) then
          if( ldime(ielty) /= element_type(ielty) % dimensions   ) print*,'ERROR  1: ',element_type(ielty) % name
          ...
          if( nface(ielty) /= element_type(ielty) % number_faces ) print*,'ERROR  6: ',element_type(ielty) % name,nface(ielty),element_type(ielty) % number_faces
  ...
  end subroutine elmgeo_element_type_retrocompatibility
  ...
  subroutine elmgeo_shape3(s,t,z,nnode,shapf,deriv,heslo)
    ...
    integer(ip)                        :: i,ii,jj, inode, idime, ind
  ...
  subroutine elmgeo_natural_coordinates(  &
    ...
    if( do_bounding_box_test ) then
       do_bounding_box_test = elmgeo_inside_element_bounding_box(ndime,pnode,elcod,coglo,toler,pelty)
    else
       do_bounding_box_test = .true.
    end if
    ...
    if( do_bounding_box_test ) then
       ...
       if( pelty < 0 ) then
          ...
          call cputim(time2)
          numb_natural_coordinates_nr(pelty) = numb_natural_coordinates_nr(pelty) + 1
          time_natural_coordinates_nr(pelty) = time_natural_coordinates_nr(pelty) + (time2-time1)
  ...
  subroutine elmgeo_newrap(&
       coglo,coloc,ndime,pnode,elcod,shapf,deriv,ifoun,pelty,USER_INITIAL_GUESS)
    ...
    integer(ip)                         :: inode,iiter,jdime,maxit,idime
    integer(ip)                         :: jnode,kdime,ierro,ielty
    integer(ip)                         :: permu(3)
    real(rp)                            :: xjacm(ndime,ndime)
    real(rp)                            :: xjaci(ndime,ndime)
...
4   format(i9,50(1x,i9))
  ...
  function elmgeo_inside_element_bounding_box_1(ndime,pnode,elcod,coglo,toler,pelty)
    ...
    if( present(pelty) ) then
       call cputim(time2)
       time_natural_coordinates_bb(pelty) = time_natural_coordinates_bb(pelty) + (time2-time1)
       numb_natural_coordinates_bb(pelty) = numb_natural_coordinates_bb(pelty) + 1
    end if
       
  end function elmgeo_inside_element_bounding_box_1

  function elmgeo_inside_element_bounding_box_2(ndime,xmini,xmaxi,coglo,pelty)
  ...
  function elmgeo_side_of_TRI03(bocod,xx1,xx2)
    real(rp),    intent(in) :: bocod(3,3)
    real(rp),    intent(in) :: xx1(3)
  ...
  subroutine elmgeo_nearest_point_on_QUA04(bocod,coglo,xx,nn)
    ...
    real(rp)                 :: n1(3),n2(3),t,normn1,pp(2)
  ...
  function elmgeo_inside_element_using_faces(pelty,elcod,coglo,toler_opt)
    ...
    integer(ip)                       :: inod1,inod2,iedge,istat,idire,iter
    real(rp)                          :: bocod(3,16)
    real(rp)                          :: vec1(3),vec2(3),toler,lastpro
    real(rp)                          :: nn(3),xx(3),dotpro,trico(3,3)
    real(rp)                          :: xx2(3),segment(3),t,xnorm,xmaxi(3),xmini(3),xdist(3),ynorm
    real(rp)                          :: xx_center(3),n1(3),n2(3),coloc(3),maxn1,norma_segment
    ...
    call cputim(time2)
    numb_natural_coordinates_ray(pelty) = numb_natural_coordinates_ray(pelty) + 1
    time_natural_coordinates_ray(pelty) = time_natural_coordinates_ray(pelty) + (time2-time1)   
  ...
  subroutine elmgeo_nearest_intersection_point_on_element_faces(ndime,pelty,elcod,xx1,xx2,xx_intersection,ifoun,toler_opt)
    ...
    integer(ip)                       :: idime
    real(rp)                          :: bocod(ndime,16),t,toler
  ...
  subroutine elmgeo_cartesian_derivatives(ndime,pnode,elcod,deriv,gpcar,gpdet_opt)
...
10 continue
  ...
  subroutine elmgeo_intersection_segment_TRI06(bocod,xx1,xx2,toler,intersection,ifoun,istat)
    ...
    real(rp)                           :: u_dot_v,s,t,z,d,a,b
    integer(ip)                        :: jstat,i,j
  ...
  subroutine elmgeo_intersection_segment_TRI03(&
    ...
    real(rp)                           :: u_dot_v,s,t,z,d,a,b,xnorm
    integer(ip)                        :: jstat
  ...
  subroutine elmgeo_intersection_segment_QUA04(&
    ...
    real(rp)                           :: t,t1,t2,u1,u2,xnorm
    integer(ip)                        :: ii,jstat,imax(1)
  ...
  subroutine elmgeo_TET04_volume(ndime,v1,v2,v3,v4,element_volume)
  ...
  subroutine elmgeo_PYR05_volume(ndime,v1,v2,v3,v4,v5,element_volume)
    ...
    if(      itask == FLAT_BUBBLE ) then
       ...
       do igaus = 1,pgaus
          ...
          call elmgeo_shapf_deriv_heslo_bubble(&
               ndime,pnode,posgp,gpsha_bub(igaus),deriv_bub(1,igaus),heslo_bub(1,igaus))
       end do

       do igaus = 1,pgaus
          call elmgeo_jacobian_matrix(&
               ndime,pnode,elcod,deriv(1,1,igaus),xjacm,xjaci,gpdet)
          do idime = 1,ndime
             gpcar_bub(idime,igaus) = dot_product(xjaci(1:ndime,idime),deriv_bub(1:ndime,igaus))
          end do
       end do
...
!!!!!!!!!!f         = 2.0_rp*( elfun - fmini ) / ( fmaxi - fmini ) -1.0_rp  !!!OJOOO
       ...
       do igaus = 1,pgaus
          ...
          gpsha_bub(igaus) = xminu - abs(xplus)
  ...
  subroutine elmgeo_element_length_vector(&
...
!!$       call velchl(pnode,elcod,elvel,chale,hleng)
  ...
  !-----------------------------------------------------------------------
    ...
    if(      ielty == BAR02 ) then
       ...
       iso_element_coord_loc(1:1,1) = (/ -1.0_rp /)
       iso_element_coord_loc(1:1,2) = (/  1.0_rp /)
       iso_element_coord_loc(1:1,3) = (/ -2.0_rp/3.0_rp /)
       iso_element_coord_loc(1:1,4) = (/  2.0_rp/3.0_rp /)
       ...
       iso_element_coord_loc(1:2,4) = (/  0.0_rp,  1.0_rp /)
  ...
  subroutine elmgeo_output_jacobian(&
    ...
    integer(ip)                         :: inode,iiter,jdime,maxit,idime
    integer(ip)                         :: jnode,kdime,ierro,ielty,ii,jj,kk,ll
       ...
       do jj = 1,maxit
          do ii = 1,maxit
             ...
             if( local_coord ) then
                ...
                call elmgeo_shapf_deriv_heslo(ndime,pnode,coloc,shapf) 
...
4   format(i9,50(1x,i9))
  ...
  pure subroutine elmgeo_gauss_to_element_s(pnode,pgaus,shapf,shaga,xx,yy,INITIALIZE)
    ...
    do inode = 1,pnode
       do igaus = 1,pgaus
          zz(inode) = zz(inode) + shaga(inode,igaus) * xx(igaus)
       end do
    end do
