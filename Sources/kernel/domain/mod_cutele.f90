module mod_cutele
  ...
  use mod_elmgeo, only     :  elmgeo_shape3
  implicit none
...
contains
  ...
  subroutine buicut (itask)
    use def_domain, only                :  nelem,lelch
...
!!$  end subroutine moveme
  ...
  subroutine moveme(ielem,norma,pcoor,lcrkf_sld)    
    ...
    integer(ip)                 :: ifou1,ifou2,ifoun,nnode_cut
    integer(ip)                 :: lfoun(16),ielpo,jelem,inter
       ...
       if ( lelch(ielem)  == ELCUT ) then
          ...
          elseif (itask == 3) then
             ...
             do inode = 1,pnode
                ...
                !     
                call dpopar(                                                                          &
                     1_ip, imbou(iimbo) % cooib(1:ndime,ipoin) , imbou(iimbo) % npoib , mnoib ,       &
  ...
  subroutine elmcar_cut(&
    ...
    use def_domain, only       :  ltopo,mgaus,ndime,ntens,nnode
    use def_domain, only       :  mnode,elmar,ngaus
    ...
    do ielem_sub = 1,cutel(ielem) % nelem
       ...
       do igaus_sub = 1,pgaus_sub
          ...
          if( ndime == 2 ) then
             ierro = 0
             call elmgeo_shape2(&
                  coloc(1),coloc(2),pnode,gpsha(1,pgaus),&
                  gpder(1,1,pgaus),heslo(1,1,pgaus))
          else if( ndime == 3 ) then
             ierro = 0
             call elmgeo_shape3(&
                  coloc(1),coloc(2),coloc(3),pnode,gpsha(1,pgaus),&
                  gpder(1,1,pgaus),heslo(1,1,pgaus))
          end if
    ...
    do ielem_sub = 1,cutel(ielem) % nelem
       ...
       do igaus_sub = 1,pgaus_sub
          ...
          if( ndime == 2 ) then
             ...
             call shape2(&
                  coloc(1),coloc(2),pnode,gpsha(1,pgaus),&
                  deriv(1,1,pgaus),heslo(1,1,pgaus),ierro)
          else if( ndime == 3 ) then
             ierro = 0
             call shape3(&
                  coloc(1),coloc(2),coloc(3),pnode,gpsha(1,pgaus),&
