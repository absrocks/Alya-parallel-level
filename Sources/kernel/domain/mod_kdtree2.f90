...
module kdtree2_priority_queue_module
  ...
  type kdtree2_result
     ...
     real(rp)    :: dis   !=0.0
     integer(ip) :: idx   !=-1   Initializers cause some bugs in compilers.
...
contains
  ...
  function pq_create(results_in) result(res)
    ...
    nalloc = size(results_in,1,KIND=ip)
    if (nalloc < 1) then
  ...
  subroutine heapify(a,i_in)
    ...
    !
    type(pq),    pointer     :: a
    integer(ip), intent(in)  :: i_in
    !
    integer(ip)              :: i, l, r, largest
    real(rp)                 :: pri_i, pri_l, pri_r, pri_largest
  ...
  real(rp) function pq_replace_max(a,dis,idx) 
    !
...
module mod_kdtree2
  ...
  integer(ip), parameter :: bucket_size = 12
  ...
  type :: tree_node
     ...
     private
     integer(ip)               :: cut_dim                      ! the dimension to cut
     real(rp)              :: cut_val                      ! where to cut the dimension
     real(rp)              :: cut_val_left, cut_val_right  ! improved cutoffs knowing the spread in child boxes.
     integer(ip)               :: l, u
  ...
  type :: kdtree2
     ...
     integer(ip)               :: dimen=0, n=0            ! dimensionality and total # of points     
     real(rp),     pointer :: the_data(:,:) => null() ! pointer to the actual data array 
     ! 
  ...
  type :: tree_search_record
     ...
     !
     integer(ip)                   :: dimen   
     integer(ip)                   :: nn, nfound
     real(rp)                      :: ballsize
     integer(ip)                   :: centeridx=999, correltime=9999
     ! exclude points within 'correltime' of 'centeridx', iff centeridx >= 0
     integer(ip)                   :: nalloc  ! how much allocated for results(:)?
     logical(lg)                   :: rearrange  ! are the data rearranged or original? 
     ! did the # of points found overflow the storage provided?
     logical(lg)                   :: overflow
     real(rp),             pointer :: qv(:)  ! query vector
     type(kdtree2_result), pointer :: results(:) ! results
     type(pq)                      :: pq
     real(rp),             pointer :: data(:,:)  ! temp pointer to data
     integer(ip),          pointer :: ind(:)     ! temp pointer to indexes
...
contains
  ...
  function kdtree2_create(input_data,idime,sort,rearrange) result (mr)
    !
    ...
    !
    ! Optional arguments:  If 'idime' is specified, then the tree
    !                      will only search the first 'idime' components
    !                      of input_data, otherwise, idime is inferred
    !                      from SIZE(input_data,1,KIND=ip).
    !
    ...
    logical(lg),   intent(in), optional      :: sort
    logical(lg),   intent(in), optional      :: rearrange
    ! ..
    ! .. Array Arguments ..
    real(rp), target :: input_data(:,:)
    !
    integer(ip):: i
    ! ..
    allocate (mr)
    ...
    if (present(idime)) then
       ...
       mr%dimen = size(input_data,1,KIND=ip)
    end if
    mr%n = size(input_data,2,KIND=ip)
  ...
  subroutine build_tree(tp)
    ...
    tp%root => build_tree_for_range(tp,1_ip,tp%n, dummy)
  ...
  recursive function build_tree_for_range(tp,l,u,parent) result (res)
    ...
    ! ..
    ! .. Scalar Arguments ..
    integer(ip), intent (in) :: l, u
    ! ..
    ! .. Local Scalars ..
    integer(ip):: i, c, m, dimen
    logical(lg):: recompute
    real(rp)    :: average
...
!!$      
    ...
    if ((u-l)<=bucket_size) then
       ...
       if (.false.) then
          ...
          if (.true.) then
             ...
             average = sum(tp%the_data(c,tp%ind(l:u))) / real(u-l+1,rp)
          else
             average = (res%box(c)%upper + res%box(c)%lower)/2.0_rp
          endif
       ...
       res%right => build_tree_for_range(tp,m+1_ip,u,res)
  ...
  integer(ip)function select_on_coordinate_value(v,ind,c,alpha,li,ui) &
       result(res)
  ...
  subroutine select_on_coordinate(v,ind,c,k,li,ui)
    ...
    ! .. Scalar Arguments ..
    integer(ip), intent (in) :: c, k, li, ui
    ! ..
    integer(ip):: i, l, m, s, t, u
    ! ..
    real(rp) :: v(:,:)
    integer(ip):: ind(:)
    ! ..
  ...
  subroutine spread_in_coordinate(tp,c,l,u,interv) 
    ...
    ! ..
    ! .. Scalar Arguments ..
    integer(ip), intent (in) :: c, l, u
    ! ..
    ! .. Local Scalars ..
    real(rp) :: last, lmax, lmin, t, smin,smax
    integer(ip):: i, ulocal
    ! ..
    ! .. Local Arrays ..
    real(rp), pointer :: v(:,:)
  ...
  subroutine kdtree2_n_nearest_around_point(tp,idxin,correltime,nn,results)
    ...
    type (kdtree2), pointer        :: tp
    integer(ip), intent (in)           :: idxin, correltime, nn
    type(kdtree2_result), target   :: results(:)

    allocate (sr%qv(tp%dimen))
    sr%qv = tp%the_data(:,idxin) ! copy the vector
    sr%ballsize = huge(1.0_rp)       ! the largest real(rp) number
    sr%centeridx = idxin
  ...
  subroutine kdtree2_r_nearest(tp,qv,r2,nfound,nalloc,results) 
    ...
    !
    !  Note that if nfound > nalloc then more neighbors were found
    ...
    integer(ip), intent(out)         :: nfound
    integer(ip), intent (in)         :: nalloc
    type(kdtree2_result), target :: results(:)
  ...
  subroutine kdtree2_r_nearest_around_point(tp,idxin,correltime,r2,&
    ...
    !
    type (kdtree2), pointer      :: tp
    integer(ip), intent (in)         :: idxin, correltime, nalloc
    real(rp), intent(in)             :: r2
    integer(ip), intent(out)         :: nfound
    type(kdtree2_result), target :: results(:)
    ! ..
  ...
  function kdtree2_r_count_around_point(tp,idxin,correltime,r2) &
    ...
    !
    type (kdtree2), pointer :: tp
    integer(ip), intent (in)    :: correltime, idxin
    real(rp), intent(in)        :: r2
    integer(ip)                :: nfound
    ! ..
    ! ..
  ...
  subroutine validate_query_storage(n)
    ...
    if (size(sr%results,1,KIND=ip) < n) then
  ...
  real(rp) function dis2_from_bnd(x,amin,amax) result (res)
    real(rp), intent(in) :: x, amin,amax
  ...
  logical(lg) function box_in_search_range(node, sr) result(res)
    !
  ...
  subroutine process_terminal_node(node)
    ...
    !
    integer(ip)               :: dimen, i, indexofi, k, centeridx, correltime
    real(rp)                   :: ballsize, sd, newpri
    logical(lg)               :: rearrange
    type(pq), pointer      :: pqp 
    !
  ...
  subroutine process_terminal_node_fixedball(node)
    ...
    !
    integer(ip)                :: nfound
    integer(ip)                :: dimen, i, indexofi, k
    integer(ip)                :: centeridx, correltime, nn
    real(rp)                   :: ballsize, sd
    logical(lg)                :: rearrange
  ...
  subroutine kdtree2_n_nearest_brute_force(tp,qv,nn,results) 
    ...
    integer(ip):: i, j, k
    real(rp), allocatable :: all_distances(:)
    ! ..
  ...
  subroutine heapsort(a,ind,n)
    ...
    !
    !
    real(rp)                         :: value   ! temporary for a value from a()
    integer(ip)                      :: ivalue  ! temporary for a value from ind()
