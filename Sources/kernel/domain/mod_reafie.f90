...
module mod_reafie
  ...
  use def_mpio,          only : mpio_flag_enabled, PAR_MPIO_ON
...
contains
  ...
  subroutine reafie_skip()
    use mod_reabcs, only : reabcs_skip
    call reabcs_skip()
  end subroutine reafie_skip
...
7   call runend('REAFI: WRONG NUMBER OF FIELD COMPONENTS  when reading a field with many components '//trim(intost(ipoin)))
  ...
  subroutine reafie_seq()
    ...
    use mod_ecoute,   only : ecoute
    implicit none
    integer(ip)             :: kfl_defau,ifiel,jstep
  ...
  subroutine reafie_read_header(ifiel,kfl_defau,defau)
    ...
    if( exists('DIMEN') ) then
       ...
       if( exists('BEGIN') ) then
          ...
          if (.NOT. mpio_flag_enabled==PAR_MPIO_ON) then
