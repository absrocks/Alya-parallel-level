...
subroutine grodom(itask)
  ...
  use def_domain,         only : c_dom,nperi
        ...
        if( INOTMASTER ) then
           ...
           if( ISEQUEN ) then
              call partition_sfc(lepar_=lgrou_dom,npart_sfc_=ngrou_dom,lenti_=coord,&
                   nenti_=npoin,sdim_=ndime,lweig_=std_int_1,boxes_coarse_=boxes_coarse_sfc,&
                   boxes_fine_=boxes_fine_sfc) 
           else
              call partition_sfc(lepar_=lgrou_dom,npart_sfc_=ngrou_dom,lenti_=coord,&
                   nenti_=npoin,sdim_=ndime,lweig_=std_int_1,boxes_coarse_=boxes_coarse_sfc,&
                   boxes_fine_=boxes_fine_sfc,lcorr_=std_real_1,PAR_COMM_=PAR_COMM_MY_CODE_WM4) 
           end if
#else
           if( ISEQUEN ) then
              ...
              call partition_sfc(lgrou_dom,ngrou_dom,coord,npoin_own,ndime,  &
                   boxes_coarse_=boxes_coarse_sfc,boxes_fine_=boxes_fine_sfc,&
                   PAR_COMM_=PAR_COMM_MY_CODE_WM4)
           end if
#endif
           call PAR_INTERFACE_NODE_EXCHANGE(lgrou_dom,'SUM')
        end if
