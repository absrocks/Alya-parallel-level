...
contains
  ...
  subroutine local_basis_parall()
    ...
    do ii = 1,num_lobas
       call PAR_BROADCAST(lobas(ii) % type)
       call PAR_BROADCAST(9_ip,lobas(ii) % param)
    end do
  ...
  !>          \endverbatim
  !>
  !> @todo    Find a better mathematical method in order to define the cylindrical
  !>          local basis in 3-d.
  !----------------------------------------------------------------------------
       ...
       do ipoin = 1, npoin
          ...
          if( ibopo > 0 ) then
             ...
             if( lobas(icsys) % type == LOCAL_BASIS_CARTESIAN ) then
                ...
                !
                ! Cylindrical LOCAL_BASIS (A = Local Axial axis)
                !
                if( ndime == 2_ip ) then
                   !
                   ! Radial axis
                   R(1) = coord(1,ipoin) - lobas(icsys) % param(1)
                   R(2) = coord(2,ipoin) - lobas(icsys) % param(2)
                   ...
                   ! Radial axis
                   !
                   if( abs(lobas(icsys) % param(3*ndime)) > 0.0_rp ) then
                      !
                      ! X-Y Plane (Axial=Z)
                      !
                      R(1) = coord(1,ipoin) - lobas(icsys) % param(1)
                   ...
                   else if( abs(lobas(icsys) % param(3*ndime-1)) > 0.0_rp  ) then
                      !
                      ! X-Z Plane (Axial=Y)
                      !
                      R(1) = coord(1,ipoin) - lobas(icsys) % param(1)
                      R(2) = 0.0_rp
                      R(3) = coord(3,ipoin) - lobas(icsys) % param(3)
                      call maths_normalize_vector(ndime,R(:),rho)

                   else if( abs(lobas(icsys) % param(3*ndime-2)) > 0.0_rp ) then
                      !
                      ! Y-Z Plane (Axial=X)
                      !
                      R(1) = 0.0_rp
                      ...
                      call runend('MOD_LOCAL_BASIS: CYLINDRICAL BASIS NOT DEFINED IN THE MAIN PLANES')

                   end if
                   !
                   ! Axial axis
                   A(1) = lobas(icsys) % param(3*ndime-2)
                   A(2) = lobas(icsys) % param(3*ndime-1)
                   A(3) = lobas(icsys) % param(3*ndime)
