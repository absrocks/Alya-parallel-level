subroutine ker_allaws()
  ...
  condu_ker % llaws(11) % wname    = 'CMCNP'        ! Unconditional conductivity at nodes
  ...
  sphea_ker % llaws(9) % wname    = 'CMCNP'        ! Unconditional specific heat
  ...
  poros_ker % llaws(2) % wname     = 'VALVE'     ! Canopy Porosity
  poros_ker % llaws(2) % lresp(1)  =  ID_NASTIN
  poros_ker % llaws(2) % where     = 'IELEM'
  poros_ker % llaws(2) % kfl_gradi = 0

  poros_ker % llaws(3) % wname     = 'DAFOR' ! Darcy-Forchheimer porosity
  poros_ker % llaws(3) % lresp(1)  =  ID_NASTIN
  poros_ker % llaws(3) % where     = 'IELEM'
  poros_ker % llaws(3) % kfl_gradi = 0


  poros_ker % llaws(4) % wname     = 'FUNCT'     ! Time function porosity
  ...
  anipo_ker % llaws(2) % wname     = 'MATRI'       ! Bifluid
  anipo_ker % llaws(2) % lresp(1)  =  ID_NASTIN  
  anipo_ker % llaws(2) % where     = 'IELEM'
  anipo_ker % llaws(2) % kfl_gradi = 0             ! Despite there is a gradient we never use it for Biflid flow 
