  ...
  !> @} 
  !-----------------------------------------------------------------------
  subroutine ker_vortexxxx(coord_vort)
    use def_parame
    use def_master
    use def_domain 
    use def_kintyp
    ...
    if( INOTMASTER ) then
       ...
       if( kfl_vortx == 1 ) then
          call ker_vorte1(vered,coord_vort)
       else if( kfl_vortx == 2 ) then
          call ker_vorte2(vered,coord_vort)
       end if
  ...
  subroutine ker_vorte2(vered,coord_vort)
    ...
    if( INOTMASTER ) then
       ...
       elements: do ielem = 1,nelem 
          ...
          if( pnode == 4 ) then
             !
             face:do iface = 1,nface(pelty)
                !
                !Shape function of triangle(shape2.f90 in kernel/mathru)
                !    
                inode1 = lface(pelty) % l(1,iface)  
                inode2 = lface(pelty) % l(2,iface) 
                inode3 = lface(pelty) % l(3,iface)
