...
module mod_ker_proper
  ...
  integer(ip), parameter :: NUMBER_OF_PROPERTIES=11
  ...
  interface ker_proper_point_to_property
     module procedure ker_proper_point_to_property_w, &
          &           ker_proper_point_to_property_ip
  end interface ker_proper_point_to_property
  ...
  public :: ker_proper_point_to_property
  public :: ker_proper
  public :: ker_updpro
...
contains
  ...
  subroutine ker_proper_point_to_property_ip(iprop,prope_ker)
    ...
    case (    1_ip ) ; prope_ker => densi_ker 
    case (    2_ip ) ; prope_ker => visco_ker 
    case (    3_ip ) ; prope_ker => poros_ker 
    case (    4_ip ) ; prope_ker => condu_ker 
    case (    5_ip ) ; prope_ker => sphea_ker 
    case (    6_ip ) ; prope_ker => dummy_ker 
    case (    7_ip ) ; prope_ker => turmu_ker 
    case (    8_ip ) ; prope_ker => absor_ker 
    case (    9_ip ) ; prope_ker => scatt_ker 
    case (   10_ip ) ; prope_ker => mixin_ker 
    case (   11_ip ) ; prope_ker => anipo_ker 
    case   default   ; call runend('KER_PROPER_LIST_PROPERTIES: UNKNOWN PROPERTY NUMBER')

    end select

  end subroutine ker_proper_point_to_property_ip

  subroutine ker_proper_point_to_property_w(wprop,prope_ker)
    ...
    case ( 'TURMU' ) ; prope_ker => turmu_ker 
    case ( 'ABSOR' ) ; prope_ker => absor_ker 
    case ( 'SCATT' ) ; prope_ker => scatt_ker 
    case ( 'MIXIN' ) ; prope_ker => mixin_ker 
    case ( 'ANIPO' ) ; prope_ker => anipo_ker 
    case   default   ; call runend('KER_PROPER_LIST_PROPERTIES: UNKNOWN PROPERTY NUMBER FOR '//wprop(1:5))

    end select

  end subroutine ker_proper_point_to_property_w
  ...
  integer(ip) function ker_proper_law_name_to_number(wprop,wlaws)
    ...
    call ker_proper_point_to_property_w(wprop,prope_ker)
...
!!$                end if
          ...
          if( prope_ker % llaws(ilaws) % where == 'CONST' ) then
             ...
             if( kfl_gradi == 1 ) then
                ...
                if( prope_ker % kfl_type == SCALAR_PROPERTY ) then
                   if( present(gpsha) ) then
                      do igaus = 1,qgaus
                         xvalu(1:VECTOR_SIZE_LOC,igaus) = prope_ker % value_const(1,imate)
                      end do
                   else
  ...
  subroutine ker_updpro(itask_xxx)
    ...
    integer(ip)                  :: iprop,ilaws,pnodb,iboun,pgaub,igaub,ii,jj,kk,ll
    integer(ip)                  :: iresp,kfl_updpr(NUMBER_OF_PROPERTIES),idime,igaus
    integer(ip)                  :: ispec,jspec,jdime,dummi
    ...
    real(rp)                     :: xcoef(2,ncoef_shaw),aux_dvari
    
    !
    ! for VALVE porosity model
    !
    integer(ip)                  :: id_module,  bound_set_P1, bound_set_P2
    real(rp)                     :: slope, poros_max 
    real(rp)                     :: delta_P_ref, mean_pressure_1, mean_pressure_2
    !
             ...
             do imate = 1,nmate
                 if( itask == ITASK_INIUNK .and. prope_ker % value_default(imate) > 0.0_rp ) then
                  ...
                  T0   = prope_ker % rlaws(3,imate)
                      ...
                      if( lmate(ielem) == imate ) then
                         ...
                         if( pelty > 0 ) then
                            ...
                            do igaus = 1,pgaus
                               ...
                               do inode = 1,pnode
                                  gp_temperature(1) = gp_temperature(1) + elmar(pelty) % shape(inode,igaus) * eltem(inode)
                               end do
                   ...
                   if ( ipass(imate)==0 ) then
                      ...
                      if (zmaxf.gt.0.01_rp) then 
                         ...
                         do icoun = 1, ncoun -1
                            ...
                            LAD_aver = LAD_aver + (((heigh -zmaxi) /(heigh - zz))**n )*exp(n*(1.0_rp-(heigh -zmaxi) /(heigh - zz)))
                         end do
                         
                         LAD_aver = LAD_aver  + 0.5_rp*(((heigh -zmaxi) /(heigh ))**6.0_rp )*exp(6.0_rp*(1.0_rp-(heigh -zmaxi) /(heigh )))
                         
                         LAD_aver = LAD_aver  + 0.5_rp*(((heigh -zmaxi) /(1.0e-4_rp))**0.5_rp )*exp(0.5_rp*(1.0_rp-(heigh -zmaxi) /(1.0e-4_rp)))
                   ...
                   !
                   !----------------------------------------------------------------------
                   bound_set_P1 = int(prope_ker % rlaws(1,imate),KIND=ip) ! set 1
                   bound_set_P2 = int(prope_ker % rlaws(2,imate),KIND=ip) ! set 2
                   poros_max    = prope_ker % rlaws(3,imate) ! maximum value parameter of the curve
                   slope        = prope_ker % rlaws(4,imate) ! slope parameter of the curve
                   delta_P_ref  = prope_ker % rlaws(5,imate) ! delta P reference 
                   id_module    = prope_ker % llaws(ilaws) % lresp(1)

                   mean_pressure_1 = momod(id_module) % postp(1) % vbset(1, bound_set_P1) 
                   mean_pressure_2 = momod(id_module) % postp(1) % vbset(1, bound_set_P2)
                    ...
                    if( lmate(ielem) == imate ) then 
                      pelty = ltype(ielem)
                        if( pelty > 0 ) then
                           ...
                           pnode = lnnod(ielem)
                           do igaus =1, pgaus
                             prope_ker % value_ielem(ielem) % a(igaus) = poros_max*(1.0_rp+tanh(((mean_pressure_2-mean_pressure_1)-delta_P_ref)/slope)) 
                           end do
                        end if 
                    end if
                   end do
                      ...
                      if( lmate(ielem) == imate ) then
                         ...
                         if( pelty > 0 ) then
                            ...
                            do igaus =1, pgaus
                               ...
                               prope_ker % value_ielem(ielem) % a(igaus) =  darcy * visco_ker % value_const(1,imate)  &
                   ...
                   aux_prop = funcre( time_function(ifunc) % parameters, &
                      ...
                      if( lmate(ielem) == imate ) then
                         ...
                         if( pelty > 0 ) then
                            ...
                            do igaub = 1,pgaub
                               ...
                               prope_ker % value_iboun(iboun) % a(igaub) =  gptur
                               !prope_ker % value_iboun(iboun) % a(igaub) =  0.0_rp
                      ...
                      if( lmate(ielem) == imate ) then
                         ...
                         if( pelty > 0 ) then
                            ...
                            igauss_loop:  do igaus = 1,pgaus
                               ...
                               if (kfl_kxmod_ker==3) then ! wilcox 2006, introduce limiter
                                  call elmca2(&
                                       pnode,pgaus,0_ip,elmar(pelty)%weigp,elmar(pelty)%shape,&
                                       elmar(pelty)%deriv,elmar(pelty)%heslo,elcod,gpvol,gpsha,&
                                       gpder,gpcar,gphes,ielem)
                                  ...
                                  omega = max(omega, 7.0_rp/8.0_rp*sqrt(seci4/Cmu_st))
                               end if
                               call ker_proper('DENSI','IGAUS',igaus,ielem,gpden,pnode,pgaus,elmar(pelty)%shape)
                               call ker_proper('VISCO','IGAUS',igaus,ielem,gpvis,pnode,pgaus,elmar(pelty)%shape)
                   ...
                   do ielem = 1,nelem
                      if( lmate(ielem) == imate ) then
                         ...
                         if( pelty > 0 ) then
                            pgaus = ngaus(pelty)
                            prope_ker % value_ielem(ielem) % a(1:pgaus) =prthe(which_time)*wmean_gp(ielem) % a(1:pgaus,which_time,1)/(gasco*tempe_gp(ielem) % a(1:pgaus,which_time,1)) 
                         end if
                      end if
                   end do
                ...
                else if( prope_ker % wlaws(imate) == 'CMCGP' ) then

                   !----------------------------------------------------------------------
                   !
                   !  Density from CHEMIC from CMC combustion model, read from matrix on 
                   !  Gauss points
                   !
                   !----------------------------------------------------------------------
                ...
                else if( prope_ker % wlaws(imate) == 'CMCNP') then

                   !----------------------------------------------------------------------
                   !
                   ! Heat conductivity is taken at nodal points for CMC combustion model
                   !
                   !----------------------------------------------------------------------

                   !print*, ''
                   !print*, ''
                   !print*, 'Imprimo conductividades'
                   do kpoin = 1,nmatn(imate)
                      ipoin = lmatn(imate) % l(kpoin)
                      prope_ker % value_ipoin(ipoin) = condk(ipoin,1)
                      !print*, ipoin, '   ', prope_ker % value_ipoin(ipoin)
                   end do
                   ...
                   !----------------------------------------------------------------------
                   !
                   ! Specific heat is taken from nodal points for CMC combustion model
                   !
                   !----------------------------------------------------------------------
