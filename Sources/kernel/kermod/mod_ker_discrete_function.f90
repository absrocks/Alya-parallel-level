...
contains
  ...
  subroutine ker_discrete_function_read()
    ...
    ! ADOC[3]>   FUNCTION_NUMBER: char, NUMBER=int, DIMENSION=int   $ Function number
    ...
    do while( words(1) /= 'ENDDI' )
       ...
       if( words(1) == 'FUNCT' ) then
          ...
          if( ifunc < 0 .or. ifunc > num_discrete_functions ) then
             call runend('KER_DISCRETE_FUNCTION_READ: WRONG FUNCION NUMBER')
          end if
          ...
          do while( words(1) /= 'ENDFU' ) 
             if ( words(1) == 'TIMES' ) then
                ...
                if( fundi(ifunc) % dim == 0 ) then             
                   call runend('KER_DISCRETE_FUNCTION_READ: DIMENSION OF THE DISCRETE FUNCTION SHOULD BE GIVEN')
                end if
                ...
                call ecoute('ker_discrete_function_read')
                fundi(ifunc) % num_tim = int(param(1))
  ...
  subroutine ker_dicsrete_function_allocate(ifunc)
    ...
    if( num_discrete_functions > 0 ) then
       if( present(ifunc) ) then
          my_dim     = fundi(num_discrete_functions) % dim
          my_num_tim = fundi(num_discrete_functions) % num_tim
  ...
  subroutine ker_discrete_function_parall()
    ...
    call PAR_BROADCAST(num_discrete_functions)
    if( ISLAVE ) call ker_dicsrete_function_allocate()
    do ifunc = 1,num_discrete_functions
       call PAR_BROADCAST(fundi(ifunc) % kfl_type)
       call PAR_BROADCAST(fundi(ifunc) % kfl_periodic)
       call PAR_BROADCAST(fundi(ifunc) % dim)
       call PAR_BROADCAST(fundi(ifunc) % num_tim)
       if( ISLAVE ) call ker_dicsrete_function_allocate(ifunc)
    end do
    do ifunc = 1,num_discrete_functions
       call PAR_BROADCAST(fundi(ifunc) % val)
       call PAR_BROADCAST(fundi(ifunc) % tim)
       call PAR_BROADCAST(5_ip,fundi(ifunc) % name,'IN MY CODE')
    end do
