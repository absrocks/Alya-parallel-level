module def_master
  ...
  integer(ip), parameter   :: &
       ...
       FUNCTION_DISCRETE            =   6_ip,&  ! 5 : Module function
       FUNCTION_USER                =   0_ip    ! 0 : User function

  ! TODO : EXM-SLD-ECC : Delete of master
  ! cell model identifiers
  integer(ip), parameter ::&
       CELL_NOMOD_EXMEDI    = 0,  &
       CELL_FITZHUGH_EXMEDI = 1,  &
       CELL_FENTON_EXMEDI   = 2,  &
       CELL_TT2006_EXMEDI   = 4,  &
       CELL_OHARA_EXMEDI    = 5,  &
       CELL_SCATRIA_EXMEDI  = 6,  &
       CELL_SCVENTRI_EXMEDI = 7,  &
       CELL_TORORD_EXMEDI   = 8
...
!       lun_time,              & ! Convergence at time step file unit
  ...
  integer(ip)              :: &
       ...
       !kfl_inpfo,             &      ! Input format (geometry)
       !kfl_rstfo,             &      ! Restart format (MPI-IO only)
       ...
       kfl_rread,             &      ! Reread ini data in READ_AND_RUN mode
       lun_livei,             &      ! Output unit
       max_secti,             &      ! Max number of sections
       kfl_color                     ! Colored output
  real(rp)                 :: &
  ...
  real(rp)                 :: &
       cpu_modul(50,mmodu),   &      ! Module CPU time
  ...
  real(rp)                 :: &
       ...
       cpu_start(10),         &      ! CPU for starting operations
  ...
  real(rp), pointer        :: &
       ...
       fisoc(:),              &      ! 29: Isochrones nodal field.
  ...
  real(rp)                 :: &
       ...
       aptim(15000),          &      ! Activation potential starting times  EXMEDI - SOLIDZ
       imass                         ! Total initial mass  for low mach

  ! TODO : EXM-SLD-ECC : Delete of master
  integer(ip), parameter   :: kfl_exm_max_nmaterials=10_ip ! max number of materials in exmedi

  ! TODO : EXM-SLD-ECC : Delete of master
  integer(ip)              :: &
     kfl_cellmod(kfl_exm_max_nmaterials)=-1_ip  ,    &    ! Electrophysiology model  EXMEDI - SOLIDZ
     kfl_eccty(kfl_exm_max_nmaterials)=1_ip               ! Electromechanical coupling type (per material) EXMEDI - SOLIDZ
  ...
  integer(ip)            :: VECTOR_SIZE = 1_ip  ! Size for vectorization
#else
#ifndef VECTOR_SIZE
  integer(ip), parameter :: VECTOR_SIZE = 1_ip  ! Size for vectorization
#endif
#endif
...
contains
  ...
  !> @details Read some optional argu,ets and assign a default value
  !> 
  !-----------------------------------------------------------------------

  function optional_argument_ch(DEFAULT,CONDITION) result(my_char)
  ...
  logical(lg) function optional_argument_lg(DEFAULT,CONDITION)
  ...
  integer(4) function optional_argument_i4(DEFAULT,CONDITION)
  ...
  integer(8) function optional_argument_i8(DEFAULT,CONDITION)
  ...
  function optional_argument_i8_1(DEFAULT,CONDITION) result(out)
  ...
  real(rp) function optional_argument_rp(DEFAULT,CONDITION)
