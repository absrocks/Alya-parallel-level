...
!!$    integer(ip) :: icolor,iproc,PAR_CACA
!!$    if( kfl_paral /= 2 ) then
!!$       icolor = 1_4
!!$    else
!!$       icolor = 1_4
!!$    end if
!!$    call PAR_COMM_SPLIT(icolor,PAR_CACA,iproc,'IN MY CODE')
!!$    print*,'P=',kfl_paral,iproc,PAR_CACA
!!$    if( kfl_paral /= 2 ) then
!!$       iproc = 1
!!$       call PAR_SUM(iproc,PAR_COMM_IN4=PAR_CACA)
!!$       print*,'sum=',iproc
!!$    end if
!!$    call runend('O.K.!')
!!$  end block
  ...
  if( kfl_timeline /= 0 ) call ker_timeline(0_ip,'END_READ_MESH')
