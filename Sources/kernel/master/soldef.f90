subroutine soldef(itask)
  ...
  use def_inpout
  use mod_memchk
  use def_kermod,        only : kfl_direct_solver
  use def_kermod,        only : ndivi 
  use def_kermod,        only : kfl_full_rows
  use def_kermod,        only : kfl_ell,kfl_coo
  ...
  integer(ip)             :: ivari,nvari,iauxi,cont,idime,i1,i2,nn
        ...
        if( solve_sol(ivari) % kfl_algso == -999 ) then
           ...
           if( INOTMASTER ) then
              ...
              do kblok = 1,solve(1) % num_blocks
                 solve(1) % block_dimensions(kblok) = solve(1+kblok-1) % ndofn
              end do
...
!!$              end if
           ...
           if( solve_sol(ivari) % kfl_preco == SOL_GAUSS_SEIDEL ) then
              ...
              else if( solve_sol(ivari) % kfl_renumbered_gs == GAUSS_SEIDEL            ) then
