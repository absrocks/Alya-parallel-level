...
contains
  ...
  subroutine par_interface_exchange(comm,TYPE_OF_COUPLING,COMM_NAME)
    ...
    integer(ip)                                      :: ipoin_slave,ipoin_master
    ...
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm,par_memor,COMM_NAME='COMMD',INITIALIZE=.true.)
    comm % PAR_COMM_WORLD = PAR_COMM_MY_CODE
    PAR_COMM_MY_CODE4     = int(PAR_COMM_MY_CODE,4)
    ...
    if( INOTMASTER ) then
       ...
       call PAR_ALLOCATE_COMMUNICATION_ARRAY(comm,par_memor,COMM_NAME=trim(my_comm_name))
