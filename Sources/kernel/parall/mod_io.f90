  ...
  subroutine io_read_merge_rp_1(xx,file_name,wtype)
    ...
    if( INOTMASTER ) then
       if(      wtype5 == 'NPOIN' ) then
          ...
          call redistribution_array(xx_in,wtype5,MEMOR=memor_dom,VARIABLE_NAME='XX_IN',COMM=commd_nboun_from_mpio,PERMUTE_RECV=.true.,VERBOSE=.false.)
       end if
    end if
    !
