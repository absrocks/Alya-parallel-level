...
subroutine par_edge_communication_array(meshe,COMM)
  ...
  if( INOTMASTER ) then
     ...
     !
     call graphs_rengra(meshe % nedge,0_ip,permu,invpr,meshe % r_edg,meshe % c_edg)
     !
     ! Check owner ship
     !
     permu = 0
  ...
  if( ierro > 0 ) then
     call runend('PAR_EDGE_COMMUNICATION_ARRAY: ERROR DETECTED ON EDGE OWNERSHIP')
  end if
