...
contains
  ...
  subroutine PAR_INITIALIZE_COMMUNICATION_ARRAY_BASIC(COMM,PAR_COMM_OPT)
    type(comm_data_par_basic), intent(inout)        :: COMM     !< Input communicator
    logical(lg),               intent(in), optional :: PAR_COMM_OPT
    logical(lg)                                     :: PAR_COMM
  ...
  subroutine PAR_ALLOCATE_COMMUNICATION_ARRAY_s(COMM_IN,memor_opt,COMM_NAME)
    ...
    integer(8)                                   :: memor(2)
    character(100)                               :: my_comm_name

    if( present(COMM_NAME) ) then
       my_comm_name = trim(COMM_NAME)
    else
       my_comm_name = 'COMM_IN'
    end if
...
!!$    call memory_alloca(memor,'COMM_IN % GHOST_RECV_BOUN_PERM','par_allocate_communication_array',COMM_IN % ghost_recv_boun_perm  , )
  ...
  subroutine PAR_COPY_COMMUNICATION_ARRAY_BASIC(COMM_IN,COMM_OUT,memor_opt,COMM_NAME)
    ...
    call memory_copy(memor,trim(my_comm_name)//' % NEIGHTS',             'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % neights               , COMM_OUT % neights,             'DO_NOT_DEALLOCATE')
    call memory_copy(memor,trim(my_comm_name)//' % BOUND_SIZE',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_size            , COMM_OUT % bound_size,          'DO_NOT_DEALLOCATE')
    call memory_copy(memor,trim(my_comm_name)//' % BOUND_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_perm            , COMM_OUT % bound_perm,          'DO_NOT_DEALLOCATE')

    if( present(memor_opt) ) memor_opt = memor

  end subroutine PAR_COPY_COMMUNICATION_ARRAY_BASIC
  
  subroutine PAR_COPY_COMMUNICATION_ARRAY_s(COMM_IN,COMM_OUT,memor_opt,MINIMUM_DATA,COMM_NAME)
    ...
    if( present(MINIMUM_DATA) ) then
       if_minimum_data = MINIMUM_DATA
    else
       if_minimum_data = .false.
    end if
    if( present(COMM_NAME) ) then
       my_comm_name = trim(COMM_NAME)
    else
    ...
    call memory_copy(memor,trim(my_comm_name)//' % NEIGHTS',             'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % neights               , COMM_OUT % neights,             'DO_NOT_DEALLOCATE')
    call memory_copy(memor,trim(my_comm_name)//' % NEIGHTS_ORDERED',     'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % neights_ordered       , COMM_OUT % neights_ordered,     'DO_NOT_DEALLOCATE')
    call memory_copy(memor,trim(my_comm_name)//' % PERM_ORDERED',        'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % perm_ordered          , COMM_OUT % perm_ordered,        'DO_NOT_DEALLOCATE')

    call memory_copy(memor,trim(my_comm_name)//' % BOUND_SIZE',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_size            , COMM_OUT % bound_size,          'DO_NOT_DEALLOCATE')
    call memory_copy(memor,trim(my_comm_name)//' % BOUND_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_perm            , COMM_OUT % bound_perm,          'DO_NOT_DEALLOCATE')
    call memory_copy(memor,trim(my_comm_name)//' % BOUND_INVP',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_invp            , COMM_OUT % bound_invp,          'DO_NOT_DEALLOCATE')

    if( .not. if_minimum_data ) then 
       call memory_copy(memor,trim(my_comm_name)//' % BOUND_SCAL',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_scal            , COMM_OUT % bound_scal,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BOUND_MULTIPLICITY',  'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_multiplicity    , COMM_OUT % bound_multiplicity,  'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BOUND_OWNER_RANK',    'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bound_owner_rank      , COMM_OUT % bound_owner_rank,    'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % NODE_NUMBER_IN_OWNER','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % node_number_in_owner  , COMM_OUT % node_number_in_owner,'DO_NOT_DEALLOCATE')
       ...
       if( associated(COMM_IN % bound_matrix) ) then
          ...
          do ii = 1,isize
             call memory_copy(memor,trim(my_comm_name)//' % BOUND_MATRIX % JA','PAR_COPY_COMMUNICATION_ARRAY',&
                  & COMM_IN % bound_matrix(ii) % ja,             COMM_OUT % bound_matrix(ii) % ja,              'DO_NOT_DEALLOCATE')
             call memory_copy(memor,trim(my_comm_name)//' % BOUND_MATRIX % NZDOM_II',        'PAR_COPY_COMMUNICATION_ARRAY',&
                  & COMM_IN % bound_matrix(ii) % nzdom_ii,       COMM_OUT % bound_matrix(ii) % nzdom_ii,        'DO_NOT_DEALLOCATE')
       ...
       if( associated(COMM_IN % bound_mat_halo_send) ) then
          ...
          do ii = 1,isize
             call memory_copy(memor,trim(my_comm_name)//' % BOUND_MAT_HALO_SEND % JA','PAR_COPY_COMMUNICATION_ARRAY',&
                  & COMM_IN % bound_mat_halo_send(ii) % ja,             COMM_OUT % bound_mat_halo_send(ii) % ja,              'DO_NOT_DEALLOCATE')
             call memory_copy(memor,trim(my_comm_name)//' % BOUND_MAT_HALO_SEND % NZDOM_II',        'PAR_COPY_COMMUNICATION_ARRAY',&
                  & COMM_IN % bound_mat_halo_send(ii) % nzdom_ii,       COMM_OUT % bound_mat_halo_send(ii) % nzdom_ii,        'DO_NOT_DEALLOCATE')
       ...
       if( associated(COMM_IN % bound_mat_halo_recv) ) then
          ...
          do ii = 1,isize
             call memory_copy(memor,trim(my_comm_name)//' % BOUND_MAT_HALO_RECV % JA','PAR_COPY_COMMUNICATION_ARRAY',&
                  & COMM_IN % bound_mat_halo_recv(ii) % ja,             COMM_OUT % bound_mat_halo_recv(ii) % ja,              'DO_NOT_DEALLOCATE')
             call memory_copy(memor,trim(my_comm_name)//' % BOUND_MAT_HALO_RECV % NZDOM_II',        'PAR_COPY_COMMUNICATION_ARRAY',&
                  & COMM_IN % bound_mat_halo_recv(ii) % nzdom_ii,       COMM_OUT % bound_mat_halo_recv(ii) % nzdom_ii,        'DO_NOT_DEALLOCATE')
             COMM_OUT % bound_mat_halo_recv(ii) % nzdom  =  COMM_IN % bound_mat_halo_recv(ii) % nzdom
          end do
       end if

       call memory_copy(memor,trim(my_comm_name)//' % BEDGE_SIZE'          ,'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bedge_size            , COMM_OUT % bedge_size,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BEDGE_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bedge_perm            , COMM_OUT % bedge_perm,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BEDGE_ADJA',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bedge_adja            , COMM_OUT % bedge_adja,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BEDGE_SCAL',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bedge_scal            , COMM_OUT % bedge_scal,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BEDGE_MULTIPLICITY',  'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bedge_multiplicity    , COMM_OUT % bedge_multiplicity,  'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BEDGE_OWNER_RANK',    'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bedge_owner_rank      , COMM_OUT % bedge_owner_rank,    'DO_NOT_DEALLOCATE')

       call memory_copy(memor,trim(my_comm_name)//' % LSEND_SIZE',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % lsend_size            , COMM_OUT % lsend_size,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % LRECV_SIZE',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % lrecv_size            , COMM_OUT % lrecv_size,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % LSEND_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % lsend_perm            , COMM_OUT % lsend_perm,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % LRECV_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % lrecv_perm            , COMM_OUT % lrecv_perm,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % LSCAT_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % lscat_perm            , COMM_OUT % lscat_perm,          'DO_NOT_DEALLOCATE')

       call memory_copy(memor,trim(my_comm_name)//' % MATRIX_IA',           'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % matrix_ia             , COMM_OUT % matrix_ia,           'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % MATRIX_JA',           'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % matrix_ja             , COMM_OUT % matrix_ja,           'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % MATRIX_AA',           'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % matrix_aa             , COMM_OUT % matrix_aa,           'DO_NOT_DEALLOCATE')

       call memory_copy(memor,trim(my_comm_name)//' % FULL_ROW_SEND_NEIGHTS','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % full_row_send_neights,COMM_OUT % full_row_send_neights,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % FULL_ROW_SEND_SIZE'   ,'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % full_row_send_size   ,COMM_OUT % full_row_send_size   ,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % FULL_ROW_SEND_PERM'   ,'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % full_row_send_perm   ,COMM_OUT % full_row_send_perm   ,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % FULL_ROW_RECV_NEIGHTS','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % full_row_recv_neights,COMM_OUT % full_row_recv_neights,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % FULL_ROW_RECV_SIZE'   ,'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % full_row_recv_size   ,COMM_OUT % full_row_recv_size   ,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % FULL_ROW_RECV_PERM'   ,'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % full_row_recv_perm   ,COMM_OUT % full_row_recv_perm   ,'DO_NOT_DEALLOCATE')

       call memory_copy(memor,trim(my_comm_name)//' % BFACE_SIZE',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bface_size            , COMM_OUT % bface_size,          'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % BFACE_PERM',          'PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % bface_perm            , COMM_OUT % bface_perm,          'DO_NOT_DEALLOCATE')

       call memory_copy(memor,trim(my_comm_name)//' % GHOST_SEND_ELEM_SIZE','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_send_elem_size  , COMM_OUT % ghost_send_elem_size,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_SEND_ELEM_PERM','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_send_elem_perm  , COMM_OUT % ghost_send_elem_perm,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_RECV_ELEM_SIZE','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_recv_elem_size  , COMM_OUT % ghost_recv_elem_size,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_RECV_ELEM_PERM','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_recv_elem_perm  , COMM_OUT % ghost_recv_elem_perm,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_SEND_NODE_SIZE','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_send_node_size  , COMM_OUT % ghost_send_node_size,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_SEND_NODE_PERM','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_send_node_perm  , COMM_OUT % ghost_send_node_perm,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_RECV_NODE_SIZE','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_recv_node_size  , COMM_OUT % ghost_recv_node_size,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_RECV_NODE_PERM','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_recv_node_perm  , COMM_OUT % ghost_recv_node_perm,'DO_NOT_DEALLOCATE')

       call memory_copy(memor,trim(my_comm_name)//' % GHOST_SEND_BOUN_SIZE','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_send_boun_size  , COMM_OUT % ghost_send_boun_size,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_SEND_BOUN_PERM','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_send_boun_perm  , COMM_OUT % ghost_send_boun_perm,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_RECV_BOUN_SIZE','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_recv_boun_size  , COMM_OUT % ghost_recv_boun_size,'DO_NOT_DEALLOCATE')
       call memory_copy(memor,trim(my_comm_name)//' % GHOST_RECV_BOUN_PERM','PAR_COPY_COMMUNICATION_ARRAY',COMM_IN % ghost_recv_boun_perm  , COMM_OUT % ghost_recv_boun_perm,'DO_NOT_DEALLOCATE')
