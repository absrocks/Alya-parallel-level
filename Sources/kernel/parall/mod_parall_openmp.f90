...
contains
  ...
  subroutine parall_openmp_output_ompss_domain(ompss_domains,meshe)
    ...
    integer(ip)                               :: NUM_NEIGHBOURS, num_nodos, nodeSize, numNodesMarket
    integer(ip)                               :: NUM_ELEMENTS, idx1, idx2, I, J, ielem, pelty, pnode, inode, ipoin
    real(rp)                                  :: x,y,z
  ...
  subroutine parall_openmp_coloring(meshe,mepoi_opt,pelpo_opt,lelpo_opt,ON_BOUNDARIES)
    ...
    if( INOTMASTER ) then
       if( par_omp_num_threads /= 0 .and. nenti /= 0 ) then
          ...
          !
          if( if_on_boundaries ) then
             call graphs_coloring_deallocate(par_omp_nboun_list_colors)
          else
             call graphs_coloring_deallocate(par_omp_list_colors)
          end if

          call graphs_eleele_deallocate(PELEL=pelel,LELEL=lelel)
          
          if( .not. present(pelpo_opt) ) call graphs_eleele_deallocate(PELPO=pelpo)
          if( .not. present(lelpo_opt) ) call graphs_eleele_deallocate(LELPO=lelpo)
          !
          ! Output
          !
          if( if_on_boundaries ) then
