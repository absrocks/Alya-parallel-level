...
contains
  ...
  subroutine redistribution_initialization()
      ...
      call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_npoin)
      call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_nelem)
      call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_nboun)
      call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_nbopo)
  ...
  subroutine redistribution_copy_communicator(comm)
    ...
    call PAR_COPY_COMMUNICATION_ARRAY(commd_npoin,comm % commd_npoin)
    call PAR_COPY_COMMUNICATION_ARRAY(commd_nelem,comm % commd_nelem)
    call PAR_COPY_COMMUNICATION_ARRAY(commd_nboun,comm % commd_nboun)
    call PAR_COPY_COMMUNICATION_ARRAY(commd_nbopo,comm % commd_nbopo)
  ...
  subroutine generate_comm_elem(new_part,comm_elem, nenti_,COMM_NAME)
    ...
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_elem,par_memor,COMM_NAME=trim(my_comm_name))
  ...
  subroutine generate_comm_bopo(new_part,comm_bopo,nenti_,COMM_NAME)
    ...
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_bopo,par_memor,COMM_NAME=trim(my_comm_name))
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm_bopo)
    !
    ! Nullify
    !
    nullify(lelems)
  ...
  subroutine generate_comm_poin(new_part,comm_poin,nenti_,COMM_NAME)
    ...
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_poin,par_memor,COMM_NAME=trim(my_comm_name))
  ...
  subroutine generate_comm_boun(new_part,comm_boun,nenti_,COMM_NAME)
    ...
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_boun,par_memor,COMM_NAME=trim(my_comm_name))
...
!!$
  ...
  subroutine redistribution_domain(lpart,PAR_COMM_IN,VERBOSE)
    ...
    integer(ip)                                           :: PAR_COMM,ifiel,knode,knodb
    integer(4)                                            :: PAR_COMM4
    logical(lg)                                           :: lfoun
  ...
  subroutine redistribution_array_IP_1(xx,wtype,posit,memor,variable_name,COMM,PERMUTE_RECV,OUTPUT_ARRAY,VERBOSE)
   ...
   if( present(VERBOSE) ) then
    ...
    if( present(COMM) ) then
       ...
       call PAR_MAX(nsize,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
    end if
    if( nsize == 0 ) return
    !
  ...
  subroutine redistribution_array_IP_2(xx,wtype,posit,memor,variable_name,COMM,PERMUTE_RECV,OUTPUT_ARRAY)
    ...
    if( present(COMM) ) then
       ...
       call PAR_MAX(nsize,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
       call PAR_MAX(ndim1,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
    end if
    if( nsize == 0 ) return
    !
    ! Redistribute and permute if required
  ...
  subroutine redistribution_array_RP_3(xx,wtype,posit,memor,variable_name,COMM,PERMUTE_RECV,OUTPUT_ARRAY,EXPLODE_DIM,LAST_COMPONENT)
    ...
    integer(ip)                                            :: ienti,jenti,nenti_send,my_posit
    integer(ip)                                            :: ndime_send,ndime_recv
    integer(ip)                                            :: nenti_recv,nsize,icomp
    logical(lg)                                            :: if_permute_recv
    real(rp),                      pointer                 :: xsend(:,:,:)
    real(rp),                      pointer                 :: xrecv(:,:,:)
    real(rp),                      pointer                 :: xx_out(:,:,:)
    ...
    if( present(COMM) ) then
       ...
       call PAR_MAX(nsize,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
       call PAR_MAX(ndim1,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
       call PAR_MAX(ndim2,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
    end if
    if( nsize == 0 ) return
    !
    ! Dimensions
  ...
  subroutine redistribution_array_RP_4(xx,wtype,posit,memor,variable_name,COMM,PERMUTE_RECV,OUTPUT_ARRAY)
    ...
    if( present(COMM) ) then
       ...
       call PAR_MAX(ndim1,INCLUDE_ROOT=.true.,PAR_COMM_IN4=int(COMM % PAR_COMM_WORLD,4))
       call PAR_MAX(ndim2,INCLUDE_ROOT=.true.,PAR_COMM_IN4=int(COMM % PAR_COMM_WORLD,4))
       call PAR_MAX(ndim3,INCLUDE_ROOT=.true.,PAR_COMM_IN4=int(COMM % PAR_COMM_WORLD,4))
    else
       call PAR_MAX(nsize,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
       call PAR_MAX(ndim1,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
       call PAR_MAX(ndim2,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
       call PAR_MAX(ndim3,INCLUDE_ROOT=.true.,PAR_COMM_IN4=PAR_COMM_RED4)
    end if
    if( nsize == 0 ) return
    !
    ! Redistribute and permute if required
  ...
  subroutine generate_comm_chunkdist_a(reqs,chunk,PAR_COMM4,comm,dist_dom)
    ...
    integer(4),                    intent(in)    :: PAR_COMM4  !< Communicator
