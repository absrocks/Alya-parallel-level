...
module mod_par_tools
  ...
  use def_domain,         only : npoin_own,npoin
...
contains
  ...
  subroutine par_tools_ownership_interfaces(npoin_new,comm,permr)
    ...
    nullify(node_type)
    nullify(size_inte)
    call memory_alloca(par_memor,'NODE_MULT','par_interface_exchange',node_mult,npoin_new)
    call memory_alloca(par_memor,'NODE_TYPE','par_interface_exchange',node_type,npoin_new)
    call memory_alloca(par_memor,'SIZE_INTE','par_interface_exchange',size_inte,comm % nneig)
    if( .not. associated(permr) ) then
       call memory_alloca(par_memor,'PERMR','par_interface_exchange',permr,npoin_new)
    end if
    !
