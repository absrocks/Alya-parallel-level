...
module mod_communications_point_to_point
  ...
  public :: PAR_COMM_NULL4                     ! NULL MPI communicator
  public :: PAR_MERGE_COMMUNICATION_ARRAY      ! Merge communicator in case of multiply defined neighbors
...
contains
  ...
  subroutine PAR_INTERFACE_NODE_EXCHANGE_IP(onwhat,ndofn,xx,what,commu,PAR_COMM_TO_USE,wsynch,dom_k,EXCLUDE_MYSELF)
    ...
    integer(ip),          optional, intent(in)    :: dom_k
    logical(lg),          optional, intent(in)    :: EXCLUDE_MYSELF
    integer(ip)                                   :: ii,nsize,jj,dom_i,kdofn,dom_min
  ...
  subroutine PAR_INTERFACE_NODE_EXCHANGE_RP_1(xx,what,wherein,wsynch,dom_k,COMMU_IN)
    implicit none
    ...
    character(*),         optional, intent(in)    :: wsynch
    integer(ip),          optional, intent(in)    :: dom_k
    type(comm_data_par),  optional, intent(in)    :: COMMU_IN
    integer(ip)                                   :: ndofn
    integer(4)                                    :: PAR_COMM_TO_USE
    type(comm_data_par),               pointer    :: commu
    ...
    if( present(COMMU_IN) ) then
       PAR_COMM_TO_USE = COMMU_IN % PAR_COMM_WORLD
       call PAR_INTERFACE_NODE_EXCHANGE_RP(ON_NODES,ndofn,xx,what,COMMU_IN,PAR_COMM_TO_USE,wsynch,dom_k)  
    else
       if( present(wherein) ) then
          call PAR_DEFINE_COMMUNICATOR(wherein,PAR_COMM_TO_USE,commu)
       else
       ...
       if( ISLAVE ) then
                ...
                if( dom_j == 0 .or. dom_j == dom_i ) then
                   ...
                   if( asynch ) then
                      ...
                      call MPI_Sendrecv(                       &
                           ...
                           MPI_DOUBLE_PRECISION, dom_i4, 0_4,  &
                           recvbuff_rp(ini:), nsize4, MPI_DOUBLE_PRECISION, dom_i4, 0_4,&
                           PAR_COMM_TO_USE, status, istat4     )
                   end if
         ...
         if( ( asynch .and. ipass == 2 ) .or. ( .not. asynch .and. ipass == 1 ) ) then
             ...
             if(         trim(what) == 'SUM'  .or. trim(what) == 'ASSEMBLY' .or. trim(what) == 'SUMI'&
                ...
                ! TAKE MIN
                !
                call runend('MOD_COMMUNICATIONS: NOT CODED')
                call PAR_COMM_RANK_AND_SIZE(PAR_COMM_TO_USE,my_rank4)
                kk = 0
                dom_min = min(int(my_rank4,ip),minval(commu % neights))
                loop_ii: do ii = 1,commu % nneig
                      dom_i = commu % neights(ii)
  ...
  subroutine PAR_RECEIVE_RP_0(nrecv,xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4)
    ...
    if( nrecv > 0 ) then
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_RP(0_ip,nrecv,yy_send,xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_RP(0_ip,nrecv,yy_send,xx_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_RECEIVE_RP_0

  subroutine PAR_RECEIVE_RP_1(xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4)
    ...
    if( associated(xx_recv) ) then
       nsize = size(xx_recv)
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_RP(0_ip,nsize,yy_send,xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_RP(0_ip,nsize,yy_send,xx_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_RECEIVE_RP_1

  subroutine PAR_RECEIVE_RP_2(xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4)
    implicit none
    real(rp),              intent(inout), pointer :: xx_recv(:,:)
    character(*),          intent(in), optional :: wherein
    integer(ip),           intent(in), optional :: dom_i
  ...
  subroutine PAR_RECEIVE_IP_0(nrecv,xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4)
    implicit none
    integer(ip),           intent(in)           :: nrecv
    ...
    if( nrecv > 0 ) then
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_IP(0_ip,nrecv,yy_send,xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_IP(0_ip,nrecv,yy_send,xx_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_RECEIVE_IP_0

  subroutine PAR_RECEIVE_IP_1(xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4)
    implicit none
    ...
    if( associated(xx_recv) ) then
       nsize = size(xx_recv)
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_IP(0_ip,nsize,yy_send,xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_IP(0_ip,nsize,yy_send,xx_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_RECEIVE_IP_1

  subroutine PAR_RECEIVE_IP_2(xx_recv,wherein,dom_i,wsynch,PAR_COMM_IN4)
    implicit none
    integer(ip),           intent(inout), pointer :: xx_recv(:,:)
    character(*),          intent(in), optional :: wherein
    integer(ip),           intent(in), optional :: dom_i
  ...
  subroutine PAR_SEND_RP_0(nsend,xx_send,wherein,dom_i,wsynch,PAR_COMM_IN4)
    ...
    if( nsend > 0 ) then
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_RP(nsend,0_ip,xx_send,yy_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_RP(nsend,0_ip,xx_send,yy_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_SEND_RP_0
  
  subroutine PAR_SEND_RP_1(xx_send,wherein,dom_i,wsynch,PAR_COMM_IN4)
    ...
    if( associated(xx_send) ) then
       nsize = size(xx_send)
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_RP(nsize,0_ip,xx_send,yy_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_RP(nsize,0_ip,xx_send,yy_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_SEND_RP_1
  
  subroutine PAR_SEND_RP_2(xx_send,wherein,dom_i,wsynch,PAR_COMM_IN4)
  ...
  subroutine PAR_SEND_IP_0(nsend,xx_send,wherein,dom_i,wsynch,PAR_COMM_IN4)
    ...
    if( nsend > 0 ) then
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_IP(nsend,0_ip,xx_send,yy_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_IP(nsend,0_ip,xx_send,yy_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_SEND_IP_0
  
  subroutine PAR_SEND_IP_1(xx_send,wherein,dom_i,wsynch,PAR_COMM_IN4)
    ...
    if( associated(xx_send) ) then
       nsize = size(xx_send)
       if( present(wsynch) ) then
          call PAR_SEND_RECEIVE_IP(nsize,0_ip,xx_send,yy_recv,wherein,dom_i,wsynch,PAR_COMM_IN4=PAR_COMM_IN4)
       else
          call PAR_SEND_RECEIVE_IP(nsize,0_ip,xx_send,yy_recv,wherein,dom_i,PAR_COMM_IN4=PAR_COMM_IN4)
       end if
    end if
    
  end subroutine PAR_SEND_IP_1
  
  subroutine PAR_SEND_IP_2(xx_send,wherein,dom_i,wsynch,PAR_COMM_IN4)
  ...
  subroutine PAR_SEND_RECEIVE_TO_ALL_IP_1(xx_send,xx_recv,wherein,wsynch,lsend_perm,PERMUTE_SEND,PERMUTE_RECV)
    ...
    if( IPARALL ) then
       ...
       if( if_permute_send .and. commu % lsend_dim > 0 ) then
          call memory_alloca(par_memor,'xx_send_perm','par_send_receive_to_all_ip_1',xx_send_perm,max(1_ip,commu % lsend_dim))
          ...
          call memory_deallo(par_memor,'xx_send_perm','par_send_receive_to_all_ip_1',xx_send_perm)
       else
  ...
  subroutine PAR_SEND_RECEIVE_TO_ALL_RP_4(xx_send,xx_recv,commu,wsynch,PERMUTE_SEND)
    ...
    if( if_permute_send ) then
       ...
       if( alltoallv ) then
          sendcount4 = sendcount4 * ndofn
          recvcount4 = recvcount4 * ndofn
          call PAR_ALLTOALLV_RP_4(xx_send_perm,xx_recv,sendcount4,recvcount4,PAR_COMM_IN4=int(commu % PAR_COMM_WORLD,4))
       else
          if( .not. associated(xx_recv) ) then
             call PAR_SEND_RECEIVE_TO_ALL_RP(ndofn,xx_send_perm,yy_recv,commu,wsynch)
          else
       ...
       if( .not. associated(xx_send) .and. associated(xx_recv) ) then
          ...
          if( alltoallv ) then
             sendcount4 = sendcount4 * ndofn
             recvcount4 = recvcount4 * ndofn
             call PAR_ALLTOALLV_RP_4(xx_send,xx_recv,sendcount4,recvcount4,PAR_COMM_IN4=int(commu % PAR_COMM_WORLD,4))
          else
             call PAR_SEND_RECEIVE_TO_ALL_RP(ndofn,yy_send,xx_recv,commu,wsynch)
          end if
       else if( .not. associated(xx_send) .and. .not. associated(xx_recv) ) then
 ...
 !----------------------------------------------------------------------
          ...
          if( ipass == 1 ) then
             ...
             do ii = 1,commu % nneig
                ...
                if( dom_j == 0 .or. dom_j == dom_i ) then
                   ...
                   if( istat4 /= MPI_SUCCESS )call PAR_MPI_RUNEND(istat4,'PAR_ARRAY_EXCHANGE_IP')
                end if
    ...
    if( ISLAVE ) then
       ...
       do ii = 1,commu % nneig
          ...
          if( dom_j == 0 .or. dom_j == dom_i ) then
             ...
             if( istat4 /= MPI_SUCCESS )call PAR_MPI_RUNEND(istat4,'PAR_INTERFACE_NODE_EXCHANGE_VALUE')
  ...
  !> @author  houzeaux
  !> @date    2019-12-09
  !> @brief   Merge a communicator
  !> @details Merge a communicator in case a neighbor appears
  !>          more than once
  !> 
  !-----------------------------------------------------------------------

  subroutine PAR_MERGE_COMMUNICATION_ARRAY(COMM)
    
    type(comm_data_par), intent(inout) :: COMM            !< Input communicator
    type(comm_data_par)                :: COMM_CPY
    ...
    call memory_alloca(par_memor,'BOUND_SIZE','PAR_MERGE_COMMUNICATION_ARRAY',bound_size,COMM % nneig+1)
    call memory_alloca(par_memor,'BOUND_PERM','PAR_MERGE_COMMUNICATION_ARRAY',bound_perm,COMM % bound_dim)
    call memory_alloca(par_memor,'BOUND_INVP','PAR_MERGE_COMMUNICATION_ARRAY',bound_invp,COMM % bound_dim)
    call memory_alloca(par_memor,'NEIGHTS'   ,'PAR_MERGE_COMMUNICATION_ARRAY',neights   ,COMM % nneig)
    !
          ...
          do jneig = ineig,COMM % nneig
             ...
             if( dom_j == dom_i ) then
                ...
                if( bound_size(nneig_new) > 0 ) then
                   call memory_resize(par_memor,'BOUND_PERM % L','PAR_MERGE_COMMUNICATION_ARRAY',bound_perm(nneig_new) % l,bound_size(nneig_new))
                   call memory_resize(par_memor,'BOUND_INVP % L','PAR_MERGE_COMMUNICATION_ARRAY',bound_invp(nneig_new) % l,bound_size(nneig_new))
    ...
    call memory_deallo(par_memor,'BOUND_SIZE','PAR_MERGE_COMMUNICATION_ARRAY',COMM % bound_size)
    call memory_deallo(par_memor,'BOUND_PERM','PAR_MERGE_COMMUNICATION_ARRAY',COMM % neights   )
    call memory_alloca(par_memor,'BOUND_SIZE','PAR_MERGE_COMMUNICATION_ARRAY',COMM % bound_size,COMM % nneig+1)
    call memory_alloca(par_memor,'BOUND_PERM','PAR_MERGE_COMMUNICATION_ARRAY',COMM % neights   ,COMM % nneig)
    !
    ...
    call memory_deallo(par_memor,'BOUND_SIZE','PAR_MERGE_COMMUNICATION_ARRAY',bound_size)
    call memory_deallo(par_memor,'BOUND_PERM','PAR_MERGE_COMMUNICATION_ARRAY',bound_perm)
    call memory_deallo(par_memor,'BOUND_INVP','PAR_MERGE_COMMUNICATION_ARRAY',bound_invp)
    call memory_deallo(par_memor,'BOUND_PERM','PAR_MERGE_COMMUNICATION_ARRAY',neights)

  end subroutine PAR_MERGE_COMMUNICATION_ARRAY
