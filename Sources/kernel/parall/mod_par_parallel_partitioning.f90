...
contains
    ...
    if( INOTMASTER .and. ( &
       ...
       if(      kfl_partition_par == PAR_ZOLTAN  &
         & .or. kfl_partition_par == PAR_SFC     ) then
          call memory_alloca(par_memor,'RWEIG','par_parallel_partitioning',lweig,nelem)
  ...
  subroutine par_partition_scatter_from_mpio()
    ...
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm_npoin)
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm_nboun)
    ...
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_npoin_from_mpio)
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_nelem_from_mpio)
    call PAR_INITIALIZE_COMMUNICATION_ARRAY(commd_nboun_from_mpio)
    
    call PAR_COPY_COMMUNICATION_ARRAY(comm_npoin,commd_npoin_from_mpio,par_memor)
    !call PAR_COPY_COMMUNICATION_ARRAY(comm_nelem,commd_nelem_from_mpio,par_memor)
    call PAR_COPY_COMMUNICATION_ARRAY(comm_nboun,commd_nboun_from_mpio,par_memor)
    
    if( INOTMASTER ) then
       call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_npoin,par_memor,INITIALIZE=.true.)
       call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_nboun,par_memor,INITIALIZE=.true.)
...
subroutine par_partition_lelbo2(chunk_nelem,chunk_npoin,comm_npoin)
   ...
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm1,par_memor,INITIALIZE=.true.)
   call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm1)
   ...
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm0,par_memor,INITIALIZE=.true.)
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm1,par_memor,INITIALIZE=.true.)
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm2,par_memor,INITIALIZE=.true.)
