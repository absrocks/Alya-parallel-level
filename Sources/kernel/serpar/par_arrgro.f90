subroutine par_arrgro(inter)
  ...
  call memchk(zero,istat,par_memor,'LNIN2_PAR','par_memory',lnin2_par)
  allocate(invpI(dsize),stat=istat)
  call memchk(zero,istat,par_memor,'invpI','par_arrays',invpI)
  allocate(invpB(dsize),stat=istat)
  call memchk(zero,istat,par_memor,'invpB','par_arrays',invpB)
  !
  ...
  !
  call memchk(two,istat,par_memor,'INVPB','par_arrgro',invpb)
  deallocate(invpb,stat=istat)
  if(istat/=0) call memerr(two,'invpb','par_arrgro',0_ip)

  call memchk(two,istat,par_memor,'INVPI','par_arrgro',invpI)
  deallocate(invpI,stat=istat)
  if(istat/=0) call memerr(two,'invpI','par_arrgro',0_ip)

  call memchk(two,istat,par_memor,'LNIN2_PAR','par_arrgro',lnin2_par)
