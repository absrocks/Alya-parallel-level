  ...
  if( IPARALL ) then
     ...
     if( party == NPOIN_TYPE ) then
        ...
        if( pardi == 1 .and. parki == 1 ) then
           ...
           call memchk(zero,istat,par_memor,'LOC_SPARI1','par_slonly',loc_spari1)

           allocate(loc_rpari1(commd % bound_dim),stat=istat)
           call memchk(zero,istat,par_memor,'LOC_RPARI1','par_slonly',loc_rpari1)
           ...
           call memchk(two,istat,par_memor,'LOC_RPARI1','par_slonly',loc_rpari1)
           deallocate(loc_rpari1,stat=istat)
           if(istat/=0) call memerr(two,'LOC_RPARI1','par_slonly',0_ip)

           call memchk(two,istat,par_memor,'LOC_SPARI1','par_slonly',loc_spari1)
