subroutine par_submsh()
...
#endif
  integer(ip)              :: iedgg,iedgb,ii,ineig,kk,ifacg,kboun
  integer(ip)              :: dom_i,bsize,kpoin,kface,ini,iboun
  ...
  real(rp)                 :: time5,time6
  ! Renumbering
  integer(ip)              :: dummi,isend(2)
  integer(ip)              :: npoi1_old,npoi2_old,npoi3_old,npoin_total_old
  ...
  if( IPARALL ) then
     ...
     if( INOTMASTER ) then
        ...
        if( nedgb > 0 ) then
           call memory_alloca(par_memor,'LEDGB_LOC','par_submsh',ledgb_loc,nedgb)
        ...
        if( nfacb > 0 ) then
           ...
           call memory_alloca(par_memor,'LFAC1',    'par_submsh',lfac1,    nfacb)
           call memory_alloca(par_memor,'LFAC2',    'par_submsh',lfac2,    nfacb)
           call memory_alloca(par_memor,'LFAC3',    'par_submsh',lfac3,    nfacb)
           call memory_alloca(par_memor,'LFAC4',    'par_submsh',lfac4,    nfacb)
        end if
        ...
        if( nfacb > 0 ) then
           call memory_deallo(par_memor,'LFAC4','par_submsh',lfac4)
           call memory_deallo(par_memor,'LFAC3','par_submsh',lfac3)
           call memory_deallo(par_memor,'LFAC2','par_submsh',lfac2)
           call memory_deallo(par_memor,'LFAC1','par_submsh',lfac1)
        end if
     ...
     !
     ! LBINV_LOC: element global and unique numbering
     !
     ...
     if( INOTMASTER ) then
        ...
        if( nedgb > 0 ) then
           call memory_deallo(par_memor,'LEDGB_LOC','par_submsh',ledgb_loc)
           call memory_deallo(par_memor,'LEDGB',    'par_submsh',ledgb)
        end if
