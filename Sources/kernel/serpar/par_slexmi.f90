subroutine par_slexmi()
  !-----------------------------------------------------------------------
  !****f* Parall/par_slexmi
  ! NAME
  !    par_slexmi
  ! DESCRIPTION
  !    This subroutine exchange arrays between slaves and takes the min
  ...
  complex(rp), pointer     :: loc_sparx1(:),   loc_rparx1(:)

  nullify(loc_spari1,loc_rpari1,loc_sparr1,loc_rparr1,loc_sparx1,loc_rparx1)
  ...
  if(kfl_paral>0) then
     ...
     if(party==1) then
        ...
        if(pardi==1.and.parki==1) then
           ...
           call memory_alloca(par_memor,'LOC_SPARI1','par_slexmi',loc_spari1,commd%bound_dim,'DO_NOT_INITIALIZE')
           call memory_alloca(par_memor,'LOC_RPARI1','par_slexmi',loc_rpari1,commd%bound_dim,'DO_NOT_INITIALIZE')
           ...
           call memory_deallo(par_memor,'LOC_RPARI1','par_slexmi',loc_rpari1)
           call memory_deallo(par_memor,'LOC_SPARI1','par_slexmi',loc_spari1)
           ...
           call memory_alloca(par_memor,'LOC_SPARR1','par_slexmi',loc_sparr1,commd%bound_dim,'DO_NOT_INITIALIZE')
           call memory_alloca(par_memor,'LOC_RPARR1','par_slexmi',loc_rparr1,commd%bound_dim,'DO_NOT_INITIALIZE')
...
10         continue 
           ...
           call memory_deallo(par_memor,'LOC_SPARR1','par_slexmi',loc_sparr1)
           call memory_deallo(par_memor,'LOC_RPARR1','par_slexmi',loc_rparr1)
           ...
           call memory_alloca(par_memor,'LOC_SPARR1','par_slexmi',loc_sparr1,pard1*commd%bound_dim,'DO_NOT_INITIALIZE')
           call memory_alloca(par_memor,'LOC_RPARR1','par_slexmi',loc_rparr1,pard1*commd%bound_dim,'DO_NOT_INITIALIZE')
           ...
           call memory_alloca(par_memor,'LOC_SPARX1','par_slexmi',loc_sparx1,commd%bound_dim,'DO_NOT_INITIALIZE')
           call memory_alloca(par_memor,'LOC_RPARX1','par_slexmi',loc_rparx1,commd%bound_dim,'DO_NOT_INITIALIZE')
           ...
           do jj = 1, commd%bound_dim
              ipoin = commd%bound_perm(jj)
              !parx1(ipoin) = min(parx1(ipoin), loc_rparx1(jj))
           enddo

           call memory_deallo(par_memor,'LOC_SPARX1','par_slexmi',loc_sparx1)
           call memory_deallo(par_memor,'LOC_RPARX1','par_slexmi',loc_rparx1)           
           ...
           call memory_alloca(par_memor,'LOC_SPARX1','par_slexmi',loc_sparx1,pard1*commd%bound_dim,'DO_NOT_INITIALIZE')
           call memory_alloca(par_memor,'LOC_RPARX1','par_slexmi',loc_rparx1,pard1*commd%bound_dim,'DO_NOT_INITIALIZE')
           ...
           do jj = 1,commd%bound_dim
              ...
              do ii = 1,pard1
                 ...
                 !parx1(poin+ii) = min(parx1(poin+ii), loc_rparx1(ji+ii))
        ...
        if( parki == 2 .and. pardi == 1 ) then
           ...
           do jj= 1, commd%bound_dim
              ...
              ibopo = lpoty(ipoin)
              if( ibopo > 0 ) &
                   parr1(ibopo) = min(parr1(ibopo), loc_rparr1(jj))
           enddo
           ...
           do jj= 1, commd%bound_dim
              ...
              if( ibopo > 0 ) then
                 do ii= 1, pard1
                    parr2(ii,ibopo) = min(parr2(ii,ibopo),loc_rparr1(pard1*(jj-1)+ii))
                 enddo
              end if
           enddo
           ...
           do jj= 1, commd%bound_dim
              ...
              if( ibopo > 0 ) then
                 do ii= 1, pard1
                    parr1(pard1*(ibopo-1)+ii) = min(parr1(pard1*(ibopo-1)+ii),loc_rparr1(pard1*(jj-1)+ii))
                 enddo
              end if
           enddo
