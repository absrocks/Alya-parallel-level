subroutine par_colgro(ndual_par)
  ...
  call memchk(zero,istat,par_memor,'comle(icoml)%colours','par_metis',comle(icoml)%colours)

  allocate(sortVec(ndual_par),stat=istat)
  !call memchk(zero,istat,par_memor,'sortVec','par_metis',sortVec)

  allocate(mask(0:maxColors),stat=istat)
  call memchk(zero,istat,par_memor,'mask','par_metis',mask)
  ...
  call memchk(two,istat,par_memor,'mask','par_colgro',mask)
  deallocate(mask,stat=istat)
  if(istat/=0) call memerr(two,'mask','par_colgro',0_ip)
  !call memchk(two,istat,par_memor,'sortVec','par_colgro',sortVec)
