...
module mod_alya2gmsh
  ...
  INTERFACE
     ...
     SUBROUTINE reMeshing(&
          &    ndime,                                             &
          &    nnodes, nodes_tags, nodes_coordinates,             &
          &    ntypes,types_tags, nelements_bytype,               & 
          &    elements_tags,elements_nodes,                      &
          &    min_size, max_size, mesh_size,                     &          
          &    new_nnodes, new_nodes_tags,new_nodes_coordinates,  &
          &    new_ntypes, new_types_tags, new_nelements_bytype,  &
          &    new_elements_tags,new_elements_nodes)              BIND (c,NAME='reMeshing')

       use, intrinsic ::  ISO_C_BINDING
       implicit none       
       integer(C_SIZE_T),     value             :: ndime
       
       integer(C_SIZE_T),     value             :: nnodes
       integer(C_SIZE_T),     dimension(*)      :: nodes_tags
       real   (C_DOUBLE),     dimension(*)      :: nodes_coordinates
       integer(C_SIZE_T),     value             :: ntypes
       integer(C_SIZE_T),     dimension(*)      :: types_tags
       integer(C_SIZE_T),     dimension(*)      :: nelements_bytype
       integer(C_SIZE_T),     dimension(*)      :: elements_tags
       integer(C_SIZE_T),     dimension(*)      :: elements_nodes
     ...
     SUBROUTINE freeArrays(&
          &    new_nodes_tags, new_nodes_coordinates, new_types_tags, &
          &    new_nelements_bytype, new_elements_tags, new_elements_nodes ) BIND (c,NAME='freeArrays')
...
contains
  ...
  subroutine alya2gmsh_alya_to_gmsh(mesh_alya,mesh_gmsh)
    ...
    do ipoin = 1,mesh_alya % npoin
       mesh_gmsh % nodes_tags8(ipoin) = ipoin !int(mesh_alya % lninv_loc(ipoin),KIND=8)
  ...
  subroutine alya2gmsh_remeshing(mesh_alya_new, mesh_alya, mesh_size, min_size, max_size)

    class(mesh_type_basic), intent(inout) :: mesh_alya_new
    class(mesh_type_basic), intent(in)    :: mesh_alya
    real(rp),      pointer, intent(in)    :: mesh_size(:)
    real(rp),               intent(in)    :: max_size, min_size

    type(gmsh_type)                       :: mesh_gmsh
    type(gmsh_type_c)                     :: mesh_gmsh_c

    if( mesh_alya % nelem == 0 ) then
       !
       ! Empty mesh
       !
       ...
       !
       call mesh_gmsh % init()
       call alya2gmsh_alya_to_gmsh(mesh_alya,mesh_gmsh)
       ...
       call reMeshing(&
            ...
            mesh_gmsh   % nnodes8,            &
            mesh_gmsh   % nodes_tags8,        &
            mesh_gmsh   % nodes_coordinates8, &
            mesh_gmsh   % ntypes8,            &
            mesh_gmsh   % types_tags8,        &
            mesh_gmsh   % nelements_bytype8,  &
            mesh_gmsh   % elements_tags8,     &
            mesh_gmsh   % elements_nodes8,    &
       ...
       !   
       call alya2gmsh_gmsh_c_to_gmsh(mesh_gmsh_c,mesh_gmsh)
       !
       ! Convert gmsh to Alya 
       !
       call mesh_alya_new % init()

       call alya2gmsh_gmsh_to_alya(mesh_gmsh,mesh_alya_new)
       
       !
       ! Deallocate gmsh_c
       !
       call mesh_gmsh_c % deallo_c()
       !call mesh_gmsh   % deallo()
    end if
