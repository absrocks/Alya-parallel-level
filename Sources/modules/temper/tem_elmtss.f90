subroutine tem_elmtss(&
  ...
  use def_master, only       :  kfl_coupl,chemical_heat,div_enthalpy_transport,&
       &                        ID_TEMPER,ID_CHEMIC
  ...
  if(kfl_coupl(ID_TEMPER,ID_CHEMIC)==1) then
     ...
     if (div_enthalpy_transport(ielem)%a(1,1,1) + chemical_heat(ielem)%a(1,1,1) /= 0.0_rp) then
        chtim = source_safet_tem*rocpT/(abs(div_enthalpy_transport(ielem)%a(1,1,1)+ chemical_heat(ielem)%a(1,1,1)))
     else
        chtim=1.0e12_rp
     endif
