...
#define DEF_VECT 1:VECTOR_DIM  
    ...
    subroutine tem_elmdir_fast(VECTOR_DIM,&
      ...
      integer(ip), intent(in)    :: list_elements_p(VECTOR_DIM)
      integer(ip)                :: inode,ipoin,jnode,knode,jpoin,ihang,ivect,ielem
      real(rp)                   :: adiag,xvalu
    ...
    subroutine tem_elmgat_fast(&
      ...
      use def_master, only     :  therm,turmu,advec,veloc_forw,velom
      ...
      integer(ip)              :: inode,ipoin,idime,itime,dummi,ivect
      real(rp)                 :: dummr(3)
    ...
    subroutine tem_rhocpdt_fast(VECTOR_DIM,pnode,pgaus,porde,lnods_loc,gpsha,gpvol,gpden,list_elements_p,dt_rho_cp_tem_loc)
      ...
      real(rp) :: eldtcprho(VECTOR_DIM,pnode), fact(VECTOR_DIM),d,T
...
#define DEF_VECT 1:VECTOR_DIM
    ...
    subroutine tem_elmpre_fast(&
      ...
      use def_kintyp,                  only : ip,rp
      use def_master,                  only : dpthe,cutim,kfl_coupl,vesgs,velom
      ...
      use def_temper,                  only : kfl_exacs_tem,kfl_regim_tem
      use def_temper,                  only : prtur_tem,sourc_tem,kfl_rhs_scal_tem
      use def_temper,                  only : kfl_ellen_tem,grtem_tem
      ...
      integer(ip)                :: idime,inode,igaus,itime,ipoin,mxdim,ivect,ielem
...
!@      use mod_tem_fast
      ...
      real(rp)    :: gpdtc(VECTOR_DIM)                                          ! Values at Gauss points
    ...
    !@ real(rp)    :: gpdiv(VECTOR_DIM,pgaus)                                    ! Divergence of convection
      ...
      integer(ip) :: k, jpoin , izsol ,jcolu 
