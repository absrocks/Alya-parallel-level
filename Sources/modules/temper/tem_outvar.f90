subroutine tem_outvar(ivari,imesh)
  ...
  integer(ip)             :: ipoin,iline,jpoin,icont,idime,ielem,pelty,pgaus
  real(rp)                :: dummr,rutim
  real(rp), pointer       :: aux(:) 
  ...
  case(22_ip)
     ...
     if( INOTMASTER ) then
        ...
        !call PAR_INTERFACE_NODE_EXCHANGE(gesca,'SUM','IN MY CODE')
           ...
           call memory_alloca(mem_modul(1:2,modul),'HEATF_TEM','tem_variational_heat_flux',heatf_tem,npoin)
           call memory_alloca(mem_modul(1:2,modul),'MASSB_TEM','tem_variational_heat_flux',massb_tem,npoin)
           
           call tem_boundary_heat_flux() ! loads variational heat flux

           gesca(1:npoin) = heatf_tem(1:npoin) 

           call memory_deallo(mem_modul(1:2,modul),'MASSB_TEM','tem_variational_heat_flux',massb_tem)
