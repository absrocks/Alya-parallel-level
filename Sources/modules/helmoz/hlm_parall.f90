  ...
  case (1_ip)     
     ...
     do parii = 1_ip,2_ip 
        ...
        if (parii == 1_ip) then
           ...
           call memchk(zero,istat,mem_modul(1:2,modul),'parin','hlm_parall',parin)
           allocate(parre(nparr),stat=istat)
           call memchk(zero,istat,mem_modul(1:2,modul),'parre','hlm_parall',parre)
           allocate(parcx(nparx),stat=istat)
           call memchk(zero,istat,mem_modul(1:2,modul),'parcx','hlm_parall',parcx)
           if (ISLAVE .or. kfl_ptask == 2_ip) call par_broadc()
        endif
     enddo

     if(IMASTER .and. kfl_ptask /= 2_ip) call par_broadc()
     call memchk(two,istat,mem_modul(1:2,modul),'parin','hlm_parall',parin)
     deallocate(parin,stat=istat)
     if (istat /= 0_ip) call memerr(two,'parin','hlm_parall',0_ip)
     call memchk(two,istat,mem_modul(1:2,modul),'parre','hlm_parall',parre)
     deallocate(parre,stat=istat)
     if (istat /= 0_ip) call memerr(two,'parre','hlm_parall',0_ip)     
     call memchk(two,istat,mem_modul(1:2,modul),'parcx','hlm_parall',parcx)
