subroutine hlm_elm_equs(pnode,pgaus,gprea,dgprea,gpvol,gprhs,gpsha,gpcar,elmat,elrhs)
	...
	do inode = 1,pnode
		...
		do jnode = 1,pnode
			elrhs(var+1) = elrhs(var+1) - gpvol(igaus) * dgprea(1,igaus) * &
						   gpsha(inode,igaus) * gpsha(jnode,igaus) * gprhs(1,jnode)
!						 + gprhs(4,jnode) * gpcar(1,jnode,igaus)) 
			elrhs(var+2) = elrhs(var+2) - gpvol(igaus) * dgprea(2,igaus) * &
						   gpsha(inode,igaus) * gpsha(jnode,igaus) * gprhs(2,jnode)
!						 + gprhs(4,jnode) * gpcar(2,jnode,igaus)) 
			elrhs(var+3) = elrhs(var+3) - gpvol(igaus) * dgprea(3,igaus) * &
						   gpsha(inode,igaus) * gpsha(jnode,igaus) * gprhs(3,jnode)
!						 + gprhs(4,jnode) * gpcar(3,jnode,igaus))   
			!elrhs(var+4) = elrhs(var+4) - gpvol(igaus) * gprea(igaus) * (&       !or -
						   !gpsha(inode,igaus) * (&
						   !gprhs(1,jnode) * gpcar(1,jnode,igaus)&
						 !+ gprhs(2,jnode) * gpcar(2,jnode,igaus)&
						 !+ gprhs(3,jnode) * gpcar(3,jnode,igaus))
			elrhs(var+4) = elrhs(var+4) - gpvol(igaus) * gpsha(jnode,igaus) * (&       !Paper version   !!! gprea originally !!!
						   dgprea(1,igaus) * gprhs(1,jnode) * gpcar(1,inode,igaus)&
						 + dgprea(2,igaus) * gprhs(2,jnode) * gpcar(2,inode,igaus)&
						 + dgprea(3,igaus) * gprhs(3,jnode) * gpcar(3,inode,igaus))
!						 - gprhs(4,jnode) * (&
	...
	do jnode = 1,pnode
		...
		do inode = 1,jnode-1
			...
			elmat(ivar+4,jvar+1) = elmat(ivar+4,jvar+1) + gpvol(igaus) * &
				gprea(1,igaus) * gpsha(jnode,igaus) * gpcar(1,inode,igaus)
!			if (abs(elmat(ivar+4,jvar+1)) > 0.0_rp .and. abs(elmat(ivar+4,jvar+1)) < 1e-8) write(*,*) '1: ', gprea(1,igaus), gpvol(igaus), gpcar(1,jnode,igaus)
		end do
		do inode = 1,jnode-1
			ivar = 4_ip * (inode - 1_ip)
			...
			elmat(ivar+4,jvar+2) = elmat(ivar+4,jvar+2) + gpvol(igaus) * &
				gprea(2,igaus) * gpsha(jnode,igaus) * gpcar(2,inode,igaus)
		end do
		do inode = 1,jnode-1
			ivar = 4_ip * (inode - 1_ip)
			...
			elmat(ivar+4,jvar+3) = elmat(ivar+4,jvar+3) + gpvol(igaus) * &
				gprea(3,igaus) * gpsha(jnode,igaus) * gpcar(3,inode,igaus)
		end do
		do inode = 1,jnode-1
			ivar = 4_ip * (inode - 1_ip)
			elmat(ivar+1,jvar+4) = elmat(ivar+1,jvar+4) + gpvol(igaus) * &
				gprea(1,igaus) * gpsha(inode,igaus) * gpcar(1,jnode,igaus)
			elmat(ivar+2,jvar+4) = elmat(ivar+2,jvar+4) + gpvol(igaus) * &
				gprea(2,igaus) * gpsha(inode,igaus) * gpcar(2,jnode,igaus)
			elmat(ivar+3,jvar+4) = elmat(ivar+3,jvar+4) + gpvol(igaus) * &
