...
subroutine sld_reaphy()
  ...
  use mod_exm_sld_eccoupling,    only : exm_sld_ecc_read_data
  use mod_sld_fibers,            only : sld_fib_read_data, sld_fib_read_data_warning
  ...
  real(rp)        :: young,poiss,lame1,lame2,K0,E
  character(300)  :: messa
  ...
  if( INOTSLAVE ) then
     ...
     kfl_invel_sld = 0_ip                     ! Initial condition: velocity
        ...
        if (      words(1) == 'PROBL' ) then
           ...
           do while(words(1)/='ENDPR')
              ...
              if(       words(1) == 'MATER' ) then
                 ...
                 !
                 ! Cohesive zone model using interface elements
                 !
                 ...
                 ! ADOC[d]> GIc, GIIc, tauI, Kp, eta, rho</li>
                 ! ADOC[d]> <li> TURON, CURRENT:
                 ! ADOC[d]> GIc, GIIc, tauI, tauII, eta, Kp, rho</li>
                 ...
                 !
                 !
                 ! ADOC[2]> CONTACT_MODEL:                                          $ Interface contact model
                 ! ADOC[d]> CONTACT_MODEL:
                 ! ADOC[d]> This option is to define contact elements between instances. The available contact methods are the following
                 ! ADOC[d]> <ul>
                 ! ADOC[d]> <li> PENALTY (Penalty method used by Turon et al. 2018):
                 ! ADOC[d]> Kn, Kres</li>
                 ! ADOC[d]> Calculation method of the Tangent Stiffness matrix. This option can be used only for Implicit analysis.
                 ! ADOC[d]> By default, the tangent stiffness matrix is calculated analytically.
                 ! ADOC[d]> <ul>
                 ! ADOC[d]> <li> Set to ANALYTICAL to use the analytical calculation of the stiffness matrix. </li>
                 ! ADOC[d]> <li> Set to NUMERICAL  to calculate it numerically. </li>
                 ! ADOC[d]> <li> Set to SECANT to use the secant method. </li>
                 ! ADOC[d]> </ul>
                 ! ADOC[d]> Corresponds to contact law 800.
                 ! ADOC[d]> </ul>
                 !
                 kfl_sdvar_sld = 1_ip
                 nposi = 2_ip ! Auxiliar variable to indicate at which position the parameters start
                 ...
                 ! Thermal properties
                 !
                 ! ADOC[2]> THERMAL_PROP: THFIN=real, DELTT=real
                 ...
                 ! Bio-Fiber model
                 ! ADOC[2]> FIBER_MODEL
                 ! ADOC[d]>    LOCATION: ELEMENTAL | NODAL 
                 ! ADOC[d]>    FIBER:    int  $ ID_FIELD with the vectors defining the direction of the fibers (MANDATORY)
                 ! ADOC[d]>    SHEET:    int  $ ID_FIELD with the vectors defining the sheet direction (OPTIONAL)
                 ! ADOC[d]>    NORMA:    int  $ ID_FIELD with the vectors defining the normal direction (OPTIONAL)
                 ! ADOC[d]> END_FIBER_MODEL
                 if( exists('FIELD') ) call sld_fib_read_data_warning()
                 call sld_fib_read_data(imate)

              else if ( words(1) == 'ORTHO' ) then
                 call sld_fib_read_data_warning()
                 ...
                 !
                  call exm_sld_ecc_read_data(1_ip,imate)
