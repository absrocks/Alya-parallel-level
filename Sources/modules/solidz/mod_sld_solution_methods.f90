...
contains
  ...
  subroutine sld_explicit_tw()
    ...
    real(rp), pointer :: bvesc(:)      !< BC essential (contact)
    ...
    if ( INOTMASTER ) then
       ...
       call memory_alloca(mem_modul(1:2,modul),'BVESC',   'mod_sld_solution_methods',bvesc,   ndime*npoin)
    ...
    if ( INOTMASTER ) then
       ...
       do ipoin = 1,npoin
          ...
          do idime = 1,ndime
             ...
             !
             if( kfl_fixno_sld(idime,ipoin) == 1_ip ) accel_sld(idime,ipoin,ITER_K) = 0.0_rp
             if( kfl_fixno_sld(idime,ipoin) == 1_ip ) veloc_sld(idime,ipoin,ITER_K) = 0.0_rp
             !
             ! Other updates
             ! - Update displacement increment
       ...
       call memory_deallo(mem_modul(1:2,modul),'BVESC',   'mod_sld_solution_methods',bvesc)
