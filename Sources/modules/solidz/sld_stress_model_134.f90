...
!> @details 20/04/2020 : Adrià Quintanas-Corominas and Alfonso Santiago move the activa part
...
subroutine sld_stress_model_134( &
    ielem,pgaus,pmate,pnode,lnods,flagt,gpsha,gpdet,gpgdi,gpcau,&
    gpfibe,gpstr,gpdds)
    ! -----------------------------------
    use def_kintyp,                only :  ip, rp
    use def_solidz,                only :  parco_sld, modfi_sld, kfl_fiber_sld
    use mod_sld_fibers
    ! -----------------------------------
    implicit none
    ! -----------------------------------
    ...
    real(rp),    intent(out)            :: gpfibe(3,pgaus)
    real(rp),    intent(out)            :: gpstr(3,3,pgaus)
    real(rp),    intent(out)            :: gpdds(3,3,3,3,pgaus)
    ! -----------------------------------
    integer(ip)                         :: ielem, igaus, idime, jdime, kdime, ldime
    ...
    real(rp), dimension(3,pgaus)        :: nfibe0, norma0, nshet0
    real(rp), dimension(3,pgaus)        :: nfibe, nshet, norma
    ...
    ! -----------------------------------
    ! Fiber: gather from element value
    call sld_fib_get_normalized_fiber_directions_at_gauss_points(& 
    &   ielem, pnode, pgaus, lnods, gpsha, nfibe0, nshet0, norma0 )
    ...
    do igaus = 1, pgaus
        ...
        i4f  =  MULT_VxMxV(nfibe0(:,igaus),3_ip,gpcau(:,:,igaus),nfibe0(:,igaus),3_ip)
        i4s  =  MULT_VxMxV(nshet0(:,igaus),3_ip,gpcau(:,:,igaus),nshet0(:,igaus),3_ip)
        i4n  =  MULT_VxMxV(norma0(:,igaus),3_ip,gpcau(:,:,igaus),norma0(:,igaus),3_ip)
        i8fs = (MULT_VxMxV(nfibe0(:,igaus),3_ip,gpcau(:,:,igaus),nshet0(:,igaus),3_ip) + &
        &       MULT_VxMxV(nshet0(:,igaus),3_ip,gpcau(:,:,igaus),nfibe0(:,igaus),3_ip))*0.5_rp

        ! Push forward of f=F'*f0 ; s=F'*s0 ; n=F'*n0
        nfibe(:,igaus) = MULT_MxV(gpgdi(:,:,igaus),3_ip,3_ip,nfibe0(:,igaus),3_ip)
        nshet(:,igaus) = MULT_MxV(gpgdi(:,:,igaus),3_ip,3_ip,nshet0(:,igaus),3_ip)
        norma(:,igaus) = MULT_MxV(gpgdi(:,:,igaus),3_ip,3_ip,norma0(:,igaus),3_ip)

        ! Set the fiber direction into a master variable
        gpfibe(:,igaus) = nfibe(:,igaus)
        ...
        fxf_0(:,:) =  OUTPROD_VxV(nfibe0(:,igaus),3_ip,nfibe0(:,igaus),3_ip)
        sxs_0(:,:) =  OUTPROD_VxV(nshet0(:,igaus),3_ip,nshet0(:,igaus),3_ip)
        fxs_0(:,:) = (OUTPROD_VxV(nfibe0(:,igaus),3_ip,nshet0(:,igaus),3_ip) + &
        &             OUTPROD_VxV(nshet0(:,igaus),3_ip,nfibe0(:,igaus),3_ip))*0.5_rp
