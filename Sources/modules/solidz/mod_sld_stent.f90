...
contains
  ...
  subroutine sld_calculation_contact_stent(ipoin,ibopo,ifunc,Rfunc)
    ...
    use def_master, only : displ,rhsid
    use def_master, only : TIME_N, ITER_K, cutim, kfl_paral, ittim
    use def_domain, only : ndime, coord, kfl_codno
    use def_solidz, only : kfl_fixno_sld,contact_friction_sld
    use def_solidz, only : jacrot_du_dq_sld, jacrot_dq_du_sld,saved_rhsid,fcont_sld
    ...
    real(rp)            :: fcont_local(ndime),fcont_local_norm,fcont_tange(ndime),friction_force
...
!!$    !
  ...
  subroutine sld_local_basis_stent(ipoin,ibopo,iroty,rotma)
     ...
     use def_solidz,      only : kfl_local_sld, kfl_fixrs_sld
  ...
  subroutine sld_nodes_release_stent()
    ...
    if ( kfl_goite_sld /= 1_ip ) then
      ...
      if (new_to_release == 1_ip) then
        new_to_release = 0_ip
        call sld_release_nodes_stent()    !EVA: creo que para Ãsto no hace falta funcion
  ...
  subroutine sld_calc_reaction_stent()
    ...
    use def_solidz, only : release_nodes, new_to_release, contactbou_sld, kfl_funno_sld, saved_rhsid
    use def_solidz, only : fcont_sld, frxid_sld
    use def_solidz, only :  kfl_goite_sld, kfl_fixno_sld
    use def_solidz, only : jacrot_du_dq_sld, jacrot_dq_du_sld

    implicit none

    integer(ip)           :: ipoin, itott, ibopo
    real(rp)              :: dummy_matrix(ndime,ndime), fcont(ndime*npoin)
    ...
    if ( INOTMASTER ) then
       ...
       if (cutim > 1.0_rp .and. kfl_fixno_sld(1,ipoin) == 3_ip) then
        ...
        !  if (any(kfl_codno(:,ipoin) == contactbou_sld) ) then
            ...
            fcont_sld(itott+1:itott+ndime) = frxid_sld(itott+1:itott+ndime)   
        ...
        !    end if
            ...
            !
            call sld_rotsys(2_ip,1_ip,1_ip,ndime,ndime,dummy_matrix,fcont_sld(itott+1:itott+ndime), &
                 jacrot_du_dq_sld(1,1,ipoin),jacrot_dq_du_sld(1,1,ipoin))
        !  end if
       end if
  ...
  subroutine sld_save_rhsid_stent()
    ...
    use def_master, only : momod, modul, soltyp, kfl_paral
    use def_domain, only : kfl_codno, npoin, npoin_own, ndime, lpoty, coord
    use def_solidz, only : conbo_sld, saved_rhsid, contactbou_sld, contact_friction_sld
    use def_solidz, only : jacrot_du_dq_sld, jacrot_dq_du_sld, fcont_sld, kfl_fixno_sld
...
! In this case Rfunc is only the displacement in vertical direction: x and z position do not vary. Always in global coordinates
    ...
    use def_master, only : displ,rhsid
    use def_master, only : TIME_N, cutim, kfl_paral
