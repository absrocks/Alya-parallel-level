subroutine sld_stress_model_103b(pgaus,pmate,gpgdi,gpstr,gpdet,flagt,gpdds,gpmof)
  !-----------------------------------------------------------------------
  !****f* Solidz/sld_elmcla
  ! NAME
  ...
  real(rp)                   :: cstc1,cstc2,cstd1,s0,s1


  ! Mooney Rivlin's law (not implemented here)
  ! W(C) = C_1*(I_1-3) + C_2*(I_2-3) + 
  ...
  do igaus=1,pgaus
     ...
     do idime=1,ndime
        do jdime=1,ndime
           gpstr(idime,jdime,igaus) = (cstc1 + cstc2*i1)*Iden(idime,jdime) - cstc2*gpcau(idime,jdime,igaus)                                    !+( RUTH)
     ...
     if (flagt == 1_ip) then
       ...
       do i=1,ndime
         do j=1,ndime
           do k=1,ndime
             do l=1,ndime  !RUTH
               gpdds(i,j,k,l,igaus) = 2.0_rp*cstd1*(tkron(i,k)*tkron(j,l)+tkron(i,l)*tkron(j,k))&
                                            + 2.0_rp*cstc2*gpdet(igaus)*((2.0_rp*gpdet(igaus)-1.0_rp)&
...
end subroutine sld_stress_model_103b
