     ...
     if ( lbset(iboun) == ibsec ) then
        ...
        if ( pelty > 0 ) then
           ...
           gauss_points: do igaub = 1,pgaub
              ...
              if( any(postp(1) % npp_setsb(17:18) /= 0_ip) ) then
        ...
        if( kfl_conta_sld /= 0_ip ) then
           ...
           do ipoin = 1,npoin
              ...
              if( kfl_fixno_sld(1,ipoin) == 3_ip ) then
                 idofn = (ipoin-1)*ndime
                 setcf(1:ndime) = setcf(1:ndime) + fcont_sld(idofn+1:idofn+ndime) / multiplicity_loc
              end if
           end do
           ! FCONT
