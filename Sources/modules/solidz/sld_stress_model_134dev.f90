...
!> @} 
!-----------------------------------------------------------------------
subroutine sld_stress_model_134dev(&
  ...
  real(rp)                   :: fporf(ndime,ndime), fporf_0(ndime,ndime), sporf_0(ndime,ndime), fpors_0(ndime,ndime),&
    ...
    !I_1=Trace(C)
     ...
     do idime = 1,ndime
        do jdime = 1,ndime
           ...
           fpors_0(idime,jdime) = nfibe0(idime,igaus) * nshet0(jdime,igaus)
           sporf_0(idime,jdime) = nshet0(idime,igaus) * nfibe(idime,igaus)
        end do
     end do
  ...
  !      print*, ' '
  !      print*, 'INVARIANTS: I1, I4F, I4S', i1, i4f, i4s
        
        !term1 = (a/gpdet(igaus))*exp(b*(i1-3.0_rp))-(a/gpdet(igaus))
        !term1 = (a/gpdet(igaus))*exp(b*(i1-3.0_rp))
     ...
     do idime = 1,ndime
       ...
       do jdime = 1,ndime
         ...
         !cauchy_iso(idime,jdime) = term1*gpcal(idime,jdime,igaus)      
...
end subroutine sld_stress_model_134dev
