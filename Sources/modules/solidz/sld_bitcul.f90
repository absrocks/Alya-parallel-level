subroutine sld_bitcul(&
  ...
  use mod_maths,  only       :  maths_eigen_2x2_symmetric_matrix, maths_eigen_3x3_symmetric_matrix
  ...
  if( itask == 3 ) then
     ...
     GAUSS_POINTS_LOOP: do igaus = 1,pgaus
        ...
        if( ndime == 3_ip ) then
           ...
           call maths_eigen_3x3_symmetric_matrix(gpcrt(:,:),eval(:),evec(:,:))
        ...
        elseif( ndime == 2_ip )then
          ...
          call maths_eigen_3x3_symmetric_matrix(gpcrt3(:,:),eval3(:),evec3(:,:))
          ...
          do k = ndime+1,3
           ...
           call runend('SLD_BITCUL: LOG STRAINS not computed for this dimension')
        endif

        do idime = 1,ndime
           do jdime = 1,ndime
              do kdime = 1,ndime
                  if( abs(eval(kdime) - 1.0_rp) < 1e-8_rp ) eval(kdime) = 1.0_rp
                  gplep(idime,jdime) = gplep(idime,jdime) + log(sqrt(abs(eval(kdime))))&
