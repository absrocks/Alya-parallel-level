...
module mod_sld_fibers
    ...
    contains 
        ...
        subroutine sld_fib_read_data( &
            ...
            use def_solidz,           only :  modfi_sld, parsp_sld
            ! -------------------------------
            implicit none
            ! -------------------------------
            ...
            select case(itask)
                ...
                case(2_ip)
                    ...
                    if( kfl_fiber_sld == 1_ip )then
                        call lexcha( fibers_sld % initialised )
                        call iexcha( fibers_sld % location )
                        call iexcha( fibers_sld % field_fiber )
                        call iexcha( fibers_sld % field_sheet )
                        call iexcha( fibers_sld % field_norma )            
                        do ii = 1, 3
                            call rexcha( fibers_sld % vector(ii) )
                        enddo
                        do ii = 1, 3
                            do jj = 1, 3
                                call rexcha( fibers_sld % basis(jj,ii) )
                            enddo
                        enddo
                     endif
