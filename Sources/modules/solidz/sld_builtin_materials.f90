...
subroutine sld_builtin_materials( &
   pgaus,pmate,gpvol,temp0,temp1,gpgi0,gpgdi,gpigd,gpcau,gpdet,gprat,gppio,gpstr,&
   ...
   use def_material,             only: MAT_MICRO_FULL, MAT_MICRO_NO_COUPLING, MAT_MICRO_ONE_WAY
   use def_solidz,               only: epsex_sld, kfl_ninex_sld, kfl_tange_sld, SLD_TANGENT_NUMERICAL, &
   &                                   lawst_sld, lawho_sld, lawpl_sld
   ...
   real(rp),    intent(in)        :: temp0(pgaus)                           ! Temperature  (previous)
   real(rp),    intent(in)        :: temp1(pgaus)                           ! Temperatrue  (current)
   ...
   if( (kfl_tange_sld == SLD_TANGENT_NUMERICAL .or. kfl_tange_sld == 2_ip) .and. flagt==1_ip )then
      ...
      if(      lawst_sld(pmate) == 100_ip )then
         ...
         call sld_stress_model_100(pgaus,pmate,gpgdi,gpstr,gpcau,gpdet, &
         ...
         &  gpstr,gprat,gpdet) 
            ...
            !
            ! Holza & Ogden with ECC coupling
            kfl_STR = STR_FROM_PK2_TO_PK1
            kfl_MAT = MAT_FROM_DSDE_TO_DPDF
            call sld_stress_model_134(ielem,pgaus,pmate,pnode,lnods,  &
            &   flagt,gpsha,gpdet,gpgdi,gpcau,gpfib,gpstr,gpdds)
            ...
            !
            ! Orthotroipic linear-elastic model
            !   {Ref : Belytschko book }
            kfl_STR = STR_FROM_PK2_TO_PK1
            kfl_MAT = MAT_FROM_DSDE_TO_DPDF
            call sld_stress_model_151(pgaus,pmate,gpgdi,gpigd,gpdet,gpstr,  &
            &   ielem,flagt,gpdds)
            ...
            kfl_STR = STR_FROM_PK2_TO_PK1
            kfl_MAT = MAT_NOT_COMPUTED
            call sld_stress_model_154(pgaus,pmate,gpgdi,gpigd,gpdet,gpstr,  &
            &   ielem,flagt,gpdds)
      ...
      !
      ! Compute tangent moduli using and explicit step
      ! 
      if (kfl_ninex_sld == 1) then
         ! TODO : Finish and test it (Mariano Vazquez)
         !
         ...
         do igaus= 1,pgaus
            do mm= 1,ndime
               do nn= 1,ndime
                  do jj=1,ndime
                     do ll=1,ndime
                        dstre = gpstr(jj,ll,igaus,2) - gpstr(jj,ll,igaus,1)
                        !                    dgree = abs(gpgre(mm,nn,igaus,2)   - gpgre(mm,nn,igaus,1))
                        dgree = epsex_sld
   
                        !                    gpdds(mm,jj,nn,ll,igaus) = 0.0_rp
                        !                    if (dgree > 0.0_rp) then
                        !                       gpdds(mm,jj,nn,ll,igaus) = dstre / dgree
                        !                    end if
                        gpdds(jj,ll,mm,nn,igaus) = 0.0_rp
                        if (dgree > 0.0_rp) then
                           gpdds(jj,ll,mm,nn,igaus) = dstre / dgree
                        end if
                     end do
                  end do
               end do
         ...
         do igaus=1,pgaus
            do kk=1,ndime
               do jj=1,ndime
                  gppio_eps(jj,kk,igaus)=0.0_rp
                  do ii=1,ndime
                     gppio_eps(jj,kk,igaus)= &
                        gppio_eps(jj,kk,igaus) + &
                        gpgdi_eps(jj,ii,igaus)*gpstr(ii,kk,igaus,2)
                  enddo
               enddo
            enddo
         end do
