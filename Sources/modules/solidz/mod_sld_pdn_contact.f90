          ...
          if ( CNT_SENDRECV(7) ) then
             ...
             if ( INOTMASTER ) then
                ...
                if ( ndime == 2_ip ) then ! 2-d problem
                   ...
                   do ipoin = 1,nrecv
                      if ( kfl_codno(1,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld ) then
                     ...
                     !if( any(kfl_codno(:,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld) .and. (0.99_rp < data_recv(8,ipoin)) .and. (data_recv(8,ipoin) < 1.01_rp) ) then
                         ...
                         jacrot_dq_du_sld(1,1,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(1,1,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(1,2,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(2,1,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(2,1,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(1,2,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(2,2,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(2,2,PLEPP_CPLNG%interior_list_j(ipoin))
                   ...
                   do ipoin = 1,nrecv
                     ...
                     !if( any(kfl_codno(:,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld) .and. (0.99_rp < data_recv(11,ipoin)) .and. (data_recv(11,ipoin) < 1.01_rp) ) then
                         ...
                         jacrot_du_dq_sld(1,3,PLEPP_CPLNG%interior_list_j(ipoin)) = tmp_tgx/sqrt(tmp_tgx*tmp_tgx + tmp_tgy*tmp_tgy + tmp_tgz*tmp_tgz) !t2x
                         jacrot_du_dq_sld(2,3,PLEPP_CPLNG%interior_list_j(ipoin)) = tmp_tgy/sqrt(tmp_tgx*tmp_tgx + tmp_tgy*tmp_tgy + tmp_tgz*tmp_tgz) !t2y
                         jacrot_du_dq_sld(3,3,PLEPP_CPLNG%interior_list_j(ipoin)) = tmp_tgz/sqrt(tmp_tgx*tmp_tgx + tmp_tgy*tmp_tgy + tmp_tgz*tmp_tgz) !t2z

                         jacrot_dq_du_sld(1,1,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(1,1,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(1,2,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(2,1,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(1,3,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(3,1,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(2,1,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(1,2,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(2,2,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(2,2,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(2,3,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(3,2,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(3,1,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(1,3,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(3,2,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(2,3,PLEPP_CPLNG%interior_list_j(ipoin))
                         jacrot_dq_du_sld(3,3,PLEPP_CPLNG%interior_list_j(ipoin)) = jacrot_du_dq_sld(3,3,PLEPP_CPLNG%interior_list_j(ipoin))

                         kfl_fixno_sld(1,PLEPP_CPLNG%interior_list_j(ipoin)) = 3_ip

                         displ(1,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(7,ipoin)*data_recv(8,ipoin) + displ(1,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) + tol
                         displ(2,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(7,ipoin)*data_recv(9,ipoin) + displ(2,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) + tol
                         displ(3,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(7,ipoin)*data_recv(10,ipoin) + displ(3,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) + tol
                ...
                do ipoin = 1,nrecv
                   ...
                   if( ibopo > 0_ip ) then
                      ...
                      call sld_rotsys(2_ip, 1_ip, 1_ip, ndime, ndime, dummy_matrix, resid_tmp(1:ndime), &
                           jacrot_du_dq_sld(1,1,PLEPP_CPLNG%interior_list_j(ipoin)), jacrot_dq_du_sld(1,1,PLEPP_CPLNG%interior_list_j(ipoin)))
                   end if
                   ! Save it
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!--PARALLEL TWEAK - SEND CONTACT BOUNDARY TO ALL SLAVES--!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine sld_rbo_bilateral_send()
    ...
    nullify(fcont)
    if( INOTMASTER ) then
       call memory_alloca(mem_modul(1:2,modul),'FCONT','sld_rbo_bilateral_send',fcont,ndime,npoin,'DO NOT INITIALIZE')
       fcont(1:ndime,1:npoin) = reshape(fcont_sld,shape=(/ndime,npoin/))
       force_dbo(1:ndime) = sum(fcont(1:ndime,PLEPP_CPLNG%interior_list_j), dim=2)
       call memory_deallo(mem_modul(1:2,modul),'FCONT','sld_rbo_bilateral_send',fcont)
       !
       ! Parall exchange
       !
