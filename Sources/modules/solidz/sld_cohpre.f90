subroutine sld_cohpre(itask,ielem,iboun,igaub,xdisp,tract,stiff)
  ...
  use def_domain, only       :  mnode,ndime,mgaus,lnods,nmate,lelch
  use def_solidz, only       :  leenr_sld,crapx_sld,cranx_sld,lawch_sld,lmate_sld
  use def_solidz, only       :  lecoh_sld,parco_sld,dfric_sld,dceff_sld,dslip_sld
  use def_solidz, only       :  kfl_cohes_sld, cohnx_sld
  use def_elmtyp
  use def_master 
  implicit none
  ...
  real(rp)                   :: stemp(2,ndime),dummr(2,2)
  integer(ip)                :: idime,jdime,kdime,ipoin,pmate
  
  logical                    :: debugging
  real(rp), parameter        :: pert = 1.0e-8_rp 
  integer(ip)                :: ipert,icomp,jcomp
