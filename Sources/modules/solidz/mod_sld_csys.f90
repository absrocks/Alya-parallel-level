...
contains
  ...
  subroutine sld_csys_tensor_cylindrical_to_cartesian(itask, pdime, pocoo, csysp, tenca, tency)
    ...
    case(2_ip)
       ...
       if(      pdime == 2_ip ) then
          ...
          if(      abs(csysp(3*pdime)) > 0.0_rp ) then
             !
             ! X-Y Plane (Axial=Z)
             theta = atan(vectr(2)/vectr(1))*180.0_rp/pi
             call sld_csys_rotation_matrix(3_ip, pdime, theta, rotma)
          else if( abs(csysp(3*pdime-1)) > 0.0_rp ) then
             !
             ! X-Z Plane (Axial=Y)
             theta = atan(vectr(3)/vectr(1))*180.0_rp/pi
             call sld_csys_rotation_matrix(2_ip, pdime, theta, rotma)
          else if( abs(csysp(3*pdime-2)) > 0.0_rp ) then
             !
             ! Y-Z Plane (Axial=X)
             theta = atan(vectr(3)/vectr(2))*180.0_rp/pi
             call sld_csys_rotation_matrix(1_ip, pdime, theta, rotma)
          else
             call runend('MOD_SLD_CSYS: CYLINDRICAL BASIS NOT DEFINED IN THE MAIN PLANES')
          end if
       end if
  ...
  subroutine sld_csys_build_jacrot()
    ...
    use def_domain,      only : npoin, ndime, lpoty, coord
    use mod_local_basis, only : local_basis_matrix
    use mod_sld_stent,   only : sld_local_basis_stent
    use def_solidz,      only : kfl_fixrs_sld , kfl_conta_stent
  ...
  subroutine sld_csys_midsurface_element_normal(ielem, pdime, pnode, N, P)
    ...
    real(rp),    intent(out), optional :: P(pdime)      !< Referece/primary vector correction
