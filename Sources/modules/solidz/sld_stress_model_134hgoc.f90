...
!> @} 
!-----------------------------------------------------------------------
subroutine sld_stress_model_134hgoc(&
  ...
  real(rp)                   :: detf0,nfibe_length,tactf,nfibe(ndime,pgaus)
  ...
  real(rp)                   :: elfib(3,mnode),elfis(3,mnode),elfin(3,mnode),elcac(mnode),gpcac
  ...
  real(rp)                   :: term5,term6,term7,term11,term22,term33,term44,term55,term66,term77, &
                                inter1,traceb_bar,trace_fporf,trace_fporf_2,trace_spors,trace_fpors,trace_sporf
  ...
  do igaus = 1,pgaus
     ...
     do idime = 1,ndime
        do jdime = 1,ndime
           ...
           i4s  =  i4s + nshet0(idime,igaus) * gpdet(igaus)**(-2.0_rp/3.0_rp)*gpcau(idime,jdime,igaus) * nshet0(jdime,igaus) ! I_4s  = s0 * C_ij * s0
           i4n  =  i4n + norma0(idime,igaus) * gpdet(igaus)**(-2.0_rp/3.0_rp)*gpcau(idime,jdime,igaus) * norma0(jdime,igaus) ! I_4n  = n0 * C_ij * n0
           i8fs = i8fs + nfibe0(idime,igaus) * gpdet(igaus)**(-2.0_rp/3.0_rp)*gpcau(idime,jdime,igaus) * nshet0(jdime,igaus) ! I_8fs = f0 * C_ij * s0 (eq. 5.3)
           ! * * * * *
           ...
           nshet(idime,igaus) = nshet(idime,igaus) + (gpdet(igaus)**(-1.0_rp/3.0_rp))*gpgdi(idime,jdime,igaus) * nshet0(jdime,igaus)
           norma(idime,igaus) = norma(idime,igaus) + (gpdet(igaus)**(-1.0_rp/3.0_rp))*gpgdi(idime,jdime,igaus) * norma0(jdime,igaus)
        end do
     end do
     ...
     term1 = a*(gpdet(igaus)**(-5.0_rp/3.0_rp))*exp(b*(gpdet(igaus)**(-2.0_rp/3.0_rp)*i1-3.0_rp))    
     ...
     if (kfl_fiber_sld > 0) then
        ...
        !term3 = 2.0_rp*af*(i6f-1.0_rp)*exp(bf*(i6f-1.0_rp)**2.0_rp)
     ...
     do idime = 1,ndime
       ...
       ! * * * * * * * *
        ...
        Dev_2 = spors_0 - (1.0_rp/3.0_rp)*i4s*invC_bar  
           ...
           do jdime = 1,ndime
              do kdime = 1,ndime
                 do ldime=1,ndime
                    ...
                    term1 = 4.0_rp*(gpdet(igaus)**(-4.0_rp/3.0_rp))*energy_a1_a1*fporf_0(idime,jdime)*fporf_0(kdime,ldime)
                    ...
                    term1_2 = 4.0_rp*(gpdet(igaus)**(-4.0_rp/3.0_rp))*energy_a1_a1_2*spors_0(idime,jdime)*spors_0(kdime,ldime)

                    term2_2 = (4.0_rp/3.0_rp)*energy_a1_2*i4s*(C4+(1.0_rp/3.0_rp)*gpcai(idime,jdime)*gpcai(kdime,ldime))

                    term3_2 = -4.0_rp*energy_a1_2*(1.0_rp/3.0_rp)*gpdet(igaus)**(-2.0_rp/3.0_rp)*spors_0(idime,jdime)*gpcai(kdime,ldime)

                    term4_2 = -4.0_rp*energy_a1_2*(1.0_rp/3.0_rp)*gpdet(igaus)**(-2.0_rp/3.0_rp)*spors_0(kdime,ldime)*gpcai(idime,jdime)
...
end subroutine sld_stress_model_134hgoc
