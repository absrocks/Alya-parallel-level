subroutine got_elmshc(&
     pnode,pgaus,pevat,ndofn,gpgcd,gpvdr,gpcdr,gpst2,&
     gprhs,resic,gpvol,gpcar,elcdr,elvdr,dtinv_got,&
     chale,elmat)
  !-----------------------------------------------------------------------
  !****f* Gotita/got_elmshc
  ! NAME
  ...
  do igaus=1,pgaus
     ...
     ! f = alpha^n/(theta*dt)
     !
     gpres=gprhs(ndofn_got(3),igaus)
     do inode=1,pnode
        do idime=1,ndime
           gpres=gpres&
                -resic(idime,inode,igaus)*elvdr(idime,inode)
        end do
        gpres=gpres&
             -resic(ndofn_got(3),inode,igaus)*elcdr(inode)
     end do
