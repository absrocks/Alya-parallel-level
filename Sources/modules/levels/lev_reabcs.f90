subroutine lev_reabcs()
  ...
  implicit none
  integer(ip)  :: dummi
  integer(ip)  :: icond, icobd, idime, ipoin, ivcod, ivcob, kdime
  integer(ip)  :: iread
  character(5) :: wfname
           ...
           do while(words(1)/='ENDPA') 
              if(words(1)=='INITI') then                 
                 ...
                 if(exists('HEIGH')) then                 
                    ...
                    if( kfl_inlev_lev == 86 )then
                       lev_dam_break(1_ip) = real(param(3),rp)
                       lev_dam_break(2_ip) = real(param(4),rp)
                       lev_dam_break(3_ip) = real(param(5),rp)
                    elseif( kfl_inlev_lev == 106)then
                       lev_dam_break(1_ip) = real(param(3),rp)
                       lev_dam_break(2_ip) = real(param(4),rp)
                       lev_dam_break(3_ip) = real(param(5),rp)
                       lev_dam_break(4_ip) = real(param(6),rp)
