subroutine lev_iniun0()
  ...
  integer(4)               :: istat
  real(rp)                 :: fx,dummy
  real(rp)                 :: x,y,z,xc,xc1,xc2,yc,zc,rc,distx,disty,distz
  real(rp)                 :: xi,xj,yi,yj,a,b,c,d
  real(rp),  allocatable   :: interfx(:),interfy(:) 
  real(rp),  allocatable   :: phi(:)
  real(rp)                 :: vx,vy,vz,rho
  real(rp)                 :: zmed,ztop,zbot
  real(rp)                 :: xmin_jump, xmax_jump, xjump, h1, h2, mramp, ylevel, ylevel_max, h, xo, omega, A1
  integer(ip)              :: kfl_value,ipoin,i,j,inter,k,ipoin1,nptot
  integer(ip)              :: icond, idime, ivcod, kdime
     ...
     if( kfl_inlev_lev < -1000 ) then
        ...
        do ipoin=1,npoin
           ...
           if ( x < xc1 ) then
              ...
              c = -(x-xc1) + zc
              if ( z < zc ) then
                 fleve(ipoin,3)= distx+1e-08_rp
              else
                 fleve(ipoin,3)=-sqrt(distz*distz+distx*distx)+1e-08_rp
              end if
              
           else if ( x>=xc1 .and. x<=xc2 ) then
              ...
              c = (x-xc2) + zc
              if ( z < zc ) then
                 fleve(ipoin,3)=-x+1e-08_rp
              else
                 fleve(ipoin,3)=-sqrt(distz*distz+distx*distx)+1e-08
              end if
           end if
           bvess_lev(1,ipoin,1) = fleve(ipoin,3)
        end do
        ...
        !
        do ipoin=1,npoin
           fleve(ipoin,3)     = height_lev - coord(3,ipoin) + 1.0e-8_rp
...
!!$             fleve(ipoin,3)= 2.0_rp*(-sqrt((x-xc)*(x-xc)+(y-yc)*(y-yc)+(z-zc)*(z-zc))+rc)
     ...
     else if(kfl_inlev_lev==86) then
        !
        ! 3D dam break
        !
	zc= lev_dam_break(1) !5.0_rp  !3.0_rp
	xc= lev_dam_break(2) !-3.0_rp !-1.5_rp
        a = lev_dam_break(3) ! 1._rp  !1.0_rp
        b=zc-a*xc
     ...
     else if(kfl_inlev_lev==106) then
        !
	h = lev_dam_break(4) !-0.02_rp ! fixed depth of water
        zc= lev_dam_break(1) !5.0_rp  !3.0_rp
        xc= lev_dam_break(2) !-3.0_rp !-1.5_rp
        a = lev_dam_break(3) ! 1._rp  !1.0_rp
        !xo = lev_dam_break(5) !-2.5_rp
	   ...
	   c=a*(x)+b
           if(x<=xc) then
              fleve(ipoin,3)= (zc -coord(ndime,ipoin) +1e-08_rp)
           elseif (x>xc) then
              fleve(ipoin,3)= (h -coord(ndime,ipoin) +1e-08_rp)
           !if (z<=zc .and. z>3*h) then
            !  fleve(ipoin,3)=xc-x+1e-08_rp
           !end if
        end if
     end do
        ...
        do ipoin=1,npoin
           ...
           if (x < -3.79) then
              fleve(ipoin,3) = 0.6 - coord(ndime,ipoin) + 1.0e-8_rp
           else if (x >= -3.79 .AND. x<=-3.8 .AND. y>= 0.225) then
              fleve(ipoin,3) = 0.6012 - coord(ndime,ipoin) + 1.0e-8_rp
           else if ( x > -3.8 ) then
              fleve(ipoin,3) = 0.06 - coord(ndime,ipoin) + 1.0e-8_rp
