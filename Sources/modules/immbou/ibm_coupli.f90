              ...
              do ipoin = 1,npoin
                 ...
                 if (lntib(ipoin) < 0_ip) then
                    ...
                    do idime = 1,ndime               
                       ...
                       bvess_ale(idime,ipoin,1)      = -(imbou(iimbo) % posil(idime,1) - imbou(iimbo) % posil(idime,2))
                    end do
        ...
        if( INOTMASTER ) then
           if( associated(kfl_fixno_ale) ) then
              if ( kfl_mvext_ibm == 1 ) then ! set x & y components of displacement = disp COG
                 do ipoin = 1,npoin
                    do idime = 1,2
                       if (kfl_fixno_ale(idime,ipoin) == 1) bvess_ale(idime,ipoin,1) = imbou(1) % posil(idime,1) & 
              ...
              do iimbo = 1,nimbo
                 if( imbou(iimbo) % kfl_typeb >= 1 ) then
                    do kpoin = 1,imbou(iimbo) % npoib
                       ...
                       do idime = 1,ndime    
                          bvess_ale(idime,ipoin,1)     = imbou(iimbo) % cooib(idime,kpoin) - coord(idime,ipoin)
                          kfl_fixno_ale(idime,ipoin) = 1
                       end do
                    end do
                 end if
