  ...
  if( nimbo > 0 ) then
     ...
     do parii = 1,2 
        ...
        if( parii == 1 ) then
           ...
           call memchk(zero,istat,mem_modul(1:2,modul),'PARIN','ibm_restar',parin)
           allocate(parre(nparr),stat=istat)
           call memchk(zero,istat,mem_modul(1:2,modul),'PARRE','ibm_restar',parre)
     ...
     call memchk(two,istat,mem_modul(1:2,modul),'PARIN','ibm_restar',parin)
     deallocate(parin,stat=istat)
     if( istat /= 0 ) call memerr(two,'PARIN','ibm_restar',0_ip)
     call memchk(two,istat,mem_modul(1:2,modul),'PARRE','ibm_restar',parre)
