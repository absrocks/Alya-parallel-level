...
contains
  ...
  subroutine nsi_element_operations_fast(&
    ...
    use def_master,                     only : amatr                                     ! out       ! real(rp), pointer     :: amatr(:)
...
#define DENOMX     denom(1:VECTOR_SIZE)
    ...
    !hh    if (kfl_cotu_nsi < 0) gpmut = gpden * gpmut                                             ! smago, wale, vreman etc   ! esto esta hecho mas abajo como gpmut(DEF_VECT,:) = gpden(DEF_VECT,:) * gpmut(DEF_VECT,:)
    !yo creo que se puede subir aca es mas no veo paar que le ponene explitas las dimensiones  
    !  bueno asi DEF_VECT es ivect paar open acc y eso lo vectoriza la GPU no creoq ue haga mayor 
    !diferencia comparado con el coste de llamar a ker_proper
    ...
    !
    ! Send both convective and viscous terms to RHS
    !
