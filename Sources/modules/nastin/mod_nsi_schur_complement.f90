...
!#endif
  ...
  subroutine nsi_solcor()
    ...
    !
    call nsi_elmcor()
    if( kfl_timin == 1 ) call PAR_BARRIER()
  ...
  subroutine nsi_solcor_consistent_mass()
    ...
    if( kfl_timin == 1 ) call PAR_BARRIER()    ! par_barrier
...
#endif
    if( postp(1) % npp_stepi(62,1) > 0 .and. INOTMASTER ) then
...
#endif
    if( postp(1) % npp_stepi(22,1) > 0  .and. INOTMASTER ) then
