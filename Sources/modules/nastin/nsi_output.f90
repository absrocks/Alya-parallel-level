subroutine nsi_output()
  ...
  if( postp(1) % npp_stepi(21,0) /= 0 ) then
     if ( nsteps_ensemble >= postp(1) % npp_stepi(21,0) ) call runend(' nsteps_ensemble >= postp(1) % npp_stepi(21) does not make sense')
     if( ( mod(ittim, postp(1) % npp_stepi(21,0) ) == nsteps_ensemble ) .and. laux_ensemble ) then
        entim_nsi = cutim    ! ensemble time
        laux_ensemble=.false.
     end if
     if( mod(ittim, postp(1) % npp_stepi(21,0) ) == 0 ) then     ! AVVEL frequency
