  ...
  if( INOTSLAVE ) then
     ...
     do while(words(1)/='ENDBO')
        ...
        if (words(1) == 'PARAM' ) then
           ...
           !
           ! Flow rates
           !.md<1>FLOW_RATE= real, CODE= int [,SPACE_TIME_FUNCTION = name, NORMALS= x, y, z]
           !
           iflow = iflow + 1
           if( iflow > size(kfl_flow_rate_codes_nsi,KIND=ip) ) &
           ...
           if (exists('NORMA')) then
              ...
              flow_rate_normals_nsi(iflow,1:ndime) = param(2:ndime+1)            ! Parameters: normal x, y [, z]
           end if
