...
module mod_nsi_eigen_time_step
  ...
  use def_master,              only : INOTEMPTY,veloc,mem_modul,modul,kfl_paral
...
contains
  ...
  subroutine nsi_eigen_time_step_element_operations(VECTOR_DIM,pnode,pgaus,list_elements)
    ...
    real(rp)                         :: gpnut(VECTOR_DIM,pgaus)
...
!!$    allocate(list_elements_p(VECTOR_DIM))          ! List of elements (always positive)
   ...
   if( kgaus == 1 ) then
    ...
    if( kgaus == 1 ) then
       ...
       if(kfl_algor_nsi /= 3) then
          call ker_proper('TURBU','COG  ',dummi,list_elements_p,gpnut,pnode,kgaus,gpsha,gpcar)
       else
       ...
       if(kfl_algor_nsi /= 3) then
          call ker_proper('TURBU','PGAUS',dummi,list_elements_p,gpnut,pnode,kgaus,gpsha,gpcar)         
       else
         gpnut(DEF_VECT,:) = 0.0_rp
       endif
    end if
