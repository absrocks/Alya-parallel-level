...
subroutine nsi_plugin(icoup)
  ...
  use def_master,    only : current_code,kfl_paral,inotmaster,lninv_loc,momentum_sink
  ...
  if( variable == 'MOMSK' ) then
     if( .not. associated(momentum_sink) ) then 
        if (INOTMASTER) then 
           call memory_alloca(mem_modul(1:2,modul),'MOMENTUM_SINK','nsi_plugin',momentum_sink,ndime,npoin)
        else
           call memory_alloca(mem_modul(1:2,modul),'MOMENTUM_SINK','nsi_plugin',momentum_sink,ndime,1_ip)
        endif
     endif
