...
subroutine nsi_solmem()
  ...
  implicit none
  integer(ip) :: ielem,pgaus,pelty,npoin_min,nelem_min,ipoin
  ...
  if( kfl_hydro_gravity_nsi /= 0 ) then
     ...
     do ielem=1,nelem
        ...
        pgaus = ngaus(pelty)
        call memory_alloca(mem_modul(1:2,modul),'HYDRO_DENSITY_NSI(IELEM)','nsi_solmem',hydro_density_nsi(ielem)%a,pgaus)
     end do
  end if
  !
  ! Boundary mass
