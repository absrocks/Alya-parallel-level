...
#define DEF_VECT 1:VECTOR_SIZE
    ...
    real(rp),allocatable       :: elauu_nsw(:,:,:)
    real(rp)                   :: gpanpor(VECTOR_SIZE,ndime,ndime) 
    real(rp)                   :: gpmve(VECTOR_SIZE) 
    real(rp)                   :: por_aux(VECTOR_SIZE) 
    real(rp)                   :: forch(VECTOR_SIZE) 
    real(rp)                   :: darcy(VECTOR_SIZE) 
    real(rp)                   :: visc_aux(VECTOR_SIZE) 
    real(rp)                   :: den_aux(VECTOR_SIZE)
    integer(ip)                :: inode,jnode,jdime
    integer(ip)                :: idofv,jdof2,jdof3,ivect
    integer(ip)                :: idof1,idof3,idof2,igaus
    integer(ip)                :: idime,jdof1,jdofv,itime,ii
    ...
    if ( kfl_noslw_ker /= 0_ip ) then   ! falta hacer todo con uprom y mu_eff_prom
       allocate(elauu_nsw(VECTOR_SIZE,pnode*ndime,pnode*ndime))
       elauu_nsw = 0.0_rp
    end if
