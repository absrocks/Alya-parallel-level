    ...
    if( INOTMASTER ) then     ! pensar que pasa con el master
       ...
       do ipoin = 1,npoin
          ...
          if(ibopo > 0_ip) then 
             ...
             auvar = sqrt(auvar)
             augra = sqrt(augra)
             if( auvar > 1.0e-30_rp) then
                fact_nsw_ker(ipoin) = augra / auvar   ! gradient based divided variational based
             else
                fact_nsw_ker(ipoin) = 1.0_rp          ! no corection but this should not happen
             end if
          else
