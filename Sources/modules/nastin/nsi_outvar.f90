...
subroutine nsi_outvar(jvari,imesh)
  ...
  integer(ip)             :: izdom,jzdom,jdime,iboun,idofn
  integer(4)              :: istat
...
!!$     end if
  ...
  case(40_ip)
     ! TTRAC  total traction ! gevec is total traction over wall boundaries
     ! the same as for TRACT but also calculates net force over boundary
     call commdom_get_total_tract()
  case(45_ip)
     !
  ...
  case(65_ip) 
     ! 
     ! Reaction at algebraic level                                        !< 2016ABRIL04 
     !
     !     if( INOTMASTER ) then
     !        call memgen(zero,ndime,npoin)
     !        do ipoin = 1,npoin
  ...
  case(83_ip)
     ...
     !  
     call memory_alloca(mem_modul(1:2,modul),'FIXNO','nsi_outvar',kfl_fixno_tmp,1_ip ,max(1_ip,npoin))
     call memory_alloca(mem_modul(1:2,modul),'BVESS','nsi_outvar',bvess_tmp    ,1_ip ,max(1_ip,npoin))
     call memory_alloca(mem_modul(1:2,modul),'NORMA','nsi_outvar',normal_tmp   ,ndime,max(1_ip,npoin))
     !call memgen(zero,ndime,npoin)      
     call memgen(zero,npoin,0_ip)
...
10   continue
  ...
  case ( 96_ip )
     ...
     !
     if( INOTMASTER ) then
        call memgen(0_ip,ndime,nboun)      
        do iboun = 1,nboun
           gevec(1:ndime,iboun) =  velav_ker(1:ndime,1,iboun)
        end do
     end if
...
contains
  ...
  subroutine get_characteristic( idofn )  
    ...
    !
  end subroutine get_characteristic
  !-----------------------------------------------------------------------||---!
  !                                                                            !
  !-----------------------------------------------------------------------||---!

  subroutine commdom_get_total_tract()
    ...
    if( INOTMASTER ) then
       !
       call memgen(zero,ndime,npoin)
       call nsi_outtra(1_ip) ! computes traction over all boundary  gevec(idime)
                             ! (not very well for wal law with complex domain)
       ! 
       allocate( aux(ndime,npoin) )
       aux(  1:ndime,1:npoin) = gevec(1:ndime,1:npoin)
       gevec(1:ndime,1:npoin) = 0.0_rp
       Ut = 0.0_rp 
       !                                        sigma             sigma.dA              \int sigma.dA     
       do idime = 1,ndime !                    /                 /                     /
          call commdom_driver_get_total_flux( aux(idime,1:npoin), gevec(idime,1:npoin), Ut(idime) )
       enddo
       !   
       deallocate( aux )
    endif
    ! 
    call PAR_SUM(ndime, Ut, 'IN MY CODE')                      !< 2016ABR04  
    !print *, "Ut", Ut  
    !
  end subroutine commdom_get_total_tract
