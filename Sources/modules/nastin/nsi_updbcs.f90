...
subroutine nsi_updbcs(itask)
  ...
  integer(ip)             :: inodb,inode,ifunc,ibopo,jdime,kpoin
  integer(ip)             :: iflow,ierro,itype
  ...
  real(rp)                :: xx,alpha,center_gravity(3),p_in,dummr
  real(rp)                :: flow_rate_stf_value,flow_rate_tf_value
  ...
  case( ITASK_BEGSTE , ITASK_INIUNK )
     ...
     if( INOTEMPTY .and. kfl_conbc_nsi == 0 ) then
              ...
              if( gisca(ipoin) > 0 ) then ! inflow boundary
                 ...
                 do idime =1, ndime                    
                    if( abs(kfl_fixno_nsi(idime,ipoin)) == 7_ip ) then
                       kfl_fixno_nsi(ndime,ipoin) = -7 !free
                       kfl_fixpr_nsi(1,ipoin) = 1   
                    end if
                 end do
              end if
        ...
        do iboun = 1,nboun
           ...
           if( kfl_fixbo_nsi(iboun) /= 0 ) then
              ...
              if( itype == FUNCTION_TIME ) then
                 ...
                 p_in  = 0.0_rp                 
                 dummr = 0.0_rp
                 call  mod_windk_interact(dummr, p_in,'NASTIN', kfl_codbo(iboun), OPT_direction='OU')
        ...
        !
        if( ker_subdomain_motion_exists() .or. kfl_coupl(ID_NASTIN,ID_ALEFOR) /= 0 ) then  !
           do ipoin = 1,npoin
              do idime = 1,ndime
                 if( kfl_fixno_nsi(idime,ipoin) == 3 ) then
                    kfl_fixno_ale(idime,ipoin) = 1 !!!OJOOOO es para cuando leo displacement con un field
     ...
     if( maxval(kfl_flow_rate_codes_nsi(1:mflow_nsi)) > 0 ) then
        ...
        call memory_alloca(mem_modul(1:2,modul),'BVESS_LOC','nsi_updbcs',bvess_loc,ndime,max(npoin,1_ip))
        ...
        do iflow = 1,size(kfl_flow_rate_codes_nsi,KIND=ip)
           ...
           !in normal run itti2 >= 0
           !in restart itti2>=1. Checking this instead of ittim for restart to work with constant flowrate
           if( itti2<=1 .and. kfl_flow_rate_codes_nsi(iflow) > 0 ) update_flow_rate = .true.
           if( kfl_flow_rate_stfn_nsi(iflow) > 0 .or. kfl_flow_rate_tfn_nsi(iflow) > 0 ) update_flow_rate = .true. 
           ...
           if( update_flow_rate ) then
              ...
              call projec_mass_conservation(&
                      bvess_loc,lboun_nsi,lpoin_nsi,'LOCAL MASS',&
                      flow_rate_values_nsi(iflow)*flow_rate_tf_value*flow_rate_stf_value,&
                      'IN MY CODE WITHOUT MASTER',ERROR=ierro)

              if( INOTMASTER ) then
                 if( ierro == 0 ) then    
                    do ipoin = 1,npoin
                       if( lpoin_nsi(ipoin) )  veloc(:,ipoin,1) = bvess_loc(:,ipoin)
                       !
        ...
        if( kfl_exist_fib20_nsi == 1 ) then
           ...
           do ifunc = 1,max_windk_systems
              if( ( windk_systems(ifunc) % sysid /= 0_ip ) .and. ( itinn(modul)==1_ip ) ) then
                 call  mod_windk_interact(outflow_mass(windk_systems(ifunc) % iflow_nsi),dummr,'NASTIN', windk_systems(ifunc) % tag_in, OPT_direction='BI')
              else
                 exit
              end if
           end do
