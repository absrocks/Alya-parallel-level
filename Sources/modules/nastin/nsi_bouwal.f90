subroutine nsi_bouwal(&
  ...
  use def_kermod, only      :  dexlo_ker,kfl_waexl_ker,kfl_waexl_imp_ker
  use def_kermod, only      :  shape_waexl,lexlo_ker,kfl_wlaav_ker,kfl_aveme_ker
  use def_domain, only      :  ndime,ywalb,lnods,ltype,nnode
  use def_nastin, only      :  zensi,delta_nsi,grnor_nsi
  ...
  !real(rp)      :: rough
  !real(rp)      :: kinen
  !real(rp)      :: tveav(ndime)   ! Average velocity for wall law  - kfl_wlaav_ker = 1
  !integer(ip)   :: igaub

  integer(ip)              :: ldime,idime,inodb,idofn
  integer(ip)              :: jnode,knode,pelty,pnode
  integer(ip)              :: jevab,ievab,jdime,jnodb
  integer(ip)              :: imeth,ipoin,ielem,indei
  ...
  real(rp)                ::  grad2u(ndime,ndime)
  real(rp)                ::  velf2,umbra,avvfr
  !
  !
  !
  real(rp)                ::  h1,h2,hs,u1,u2,u3, reguk
  ...
  if( ( delta_nsi > zensi ) .or. ( kfl_delta == 1 ) ) then
     ...
     if( imeth == 1 ) then
        ...
        if (kfl_wlaav_ker == 1_ip.and.kfl_aveme_ker/=2) then  ! if averaged velocity for wall law
           ...
           call frivel(delta_aux,rough,tven2,vikin,avvfr)   
        else  
           avvfr = velfr
           tven2 = tveno
        end if
