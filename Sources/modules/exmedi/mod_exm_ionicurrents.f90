       ...
       if (INOTMASTER) then
          ...
          do ipoin=1,npoin              
             ...
             if (kmodel_ipoin == CELL_NOMOD_EXMEDI) then                       !no model
                ...
                call exm_oharaf_ionicurrents(ipoin,xioni,dioni,cai)  !!! compute CURRENTS O'Hara-Rudy

             else if (kmodel_ipoin == CELL_TORORD_EXMEDI) then ! ToR-ORd 2020
            ...
            if ((kmodel_ipoin > CELL_FENTON_EXMEDI) .and. (kmodel_ipoin < CELL_SCATRIA_EXMEDI) .or. &
              & (kmodel_ipoin == CELL_TORORD_EXMEDI)) then
