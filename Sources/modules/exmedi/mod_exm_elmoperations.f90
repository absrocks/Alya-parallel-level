       ...
       gauss_points_rhs: do igaus=1,pgaus(ivelem)
          ...
          if (flag_pseudecg) then
             ...
             do iroot= 1,nrootecg_exm
                ...
                dauxi = 0.0_rp
                ! Compute (D_ij*(-grad(V)_i*grad(1/R)_j))  ** this needs to be verified with Francesca Margara
                do idime= 1,ndime
                   do jdime= 1,ndime
                      dauxi = dauxi + difin(ivelem,idime,jdime,igaus) * (-griap(ivelem,idime) *  gridi(jdime) )
                   end do
                end do
