...
subroutine exm_outvar(ivari,imesh)
  ...
  !
     if(INOTMASTER) then
       ...
       if(modfi_exm.lt.0_ip) then
            gevec(1:ndime,1:npoin) = fiber_exm(1:ndime,1:npoin)
       elseif(modfi_exm.eq.1) then
            gevec=0.0_rp
            gevec(1,1:npoin) = 1.0_rp
       elseif(modfi_exm.eq.2) then
            gevec=0.0_rp
            gevec(2,1:npoin) = 1.0_rp
       elseif(modfi_exm.eq.3) then
            gevec=0.0_rp
  ...
  else if  ( ivari == 13_ip ) then
     !
     ! VAUXI_EXM
     !
     call arrays(ivari,'POSTPROCESS',vauxi_exm)
     return

  else if  ( ivari == 14_ip ) then
     !
     ! VICEL_EXM
     !
     call arrays(ivari,'POSTPROCESS',vicel_exm)
     return


  else if ( ivari == 15_ip ) then 
     !
  ...
  !
     if(INOTMASTER) then
       call memgen(zero,ndime,npoin)
       if(modor_exm(1).lt.0_ip) then
            gevec(1:ndime,1:npoin) = sheet_exm(1:ndime,1:npoin)
       elseif(modor_exm(1).eq.1) then
            gevec=0.0_rp
            gevec(1,1:npoin) = 1.0_rp
       elseif(modor_exm(1).eq.2) then
            gevec=0.0_rp
            gevec(2,1:npoin) = 1.0_rp
       elseif(modor_exm(1).eq.3) then
            gevec=0.0_rp
            gevec(3,1:npoin) = 1.0_rp
       endif
     endif
  ...
  !
     if(INOTMASTER ) then
       call memgen(zero,ndime,npoin)
       if(modor_exm(2).lt.0_ip) then
            gevec(1:ndime,1:npoin) = normal_exm(1:ndime,1:npoin)
       elseif(modor_exm(2).eq.1) then
            gevec=0.0_rp
            gevec(1,1:npoin) = 1.0_rp
       elseif(modor_exm(2).eq.2) then
            gevec=0.0_rp
            gevec(2,1:npoin) = 1.0_rp
       elseif(modor_exm(2).eq.3) then
            gevec=0.0_rp
            gevec(3,1:npoin) = 1.0_rp
       endif
     endif
     ...
     call runend('Variable 25 is free and not postprocessing anything')
     ...
     call runend('Variable 30 is free and not postprocessing anything')
