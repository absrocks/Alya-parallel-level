module mod_exm_oharaequations
  ...
  integer(ip), parameter :: &
     OHR_CELLMODEL_OHARA_RUDY    = 1_ip, &
     OHR_CELLMODEL_OHARA_PASSINI = 2_ip
...
contains
   ...
   !cellmodel = OHR_CELLMODEL_OHARA_RUDY | OHR_CELLMODEL_OHARA_PASSINI 
   subroutine exm_ohara_infs(v, res, cellmodel)

     ! Computes for each i in [1..neqs]:
     ! res(i) = a(i) + b(i) / ( 1 + exp(c(i) ( v + d(i) ) ) )
     real(rp), intent(in)    :: v
     real(rp), intent(out)   :: res(EXM_OHR_INF_EQUATIONS)
     integer(ip), intent(in) :: cellmodel
     ...
     real(rp), pointer, dimension (:,:) :: eq_inf_coefs
     integer(ip) :: ieq

      select case (cellmodel)
         case (OHR_CELLMODEL_OHARA_RUDY) !default
            eq_inf_coefs => eq_inf_coefs_ohara
         case (OHR_CELLMODEL_OHARA_PASSINI) 
            eq_inf_coefs => eq_inf_coefs_passini
         case default
            call runend("exm_ohara_infs(): undefined cell type - "//trim(intost(cellmodel)))
      end select

     do ieq = 1,EXM_OHR_INF_EQUATIONS
        res(ieq) = eq_inf_coefs(ieq,1) &
                 + eq_inf_coefs(ieq,2) / ( 1.0_rp + safe_exp( eq_inf_coefs(ieq,3) * ( v + eq_inf_coefs(ieq,4) ) ) )
     end do
   ...
   !cellmodel = OHR_CELLMODEL_OHARA_RUDY | OHR_CELLMODEL_OHARA_PASSINI 
  subroutine exm_ohara_taus(v, res, cellmodel)
     ...
     real(rp), pointer, dimension (:,:) :: eq_tau_coefs
     integer(ip) :: ieq
     ...
     do ieq = 1,EXM_OHR_TAU_EQUATIONS
        res(ieq) = eq_tau_coefs(ieq,1) &
             + eq_tau_coefs(ieq,2) / ( eq_tau_coefs(ieq,3) * safe_exp( eq_tau_coefs(ieq,4) * (v + eq_tau_coefs(ieq,5)) ) +&
             eq_tau_coefs(ieq,6) * safe_exp( eq_tau_coefs(ieq,7) * (v + eq_tau_coefs(ieq,8)) ) )        
     end do
  ...
  pure function safe_exp(x) result(r)
