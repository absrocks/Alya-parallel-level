...
subroutine exm_torord_model(cell_type,U,tconc_exm,tgates_exm,ti_exm,dt,a2bas,U_n,flag_land,flag_3D,flag_border,flag_drug,GKr_scaling,drugd,statvar,ipoin)
   ...
   use      def_exmedi
   use      def_domain
   use      def_master, only: CELL_TORORD_EXMEDI
   use      mod_exm_sld_eccoupling, only: get_active_traction_LANDS
   ...
   real(rp) :: ENa, EK, EKs, ECl, Jrel, Jup, Jtr, Jdiff, JdiffNa, JdiffK,JdiffCl, Jleak, Farad
   ...
   real(rp) ::     Bcajsr, zcl, clo, cli,clss, ah, bh, aj, bj, dielConstant, Io, Iss
   ...
   real(rp) :: lambda,dCaTRPN,Ca50_ref,Ca50,land_beta_1,k_TRPN,n_TRPN,cbracket
   real(rp) :: dcatrpndt,sac,param(25),lambda_rate,jac(6,6),tau_relp_scaling,aCaMK_scaling
   real(rp) :: dosis1,dosis2,dosis3,dosis4,i50cal,i50kr,i50na,i50ito,gcaldrug,gkrdrug,gnadrug,gitodrug
   real(rp) :: props(17),Tact,dTact
