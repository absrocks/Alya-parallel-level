module mod_exm_ocetor
   ...
   public :: exm_ocetor, exm_ocetla, exm_ocetor_general
...
subroutine exm_ocetor_general(mat, ituss_exm, success_status, torord_stats, model_type, extra_outputs)
   ...
   real(rp)  :: statvar(7,2),atim,batim,celltype,drugd(24),gkr_scaling
  ...
  ! end if
   ...
   if (kfl_drugsmate_exm(mat) == 1) then
      flag_drug = .true.
      drugd = drugdmate_exm(:,mat)
   else
      flag_drug = .false.
      drugd = 1.0_rp
   end if
      ...
      do while (atim < moneclmate_exm(2, mat))
         ...
         if (save_variables) then
            if (j >= post_frequency) then  !it was 10 for saving
               ...
               SELECT CASE (model_type)
               CASE (EXM_OCETOR_PURE)
                  write (post_file_handle, "(4(e16.8E3, ' '))") batim + itim*dtimon, atim, vcolo_exm(5, 2), elmlo_exm(2)
               CASE (EXM_OCETOR_LAND)
                  write (post_file_handle, "(5(e16.8E3, ' '))") batim + itim*dtimon, atim, vcolo_exm(5, 2), elmlo_exm(2),Tactf
               CASE DEFAULT
                  call runend("mod_exm_ocetor: Unknown cell model: "//trim(intost(model_type)))
               END SELECT
