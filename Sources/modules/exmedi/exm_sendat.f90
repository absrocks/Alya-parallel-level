subroutine exm_sendat(order)
  ...
  integer(ip)             :: imate,iroot,idime,icelltype
  ...
  case(1)
     ...
     do parii=1,2 
        ...
        do imate=1,size(kfl_user_specified_celltypes_exm, 1, KIND=ip)
           do icelltype=1,size(kfl_user_specified_celltypes_exm, 2, KIND=ip)
              call iexcha( kfl_user_specified_celltypes_exm( imate, icelltype ) )
              call iexcha( kfl_ignore_steadystate_celltypes_exm( imate, icelltype ) )
           end do
           call iexcha( kfl_steadystate_variable(imate) )
           call rexcha( kfl_steadystate_tolerance(imate) )
       end do

    

        call iexcha(nrootecg_exm)
        do iroot = 1,size(coordecg_exm,2,KIND=ip)                     ! All possible lead roots
           do idime = 1,ndime
        ...
        do imate=1,nmate
           ...
           do jr=1,size(ttparmate_exm, 2, KIND=ip)
           ...
           do jr=1,size(drugdmate_exm,1,KIND=ip)
              call rexcha(drugdmate_exm(  jr,imate))
           end do
        ...
        call iexcha(modor_exm(1))
        call iexcha(modor_exm(2))
        ...
        do jr=1,size(ttpar_exm,1,KIND=ip)
           do kr=1,size(ttpar_exm,2,KIND=ip)
              call rexcha(ttpar_exm(jr,kr))                  ! Current Remodelling of cell models
           end do
        end do
        do jr=1,size(drugd_exm,KIND=ip)
           call rexcha(drugd_exm(jr))                  !  Drug definitions
        end do

        do jr= 1,size(aptim, KIND=ip)

           call rexcha(aptim(jr))
           call rexcha(apval_exm(jr))
           call rexcha(aplap_exm(jr))
           call rexcha(aprea_exm(jr))
           do kr=1,size(apcen_exm,1,KIND=ip)
              call rexcha(apcen_exm(kr,jr))
           end do
        end do


        do jr=1,size(gdiff_exm, 1, KIND=ip)
           do kr=1,size(gdiff_exm, 2, KIND=ip)
              do imate=1,size(gdiff_exm, 3, KIND=ip)
        ...
        do imate=1,nmate
           do jr=1,size(xmopa_exm,1,KIND=ip)
              call rexcha(xmopa_exm(jr,imate))
           end do
           call rexcha(voini_exm(imate))
           call iexcha(kfl_voini_exm(imate))          
           call iexcha(kfl_fract_diffusion_exm(imate))
           call rexcha(fract_diff_coef_exm(imate))
           call iexcha(fract_diff_nintp_exm(imate))
        end do
        ...
        do ji=1,size(err01_exm,KIND=ip)
        ...
        do ji=1,size(staco_exm,KIND=ip)
           call rexcha(staco_exm(ji))           ! 
        end do
        do ji=1,size(resid_exm,KIND=ip)
        ...
        !
        call iexcha(kfl_save_convergence_cellmodel)               
        call iexcha(kfl_save_init_cellmodel)               
        ...
        !
        if(parii==1) then
           call memory_alloca(mem_modul(1:2,modul), 'PARIN', 'exm_sendat', parin, npari)
           call memory_alloca(mem_modul(1:2,modul), 'PARRE', 'exm_sendat', parre, nparr)
     ...
     call memory_deallo(mem_modul(1:2,modul), 'PARIN', 'exm_sendat', parin)
     call memory_deallo(mem_modul(1:2,modul), 'PARRE', 'exm_sendat', parre)
