subroutine rad_outlat(itask)
  !-----------------------------------------------------------------------
  !****f* Radiat/rad_outlat
  ! NAME 
  !    rad_outlat
  ! DESCRIPTION
  !    This routine writes info on the radiation equation in latex format
  ! USED BY
  !    rad_turnon
  !***
  !-----------------------------------------------------------------------
...
!!$        if(kfl_timei_rad==1) then
!!$           equat='transient'
!!$        else
!!$           equat='stationary'
!!$        end if
!!$        equat=trim(equat)//' Heat Transfer equation.'
...
!!$        end if
!!$        lvisc='k'
!!$        if(kfl_cotur_rad/=0) lvisc='(k+k_t)'
...
!!$        equat=''
!!$
!!$        if(kfl_sourc_rad/=0)  then
!!$           ierhs=1
!!$           equat=trim(equat)//'+Q'
!!$        end if
!!$        if(kfl_joule_rad/=0) then
!!$
!!$        end if
!!$        if(ierhs==0) then
...
!!$        write(lun_latex,1) '\begin{eqnarray*}'
!!$        write(lun_latex,4) '    \rho &=& ',densi_rad(1,1)
!!$        write(lun_latex,4) ' \\  C_p &=& ',sphea_rad(1,1)
!!$        write(lun_latex,4) ' \\    k &=& ',tcond_rad(1,1)
!!$        if(kfl_joule_rad/=0) write(lun_latex,4) ' \\   \mu &=& ',tcond_rad(1,1)
!!$        write(lun_latex,1) '\end{eqnarray*}'
!!$        !
!!$        ! Numerical treatment
!!$        !
...
!!$        if(kfl_sgsti_rad==1) write(lun_latex,1) 'with time tracking of the subscales'                
!!$        write(lun_latex,1)    '          \begin{eqnarray*}'
!!$        equat='\tau_1 = \frac{1}{{\displaystyle c_1 \frac{'//trim(lvisc)//'}{h^2}}'
!!$        if(kfl_advec_rad/=0) equat=trim(equat)//'+ {\displaystyle c_2 \rho \frac{|\mbox{\boldmath $u$}|}{h}}'
!!$        if(react_rad>zetem)      equat=trim(equat)//'+ c_3 \sigma '
!!$        equat=trim(equat)//'}'
!!$        write(lun_latex,1) trim(equat)
!!$        write(lun_latex,1) '          \end{eqnarray*}'
!!$        write(lun_latex,1) '          with the fololowing values for the constants'
!!$        write(lun_latex,1) '          \begin{eqnarray*}'
!!$        write(lun_latex,4) '              c_1 &=& ',staco_rad(1)
!!$        write(lun_latex,4) '           \\ c_2 &=& ',staco_rad(2)
!!$        write(lun_latex,1) '          \end{eqnarray*}'
!!$        write(lun_latex,1) '  \end{itemize}' 
!!$        if(kfl_timei_rad==1) then
!!$           write(lun_latex,1) '  \item Temporal treatment:'
!!$           write(lun_latex,1) '  \begin{itemize}'
!!$           write(lun_latex,6) '    \item Time integration strategy:'
!!$           write(lun_latex,1) ' Trapezoidal'
!!$           write(lun_latex,7) '     \item Order of integration: ',kfl_tiacc_rad
!!$           write(lun_latex,1) '\end{itemize}'
!!$        end if
!!$        write(lun_latex,1) '\end{itemize}'
!!$        !
...
!!$        cgnup='plot '''//trim(momod(modul)%fil_conve)//''' u 5 t ''Temprature'' w l'
!!$        if(kfl_sgsti_rad==1) then
!!$           cgnup=trim(cgnup)//', '''//trim(momod(modul)%fil_conve)//''' u 7 t ''Temprature SGS'' w l'
!!$        end if
