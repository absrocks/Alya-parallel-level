...
contains
    ...
    if(kfl_htran == 0) then
       ...
       if (INOTMASTER) then
          ...
          do ipoin = 1,npoin
             ...
             !
             tem  = temperature(gas_chm)
             call getenthalpies_rt(gas_chm, entha_chm(1:nspec_chm,ipoin) )   !! entha_chm also contains soot enthalpies hs (nclas_chm,npoin)
  ...
  subroutine chm_element_operations_finiteRate(order,pnode,pgaus,list_elements)
    ...
    real(rp)                         :: del_gpdif(mgaus,nclas_chm)               ! change in diffusivity from entropy stable stabilization
    ...
    elements: do kelem = 1,size(list_elements)
       ...
       if( ielem > 0 ) then
          ...
          call chm_turbul_finiteRate(pgaus,gpdif,gpden,gptur) ! At this point gpdif contains the laminar, turbulent and entropy visc. contributions
          !
  ...
  subroutine chm_turbul_finiteRate(pgaus,gpdif,gpden,gptur)
    ...
    !***
    !-----------------------------------------------------------------------
    use def_chemic, only      : kfl_cotur_chm,diffu_chm
  ...
  subroutine chm_elmprc_finiteRate(&
    ...
    !***
    !-----------------------------------------------------------------------
    use def_chemic, only      : kfl_cotur_chm,diffu_chm, Le_k
          ...
          if( pelty > 0 ) then
              ...
              if (kfl_transport_chm == 1) then
                 ...
                 do igaus = 1,pgaus
                    ...
                    hk_gp (ielem) % a (1:nspec_chm,igaus,1) = hk_gp (ielem) % a (1:nspec_chm,igaus,1) * gasco / W_k(1:nspec_chm) * 298.15_rp
  ...
  subroutine chm_calc_enthalpy_transport_finiteRate(ielem,pgaus,gpdif,gpdit,gphco, &
    ...
    if (kfl_htran == 0) then
       ...
       do igaus=1,pgaus
          ...
          do iclas=1,nspec_chm
             enthalpy_transport(ielem)%a(igaus,1:ndime,1) = enthalpy_transport(ielem)%a(igaus,1:ndime,1) + gpdif(igaus,iclas) * grad_Yk_hk_gp(igaus,iclas,1:ndime)
          end do
          enthalpy_transport(ielem)%a(igaus,1:ndime,1) = enthalpy_transport(ielem)%a(igaus,1:ndime,1) + gpdit(igaus) * grad_h_gp(igaus,1:ndime) 
       ...
       do igaus=1,pgaus
          ...
          enthalpy_transport(ielem)%a(igaus,1:ndime,1) = enthalpy_transport(ielem)%a(igaus,1:ndime,1) + gphco(igaus) * grad_T_gp(igaus,1:ndime)
  ...
  subroutine chm_calc_div_enthalpy_transport_finiteRate
    ...
    do ielem = 1,nelem
       pelty = ltype(ielem)
       pgaus = ngaus(pelty)
       call memory_alloca(mem_modul(1:2,modul),'AUX_R2P','chm_calc_div_enthalpy_transport_finiteRate',aux_r2p(ielem)%a,pgaus,ndime)
    ...
    if (INOTMASTER) then
       ...
       !
          ...
          if(ittot_chm == 1_ip) then
             ...
             nullify(retva)
             call memory_alloca(mem_modul(1:2,modul),'TAB_SCALE_CONTROL'  ,'chm_IntegrateSource_finiteRate', tab_scale_control, table_fw % main_table % ndim )
             ...
             do ipoin=1,npoin
                ...
                ! Look up table 
                !
                call tab_interp(table_fw % main_table, tab_scale_control, retva, snap_on_nearest=.true., irow=irow)
