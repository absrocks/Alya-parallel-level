...
contains 
   ...
   subroutine chm_entropy_memory()
      ...
      do ielem = 1,nelem
         ...
         do iclas = 1,nclas_chm
            call memory_alloca(mem_modul(1:2,modul),'ENVIC_GP','chm_entropy_allocate', envic_gp(iclas,ielem)%a, pgaus)
         enddo
      end do
   ...
   subroutine chm_entropy_eval(iclai,iclaf,istep)
      ...
      integer(ip)              :: ipoin,iclass,kpoin,iclaf_gas,iclai_liq,iclas
      ...
      !
      if(istep == 4_ip) then
         call chm_entropy_solution_sij(iclai,iclaf,4_ip,sigma,1.0_rp)
      else
         call chm_entropy_solution_sij(iclai,iclaf,istep,b(istep+1,:),a(istep+1))
      endif

   end subroutine chm_entropy_eval

   subroutine chm_entropy_solution_sij(iclai,iclaf,istep,weight1,weight2)
 ...
 subroutine chm_entropy_viscosity(ielem,pnode,pgaus,igaui,igauf,iclas,gpsha,gpcar,elvel,gpden,hleng,gpvol,del_gpdif)
    ...
    integer(ip)               :: igaus,inode,idime, ipoin
    real(rp)                  :: fact
