module mod_chm_levSet
   ...
   real(rp),    pointer  :: rhs_ls(:,:)     ! RHS
   real(rp),    pointer  :: phi(:)   ! fraction of volume
   real(rp),    pointer  :: aux_rhs(:)  
   real(rp),    pointer  :: phig(:)   ! fraction of volume
   real(rp),    pointer  :: phi0(:)  ! fraction of volume 0 in pseudo time
   real(rp),    pointer  :: r(:,:)   ! residual
   real(rp),    pointer  :: rg(:,:)   ! residual
   real(rp),    pointer  :: Mass(:)  ! lumped mass
   real(rp),    pointer  :: gradPhi(:,:) 
   real(rp),    pointer  :: mass_inv(:)
   real(rp),    pointer  :: epsil(:)   ! fraction of volume
   real(rp),    pointer  :: tau(:) 

   real(rp)              :: a(5)
   real(rp)              :: b(5,5)
   real(rp)              :: sigma(5)
   real(rp)              :: pseudo_dtinv
...
contains
   ...
   subroutine chm_multi_step_fs_solution_sij_levSet(istep,weight1,weight2)
      ...
      integer(ip)       :: idofn,ipoin,idime,jstep,kpoin,iter
      real(rp) :: aux2, factor
      ...
      if( INOTMASTER ) then
         ...
         do ipoin = 1,npoin
            ...
            !phi(ipoin) = phi0(ipoin)  + (aux2)/(dtinv*Mass(ipoin)) 
            aux_rhs(ipoin) = aux2/dtinv
         end do
      ...
      solve1 => solve(1:)
      solve2 => solve(2:)
      call solver_explicit(solve1,aux_rhs,EXCHANGE=.false.,solve_consistent=solve2)
      do ipoin = 1,npoin
       phi(ipoin) = phi0(ipoin) + aux_rhs(ipoin)
      end do
   ...
   subroutine chm_multi_step_fs_solution_sij_levSet_galerkin(istep,weight1,weight2)
      ...
      call solver_explicit(solve1,aux_rhs,EXCHANGE=.false.,solve_consistent=solve2)
      do ipoin = 1,npoin
       phig(ipoin) = phi0(ipoin) + aux_rhs(ipoin)
      end do
   ...
   subroutine chm_levSet_galerkin()
      ...
      real(rp) :: aux(ndime,npoin),aux4(ndime,npoin),aux2(npoin), aux3 , mod_grad
      real(rp) :: rc(npoin),res
      real(rp) :: factor
      ...
      do ipoin = 1,npoin
         aux_rhs(ipoin) = rg(ipoin,1)/ (dtinv)
      end do
   ...
   subroutine chm_pseudo_time_reinit_levSet()
      ...
      !call chm_multi_step_fs_eval_levSet_galerkin(2_ip)
      ...
      if (do_reinit == 1_ip) then 
         ...
         do iiter = 1, 2 
            ...
            call gradie(phi(1:npoin),grad)
            gradc(1:ndime,1:npoin) = 0.0_rp
            do ipoin=1,npoin
               gradc(1:ndime,ipoin) =-max(phi(ipoin)*(1.0_rp - phi(ipoin)),0.0_rp)*gradPhi00(1:ndime,ipoin)
               gradc(1:ndime,ipoin) = gradc(1:ndime,ipoin) + & 
                                      grad(1:ndime,ipoin)*epsil(ipoin)
                                      !(dot_product(grad(1:ndime,ipoin),gradPhi00(1:ndime,ipoin))*epsil(ipoin)*gradPhi00(1:ndime,ipoin))
            end do

            call divvec(gradc,compress)
            do ipoin = 1,npoin
               r(ipoin,1) = compress(ipoin)*vmass(ipoin)
            end do

            call rhsmod(1_ip,r(:,1))

            do ipoin = 1,npoin
               phi(ipoin) = phi00(ipoin) + ((3.0_rp/2.0_rp)*r(ipoin,1) - 0.5_rp*r(ipoin,2) ) / (vmass(ipoin)*pseudo_dtinv)
               r(ipoin,2) = r(ipoin,1)
            end do
   ...
   subroutine chm_reinit_allocate_levSet()
      ...
      use def_chemic, only      : kfl_ellen_chm,ADR_chm,        &
                                  ncomp_chm,nclas_chm
      ...
      integer(ip), save :: ipass = 0
      integer(ip)       :: idofn,idime
      integer(ip)       :: ielem, igaus, inode, ipoin
      integer(ip)       :: pelty,pnode,pgaus
      integer(ip)       :: plapl,porde,ptopo,jnode
      integer(4)        :: istat,kpoin,jpoin
      ...
      real(rp)          :: dummy(mnode,nclas_chm,1)
      real(rp)          :: dummy2(ndime,mnode)
      real(rp)          :: dummy3(mnode),mod_grad,mass_check
      real(rp)          :: elmat(mnode,mnode),aux 
      real(rp)          :: all_mass,aux_mass(mnode)
      ...
      if( ipass == 0 ) then
         ...
         if( INOTMASTER ) then
            ...
            call memory_alloca(mem_modul(1:2,modul),'AUX_RHS'    ,'chm_reinit_allocate_levSet',aux_rhs, npoin)
            ...
            call memory_alloca(mem_modul(1:2,modul),'AUX_RHS'    ,'chm_reinit_allocate_levSet',aux_rhs, 1_ip)
   ...
   subroutine matrix_jacobi_CSR(n1,n2,ia,ja,an,b,x)
      ...
      do iter=1,5
         call matrix_CSR_SpMV(1_ip,npoin,1_ip,1_ip,1_ip,1_ip,ia,ja,an,x,r)
   ...
   subroutine chm_gather_levSet(pnode,lnods,elcon,elcod,elvel,elgra)
     ...
     use def_chemic, only     :  nclas_chm,ADR_chm,lap_phi_levSet_chm, &
                                 grad_phi_levSet_chm,grad_phic_levSet_chm
     ...
     integer(ip)              :: inode,ipoin,iclas,idime,jpoin,jnode
   ...
   subroutine chm_element_operations_levSet_galerkin()
     ...
     use def_domain, only      : ndime,mnode,mgaus,nelem,ltype,nnode,     &
                                 ...
                                 hnatu,ntens
     use def_chemic, only      : kfl_ellen_chm,ADR_chm,nclas_chm,         &
                                 ncomp_chm,grad_phic_levSet_chm, &
                                 lap_phi_levSet_chm,kfl_fixno_chm
     ...
     integer(ip)               :: pelty,pnode,pgaus
     integer(ip)               :: plapl,porde,ptopo,knode,jpoin

     real(rp)                  :: elcon(mnode,nclas_chm,3)
     real(rp)                  :: elcod(ndime,mnode)                       
     real(rp)                  :: elrb(mnode),elrb2(mnode)
     ...
     real(rp)                  :: gprhs(mgaus)
     real(rp)                  :: chale(3),chave(3),hleng(3),tragl(9)
     real(rp)                  :: dummr(mgaus*ndime), c_eps
     real(rp)                  :: gpsha(mnode,mgaus)
     real(rp)                  :: aux,aux_min,aux_max
     real(rp)                  :: cij(ndime,mnode,mnode)
     real(rp)                  :: nij(ndime,mnode,mnode),nl2
     real(rp)                  :: dij(mnode,mnode), d1,d2, d3, phi_mean
     real(rp)                  :: Emax,Emin,elEres(mnode),E,Eold,Eold2
     real(rp)                  :: fact(mnode),fact2,gppr(mgaus),fact3
     real(rp)                  :: gpadv(mnode,mgaus),elres(mnode)
     real(rp)          :: elmat(mnode,mnode),gpb(mgaus),ellap(mnode,mnode)
   ...
   subroutine chm_element_operations_levSet()
     ...
     use def_domain, only      : ndime,mnode,mgaus,nelem,ltype,nnode,     &
                                 ...
                                 hnatu,ntens
     use def_chemic, only      : kfl_ellen_chm,ADR_chm,                    &
                                 ncomp_chm,nclas_chm,grad_phic_levSet_chm, &
                                 lap_phi_levSet_chm,kfl_fixno_chm,&
                                 diffu_chm,kfl_cotur_chm
    use mod_ker_proper 
     use mod_communications, only : PAR_MIN, PAR_MAX
      use mod_matrix
      use mod_solver
     ...
     real(rp)                  :: fact(mnode),fact2,gppr(mgaus),fact3
     real(rp)                  :: gpadv(mnode,mgaus),elres(mnode)
     real(rp)          :: elmat(mnode,mnode),gpb(mgaus),ellap(mnode,mnode), stab, ve, betae
     ...
     elements: do ielem = 1,nelem
        ...
        do igaus=1,pgaus
           do inode=1,pnode
              ...
              elEres(inode) = elEres(inode) + fact3*((elcon(inode,1,2)-elcon(inode,1,1))*dtinv+dot_product(elvel(1:ndime,inode),elgra(1:ndime,inode)))
           end do
        end do
        ve = 0.0_rp
        betae = 0.0_rp
        ...
        ellap = 0.0_rp
        !stab = min(0.5_rp*betae*aux,1.0_rp*ve*aux**2)
        ...
        stab = stab*max((1.0_rp - max(aux*(1.0-aux),0.0_rp)/(abs(aux_max-aux_min) + 1.0e-14_rp)), 0.0_rp)

        do igaus=1,pgaus
           do jnode=1,pnode
              do inode = 1,pnode
                 ellap(inode,jnode) = ellap(inode,jnode) + stab*dot_product(gpcar(1:ndime,inode,igaus), gpcar(1:ndime,jnode,igaus))*gpvol(igaus)
              end do
           end do
        end do
        ...
        if(kfl_cotur_chm < 0) then
           ...
           do igaus=1,pgaus
              do jnode=1,pnode
                 do inode = 1,pnode
                    ellap(inode,jnode) = ellap(inode,jnode) + (gpmuturb(igaus)/diffu_chm(1,1))*dot_product(gpcar(1:ndime,inode,igaus), gpcar(1:ndime,jnode,igaus))*gpvol(igaus)
                 end do
              end do
           end do
  ...
  subroutine chm_levSet_tau()
     ...
     real(rp)                  :: gpsha(mnode,mgaus)
     real(rp)                  :: aux,aux2
     real(rp)                  :: fact(mnode),fact2,gppr(mgaus)
