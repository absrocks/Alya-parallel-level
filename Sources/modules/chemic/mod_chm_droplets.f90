...
!------------------------------------------------------------------------
  ...
  subroutine chm_droplet_id()
    ...
    do iclus=1,nclus
       ...
       if ( compactness2_drop_chm(iclus) >= droplet_compactness_limit_chm .and. diameter_drop_chm(iclus) <= droplet_max_diameter_chm ) then
...
!------------------------------------------------------------------------
  ...
  subroutine chm_droplet_output()
    ...
    if( INOTSLAVE ) then
       ...
       do idrop=1,ndrop_chm
          write(lun_droplet_chm,130) idrop, volume_cluster_chm(idrop), volume_drop_chm(idrop), compactness2_drop_chm(idrop), diameter_drop_chm(idrop), (centroid_drop_chm(idime,idrop), idime=1,ndime)
       end do
  ...
  100 format('# --| ALYA Eulerian Droplet Data                ',/,&
         ...
         &   ' ')
  110 format('# Time step =', i9, '   Global Iteration =', i9, '   Inner Iteration =', i9, '   Current time =', e12.5, '   Total number of droplets =', i9)
