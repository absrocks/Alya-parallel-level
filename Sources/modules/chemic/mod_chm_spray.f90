...
contains 
   ...
   subroutine chm_elmpre_spray(ielem,pnode,pgaus,elvel,gpcar,gpcon,gpden,gptur,hleng,gpsigma)
     ...
     do igaus = 1,pgaus
        ...
        if ( sigma_gp_chm(ielem) % a(igaus,1,1) > 0.0_rp ) then
           d32_gp_chm(ielem) % a(igaus,1,1) = 4.0_rp * phi_clip * (1.0_rp - phi_clip) * max(0.0_rp,6.0_rp * phi_clip / sigma_gp_chm(ielem) % a(igaus,1,1))
        else
        ...
        if (kfl_cotur_chm < 0_ip) then
           ...
           if ( rtau_turb == 0.0_rp ) then
              ...
              !
              sigma_crit = max(0.0_rp,sigma0_gp_chm(ielem) % a(igaus,1,1) + phi_clip * (1.0_rp - phi_clip) * gpden(igaus) * k_sgs / surf_tension_chm)

              if ( sigma_crit == 0.0_rp ) then
                 gpsigma(igaus) = 0.0_rp
              else
                 gpsigma(igaus) = sigma_gp_chm(ielem) % a(igaus,1,1) * rtau_turb * ( 1.0_rp - sigma_gp_chm(ielem) % a(igaus,1,1) / sigma_crit )
              endif
   ...
   subroutine chm_elmprc_spray(&
     ...
     ! LES calculation
     !     
     if (kfl_cotur_chm < 0_ip .and. kfl_spray_chm == 1) then
