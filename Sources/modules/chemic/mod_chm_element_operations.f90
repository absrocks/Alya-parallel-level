...
contains
  ...
  subroutine chm_element_operations(order,pnode,pgaus,list_elements)
    ...
    real(rp)                         :: gprrt(mgaus,nclas_chm)                   ! Transport of reaction rate fluctuations
    ...
    elements: do kelem = 1,size(list_elements)
       ...
       if( ielem > 0 ) then
          ...
          call chm_elmgac_flamLet(&
               ielem,pnode,lnods(1,ielem),elcod,elcon(1:pnode,:,:),elvel,elmas,&
               elkey,eleps,elrrt)
          ...
          call chm_elmpre_flamLet(&
               ielem,pnode,pgaus,elcon(1:pnode,:,:),elvel,elmas,elkey,eleps,elrrt,elmar(pelty)%shape,&
               gpcar,gplap,gpcon(1:pgaus,:,:),gpvel,gpmas,hleng,gpdiv,gpdis(1:pgaus,:),&
               gpprd(1:pgaus,:),gprrt(1:pgaus,:),gptur,gpden,gphco,gpsph) 
          ...
          ASSEMBLY_ICLAS: do iclas = iclas_start,iclaf_chm
             ...
             !
                ...
                call chm_elmprc_flamLet(&
                     ...
                     gpprd(1:pgaus,:),gprrt(1:pgaus,:),gpdif(1:pgaus,1:nclas_chm),gpgrd,gprhs)
             ...
             !
             if(kfl_entropy_chm == 1_ip) then
             !!   call chm_entropy_viscosity(ielem,pnode,pgaus,1_ip,pgaus,iclas,elmar(pelty)%shape,gpcar,elvel,gpden,hleng,gpvol,gpdif)
