subroutine tur_elmmsu(&
  ...
  real(rp)                   :: Dsupg,umbra, CD, SD, factt, gplap, rhnvv, auxve(ndime), gppe2(pnode)
...
!               gpad1(inode) = gpad1(inode) + conve(idime,igaus) * gpcar(idime,inode,igaus)
              ...
              gpad2(inode) = gpad2(inode) + gpvel(idime,igaus) * gpcar(idime,inode,igaus)   
        ...
        do itime = 3,nbdfp_tur+1
           rhsit = gprhs(igaus) - fact2_time*dtinv_res * pabdf_tur(itime-1) * gpden(igaus) * gptur(iunkn_tur,itime,igaus)
        end do
        !
        ! Shock capturing
        !
           ...
           do inode = 1,pnode
              do jnode = 1,pnode
                 elmat(inode, jnode) = elmat(inode, jnode) + resid(jnode)* gpsha(inode,igaus)*gpvol(igaus) &
                      + gpadv(jnode)*tau*gpadv(inode) * gpvol(igaus)  &
