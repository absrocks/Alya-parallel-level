subroutine tur_outerr
  ...
  if(TUR_SPALART_ALLMARAS) then
     if( output_postprocess_check_variable_postprocess(1_ip) ) then
        iwarn=iwarn+1
        call outfor(2_ip,momod(modul)%lun_outpu,&
     ...
     if( output_postprocess_check_variable_postprocess(2_ip) ) then
        iwarn=iwarn+1
        call outfor(2_ip,momod(modul)%lun_outpu,&
     ...
     if( output_postprocess_check_variable_postprocess(3_ip) ) then
        iwarn=iwarn+1
        call outfor(2_ip,momod(modul)%lun_outpu,&
  ...
  if(nturb_tur==1.and. output_postprocess_check_variable_postprocess(16_ip) ) then
     iwarn=iwarn+1
     call outfor(2_ip,momod(modul)%lun_outpu,&
  ...
  if(.not.TUR_SPALART_ALLMARAS) then
     if( output_postprocess_check_variable_postprocess(4_ip) ) then
        iwarn=iwarn+1
        call outfor(2_ip,momod(modul)%lun_outpu,&
