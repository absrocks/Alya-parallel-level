subroutine tur_kowilc(&
  ...
  integer(ip), parameter     :: WIL88 =1_ip, WIL98=2_ip, WIL06=3_ip
  ...
  if ( ipara_tur(1) == WIL06 ) then ! only for k-w 06 
     Clim = 7.0_rp/8.0_rp
  else 
  ...
  ! turb visco
  omehat = max(gpome, Clim*sqrt(gppr2/bast0))  ! = gpome for k-w 88 and 98
  omehao = max(gpomo, Clim*sqrt(gppr2/bast0)) 
  gpmut(1) = max(gpden*gpkin/omehat, 0.000001_rp*gpvis) ! varies along iterations (nonlinear term)
  gpmut(2) = max(gpden*gpkio/omehao, 0.000001_rp*gpvis) ! frozen along iterations
  ! TKE production 
  gppro = gpmut(2) * gppr2

  ! diffusive term = rho *K/w
  if (ipara_tur(1).ne.WIL06) then
     diffu = gpmut(2)
  else   
     diffu = gpden*gptur(1,iunkn_tur)/gptur(2,iunkn_tur)
  end if
  ...
  if(iunkn_tur==1) then ! TKE equation
     ...
     if( ipara_tur(1) == WIL88.or.ipara_tur(1)==WIL06) then
        beast = bast0  ! beta_ast
     else if ( ipara_tur(1) == WIL98 ) then !load beta_ast
        gradk = 0.0_rp
        gradw = 0.0_rp
        do inode = 1,pnode
           do idime = 1,ndime
...
!     print *, 'balance', gprhs - gprea*gpkin, gprhs
     ...
     if ( ipara_tur(1) == WIL88) then
        gamma = 5.0_rp/9.0_rp 
        beta  = 3.0_rp/40.0_rp
        gammap = gamma + (beta/bast0 - gamma)*lm*inv_l_max
        gprhs = gammap*gpden*gppr2 
     else if ( ipara_tur(1) == WIL98.or.ipara_tur(1)==WIL06) then
        ...
        if ( ipara_tur(1) == WIL98) then
           ...
           gammap = gamma + (beta/bast0 - gamma)*lm*inv_l_max
           gprhs  = gammap*gpden*gppr2    
        else if ( ipara_tur(1) == WIL06) then
           beta0 = 0.0708_rp
