...
module mod_pts_transport
  ...
  use def_master,              only : ISLAVE,IMASTER,IPARALL,kfl_paral,INOTEMPTY
    ...
    do while( t < tf-dtmin_pts .and. inscont )
       ...
       if( kfl_thermo_pts /= 0 ) then
          call pts_thermodynamic_transport(               &
               ...
               lnods(1:pnode,ielem),shapf,dt_k,mass_kp1,tempe_kp1,conv_heat_flux)
       end if
       ...
       if( ifoun /= 0 ) then          
          ...
          call pts_transport_special_walls(&
               lagrtyp(ilagr),ielem,pnode,shapf,diame,coord_kp1,veloc_kp1)          
          !
          ! Residence time
          !
...
20     continue
       ...
       if( ( dt_k < 0.1_rp * dtmin_pts .or. itint > itmax ) .and. iwall /= 1 ) then
          ...
          if( .not. newmark ) then                 
             print '(A,I8,A,3(1X,E18.8),A,1X,E18.8)', 'Newmark did not converge after all iteraitons. Particle id=', lagrtyp(ilagr) % ilagr,'; coords=',lagrtyp(ilagr) % coord(1:ndime),'; diameter=',diame 
          end if
       end if
    ...
    if( INOTMASTER ) then
       do ilagr = 1,mlagr
          ...
          if( lagrtyp(ilagr) % kfl_exist == PTS_PARTICLE_HITS_WALL .or. &
              ...
              lagrtyp(ilagr) % kfl_exist == PTS_PARTICLE_EVAPORATED) then
             !
...
112 format((e12.6,2x,4(2x,i7),3(2x,e12.6),3(2x,i7),66(2x,e12.6)))
  ...
  subroutine pts_transport_special_walls(&
       particle,ielem,pnode,shapf,diame,coord_kp1,veloc_kp1)
    ...
    integer(ip)                :: idime
    real(rp)                   :: ubas(3)
    real(rp)                   :: dista,bnorm,radius
    real(rp)                   :: basis(ndime,ndime)
    ...
    if( kfl_slip_wall_pts > 0 ) then
       ...
       if( dista <= radius ) special_condition = PTS_SLIP_CONDITION
    end if
    if( kfl_bouncing_wall_pts > 0 ) then
       dista = dot_product(shapf(1:pnode),walld_bouncing_pts(lnods(1:pnode,ielem)))
       if( dista <= radius ) special_condition = PTS_BOUNCING_CONDITION
    end if
