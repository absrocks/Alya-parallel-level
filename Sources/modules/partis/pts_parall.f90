subroutine pts_parall(order)
  ...
  integer(ip)             :: ii,dummi,jj, kk
  integer(ip)             :: size_injve_pts
  integer(ip)             :: size_injte_pts
  integer(ip)             :: size_injma_pts
  ...
  case( 1_ip )
     ...
     size_injve_pts = size(kfl_injve_pts,KIND=ip)
     size_injte_pts = size(kfl_injte_pts,KIND=ip)
     size_injma_pts = size(kfl_injma_pts,KIND=ip)
     ...
     do parii = 1,2 
        ...
        do ii = 1,mtyla
           ...
           call PAR_EXCHANGE(parttyp(ii) % d_min,      parre,nparr,parii) 
        ...
        call PAR_EXCHANGE(size_injve_pts,kfl_injve_pts,parin,npari,parii)        
        call PAR_EXCHANGE(size_injte_pts,kfl_injte_pts,parin,npari,parii) 
        call PAR_EXCHANGE(size_injma_pts,kfl_injma_pts,parin,npari,parii)
        call PAR_EXCHANGE(size(kfl_injVelocFunction_pts,KIND=ip),kfl_injVelocFunction_pts,parin,npari,parii)
        call PAR_EXCHANGE(size(kfl_injMassFunction_pts,KIND=ip),kfl_injMassFunction_pts,parin,npari,parii)
        call PAR_EXCHANGE(size(kfl_injsd_pts,KIND=ip),kfl_injsd_pts,parin,npari,parii) 
        ...
        do ii = 1,pts_minj
           do jj = 1,mpara_inj
              call PAR_EXCHANGE(param_veloc_pts(jj,ii),parre,nparr,parii)
              call PAR_EXCHANGE(param_tempe_pts(jj,ii),parre,nparr,parii)
              call PAR_EXCHANGE(param_mass_pts (jj,ii),parre,nparr,parii)
              call PAR_EXCHANGE(param_sized_pts(jj,ii),parre,nparr,parii)
           end do
        end do
  ...
  do ii = 1,mtyla
     if (parttyp(ii) % kfl_therm == 2) then 
        call tab_par_exchange(parttyp(ii) %h_scaling_coords,  parttyp(ii) %h_scaling_tab) 
        call tab_par_exchange(parttyp(ii) %yc_scaling_coords, parttyp(ii) %yc_scaling_tab) 
        call tab_par_exchange(parttyp(ii) %table_coords,      parttyp(ii) %table_tab) 

        if (parttyp(ii) % kfl_tab_fw == 0) then
            if (ISLAVE) then
                allocate(parttyp(ii) % table_fw)
                call tab_init_fw(parttyp(ii) % table_fw)
            endif
            call fw_par_exchange(1_ip,parttyp(ii) % table_fw)
            parttyp(ii) % table_fw % main_table => parttyp(ii) % table_tab
            call fw_par_exchange(2_ip,parttyp(ii) % table_fw)
            if (ISLAVE) call fw_allocate(2_ip,parttyp(ii) % table_fw,0_ip)
            do jj = 1, parttyp(ii) % table_fw % main_table % ndim
                if (parttyp(ii) % table_fw % kfl_scale(jj) == 1) then 
                    select case (parttyp(ii) % table_fw % main_table % coords(jj) % name)
                    case ('CMEAN','C    ')
                       parttyp(ii) % table_fw % scaling(jj) % tab => parttyp(ii) % yc_scaling_tab
                    case ('IMEAN','I    ')
                       parttyp(ii) % table_fw % scaling(jj) % tab => parttyp(ii) % h_scaling_tab
                    end select
                endif
                if (ISLAVE) call fw_allocate(2_ip,parttyp(ii) % table_fw,jj)
            enddo
            
        else if(parttyp(ii) % kfl_tab_fw > 0) then
