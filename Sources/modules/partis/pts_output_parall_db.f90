...
subroutine pts_output_parall_db()
  ...
  integer(ip)           :: kfl_ifpos,kfl_ifdep,iprop,ilimi,pos_set
  integer(ip)           :: pos_tim,isend
  integer(4)            :: nvar2_4
  integer(ip), save     :: ittim_last=-1
  integer, dimension(8) :: values
  real(rp)              :: dt1,dt2,dt3,cutim_pts
           ...
           do ilagr = 1,mlagr
              if( lagrtyp(ilagr) % kfl_exist <= ilimi ) then
                 ...
                 if( kfl_posla_pts >= 4 ) then
                    do iprop=1,nlapr
                       ipars             = ipars + 1
                       sendbuf_rp(ipars) = lagrtyp(ilagr) % prope(iprop) ! 29 + nlapr
                    end do
                 endif
                 ipars             = ipars + 1
                 sendbuf_rp(ipars) = real(lagrtyp(ilagr) % boundary_set,rp) ! 29 + nlapr + 1
                 ipars             = ipars + 1
                 sendbuf_rp(ipars) = lagrtyp(ilagr) % t                     ! 29 + nlapr + 2
              end if
           end do
#ifdef DBPARTICLES
           if( kfl_ifpos == 1 ) then
