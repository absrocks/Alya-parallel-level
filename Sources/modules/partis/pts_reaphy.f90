...
subroutine pts_reaphy()
  ...
  implicit none
  integer(ip) :: itype,iprop,order_loc,ii
  ...
  if( INOTSLAVE ) then
     ...
     do itype = 1,mtyla
        ...
        parttyp(itype) % d_min     = 0.0_rp                 ! Minimum diameter below which droplets are considered fully evaporated
              ...
              do while(words(1) /= 'ENDTY' )
                 ...
                 if( words(1) == 'MODEL' .or. words(1) == 'MOMEN' ) then
                    ...
                    if( exists('CANTE') ) then
                        ...
                        !
                        ! nclas_pts will have to be read from the table
                        !
                    endif
                    ...
                    ! destroy the droplet. 
                    !
                    parttyp(itype) % d_min = getrea('DMINI',0.0_rp,'#PARTICLE MINIMUM DIAMETER')
                    ...
                    !
                    ! Input table for flamelet gas phase
                    !
                    if (exists('ORDER')) then
                       order_loc = getint('ORDER',1_ip,'#Order of interpolation.')
                    endif
                    if(parttyp(itype) % kfl_therm == 2) then
                        call tab_load_file(parttyp(itype) % table_coords, parttyp(itype) % table_tab, order=order_loc) 
                        parttyp(itype) % kfl_tab_fw = 0 
                        allocate( parttyp(itype) % table_fw )
                        call tab_init_fw(parttyp(itype) % table_fw)
                        parttyp(itype) % table_fw % main_table => parttyp(itype) % table_tab
                        call fw_allocate(1_ip,parttyp(itype) % table_fw)

                        !
                        ! Decide scaling method
                        !
                        parttyp(itype) % table_fw % kfl_scale = 0_ip
                        do ii = 1, parttyp(itype) % table_fw % main_table % ndim
                            if (parttyp(itype) % table_fw % main_table % coords(ii) % leng > 1) then 
                               select case (parttyp(itype) % table_fw % main_table % coords(ii) % name)
                               case ('CMEAN','C    ')
                                  parttyp(itype) % table_fw % kfl_scale(ii) = 1
                               case ('CVAR ')
                                  parttyp(itype) % table_fw % kfl_scale(ii) = 100 + ii - 1
                               case ('CHIST')
                                  parttyp(itype) % table_fw % kfl_scale(ii) = 1
                               case ('ZMEAN','Z    ')
                                  parttyp(itype) % table_fw % kfl_scale(ii) = 0
                               case ('ZVAR ')
                                  parttyp(itype) % table_fw % kfl_scale(ii) = 100 + ii - 1
                               case ('IMEAN','I    ')
                                  parttyp(itype) % table_fw % kfl_scale(ii) = -1
                               case default
                                  call runend('pts_reaphy: cannot understand coordinate name: ' // parttyp(itype) % table_fw % main_table % coords(ii) % name)
                               end select
                            else
                               parttyp(itype) % table_fw % kfl_scale(ii) = -1
                            endif
                        ...
                        !
                        call fw_allocate(2_ip,parttyp(itype) % table_fw,0_ip)
                    endif
                    do while( words(1) /= 'ENDTA' )
                        call ecoute('pts_reaphy')
                    end do
                
                else if( words(1) == 'YCSCA' ) then
                    !
                    ! Input table for scaling progress variable
                    !
                    if(parttyp(itype) % kfl_therm == 2) then
                        call tab_load_file(parttyp(itype) % yc_scaling_coords, parttyp(itype) % yc_scaling_tab, order=order_loc)
                        do ii = 1, parttyp(itype) % table_fw % main_table % ndim
                            if  (parttyp(itype) % table_fw % main_table % coords(ii) % name == 'CMEAN' .or. parttyp(itype) % table_fw % main_table % coords(ii) % name == 'C    ') then
                               parttyp(itype) % table_fw % scaling(ii) % tab => parttyp(itype) % yc_scaling_tab
                               call fw_allocate(2_ip,parttyp(itype) % table_fw,ii)
                            endif
                        enddo
                    endif
                    do while( words(1) /= 'ENDYC' )
                        call ecoute('pts_reaphy')
                    end do

                else if( words(1) == 'HEATL' ) then
                    !
                    ! Input table for scaling enthalpy
                    !
                    if(parttyp(itype) % kfl_therm == 2) then
                        call tab_load_file(parttyp(itype) % h_scaling_coords, parttyp(itype) % h_scaling_tab, order=order_loc)
                        do ii = 1, parttyp(itype) % table_fw % main_table % ndim
                            if  (parttyp(itype) % table_fw % main_table % coords(ii) % name == 'IMEAN' .or. parttyp(itype) % table_fw % main_table % coords(ii) % name == 'I    ') then
                               parttyp(itype) % table_fw % scaling(ii) % tab => parttyp(itype) % h_scaling_tab
                               parttyp(itype) % table_fw % kfl_scale(ii) = 1
                               call fw_allocate(2_ip,parttyp(itype) % table_fw,ii)
                            endif
                        enddo
