...
subroutine pts_cross_wall(&
  ...
  use def_domain, only : lnnod,walld,mnode
  ...
  use def_partis, only : walld_slip_pts
  ...
  integer(ip)                   :: pnodb,kboun,iinte,pblty,ipoin,idime
  real(rp)                      :: facod(ndime,mnodb),p2(ndime),dista
  real(rp)                      :: s1,s2,udotn,adotn,vect(ndime),p1(ndime)
  real(rp)                      :: norma,vect_normal(ndime),caca1,caca2
     ...
     if( iinte /= 0 ) then
        ...
        if(      kfl_fixbo_pts(iboun) == PTS_OUTFLOW_CONDITION ) then
           ...
           if( bvnat_pts(1,iboun) > zeror ) then
              ...
              !print*,'ENTROOOO A LA WALL CON IBOUN',iboun
              coord_kp1(1:ndime) = xinte(1:ndime)
              if( nbset > 0 ) lagrtyp(ilagr) % boundary_set = lbset(iboun)
           end if
           return
           ...
           ! v => v-2(v.n)n
           !
           !udotn              = dot_product(bouno_pts(1:ndime,iboun),veloc_kp1(1:ndime))
           !t                  = t - dt_k
           !dt_k               = dt_k * sqrt(dot_product(xinte(1:ndime)-coord_kp1(1:ndime),xinte(1:ndime)-coord_kp1(1:ndime))) &
           !     &                    / (sqrt(dot_product(vect(1:ndime),vect(1:ndime)))+zeror)
           !t                  = t + dt_k
           !veloc_kp1(1:ndime) = veloc_kp1(1:ndime) - 2.0_rp * udotn * bouno_pts(:,iboun)
           !coord_kp1(1:ndime) = xinte(1:ndime)
           !return
        ...
        if( nbset > 0 ) then
           ...
           if( memory_size(leleboun_pts(ielem) % l) > 0 ) then
              ...
              do iboun = 1,nboun
                 pnodb = lnnob(iboun)
                 do idime = 1,ndime                 
                    xboun(idime) = sum(coord(idime,lnodb(1:pnodb,iboun)))
              ...
              if( kboun > 0 ) lagrtyp(ilagr) % boundary_set = lbset(kboun)
           end if
