...
subroutine pts_brown(itint,ielem,itype,pnode,shapf,dt_k,r,mu,rho,Cc,D,epsi1,epsi2,epsi3)
  ...
  use def_partis, only : parttyp,lagrtyp
  ...
  real(rp)                   :: Temp,CBrown,denpa,diame,U1,U2,U3,U4,S_0,mass
  ...
  if( itint == 1 )then
     if( ndime == 2 )then
        U1    = random_generate_number()
        U2    = random_generate_number()
        ...
        U1    = random_generate_number()
        U2    = random_generate_number()
        U3    = random_generate_number()
        U4    = random_generate_number()
        !call random_number(U1)
        !call random_number(U2)
