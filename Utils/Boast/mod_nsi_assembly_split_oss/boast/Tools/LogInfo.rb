...
module LogInfo
  ...
  def self.get_info
    ...
    #MAC
    #@doc[:body][:environment][:hardware][:cpu] = `sysctl -n machdep.cpu.brand_string`
#    @doc[:body][:environment][:hardware][:cpu] = `system_profiler SPHardwareDataType`
...
#    @doc[:body][:environment][:software][:users] = `who`
  ...
  def self.dump_info
    File::open( @path, "w") { |f|
    #File::open( "/Users/emmanuellesaillard/Documents/POSTDOC_INRIA/BOAST_tests/alya-boast/src/boast/Split/TEST/resfile.txt", "w") { |f|
