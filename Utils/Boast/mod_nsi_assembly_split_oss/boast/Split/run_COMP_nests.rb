...
class Experiment
  ...
  nests.each{|n|
    ...
    puts optimizer.optimize { |opt|
      ...
      k_ref_params = {:kernel => :ref, :vector_length => opt[:vector_size], :preprocessor => false, :nests => [n], :unroll => false, :inline => opt[:inline], :CFLAGS => "#{opt[:OFLAGS]} -cpp", :FCFLAGS => "-fimplicit-none #{opt[:OFLAGS]}", :LDFLAGS => "-lgfortran"}
      k_boast_params = {:kernel => :boast, :vector_length => opt[:vector_size], :preprocessor => false, :nests => [n], :unroll => opt[:unroll], :inline => opt[:inline], :CFLAGS => "#{opt[:OFLAGS]} -cpp", :FCFLAGS => "-fimplicit-none #{opt[:OFLAGS]}", :LDFLAGS => "-lgfortran"}
      ...
      #CommonArgs.init(charac[:pgaus],charac[:pnode],dimension,vector_size,1,seed)
			ndime=3
...
stats, kernels = Experiment.run(options[:kernel_path], options[:lang])
