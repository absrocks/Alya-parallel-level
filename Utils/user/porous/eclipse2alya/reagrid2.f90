!  cd /Users/howen/svn/Alya/Utils/user/porous/eclipse2alya && ./com_gfort.sh


subroutine reagrid2(coord,zcorn,ndi,ndime,nzerom,nz1,nz2)
  use def_kintyp
  implicit none
  ...
  integer(ip)               :: nelemtomod,kprism(2,6),npnew2
  integer(ip),allocatable   :: kzeromn(:)
  integer(ip),allocatable   :: kelemtomod(:),kfl_usedn(:),lp2(:)
  integer(ip),allocatable   :: lnadit(:,:)
  integer(ip),allocatable   :: elem_ijk(:,:), poin_ijk(:,:)
  integer(ip),allocatable   :: iel_orig(:)
  integer(ip),allocatable   :: itouched(:,:),linel(:)
  integer(ip),allocatable   :: mink(:,:)
  integer(ip)               :: nlinel
  real(rp)                  :: perm_conver
  ...
  allocate(itouched(ndi(1)+1,ndi(2)+1))
  allocate(mink(ndi(1)+1,ndi(2)+1))
...
!           read(10,*)linea
  ...
  kfl_shift(8)=7
!
! Futher cleaning - unused nodes
!
  allocate(kfl_usedn(npnew))
  allocate(lp2(npnew))
  allocate(linel(npnew))
  kfl_usedn = 0
  lp2 = 0
  linel = 0
!
  ...
  do ipoin=1,npnew
     if ( kfl_usedn(ipoin) /= 0 ) write(11,'(i9,3(2x,e17.10))'),lp2(ipoin),(co_alya(idime,ipoin),idime=1,3)
  end do
  ...
  nlinel = 0
  do ipoin=1,npnew
     if ( kfl_usedn(ipoin) /= 0 ) then
        mink ( poin_ijk(ipoin,1),poin_ijk(ipoin,2) ) = min ( mink ( poin_ijk(ipoin,1),poin_ijk(ipoin,2) ) , poin_ijk(ipoin,3) )
        if ( itouched(poin_ijk(ipoin,1),poin_ijk(ipoin,2)) == 0 )  then 
           nlinel = nlinel + 1
           itouched(poin_ijk(ipoin,1),poin_ijk(ipoin,2)) = nlinel
        end if
        linel(ipoin) = itouched(poin_ijk(ipoin,1),poin_ijk(ipoin,2)) 
     end if
  end do

  open(26,file='linel.dat')
  do ipoin=1,npnew
     if ( kfl_usedn(ipoin) /= 0 ) write(26,'(3(i9,2x))'),lp2(ipoin),linel(ipoin), &
          poin_ijk(ipoin,3) + 1 - mink ( poin_ijk(ipoin,1),poin_ijk(ipoin,2) )
  end do
  close(26)
  !
  ! Zero mass file
...
!Lo s nodos  de zero mass son los que lueg indican que hay que partir el hexa en prismas
  ...
  do ielem=1,nelem
     if (kelemtomod(ielem) == 0) then
        write(11,'(9(i9,2x))'),ielem,(lp2(lpopo(lnods(kfl_shift(inode),ielem))),inode=1,8)
     else
        write(11,'(9(i7,2x))'),ielem,(lp2(lpopo(lnods(kfl_shift(kprism(1,inode)),ielem))),inode=1,6)
        nelemtomod = nelemtomod + 1
        iel_orig(nelemtomod) = ielem
        do inode=1,6
           lnadit(inode,nelemtomod) = lp2(lpopo(lnods ( kfl_shift( kprism(2,inode) ) ,ielem) ) )
        end do
     end if
  end do
  ...
  perm_conver = 9.869233e-16

  do ielem=1,nelem
     write(14,'(i9,3(2x,e17.10))'), ielem, perm_conver * perm(elem_ijk(ielem,1),elem_ijk(ielem,2),elem_ijk(ielem,3),1:ndime)
  end do
  do ielem=1,nelemtomod
     write(14,'(i9,3(2x,e17.10))'), nelem+ielem, perm_conver * perm(elem_ijk(iel_orig(ielem),1),elem_ijk(iel_orig(ielem),2), &
...
! write(26,'(a)') 'WELLS'
  do ipoin=1,npnew
     if ( kfl_usedn(ipoin) /= 0 ) & 
         write(26,'(i9,i9)'), lp2(ipoin), iabs(welli(poin_ijk(ipoin,1),poin_ijk(ipoin,2),poin_ijk(ipoin,3)) )
  end do
