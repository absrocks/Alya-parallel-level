...
# chec parameters
if [ "$#" -ne 6 ]; then 
    echo "Parameters it must be 6: [case name],[iteration],[relax factor/10],[refbound],[spmv/assembly],[nparts]"
    exit 1
fi 
...
if [ $phase != "assembly" ] && [ $phase != "spmv" ]; then 
   echo "   ERROR: 5th argument must be <spmv> or <assembly> "
   exit 1 
fi
if (( $npart < 2 )); then 
...
if (( $iter \< 10 )); then
   ...
   fileres="$dir/$1-"0"$iter-partition.par.post.res"
   filelog="$dir/$1-$phase"0"$iter.log"
else
   fileimb="$dir/$1-$phase$part$iter.imb"
   filecor="$dir/$1-$phase$part$iter.cor"
   fileent="$dir/$1-$phase$part$iter.ent"
   fileres="$dir/$1-$iter-partition.par.post.res"
   filelog="$dir/$1-$phase$iter.log"
...
imbalan=`bc <<<"scale=8; 1.0 - ($avgtime/$maxtime)" | awk '{printf "%08f\n", $0}'`

maxbound=`awk -v ref="$refbound" -v imb="$imbalan" 'BEGIN {BOUND=1+ref}; END{if (imb/2<ref){BOUND=1+(imb/2)}; printf "%08f\n", BOUND}'`
minbound=`awk -v ref="$refbound" -v imb="$imbalan" 'BEGIN {BOUND=1-ref}; END{if (imb/2<ref){BOUND=1-(imb/2)}; printf "%08f\n", BOUND}'`
...
cp $filecor $alyafile
cp $1.log $filelog
cp $1-partition.par.post.res $fileres
...
echo -e "Bounds:       \t\t\t[$minbound, $maxbound]"
echo "-----------------------------------------------------------------------------------"
