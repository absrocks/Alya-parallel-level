...
!----------------------------------------------------------------------
  subroutine PAR_MEASURE_TIME(itask, name)
    ...
    use def_master,           only :  ini_tim, fin_tim, mem_servi, servi, init_ti, fini_ti, crate_ti, cmax_ti
    ...
    if(  .not. associated(tim_difs) .and. PAR_MY_CODE_RANK == 0_ip ) call memory_alloca( mem_servi(1:2,servi), 'tim_difs', 'par_measure_time', tim_difs, npart+1_ip )
