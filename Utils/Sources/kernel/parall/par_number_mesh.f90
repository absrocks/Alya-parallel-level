subroutine par_number_mesh(itask)
  ...
  integer(ip)              :: ipoin,jpoin,ielem,kelem,ii,kpoin
  ...
  case ( 1_ip )
     ...
     ! Copy old LNINV_LOC
     !
     call memory_alloca(mem_servi(1:2,servi),'LNINV_TMP','par_submsh',lninv_tmp,npoin_old) 
     do ipoin = 1,npoin_old
        lninv_tmp(ipoin) = lninv_loc(ipoin)
     end do
     call memory_deallo(mem_servi(1:2,servi),'LNINV_LOC','par_submsh',lninv_loc)
     call memory_alloca(mem_servi(1:2,servi),'LNINV_LOC','par_submsh',lninv_loc,npoin)
     do ipoin = 1,npoin_old
        lninv_loc(ipoin) = lninv_tmp(ipoin) 
     end do
     call memory_deallo(mem_servi(1:2,servi),'LNINV_TMP','par_submsh',lninv_tmp)
  ...
  case ( 2_ip )
     ...
     call memory_alloca(mem_servi(1:2,servi),'ONW_NODES','par_submsh',own_nodes,npart+1_ip,'INITIALIZE',0_ip)
     ...
     call memory_deallo(mem_servi(1:2,servi),'ONW_NODES','par_submsh',own_nodes)
