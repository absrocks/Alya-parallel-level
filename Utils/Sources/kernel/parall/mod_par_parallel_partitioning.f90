...
contains
  ...
  subroutine par_parallel_partitioning()

    use def_domain
    use mod_memory
    use def_coupli
    use def_mpio, only : kfl_mpio_input, IO_CLASSIC, IO_DISABLED, IO_MPIO_PAR, IO_MPIO_POST, IO_MPIO_SEQ

    integer(ip)             :: pnode,npart_sav,ifiel,ipart
    ...
    if(      kfl_mpio_input == IO_CLASSIC .or. kfl_mpio_input == IO_MPIO_SEQ ) then
       !
       ! Normal sequential reading, data not scattered yet
       !
       call par_partition_scatter_from_master()

    else if( kfl_mpio_input == IO_MPIO_PAR ) then
       !
    ...
    if( INOTMASTER .and. ( kfl_partition_par == PAR_SFC .or. kfl_partition_par == PAR_ORIENTED_BIN ) ) then
       call memory_alloca(par_memor,'XCOOR','par_parallel_partitioning',xcoor,ndime,nelem)
       if(      kfl_partition_par == PAR_SFC ) then
          call memory_alloca(par_memor,'RWEIG','par_parallel_partitioning',rweig,nelem)
          call par_partition_weights(rweig)
    ...
    if( INOTMASTER ) then
       ...
       if( kfl_partition_par < 0 ) then
          ...
          call PAR_MAX(max_subd,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
          !if( max_subd /= npart ) call runend('PAR_PARALLEL_PARTITIONING: WRONG FIELD... NOT SAME NUMBER OF SUBDOMAINS')
       else
          !
          ! Normal partitioning
          !
          npart_sav = npart
          call partitioning(&
               kfl_partition_par,nelem,npart,lepar_par,lweig=lweig,&
               rweig=rweig,COMM4=PAR_COMM_MY_CODE_WM4,ia=ia,ja=ja,lmask=lmask,&
               xcoor=xcoor,boxes_coarse=boxes_coarse_par,boxes_fine=boxes_fine_par,&
               direction=vect_partition_par,ndime_=ndime)
          if( npart_sav /= npart ) &
    ...
    if( kfl_partition_par /= PAR_USING_RANK ) then
       ...
       call redistribution_initialization(PAR_COMM_WORLD)
       call redistribution_domain(lepar_par,VERBOSE=.false.)
       call messages_live('REDISTRIBUTE GEOMETRY AMONG SLAVES','END SECTION')
    end if
    !if( INOTMASTER .and. nelem == 0 ) call runend('PAR_PARALLEL_PARTITIONING: ONE SUBDOMAIN DOES NOT HAVE ELEMENTS, CHECK YOUR PARTITIONER OPTIONS')
    ...
    call messages_live('CONSTRUCT COMMUNICATION ARRAYS')
  ...
  subroutine par_partition_weights_rp(rweig,TOTAL_WEIGHT)
    ...
    if( kfl_weigh_par == PAR_WEIGHT_GAUSS  ) then
       ...
       do ielem = 1,nelem
          ...
          if( present(rweig) ) rweig(ielem) =  real(ngaus(ielty),rp)
          if( present(TOTAL_WEIGHT) ) TOTAL_WEIGHT = TOTAL_WEIGHT + real(ngaus(ielty),rp)
       end do
       ...
       do ielem = 1,nelem
          ielty = abs(ltype(ielem))
          if( present(rweig) ) rweig(ielem) =  real(ngaus(ielty),rp)**2
          if( present(TOTAL_WEIGHT) ) TOTAL_WEIGHT = TOTAL_WEIGHT + real(ngaus(ielty),rp)**2
       end do
  ...
  subroutine par_partition_weights_ip(lweig)
    ...
    if( kfl_weigh_par == PAR_WEIGHT_GAUSS ) then
       ...
       do ielem = 1,nelem
          ielty = abs(ltype(ielem))
          lweig(ielem) = ngaus(ielty)*ngaus(ielty)
       end do
  ...
  subroutine par_partition_total_weight_ip(TOTAL_WEIGHT)
    ...
    if( kfl_weigh_par == 0 ) then
       ...
       do ielem = 1,nelem
          ielty = abs(ltype(ielem))
          TOTAL_WEIGHT = TOTAL_WEIGHT + ngaus(ielty)
       end do
       ...
       do ielem = 1,nelem
          ielty = abs(ltype(ielem))
          TOTAL_WEIGHT = TOTAL_WEIGHT + ngaus(ielty)*ngaus(ielty)
       end do
  ...
  subroutine par_partition_scatter_from_mpio()
    ...
    if( INOTMASTER ) then
      ...
      call htaini( ht, max_npoin, lidson=.false., AUTOMATIC_SIZE=.true.)
       ...
       call generate_comm_chunkdist(list_npoin,chunk_npoin,PAR_COMM_MY_CODE_WM4,comm_npoin,dist_dom='SOURCE')
    ...
    if( INOTMASTER ) then       
       ...
       do ifiel = 1,nfiel
          if( kfl_field(2,ifiel) == NPOIN_TYPE ) then
             call redistribution_array_RP_3(xfiel(ifiel) % a,'NPOIN',POSIT=2_ip,MEMOR=memor_dom,VARIABLE_NAME='FIELD',COMM=comm_npoin,PERMUTE_RECV=.true.)
          end if
       end do
    ...
    if( INOTMASTER ) then
       ...
       !   rank = int(PAR_MY_WORLD_RANK_WM,ip)
       ...
       call generate_comm_chunkdist(list_nelem,chunk_nelem,PAR_COMM_MY_CODE_WM4,comm_nboun,dist_dom='TARGET')
    ...
    if( INOTMASTER ) then
       ...
       do ifiel = 1,nfiel
          if( kfl_field(2,ifiel) == NBOUN_TYPE ) then
             call redistribution_array_RP_3(xfiel(ifiel) % a,'NBOUN',POSIT=2_ip,MEMOR=memor_dom,VARIABLE_NAME='FIELD',COMM=comm_nboun,PERMUTE_RECV=.false.)
          end if
       end do
    end if
    ...
    if( INOTMASTER ) then
       call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_npoin,par_memor)
       call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm_nboun,par_memor)
  ...
  subroutine par_partition_lelbo_old(chunk_nelem)
    ...
    call PAR_ALLGATHER(ipoin_min_lnods,ipoin_min_lnods_gat,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
    call PAR_ALLGATHER(ipoin_max_lnods,ipoin_max_lnods_gat,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
    ...
    do ipart = 0,npart-1
       ...
       call PAR_SEND_RECEIVE(isend,irecv,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
       if( irecv == 1 ) then
    ...
    call PAR_ALLTOALL(1_ip,1_ip,nelem_send,nelem_recv,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
    ...
    do ipart = 0,npart-1
       if( nelem_send(ipart) /= 0 .or. nelem_recv(ipart) /= 0 ) then
          ...
          call PAR_SEND_RECEIVE_IP(nelem_send(ipart)      ,nelem_recv(ipart)      ,ltype,ltype_typ(ipart) % l,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
          call PAR_SEND_RECEIVE_IP(nelem_send(ipart)*mnode,nelem_recv(ipart)*mnode,lnods,lnods_typ(ipart) % l,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
          call PAR_SEND_RECEIVE_IP(nelem_send(ipart)      ,nelem_recv(ipart)      ,lperm,lperm_typ(ipart) % l,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
       end if
    end do
  ...
  subroutine par_partition_lelbo(chunk_nelem,chunk_npoin,htable_npoin)
    ...
    use mod_htable,          only : htades
    use mod_htable,          only : htalid
    use mod_memory,          only : memory_copy,mem_curre
    ...
    integer(ip)                      :: icoun1,icoun2,icoun3,iboun
    integer(ip)                      :: ipoin,inode,npoin_loc,max_npoin
    integer(ip)                      :: ipoin_loc,ii
    ...
    call PAR_ALLGATHER(comin,comin_gat,3_4,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
    call PAR_ALLGATHER(comax,comax_gat,3_4,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
    ...
    do ipart = 0,npart-1
       if( nelem_send(ipart) /= 0 .or. nelem_recv(ipart) /= 0 ) then
          ...
          call PAR_SEND_RECEIVE_IP(nelem_send(ipart)      ,nelem_recv(ipart)      ,ltype,ltype_rcv,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
          call PAR_SEND_RECEIVE_IP(nelem_send(ipart)*mnode,nelem_recv(ipart)*mnode,lnods,lnods_rcv,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
          call PAR_SEND_RECEIVE_IP(nelem_send(ipart)      ,nelem_recv(ipart)      ,lperm,lperm_rcv,dom_i=ipart,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4)
          if( nboun_part_p > 0 .and. nelem_recv(ipart) > 0 ) then
             call par_parallel_partitioning_boundary_elements(&
...
subroutine par_partition_lelbo2(chunk_nelem,chunk_npoin,comm_npoin)
   ...
   call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm0)
   call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm1)
   call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm2)
   call PAR_COMM_RANK_AND_SIZE(PAR_COMM_MY_CODE_WM4,ipart4,PAR_CODE_SIZE_WM4)
   ipart = int(ipart4,ip)
   !
   ...
   call generate_comm_chunkdist(list_bpoin,chunk_npoin,PAR_COMM_MY_CODE_WM4,comm0,dist_dom='SOURCE')
   ...
   call PAR_MAX(mranks,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4,INCLUDE_ROOT=.true.)
   ...
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm1,par_memor)
   call PAR_INITIALIZE_COMMUNICATION_ARRAY(comm1)
   call generate_comm_sizes(lnsend,nsendp,comm1,int(PAR_COMM_MY_CODE_WM4,ip))
   ...
   !
   call PAR_COPY_COMMUNICATION_ARRAY(comm1,comm2)
   ...
   call PAR_MAX(pos,rank_max_owner=iboun,PAR_COMM_IN4=PAR_COMM_MY_CODE_WM4,INCLUDE_ROOT=.true.)
   ...
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm0,par_memor)
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm1,par_memor)
   call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm2,par_memor)
