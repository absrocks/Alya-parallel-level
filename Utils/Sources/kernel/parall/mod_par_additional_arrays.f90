...
module mod_par_additional_arrays
  ...
  use def_master,         only : comm_data_par,routp,ioutp
  use def_master,         only : kfl_paral,ISLAVE,INOTMASTER
...
contains
  ...
  subroutine par_multiplicity_ownership(meshe,COMM,where)
    ...
    if( ISLAVE .and. INOTEMPTY ) then
       ...
       nullify(iexchange)
       call memory_alloca(par_memor,'BOUND_OWNER_RANK'  ,'par_edge_communication_array',COMM % bound_owner_rank  , meshe % npoin_2-meshe % npoi1)
       call memory_alloca(par_memor,'BOUND_MULTIPLICITY','par_edge_communication_array',COMM % bound_multiplicity, meshe % npoin_2-meshe % npoi1)
       call memory_alloca(par_memor,'IEXCHANGE'         ,'par_edge_communication_array',iexchange                , meshe % npoin_2 )
  ...
  subroutine par_matrix_exchange_on_interface_nodes(meshe,COMM)
    ...
    if( ISLAVE ) then
       ...
       do ineig = 1,COMM % nneig
          ...
          call memory_alloca(par_memor,'JA'            ,'par_matrix_exchange',COMM % bound_matrix(ineig) % ja,bsize)
          call memory_alloca(par_memor,'NZDOM_II'      ,'par_matrix_exchange',COMM % bound_matrix(ineig) % nzdom_ii,bsize)
          ...
          do ii = COMM % bound_size(ineig),COMM % bound_size(ineig+1)-1
             ...
             !
             if( kk > 0 ) then
                call memory_alloca(par_memor,'JA','par_matrix_exchange',COMM % bound_matrix(ineig) % ja(jj) % l,kk)
                do ll = 1,kk
                   jpoin              = list_nodes(ll)
  ...
  subroutine par_matrix_w_halos_exchange_on_interface_nodes(meshe,COMM)
    ...
    if( ISLAVE ) then
       ...
       do ineig = 1,comm % nneig
          bsize = COMM % bound_size(ineig+1) - COMM % bound_size(ineig)
          call memory_alloca(par_memor,'NZDOM_II'          ,'par_matrix_w_halos_exchange',COMM % bound_mat_halo_send(ineig) % nzdom_ii,bsize)
          call memory_alloca(par_memor,'NZDOM_II'          ,'par_matrix_w_halos_exchange',COMM % bound_mat_halo_recv(ineig) % nzdom_ii,bsize)
       end do
...
!          dom_i = COMM % neights(ineig)
       ...
       do ineig = 1,comm % nneig
          ...
          !
          call memory_alloca(par_memor,'JA','par_matrix_w_halos_exchange',COMM % bound_mat_halo_send(ineig) % ja,bsize)
          call memory_alloca(par_memor,'JA','par_matrix_w_halos_exchange',COMM % bound_mat_halo_recv(ineig) % ja,bsize)
          ...
          do ii = 1, bsize
             ...
             if ( COMM % bound_mat_halo_send(ineig) % nzdom_ii (ii) /= 0 ) call memory_alloca(par_memor,'JA(ii)%L','par_matrix_w_halos_exchange',  &
                COMM % bound_mat_halo_send(ineig) % ja(ii) % l,COMM % bound_mat_halo_send(ineig) % nzdom_ii (ii)  )
             if ( COMM % bound_mat_halo_recv(ineig) % nzdom_ii (ii) /= 0 ) call memory_alloca(par_memor,'JA(ii)%L','par_matrix_w_halos_exchange',  &
...
!                   if (ifoun==0) call runend('par_matrix_w_halos_exchange_on_interface_nodes:connectivy not found')
  ...
  subroutine par_matrix_computational_halo_exchange(meshe,COMM)
    use def_domain,         only : npoin_2,nelem_2     ! I understand this will later come as part of meshe
    use mod_graphs,         only : graphs_poipoi
    use mod_communications, only : PAR_COMM_RANK_AND_SIZE
    use mod_communications, only : PAR_START_NON_BLOCKING_COMM
...
!                   if (ifoun==0) call runend('par_matrix_w_halos_exchange_on_interface_nodes:connectivy not found')
  ...
  subroutine par_node_number_in_owner(meshe,COMM)
    ...
    if( ISLAVE .and. meshe % npoin_2 > 0 ) then
       ...
       nullify(iexchange)
       call memory_alloca(par_memor, 'NODE_NUMBER_IN_OWNER', 'par_edge_communication_array', COMM % node_number_in_owner  ,&
  ...
  subroutine par_full_row_communication_arrays(meshe,COMM)
    ...
    integer(ip)                        :: ineig_send,ineig_recv
    integer(ip)                        :: kneig,jneig,ipoin_global
    integer(ip)                        :: mneig(1),idime,ierro
    ...
    call livinf(0_ip,'COMPUTE FULL ROW COMMUNICATOR FOR COMPUTATIONAL HALO EXCHANGE',0_ip)
    ...
    if( ISLAVE ) then
       ...
       do ineig = 1,COMM % nneig
          ...
          call memory_alloca(par_memor,'LNEIG_SECOND_RECV','par_full_row_communication_arrays',lneig_second_recv(ineig)%l,nneig_second_recv)
             ...
             do jneig = 1,size(lneig_second_recv(ineig)%l)
                ...
                if( dom_i /= kfl_paral ) then
                   if( count(lneig_second==dom_i,KIND=ip) == 0 ) then
                      ...
                      if( nneig_second > size(lneig_second) ) call memory_resize(par_memor,'LNEIG','par_full_row_communication_arrays',lneig_second,2_ip*int(size(lneig_second),ip))
       ...
       call memory_alloca(par_memor,'FULL_ROW_RECV_SIZE','par_full_row_communication_arrays',COMM % full_row_recv_size,   COMM % full_row_recv_nneig+1_ip)
       call memory_alloca(par_memor,'FULL_ROW_RECV_SIZE','par_full_row_communication_arrays',COMM % full_row_recv_neights,COMM % full_row_recv_nneig)
       ...
       !
       call memory_copy(par_memor,'FULL_ROW_RECV_NEIGHTS','par_full_row_communication_arrays',COMM % full_row_recv_neights,full_row_recv_neights,'DO_NOT_DEALLOCATE')
       ...
       call memory_deallo(par_memor,'FULL_ROW_RECV_SIZE','par_full_row_communication_arrays',full_row_recv_neights)
       ...
       !
       call memory_alloca(par_memor,'FULL_ROW_RECV_PERM','par_full_row_communication_arrays',COMM % full_row_recv_perm,COMM % full_row_recv_dim)
       COMM % full_row_recv_dim = 0
       do ineig = 1,COMM % full_row_recv_nneig
          dom_i = COMM % full_row_recv_neights(ineig)
          jj    = 0
      ...
      !if( debug) print*,'B) Neighbors of ',kfl_paral,': ',COMM % full_row_recv_neights(:)
       ...
       do ineig = 1,nneig_second
          ...
          !mneig = maxloc( COMM % full_row_recv_neights , COMM % full_row_recv_neights == dom_i )
          !if( kfl_paral == 1 .and. kneig /= mneig(1) ) write(*,'(100(1x,i3))') dom_i,COMM % full_row_recv_neights
          !mneig = maxloc( COMM % full_row_recv_neights , COMM % full_row_recv_neights == dom_i )
          !if( kneig /= mneig(1) ) print*, COMM % full_row_recv_neights
          !print*,kneig,mneig(1)
          !mneig  = 0
          !mneig(1) = maxloc( COMM % full_row_recv_neights , COMM % full_row_recv_neights == dom_i )
          !kneig = mneig(1)
          if( kneig == 0 ) then
             nneig_second_recv = 0
          else
          ...
          !if(kfl_paral==9) write(*,'(a,20(1x,i3))') 'FIRST= ',nneig_second_recv,dom_i,COMM % full_row_recv_neights
          ...
          !if( debug ) print*,kfl_paral,' recvs ',nneig_second_recv,' from ',dom_i
          !if( debug ) print*,kfl_paral,' sends ',nneig_second_send,'  to  ',dom_i
          !
       ...
       !
       call memory_alloca(par_memor,'FULL_ROW_SEND_SIZE'   ,'par_full_row_communication_arrays',COMM % full_row_send_size,   COMM % full_row_send_nneig+1)
       call memory_alloca(par_memor,'FULL_ROW_SEND_NEIGHTS','par_full_row_communication_arrays',COMM % full_row_send_neights,COMM % full_row_send_nneig)
       do ineig = 1,COMM % full_row_send_nneig
          ...
          !if( debug ) print*,'-> ',kfl_paral,' sends ',COMM % full_row_send_size(ineig),'  to  ',dom_i,' out of ',COMM % full_row_send_nneig
       end do
       !!!!call runend('O.K.!')
       call graphs_number_to_linked_list(COMM % full_row_send_nneig,COMM % full_row_send_size)
       !
       ! Exchange list of ndoes to be sent
       !
       call memory_alloca(par_memor,'FULL_ROW_SEND_PERM','par_full_row_communication_arrays',COMM % full_row_send_perm,COMM % full_row_send_dim)
    ...
    if( ISLAVE ) then
       ...
       do ipoin = meshe % npoi1+1,meshe % npoin_halo
          ...
          if( dom_i /= lcheck(ipoin) ) then
             ierro = ierro + 1
             !print*,'PROBLEM: ',kfl_paral,dom_i,lcheck(ipoin),lninv_loc(ipoin),meshe % npoin_own
          end if
       end do
    end if
    call PAR_MAX(ierro)
