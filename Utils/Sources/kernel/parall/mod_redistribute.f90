...
module mod_redistribute
   ...
   use def_master,          only : IPARSLAVE, mem_servi, IMASTER,servi,INOTMASTER
...
contains
   ...
   subroutine par_slaves_communicator()
      ...
      implicit none
      integer(ip)           :: ipart,kfl_paral_proc_node,ndims, auxi
...
!      if( INOTMASTER ) then
 ...
 !     icolor = 1_4
   ...
   subroutine par_exchange_coordinates
      ...
      use def_master,  only : IPARSLAVE, mem_servi, servi
      use def_domain,  only : nnode
      use mod_mrgrnk,  only : mrgrnk
      ...
      if( IPARSLAVE )then
         ...
         !
         call memory_alloca(mem_servi(1:2,servi),'communication_table','par_exchange_coordinates_sfc',communication_table,&
         ...
         !
         call memory_alloca(mem_servi(1:2,servi),' bool_coord_needed ',' par_exchange_coordinates_sfc ',&
            bool_coord_needed,max_ipoin_needed-min_ipoin_needed+1_ip)
         call memory_alloca(mem_servi(1:2,servi),' invdict           ',' par_exchange_coordinates_sfc ',&
         ...
         !
         call memory_alloca(mem_servi(1:2,servi),' max_coord_needed ',' par_exchange_coordinates_sfc ',max_coord_needed,2_ip,mnode*nelem_part)
         !
         ...
         call memory_deallo(mem_servi(1:2,servi),' bool_coord_needed ',' par_exchange_coordinates_sfc ',bool_coord_needed)
         !
         ...
         call memory_alloca(mem_servi(1:2,servi),' temp_coord ',' par_exchange_coordinates_sfc ',temp_coord,ndime,npoin_local)
         call memory_alloca(mem_servi(1:2,servi),' diction    ',' par_exchange_coordinates_sfc ',diction,npoin_local)
         ...
         !
         call PAR_SUM(communication_table, 'IN SFC PARTITION','INCLUDE MASTER' )
         ...
         !
         call memory_alloca(mem_servi(1:2,servi),' point_list_send '  ,' par_exchange_coordinates_sfc ',point_list_send,  nboxc,'INITIALIZE',0_ip)
         call memory_alloca(mem_servi(1:2,servi),' point_list_recv '  ,' par_exchange_coordinates_sfc ',point_list_recv,  nboxc,'INITIALIZE',0_ip)
         call memory_alloca(mem_servi(1:2,servi),' recv_coord_buffer ',' par_exchange_coordinates_sfc ',recv_coord_buffer,nboxc,'INITIALIZE',0_ip)
         call memory_alloca(mem_servi(1:2,servi),' send_coord_buffer ',' par_exchange_coordinates_sfc ',send_coord_buffer,nboxc,'INITIALIZE',0_ip)
         do cpart = 0_ip, nboxc-1_ip
            ...
            if( PAR_MY_SFC_RANK_WM /= cpart )then
               ...
               call memory_alloca(mem_servi(1:2,servi),' point_list_send '  ,' par_exchange_coordinates_sfc ',point_list_send(cpart) % l, buffer_size_send)
               call memory_alloca(mem_servi(1:2,servi),' point_list_recv '  ,' par_exchange_coordinates_sfc ',point_list_recv(cpart) % l, buffer_size_recv)
               call memory_alloca(mem_servi(1:2,servi),' recv_coord_buffer ',' par_exchange_coordinates_sfc ',recv_coord_buffer(cpart) % a, ndime, buffer_size_send)
               call memory_alloca(mem_servi(1:2,servi),' send_coord_buffer ',' par_exchange_coordinates_sfc ',send_coord_buffer(cpart) % a, ndime, buffer_size_recv)
         ...
         !
         call memory_alloca(mem_servi(1:2,servi),' ranking '  ,' par_exchange_coordinates_sfc ',ranking,npoin_local)
         call memory_alloca(mem_servi(1:2,servi),' coord_loc ',' par_exchange_coordinates_sfc ',coord_loc,ndime,npoin_local)
         call memory_alloca(mem_servi(1:2,servi),' lnods_loc ',' par_exchange_coordinates_sfc ',lnods_loc,mnode,nelem_part)
         ...
         call memory_deallo(mem_servi(1:2,servi),' diction '          ,' par_exchange_coordinates_sfc ',diction          )
         call memory_deallo(mem_servi(1:2,servi),' point_list_send '  ,' par_exchange_coordinates_sfc ',point_list_send  )
         call memory_deallo(mem_servi(1:2,servi),' point_list_recv '  ,' par_exchange_coordinates_sfc ',point_list_recv  )
         call memory_deallo(mem_servi(1:2,servi),' recv_coord_buffer ',' par_exchange_coordinates_sfc ',recv_coord_buffer)
         call memory_deallo(mem_servi(1:2,servi),' send_coord_buffer ',' par_exchange_coordinates_sfc ',send_coord_buffer)
         call memory_deallo(mem_servi(1:2,servi),' bool_coord_needed ',' par_exchange_coordinates_sfc ',bool_coord_needed)

         if(associated(lnods_par)) call memory_deallo(mem_servi(1:2,servi),' lnods_par ',' par_exchange_coordinates_sfc ',lnods_par)
         if(associated(communication_table)) call memory_deallo(mem_servi(1:2,servi),' communication_table ',&
            'par_exchange_coordinates_sfc ',communication_table)
         if(associated(max_coord_needed)) call memory_deallo(mem_servi(1:2,servi),' max_coord_needed ',' max_coord_needed ',max_coord_needed)
         if(associated(ranking)) call memory_deallo(mem_servi(1:2,servi),' ranking ',' par_exchange_coordinates_sfc ',ranking)
         if(associated(temp_coord)) call memory_deallo(mem_servi(1:2,servi),' temp_coord ',&
   ...
   subroutine par_distribute_mesh()
      ...
      if( IMASTER )then
         ...
         call memory_alloca(mem_servi(1:2,servi),' nelem_part_master ',' par_distribute_mesh ' ,nelem_part_master4,int(nboxc+1_ip,4_ip),'DO_NOT_INITIALIZE',0_4)
         call memory_alloca(mem_servi(1:2,servi),' npoin_part_master ',' par_distribute_mesh ' ,npoin_part_master4,int(nboxc+1_ip,4_ip),'DO_NOT_INITIALIZE',0_4)
         ...
         do ipart = 1_ip, nboxc-1_ip
            ...
            ! lnods
            !
            call memory_alloca(mem_servi(1:2,servi),' send_lnods_buff ',' par_distribute_mesh ' ,send_lnods_buff,mnode,message_size)
            indexi = 1_ip
            ...
            call memory_deallo(mem_servi(1:2,servi),' send_lnods_buff ',' par_distribute_mesh ' ,send_lnods_buff)
            !
            ! coordinates
            !
            ...
            call memory_alloca(mem_servi(1:2,servi),' send_coord_buff ',' par_distribute_mesh ' ,send_coord_buff,ndime,message_size)
            indexi = 1_ip
            ...
            call memory_deallo(mem_servi(1:2,servi),' send_coord_buff ',' par_distribute_mesh ' ,send_coord_buff)
         ...
         !
         call memory_alloca(mem_servi(1:2,servi),'coord_par','par_distribute_mesh',coord_par,ndime,npoin_part)
         call memory_alloca(mem_servi(1:2,servi),'ltype_par','par_distribute_mesh',ltype_par,nelem_part)
         call memory_alloca(mem_servi(1:2,servi),'lnods_par','par_distribute_mesh',lnods_par,mnode,nelem_part)
         !
   ...
   subroutine par_sfc_input()
      ...
      real(rp)                  :: cent_mass(3), iweig
      integer(ip)               :: ienti
      integer(ip)               :: ielty, inode, ipoin
      ...
      if( IPARSLAVE ) then
         ...
         call memory_alloca(mem_servi(1:2,servi),' lenti ',vacal,lenti,ndime,nenti)
         call memory_alloca(mem_servi(1:2,servi),' lweig ',vacal,lweig,nenti)
         ...
         if(sfc_criteria >= 1_ip) then
            ...
            do ienti = 1_ip, nenti
               ...
               if(sfc_criteria == 2_ip) then
                  iweig = real(ngaus(abs(ltype_par(ienti))),rp)
               else
                  iweig = 1_rp
               endif
               lweig(ienti) = iweig
   ...
   subroutine par_deallocate
      ...
      if(associated(coord_loc)) call memory_deallo(mem_servi(1:2,servi),' coord_loc ',vacal,coord_loc)
      if(associated(coord_par)) call memory_deallo(mem_servi(1:2,servi),' coord_par ',vacal,coord_par)
      if(associated(ltype_par)) call memory_deallo(mem_servi(1:2,servi),' ltype_par ',vacal,ltype_par)
      if(associated(lnods_loc)) call memory_deallo(mem_servi(1:2,servi),' lnods_loc ',vacal,lnods_loc)
   ...
   subroutine gather_to_master(lepar_par,npart)
      ...
      integer(ip), pointer                 :: lppar(:)
      integer(ip)                          :: ielem, ibuff, iboxl, iboxf,ipart
      integer(ip)                          :: ipoin, ielty, inode,auxpar,lpoin,nnoda
      integer(ip)                          :: iweig, minw, minp,ibal,prevp,intel,imax
      integer(ip)                          :: icont, icont2, max_in,max_par,nexp,jcont
      integer(ip)                          :: minorpar,maxpar,minorpar_prev,maxpar_prev
      integer(ip), pointer                 :: maxprevs(:)
      integer(ip)                          :: nmaxp
      integer(ip)                          :: iweig_sum,iweig_rest,maxp,maxw,moved,possible
      real(rp)                             :: mincoe
      logical(lg), pointer                 :: interior(:)
      logical(lg)                          :: majory, minorel, auxlo, down
      ...
      nullify(dummy)
      call memory_alloca(mem_servi(1:2,servi),' dummy ',vacal,dummy,1_ip)
      ...
      if( IMASTER )then
         nullify(lepar_par)
         call memory_alloca(mem_servi(1:2,servi),' lepar_par ',vacal,lepar_par,nelem)
         lepar_par  = -1_ip
      ...
      call memory_deallo(mem_servi(1:2,servi),' dummy ',vacal,dummy)
   ...
   subroutine par_visualization_sfc(lepar,visop,dboxc,dim_bin_core)
      ...
      if( IMASTER .and. visop > 0_ip ) then
         ...
         nullify(visfd)
         call memory_alloca(mem_servi(1:2,servi),' visfd ',vacal,visfd,nboxf)
         ...
         call memory_deallo(mem_servi(1:2,servi),' visfd ',vacal,visfd)
