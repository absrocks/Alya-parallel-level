...
subroutine par_extgra()
  ...
  if( INOTMASTER ) then
     call memory_alloca(mem_servi(1:2,servi),'GRAPH',       'par_extgra',graph,npoin)
     call memory_alloca(mem_servi(1:2,servi),'GRAPH_NUMBER','par_extgra',graph_number,npoin)
     msize = 0
     do ipoin = 1,npoin
  ...
  if( INOTMASTER ) then
     ...
     do ineig = 1,commd % nneig
        ...
        call memory_alloca(mem_servi(1:2,servi),'GRAPH_SEND','par_extgra',graph_send,msize,bsize)
        call memory_alloca(mem_servi(1:2,servi),'GRAPH_RECV','par_extgra',graph_recv,msize,bsize)
           ...
           do while( kk <= msize )
              ...
              if( jpoin == 0 ) then
                 ...
                 if( .not. ifoun ) then
                    ...
                    nsize = nsize + 1
                    call memory_resize(mem_servi(1:2,servi),'GRAPH','par_extgra',graph(ipoin) % l,nsize)
        ...
        call memory_deallo(mem_servi(1:2,servi),'GRAPH_SEND','par_extgra',graph_send)
        call memory_deallo(mem_servi(1:2,servi),'GRAPH_RECV','par_extgra',graph_recv)
  ...
  if( INOTMASTER ) then
     ...
     ! Deallocate memory
     !
     call memory_deallo(mem_servi(1:2,servi),'GRAPH',       'par_extgra',graph)
     call memory_deallo(mem_servi(1:2,servi),'GRAPH_NUMBER','par_extgra',graph_number) 
