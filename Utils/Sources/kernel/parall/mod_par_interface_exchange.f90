...
module mod_par_interface_exchange
  ...
  use def_domain,          only : nelem_total,npoin_total
  ...
  use mod_elmgeo,          only : elmgeo_element_type_initialization ! remove asap
...
contains
  ...
  subroutine par_interface_exchange(comm,TYPE_OF_COUPLING)
    ...
    !
    !if( nperi > 0 ) then
    !   call memory_copy(par_memor,'COORD_WET','par_interface_exchange',lninv_loc,lninv_cpy,'DO_NOT_DEALLOCATE')
    !   do ipoin = 1,npoin
    !      if( lmast(ipoin) > 0 ) then
    !         lninv_loc(ipoin) = lmast(ipoin)
    !         lmast(ipoin) = 0 
    !      end if
    !   end do
    !   toler_absolute_cou = 100.0_rp                          ! Enable multiple source nodes
    !end if
    ...
    if( INOTMASTER ) then
       ...
       call memory_alloca(par_memor,'COORD_WET'   ,'par_interface_exchange',my_coupling % wet % coord_wet,ndime,my_coupling % wet % npoin_wet)
    ...
    if( INOTMASTER ) then
       ...
       ! Periodicity
       !
       !if( nperi > 0 .and. INOTMASTER ) then
       !   lninv_loc = lninv_cpy
       !   call memory_deallo(par_memor,'COORD_WET','par_interface_exchange',lninv_cpy)
       !   nperi = 0
       !   !call memory_deallo(par_memor,'COORD_WET','par_interface_exchange',lmast)
       !end if
       ...
       !
       call memory_alloca(par_memor,'PERMR','par_interface_exchange',permr,npoin)
       kpoin = 0
       do ipoin = 1,npoin
    ...
    !
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(comm,par_memor,OPTION='ONLY NODES')
    comm % PAR_COMM_WORLD = PAR_COMM_MY_CODE
    ...
    if( INOTMASTER ) then
       ...
       call memory_alloca(par_memor,'BOUND_PERM','par_interface_exchange',comm % bound_perm,   comm % bound_dim)
       call memory_alloca(par_memor,'NEIGHTS'   ,'par_interface_exchange',comm % neights,      comm % nneig)
       call memory_alloca(par_memor,'BOUND_SIZE','par_interface_exchange',comm % bound_size,   comm % nneig+1)
       ...
       call memory_deallo(par_memor,'PERMR    ','par_interface_exchange',permr    )
    ...
    !
    if( IMASTER ) then
       call memory_deallo(par_memor,'NPOIN_PAR' ,'par_interface_exchange' , npoin_par  )
       call memory_deallo(par_memor,'NELEM_PAR' ,'par_interface_exchange' , nelem_par  )
       call memory_deallo(par_memor,'NBOUN_PAR' ,'par_interface_exchange' , nboun_par  )
       call memory_alloca(par_memor,'NPOIN_PAR' ,'par_interface_exchange' , npoin_par  , nsubd )
       call memory_alloca(par_memor,'NELEM_PAR' ,'par_interface_exchange' , nelem_par  , nsubd )
       call memory_alloca(par_memor,'NBOUN_PAR' ,'par_interface_exchange' , nboun_par  , nsubd )
       call memory_alloca(par_memor,'NPOIN_PAR0','par_interface_exchange' , npoin_par0 , nsubd+1_ip ,lboun=0_ip )
       call memory_alloca(par_memor,'NELEM_PAR0','par_interface_exchange' , nelem_par0 , nsubd+1_ip ,lboun=0_ip )
       call memory_alloca(par_memor,'NBOUN_PAR0','par_interface_exchange' , nboun_par0 , nsubd+1_ip ,lboun=0_ip )
    end if
    ...
    if( IMASTER ) then
       npoin_par(1:nsubd) = npoin_par0(1:nsubd)
       nelem_par(1:nsubd) = nelem_par0(1:nsubd)
       nboun_par(1:nsubd) = nboun_par0(1:nsubd)
       npoi1              =  0
       npoi2              =  0
       npoi3              = -1
       npoin              =  0
       nelem              =  0
       nboun              =  0
       call memory_deallo(par_memor,'NPOIN_PAR0','par_interface_exchange' , npoin_par0 )
       call memory_deallo(par_memor,'NELEM_PAR0','par_interface_exchange' , nelem_par0 )
       call memory_deallo(par_memor,'NBOUN_PAR0','par_interface_exchange' , nboun_par0 )
    end if
    comm % npoi1 = npoi1
    comm % npoi2 = npoi2
    comm % npoi3 = npoi3
  ...
  subroutine par_interface_exchange_scheduling(comm)
    ...
    !
    call PAR_COMM_RANK_AND_SIZE(comm % PAR_COMM_WORLD,PAR_CURRENT_RANK,PAR_CURRENT_SIZE)
    PAR_COMM4 = int(comm % PAR_COMM_WORLD,4)
    ...
    call PAR_ALLGATHER(comm % nneig,number_neighbors,PAR_COMM_IN4=PAR_COMM4)   
    ...
    call PAR_ALLGATHERV(my_list_neighbors,list_neighbors,number_neighbors,PAR_COMM_IN4=PAR_COMM4)
    ...
    call memory_deallo(par_memor,'IADOMDUAL'        ,'par_interface_exchange_scheduling',iaDomDual)                
    call memory_deallo(par_memor,'JADOMDUAL'        ,'par_interface_exchange_scheduling',jaDomDual)                
    call memory_deallo(par_memor,'TRANSDUAL'        ,'par_interface_exchange_scheduling',translDual)               
    call memory_deallo(par_memor,'COLORS'           ,'par_interface_exchange_scheduling',colors)                   
  ...
  subroutine par_interface_exchange_order_communications(nncolors,nsubd,isubd,nnDual,ia,ja,translDual,colors,lcomm)
    ...
    nullify(invTransl)
    call memory_alloca(par_memor,'LCOMM'     ,'par_interface_exchange_order_communications',lcomm,nncolors)
