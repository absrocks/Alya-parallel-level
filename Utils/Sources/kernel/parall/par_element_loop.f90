...
subroutine par_element_loop()
  ...
  if( associated(list_elements_par) ) then
     do isubd = 1,size(list_elements_par) 
        call memory_deallo(par_memor,'LIST_ELEMENTS_PAR(ISUBD) % PACKS','par_element_loop',list_elements_par(isubd) % packs)
     end do
  ...
  if( associated(list_elements_norace_par) ) then
     do isubd = 1,size(list_elements_norace_par) 
        call memory_deallo(par_memor,'LIST_ELEMENTS_NORACE_PAR(ISUBD) % PACKS','par_element_loop',list_elements_norace_par(isubd) % packs)
     end do
  ...
  if( INOTMASTER ) then
     ...
     call memory_alloca(par_memor,'LIST_ELEMENTS_NORACE_PAR','par_element_loop',list_elements_norace_par(isubd) % packs,num_pack_norace_par(isubd))
     ...
     if( adjust_vector_size ) then
        ...
        do itype = 1,num_types
           if( num_element_types(itype) > 0 ) then
              ipack = ipack + 1
              call memory_alloca(par_memor,'LIST_ELEMENTS_NORACE_PAR','par_element_loop',list_elements_norace_par(isubd) % packs(ipack) % l,num_element_types(itype))
           end if
        end do
     else
        do ipack = 1,num_pack_norace_par(isubd)
           call memory_alloca(par_memor,'LIST_ELEMENTS_NORACE_PAR','par_element_loop',list_elements_norace_par(isubd) % packs(ipack) % l,VECTOR_SIZE_LOC) 
        end do
     end if
  ...
  if( par_hybrid == PAR_HYBRID_OFF .or. par_hybrid == PAR_OPENMP_NO_COLORING ) then
     ...
     if( INOTMASTER ) then
        ...
        call memory_alloca(par_memor,'LIST_ELEMENTS_PAR','par_element_loop',list_elements_par(isubd) % packs,num_pack_par(isubd))
        if( adjust_vector_size ) then
           ipack = 0
           do itype = 1,num_types
              if( num_element_types(itype) > 0 ) then
                 ipack = ipack + 1
                 call memory_alloca(par_memor,'LIST_ELEMENTS_PAR','par_element_loop',list_elements_par(isubd) % packs(ipack) % l,num_element_types(itype))
              end if
           end do
        else
           do ipack = 1,num_pack_par(isubd)
              call memory_alloca(par_memor,'LIST_ELEMENTS_PAR','par_element_loop',list_elements_par(isubd) % packs(ipack) % l,VECTOR_SIZE_LOC) 
           end do
        end if
