...
contains
  ...
  subroutine parall_openmp_coloring(meshe,mepoi_opt,pelpo_opt,lelpo_opt,ON_BOUNDARIES)
    ...
    if_on_boundaries = .false.
    if( present(ON_BOUNDARIES) ) if_on_boundaries = ON_BOUNDARIES
    if( if_on_boundaries ) then
       nenti = meshe % nboun
    else
       nenti = meshe % nelem
    ...
    if( INOTMASTER ) then
       if( par_omp_num_threads /= 0 .and. nenti /= 0 ) then
          ...
          if( par_omp_coloring_alg == 0 ) then
             if( if_on_boundaries ) then
                call graphs_coloring_greedy(&
                     ...
                     par_omp_nboun_num_colors,par_omp_nboun_ia_colors,par_omp_nboun_ja_colors)
             else
                call graphs_coloring_greedy(&
          ...
          if( .not. present(pelpo_opt) ) call graphs_deallocate(pelpo)
          if( .not. present(lelpo_opt) ) call graphs_deallocate(lelpo)
          !
          ! Output
          !
          if( if_on_boundaries ) then
