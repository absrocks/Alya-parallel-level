...
contains
  ...
  subroutine par_output_partition(itask)
    ...
    case ( 1_ip ) 
       ...
       !print*,'popo=',par_omp_num_threads_gat,par_hybrid_ompss_gat,par_hybrid_openmp_gat
       !call runend('O.K.!')
  ...
  subroutine par_output_solvers(solve)

    use def_master, only : solve_sol
    use def_master, only : mmodu,momod,modul,kfl_modul

    type(soltyp), intent(in), optional :: solve !< Solver structure
    integer(ip)                        :: ivari

    if( IPARALL .and. current_code == 1 ) then

       !if( iNOTSLAVE ) &
       !     call iofile(zero,lun_parti_res,fil_parti_res,'PARALL PARTITION RESULT','old','formatted','append')

       if( present(solve) ) then
          call par_output_single_solver(solve)
       else
          do modul = 1,mmodu 
             if( kfl_modul(modul) == 1 ) then 
                if( associated(momod(modul) % solve) ) then
                   do ivari = 1,size(momod(modul) % solve)
                      ...
                      call par_output_single_solver(solve_sol(1))
                   end do
                end if
             end if
          end do
  ...
  subroutine par_output_single_solver(solve)
    ...
    if(    solve % kfl_solve /= 0 .and. solve % kfl_algso >  0 .and.   &
         & solve % kfl_algso /= 9 ) then
       !
       ! Iterative solvers
       !
       call par_output_value(0_ip,solve % nzmat,'MATRIX_NUM_COEFF_'//trim(solve % wprob),'SOLVERS',wherein='IN MY CODE') 
       !
       ! SpMV timings
       !
       spmv_total    = solve % cpu_spmv(1) + solve % cpu_spmv(2)
       spmv_ratio(1) = solve % cpu_spmv(1)
       spmv_ratio(2) = spmv_total
       call PAR_AVERAGE(2_ip,spmv_ratio)
       spmv_ratio(1) = solve % cpu_spmv(1) / (spmv_ratio(1)+zeror)
       spmv_ratio(2) = spmv_total          / (spmv_ratio(2)+zeror)
       call par_output_value(0_ip,solve % cpu_spmv(1),'SPMV_TIME_COMPUTATION_'//trim(solve % wprob) ,'SOLVERS',wherein='IN MY CODE')
       call par_output_value(0_ip,spmv_total,         'SPMV_TIME_TOTAL_'//trim(solve % wprob)       ,'SOLVERS',wherein='IN MY CODE')
       call par_output_value(0_ip,spmv_ratio(1),      'SPMV_RATIO_COMPUTATION_'//trim(solve % wprob),'SOLVERS',wherein='IN MY CODE')
       call par_output_value(0_ip,spmv_ratio(2),      'SPMV_RATIO_TOTAL_'//trim(solve % wprob)      ,'SOLVERS',wherein='IN MY CODE')
       !
       ! Number of groups
       !
       if(    solve % kfl_algso == SOL_SOLVER_DEFLATED_CG           .or. &
          ...
          call par_output_value(0_ip,my_ngrou,'NUMBER_OF_GROUPS_'//trim(solve % wprob),'SOLVERS',wherein='IN MY CODE')          
       end if
       !
       ! Chunk size
       !
