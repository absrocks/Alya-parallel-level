...
module mod_std
  ...
  interface execute_command_line
     module procedure alya_execute_command_line
  end interface execute_command_line
  interface lbound
...
contains
  ...
  !> @author  houzeaux
  !> @date    2019-03-06
  !> @brief   Intrinsic execute_command_line for PGI
  !> @details For PGI, count does not include execute_command_line
  !>
  !-----------------------------------------------------------------------

  subroutine alya_execute_command_line(command)

    character(*), intent(in)           :: command
#if defined(__PGI) || defined(__aarch64__)
    call system(command)
#else
