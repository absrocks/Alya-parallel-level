...
module mod_mpio_par_readom
    ...
    use def_domain,                 only : coord, ltype, lnods, nnode, ndime, nelem, npoin, nelty, mnode,&
                                           ...
                                           nbset, neset, nnset, mcono, lmast, nsteps_fiel_ondemand
...
!-----------------------------------------------------------------------------------------------------------------------------
    ...
    character(150)                          ::  filename                            ! current file name
    ...
    character(150)                          ::  me="mod_mpio_par_readom"                !
...
!-----------------------------------------------------------------------
    ...
    subroutine reageo_par_mpio()
#ifndef MPI_OFF
        call livinf(-4_ip,'START GEOMETRY READING (PARALLEL)',0_ip)
        ...
        call livinf(-5_ip,'END GEOMETRY READING (PARALLEL)',0_ip)
#endif
        return
    end subroutine
...
#ifndef MPI_OFF
      call livinf(-4_ip,'START SET READING (PARALLEL)',0_ip)
        ...
        if (FILE_EXIST(fil_leset)) then
            ...
            call par_parse_int_sca(fil_leset, leset, nelem)
            if (mpio_flag_all_par /= PAR_MPIO_ON) then
               call mpio_gather(leset, leset, nelem_vec)
            end if
            neset = 1_ip
         end if
        ...
        if (FILE_EXIST(fil_lbset)) then
            ...
            call par_parse_int_sca(fil_lbset, lbset, nboun)
            if (mpio_flag_all_par /= PAR_MPIO_ON) then
                call mpio_gather(lbset, lbset, nboun_vec)
            end if
            nbset = 1_ip
         end if
        ...
        if (FILE_EXIST(fil_lnset)) then
            ...
            call par_parse_int_sca(fil_lnset, lnset, npoin)
            if (mpio_flag_all_par /= PAR_MPIO_ON) then
               call mpio_gather(lnset, lnset, npoin_vec)
            end if
            nnset = 1_ip
         end if

        call livinf(-5_ip,'END SET READING (PARALLEL)',0_ip)
    ...
    subroutine reabcs_par_mpio()
      ...
      kfl_icodn = 0
      kfl_icodb = 0
      call livinf(-4_ip,'START BCS READING (PARALLEL)',0_ip)
      if (FILE_EXIST(fil_codno)) then
         call livinf(0_ip,'READ CODNO FILE',0_ip)
         call par_parse_int_vec(fil_codno, kfl_codno, mcono, npoin)
         if (mpio_flag_all_par /= PAR_MPIO_ON) then
            call mpio_gather(kfl_codno, kfl_codno, npoin_vec, mcono)
         end if
         kfl_icodn = 1_ip
      end if
      if (FILE_EXIST(fil_codbo)) then
         call livinf(0_ip,'READ CODBO FILE',0_ip)
         call par_parse_int_sca(fil_codbo, kfl_codbo, nboun)
         if (mpio_flag_all_par /= PAR_MPIO_ON) then
            call mpio_gather(kfl_codbo, kfl_codbo, nboun_vec)
         end if
         kfl_icodb = 1_ip
      end if
      call livinf(-5_ip,'END BCS READING (PARALLEL)',0_ip)
#endif
    ...
    subroutine reafie_par_mpio()
      ...
      call livinf(-4_ip,'START FIELD READING (PARALLEL)',0_ip)
      if( IMASTER ) then
         call memgeo(28_ip)
      end if
      do ifiel = 1,nfiel
         ...
         do istep = 1,nsteps
            ...
            fil_field = xfiel_name(ifiel, istep)
            if( kdime == 1 ) then
               call par_parse_real_sca(fil_field, a1_tmp, size_p) !, time_field(ifiel) % a(istep))
               if (mpio_flag_all_par /= PAR_MPIO_ON) then
                  call mpio_gather(a1, a1_tmp, var_vec)
                  if (IMASTER.and.size_p>0) then
                     xfiel(ifiel) % a(1,:,istep)=a1(:)
                  end if
               else
                  if (ISLAVE.and.size_p>0) then
                     xfiel(ifiel) % a(1,:,istep)=a1_tmp(:)
                  end if
               end if
            else
               call par_parse_real_vec(fil_field, a2_tmp, kdime, size_p) !, time_field(ifiel) % a(istep))
               if( size_p > 0 ) then
                  if (mpio_flag_all_par /= PAR_MPIO_ON) then
                     call mpio_gather(a2, a2_tmp, var_vec, kdime)
                     if (IMASTER.and.size_p>0) then
                        xfiel(ifiel) % a(:,:,istep)=a2
                     end if
                  else
                     if (ISLAVE.and.size_p>0) then
                        xfiel(ifiel) % a(:,:,istep)=a2_tmp
                     end if
                  end if
               end if
            end if
         end do
         if (ISLAVE .and. mpio_flag_all_par /= PAR_MPIO_ON) then
            call memgeo(-29_ip)
         end if
      end do
      call livinf(-5_ip,'END FIELD READING (PARALLEL)',0_ip)
#endif
    ...
    subroutine par_parse_coord()
        ...
        if (mpio_flag_all_par /= PAR_MPIO_ON) then
            call mpio_gather(coord, coord, npoin_vec, ndime)
        end if
    end subroutine
    ...
    subroutine par_parse_lnoch()
      ...
      if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lnoch, lnoch, npoin_vec)
      end if
    end subroutine
    ...
    subroutine par_parse_lelch()
      ...
      if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lelch, lelch, nelem_vec)
      end if
    end subroutine
    ...
    subroutine par_parse_lboch()
      ...
      if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lboch, lboch, nboun_vec)
      end if
    end subroutine
    ...
    subroutine par_parse_ltype()
        ...
        if (mpio_flag_all_par /= PAR_MPIO_ON) then
            call mpio_gather(ltype, ltype, nelem_vec)
        end if
    end subroutine
    ...
    subroutine par_parse_lnods()
        ...
        if (mpio_flag_all_par /= PAR_MPIO_ON) then
            call mpio_gather(lnods, lnods, nelem_vec, mnode)
        end if
    end subroutine
    ...
    subroutine par_parse_lmate()
      ...
      if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lmate, lmate, nelem_vec)
      end if
    end subroutine
    ...
    subroutine par_parse_lmast()
      ...
      if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lmast, lmast, npoin_vec)
      end if
    end subroutine
    ...
    subroutine par_parse_lesub()
      ...
      if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lesub, lesub, nelem_vec)
      end if
    end subroutine
    ...
    subroutine par_parse_lnodb()
      integer(ip)                                  ::              knode, iblty
        ...
        if (mpio_flag_all_par /= PAR_MPIO_ON) then
          call mpio_gather(lnodb, lnodb, nboun_vec, mnodb)
          call mpio_gather(ltypb, ltypb, nboun_vec)
        end if
    end subroutine
    ...
    subroutine par_parse_lelbo()
        ...
        if (mpio_flag_all_par /= PAR_MPIO_ON) then
            call mpio_gather(lelbo, lelbo, nboun_vec)
        end if
    end subroutine
    ...
    subroutine par_parse_int_sca(fil, var_p, size_p)
    ...
    subroutine par_parse_int_vec(fil, var_p, size1_p, size2_p, time)
        character(*),           intent(in)       ::    fil
        integer(ip), pointer,   intent(inout)    ::    var_p(:,:)
    ...
    subroutine par_parse_real_sca(fil, var_p, size_p, time)
        character(*),           intent(in)       ::    fil
    ...
    subroutine par_parse_real_vec(fil, var_p, size1_p, size2_p, time)
        character(*),           intent(in)       ::    fil
        real(rp), pointer,   intent(inout)       ::    var_p(:,:)
        integer(ip),            intent(in)       ::    size1_p, size2_p
        real(rp),         intent(out), optional  ::    time
    ...
    subroutine mpio_gather_int_sca(var, var_p, var_vec)
    ...
    subroutine mpio_gather_real_sca(var, var_p, var_vec)
    ...
    subroutine mpio_gather_int_vec(var, var_p, var_vec, size1_p)
      integer(ip), pointer,   intent(inout) ::    var(:,:)
      integer(ip), pointer                  ::    var_tmp(:,:)
      integer(ip), pointer,   intent(inout) ::    var_p(:,:)
      integer(4), pointer, intent(in)       ::    var_vec(:)
    ...
    subroutine mpio_gather_real_vec(var, var_p, var_vec, size1_p)
      real(rp), pointer,   intent(inout)    ::    var(:,:)
      real(rp), pointer                     ::    var_tmp(:,:)
      real(rp), pointer,   intent(inout)    ::    var_p(:,:)
      integer(4), pointer, intent(in)       ::    var_vec(:)
