...
#define MPI_OFFSET_KIND 8
    ...
    interface PAR_FILE_SET_VIEW
        module procedure                        PAR_FILE_SET_VIEW_INT4_V,&
    ...
    subroutine PAR_FILE_OPEN_READ(fh, filename, wherein)
        ...
        if (ierr/=MPI_SUCCESS) then
            call runend("File "//filename//" could not be open. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    end subroutine

    subroutine PAR_FILE_OPEN_WRITE(fh, filename, wherein)
        integer(ip),           intent(inout)           :: fh
        character(*),          intent(in)              :: filename
        character(*),          optional, intent(in)    :: wherein
        type(comm_data_par),   pointer                 :: commu
        ...
        if (ierr/=MPI_SUCCESS) then
            call runend("File "//filename//" could not be open nor created. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    end subroutine
    ...
    subroutine PAR_FILE_READ_INT4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_READ(fh, buf, size, MPI_INTEGER4, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be read. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_READ_INT4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_READ_INT8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_READ(fh, buf, size, MPI_INTEGER8, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be read. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_READ_INT8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_READ_REAL8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_READ(fh, buf, size, MPI_REAL8, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be read. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_READ_REAL8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_READ_REAL4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
    ...
    subroutine PAR_FILE_READ_REAL4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_READ_REAL16_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
    ...
    subroutine PAR_FILE_READ_REAL16_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_READ_CHAR8_P(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        character(8), dimension(*), intent(inout)      :: buf
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_READ(fh, buf, 8*size, MPI_CHARACTER, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be read. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_READ_ALL_INT4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
        ...
        if (size==0 .or. .not.associated(buf)) then
            call memory_alloca(mpio_memor,'dummy',' ',  dummy, 1_4)
            call MPI_FILE_READ_ALL(fh, dummy, 0_ip, MPI_INTEGER4, status, ierr)
            call memory_deallo(mpio_memor,'dummy',' ',  dummy)
        else
            call MPI_FILE_READ_ALL(fh, buf, size, MPI_INTEGER4, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
    ...
    subroutine PAR_FILE_READ_ALL_INT4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
        integer(4), pointer                            :: dummy(:,:)
...
#ifndef MPI_OFF
        if (size==0 .or. .not.associated(buf)) then
            call memory_alloca(mpio_memor,'dummy',' ',  dummy, 1_4, 1_4)
    ...
    subroutine PAR_FILE_READ_ALL_INT8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
...
#ifndef MPI_OFF
        if (size==0 .or. .not.associated(buf)) then
            call memory_alloca(mpio_memor,'dummy',' ',  dummy, 1_8)
            call MPI_FILE_READ_ALL(fh, dummy, 0, MPI_INTEGER8, status, ierr)
            call memory_deallo(mpio_memor,'dummy',' ',  dummy)
        else
            call MPI_FILE_READ_ALL(fh, buf, size, MPI_INTEGER8, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be READ_ALL. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_READ_ALL_INT8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
        integer(8), pointer                            :: dummy(:,:)
...
#ifndef MPI_OFF
        if (size==0 .or. .not.associated(buf)) then
            call memory_alloca(mpio_memor,'dummy',' ',  dummy, 1_8, 1_8)
            call MPI_FILE_READ_ALL(fh, dummy, 0, MPI_INTEGER8, status, ierr)
            call memory_deallo(mpio_memor,'dummy',' ',  dummy)
        else
            call MPI_FILE_READ_ALL(fh, buf, size, MPI_INTEGER8, status, ierr)
    ...
    subroutine PAR_FILE_READ_ALL_REAL8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
...
#ifndef MPI_OFF
        if (size==0 .or. .not.associated(buf)) then
            call memory_alloca(mpio_memor,'dummy',' ',  dummy, 1_ip)
            call MPI_FILE_READ_ALL(fh, dummy, 0, MPI_REAL8, status, ierr)
            call memory_deallo(mpio_memor,'dummy',' ',  dummy)
        else
            call MPI_FILE_READ_ALL(fh, buf, size, MPI_REAL8, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be READ_ALL. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_READ_ALL_REAL8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(8), pointer                               :: dummy(:,:)
...
#ifndef MPI_OFF
        if (size==0 .or. .not.associated(buf)) then
            call memory_alloca(mpio_memor,'dummy',' ',  dummy, 1_ip, 1_ip)
            call MPI_FILE_READ_ALL(fh, dummy, 0, MPI_REAL8, status, ierr)
            call memory_deallo(mpio_memor,'dummy',' ',  dummy)
        else
            call MPI_FILE_READ_ALL(fh, buf, size, MPI_REAL8, status, ierr)
    ...
    subroutine PAR_FILE_READ_ALL_REAL4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
    ...
    subroutine PAR_FILE_READ_ALL_REAL4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(4), pointer                               :: dummy(:,:)
...
#ifndef __PGI
    subroutine PAR_FILE_READ_ALL_REAL16_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(inout)           :: buf(:)
        integer(ip),           intent(in)              :: size
    ...
    subroutine PAR_FILE_READ_ALL_REAL16_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(inout)           :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(16), pointer                               :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_INT4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_WRITE(fh, buf, size, MPI_INTEGER4, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_WRITE_INT4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_INT8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_WRITE(fh, buf, size, MPI_INTEGER8, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_WRITE_INT8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_REAL8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_WRITE(fh, buf, size, MPI_REAL8, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_WRITE_REAL8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_REAL4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_REAL4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_REAL16_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_REAL16_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
    ...
    subroutine PAR_FILE_WRITE_CHAR8_P(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        character(8), dimension(*), intent(in)         :: buf
        integer(ip),           intent(in)              :: size
#ifndef MPI_OFF
        call MPI_FILE_WRITE(fh, buf, 8*size, MPI_CHARACTER, status, ierr)
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
#endif
    ...
    subroutine PAR_FILE_WRITE_ALL_INT4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        integer(4), pointer                            :: dummy(:)
        ...
        if (size==0 .or. .not.associated(buf)) then
            ...
            call MPI_FILE_WRITE_ALL(fh, buf, size, MPI_INTEGER4, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_WRITE_ALL_INT4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        integer(4), pointer                            :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_INT8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        integer(8), pointer                            :: dummy(:)
        ...
        if (size==0 .or. .not.associated(buf)) then
            ...
            call MPI_FILE_WRITE_ALL(fh, buf, size, MPI_INTEGER8, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_WRITE_ALL_INT8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        integer(8), pointer                            :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_REAL8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        real(8), pointer                               :: dummy(:)
        ...
        if (size==0 .or. .not.associated(buf)) then
            ...
            call MPI_FILE_WRITE_ALL(fh, buf, size, MPI_REAL8, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_WRITE_ALL_REAL8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(8), pointer                               :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_REAL4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        real(4), pointer                               :: dummy(:)
    ...
    subroutine PAR_FILE_WRITE_ALL_REAL4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(4), pointer                               :: dummy(:,:)
...
#ifndef __PGI
    subroutine PAR_FILE_WRITE_ALL_REAL16_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        real(16), pointer                               :: dummy(:)
    ...
    subroutine PAR_FILE_WRITE_ALL_REAL16_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(16), pointer                               :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_INT4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(4), pointer,   intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        integer(4), pointer                            :: dummy(:)
        ...
        if (.not.associated(buf)) then
            ...
            call MPI_FILE_WRITE_ALL_BEGIN(fh, buf, size, MPI_INTEGER4, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_INT4_M(fh, buf, size)
        integer(ip),               intent(in)          :: fh
        integer(4), pointer,       intent(in)          :: buf(:,:)
        integer(ip),               intent(in)          :: size
        integer(4), pointer                            :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_INT8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        integer(8), pointer,   intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        integer(8), pointer                            :: dummy(:)
        ...
        if (.not.associated(buf)) then
            ...
            call MPI_FILE_WRITE_ALL_BEGIN(fh, buf, size, MPI_INTEGER8, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_INT8_M(fh, buf, size)
        integer(ip),               intent(in)          :: fh
        integer(8), pointer,       intent(in)          :: buf(:,:)
        integer(ip),               intent(in)          :: size
        integer(8), pointer                            :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_REAL8_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        real(8), pointer                               :: dummy(:)
        ...
        if (.not.associated(buf)) then
            ...
            call MPI_FILE_WRITE_ALL_BEGIN(fh, buf, size, MPI_REAL8, status, ierr)
        end if
        if (ierr/=MPI_SUCCESS) then
            call runend("File descriptor "//trim(intost(fh))//" could not be written. MPI error code: "//trim(intost(ierr)))
        end if
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_REAL8_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(8), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(8), pointer                               :: dummy(:,:)
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_REAL4_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        real(4), pointer                               :: dummy(:)
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_REAL4_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(4), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(4), pointer                               :: dummy(:,:)
...
#ifndef __PGI
    subroutine PAR_FILE_WRITE_ALL_BEGIN_REAL16_V(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(in)              :: buf(:)
        integer(ip),           intent(in)              :: size
        real(16), pointer                               :: dummy(:)
    ...
    subroutine PAR_FILE_WRITE_ALL_BEGIN_REAL16_M(fh, buf, size)
        integer(ip),           intent(in)              :: fh
        real(16), pointer,      intent(in)              :: buf(:,:)
        integer(ip),           intent(in)              :: size
        real(16), pointer                               :: dummy(:,:)
    ...
    subroutine PAR_FILE_SET_SIZE(fh, size)
        integer(ip),           intent(in)              :: fh
        integer(MPI_OFFSET_KIND),           intent(in) :: size
#ifndef MPI_OFF
        ierr=0
        call MPI_FILE_SET_SIZE(fh, int(size,MPI_OFFSET_KIND), ierr)
        if (ierr/=MPI_SUCCESS) then
    ...
    subroutine PAR_FILE_GET_SIZE(fh, size)
        integer(ip),           intent(in)              :: fh
        integer(MPI_OFFSET_KIND),           intent(inout):: size
#ifndef MPI_OFF
        ierr=0
        call MPI_FILE_GET_SIZE(fh, int(size,MPI_OFFSET_KIND), ierr)
        if (ierr/=MPI_SUCCESS) then
