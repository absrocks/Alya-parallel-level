...
!-----------------------------------------------------------------------------------------------------------------------------
    ...
    character(150)                          :: me="mod_mpio_seq_readom"                !
...
!-----------------------------------------------------------------------
    ...
    subroutine reageo_seq_mpio()
        if (IMASTER .or. ISEQUEN) then
            call livinf(-4_ip,'START GEOMETRY READING (SEQUENTIAL)',0_ip)
            call memgeo(1_ip)
            ...
            call livinf(-5_ip,'END GEOMETRY (SEQUENTIAL)',0_ip)
    ...
    subroutine reaset_seq_mpio()
        ...
        if (IMASTER .or. ISEQUEN) then
            call livinf(-4_ip,'START SET READING (SEQUENTIAL)',0_ip)
            ...
            call livinf(-5_ip,'END SET READING (SEQUENTIAL)',0_ip)
        end if
    end subroutine
    ...
    subroutine reabcs_seq_mpio()
        ...
        kfl_icodb = 0
        if (IMASTER .or. ISEQUEN) then
            call livinf(-4_ip,'START BCS READING (SEQUENTIAL)',0_ip)
            if (FILE_EXIST(fil_codno)) then
                call livinf(0_ip,'READ CODNO FILE',0_ip)
                call memory_alloca(memor_dom,'codno',vacal,kfl_codno, mcono, npoin)
            ...
            call livinf(-5_ip,'END BCS READING (SEQUENTIAL)',0_ip)
        endif
    end subroutine
    ...
    subroutine reafie_seq_mpio()
        ...
        nullify(a1,a2)
        call livinf(-4_ip,'START FIELD READING (SEQUENTIAL)',0_ip)
        if (IMASTER .or. ISEQUEN) then
            call memgeo(28_ip)
            do ifiel = 1,nfiel
                igene = ifiel
             ...
             call livinf(-5_ip,'END FIELD READING (SEQUENTIAL)',0_ip)
        end if
