...
!TO BE SHARED
    ...
    subroutine mpio_postpr_filter()
        call fildef(3_ip)
        if (kfl_filte/=0) then
            call runend("Oops, filter management is not implemented yet!!")
        end if
    end subroutine
    ...
    subroutine mpio_postpr_permu_init()
        ...
        if (kfl_perm .and. mpio_flag_post_merge==PAR_MPIO_ON) then
    ...
    subroutine mpio_postpr_permu_int_v()
      ...
      else if (mpio_flag_post_merge == PAR_MPIO_ON) then
         call memory_alloca(mpio_memor,'isca', vacal, isca, dim)
         if (dim>0) then
            do ii = 1,dim
    ...
    subroutine mpio_postpr_permu_end()
        character(100), PARAMETER :: vacal = " mpio_postpr_permu_end"
        if (kfl_perm .or. mpio_flag_post_merge==PAR_MPIO_ON) then
