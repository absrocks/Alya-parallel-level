...
module mod_mpio_par_io
    ...
    subroutine par_write_header(fh, header)
    ...
    subroutine PAR_INFO_AUTOROMIO_WRITE(file_size)
        integer(8),               intent(in)              :: file_size
#ifndef MPI_OFF
        if (mpio_flag_autoromio == PAR_MPIO_ON) then
            call PAR_INFO_SET("romio_ds_write", "disable")
            ! call PAR_INFO_SET("cb_buffer_size", "4194304")
            ! call PAR_INFO_SET("romio_no_indep_rw", "true")
            ! Manage small files
!            if (file_size<SMALL_FILE_SIZE) then
!                call PAR_INFO_SET("cb_nodes", "20")
!                call PAR_INFO_SET("romio_cb_write", "automatic")
!            else
                call PAR_INFO_SET("romio_cb_write", "enable")
!            end if
        end if
#endif
    end subroutine

    subroutine PAR_INFO_AUTOROMIO_READ(file_size)
        integer(8),               intent(in)              :: file_size
#ifndef MPI_OFF
        if (mpio_flag_autoromio == PAR_MPIO_ON) then
            call PAR_INFO_SET("romio_ds_read", "disable")
            call PAR_INFO_SET("romio_cb_read", "enable")
        end if
#endif
    end subroutine
    ...
    subroutine PAR_FILE_WRITE_ALL_INT_V(buf, filename, object, resultson, dim, divi, tag1, tag2, ittim, cutim, opt, nsubd)
        ...
        call PAR_INFO_AUTOROMIO_WRITE(header%file_size)
        ...
        call PAR_BARRIER(wherein_code)
        call par_write_header(fh, header)
        call PAR_FILE_SET_VIEW(fh, buf, int(header_size,8))
        offset = myoffset
        call PAR_FILE_SEEK_SET(fh, offset)
        if (mpio_flag_synchro==PAR_MPIO_ON) then
            call PAR_FILE_WRITE_ALL(fh, buf, dim)
            call PAR_FILE_CLOSE(fh)
        else
            call PAR_ASYNCHRONOUS_WRITE_POP_ALL(mpio_val_asyncbuffer, ittim)
    ...
    subroutine PAR_FILE_WRITE_ALL_INT_M(buf, filename, object, resultson, dim1, dim2, divi, tag1, tag2, ittim, cutim, opt, nsubd)
        ...
        call par_write_header(fh, header)
        call PAR_FILE_SET_VIEW(fh, buf, int(header_size,8))
        offset = myoffset * header%columns
        call PAR_FILE_SEEK_SET(fh, offset)
        if (mpio_flag_synchro==PAR_MPIO_ON) then
            call PAR_FILE_WRITE_ALL(fh, buf, int(header%columns*dim2, ip))
            call PAR_FILE_CLOSE(fh)
        else
            call PAR_ASYNCHRONOUS_WRITE_POP_ALL(mpio_val_asyncbuffer, ittim)
    ...
    subroutine PAR_FILE_READ_ALL_INT_V(buf, filename, dim, wherein, header, force_subd)
        ...
        call PAR_INFO_CREATE()
        call PAR_INFO_AUTOROMIO_READ(h%file_size)
    ...
    subroutine PAR_FILE_READ_ALL_INT_M(buf, filename, dim1, dim2, wherein, header, force_subd)
        ...
        if (IREADER .or. IMASTER) then
            offset = myoffset * h%columns
            call PAR_FILE_SEEK_SET(fh, offset)
            if (h%columns==int(dim1, header_ip)) then
                if (convert4to8) then
                    call PAR_FILE_READ_ALL(fh, buf4, int(h%columns*dim2, ip))
                    do j=1,h%columns
                        do i=1,dim2
                    ...
                    call PAR_FILE_READ_ALL(fh, buf, int(h%columns*dim2, ip))
                end if
                ...
                if (convert4to8) then
                    ...
                    call PAR_FILE_READ_ALL(fh, temp, int(h%columns*dim2, ip))
                    if (IREADER) then
