...
!       fil_time,              &      ! Time values file
  ...
  real(rp), pointer        :: &
       ...
       stretlam(:)                   ! 38: Fibers stretch (lamda(1) in stress model 134)            SOLIDZ
  ...
  real(rp)                 :: &
       volst(12,4) = 0.0_rp,  &      ! Cavities' volumes ,     NASTIN, ALEFOR, SOLIDZ)
       pwink(12,4) = 0.0_rp,  &      ! Windkessel pressures ,  NASTIN, ALEFOR, SOLIDZ)
       funin(3),              &      ! Function in
  ...
  real(rp),    pointer     :: &
       ...
       momen(:,:),            &      ! Accumulated momentum       PARTIS
       troponin(:),           &      ! Troponin vector for the coupled O'Hara-Land EXMEDI - SOLIDZ
       cell_ca0_ecc(:,:),     &      ! Ca0 concentration to automatically calibrate Hunter-McCulloch ECC model  EXMEDI - SOLIDZ
       troponin_prev(:)              ! Previous troponin vector for the coupled O'Hara-Land EXMEDI - SOLIDZ
  ...
  !
  ! EXMEDI - SOLIDZ: state variables
  !
  real(rp),    pointer     :: &
       stateland(:,:,:,:)            ! Land model state variables EXMEDI - SOLIDZ
