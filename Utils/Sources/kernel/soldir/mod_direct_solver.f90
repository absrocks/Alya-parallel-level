...
module mod_direct_solver
  ...
  use def_master,             only : kfl_paral,npart,lninv_loc
  use def_master,             only : npoi1
  use def_domain,             only : npoin,npoin_own,meshe !Paula_Debug_mUMPS  
  ...
  use mod_parall,             only : PAR_COMM_MY_CODE_WM4,PAR_COMM_MY_CODE4
  use def_domain,             only : nzsol
  use def_solver,             only : solve_sol,memdi
!  use mod_renumbering,        only : renumbering_lexical_order
  use mod_matrix,             only : matrix_heap_sort
  use mod_matrix,             only : matrix_copy_matrix

  use mod_mumps2alya,         only : direct_solver_initialization_mumps
  use mod_mumps2alya,         only : direct_solver_factorization_mumps
  use mod_mumps2alya,         only : direct_solver_solution_mumps
  use mod_mumps2alya,         only : direct_solver_cleaning_mumps
...
contains
  ...
  subroutine direct_solver_cleaning(direct_solver) 
    ...
    if(      direct_solver % kfl_solver == SOL_DIRECT_SOLVER_ALYA ) then
       ...
       direct_solver % memor = alya_CSR_memor
       call memory_deallo(direct_solver % memor,'DIRECT_SOLVER % IA_NEW','solver_direct_solver_initialize',direct_solver % ia_new)
       call memory_deallo(direct_solver % memor,'DIRECT_SOLVER % JA_NEW','solver_direct_solver_initialize',direct_solver % ja_new)
       call memory_deallo(direct_solver % memor,'DIRECT_SOLVER % PERMR' ,'solver_direct_solver_initialize',direct_solver % permr)
       call memory_deallo(direct_solver % memor,'DIRECT_SOLVER % INVPR' ,'solver_direct_solver_initialize',direct_solver % invpr)
...
!    use mod_renumbering, only : renumbering_lexical_order
    ...
    type(direct_solver_typ), intent(inout)          :: direct_solver       
    real(rp),               intent(in),   optional  :: aa(*)
    integer(ip)                                     :: ierro,ntotn,ii,nnz,ndofn
...
!    use mod_renumbering, only : renumbering_lexical_order
    ...
    if(       direct_solver % kfl_solver == SOL_DIRECT_SOLVER_SKYLINE_ALYA ) then
       ...
       !PAR_COMM_MY_CODE4 = MPI_COMM_WORLD
       !!call MPI_COMM_RANK(PAR_COMM_MY_CODE4,my_rank4,istat4)
       ...
       !!call MPI_COMM_SPLIT(PAR_COMM_MY_CODE4,jcolor4,ikey4,PAR_COMM_MY_CODE_WM4,istat4)
       !!call MPI_COMM_RANK(PAR_COMM_MY_CODE_WM4,my_rank_wm4,istat4)
...
!       call renumbering_lexical_order(meshe(ndivi),lninv_lex)  !  lo elimino guillauem comenzara wsmp de 0
       ...
       if( INOTMASTER ) then
...
#ifdef PWSMP
          call wsetmpicomm(PAR_COMM_MY_CODE_WM4)
#endif
          print*,'pasa2'
          !!end if
          !if (INOTMASTER) then
          ...
          !print*,'A=',kfl_paral
          !call wsetmaxthrds(1)
          !print*,'B=',kfl_paral,PAR_COMM_MY_CODE_WM4
