  ...
  subroutine Symbolical_CSR_LU(nbrows,iA,jA,iL,jL,iU,jU,memor_opt)
    ...
    if( associated(iL) .or. associated(jL) .or. associated(iU) .or. associated(jU) ) then
       ...
       do i= 1, nbrows
          ...
          totalL = totalL + nxt
          nzL(i) = nxt
          call memory_alloca(memor,'ptL(i)','Symbolical_CSR_LU',ptL(i) % l,nxt,'DO_NOT_INITIALIZE')          
          ...
          totalU = totalU + nxt
          nzU(i) = nxt
          call memory_alloca(memor,'ptU(i)','Symbolical_CSR_LU',ptU(i) % l,nxt,'DO_NOT_INITIALIZE')                    
