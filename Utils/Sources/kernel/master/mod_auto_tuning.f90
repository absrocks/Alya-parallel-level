...
contains
  ...
  subroutine auto_tuning_SpMV_OpenMP()
    ...
    use def_master,         only : INOTMASTER,IMASTER,solve_sol,kfl_paral
          ...
          if( associated(momod(modul) % solve) ) then
             do ivari = 1,size(momod(modul) % solve,KIND=ip)
                ...
                if(    solve_sol(1) % kfl_algso >  0 .and.   &
                   ...
                   if( ipass == 0 ) then
                      call livinf(-4_ip,'START AUTO-TUNING OF OPENMP FOR SpMV',0_ip)
                      ipass = 1
                      call outfor(87_ip,lun_outpu,' ')
                   end if
                   call livinf(-4_ip,'START AUTO-TUNING FOR '//trim(solve_sol(1) % wprob),0_ip)
                   ...
                   call livinf(-5_ip,'END AUTO-TUNING',0_ip)
    ...
    if( ipass == 1 ) call livinf(-5_ip,'END AUTO-TUNING OF OPENMP FOR SpMV',0_ip)
