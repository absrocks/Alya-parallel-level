...
subroutine solver(rhsid,unkno,amatr,pmatr)
  ...
  if(  solve_sol(1) % kfl_clean_precond == 0 .and. &
       solve_sol(1) % nsolv >= 1 ) then
  ...
  !if(  solve_sol(1) % kfl_algso == SOL_SOLVER_SPARSE_DIRECT .and. &
  !     solve_sol(1) % direct_solver % kfl_solver == SOL_DIRECT_SOLVER_MUMPS ) then
  !   exchange = .false.
  !end if


  if( solve_sol(1) % kfl_recov /= 2 .and. exchange .and. npoin > 0 ) then
        ...
        if( INOTMASTER ) then
           ...
           if( solve_sol(1) % nblok < 0 ) then
              if( solve_sol(1) % kfl_full_rows == 0 ) then
                 ...
                 call matrix_blokgs(&
                      1_ip,solve_sol(1) % nequa,solve_sol(1) % ndofn_per_block,nblok,iblok,r_dom,c_dom,amatr_full,rhsid,unkno,&
                      aa_idofn,bb_idofn,xx_idofn,memma, solve_sol(1) % lperm_block)
              end if
           else
              if( solve_sol(1) % kfl_full_rows == 0 ) then
        ...
        if( INOTMASTER ) then
           ...
           if( solve_sol(1) % nblok < 0 ) then
              if( solve_sol(1) % kfl_full_rows == 0 ) then
                 ...
                 call matrix_blokgs(&
                      2_ip,solve_sol(1) % nequa,solve_sol(1) % ndofn_per_block,nblok,iblok,r_dom,c_dom,amatr_full,rhsid,unkno,&
                      aa_idofn,bb_idofn,xx_idofn,memma, solve_sol(1) % lperm_block)
              end if
           else
              if( solve_sol(1) % kfl_full_rows == 0 ) then
     ...
     if( INOTMASTER ) then
        if( solve_sol(1) % nblok < 0 ) then
           if( solve_sol(1) % kfl_full_rows == 0 ) then
              ...
              call matrix_blokgs(&
                   3_ip,solve_sol(1) % nequa,solve_sol(1) % ndofn_per_block,nblok,iblok,r_dom,c_dom,amatr_full,rhsid,unkno,&
                   aa_idofn,bb_idofn,xx_idofn,memma, solve_sol(1) % lperm_block)
           end if
        else
           if( solve_sol(1) % kfl_full_rows == 0 ) then
