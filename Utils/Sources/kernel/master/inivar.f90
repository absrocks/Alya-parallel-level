              ...
              do ivari = 1,size(momod(modul) % solve,KIND=ip)
                 ...
                 if( solve_sol(1) % kfl_algso /= -999 ) then
                    ...
                    if( solve_sol(1) % kfl_save_krylov /= 0 ) then
                       ...
                       if( INOTMASTER ) then
                          call memory_alloca(mem_modul(1:2,modul),'SOLVE_SOL(1) % KRYLOV_DIR','inivar',&
                               solve_sol(1) % krylov_dir,solve_sol(1) % nunkn,solve_sol(1) % kfl_save_krylov)
                          call memory_alloca(mem_modul(1:2,modul),'SOLVE_SOL(1) % KRYLOV_DOT_PRODUCTS','inivar',&
                    ...
                    if(  solve_sol(1) % kfl_algso == SOL_SOLVER_DIRECT        .or. &
                      ...
                      if(       solve_sol(1) % kfl_algso == SOL_SOLVER_DIRECT ) then
                       ...
                       if( solve_sol(1) % kfl_symeq == 1 ) then
                          ...
                          solve_sol(1) % direct_solver_RAS       % kfl_symmetric = 1
...
!!$                                   solve_sol(1) % kfl_react = max(solve_sol(1) % kfl_react,1)
