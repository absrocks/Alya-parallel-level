subroutine moddef(itask)
  ...
  integer(ip)             :: ivari,jtask,imod0,imod1,imod2,nsolv
  ...
  if ( jtask == 1_ip .and. INOTSLAVE ) then
     ...
     do modul = imod1,imod2
        ...
        !if (modul==10_ip) then   !only for solidz module
        !   momod(modul) % lun_time = modul * lun_base + 11
        !end if
     ...
     tgcod     => momod(modul) % tgcod
     tbcod     => momod(modul) % tbcod
!     print*,'kfl_paral,modul,associated(momod(modul) % postp)',kfl_paral,modul,associated(momod(modul) % postp)
!     flush(6)
     veset     => postp(1)     % veset
     vbset     => postp(1)     % vbset
     vnset     => postp(1)     % vnset
