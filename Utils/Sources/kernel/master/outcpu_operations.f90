...
subroutine outcpu_operations(meshe)
  ...
  real(rp)                    :: alpha,xfact1,xfact2,xfact3,xfact4,bet
  real(rp)                    :: max1,max2
  real(rp)                    :: time1,time2,time3
  real(rp)                    :: my_timing(3)
  integer(4)                  :: NNDZ,V,NV,ND,maxell,tmp,resl,kk
  integer(8)                  :: emptystr,sz
  real(rp),    allocatable    :: valell(:,:)
  integer(ip), allocatable    :: colell(:,:)
  type(soltyp)                :: solve

  if(  kfl_algebra_operations > 0   .and. &
