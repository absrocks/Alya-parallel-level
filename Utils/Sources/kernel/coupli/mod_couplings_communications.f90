...
module mod_couplings_communications
  ...
  use def_kintyp,         only : ip,rp,lg,r1p,i1p,spmat,comm_data_par  
...
contains
  ...
  !>
  !----------------------------------------------------------------------
  subroutine COU_GENERATE_LOCAL_TRANSMISSION_MATRICES(coupling)
    ...
    call memory_alloca(memor_cou,'permm'     ,vacal,permm,npoin)
       ...
       if( coupling % itype == ELEMENT_INTERPOLATION ) then
          ...
          if( coupling % geome % nelem_source > 0 ) then
             ...
             do ineig = 1,nneig
                ...
                call memory_alloca(memor_cou,'spmat_send',vacal,spmat_send(ineig) % a,icont*3)
                call memory_alloca(memor_cou,'perms',vacal,perms(ineig) % l,kcont)
          ...
          if( coupling % geome % npoin_source > 0 ) then
             ...
             do ineig = 1,nneig
                ...
                call memory_alloca(memor_cou,'spmat_send',vacal,spmat_send(ineig) % a,nsend*3)
                call memory_alloca(memor_cou,'perms',vacal,perms(ineig) % l,kcont)
    ...
    if(I_AM_TARGET) then
       ...
       !
       nullify(inv_status)
       call memory_alloca(memor_cou,'status_inv',vacal,inv_status,coupling % commd % lrecv_dim)
       if( coupling % itype /= PROJECTION .and. coupling % itype /= STRESS_PROJECTION) then
       ...
       do ineig = 1,nneig
          ...
          call memory_alloca(memor_cou,'ltransmat_target(ineig)',vacal,& 
               coupling % ltransmat_target(ineig),1_ip,nentr_ineig)
       ...
       do ineig = 1,nneig
          ...
          coupling % ltransmat_target(ineig) % ndof  = 1
       end do
       !
       ! For projection schemes, integrate gaussian points: multiply received matrix by projection matrix
       !
       if( coupling % itype == PROJECTION .or. coupling % itype == STRESS_PROJECTION) then
          if(coupling % wet % npoin_wet > 0) then 
             ...
             do ineig = 1,nneig
                if(associated(coupling % ltransmat_target(ineig) % iA)) then
                   ...
                   call memory_deallo(memor_cou,'coupling % ltransmat_target(ineig)',&
                        vacal,coupling % ltransmat_target(ineig))
                   call memory_alloca(memor_cou,'ltransmat_target(ineig)',vacal,&
                        coupling % ltransmat_target(ineig), 1_ip, int(size(M_aux % iA),ip))
                   coupling % ltransmat_target(ineig) % ndof  = M_aux % ndof
    ...
    !--------------------------------------------------------------------
    !
    ! Generate new communicatio schemme   
    !
    !--------------------------------------------------------------------
    call cputim(time1)
    if( nneig > 0) then
       if(I_AM_SOURCE) then 
          ...
          call memory_alloca(memor_cou,'lsend_perm',vacal,coupling % commd % lsend_perm,&
 ...
 subroutine COU_GENERATE_TRANSPOSED_LOCAL_TRANSMISSION_MATRICES(coupling)
    ...
    if( coupling % itype == TRANSPOSE_MIRROR) then
       ...
       if(nneig > 0) then
          ...
          call memory_alloca(memor_cou,'neights'                      ,vacal,coupling % commd % neights   ,nneig)
          call memory_alloca(memor_cou,'spmat_send'                   ,vacal,spmat_send                   ,nneig)
          call memory_alloca(memor_cou,'spmat_recv'                   ,vacal,spmat_recv                   ,nneig)
          call memory_alloca(memor_cou,'nnent_recv'                   ,vacal,nentr_recv                   ,nneig)
          call memory_alloca(memor_cou,'nentr_send'                   ,vacal,nentr_send                   ,nneig)
          call memory_alloca(memor_cou,'npoin_send'                   ,vacal,npoin_send                   ,nneig)
          call memory_alloca(memor_cou,'npoin_recv'                   ,vacal,npoin_recv                   ,nneig)
          ...
          if( I_AM_SOURCE .and. INOTMASTER) then
             ...
             call memory_alloca(memor_cou,'perms',vacal,multip_source,npoin)
             multip_source(1:npoin) = 1.0_rp
             ...
             do ineig = 1, nneig
                if(associated(coupling_type(micou) % ltransmat_target(ineig) % iA)) then
                   ...
                   call memory_alloca(memor_cou,'spmat_send',vacal,spmat_send(ineig) % a,inentr*3_ip)
                   ...
                   call memory_alloca(memor_cou,'perms',vacal,perms(ineig) % l,npose)
             ...
             call memory_deallo(memor_cou,'perms',vacal,multip_source)
          ...
          if(I_AM_TARGET .and. INOTMASTER) then
             ...
             cont_recv = 0
             call memory_alloca(memor_cou,'coupling % ltransmat_target',vacal,coupling % ltransmat_target,nneig)
             ...
             do ineig = 1,nneig
                ...
                if(inentr > 0) then
                   call memory_alloca(memor_cou,'ltransmat_target(ineig)',vacal,coupling % ltransmat_target(ineig),1_ip,inentr)
                   do ientr = 1, inentr
 ...
 subroutine COU_PARALLELIZE_TRANSMISSION_MATRICES(coupling)
    ...
    integer(ip)                                    :: dummi(1) 
    integer(ip)                                    :: PAR_COMM_COUP, PAR_COUP_RANK, PAR_COUP_SIZE
    integer(ip)                                    :: PAR_COMM_TARG, PAR_TARG_RANK, PAR_TARG_SIZE
    ...
    if(I_AM_TARGET) then
       ...
       !    process in the target communicator, PAR_COUP_RANK, and in the coupling 
    ...
    if(I_AM_TARGET .and. INOTMASTER) then
       ...
       do ipoin =1,npoin
          if(lnent(ipoin)>0) then
             call memory_alloca(memor_cou,'llent(ipoin)',vacal,llent(ipoin) % a,lnent(ipoin)*3)
             lnent(ipoin)=0
          endif
       end do
       if(associated(M)) then
       ...
       do ipart = 1, PAR_COUP_SIZE
          if(lncol_demand_tar(ipart) > 0 ) then
             call memory_alloca(memor_cou,'llcol_demand_tar(ipart) % l',vacal,llcol_demand_tar(ipart) % l,&
       ...
       ! 
       do ineig = 1, nneig_tar
          call memory_alloca(memor_cou,'llent_send(ineig)',vacal,llent_send(ineig) % a,4*lnent_send(ineig))
          call memory_alloca(memor_cou,'llent_recv(ineig)',vacal,llent_recv(ineig) % a,4*lnent_recv(ineig))
       ...
       do ipart = 1,PAR_COUP_SIZE
         if(lnent_add(ipart) > 0) then
            call memory_alloca(memor_cou,'llent_add(ipart)',vacal,llent_add(ipart) % a,lnent_add(ipart)*4)
            lnent_add(ipart) = 0
         endif
       end do
       do ineig =1,nneig_tar
       ...
       if(dim_prev_tar /= dim_new_tar) then
          ...
          do ineig = 1,nneig_new_tar
             ...
             call memory_alloca(memor_cou,'M_new',vacal,M_new(ineig),1_ip,nentr)
    ...
    if(I_AM_SOURCE .and. INOTMASTER) then
       ...
       do ipart = 1,PAR_COUP_SIZE
          if(lnsend_add(ipart) > 0) then
             call memory_alloca(memor_cou,'lnsend_add(ipart)',vacal,llsend_add(ipart) % l,lnsend_add(ipart))
             lnsend_add(ipart) = 0
          end if
       end do
       do ineig = 1,nneig_prev
    ...
    if(INOTMASTER .and. I_AM_TARGET) then
       if(dim_prev_tar /= dim_new_tar) then
          ...
          if(associated(coupling % commd % lrecv_size)) &
             call memory_deallo(memor_cou,'lrecv_size',vacal, coupling % commd % lrecv_size)
          if(associated(coupling % commd % lsend_size) .and. (.not. I_AM_SOURCE)) &
             call memory_deallo(memor_cou,'lsend_size',vacal, coupling % commd % lsend_size)
          if(associated(coupling % ltransmat_target)) &
             call memory_deallo(memor_cou,'coupling % ltransmat_target',vacal,coupling % ltransmat_target)
          ...
          call memory_alloca(memor_cou,'coupling % ltransmat_target',vacal,coupling % ltransmat_target,nneig_new_tar)
          M => coupling % ltransmat_target
          ...
          do ineig = 1,nneig_new_tar
             if(associated(M_new(ineig) % iA)) then
                ...
                call memory_alloca(memor_cou,'coupling % ltransmat_target(ineig)',vacal,M(ineig),1_ip,nentr)
                M(ineig) % nrows = M_new(ineig) % nrows
                M(ineig) % ncols = M_new(ineig) % ncols
                M(ineig) % ndof  = M_new(ineig) % ndof 
                do ientr = 1,nentr
