...
contains
  ...
  subroutine COU_DEALLOCATE_WET(wet)
    ...
    call memory_deallo(memor_cou,'VMASB_WET','DEALLOCATE_COUPLING_STRUCTURE_WET',wet % vmasb_wet  )
    call memory_deallo(memor_cou,'LBOUN',    'DEALLOCATE_COUPLING_STRUCTURE_WET',wet % lboun_wet  )
    call memory_deallo(memor_cou,'LPOIN',    'DEALLOCATE_COUPLING_STRUCTURE_WET',wet % lpoin_wet  )
    call memory_deallo(memor_cou,'COORD',    'DEALLOCATE_COUPLING_STRUCTURE_WET',wet % coord_wet  )
    call memory_deallo(memor_cou,'WEIGH',    'DEALLOCATE_COUPLING_STRUCTURE_WET',wet % weight_wet )

    call memory_deallo(memor_cou,'SCHED','DEALLOCATE_COUPLING_STRUCTURE_WET',wet % mass_ratio_wet )
  ...
  subroutine COU_DEALLOCATE_GEOMETRY(geome)
    ...
    call memory_deallo(memor_cou,'LELEM' , 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % lelem_source )
    call memory_deallo(memor_cou,'LBOUN' , 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % lboun_source )
    call memory_deallo(memor_cou,'LPOIN' , 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % lpoin_source )
    call memory_deallo(memor_cou,'STATUS', 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % status       )
    call memory_deallo(memor_cou,'SHAPF' , 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % shapf        )    
    call memory_deallo(memor_cou,'VMASB' , 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % vmasb        )
    call memory_deallo(memor_cou,'SCHED' , 'DEALLOCATE_COUPLING_STRUCTURE_GEOME',geome % sched_perm   )
  ...
  subroutine COU_DEALLOCATE_TRANSMISSION(ltransmat)
    ...
    if( associated(ltransmat) ) then
       ...
       ltransmat(:) % nrows = 0
       ltransmat(:) % ncols = 0  
       call memory_deallo(memor_cou,'coupling % values','COU_DEALLOCATE_SINGLE_COUPLING',ltransmat)
    end if
  ...
  subroutine COU_DEALLOCATE_SINGLE_COUPLING(coupling,WET)
    ...
    call PAR_DEALLOCATE_COMMUNICATION_ARRAY(coupling % commd,memor_cou,PAR_COMM_OPT=.false.)
