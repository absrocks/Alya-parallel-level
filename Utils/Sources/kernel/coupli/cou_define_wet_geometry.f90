...
subroutine cou_define_wet_geometry()
  ...
  use def_master,         only :  kfl_timin,kfl_async
  use def_master,         only :  lzone,NBOUN_TYPE
  use def_master,         only :  momod,mmodu
  use def_domain,         only :  lnnob
  use def_domain,         only :  nboun,lbset
  use def_domain,         only :  xfiel,kfl_codbo
  use def_domain,         only :  npoin,coord,ndimb
  use def_domain,         only :  elmar,ngaus,lnoch
...
use mod_coupling_timer 
  ...
  implicit none
  integer(ip)                  :: icoup,ipoin,iboun,kboun,inodb,kpoin
  integer(ip)                  :: set_target,zone_target,code_target,bcode_target
  integer(ip)                  :: nboun_wet,pblty,igaub,ipass,ipoin_global
  integer(ip)                  :: field_target,number_wet_nodes,iboun_wet
  integer(ip)                  :: number_wet_points,ngaub,pnodb,iboun_global
  ...
  real(rp)                     :: time1,time2,time3,time4,time5,time6

  real(rp)                     :: posgb(ndime-1,3_ip)
  real(rp)                     :: weigb(3_ip),shapb(64)
  ...
  if( mcoup > 0 ) then
     ...
     !do icoup = 1,mcoup 
     !   call COU_INITIALIZE_COUPLING_STRUCTURE(coupling_type(icoup))
     !end do
     ...
     do ipass = 1,2
        ...
        do icoup = 1,mcoup
           ...
           if( I_must_compute_something ) then
              ...
              if( coupling_type(icoup) % where_type == ON_SET ) then
                 ...
                 call memory_alloca(memor_cou,'LBOUN_WET','cou_initialize_coupling',coupling_type(icoup) % wet % lboun_wet,nboun_wet)
              ...
              call memory_alloca(memor_cou,'LPOIN_WET' , 'cou_initialize_coupling',coupling_type(icoup) % wet % lpoin_wet, number_wet_nodes)
              call memory_alloca(memor_cou,'WEIGHT_WET', 'cou_initialize_coupling',coupling_type(icoup) % wet % weight_wet,number_wet_nodes) 
              !
              ...
              if(    coupling_type(icoup) % itype == STRESS_PROJECTION .or. &
                 ...
                 call memory_alloca(memor_cou,'coupling_type(icoup) % wet % COORD_WET','cou_initialize_coupling',&
                 ...
                 coupling_type(icoup) % wet % number_wet_points = number_wet_points
                 call memory_alloca(memor_cou,'COORD_WET','cou_initialize_coupling',&
                      coupling_type(icoup) % wet % coord_wet,ndime,number_wet_points)   
                 number_wet_points = 0
                 do ipoin = 1,npoin
                    if( list_wet_nodes(ipoin) /= 0 ) then
           ...
           !call coupling_timer_dynamic_statistics( coupling_type(icoup)%geome%npoin_wet, -1_ip ) 
           call coupling_timer_dynamic_statistics( number_wet_nodes, -1_ip )   !< 2017ABR06
           call cputim(time6)
           coupling_type(icoup) % cputim(1) = coupling_type(icoup) % cputim(1) + time6 - time5
        end do
     end do

     if ( INOTMASTER ) call memory_deallo(memor_cou,'LIST_WET_NODES','cou_initialize_coupling',list_wet_nodes)
