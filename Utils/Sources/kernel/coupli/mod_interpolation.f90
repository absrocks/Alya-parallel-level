...
module mod_interpolation
  ...
  use def_kintyp,         only : ip,rp,lg,r1p,r2p,i1p,spmat
  ...
  !public :: COU_GET_DIAGONAL_MATRIX
...
contains 
  ...
  subroutine COU_GET_INTERPOLATE_POINTS_VALUES_RP_1(xx_in,xx_recv,coupling)
    ...
    if( associated(xx_in) ) then
       if( size(xx_in) < npoin .and. PAR_MY_CODE_RANK /= 0 ) call runend('WRONG SIZE 1: CANNOT INTERPOLATE')
    end if
  ...
  subroutine COU_GET_INTERPOLATE_POINTS_VALUES_RP_2(xx_in,xx_recv,coupling)
    ...
    if( associated(xx_in) ) then
       if( size(xx_in,2) < npoin .and. PAR_MY_CODE_RANK /= 0 ) call runend('WRONG SIZE 2: CANNOT INTERPOLATE')
    end if
  ...
  subroutine COU_GET_INTERPOLATE_POINTS_VALUES_RP(ndofn,xx_in,xx_out,coupling,what_to_do)
    ...
    !
    call cputim(time1)
    call PAR_SEND_RECEIVE_TO_ALL(ndofn,xx_in,xx_recv,coupling % commd,'ASYNCHRONOUS',PERMUTE_SEND=.true.)
    call cputim(time2)
    !
...
!!$    end if
  ...
  subroutine COU_CREATE_A_SUBMESH(mask,coupling)
    ...
    integer(ip),              intent(in),   pointer :: mask(:)
    type(typ_color_coupling), intent(inout)         :: coupling
    ...
    if( nboun_wet > 0 ) then
       ...
       kpoin = 0
       do iboun_wet = 1,nboun_wet
          iboun = coupling % wet % lboun_wet(iboun_wet) 
  ...
  subroutine COU_ELEMENTS_IN_COLOR(icolo,necol,lecol)
    ...
    logical(lg), pointer, intent(inout) :: lecol(:)
  ...
  subroutine COU_NODES_IN_COLOR(icolo,nncol,lncol)
    ...
    logical(lg), pointer, intent(inout) :: lncol(:)
    integer(ip)                         :: isubd,izone,icode
  ...
  subroutine COU_BOUNDARIES_IN_COLOR(icolo,nbcol,lbcol)
    ...
    logical(lg), pointer, intent(inout) :: lbcol(:)
    integer(ip)                         :: isubd,izone,icode
    integer(ip)                         :: kelem,ielem,necol
...
!!$ subroutine COU_DIAGONAL_DIRICHLET(ndofn,xx_in,xx_out,coupling,what_to_do)
!!$
!!$    implicit none
!!$    integer(ip),              intent(in)           :: ndofn
!!$    real(rp),                 intent(in)           :: xx_in(ndofn,*)
...
!!$    xx_recv = 1.0_rp
!!$    !
!!$    ! Operate Transmission matrix Td I
!!$    !
!!$    if(associated(coupling % ltransmat_target)) then
!!$       done_ini = .false.
!!$       do ineig = 1,coupling % commd % nneig
...
!!$    !
!!$    do ipoin = 1,npoin
!!$       xx_out(ipoin) = xx_out(ipoin) * xx_in(ipoin)
!!$    end do
!!$    !
!!$    ! Deallocate arrays
!!$    !
!!$    call memory_deallo(memor_cou,'xx_recv',vacal,xx_recv)
!!$
!!$  end subroutine COU_DIAGONAL_DIRICHLET
!!$
!!$  subroutine COU_GET_DIAGONAL_MATRIX(ndofn,xx_in,xx_out,coupling,what_to_do)
!!$
!!$    implicit none
!!$    integer(ip),              intent(in)           :: ndofn
!!$    real(rp),                 intent(in)           :: xx_in(ndofn,*)
...
!!$    character(*),             intent(in), optional :: what_to_do
!!$
!!$    if( coupling % kind == BETWEEN_SUBDOMAINS  .and. &
!!$        coupling % what == UNKNOWN ) then
!!$       call COU_DIAGONAL_DIRICHLET(ndofn,xx_in,xx_out,coupling,what_to_do)
!!$    end if
!!$    
!!$    call COU_GET_INTERPOLATE_POINTS_VALUES_RP(ndofn,xx_in,xx_out,coupling,what_to_do)
!!$    
!!$  end subroutine COU_GET_DIAGONAL_MATRIX
!!$    
