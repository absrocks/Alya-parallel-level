...
subroutine out_direct_solvers()
  ...
  use def_master,        only : kfl_modul,mmodu,momod,IMASTER,kfl_paral
              ...
              if( solve_sol(1) % kfl_algso /= -999 ) then
                 ...
                 do while( ipass <= 6 )
                    if(      ipass == 1 ) then
                       ...
                       kfl_solver = 0
                       !ii = ii + 1
                       !print*,'a=',ii,associated(solve_sol(1) % direct_solver_RAS),size(solve_sol(1) % direct_solver_RAS,KIND=ip)
                       ...
                       !end if
                       !if( kfl_solver /= 0 ) call direct_solver_statistics(solve_sol(1) % direct_solver_RAS(ii),solver_statistics)

                       if( ii == size(solve_sol(1) % direct_solver_RAS,KIND=ip) .or. kfl_solver == 0 ) ipass = 6
