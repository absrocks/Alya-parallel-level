subroutine posppm()
  ...
  if( kfl_pixel > 0 .and. mod(ittim,kfl_pixel) == 0 ) then
...
subroutine marpla(a,b,c,d,marked_nodes)
  ...
  if( INOTMASTER ) then
     ...
     !  
     call parari('SLX',NPOIN_TYPE,npoin,marked_nodes)
     do ipoin = 1,npoin
