...
contains
  ...
  subroutine outfor(itask,nunit,messa,ilist)
    ...
    integer(ip),      pointer,  optional, intent(in) :: ilist(:)                                !< Optional list
    integer(ip)                                      :: jtask,ii,nsolv,isolv,memor_value4,dummi
    integer(ip)                                      :: dumm1,dumm2
    integer(ip)                                      :: chunk_min,chunk_max,chunk_ave,ngpu
    integer(ip),  save                               :: ipass=0
    real(rp),     save                               :: xtime=0.0_rp
    real(rp)                                         :: dummr,load_balance_assembly
    real(rp)                                         :: load_balance_boundary,unit_factor,unit_dot_factor
    ...
    if( INOTSLAVE .or. itask <= 0 ) then
       ...
       case(0_ip)
          ...
          ! End of Alya (runend) 
          !
          if( present(messa) ) write(nunit4,1) trim(messa)
       ...
       case(2_ip)
          ...
          if( present(messa) ) then
             if( nunit4 == 0 ) then
                ...
                call livinf(-17_ip,trim(messa),0_ip)
       ...
       case(7_ip)
          ...
          ! Time step (Begste)
          !
          !if( kfl_outpu == 1) write(nunit4,700) ittim
       ...
       case(9_ip)
          ...
          !
          if( kfl_outpu == 1) write(nunit4,900) coutp(1),routp(1)

       case(10_ip)
          !
          ! Coupling convergence (Concou)
          !
          if( kfl_outpu == 1 ) write(nunit4,1000) itcou,iblok,routp(1),routp(1)-xtime
          xtime = routp(1)
       ...
       case(15_ip)
          ...
          ! Time step (setgts)
          !
          if( kfl_outpu == 1) write(nunit4,1500) ioutp(1),routp(1),routp(2)
       ...
       case(16_ip)
          ...
          if( kfl_rstar == 2 ) then
             ...
             write(nunit4,1610) adjustl(trim(title))
          end if
       ...
       case(17_ip)
          ...
          elem1 = intost(ioutp(3))
          elem2 = intost(ioutp(4))
          write(nunit4,1700)&
       ...
       case(23_ip)
          ...
          ! Memory (outmem)
          !
          write(nunit4,2300) coutp(1)(1:6),routp(1),coutp(2)(1:6)
       ...
       case(26_ip)
          ...
          ! End of run (Turnof)
          !
          write(nunit4,600) trim(naser(servi))

       case(27_ip)
          !
          ! Title (*_outinf)
          !
          write(nunit4,1200) naser(servi),trim(title)
       ...
       case(40_ip)
          ...
          if( adjustl(coutp(1)(1:4)) == 'Path'.and.adjustl(coutp(1)(1:3))/='END' ) then
             ...
             do while(coutp(ii)/='END')
                write(nunit4,4001) trim(coutp(ii))
                ii=ii+1
             end do
          end if
       ...
       case(41_ip)
          ...
          if( adjustl(coutp(1)(1:3))/='END' ) then
             ...
             do while(coutp(ii)/='END'.and.ii<10)
                write(nunit4,4101) adjustl(trim(coutp(ii)))
                ii=ii+1
             end do
             write(nunit4,4103)
             ii=10
       ...
       case(73_ip)
          ...
          if( IMASTER ) then
             ...
             if( VECTOR_SIZE == -1 ) then
                write(lun_outpu4,7306)
             else
                write(lun_outpu4,7305) VECTOR_SIZE
             end if
#else
       ...
       case(77_ip)
          ...
          if(      kfl_partition_par == PAR_METIS4 ) then
             ...
             write(lun_outpu4,7700) 'READ FROM ELEMENT FIELD '//trim(intost(-kfl_partition_par))
       ...
       case(91_ip)
          ...
          if( kfl_outpu == 1) write(nunit4,9100)
       ...
       case(97_ip)
          ...
          if( present(ilist) ) then
             ...
             do ii = 1,ioutp(2)                
                write(nunit4,9701) element_type(ilist(ii)) % name,routp(ii+1)
             end do
          end if
