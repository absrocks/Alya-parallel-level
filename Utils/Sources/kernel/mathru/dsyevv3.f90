...
!     .. Local Variables ..
      real(rp) :: NORM, N1, N2, N1TMP, N2TMP
      real(rp) :: THRESH, ERROR, WMAX, F, T
      INTEGER(ip) ::           I, J
...
!           Find nonzero element of v[0] and swap it with the next one
            IF (Q(J, 1).NE.0.0D0) THEN
              NORM = 1.0D0 / SQRT(Q(J, 1)**2 + Q(1 + MOD(J,3_ip), 1)**2)
              Q(J, 2)              = Q(1 + MOD(J,3_ip), 1) * NORM
              Q(1 + MOD(J,3_ip), 2)   = -Q(J, 1) * NORM
              Q(1 + MOD(J+1,3_ip), 2) = 0.0D0
              GO TO 80
            END IF
   70     CONTINUE
        END IF
