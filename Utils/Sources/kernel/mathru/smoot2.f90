...
!> @date    13/05/2013
!> @author  Herbert Owen
!> @brief   Smooth a variable defined on elements ( similar smooth but constant per element) - Perhaps use molule with interface
!> @details Smooth a variable defined on elements ( similar smooth but constant per element)
!> @} 
!------------------------------------------------------------------------
subroutine smoot2(velem,vpoin)
        ...
        if( pelty > 0 ) then
           ...
           gauss_points: do igaus = 1,pgaus
              ...
              do inode = 1,knode
                 ...
                 vpoin(ipoin) = vpoin(ipoin) + xfact * velem(ielem)
                 do jnode = 1,pnode
