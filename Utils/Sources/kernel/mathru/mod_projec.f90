...
module mod_projec
  ...
  use def_master,         only : party,pardi,parki,parr2,IMASTER,&
       ...
       &                         INOTSLAVE,zeror,gevec,kfl_paral
  ...
  use def_domain,         only : ndime,npoin,nelem,nnode,mnode,ntens,&
       ...
       &                         lelbo,mgaus,lelch,nboun,ltypb,ndimb,&
       &                         lnnob,mgaub,memor_dom,mnode
...
contains
  ...
  subroutine projec_mass_conservation(unkno,lboun,lpoin,what,xmass_opt,where_opt,given_normal_opt,ERROR)
    ...
    real(rp),     intent(inout)                    :: unkno(ndime,*)
    ...
    real(rp)                                       :: ulocnorm, cosang, ugivenorm
    ...
    if( INOTMASTER ) then
       ...
       if( global_mass_normal ) then
          call PAR_SUM(rrr,trim(where_message))
          rr = rr + rrr
       end if
