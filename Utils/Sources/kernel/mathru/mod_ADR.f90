...
contains
  ...
  subroutine ADR_parallel_data(ADR)
    ...
    call iexcha(ADR % kfl_time_integration   )
    call iexcha(ADR % kfl_time_step_strategy )
    call iexcha(ADR % kfl_stabilization      )
    call iexcha(ADR % kfl_shock              )
    call iexcha(ADR % kfl_time_lumped        )
    call iexcha(ADR % kfl_linearization      )
    call iexcha(ADR % kfl_tau_strategy       )
    call iexcha(ADR % kfl_laplacian          )
    call iexcha(ADR % kfl_time_sgs           )
    call iexcha(ADR % kfl_nonlinear_sgs      )
    call iexcha(ADR % kfl_time_bubble        )
    call iexcha(ADR % kfl_time_scheme        )
    call iexcha(ADR % kfl_time_order         )
    call iexcha(ADR % kfl_manufactured       )
    call iexcha(ADR % kfl_length             )
    call iexcha(ADR % kfl_first_order_sgs    )
    call iexcha(ADR % kfl_discretization     )
    call iexcha(ADR % number_euler_steps     )

    call rexcha(ADR % bemol                  ) 
    do ii = 1,size(ADR % tau_parameters,KIND=ip)
       call rexcha(ADR % tau_parameters(ii)  )
    end do
    call rexcha(ADR % shock                  ) 
    call rexcha(ADR % relax_sgs              ) 

    call iexcha(ADR % ntime) 
    do ii = 1,size(ADR % time_parameters,KIND=ip)
       call rexcha(ADR % time_parameters     )    
    end do
    call iexcha(ADR % number_components      ) 
    call rexcha(ADR % dtinv                  ) 
    do ii = 1,size(ADR % dtinv_old,KIND=ip)
       call rexcha(ADR % dtinv_old(ii)       ) 
    end do
  ...
  subroutine ADR_assemble_extension(ndofn,meshe_in,elmar_in,veloc,amatr,rhsid)
    ...
    if( INOTMASTER ) THEN
       ...
       ADR % kfl_time_integration   = OFF           ! Time flag              
       ADR % kfl_time_step_strategy = PRESCRIBED    ! Time step strategy
       ADR % kfl_stabilization      = ASGS          ! Stabilization strategy
       ADR % kfl_shock              = OFF           ! Shock capturing flag            
       ADR % kfl_time_lumped        = OFF           ! Lumped time evolution
...
!!$       end do
  ...
  subroutine ADR_initialize_projections(ADR)
    ...
    if( INOTMASTER .and. ADR % kfl_stabilization > 0 ) then
       if( .not. associated(ADR % proje1_tmp) )&
  ...
  subroutine ADR_initialize_type(ADR)
    ...
    !     ADR % kfl_time_scheme,ADR % kfl_time_order,ADR % time_parameters)
  ...
  subroutine ADR_element_assembly(&
    ...
    real(rp)                           :: gpnve,grvgr,resi2(pnode)
    ...
    if( ( kfl_stabilization == GALERKIN .or. kfl_stabilization == BUBBLE ) .and. kfl_shock == 0 ) then
       ...
       call ADR_tau(&
            pgaus,kfl_tau_strategy,tau_parameters,gpden,gpvel,gpdif,sreac,chale(1),chale(2),gptau)
    ...
    if( kfl_stabilization == GALERKIN .or. kfl_stabilization == SU .or. kfl_stabilization == A_OSS .or. kfl_stabilization == AR_OSS ) then
       ...
       do igaus = 1,pgaus
          ...
          !          
          do inode = 1,pnode
             fact1 = gpsha(inode,igaus) * bemol * gpvol(igaus)
             do jnode = 1,pnode
  ...
  subroutine ADR_critical_time_step(ADR,gpden,gpvel,gpdif,gprea,dtcri,h1,h2)
    ...
    if( present(h1) .and. present(h2) ) then 
       call ADR_tau(1_ip,ADR % kfl_tau_strategy,ADR % tau_parameters,gpden,gpvel,gpdif,gprea,h1,h2,dtcri)
    else
  ...
  subroutine ADR_tau(pgaus,kfl_tau_strategy,tau_parameters,gpden,gpvel,gpdif,react,h1,h2,gptau)
  ...
  subroutine ADR_shock_capturing(&
    ...
    integer(ip)              :: idime,inode,igaus,ii
...
!!$     return
  ...
  subroutine ADR_manufactured_error(ADR,ittim,cutim,unkno)
    ...
    real(rp),      intent(in) :: cutim
    real(rp),      intent(in) :: unkno(ADR % nunkn,*)
...
100 format(///,&
  ...
  subroutine ADR_end_time_step_1(nsize,ADR,unkno)
    ...
    integer(ip)                            :: ipoin,idime,isize,ielem
  ...
  subroutine ADR_begin_time_step_1(nsize,ADR,unkno)
    ...
    real(rp),      intent(inout), optional :: unkno(:,:,:)
    integer(ip)                            :: ipoin,idime,isize,ielem
    integer(ip)                            :: ndime_unknow,pgaus,icomp
    ...
    do isize = 1,nsize
       ...
       if( present(unkno) ) then
          ...
          !
          do ipoin = 1,ADR(isize) % nunkn
             unkno(isize,ipoin,2) = unkno(isize,ipoin,icomp)
          end do
       end if
       if( ADR(isize) % kfl_time_bubble /= 0 ) then
          !
          ...
          !
          do ielem = 1,nelem 
             ADR(isize) % bubble(ielem,2) = ADR(isize) % bubble(ielem,icomp) 
          end do
       else if( ADR(isize) % kfl_time_sgs /= 0 ) then
          !
          ! SGS
          ...
          do ielem = 1,nelem 
             pgaus = size(ADR(isize) % sgs(ielem) % a,2,KIND=ip)
             ADR(isize) % sgs(ielem) % a(1,1:pgaus,2) = ADR(isize) % sgs(ielem) % a(1,1:pgaus,icomp)
          end do
       end if
    end do
  ...
  subroutine ADR_begin_time_step_s(ADR,unkno)
    ...
    if( INOTMASTER ) then
       ...
       if( present(unkno) ) then
          ...
          !
          do ipoin = 1,ADR % nunkn
             unkno(ipoin,2) = unkno(ipoin,icomp)
          end do
       end if
       if( ADR % kfl_time_bubble /= 0 ) then
          !
          ...
          !
          do ielem = 1,nelem 
             ADR % bubble(ielem,2) = ADR % bubble(ielem,icomp) 
          end do
       else if( ADR % kfl_time_sgs /= 0 ) then
          !
          ! SGS
          ...
          do ielem = 1,nelem 
             pgaus = size(ADR % sgs(ielem) % a,2,KIND=ip)
             ADR % sgs(ielem) % a(1,1:pgaus,2) = ADR % sgs(ielem) % a(1,1:pgaus,icomp)
          end do
       end if
