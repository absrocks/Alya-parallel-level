...
contains
  ...
  subroutine iterative_solver_conjugate_gradient(ndofn,nn,ia,ja,aa,bb,xx,TOLERANCE,ITERATIONS,RELATIVE_TOLERANCE)
    ...
    call matrix_CSR_SpMV(1_ip,nn,ndofn,ndofn,ia,ja,aa,xx,rr,OPENMP=.false.,INITIALIZATION=.true.)
    ...
    do while( iiter < maxiter .and. resid > toler )
       iiter = iiter + 1
       call matrix_CSR_SpMV(1_ip,nn,ndofn,ndofn,ia,ja,aa,pp,qq,OPENMP=.false.,INITIALIZATION=.true.)
