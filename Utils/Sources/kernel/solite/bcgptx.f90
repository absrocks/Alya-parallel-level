subroutine bcgptx(npoin,nbvar,maxiter,precond,eps,an,ja,ia,bb,xx)
  ...
  use def_master, only       :  IMASTER,INOTMASTER,INOTSLAVE,ISEQUEN,kfl_paral 
  use def_domain, only       :  nzmax
  use mod_memchk                               
  use mod_csrdirx
  use mod_precon
  ...
  logical                  :: FINISH	
  integer(ip)              :: ii,ih,jj,kk,ierr,ierror,info
  integer(ip)              :: istat,nbnodes,totnode
  integer(ip)              :: igrou,ngrou,ncors
