...
subroutine gmrpls( &
  ...
  call memory_alloca(memit,'HH'     ,'gmrpls',hh     ,kryldim+((kryldim*(kryldim+1_ip))/2_ip))
  call memory_alloca(memit,'WA1'    ,'gmrpls',wa1    ,max(1_ip,ncols,nrows))
  call memory_alloca(memit,'WA3'    ,'gmrpls',wa3    ,max(1_ip,ncols,nrows))
  call memory_alloca(memit,'KRYL'   ,'gmrpls',kryl   ,max(1_ip,nrows),kryldim+1_ip)
  ...
  do while( .not. convergence )
     ...
     !print*,'KRYL 1=',kryl(204,1),wa3(204)
     do kk = 1,nrows
        kryl(kk,1) = wa3(kk) - kryl(kk,1)
     end do
     !print*,'KRYL 2=',kryl(204,1)
     !call runend('O.K.!')
