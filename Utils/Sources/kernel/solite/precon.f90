     ...
     if ( idprecon == SOL_SQUARE ) then 
        ...
        if ( idprecon == SOL_DIAGONAL ) then
           ...
           call matrix_CSR_SpMV(&
                1_ip,solve_sol(1) % direct_solver_block_LU % nn, nbvar, nbvar , &
           ...
           if( INOTMASTER ) then 
              ...
              if( solve_sol(1) % kfl_block_ras == 0 ) then
                 call matrix_CSR_SpMV(&
                      1_ip,solve_sol(1) % direct_solver_RAS(1) % nn,nbvar,nbvar , &
                      solve_sol(1) % direct_solver_RAS(1) % iA, solve_sol(1) % direct_solver_RAS(1) % jA   , &
                      solve_sol(1) % direct_solver_RAS(1) % aa, qq , w2)
              else
                 allocate( qq_1(npoin), w2_1(npoin) )
                 do ii = 1,nbvar
                    do jj = 1,npoin
                       qq_1(jj) = qq( (jj-1)*nbvar+ii )
                    end do
                    call matrix_CSR_SpMV(&
                         1_ip, solve_sol(1) % direct_solver_RAS(ii) % nn, 1_ip, 1_ip                            , &
                         solve_sol(1) % direct_solver_RAS(ii) % iA, solve_sol(1) % direct_solver_RAS(ii) % jA   , &
                         solve_sol(1) % direct_solver_RAS(ii) % aa, qq_1 , w2_1)                    
                    do jj = 1,npoin
                       w2( (jj-1)*nbvar+ii ) = w2_1(jj)
                    end do
                 end do
                 deallocate(qq_1,w2_1)
              end if
#ifdef BLAS
              if( INOTMASTER ) call DCOPY(nrows,qq,1_ip,ww,1_ip) 
              if( INOTMASTER ) call DCOPY(nrows,w2,1_ip,qq,1_ip) 
