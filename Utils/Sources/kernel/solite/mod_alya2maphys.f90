...
contains 
  ...
  subroutine alya2maphys_initialization(meshe,COMM,solve)
    ...
    if( INOTMASTER ) then
       ...
       !
       solve % mphs % comm = PAR_COMM_MY_CODE_WM4 
       solve % mphs % job  = -1
       ...
       if( PAR_MY_WORLD_RANK_WM == 0 ) then
       ...
       if( solve % kfl_symeq == 1 ) then
          call graphs_csr_to_coo(solve % nequa,ndofn,solve % ia,solve % ja,nnz,rows,cols,'SYMMETRIC')
       else
          call graphs_csr_to_coo(solve % nequa,ndofn,solve % ia,solve % ja,nnz,rows,cols)
       end if
  ...
  subroutine alya2maphys_solution(solve,amatr,rhsid,unkno)
    ...
    if( INOTMASTER ) then
       ...
       call matrix_CSR_SpMV(1_ip,solve % nequa,ndofn,ndofn,solve % ia,solve % ja,amatr,unkno,resid)
       ...
       if( INOTMASTER ) then
          ...
          if( ndofn == 1 ) then
             ...
             if( solve % kfl_symeq == 1 ) then
                ...
                call matrix_csr_to_coo(solve % nequa,solve % ndofn,solve % ia,solve % ja,amatr,solve % mphs % values,'SYMMETRIC')
             else
                !
                ! dof>1, unsymmetric matrix
                !
                ...
                nzmat        = solve % nzmat
                allocate( solve % mphs % values(nzmat) )
                call matrix_csr_to_coo(solve % nequa,ndofn,solve % ia,solve % ja,amatr,solve % mphs % values)             
             end if
