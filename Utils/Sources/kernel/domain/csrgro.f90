subroutine csrgro()
  ...
  !
  call memory_deallo(memor_dom,'SOLVE_SOL(1) % IAGRO','csrgro',solve_sol(1) % iagro)
  call memory_deallo(memor_dom,'SOLVE_SOL(1) % JAGRO','csrgro',solve_sol(1) % jagro)
  ...
  call memory_alloca(memor_dom,'IAGRO','csrgro',solve_sol(1) % iagro,ngrou+1_ip)
  ...
  if( INOTMASTER ) then
     ...
     !
     call memory_alloca(memor_dom,'SOLVE_SOL(1) % JAGRO','csrgro',solve_sol(1) % jagro,solve_sol(1) % nzgro)
     ...
     solve_sol(1) % nzgro = 0
     nzgro = 0
     call memory_alloca(memor_dom,'SOLVE_SOL(1) % JAGRO','csrgro',solve_sol(1) % jagro,1_ip)
  ...
  if( IPARALL .and. solve_sol(1) % kfl_defas == 1 ) then
     ...
     parin => recvcounts_1
     call parari('AGA',0_ip,1_ip,izdom)
     !
     ...
     ! Merge the graphs
     !
     call memory_alloca(memor_dom,'IA','csrgro',ita,ngrou)
     ...
     do igrou = 1,ngrou
        ...
        lsize = lsize - 1
        call memory_alloca(memor_dom,'ITA(IGROU) % L','csrgro',ita(igrou) % l,lsize)
        do ll = 1,lsize
     ...
     call memory_alloca(memor_dom,'SOLVE_SOL(1) % JAGRO','csrgro',solve_sol(1) % jagro,nzgro )
     ja => solve_sol(1) % jagro
     ia(1) = 1
     do igrou = 1,ngrou
...
#define PGI8
  ...
  call memory_alloca(memor_dom,'PERMR','csrgro',permr,ngrou)
  call memory_alloca(memor_dom,'INVPR','csrgro',invpr,ngrou)
  do igrou = 1,ngrou
    permr(igrou) = igrou
    invpr(igrou) = igrou
  end do
