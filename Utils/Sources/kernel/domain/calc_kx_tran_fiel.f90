subroutine calc_kx_tran_fiel()
  ...
  integer(ip)        :: kdime, size_p, size_tot
  ...
  do ifiel = 1,nfiel
   if ( kfl_field(4,ifiel) > 1 ) then !number of timesteps
      ...
      if( kfl_field(7,ifiel) == 1 ) then
         ...
         !----------------------------------------------------------
         !
         !for periodic field, the last step should be equal to the first step
         !for example if the field has period 1 second:
         ...
         kk=1  !if the next loop does not find kk, it means that somthing like the folowing happened: time_field(ifiel) % a(istep) - cortime =   1.1102230246251565E-016. And it's time to loop back to the beginning anyway
         !
         !remember that the last step is equal to the first step. 
         !
         stepsp: do istep = 2, kfl_field(4,ifiel)

            !print *,'cortime=', cortime
         ...
         !sanity check
         if( cortime>time_field(ifiel) % a(kk+1) + (1e-14) ) then
            print *,'Maxtime:',maxtime,', cutim:', cutim,', corrtime:',cortime,',ifiel:',ifiel,',kfl_field(4,ifiel):',kfl_field(4,ifiel),', kk:',kk
            call runend('calc_kx_tran_field: Timestep time (corrtime) is not between the timesteps kk and kk+1. Probaly some failure in numerical precision!')
         end if
         ...
         if(INOTMASTER) then 
            ...
            !sanity check
            if ( abs(x_tran_fiel(ifiel))>(1+1e-14) )  then
                print *,'Maxtime:',maxtime,', cutim:', cutim,', corrtime:',cortime,',ifiel:',ifiel,',kfl_field(4,ifiel):',kfl_field(4,ifiel),', kk:',kk
                call runend('calc_kx_tran_field: Sanity check failure: |x_tran_fiel(ifiel)|>1')
            end if
         end if
         ...
         if (abs(time_field(ifiel) % a(1)) > 1.0e-15) then   ! I am not sure if this check is actually needed ( ej for a conti )
            print *,'field number', ifiel,'the first time field is supposed to be zero'
            call runend('calc_kx_tran_fiel: the first time field is supposed to be zero')
         end if
         kk = 0
         ...
         if (kk==0) then
            call runend('calc_kx_tran_fiel: cannot interpolate fields, maximum time value of the fields is smaller than simulation time')
         end if
 
         if (INOTMASTER) then
            x_tran_fiel(ifiel) = &
                ( time_field(ifiel) % a(kk+1) - cutim                     ) / &
                ( time_field(ifiel) % a(kk+1) - time_field(ifiel) % a(kk) )

            !sanity check
            if ( abs(x_tran_fiel(ifiel))>(1+1e-14) )  then
                print *,'Maxtime:',maxtime,', cutim:', cutim,', ifiel:',ifiel,',kfl_field(4,ifiel):',kfl_field(4,ifiel),', kk:',kk
                call runend('calc_kx_tran_field: Sanity check failure: |x_tran_fiel(ifiel)|>1')
            end if
         end if

      end if ! Periodic/non-periodic
