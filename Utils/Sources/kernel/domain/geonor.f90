...
subroutine geonor(itask)
  ...
  integer(ip)              :: nboun_tmp,npoin_tmp,pblty,jerro,iposi
  ...
  if( kfl_geome == 1 ) then
     ...
     call parari('MAX',0_ip,1_ip,ierro)
     if( ierro /= 0 ) then
        ioutp(1) = ierro
        ioutp(2) = ipoi1
        ioutp(3) = ipoi2
     ...
     call parari('SUM',0_ip,1_ip,ninve)
     if( ninve > 0 ) then
     ...
     if( INOTMASTER ) then
        ...
        if( ndime == 2 ) then
           ...
           !$OMP           kboun,vnorm,tange,vecau,jerro,ibott)                  &
           ...
           do iboun = 1,nboun
              if( kfl_geobo(iboun) == 20 ) then 
                 do inodb = 1,nnode(ltypb(iboun))
                    ...
                    xangl(    1) = xfiel(kfl_frees) % a(    1,ipoin,1)
                    xangl(    2) = xfiel(kfl_frees) % a(    2,ipoin,1)
                    xangl(ndime) = xfiel(kfl_frees) % a(ndime,ipoin,1)
        ...
        !
        call memgeo(-19_ip) ! Deallocate boundary graph R_BOU and C_BOU
     ...
     call parari('SUM',0_ip,1_ip,jerro)
     if( jerro > 0 ) then
     ...
     if( itask == 2 ) then
        call parari('SUM',0_ip,1_ip,ierr2)
        if( ierr2 /= 0 ) then
