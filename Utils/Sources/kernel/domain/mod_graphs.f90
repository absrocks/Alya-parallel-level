...
module mod_graphs
  ...
  interface graphs_poipoi
     module procedure graphs_poipoi_1,&
          &           graphs_poipoi_2
...
contains
  ...
  subroutine graphs_poipoi_1(&
    ...
    call graphs_dealep(pelpo,lelpo)
  ...
  subroutine graphs_poipoi_go(&
    ...
    integer(ip),          intent(in)              :: lnnod(nelem)             !< Array of number of element nodes
    integer(ip),          intent(in)              :: ltype(nelem)             !< Array of element types
    !integer(ip),          intent(in), pointer            :: lnods(:,:)       !< Connectivity array
    !integer(ip),          intent(in), pointer            :: lnnod(:)             !< Array of number of element nodes
    !integer(ip),          intent(in), pointer            :: ltype(:)             !< Array of element types
    integer(ip), pointer, intent(inout)           :: ia(:)                    !< Linked list of (node-node) pointer 
    integer(ip), pointer, intent(inout)           :: ja(:)                    !< Linked list of (node-node) elements
  ...
  subroutine graphs_deallocate_ip14(xarray)
    implicit none  
  ...
  subroutine graphs_deallocate_ip18(xarray)
    implicit none  
    integer(8),  pointer, intent(inout) :: xarray(:)
    call memory_deallo(memor_dom,'XARRAY','graphs_deallocate',xarray)    
  end subroutine graphs_deallocate_ip18
  subroutine graphs_deallocate_rp1(xarray)
    implicit none  
  ...
  subroutine graphs_dealep(pelpo,lelpo)
    ...
    implicit none  
    integer(ip), pointer, intent(inout) :: pelpo(:)
    integer(ip), pointer, intent(inout) :: lelpo(:)
    integer(4)                        :: istat
  ...
  subroutine graphs_rengra(nn,nnz,permr,invpr,ia,ja)
    ...
    call memory_alloca(memor_dom,'JACPY','graphs_rendia',jacpy,nnz2)
    !
  ...
  subroutine graphs_copyij(nn,ia,ja,iacpy,jacpy,memor_opt)
    !------------------------------------------------------------------------
    ...
    integer(8),  optional, intent(inout) :: memor_opt(2)
  ...
  subroutine graphs_BuildEliminationTree(nn,iL,jL,tree)
    ...
    do ii = 1,nn
       kk = nant(ii)
       if( kk > 0 ) then
          call memory_alloca(memor_dom,'tree % antecesores(ii) % l','graphs_BuildEliminationTree', antecesores(ii) % l,kk)
  ...
  subroutine graphs_deallocatetree(tree)
    ...
    call memory_deallo(memor_dom,'tree % descendientes','graphs_deallocatetree',tree % descendiente)
...
999 continue
  ...
  subroutine graphs_element_element_graph(&
    ...
    if(      trim(whow) == 'SHARING NODES' ) then
       ...
       call graphs_eleele_faces(&
            meshe % nelem  ,nelem_2      ,meshe % mnode,meshe % lnods  ,&
            meshe % lnnod  ,meshe % ltype,pelpo_2,lelpo_2,&
    ...
    if( .not. use_r_dom_c_dom ) call graphs_dealep(pelpo_2,lelpo_2)
  ...
  subroutine graphs_eleele_faces(& 
       nelem,nelem_2,mnode,lnods,lnnod,ltype,pelpo,&
  ...
  subroutine graphs_permut_metis_postordering(nn,nnz,ia,ja,permr,invpr)
    integer(ip),          intent(in)    :: nn         !< Number of rows
    integer(ip),          intent(in)    :: nnz        !< Size of graph
  ...
  subroutine graphs_coloring(nvert,ia,ja,lcolo,ncolo,ia_color,ja_color)
  ...
  !
  !------------------------------------------------------------------------
  subroutine graphs_coloring_greedy(nvert,ia,ja,lcolo,ncolo,ia_color,ja_color)
    integer(ip), intent(in)                     :: nvert
    integer(ip), intent(in),  pointer           :: ia(:)
    integer(ip), intent(in),  pointer           :: ja(:)
    integer(ip), intent(inout), pointer           :: lcolo(:)
    ...
    call memory_alloca(memor_dom,'VERTEXORDER','graphs_coloring',vertexRank,nvert)
    ...
    call memory_deallo(memor_dom,'LSTACK','graphs_coloring',clrOrder)
    call memory_deallo(memor_dom,'LSTACK','graphs_coloring',clrCnt)
    call memory_deallo(memor_dom,'LSTACK','graphs_coloring',vertexOrder)
    call memory_deallo(memor_dom,'LSTACK','graphs_coloring',adjacentColors)
...
10  continue
  ...
  subroutine graphs_edges(meshe)
    ...
    ! Allocate memory for NEPOI and compute it
    ! 
    call memory_alloca(memor_dom,'NEPOI','graphs_list_of_edges',nepoi,npoin)
    mpop2 = 0
    do ielem = 1,nelem
    ...
    ! Allocate memory for PELPO and compute it
    !
    call memory_alloca(memor_dom,'PELPO','graphs_list_of_edges',pelpo,npoin+1_ip)
    pelpo(1) = 1
    do ipoin = 1,npoin
       pelpo(ipoin+1) = pelpo(ipoin) + nepoi(ipoin)
    end do
    ...
    !
    nlelp = pelpo(npoin+1)
    call memory_alloca(memor_dom,'LELPO','graphs_list_of_edges',lelpo,nlelp)
    do ielem = 1,nelem
       do inode = 1,lnnod(ielem)
          ipoin = lnods(inode,ielem)
          lelpo(pelpo(ipoin)) = ielem
    ...
    ! Allocate memory
    !
    call memory_alloca(memor_dom,'LEDGP','graphs_list_of_edges',ledgp,mpop2)
    call memory_alloca(memor_dom,'PEDGP','graphs_list_of_edges',pedgp,npoin+1_ip)
    !
    ! Construct the array of indexes
    !     
    pedgp(1) = 1
    ...
    !
    call memory_alloca(memor_dom,'MESHE % EDGE_TO_NODE','graphs_list_of_edges',meshe % edge_to_node,2_ip,meshe % nedge)
    iedgg = 0
    do ipoin = 1,npoin
    ...
    !
    call memory_alloca(memor_dom,'MESHE % LEDGS','graphs_list_of_edges',meshe % ledgs,meshe % medge,nelem)
    call memory_alloca(memor_dom,'MESHE % LNNED','graphs_list_of_edges',meshe % lnned,nelem)
    do ielem = 1,nelem
       pelty = ltype(ielem)
    ...
    ! LLNED(IELEM) ............. Number of edges
    !
    call memory_alloca(memor_dom,'MESHE % LEDGB','graphs_list_of_edges',meshe % ledgb,meshe % medge,nboun)
    call memory_alloca(memor_dom,'MESHE % LNNEB','graphs_list_of_edges',meshe % lnneb,nboun)
    do iboun = 1,nboun
    ...
    ! Deallocate memory
    !
    call memory_deallo(memor_dom,'LEDGP','graphs_list_of_edges',ledgp)
    call memory_deallo(memor_dom,'PEDGP','graphs_list_of_edges',pedgp)
    call memory_deallo(memor_dom,'LELPO','graphs_list_of_edges',lelpo)
    call memory_deallo(memor_dom,'PELPO','graphs_list_of_edges',pelpo)
    call memory_deallo(memor_dom,'NEPOI','graphs_list_of_edges',nepoi)
  ...
  subroutine graphs_color_graph(nn,ia,ja,nncolors,colors,memor)
    ...
    integer(ip)                             :: colNode, ii, vv, ww, node,nn0
    integer(8)                              :: memor_loc(2)
    ...
    call memory_alloca(memor_loc,'mask'         ,'graphs_color_graph',mask,maxColors+1,lboun=0_ip)
    ...
    do vv = 2,nn
       ...
       do ii = ia(node),ia(node+1)-1
          ...
          if( colors(ja(ii)) > maxColors ) write(*,*) "colors(ja(ii)) ",colors(ja(ii))
