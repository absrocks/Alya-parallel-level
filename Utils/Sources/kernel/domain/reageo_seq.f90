  ...
  if( ISEQUEN .or. ( IMASTER .and. kfl_ptask /= 2 ) ) then
     ...
     do while(words(1)/='ENDGE')
        ...
        if( words(1) == 'NODES' ) then
           ...
           call end_timer()
           if( kfl_chege == 1 ) call mescek(2_ip)
           ...
           call end_timer()
           if( kfl_chege == 1 ) call mescek(3_ip)
           ...
           if( kfl_bouel == 0 ) then
              ...
              call end_timer()
              if( kfl_chege == 1 ) call mescek(4_ip)
              if( kfl_chege == 1 ) call mescek(6_ip)
           else
              ...
              if( kfl_chege == 1 ) call mescek(4_ip)
              if( kfl_chege == 1 ) call mescek(6_ip)
              if( kfl_chege == 1 ) call mescek(5_ip)
           end if
           ...
           !
           !.md<1>PERIODIC_NODES
           !.md<2>int int                                                                     $ Master, slave
           !.md<2>...
           !.md<1>END_PERIODIC_NODES
           !.md<field>PERIODIC
           !.md<com>This field contains the list of master (first figure) and slave (second figure) periodic nodes.
           !
           call messages_live('READ PERIODICITY')
           call messages_live('WARNING: PERIODIC IS DEPRECATED, YOU SHOULD USE LMAST INSTEAD. CONVERT YOUR GEOMETRY FILE WITH THE PERIODIC2LMAST APPLICATION')
           call ecoute('reageo')
           do while( words(1) /= 'ENDPE' )
              if( nnpar > 0 ) then
                 ipoin_master       = int(param(1),ip)
                 ipoin_slave        = int(param(2),ip)
                 lmast(ipoin_slave) = ipoin_master
              end if
              call ecoute('reageo')
           end do

       else if( words(1) == 'LMAST' .and. nperi > 0 ) then
           !
           !.md<1>LMAST
           !.md<2>int int                                                                     $ Master, slave
           !.md<2>...
