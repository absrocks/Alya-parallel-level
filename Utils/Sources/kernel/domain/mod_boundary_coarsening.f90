...
!
  subroutine boundary_coarsening_graph(&
    ...
    use def_domain,         only :  cenam
    use def_domain,         only :  ltypf
    implicit none  
    ...
    type(i2p),            intent(in)           :: lface(nelty)                    !< Number of face of element types   
    ...
    do ifacg = 1,nfacg
       if( lfacg(2,ifacg) == 0 ) then
          ...
          if( pblty == TRI03 ) then
             ...
             ipoi2    = lnods(lface(ielty) % l(2,iface),ielem)
             ipoi3    = lnods(lface(ielty) % l(3,iface),ielem)
             ipoi4    = lnods(lface(ielty) % l(4,iface),ielem)
...
subroutine boundary_coarsening(&
  ...
  call setSurfaceMeshInfo(ndime,mnodb,nboun,npoin,lnodb,ltypb,coord)
...
!
!
subroutine setSurfaceMeshInfo(ndime,mnodb,nboun,npoin,lnodb,ltypb,coord)
  ...
  integer(ip)  :: maxNumEdgesNode, numElemNodes, createdEdge
  ...
  do ielem=1,nboun
    ...
    do inode = 1,numElemNodes
      ...
      createdEdge = 0
      aux_i = 1
      findCreatedEdge : do while( (aux_i.le.maxNumEdgesNode).and.(nodeEdges(aux_i,lnode).ne.0) )
...
subroutine minimizeEnergy(numBoundVorEdges,countEmptyCells,ltypb)
  ...
  !  internal variables
  integer(ip):: ielem, jelem, kelem, inode,jnode, gnode, newNode, lnode, newEdge, numElemNodes, nextNode, prevNode, aux_i, aux_j
  integer(ip):: ivor, iedge, jedge, iboundEdge, edgeId,  edgeIdChange, vor1, vor2, vorWin, vorLost, elemChanged, iter, countChanges
  integer(ip):: elem1,elem2
  logical  :: isNotFinished, hasEvolved
  real(rp)  :: sgamma1a(numCoords),sgamma2a(numCoords),srho1a,srho2a
  real(rp)  :: sgamma1b(numCoords),sgamma2b(numCoords),srho1b,srho2b
  real(rp)  :: sgammaWin(numCoords),sgammaLost(numCoords),srhoWin,srhoLost
  real(rp)  :: areaCheck,cmCheck(numCoords),F0,F1,F2,F(3)
...
subroutine generateCoarseTopology(coord,nboun_coarse,lnodb_coarse)
  ...
  integer(ip),   pointer,    intent(inout) :: lnodb_coarse(:,:)     !< Boundary connectivity
  ! internal variables
  integer(ip)      :: inode, aux_i, ivor, jnode, aux_k, countPos,minLoci
  integer(ip)      :: newElem,auxPerm(5,6)
  integer(ip)      :: selectedNode
  real(rp)         :: x1(3),x2(3),aux_r,aux_vec(3),angleMax,minDist,dist
...
recursive subroutine elementCheck(value,ielem,ivor,visitedElems, visitedList)
  ...
  integer(ip):: maxElemEdges,iedge, edgeId, aux_i
  maxElemEdges = 4
