...
!!$       end do
  ...
  subroutine renumbering_element_arrays(permr)
    ...
    call memory_copy(memor_dom,'LNODS_TMP','renelm',lnods    ,lnods_tmp,'DO_NOT_DEALLOCATE')
    ...
    if( associated(lelev) ) then
       ...
       call memory_copy(memor_dom,'LELEV_TMP','renelm',lelev,lelev_tmp,'DO_NOT_DEALLOCATE')
       do ielem_old = 1,nelem
          ielem_new        = permr(ielem_old)
          lelev(ielem_new) = lelev_tmp(ielem_old)
       end do
       call memory_deallo(memor_dom,'LELEV_TMP','renelm',lelev_tmp)
    ...
    if( neset > 0 .and. associated(leset) ) then
       ...
       call memory_copy(memor_dom,'LESET_TMP','renelm',leset,leset_tmp,'DO_NOT_DEALLOCATE')
       do ielem_old = 1,nelem
          ielem_new        = permr(ielem_old)
    ...
    do ifiel = 1,nfiel
       if( kfl_field(2,ifiel) == NELEM_TYPE ) then
          ...
          do istep = 1,nsteps
             nullify(xfiel_tmp)
             call memory_alloca(memor_dom,'XFIEL_TMP','renelm',xfiel_tmp,kfl_field(1,ifiel),nelem)
             xfiel_tmp(:,:) = xfiel(ifiel) % a(:,:,istep)
             call memory_renumber(memor_dom,'XFIEL','renelm',xfiel_tmp,permr)
             xfiel(ifiel) % a(:,:,istep) = xfiel_tmp(:,:)
             call memory_deallo(memor_dom,'XFIEL_TMP','renelm',xfiel_tmp) 
          end do
       end if
    end do
    ...
    call memory_deallo(memor_dom,'PERMR','renelm',permr)
  ...
  subroutine renumbering_node_arrays(permr,npoin_opt,nelem_opt,nboun_opt,PERMUTE_LMAST)
    ...
    type(linno_type), pointer                 :: linno_tmp(:)       
       ...
       do ifiel = 1,nfiel
          if( kfl_field(1,ifiel) > 0 ) then
             ...
             if( kfl_field(2,ifiel) == NPOIN_TYPE ) then
                ...
                call memory_alloca(memor_dom,'XFIEL','par_renumber_nodes',xfiel_tmp(ifiel)%a,kfl_field(1,ifiel),npoin_min,kfl_field(4,ifiel))
                do istep = 1,nsteps
                   do ipoin = 1,npoin_min
