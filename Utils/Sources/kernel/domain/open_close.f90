...
subroutine open_close(itask,kfl_fixno,bvess,ndprb)
  use def_parame
  use def_master, only         : INOTMASTER,gisca,veloc,npoin_type,kfl_paral,lninv_loc
  ...
  integer(ip)             :: inodb,ipoin,nnodb,kount,ninve
  integer(ip)             :: iboun
  integer(ip)             :: ibopo
...
!     call bougra()   Perhaps I may need it for bounor - Initial check says I don't
     ...
     if( nboun > 0 ) then         ! guillaume says we could save it instead of recalculating
        call bounor(nboun,lnodb,ltypb,lelbo,ninve,bouno)
     end if
     !
     ...
     do iboun = 1,nboun
        ...
        if( kount == nnodb ) then  ! all nodes have the first fixno == 7 
           do inodb = 1,nnodb
              ipoin = lnodb(inodb,iboun)
              if (itask==1)then
!                 do idime=1,ndime
