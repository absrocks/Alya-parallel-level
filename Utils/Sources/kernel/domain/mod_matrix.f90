...
module mod_matrix
  ...
  use def_master, only : INOTMASTER,NPOIN_TYPE,ISLAVE
...
contains
  ...
  subroutine matrix_assexp(ndofn_glo,ndofn_ele,pnode,npoin,lnods,elrhs,elmat,elunk,rhsid,kdofn)
    ...
    real(rp),     intent(in)           :: elunk(pnode*ndofn_ele,2)               !< Element unknwon
    real(rp),     intent(inout)        :: rhsid(*)                               !< Global RHS
    integer(ip),  intent(in), optional :: kdofn                                  !< Local dof to assemble
    integer(ip)                        :: inode,ipoin,idofl,idofg
    integer(ip)                        :: idofn,ndof2,jnode
       ...
       if( ndofn_glo == 1 ) then
          ...
          if( ndofn_ele == 1 ) then
             do inode = 1,pnode
                ...
                rhsid(idofg) = rhsid(idofg) + elrhs(inode)
                do jnode = 1,pnode
                   rhsid(idofg) = rhsid(idofg) - elmat(inode,jnode) * elunk(jnode,2)
                end do
             end do
          else
  ...
  subroutine matrix_diagonal_ELL(nn,ndof,cols_ell,an,diagonal)
    ...
    real(rp),     intent(in)          :: an(ndof,ndof,*)
    real(rp),     intent(out)         :: diagonal(*)
    integer(ip)                       :: ii,kk,ll,jj,iz,ncols
  ...
  subroutine matrix_invdia(npoin,nbvar,kfl_symme,ia,ja,an,invdiag,memit,what)
    ...
    if( INOTMASTER ) then
       ...
       !
       if( exchange ) call pararr('SLX',NPOIN_TYPE,nbvar*npoin,invdiag)
       !
       ! Inverse diagonal
       !
       do ii = 1,npoin*nbvar
...
!!$    end if
  ...
  subroutine matrix_copy_matrix(nbrows,ndof,iA,jA,Ain,Aout,iAout,jAout)
  ...
  subroutine matrix_copy_matrix_block(nbrows,ndof,iblock,iA,jA,Ain,Aout)
    ...
    integer(ip), intent(in)             :: jA(*)             !< Matrix graph
    real(rp)                            :: Ain(ndof,ndof,*)  !< Input matrix
    real(rp),    intent(out),  optional :: Aout(*)           !< Output matrix
  ...
  subroutine matrix_CSR_SpMV(n1,n2,ndofr,ndofc,ia,ja,an,xx,yy,OPENMP,INITIALIZATION,CHUNK,invpr,ia_opt,SCHEDULE)
    !
    ! Dummy arguments
    !
  ...
  subroutine matrix_CSR_parallel_SpMV(nbnodes,ndofn_rows,ndofn_cols,iA,jA,An,x,y,times,hybrid_opt,what)
    ...
    if( INOTMASTER ) then
       ...
       if( use_mpi .and. ISLAVE ) call pararr('SLX',NPOIN_TYPE,ndofn_rows*nbnodes,y)
  ...
  subroutine matrix_COO_SpMV_SPMAT(mat,ndof,x,y,invpr,noini)
    ...
    if(present(invpr)) then
       call matrix_COO_SpMV_BASE(mat % nrows,size(mat % iA,1,KIND=ip),mat % ndof,ndof,&
    ...
    if(associated(A % iA)) then
       ...
       do icont = 1,nrows
          if(nrowent(icont) > 1_ip) then
             ...
             call memory_alloca(memor,'rowent(icont) % l',vacal,rowent(icont) % l,nrowent(icont))
          endif
       enddo
       do icont = 1,nentr
  ...
  subroutine matrix_transpose(nbnodes,ndof1,ndof2,ia,ja,aa_in,aa_out,xscal_opt)
    ...
    integer(ip), intent(in)              :: ia(*)
    integer(ip), intent(in)              :: ja(*)
    real(rp),    intent(out)             :: aa_in(ndof1,ndof2,*)
...
500 continue
  ...
  subroutine matrix_sparsify(eps,nbrows,ndof,nzin,iAin,jAin,Ain,nzout,iAout,jAout,Aout,memor_opt)
    ...
    integer(8),  intent(out),            optional :: memor_opt(2)
  ...
  subroutine matrix_rotate_system(nn,ndofn,irow,iA,jA,rotma,bb,aa)
    ...
    integer(ip)                          :: ii,jj,kk,idofn,jdofn,kdofn,iz
