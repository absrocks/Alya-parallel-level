...
subroutine readim()
  ...
  use def_mpio,   only : mpio_flag_geometry, PAR_MPIO_ON

  implicit none

  integer(ip) :: ielty,ipara,inode,ifiel,ndime_aux
  ...
  if( INOTSLAVE ) then
     ...
     do while( words(1) /= 'ENDDI')
        ...
        if( words(1) == 'NODAL' ) then
           ...
           !
           !.md<1>PERIODIC_NODES = int                                                          $ Number of periodic nodes
           !.md<field>PERIODIC_NODES
           !.md<com>Number of periodic slave nodes. If a master has various slaves, all the slave nodes must be taken into account.
           !
