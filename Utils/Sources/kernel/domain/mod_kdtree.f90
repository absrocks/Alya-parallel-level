module mod_kdtree
  ...
  use def_master, only : netyp,kfl_paral
...
contains
  ...
  subroutine kdtree_nearest_boundary(coord_test,kdtree_typ,iboun,dista,proje,norma,lperm,lmask)
    implicit none
    ...
    do while( indst > 0_ip )
       ...
       if ( ldist_loc(curr) < dista ) then
          ...
          if( blink_loc(curr) < 0_ip ) then
             ...
             consider_iboun = .true.
             if( if_mask ) consider_iboun = lmask(iboun)
             ...
             if( consider_iboun ) then
                ...
                if( ndime == 3 .and. pnodb == 2 ) then
                   ...
                   dismi1 = dot_product(x01(1:ndime),x12(1:ndime))
                   dismi2 = dot_product(x12(1:ndime),x12(1:ndime))
                   t      = - dismi1 / dismi2
                   if( t < 0.0_rp ) then
                      temdi = sqrt(dot_product(x01(1:ndime),x01(1:ndime)))
                      t     = 0.0_rp 
                   else if( t > 1.0_rp ) then
                      temdi = sqrt(dot_product(x02(1:ndime),x02(1:ndime)))
                      t     = 1.0_rp 
                   else
                      call kdtree_vecpro(x01,x02,x012,ndime)
                      dismi1 = dot_product(x012(1:ndime),x012(1:ndime))
                      temdi  = sqrt(dismi1) / sqrt(dismi2)
                   end if
                   temdi          = temdi * temdi
                   propo(1:ndime) = bocod(1:ndime,1) + x12(1:ndime) * t
                else
                   !
                   ! Distance to plane in 3D and segment in 2D
                   ! TENOR= Exterior normal           
                   !
                   call extbou(itask,pnodb,lnodb(1,iboun),coord,tenor,1)
          ...
          if( ndime == 3 ) then
             ...
             if ( node2 /= 0_ip ) then
                ...
                do ilist = 1,lnele_loc(node1)%nelem
                   do jlist = 1,lnele_loc(node2)%nelem
                      ...
                      if (jboun == kboun) then
                         ...
                         call extbou(itask,qnodb,lnodb(1,jboun),coord,tenor,2_ip)
                         do idime = 1,ndime  
                ...
                do ilist = 1,lnele_loc(node1)%nelem
                   jboun = lnele_loc(node1)%eleme(ilist)
                   qnodb = nnode(lnele_loc(node1)%ltype(ilist))
                   call extbou(itask,qnodb,lnodb(1,jboun),coord,tenor,3_ip)
                   do idime = 1,ndime  
                      norma(idime) = norma(idime) + tenor(idime)
                   end  do
                   call vecuni(ndime,norma,dummi)
             ...
             if ( node1 /= 0_ip ) then
                ...
                do ilist = 1,lnele_loc(node1)%nelem
                   jboun = lnele_loc(node1)%eleme(ilist)
                   qnodb = nnode(lnele_loc(node1)%ltype(ilist))
                   call extbou(itask,qnodb,lnodb(1,jboun),coord,tenor,4)
                   do idime = 1,ndime  
                      norma(idime) = norma(idime) + tenor(idime)
                      call vecuni(ndime,norma,dummi)
                   end  do
...
subroutine kdtree_vecpro(v1,v2,v3,n)
