...
contains
  ...
  subroutine reabcs_skip()
    ...
    if( ISEQUEN .or. ( IMASTER .and. kfl_ptask /= 2 ) ) then
       if (kfl_rread == 2) then ! GrowSmarter in READ_AND_RUN mode
          ! Skip GEOMETRY section
          call ecoute('NOREAD', DO_NOT_READ_INCLUDE=.true.)
          do while( words(1) /= 'ENDGE' )
             call ecoute('NOREAD', DO_NOT_READ_INCLUDE=.true.)
          end do
          call livinf(42_ip, 'GEOMETRY SECTION SKIPPED', one)
          ...
          call livinf(42_ip, 'SET SECTION SKIPPED', one)
       end if
    end if
  end subroutine reabcs_skip
