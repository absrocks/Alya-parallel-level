subroutine cregro()
  ...
  integer(ip)          :: ndofn,ierro,nmarkt,nmark,iperi,inode
  ...
  if( lggro ) then
     ...
     call livinf(0_ip,trim(namod(modul))//': COMPUTE GROUPS FOR DEFLATION',modul)
     ...
     if( ngrou_dom == 0 ) then
        ...
        call parari('SUM',1_ip,1_ip,npoin_all)
        !
     ...
     if( INOTMASTER ) then
        ...
        call memory_alloca(memit,'LGROU','cregro',solve_sol(1) % lgrou,npoin)
     ...
     if( INOTMASTER ) then
        ...
        if(  solve_sol(1) % ifbop == 1 ) then      ! Boundary nodes
           ...
           call PAR_MIN(kpoin,'IN MY CODE WITHOUT MASTER')
  ...
  if( ngrou_dom == 0 ) then
     ...
     if( IPARALL ) then
        ...
        !call parari('SUM',1_ip,npart+1,lngro)
        lngro(1) = 0
        ngrou    = 0
        do ii = 2,npart+1
     ...
     if( ISLAVE ) then
        ...
        call Parall(801_ip) ! Uniquely define goups on interface nodes
     end if
  ...
  call PAR_SUM(nimpo)
  !call parari('SUM',0_ip,1_ip,nimpo)
  !
  ...
  if( IPARALL ) then
     ...
     !
     if( solve_sol(1) % kfl_gathe == 1 ) call Parall( 35_ip)
     !
     ! Parallel exchange
     !
     !if( solve_sol(1) % ngrou /= 0 )     call Parall(26_ip)
