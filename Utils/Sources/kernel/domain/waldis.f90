subroutine waldis(itask)
  ...
  if( INOTMASTER ) then
     ...
     if( itask == 1 ) call memgeo(24_ip) ! YWALB(NBOUN) and YWALP(NBOPO)
     ...
     call memgen(zero,2_ip,max(1_ip,nbopo))  
     ...
     boundaries: do iboun=1,nboun
        ...
        do inodb = 1,pnodb
           ipoin = lnodb(inodb,iboun)
           ibopo = lpoty(ipoin)
           gevec(1,ibopo) = gevec(1,ibopo) + bomas(inodb)  ! boundary mass matrix
           gevec(2,ibopo) = gevec(2,ibopo) + borhs(inodb)  ! boundary rhs
        end do
     ...
     ichkm = 0
     aux   = epsilon(1.0_rp)
     boundary_nodes: do ibopo = 1,nbopo
        if( gevec(1,ibopo) < aux ) then
           ichkm = 1
           chnod = intost(ibopo)
           call livinf(10000_ip, &
                ...
                // ' HAS ZERO MASS)',0_ip) 
           if(1==1) then
              auxtest: do ipoin=1,npoin
                 if (lpoty(ipoin)==ibopo) then
                    chnod = intost(ipoin)
     ...
     do ibopo = 1,nbopo
        ywalp(ibopo) = gevec(2,ibopo)/gevec(1,ibopo)
     end do

     call memgen(two,2_ip,max(1_ip,nbopo))  ! deallocates gevec
