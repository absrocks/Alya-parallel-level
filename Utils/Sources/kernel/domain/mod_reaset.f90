...
contains
  ...
  subroutine reaset_seq()
    ...
    integer(ip) :: iboun,jboun,kbset,jbset,ipara,ipoin,kpoin
    integer(ip) :: idime
    ...
    if( ISEQUEN .or. ( IMASTER .and. kfl_ptask /= 2 ) ) then
       ...
       do while( words(1) /='ENDSE')
          ...
          if( words(1) =='ELEME' ) then
             ...
             !.md<1>END_NODES
             !.md<field>NODES
             !.md<com>This field defines the node sets. It contains a list of nodes (int1).
             !
  ...
  subroutine reaset_set_connectivity()
    ...
    if( IPARALL ) then
       ...
       call par_tools_merge_lists(lesec,memor_dom)
       call par_tools_merge_lists(lbsec,memor_dom)
  ...
  subroutine reaset_boundary_set_connectivity
    ...
    if( nbset == 1 .and. INOTMASTER ) then
       nbset = 0
       do iboun = 1,nboun
  ...
  subroutine reaset_node_set_connectivity
    ...
    if( nnset >= 1 ) then
       nnset = 0
       do ipoin = 1,npoin
          if( lnset(ipoin) > 0 .and. PAR_THIS_NODE_IS_MINE(ipoin) ) then
             nnset = nnset + 1
          end if
       end do
       call memose(three)       
  ...
  subroutine reaset_node_parall()
    ...
    use def_domain,         only : memor_dom
    use def_domain,         only : neset,lesec
    ...
    if( nnset_sum > 0 ) then
       ...
       if( nnset_max-nnset_min+1 /= nnset_sum ) call runend('NODE SETS MUST BE CONSECUTIVE')
