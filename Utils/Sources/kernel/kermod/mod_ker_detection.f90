...
contains 
  ...
  subroutine ker_detection_maximum_residual(&
    ...
    !
    call PAR_MAX(max_residual,'IN MY ZONE','EXCLUDE MASTER',rank_max_owner)
  ...
  subroutine ker_detection_min_max_value(&
    ...
    !
    if( what == 'MAX' ) then
       call PAR_MAX(max_value,'IN MY ZONE','EXCLUDE MASTER',rank_max_owner)
    else if( what == 'MIN' ) then
       call PAR_MIN(max_value,'IN MY ZONE','EXCLUDE MASTER',rank_max_owner)
    else
...
2   format('# Time------>',2x,'Event ',2x,'Module',2x,'Detection type----->',2x,&
  ...
  subroutine ker_detection_boundaries()
    ...
    integer(ip) :: igaus,ibopo,jbopo,izdom
    ...
    real(rp)    :: norm_wall_shear_stress
...
!print*,min_wall_shear_stress_node
  ...
  subroutine ker_detection_submesh(&
    ...
    integer(ip)                          :: ipoin,idime,knode,ielem,kpoin
    integer(ip)                          :: inode
