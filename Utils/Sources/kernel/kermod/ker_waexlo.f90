...
subroutine ker_waexlo()
  ...
  use def_kermod,                   only : kfl_delta, dexlo_ker, velel_ker, temel_ker, lexlo_ker, wallcoupling_waexl, cooel_ker
  use def_kermod,                   only : kfl_waexl_ker,kfl_waexl_imp_ker,  shape_waexl, ielem_waexl 
  ...
  use mod_ker_proper,               only : delta_dom,ielse,relse,ndivi
  ...
  if ( kfl_waexl_ker == 0_ip ) then 
     ...
     if( INOTMASTER ) then
        ...
        allocate( velel_ker(ndime,kount) )
        allocate( cooel_ker(ndime,kount) )
        allocate( lexlo_ker(mgaub,nboun) )
        allocate( shape_waexl(mnode,kount) )
        allocate( ielem_waexl(kount))
        if (kfl_modul(ID_TEMPER) == 1)     allocate(temel_ker(1_ip,kount) )
