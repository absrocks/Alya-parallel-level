module def_kermod
  ...
  integer(ip),   parameter       ::   &
       ...
       mprop_ker                = 10, & ! Max # of properties tracked by kermod
  ...
  integer(ip)              :: &
       ...
       kfl_kemod_ker,         &      ! k eps model modification  copy to kernel
...
!        untur_fix(:),                   &      !
  ...
  integer(ip), pointer     ::    &
       ...
       ielem_waexl(:)                ! element where exchange location point belongs
  type(r1p),   pointer     ::    &
  ...
  real(rp),    pointer     ::    &
       ...
       cooel_ker(:,:)                ! Coordinates at the exchange location for wall law
