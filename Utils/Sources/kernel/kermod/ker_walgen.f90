subroutine ker_walgen(itask)
  ...
  real(rp)                :: distance_min,distance2,distance
  ...
  if( kfl_walld == 1 ) then
     ...
     if( solve_sol(1) % kfl_algso /= -999 ) then
        ...
        if( kfl_delta == 1 ) then
           ...
           ! Initialize solver
           !
           call livinf(79_ip,'EXTEND DISTANCE FROM THE WALL',modul)
           call cputim(time1)
        ...
        call livinf(79_ip,'COMPUTE DISTANCE TO THE WALL',modul)
        call cputim(time1)
...
!                         if (lninv_loc(ipoin) == 4160 .and. idime == 2) elcod(idime,inode) = elcod(idime,inode) - 0.000001_rp
  ...
  elseif (kfl_walld == 2) then
     ...
     !-------------------------------------------------------------------
     nullify(auxwi)
     call livinf(79_ip,'COMPUTE DISTANCE TO THE WALL EXHAUSTIVE SEARCH',modul)
  ...
  elseif (kfl_walld == 3) then
    ...
    call livinf(79_ip,'COMPUTE DISTANCE TO THE WALL READ FILES',modul)
    !
