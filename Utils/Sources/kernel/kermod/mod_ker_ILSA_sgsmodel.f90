...
contains 
  ...
  subroutine ker_ILSA_temporal_average(pgaus,igaui,igauf,gpvis,gpmut,gpvel,gpgve,ielem,hleng)
     ...
     ! k res, epsilon total and R^a_{ij}
        ...
        ! X1,X2 and X3 
        gpx1(igaus) = 2.0_rp*(gplest(ielem,igaus)**4_ip)*(gpstrain(ielem,igaus)**4_ip)
        gpx2(igaus) = 0.0_rp
        gpx3(igaus) = 0.0_rp
        do idime = 1,ndime
           do jdime = 1,ndime
              gpx2(igaus) = gpx2(igaus) + 4.0_rp*(gplest(ielem,igaus)**2_ip)*gpstrain(ielem,igaus)*(gpsij(idime,jdime,igaus)*gprij(idime,jdime,igaus))
