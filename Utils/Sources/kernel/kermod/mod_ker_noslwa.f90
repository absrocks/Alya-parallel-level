...
subroutine ker_noslwa()
  ...
  use def_kermod,                   only : avta1_nsw_ker, kfl_fixbo_nsw_ker,lnsw_exch
  use def_master,                   only : modul,mem_modul
  use def_domain,                   only : lnods,lmate,nelem,npoin,nboun,ltypb,nnode,lelbo,ltype,ngaus,nmate,coord,lnodb
  use def_domain,                   only : ndimb,elmar,ndime,mnodb,mnode
  use mod_memory,                   only : memory_alloca,memory_deallo
  use mod_communications,           only : PAR_INTERFACE_NODE_EXCHANGE
  implicit none

  integer(ip)              :: ielem,inode,ipoin,idime,ii
  ...
  if ( kfl_noslw_ker /= 0_ip ) then 
     ...
     if( INOTMASTER ) then     ! pensar que pasa con el master
        ...
        elem0: do ielem = 1,nelem
           ...
           if(  kfl_nswel_ker(ielem) == 1_ip ) then
              !
              ! Element properties and dimensions
              !
              kount_nsw_ele_ker = kount_nsw_ele_ker + 1_ip
              kfl_nswel_ker(ielem)  = kount_nsw_ele_ker  ! lo reuso 
              pelty = ltype(ielem) 
              pnode = nnode(pelty)
              do inode = 1,pnode
              ...
              lnsw_exch(ielem)%fact = 1.0_rp / lnsw_exch(ielem)%nbogp
           end if
     ...
     !
     if( INOTMASTER ) then
        call memory_alloca(mem_modul(1:2,modul),'NORMAL_NSW_KER','ker_noslwa',normal_nsw_ker,ndime,max(1_ip,kount_nsw_ele_ker))
        call memory_alloca(mem_modul(1:2,modul),'AVTA1_NSW_KER' ,'ker_noslwa',avta1_nsw_ker,ndime ,max(1_ip,kount_nsw_ele_ker))
        call memory_alloca(mem_modul(1:2,modul),'KOUNT_BOUN','ker_noslwa',kount_boun,max(1_ip,kount_nsw_ele_ker))

        kount_boun = 0_ip
        normal_nsw_ker = 0.0_rp
        avta1_nsw_ker = 0.0_rp
        boundaries: do iboun =1, nboun
           ...
           if(  kfl_fixbo_nsw_ker(iboun) ==  1_ip ) then      ! Wall law with no slip
              ...
              if( nmate > 1 ) then
                 pmate = lmate(ielem)  ! revisar bien que es esto de lmate
              end if
              ...
              if ( ( pmate /= -1 ) )  then 
                 ...
                 !
                 kount_boun(kfl_nswel_ker(ielem)) = kount_boun(kfl_nswel_ker(ielem)) + 1_ip
                 normal_nsw_ker(:,kfl_nswel_ker(ielem)) = normal_nsw_ker (:,kfl_nswel_ker(ielem)) + ( normal_aux / real(pgaub,rp))
        ...
        do ii =1,kount_nsw_ele_ker
           normal_nsw_ker(:,ii) = normal_nsw_ker(:,ii) / real(kount_boun(ii),rp)
        end do
