...
module mod_elsest
  ...
  use mod_kdtree2, only : kdtree2
  use mod_kdtree2, only : kdtree2_result
  use mod_kdtree2, only : kdtree2_create
  use mod_kdtree2, only : kdtree2_n_nearest_around_point
  use mod_kdtree2, only : kdtree2_destroy
  use mod_kdtree2, only : kdtree2_n_nearest
  implicit none
  private 
  ...
  type(treetype),  pointer  :: tree_struc(:) => null()
  ...
  type(octtype), parameter :: oct_struc_init=octtype(&
       ...
       null(),&                    ! comax(3)     
       null())                     ! Bounding boxes
  type(treetype),parameter :: tree_struc_init=treetype(&
       0_ip,&                      ! iallo      
       null(),&                    ! kd-tree       
       null(),&                    ! pelpo      
       null(),&                    ! pelpo      
       null())                     ! element bounding box
  ...
  integer(ip)            :: num_elem
  integer(ip)            :: num_total
  real(rp)               :: time1,time2,time_total
...
contains
  ...
  subroutine elsest_deallocate(ipara)
    ...
    if( ipara(8) == ELSEST_BIN_STRATEGY ) then                           
       ...
       if( associated(tree_struc) ) then
          call elsest_tree_deallocate(imesh) 
          deallocate(tree_struc)
       end if
  ...
  subroutine elsest_initialization()
  ...
  subroutine elsest_bin_preprocess(&
    ...
    call memory_alloca(memor8,'CPUTI','elsest_bin_preprocess',bin_struc(imesh) % cputi,10_ip)
    call memory_alloca(memor8,'KSTAT','elsest_bin_preprocess',bin_struc(imesh) % kstat,10_ip)
    call memory_alloca(memor8,'ELEBB','elsest_bin_preprocess',bin_struc(imesh) % element_bb,ndime,2_ip,nelem)
    ...
    if( dataf == ELSEST_TYPE_BIN_STRUCTURE ) then
       call memory_alloca(memor8,'TBOEL','elsest_bin_preprocess',bin_struc(imesh) % tboel,nboxe)
    else if( dataf == ELSEST_LINKED_LIST_BIN_STRUCTURE ) then
       call memory_alloca(memor8,'PBOEL','elsest_bin_preprocess',bin_struc(imesh) % pboel,nboxe+1_ip)
    end if
    ...
    if( dataf == ELSEST_TYPE_BIN_STRUCTURE ) then
       ...
       do iboxe = 1,bin_struc(imesh) % nboxe
          call memory_alloca(memor8,'TBOEL(IBOXE) % L','elsest_bin_preprocess',bin_struc(imesh) % tboel(iboxe) % l,nbono(iboxe))
          nbono(iboxe) = 0
       end do
       ...
       call memory_alloca(memor8,'PBOEL(IBOXE) % L','elsest_bin_preprocess',bin_struc(imesh)%lboel,bin_struc(imesh) % pboel(bin_struc(imesh) % nboxe+1_ip))
       lboel => bin_struc(imesh) % lboel
    ...
    if( ipara(16) == 0 ) call memory_deallo(memor8,'XMIMA','elsest_bin_preprocess',bin_struc(imesh) % element_bb)
...
!!!call elsest_binpoi(imesh)
  ...
  subroutine elsest_host_element_a_toda_costa(&
    ...
    real(rp),    intent(out)          :: dista
    integer(ip), intent(in), optional :: lchec(*)
    integer(ip)                       :: ielem,pnode,inode,ipoin,pelty,jelem
    integer(ip)                       :: inod1,inod2,iedge,idime
    ...
    if( jelem > 0 ) then
       ...
       call elmgeo_nearest_element_node(             &
            ndime,pelty,pnode,elcod,xx_intersection, &
  ...
  subroutine elsest_oct_preprocess(&
    ...
    ! Allocate memory
    !
    call memory_alloca(memor8,'CPUTI','elsest_bin_preprocess',oct_struc(imesh) % cputi,10_ip)
    call memory_alloca(memor8,'KSTAT','elsest_bin_preprocess',oct_struc(imesh) % kstat,10_ip)
    call memory_alloca(memor8,'ELEBB','elsest_bin_preprocess',oct_struc(imesh) % element_bb,ndime,2_ip,nelem)
    !
    ! Point to current mesh (IMESH) structure
    !
    ...
    call memory_alloca(memor8,'NODES','elsest_oct_preprocess',oct_struc(imesh) % tree_root % nodes,npoin)
    ...
    do while( conti )
       ...
       if( current_o % npoinbox > limit ) then
          ...
          do i = 1,divmax
             ...
             call memory_alloca(memor8,'NODES','elsest_oct_preprocess',current_o % children(i) % nodes,current_o % npoinbox)
          end do
          !
          ...
          call memory_deallo(memor8,'CURRENT % NODES','elsest_oct_preprocess',current_o % nodes)
          ...
          !
          call memory_alloca(memor8,'TREE_ROOT % ELEMS','elsest_oct_preprocess',current_o % elems,nelem)
          ...
          if( current_o % nelembox /= 0 ) then
             ...
             call memory_alloca(memor8,'CURRENT%ELEMS','elsest_oct_preprocess',current_o % elems,current_o % nelembox)
...
10     continue
    ...
    if( ipara(16) == 0 ) call memory_deallo(memor8,'XMIMA','elsest_bin_preprocess',oct_struc(imesh) % element_bb)
...
4   format(i7,50(1x,i7))
  ...
  subroutine elsest_bin_deallocate(imesh)
    ...
    if( bin_struc(imesh) % iallo /= 0 ) then
       ...
       call memory_deallo(memor8,'TBOEL','elsest_bin_deallocate',bin_struc(imesh) % tboel)
       call memory_deallo(memor8,'LBOEL','elsest_bin_deallocate',bin_struc(imesh) % lboel)
       call memory_deallo(memor8,'TBOEL','elsest_bin_deallocate',bin_struc(imesh) % pboel)
       call memory_deallo(memor8,'CPUTI','elsest_bin_deallocate',bin_struc(imesh) % cputi)
       call memory_deallo(memor8,'KSTAT','elsest_bin_deallocate',bin_struc(imesh) % kstat)
       call memory_deallo(memor8,'KSTAT','elsest_bin_deallocate',bin_struc(imesh) % element_bb)
  ...
  subroutine elsest_tree_deallocate(imesh)
    ...
    if( tree_struc(imesh) % iallo /= 0 ) then

       call kdtree2_destroy(tree_struc(imesh) % tree)  
       call memory_deallo(memor8,'TBOEL','elsest_tree_deallocate',tree_struc(imesh) % pelpo)
       call memory_deallo(memor8,'LBOEL','elsest_tree_deallocate',tree_struc(imesh) % lelpo)
       call memory_deallo(memor8,'TBOEL','elsest_tree_deallocate',tree_struc(imesh) % element_bb)
    ...
    if( oct_struc(imesh) % iallo /= 0 ) then
       ...
       do while(conti)
          ...
          if( current_o % whoiam == 1 ) then
             call memory_deallo(memor8,'CURRENT % ELEMS','elsest_oct_deallocate',current_o % elems)
          end if
       ...
       call memory_deallo(memor8,'CPUTI','elsest_oct_deallocate',oct_struc(imesh) % cputi)
       call memory_deallo(memor8,'KSTAT','elsest_oct_deallocate',oct_struc(imesh) % kstat)
       call memory_deallo(memor8,'KSTAT','elsest_oct_deallocate',oct_struc(imesh) % element_bb)
  ...
  subroutine elsest_kdtree_preprocess(&
    ...
    tree_struc(imesh) % iallo = 1
    
    tree_struc(imesh) % tree => kdtree2_create(coord,sort=.true.,rearrange=.true.)  ! this is how you create a tree.
    
    call graphs_elepoi(npoin,nelem,mnode,lnods,lnnod,mepo2,tree_struc(imesh) % pelpo,tree_struc(imesh) % lelpo)
    
    if( ipara(16) == 1 ) then
       call memory_alloca(memor8,'ELEBB','elsest_bin_preprocess',tree_struc(imesh) % element_bb,ndime,2_ip,nelem)
       call elsest_element_bounding_boxes(&
            mnode,ndime,npoin,nelem,lnnod,lnods,coord,tree_struc(imesh) % element_bb)
    end if
  ...
  subroutine elsest_kd_host_element(&
    ...
    integer(ip)                       :: curr_box_coor(3),box_nr
    integer(ip)                       :: array_size,ielem,ii,inode,ipoin,ilook,kk
    integer(ip)                       :: pnode,pelty,idime,kelem,ielpo
    real(rp)                          :: toler
    real(rp)                          :: elcod(ndime,mnode)
    integer(ip)                       :: nf
    integer(ip)                       :: nn
    ...
    !
    if( tree_struc(imesh) % iallo == 0 ) then
       call runend('ELSEST_TREE_HOST_ELEMENT: MEMORY HAS NOT BEEN ALLOCATED')
    end if
    !
    ...
    nn = 1
    allocate(results(nn)) 
    call kdtree2_n_nearest(tp=tree_struc(imesh) % tree,qv=point_x,nn=nn,results=results) 
    ipoin = results(1) % idx
    deallocate(results) 
    !
    ! Loop over elements
    ...
    toler = abs(rpara(1))    
    ifoun = 0
    do ielpo = tree_struc(imesh) % pelpo(ipoin),tree_struc(imesh) % pelpo(ipoin+1)-1
       ielem = tree_struc(imesh) % lelpo(ielpo)
       ifoun = 0
       ilook = 1
       if( ipara(14) /= 0 .and. present(lchec) ) then
          if( lchec(ielem) /= ipara(14) ) ilook = 0
       ...
       if( ilook == 1 ) then
          ...
          if( ipara(16) == 1 ) then
             if( elmgeo_inside_element_bounding_box(ndime,tree_struc(imesh) % element_bb(:,1,ielem),tree_struc(imesh) % element_bb(:,2,ielem),point_x,pelty) ) then
                call elmgeo_natural_coordinates(&
                     ndime,pelty,pnode,elcod,shapt,derit,point_x,&
                     coloc,ifoun,toler,BOUNDING_BOX=.false.)             
             end if
          else
             call elmgeo_natural_coordinates(&
                  ndime,pelty,pnode,elcod,shapt,derit,point_x,&
