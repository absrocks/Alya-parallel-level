subroutine par_sleskc()
...
#endif
  integer(ip)              :: ipoin,ii,jj,bsize,ini,dom_i,ibopo,kk,mm
  ...
  if( ISLAVE ) then
     ...
     call memchk(zero,istat,mem_servi(1:2,servi),'LOC_SPARII','par_slexch',loc_sparii)

     allocate(loc_rparii(commd%bound_dim),stat=istat)
     call memchk(zero,istat,mem_servi(1:2,servi),'LOC_RPARII','par_slexch',loc_rparii)
     ...
     call memchk(zero,istat,mem_servi(1:2,servi),'LOC_SPARR1','par_sleskc',loc_sparr1)

     allocate(loc_rparr1(pard1*commd%bound_dim),stat=istat)
     call memchk(zero,istat,mem_servi(1:2,servi),'LOC_RPARR1','par_sleskc',loc_rparr1)
     ...
     call memchk(two,istat,mem_servi(1:2,servi),'LOC_RPARR1','par_sleskc',loc_rparr1)
     deallocate(loc_rparr1,stat=istat)
     if(istat/=0) call memerr(two,'LOC_RPARR1','par_sleskc',0_ip)

     call memchk(two,istat,mem_servi(1:2,servi),'LOC_SPARR1','par_sleskc',loc_sparr1)
     ...
     call memchk(two,istat,mem_servi(1:2,servi),'LOC_SPARII','par_slexch',loc_sparii)
     deallocate(loc_sparii,stat=istat)
     if(istat/=0) call memerr(two,'LOC_SPARII','par_slexch',0_ip)

     call memchk(two,istat,mem_servi(1:2,servi),'LOC_RPARII','par_slexch',loc_rparii)
