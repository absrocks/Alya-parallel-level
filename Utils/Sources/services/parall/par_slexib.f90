subroutine par_slexib(itask)
  ...
  integer(ip)              :: ipoin,ii,jj,bsize,ini,dom_i,kk,ll
  ...
  if( ISLAVE ) then
     ...
     if( itask == 1 ) then
        ...
        call memchk(zero,istat,mem_servi(1:2,servi),'LOC_SPARR1','par_slexib',loc_sparr1)

        allocate(loc_rparr1(commd%bound_dim),stat=istat)
        call memchk(zero,istat,mem_servi(1:2,servi),'LOC_RPARR1','par_slexib',loc_rparr1)
        !
        ! Gather
        !
              ...
              if( pari1(ipoin) /= 0 ) then
                 ...
                 if( loc_rparr1(jj) < parr1(ipoin) ) then
                    ...
                    if( pari1(ipoin) > 0 ) then
                       ...
                       pari1(ipoin) = min(-dom_i,pari1(ipoin))
                    end if
        ...
        call memchk(two,istat,mem_servi(1:2,servi),'LOC_RPARR1','par_slexib',loc_rparr1)
        deallocate(loc_rparr1,stat=istat)
        if(istat/=0) call memerr(two,'LOC_RPARR1','par_slexib',0_ip)

        call memchk(two,istat,mem_servi(1:2,servi),'LOC_SPARR1','par_slexib',loc_sparr1)
        ...
        call memchk(zero,istat,mem_servi(1:2,servi),'LOC_SPARI1','par_slexib',loc_spari1)

        allocate(loc_rpari1(commd%bound_dim),stat=istat)
        call memchk(zero,istat,mem_servi(1:2,servi),'LOC_RPARR1','par_slexib',loc_rpari1)
        !
        ! Gather
        !
        do jj= 1, commd%bound_dim
        ...
        call memchk(two,istat,mem_servi(1:2,servi),'LOC_RPARI1','par_slexib',loc_rpari1)
        deallocate(loc_rpari1,stat=istat)
        if(istat/=0) call memerr(two,'LOC_RPARI1','par_slexib',0_ip)

        call memchk(two,istat,mem_servi(1:2,servi),'LOC_SPARI1','par_slexib',loc_spari1)
