...
!!            L_COMM_PAR( 3, 2 ) = 4  <= Stage 4, only 2 and 4 communicate
...
subroutine par_commun(nbcolours,nbdual)
 use def_parame
  ...
  !allocate( communSort(nbcolours,npart_par), stat= ierr )
  !if (ierr /= 0) then
  !  call error( ierr )
  !endif
  allocate(lcomm_par(nbcolours,npart_par),stat=istat)
  call memchk(zero,istat,mem_servi(1:2,servi),'lcomm_par','par_commun',lcomm_par)  

  allocate(invTransl(nbdual),stat=istat)
  call par_memchk(zero,istat,mem_servi(1:2,servi),'INVTRANSL','par_commun',invTransl)
  !
  ...
  call par_memchk(two,istat,mem_servi(1:2,servi),'INVTRANSL','par_commun',invTransl)
  deallocate(invTransl,stat=istat)
  if(istat/=0) call memerr(two,'invTransl','par_commun',0_ip)
