subroutine par_schurr()
  ...
  use mod_parall, only : PAR_COMM_MY_CODE4,commd
  implicit none 
#ifdef MPI_OFF
#else
  include  'mpif.h'
  ...
  integer(ip)             :: dom_i,ifoun,spoin,kzdom,pzdom,jzdom
  integer(ip)             :: nsolv,isolv,imodu,dummi
  integer(4)              :: istat,bsizs4,bsizr4
  ...
  if( ( kfl_schur == 2 .or. kfl_schur == 3 ) .and. INOTMASTER ) then
     ...
     call memchk(zero,istat,mem_servi(1:2,servi),'R_DOM_PREC','par_schurr',r_dom_prec)     
     ...
     call memchk(zero,istat,mem_servi(1:2,servi),'C_DOM_PREC','par_schurr',c_dom_prec)     
     ...
     if( dummi == 1 ) then
        ...
        ! 
        call graphs_permut(&
             npoin_bb,nzdom_prec,r_dom_prec,c_dom_prec,permr_prec,invpr_prec)  
        !
        ...
        !
        call graphs_iniper(&
             npoin_bb,permr_tmp,invpr_tmp)
        call graphs_postorder(&
             npoin_bb,r_dom_prec,c_dom_prec,permr_tmp,invpr_tmp)        
        call graphs_rengra(&
             npoin_bb,nzdom_prec,permr_tmp,invpr_tmp,r_dom_prec,c_dom_prec)
        !
        ! 1. Compose permutation PERMR_PREC, INVPR_PREC and PERMR_TMP, INVPR_TMP
        ! 2. Deallocate memory
        !
        call graphs_comper(&
             npoin_bb,permr_prec,invpr_prec,permr_tmp,invpr_tmp)
        call graphs_deaper(&
             permr_tmp,invpr_tmp)
     else
        !
        ...
        ! 2. Order graph
        !
        call graphs_iniper(npoin_bb,permr_prec,invpr_prec)
        do kpoin = 1,npoin_bb
