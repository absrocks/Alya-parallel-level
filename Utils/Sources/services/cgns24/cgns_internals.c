...
static void *cgi_get_user_data_posit(cgns_user_data *user_data, 
				     int nuser_data, int* index, 
				     char **label, int n, int m)
{
    cgns_user_data *data;

    if (nuser_data == 0 || user_data == 0)
        return 0;
    if (index[m] < 1 || index[m] > nuser_data) {
        cgi_error("UserDefinedData_t node number %d invalid",index[m]);
        return 0;
    }
    data = &user_data[index[m]-1];
    if (++m >= n)
        return (void *)data;
    if (strcmp(label[m], "UserDefinedData_t") == 0)
        return cgi_get_user_data_posit(data->user_data,
            data->nuser_data, index, label, n, m);
    if ((m + 1) == n && strcmp(label[m], "DataArray_t") == 0)
        return cgi_get_darray_posit(data->array, data->narrays,
            index, n, m);
...
/* begin KMW */
static void *cgi_get_fambc_posit(cgns_fambc *fambc, int nfambc, 
				   int *index, int n, int m)
{
    if (fambc == 0)
        return 0;
    else if (! (index[m] <= nfambc && index[m] > 0))
        cgi_error("FamilyBC_t node number %d invalid",index[m]);
    else if (index[m] <= nfambc)
        return (void *)&fambc[index[m]-1];

    return 0;
}

static void *cgi_get_dataset_posit(cgns_dataset *dataset, int ndataset, 
				   int *index, int n, int m)
{
    if (dataset == 0)
        return 0;
    else if (! (index[m] <= ndataset && index[m] > 0))
        cgi_error("BCDataSet_t node number %d invalid",index[m]);
    else if (index[m] <= ndataset)
        return (void *)&dataset[index[m]-1];

    return 0;
}

static void *cgi_get_bcdata_posit(cgns_dataset *dataset, BCDataType_t type)
{
    if (dataset==0) return 0;

    if (type==Dirichlet) {
        if (dataset->dirichlet==0) {
            cgi_error("BCData_t type Dirichlet doesn't exist under this BCDataSet");
            return 0;
        } else return (void *)dataset->dirichlet;
    } else if (type==Neumann) {
        if (dataset->neumann==0) {
            cgi_error("BCData_t type Neumann doesn't exist under this BCDataSet");
            return 0;
        } else return (void *)dataset->neumann;
    } else {
        cgi_error("BCData must be of type Dirichlet or Neumann");
        return 0;
    }
    ...
    /* Zone_t */
        ...
        /* GridCoordinates_t */
            ...
            /* DataArray_t */
                ...
                (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(zcoor->coord, zcoor->ncoords,
                    index, n, 2);

            /* UserDefinedData_t */

            else if (zcoor && (n > 2) &&
                (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(zcoor->user_data,
                    zcoor->nuser_data, index, label, n, 2);

        /* ZoneIterativeData_t */

        } else if ((n > 1) &&
            (strcmp(label[1], "ZoneIterativeData_t") == 0)) {
            ...
            /* DataArray_t */
                ...
                (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(ziter->array, ziter->narrays,
                    index, n, 2);
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(ziter->user_data,
                    ziter->nuser_data, index, label, n, 2);
        ...
        /* Elements_t */
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(section->user_data,
                    section->nuser_data, index, label, n, 2);
        ...
        /* FlowSolution_t */
            ...
            /* DataArray_t */
                ...
                posit = cgi_get_darray_posit(sol->field, sol->nfields,
                    index, n, 2);
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(sol->user_data,
                    sol->nuser_data, index, label, n, 2);
        ...
        /* RigidGridMotion_t */
            ...
            /* DataArray_t */
                ...
                (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(rmotion->array, rmotion->narrays,
                    index, n, 2);
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(rmotion->user_data,
                    rmotion->nuser_data, index, label, n, 2);

        /* ArbitraryGridMotion_t */

        } else if ((n > 1) &&
            (strcmp(label[1], "ArbitraryGridMotion_t") == 0)) {
            ...
            /* DataArray_t */
                ...
                (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(amotion->array, amotion->narrays,
                    index, n, 2);
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(amotion->user_data,
                    amotion->nuser_data, index, label, n, 2);

        /* ZoneGridConnectivity_t */

        } else if ((n > 1) &&
            (strcmp(label[1], "ZoneGridConnectivity_t") == 0)) {
            ...
            /* OversetHoles_t */
                ...
                (strcmp(label[2], "OversetHoles_t") == 0)) {
                ...
                /* UserDefinedData_t */
                    ...
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(hole->user_data,
                        hole->nuser_data, index, label, n, 3);

            /* GridConnectivity_t */

            } else if (zconn && (n > 2) &&
                (strcmp(label[2], "GridConnectivity_t") == 0)) {
                ...
                /* UserDefinedData_t */
                    ...
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(conn->user_data,
                        conn->nuser_data, index, label, n, 3);

                /* GridConnectivityProperty_t */

                else if (conn && (n > 3) &&
                    (strcmp(label[3], "GridConnectivityProperty_t") == 0)) {
                    ...
                    /* Periodic_t */
                        ...
                        /* UserDefinedData_t */
                            ...
                            (strcmp(label[5], "UserDefinedData_t") == 0))
                            posit = cgi_get_user_data_posit(cperio->user_data,
                                cperio->nuser_data, index, label, n, 5);

                        /* DataArray_t */

                        else if (cperio && (n > 5) &&
                            (strcmp(label[5], "DataArray_t") == 0))
                            posit = cgi_get_darray_posit(cperio->array,
                                cperio->narrays, index, n, 5);

                    /* AverageInterface_t */

                    } else if (cprop && (n > 4) &&
                        (strcmp(label[4], "AverageInterface_t") == 0)) {
                        cgns_caverage *caverage = cprop -> caverage;
                        if (n == 5)
                            posit = (void *) caverage;
                        ...
                        else if (caverage && (n > 5) &&
                            (strcmp(label[5], "UserDefinedData_t") == 0))
                            posit = cgi_get_user_data_posit(caverage->user_data,
                                caverage->nuser_data, index, label, n, 5);

                    /* UserDefinedData_t */

                    } else if (cprop && (n > 4) &&
                        (strcmp(label[4], "UserDefinedData_t") == 0))
                        posit = cgi_get_user_data_posit(cprop->user_data,
                            cprop->nuser_data, index, label, n, 4);
                }

            /* GridConnectivity1to1_t */

            } else if (zconn && (n > 2) &&
                (strcmp(label[2], "GridConnectivity1to1_t") == 0)) {
                ...
                else if (conn1to1 && (n > 3) &&
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(conn1to1->user_data,
                        conn1to1->nuser_data, index, label, n, 3);
...
/* begin KMW */
                /* GridConnectivityProperty_t */
                    ...
                    (strcmp(label[3], "GridConnectivityProperty_t") == 0)) {
                    cgns_cprop *cprop = conn1to1->cprop;
                    if (n == 4)
                        posit = (void *) cprop;
...
/* end KMW */
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(zconn->user_data,
                    zconn->nuser_data, index, label, n, 2);
        ...
        /* ZoneBC_t */
            ...
            /* BC_t */
                ...
                /* BCDataSet_t */
                    ...
                    (strcmp(label[3], "BCDataSet_t") == 0)) {
                    cgns_dataset *dataset = cgi_get_dataset(cg, B, Z,
                        index[2], index[3]);
                    if (n == 4)
                        posit = (void *)dataset;
                    ...
                    /* BCData_t */
                        ...
                        (strcmp(label[4], "BCData_t") == 0)) {
                        cgns_bcdata *bcdata = cgi_get_bcdata(cg, B, Z,
                            index[2], index[3], (BCDataType_t)index[4]);
                        if (n == 5)
                            posit = (void *)bcdata;
                        ...
                        /* DataArray_t */
                            ...
                            (strcmp(label[5], "DataArray_t") == 0) )
                            posit = cgi_get_darray_posit(bcdata->array,
                                bcdata->narrays, index, n, 5);

                        /* UserDefinedData_t */

                        else if (bcdata && (n > 5) &&
                            (strcmp(label[5], "UserDefinedData_t") == 0))
                            posit = cgi_get_user_data_posit(bcdata->user_data,
                                bcdata->nuser_data, index, label, n, 5);

                    /* ReferenceState_t */

                    } else if (dataset && (n > 4) &&
                        (strcmp(label[4], "ReferenceState_t") == 0)) {
                        cgns_state *state = cgi_get_state(cg, B, Z, index[1],
                            index[2], index[3]);
                        if (n == 4)
                            posit = (void *)state;
                        ...
                        /* UserDefinedData_t */
                            ...
                            (strcmp(label[4], "UserDefinedData_t") == 0))
                            posit = cgi_get_user_data_posit(state->user_data,
                                state->nuser_data, index, label, n, 4);

                    /* UserDefinedData_t */

                    } else if (dataset && (n > 4) &&
                        (strcmp(label[4], "UserDefinedData_t") == 0))
                        posit = cgi_get_user_data_posit(dataset->user_data,
                            dataset->nuser_data, index, label, n, 4);

                /* ReferenceState_t */

                } else if (boco && (n > 3) &&
                    (strcmp(label[3], "ReferenceState_t") == 0)) {
                    cgns_state *state = cgi_get_state(cg, B, Z, index[1],
                        index[2], 0);
                    if (n == 4)
                        posit = (void *)state;
                    ...
                    /* DataArray_t */
                        ...
                        (strcmp(label[4], "DataArray_t") == 0))
                        posit = cgi_get_darray_posit(state->array,
                            state->narrays, index, n, 4);
                ...
                /* BCProperty_t */
                    ...
                    /* WallFunction_t */
                        ...
                        (strcmp(label[4], "WallFunction_t") == 0)) {
                        cgns_bcwall *bcwall = bprop -> bcwall;
                        if (n == 5)
                            posit = (void *) bcwall;
                        ...
                        /* UserDefinedData_t */
                            ...
                            (strcmp(label[5], "UserDefinedData_t") == 0))
                            posit = cgi_get_user_data_posit(bcwall->user_data,
                                bcwall->nuser_data, index, label, n, 5);

                    /* Area_t */

                    } else if (bprop && (n > 4) &&
                        (strcmp(label[4], "Area_t") == 0)) {
                        cgns_bcarea *bcarea = bprop -> bcarea;
                        if (n == 5)
                            posit = (void *) bcarea;
                        ...
                        /* UserDefinedData_t */
                            ...
                            (strcmp(label[5], "UserDefinedData_t") == 0))
                            posit = cgi_get_user_data_posit(bcarea->user_data,
                                bcarea->nuser_data, index, label, n, 5);

                        /* DataArray_t */

                        else if (bcarea && (n > 5) &&
                            (strcmp(label[5], "DataArray_t") == 0))
                            posit = cgi_get_darray_posit(bcarea->array,
                                bcarea->narrays, index, n, 5);

                    /* UserDefinedData_t */

                    } else if (bprop && (n > 4) &&
                        (strcmp(label[4], "UserDefinedData_t") == 0))
                        posit = cgi_get_user_data_posit(bprop->user_data,
                            bprop->nuser_data, index, label, n, 4);

                /* UserDefinedData_t */

                } else if (boco && (n > 3) &&
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(boco->user_data,
                        boco->nuser_data, index, label, n, 3);

            /* ReferenceState_t */

            } else if (zboco && (n > 2) &&
                (strcmp(label[2], "ReferenceState_t") == 0)) {
                ...
                /* DataArray_t */
                    ...
                    (strcmp(label[3], "DataArray_t") == 0))
                    posit = cgi_get_darray_posit(state->array, state->narrays,
                        index, n, 3);
                ...
                /* UserDefinedData_t */
                    ...
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(state->user_data,
                        state->nuser_data, index, label, n, 3);

            /* UserDefinedData_t */

            } else if (zboco && (n > 2) &&
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(zboco->user_data,
                    zboco->nuser_data, index, label, n, 2);
        ...
        /* DiscreteData_t */
            ...
            else if (discrete && (n > 2) &&
                (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(discrete->array,
                    discrete->narrays, index, n, 2);

            /* UserDefinedData_t */

            else if (discrete && (n > 2) &&
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(discrete->user_data,
                    discrete->nuser_data, index, label, n, 2);
        ...
        /* FlowEquationSet_t */
            ...
            /* GoverningEquations_t */
                ...
                (strcmp(label[2], "GoverningEquations_t") == 0)) {
                ...
                else if (governing && (n > 3) &&
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(governing->user_data,
                        governing->nuser_data, index, label, n, 3);
...
/* begin KMW */
		|| strcmp(label[2], "EMElectricFieldModel_t") == 0
		|| strcmp(label[2], "EMMagneticFieldModel_t") == 0
		|| strcmp(label[2], "EMConductivityModel_t") == 0)) {
/* end KMW */
                ...
                /* DataArray_t */
                    ...
                    (strcmp(label[3], "DataArray_t") == 0))
                    posit = cgi_get_darray_posit(model->array, model->narrays,
                        index, n, 3);
                ...
                /* UserDefinedData_t */
                    ...
                    (strcmp(label[3], "UserDefinedData_t") == 0))
                    posit = cgi_get_user_data_posit(model->user_data,
                        model->nuser_data, index, label, n, 3);
            ...
            /* UserDefinedData_t */
                ...
                cgns_equations *equations = cgi_get_equations(cg, B, Z);
                if (equations)
                    posit = cgi_get_user_data_posit(equations->user_data,
                        equations->nuser_data, index, label, n, 2);
            }
        ...
        /* ConvergenceHistory_t */
            ...
            /* DataArray_t */
                ...
                (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(converg->array, converg->narrays,
                    index, n, 2);
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(converg->user_data,
                    converg->nuser_data, index, label, n, 2);
        ...
        /* IntegralData_t */
            ...
            else if (integral && (n > 2) &&
                (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(integral->array,
                    integral->narrays, index, n, 2);

            /* UserDefinedData_t */

            else if (integral && (n > 2) &&
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(integral->user_data,
                    integral->nuser_data, index, label, n, 2);
        ...
        /* ReferenceState_t */
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(state->user_data,
                    state->nuser_data, index, label, n, 2);
        ...
        /* UserDefinedData_t */
            ...
            cgns_zone *zone = cgi_get_zone(cg, B, Z);
            if (zone)
                posit = cgi_get_user_data_posit(zone->user_data,
                    zone->nuser_data, index, label, n, 1);

        /* RotatingCoordinates_t */

        } else if ((n > 1) &&
            (strcmp(label[1], "RotatingCoordinates_t") == 0)) {
            ...
            else if (rotating && (n > 2) &&
                (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(rotating->array,
                    rotating->narrays, index, n, 2);

            /* UserDefinedData_t */

            else if (rotating && (n > 2) &&
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(rotating->user_data,
                    rotating->nuser_data, index, label, n, 2);
        }
    ...
    /* ReferenceState_t */
        ...
        /* UserDefinedData_t */
            ...
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(state->user_data,
                state->nuser_data, index, label, n, 1);
    ...
    /* Family_t */
        ...
        /* GeometryReference_t */
            ...
            (strcmp(label[1], "GeometryReference_t") == 0)) {
            cgns_geo *geo = 0;
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(geo->user_data,
                    geo->nuser_data, index, label, n, 2);

        /* UserDefinedData_t */

        } else if (family && (n > 1) &&
            (strcmp(label[1], "UserDefinedData_t") == 0)) {
            posit = cgi_get_user_data_posit(family->user_data,
                family->nuser_data, index, label, n, 1);

/* begin KMW */
        /* FamilyBC_t */

	} else if (family && (n > 1) &&
            (strcmp(label[1], "FamilyBC_t") == 0)) {
	    cgns_fambc *fambc = (cgns_fambc *)cgi_get_fambc_posit(family->fambc,
                family->nfambc, index, n, 1);
	    if (n == 2)
                posit = (void *)fambc;
            ...
            /* BCDataSet_t */
                ...
                (strcmp(label[2], "BCDataSet_t") == 0)) {
		cgns_dataset *dataset = (cgns_dataset *)cgi_get_dataset_posit(
                    fambc->dataset, fambc->ndataset, index, n, 2);
		if (n == 3)
		    posit = (void *)dataset;
                ...
                /* BCData_t */
                    ...
                    (strcmp(label[3], "BCData_t") == 0)) {
		    cgns_bcdata *bcdata = (cgns_bcdata *)cgi_get_bcdata_posit(
                        dataset, (BCDataType_t)index[3]);
		    if (n == 4)
			posit = (void *)bcdata;
			...
			posit = cgi_get_darray_posit(bcdata->array,
                    ...
                    /* UserDefinedData_t */
                        ...
                        (strcmp(label[4], "UserDefinedData_t") == 0))
		        posit = cgi_get_user_data_posit(bcdata->user_data,
				      bcdata->nuser_data, index, label, n, 4);

                /* ReferenceState_t */

		} else if (dataset && (n > 3) &&
                    (strcmp(label[3], "ReferenceState_t") == 0)) {
		    cgns_state *state = dataset->state;
		    if (n == 4)
			posit = (void *)state;
			...
			posit = cgi_get_darray_posit(state->array,
                ...
                /* UserDefinedData_t */
                    ...
                    (strcmp(label[3], "UserDefinedData_t") == 0))
		    posit = cgi_get_user_data_posit(dataset->user_data,
				      dataset->nuser_data, index, label, n, 3);
	    }
        ...
        /* RotatingCoordinates_t */
            ...
            (strcmp(label[1], "RotatingCoordinates_t") == 0)) {
	    cgns_rotating *rotating = family->rotating;
	    if (n == 2)
		posit = (void *)rotating;
...
/* end KMW */
    ...
    /* BaseIterativeData_t */
        ...
        /* DataArray_t */
            ...
            posit = cgi_get_darray_posit(biter->array, biter->narrays,
                index, n, 1);
        ...
        /* UserDefinedData_t */
            ...
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(biter->user_data,
                biter->nuser_data, index, label, n, 1);
    ...
    /* ConvergenceHistory_t */
        ...
        /* UserDefinedData_t */
            ...
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(converg->user_data,
                converg->nuser_data, index, label, n, 1);
    ...
    /* FlowEquationSet_t */
        ...
        /* GoverningEquations_t */
            ...
            (strcmp(label[1], "GoverningEquations_t") == 0)) {
            ...
            else if (governing && (n > 2) &&
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(governing->user_data,
                    governing->nuser_data, index, label, n, 2);
...
/* begin KMW */
	    || strcmp(label[1], "EMElectricFieldModel_t") == 0
	    || strcmp(label[1], "EMMagneticFieldModel_t") == 0
	    || strcmp(label[1], "EMConductivityModel_t") == 0)) {
/* end KMW */
            ...
            /* UserDefinedData_t */
                ...
                (strcmp(label[2], "UserDefinedData_t") == 0))
                posit = cgi_get_user_data_posit(model->user_data,
                    model->nuser_data, index, label, n, 2);
        ...
        /* UserDefinedData_t */
            ...
            if (equations)
                posit = cgi_get_user_data_posit(equations->user_data,
                    equations->nuser_data, index, label, n, 1);
        }
    ...
    /* IntegralData_t */
        ...
        /* DataArray_t */
            ...
            posit = cgi_get_darray_posit(integral->array, integral->narrays,
                index, n, 1);

        /* UserDefinedData_t */

        else if (integral && (n > 1) &&
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(integral->user_data,
                integral->nuser_data, index, label, n, 1);
    ...
    /* UserDefinedData_t */
        ...
        cgns_base *base = cgi_get_base(cg, B);
        if (base)
            posit = cgi_get_user_data_posit(base->user_data,
                base->nuser_data, index, label, n, 0);
    ...
    /* Gravity_t */
        ...
        /* DataArray_t */
            ...
            posit = cgi_get_darray_posit(gravity->vector, gravity->narrays,
                index, n, 1);
        ...
        /* UserDefinedData_t */
            ...
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(gravity->user_data,
                gravity->nuser_data, index, label, n, 1);
    ...
    /* Axisymmetry_t */
        ...
        /* DataArray_t */
            ...
            posit = cgi_get_darray_posit(axisym->array, axisym->narrays,
                index, n, 1);
        ...
        /* UserDefinedData_t */
            ...
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(axisym->user_data,
                axisym->nuser_data, index, label, n, 1);
    ...
    /* RotatingCoordinates_t */
        ...
        /* DataArray_t */
            ...
            posit = cgi_get_darray_posit(rotating->array, rotating->narrays,
                index, n, 1);

        /* UserDefinedData_t */

        else if (rotating && (n > 1) &&
            (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(rotating->user_data,
                rotating->nuser_data, index, label, n, 1);
...
/* begin KMW */
	   strcmp(posit_label, "EMElectricFieldModel_t") == 0 ||
	   strcmp(posit_label, "EMMagneticFieldModel_t") == 0 ||
	   strcmp(posit_label, "EMConductivityModel_t") == 0) {
/* end KMW */
...
/* begin KMW */
    } else if (strcmp(posit_label,"FamilyBC_t")==0) {
          cgns_fambc *parent = (cgns_fambc *)posit;
          (*posit_id) = parent->id;
/* end KMW */
    } else {
    ...
    char const *given_name, int *ier)
{
     ...
     *		BC_t
     */
    if (strcmp(posit_label,"FamilyBC_t")==0)
        ADDRESS4MULTIPLE(cgns_fambc, ndataset, dataset, cgns_dataset)
    else {
        cgi_error("BCDataSet_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1) {
 ...
 *  GridConnectivity1to1_t
 */
    if (strcmp(posit_label,"UserDefinedData_t")==0)
	...
	ADDRESS4SINGLE_ALLOC(cgns_conn, ptset)
        ...
        cgi_error("PointSet node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }

    if (error1==1) {
        cgi_error("IndexArray/Range_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!ptset && local_mode == MODE_READ) {
        cgi_error("IndexArray/Range_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
...
void cgi_free_dataset(cgns_dataset *dataset) {
    ...
    /* begin KMW */
    if (dataset->ptset) cgi_free_ptset(dataset->ptset);
    /* end KMW */
}
...
void cgi_free_user_data(cgns_user_data *user_data) {
    ...
    /* begin KMW */
    if (user_data->ptset) cgi_free_ptset(user_data->ptset);
