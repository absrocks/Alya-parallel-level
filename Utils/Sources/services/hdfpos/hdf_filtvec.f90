subroutine hdf_filtvec
  ...
  !
  !  Create open a group and open a dataset in this group
  !
  ...
  dims(1) = ndime
  dims(2) = npoin_filt
  !write(*,*)'npoin_filt',npoin_filt

  !
  ! new array to put the filtered data (gevec_hdf>gevec_filter)
  !
  allocate(gevec_filter(ndime,npoin_filt))
  i=1_ip
  if (ndime==3) then
     do ipoin=1,npoin
        if(gefil(ipoin)/=0) then
           gevec_filter(1,i) = gevec_hdf(1,ipoin)
           gevec_filter(2,i) = gevec_hdf(2,ipoin)
           gevec_filter(3,i) = gevec_hdf(3,ipoin)
           i=i+1
        end if
     end do
  end if
...
#ifdef HDF5_DEBUG
  write(*,*) kfl_paral, 'hdf_filter: offset(hdf5_rank+1)', offset(1:2,hdf5_rank+1)
  ...
  !
#ifdef R4
  call h5dwrite_f( dset_id, H5T_NATIVE_REAL, gevec_filter, dims, ierr, &
       file_space_id = filespace, mem_space_id = memspace, xfer_prp = hdf5_dxplist )
#else
  call h5dwrite_f( dset_id, H5T_NATIVE_DOUBLE, gevec_filter, dims, ierr, &
       file_space_id = filespace, mem_space_id = memspace, xfer_prp = hdf5_dxplist )
#endif
#ifdef HDF5_DEBUG
  do i=1, npoin_filt
     !write(*,*) kfl_paral, 'hdf_filter gevec_hdf:', gevec_hdf(1:SIZE(gevec_hdf, DIM=1), i)
     write(*,*) kfl_paral, 'hdf_filter gevec_filter:', gevec_filter(1,i),gevec_filter(2,i),gevec_filter(3,i)
  enddo
#endif
  !
  ! Close the group
