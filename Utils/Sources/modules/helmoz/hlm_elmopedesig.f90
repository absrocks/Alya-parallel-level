subroutine hlm_elmopedesig()
  ...
  !print *, kfl_paral,'paso elmopefirst', lninv_loc(ipoin),size(diffj_isInside) 
  !stop

                if(diffj_isInside(lninv_loc(ipoin))==1_ip)then
                   !dsigmatmp =  exp(design_vars(lninv_loc(ipoin)))
                   dsigmatmp = dsigmatmp + exp(design_vars(lninv_loc(ipoin))) / real(pnode)
                   isInnerElem = 1_ip
                end if
	end do
        ...
        !dsigmatmp = dsigmatmp / pnode
        !design_vars_ref(lninv_loc(ipoin))= log(dsigmatmp+bckco_hlm(1)) 

        if(isInnerElem==0_ip)then
           dsigmatmp = dsigma_hlm(1,pmate) + bckco_hlm(1) 
        end if
	...
	do igaus = 1,pgaus
		do ii=1,ncond_hlm
			...
			dgprea(ii,igaus) = cmplx(0.0_rp,anguf_hlm*perma_hlm(pmate)*(dsigmatmp - bckco_hlm(ii) ),kind=rp)     !Reaction term: i * omega * mu * dsigma 
		end do
	end do
...
end subroutine hlm_elmopedesig
