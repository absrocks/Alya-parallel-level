subroutine ale_sorbjc(dt)
  ...
  do iimbo = 1,nrbod
        ...
        if( pass==0_ip) then
           ...
           ! force_rb = 0.001_rp
           ! posia(1_ip,:) = pi*90.0_rp/180_rp
           ! veloa(1_ip,1_ip) = 0_rp
           ! posia(2_ip,:) = pi*90.0_rp/180_rp !0.5_rp
           ! veloa(2_ip,1_ip) = 0_rp
           pass = pass + 1_ip
           ! return

        else if( pass == 1_ip) then

           ! posia(2_ip,1_ip) = 3.141592/2_rp
        ...
        if( IMASTER )  posia(:,1_ip) = -1e10_rp ! The master doesn't have a meaningful value
        if( IMASTER )  veloa(:,1_ip) = -1e10_rp ! The master doesn't have a meaningful value
        if( IMASTER )  vpfor(:,:)    = -1e10_rp ! The master doesn't have a meaningful value

        do idime = 1_ip, ndime
           call PAR_MAX( posia(idime,1_ip), 'IN MY CODE' )
           call PAR_MAX( veloa(idime,1_ip), 'IN MY CODE' )
           call PAR_MAX( vpfor(idime,1_ip), 'IN MY CODE' )
           call PAR_MAX( vpfor(idime,2_ip), 'IN MY CODE' )
           call PAR_MAX( vpfor(idime,3_ip), 'IN MY CODE' )
           call PAR_MAX( vpfor(idime,4_ip), 'IN MY CODE' )
        end do
        !----------------------------------------------------------------------
        !
