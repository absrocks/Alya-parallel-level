subroutine ale_inirbo
  ...
  do iimbo = 1,nrbod
     ...
     rbbou(iimbo) % quate(1,4) = rbbou(iimbo) % quate(1,1)
     rbbou(iimbo) % q_dot(1,2) = rbbou(iimbo) % q_dot(1,1)
     rbbou(iimbo) % q_dot(1,3) = rbbou(iimbo) % q_dot(1,1)
     rbbou(iimbo) % q_dot(1,4) = rbbou(iimbo) % q_dot(1,1)
     do idime = 1,ndime
        rbbou(iimbo) % posil(idime,2) = rbbou(iimbo) % posil(idime,1)
        rbbou(iimbo) % velol(idime,2) = rbbou(iimbo) % velol(idime,1)
        rbbou(iimbo) % accel(idime,2) = rbbou(iimbo) % accel(idime,1)
        rbbou(iimbo) % posia(idime,2) = rbbou(iimbo) % posia(idime,1)
        rbbou(iimbo) % veloa(idime,2) = rbbou(iimbo) % veloa(idime,1)
        rbbou(iimbo) % accea(idime,2) = rbbou(iimbo) % accea(idime,1)
        rbbou(iimbo) % quate(idime+1,2) = rbbou(iimbo) % quate(idime+1,1)
        rbbou(iimbo) % q_dot(idime+1,2) = rbbou(iimbo) % q_dot(idime+1,1)

        rbbou(iimbo) % posil(idime,3) = rbbou(iimbo) % posil(idime,1)
        rbbou(iimbo) % velol(idime,3) = rbbou(iimbo) % velol(idime,1)
        rbbou(iimbo) % accel(idime,3) = rbbou(iimbo) % accel(idime,1)
        rbbou(iimbo) % posia(idime,3) = rbbou(iimbo) % posia(idime,1)
        rbbou(iimbo) % veloa(idime,3) = rbbou(iimbo) % veloa(idime,1)
        rbbou(iimbo) % accea(idime,3) = rbbou(iimbo) % accea(idime,1)
        rbbou(iimbo) % quate(idime+1,3) = rbbou(iimbo) % quate(idime+1,1)
        rbbou(iimbo) % q_dot(idime+1,3) = rbbou(iimbo) % q_dot(idime+1,1)

        rbbou(iimbo) % posil(idime,4) = rbbou(iimbo) % posil(idime,1)
        rbbou(iimbo) % velol(idime,4) = rbbou(iimbo) % velol(idime,1)
        rbbou(iimbo) % accel(idime,4) = rbbou(iimbo) % accel(idime,1)
        rbbou(iimbo) % posia(idime,4) = rbbou(iimbo) % posia(idime,1)
        rbbou(iimbo) % veloa(idime,4) = rbbou(iimbo) % veloa(idime,1)
        rbbou(iimbo) % accea(idime,4) = rbbou(iimbo) % accea(idime,1)
        rbbou(iimbo) % quate(idime+1,4) = rbbou(iimbo) % quate(idime+1,1)
        rbbou(iimbo) % q_dot(idime+1,4) = rbbou(iimbo) % q_dot(idime+1,1)
     end do
  end  do
  ...
  !
  do iimbo = 1,nrbod
     if (kfl_calgr(iimbo) == 1_ip)  call pararr('SUM',0_ip,ndime,rbbou(iimbo) % posgr)
     call pararr('SUM',0_ip, 1_ip,rbbou(iimbo) % volum)
  ...
  call ale_fixirb         ! obtain bvess_ale - using % cooib coord
