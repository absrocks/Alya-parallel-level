...
contains
  ...
  subroutine SHELL_elemental_operations(itask, ielem, elcoor, eldisp, &
    ...
    do igaus = 1,pgaus
       ...
       do jgaus = 1,pgaus
          ...
          gausspointsthickness: do igausT = 1, pgausT
             ...
             !
             call material_law(ielem, gmrkov(:,:), gmckov(:,:), gmrkon(:,:), grkon(:,:), vodds(:,:), stress(:))
             !
    ...
    if ( kfl_timei_sld == SLD_DYNAMIC_PROBLEM ) then
       ...
       ! Mass matrices
       !
       call compute_mass_matrix(pmate,pgausMass,gpvol,gpsha,elmas,elmuu)
  ...
  subroutine material_law(ielem,gmrkov,gmckov,gmrkon,grkon,stiff,stress)
  ...
  subroutine cshell_stress_model_151(ielem,pmate,grkon,strain,stress,stiff)
    ...
    !
    gpgdi = 0.0_rp ! <GGU> Per l'ortho elastic no es necessita i tampoc fa res dins la rutina, pero es input en aquesta funcio
    call SM_rotate_basis_creation(ielem, gpgdi, rotmat)
  ...
  subroutine compute_mass_matrix(pmate,pgausMass,gpvol,gpsha,elmas,elmuu)
    ...
    real(rp),    intent(out) :: elmas(pnode,pnode)     !< Consistent mass matrix
    real(rp),    intent(out) :: elmuu(pnode)           !< Lumped mass matrix
    ...
    do igaus = 1,2
       ...
       do jgaus = 1,2
          ...
          gausspointsthicknessi: do igausT = 1, pgausT
             ...
             ! Constitutive relationship tensors
             !
             call material_law(ielem, gmrkovi(:,:), gmckovi(:,:), gmrkoni(:,:), grkoni(:,:), &
                  vodds(:,:), stress(:))
             !
  ...
  subroutine sld_local_axes_rhs_and_amatr(itask, ielem, pnode, ndofn, elrhs, elmat)
    ...
    case(1_ip)
       ...
       do inode = 1, pnode
          ...
          itott = (inode - 1) * ndofn
          if ( ibopo > 0 ) then
             if ( kfl_fixno_sld(1,ipoin) == 3_ip .or. kfl_fixrs_sld(ibopo) /= 0_ip ) then
          ...
          if ( ibopo > 0 ) then
             if ( kfl_fixno_sld(1,ipoin) == 3_ip .or. kfl_fixrs_sld(ibopo) /= 0_ip ) then
                ...
                call sld_rotsys(1_ip,inode,pnode,ndofn,ndofn*pnode,&
                     elmat,elrhs,jacrot_du_dq_sld(1,1,ibopo),jacrot_dq_du_sld(1,1,ibopo))
