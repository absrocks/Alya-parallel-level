...
module mod_sld_solution_methods
  ...
  use def_master,          only : itinn, modul, dtime
...
contains
  ...
  subroutine sld_explicit()
    ...
    real(rp)      :: bvesc(ndofn_sld*npoin) !< BC essential (contact)
    ...
    if ( INOTMASTER ) then
       ...
       do ipoin = 1,npoin
          ...
          do idime=1,ndime
             ...
             if ( kfl_fixno_sld(1,ipoin) == 3_ip ) bvesc(itott+idime) = displ(idime,ipoin,TIME_N)
             !
          ...
          ! Local Axes
          !
          call sld_bclocal_displ(ndofn_sld,ipoin,bvesc(:))
    ...
    dunkn_sld = 0.0_rp !<------- important for iterative solvers - it makes a difference...
  ...
  subroutine sld_implicit()
    ...
    if ( INOTMASTER ) then
       ...
       if ( itinn(modul) == FIRSTITERATION ) then
          ...
          if ( kfl_timei_sld == SLD_DYNAMIC_PROBLEM ) then    ! in case of dynamic problem
             ...
             do ipoin = 1, npoin
                ...
                do idime=1,ndime
                   ! Free nodes
                   unknotmp_sld(itott+idime) = displ(idime,ipoin,TIME_N) + veloc_sld(idime,ipoin,TIME_N)*dt + (0.5_rp - nmkbeta)*accel_sld(idime,ipoin,TIME_N)*dt2
       ...
       if ( kfl_timei_sld == SLD_DYNAMIC_PROBLEM ) then
          do ipoin = 1,npoin
             itott = (ipoin-1)*ndofn_sld
             accel_sld(1:ndime,ipoin,ITER_K) = (1.0_rp / (nmkbeta*dt2)) * ( unkno(itott+1:itott+ndime) - unknotmp_sld(itott+1:itott+ndime) )
  ...
  subroutine sld_explicit_tw()
    ...
    if ( INOTMASTER ) then
       ...
       do ipoin = 1,npoin
          ...
          do idime=1, ndime
             !
             ! First partial update nodal velocities: {v}^{n+1}
             !
             ! - Free nodes
             veloctmp_sld(idime,ipoin) = veloc_sld(idime,ipoin,TIME_N) + dtime*accel_sld(idime,ipoin,TIME_N)
             !
             ! Update nodal displacements: {d}^{n+1}
             !
             unkno(itott+idime) = displ(idime,ipoin,TIME_N) + dtime*veloc_sld(idime,ipoin,TIME_N) + phi*(dtime**2)*accel_sld(idime,ipoin,TIME_N)
             ! - Fixed displacements
             if ( kfl_fixno_sld(idime,ipoin) == 1_ip ) unkno(itott+idime) = bvess_sld(idime,ipoin,ITER_K)
             ! - Fixed displacements (PDN Contact nodes)
             if ( kfl_fixno_sld(1,ipoin) == 3_ip )     bvesc(itott+idime) = displ(idime,ipoin,TIME_N)
    ...
    vmass_sld => vmass
    dunkn_sld(:) = rhsid(:)
    call solver_lumped_mass_system(ndime,dunkn_sld(:))
    ...
    if ( INOTMASTER ) then
       ...
       do ipoin = 1,npoin
          ...
          ! Second partial updated nodal velocities: {v}^{n+1}
          !
          veloc_sld(1:ndime,ipoin,ITER_K) = veloctmp_sld(1:ndime,ipoin) + dtime*accel_sld(1:ndime,ipoin,ITER_K)
          !
          ! Other updates
          ! - Update displacement increment
          ddisp_sld(1:ndime,ipoin,ITER_K) = displ(1:ndime,ipoin,ITER_K) - displ(1:ndime,ipoin,TIME_N)
  ...
  subroutine sld_rk4(itask,t,dt,nderi,u)
    ...
    !
    call sld_rk4_derivs(itask,nderi,t, u(:),k1(:))

    uk(1:nderi) = u(1:nderi) + k1(1:nderi)*dt/2.0_rp
    call sld_rk4_derivs(itask,nderi,t,uk(:),k2(:))

    uk(1:nderi) = u(1:nderi) + k2(1:nderi)*dt/2.0_rp
    call sld_rk4_derivs(itask,nderi,t,uk(:),k3(:))

    uk(1:nderi) = u(1:nderi) + k3(1:nderi)*dt
    call sld_rk4_derivs(itask,nderi,t,uk(:),k4(:))
    !
  ...
  subroutine sld_rk4_derivs(itask,nderi,t,u,deriv)
