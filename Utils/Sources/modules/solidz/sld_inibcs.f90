...
!>          =  0 ... Global axes\n
!>          =  1 ... Local axes\n
!>          The different codes for kfl_fixrs_sld(ibopo) are:\n
!>          =  0 ... Exterior normal\n
!>          = -1 ... User Local axes\n
!>
...
subroutine sld_inibcs()
  ...
  use def_domain,  only : kfl_funno, kfl_funbo
  ...
  if ( INOTMASTER ) then
     ...
     if ( kfl_conbc_sld == 0 ) then
        bvess_sld(1:ndime,1:npoin,ITER_AUX) = bvess_sld(1:ndime,1:npoin,ITER_K)
     end if
