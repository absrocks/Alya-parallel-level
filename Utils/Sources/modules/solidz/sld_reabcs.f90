...
subroutine sld_reabcs()
  ...
  use def_solidz, only : tzero_sld, tpstr_sld, pzero_sld, cpres_sld, rpres_sld
  use def_solidz, only : part0_sld, ppost_sld, gfill_sld, pstr0_sld
  use def_solidz, only : kfl_funty_sld, nfunc_sld, mtloa_sld, tload_sld
  use def_solidz, only : fubcs_sld,ocavi_sld,iocav_sld,mcavi_sld,kfl_volca_sld
  ...
  use def_solidz, only : kfl_cycle_sld, parcc_sld, ncoef_sld
  ...
  real(rp)      :: rauxi(2), vect_plane(3,3), v1(ndime), v2(ndime)
  character(5)  :: wcsys
  ...
  if( INOTSLAVE ) then
     ...
     kfl_cycle_sld  = 0                    ! Flag for cardiac cycle management (by default is OFF)
     kfl_bodyf_sld  = 0                    ! Number of body forces (by default is OFF)
     kfl_windk_sld  = 0                    ! Flag for Windkessel functions (by default is OFF)
     kfl_volca_sld  = 0                    ! Dont compute cavity volumes
     ...
     mcavi_sld     = 0                     ! number of cavities (ventricles): to be checked at the end
     tzero_sld     = 0.0_rp
     tpstr_sld     = 0.0_rp
     pzero_sld     = 0.0_rp
     pstr0_sld     = 0.0_rp
     cpres_sld     = 0.0_rp                ! C constant for the windkessel pressure model
     rpres_sld     = 0.0_rp                ! R constant for the windkessel pressure model
     part0_sld     = 0.0_rp
     ppost_sld     = 0.0_rp
     gfill_sld     = 0.0_rp
     ...
     do while (words(1) /= 'ENDBO')
        ...
        if (words(1) == 'PARAM' ) then
           ...
           ! LO QUITE DEL MANUAL PERO NO QUIERO BORRARLO AUN, por eso le puse un ADO
           ! ADO[1]> CYCLE: 8xreal                                         $ Simple cardiac cycle managment (AlyaRED)
           ! ADO[d]> CYCLE:
           ! ADO[d]> <li> CYCLE: tfinf,pfinf,pfins,pfinr,pfirf,C,R1,R2.
           ! ADO[d]> tfinf=time end inflation; pfinf=pressure end inflation; pfins=pressure end systole; pfinr= pressure end iso relaxation;
           ! ADO[d]> pfirf= pressure end rapid fillig; C R1 R2 = windkessel model parameters</li>
           imate = 1
           kfl_cycle_sld = 1
           ...
           do while(words(1)/='ENDCA')
              if (words(1)=='ORIGI') then
                 ocavi_sld(1:ndime,icavi)= param(1:ndime)

              else if (words(1)=='PLANE') then
                 vect_plane= 0.0_rp
                 vect_plane(1,1:ndime)= param(1        :  ndime)
                 vect_plane(2,1:ndime)= param(1+ndime  :2*ndime)
                 vect_plane(3,1:ndime)= param(1+2*ndime:3*ndime)

                 do idime=1,ndime
                    v1(idime)= vect_plane(1,idime) - vect_plane(2,idime)
                    v2(idime)= vect_plane(1,idime) - vect_plane(3,idime)
                 end do

                 call vecpro(v1,v2,ocavi_sld,ndime)

                 ocavi_sld(1:ndime,icavi) = ocavi_sld(1:ndime,icavi) /           &
                  sqrt(ocavi_sld(1,icavi)**2.0_rp + ocavi_sld(2,icavi)**2.0_rp + &
                  ocavi_sld(3,icavi)**2.0_rp)

              else if (words(1)=='BOUND') then          ! Boundary set that defines de inner surface
                 iocav_sld(icavi)= int(param(1))
              else if( words(1) == 'WINDK') then
                 !
                 ! ADOC[3]> WINDKESSEL
                 ! ADOC[4]>   C_PARAMETER= cpres_sld(icavi), R_PARAMETER= rpres_sld(icavi)
                 ! ADOC[4]>   ...
                 ! ADOC[3]> END_WINDKESSEL
                 ! ADOC[d]> <li><b>PRESSURE WINKDKESSEL MODEL:</b> Pressure model to account for arterial networks
                 ! ADOC[d]> outside of the scenario.
                 ! ADOC[d]> Pressure will depend on the volume of this cavity.</li>
                 !
                 kfl_windk_sld= 1_ip              ! windkessel condition is present
                 call ecoute('sld_reabcs')
                 do while( words(1) /= 'ENDWI' )

                    if ( words(1) == 'TZERO') tzero_sld(icavi) = param(1)
                    if ( words(1) == 'TPSTR') tpstr_sld(icavi) = param(1)
                    if ( words(1) == 'PZERO') pzero_sld(icavi) = param(1)
                    if ( words(1) == 'PSTR0') pstr0_sld(icavi) = param(1)
                    if ( words(1) == 'CPRES') cpres_sld(icavi) = param(1)
                    if ( words(1) == 'RPRES') rpres_sld(icavi) = param(1)
                    if ( words(1) == 'PART0') part0_sld(icavi) = param(1)
                    if ( words(1) == 'PPOST') ppost_sld(icavi) = param(1)
                    if ( words(1) == 'GFILL') gfill_sld(icavi) = param(1)
...
!!$              parcc_sld(1:ncoef_sld,imate)=param(1:ncoef_sld)
              ...
              if(kfl_conbc_sld == 0) then                   ! non-constant (transient) BC
                 ...
                 do while(words(1) /= 'ENDCO')
                    if (words(1) == 'FUNCT') then
                       ...
                       else if (words(2)=='WINDK') then    ! windkessel
                          kfl_windk_sld= 1_ip
                          kfl_funty_sld(1,ifunc)=4
     ...
     if (kfl_volca_sld == 0) mcavi_sld=1  ! although no volume is computed, consider one cavity
