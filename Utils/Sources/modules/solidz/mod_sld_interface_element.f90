...
!>
!> @todo    To do list:\n
!>          - Cohesive elements for contact (damaged).
...
module mod_sld_interface_element
  ...
  use def_solidz, only                     :  &
      ...
      parch_sld,                              & ! Cohesive law material parameters
  ...
  integer(ip), parameter                   :: &
    nprop = 8                                   ! Number of material properties
  integer(ip), parameter                   :: &
  ...
  private                                  :: &
    ...
    LAW_COH_TURON_ORIGINAL,                   & ! FUN Cohesive law Turon et al. (Original, 2006)
    LAW_COH_TURON_CURRENT,                    & ! FUN Cohesive law Turon et al. (2018 version)
    midcoord_and_djump,                       &
    shape_functions,                          &
    rotation_matrix,                          &
  ...
  subroutine ELINT_elemental_operations( &
    ...
    real(rp)                               :: &
      ...
      gpderi(pdime-1,pnode/2),                & ! Integration point midsurface derivative of the shape function dN/dn (mid_dime, mid_gauss)
      ...
      gpdtdj(pdime,pdime),                    & ! Integration point midsurface derivative of tractions wrt displ. jump (local coordinate system) (dime, dime)
    ...
    ! Auxiliar dimensions
    !
    sgaus = int(pnode/2) ! Mid-surface gauss points (It has to be equal to half of the number of nodes)
    snode = int(pnode/2) ! Mid-surface number of nodes
    sdime = int(pdime-1) ! Mid-surface number of dimensions
    pevah = int(pevat/2) ! Mid-surface number of degree of freedom
    !
    ...
    gausspoints: do igaus = 1, sgaus
      ...
      !   - save the normal as state variable
      gpSVnew(3:2+pdime) = gprota(:,pdime) ! <AQC> PROVISIONAL
      !
      ! B matrix (B)
      !
  ...
  subroutine LAW_COH_TURON_ORIGINAL( &
    ...
    !
    if (properties(7) > 1.0e-15_rp ) then
      dhist = properties(7)
    else
      dhist = stateOld(1)
    endif
    ...
    elseif( itang == ELINT_TANGENT_ANALYTICAL) then
      ...
      !   - Loading
      if (fload) then
         auxs1 =  (dfin*dini*kpen)/(r**3.0_rp*(dfin - dini))
         do idime = 1, pdime
            do jdime = 1, pdime
    ...
    subroutine LAW_COH_TURON_CURRENT( &
      ...
      K11 = K33*(gI/gII)*(tauII/tauI)**2.0_rp
      if (pdime == 2_ip) then
         K22 = 0.0_rp
      else
         K22 = K11            ! 3-d (K22 = K11 assumption)
      end if
      !
      ! Pure mode openings
      JI0  = tauI/(K33)
      ...
      ! Mix-mode ratio
      if (pdime == 2_ip) then  ! 2-d
         denom = (K11*(J1**2.0_rp) + K33*(J3p**2.0_rp))
         if( denom /= 0.0_rp ) then
            B = (K11*(J1**2.0_rp))/denom
         else
            B = 0.0_rp
         endif
      else                     ! 3-d
         denom = (K11*(J1**2.0_rp) + K22*(J2**2.0_rp) + K33*(J3p**2.0_rp))
         if( denom /= 0.0_rp ) then
            B = (K11*(J1**2.0_rp) + K22*(J2**2.0_rp))/denom
         else
            B = 0.0_rp
         endif
      end if
      ...
      ! - softening
      if( KB /= 0.0_rp ) then
          h0 = sqrt((K33*(JI0**2.0_rp) + (KS*(JII0**2.0_rp) - K33*(JI0**2.0_rp))*(B**eta))/KB)
      else
          h0 = 0.0_rp
      end if
      ! - propagation
      ...
      ! - current jump
      if (pdime == 2_ip ) then  ! 2-d
         denom = sqrt((K11**2.0_rp)*(J1**2.0_rp) + (K33**2.0_rp)*(J3p**2.0_rp))
         if( denom /= 0.0_rp ) then
            h = (K11*(J1**2.0_rp) + K33*(J3p**2.0_rp))/denom
         else
            h = 0.0_rp
         endif
      else                      ! 3-d
         denom = sqrt((K11**2.0_rp)*(J1**2.0_rp) + (K22**2.0_rp)*(J2**2.0_rp) + (K33**2.0_rp)*(J3p**2.0_rp))
         if( denom /= 0.0_rp ) then
            h = (K11*(J1**2.0_rp) + K22*(J2**2.0_rp) + K33*(J3p**2.0_rp))/denom
         else
            h = 0.0_rp
         endif
      end if
      ...
      tu(pdime) =  (1.0_rp - d)*K33*J3p + KCONT*J3n
      !
      ! Elastic Energy
      if (pdime == 2_ip) then  ! 2-d
         Ep = 0.5_rp*(K11*(J1**2.0_rp) + K33*(J3p**2.0_rp))
      else                     ! 3-d
         Ep = 0.5_rp*(K11*(J1**2.0_rp) + K22*(J2**2.0_rp) + K33*(J3p**2.0_rp))
      end if
      !
      ! Material tangent
      if(     itang == ELINT_TANGENT_NUMERICAL ) then
      ...
      elseif( itang == ELINT_TANGENT_ANALYTICAL) then
        !
        call runend('MOD_SLD_INTERFACE_ELEMENT: CUU AND CPP ARE NOT PROGRAMMED YET')

      else! ELINT_TANGENT_EXPLICIT or ELINT_TANGENT_NUMERICAL
        Cuu(:,:) = 0.0_rp
  ...
  subroutine rotation_matrix( &
    ...
    if (pdime == 2_ip ) then      ! 2-d
       ...
       ! Rotation matrix
       gparea = sqrt(r1(1)**2.0_rp + r1(2)**2.0_rp)
       ! Tangential direction
       gprota(1,:) = r1(:)/gparea
       ! Normal direction
       gprota(2,1) = -gprota(1,2)
       ...
       rM = sqrt(r1(1)**2.0_rp + r1(2)**2.0_rp + r1(3)**2.0_rp)
       gprota(:,:) = 0.0_rp
       gprota(1,1) = r1(1)/rM
       gprota(2,1) = r1(2)/rM
       gprota(3,1) = r1(3)/rM
       !
       gparea = sqrt(r3(1)**2.0_rp + r3(2)**2.0_rp + r3(3)**2.0_rp)
  ...
  subroutine nodes_dirichlet_correction( &
    ...
    real(rp), intent(inout)                :: &
      elmat(pevat,pevat),                     &  ! ELEMENT AMAT
      elrhs(pevat)                               ! ELEMENT RHS
    ! ---------------------------------------
    integer(ip)                            :: &
  ...
  subroutine nodes_dirichlet_reaction_forces( &
    ...
    integer(ip), intent(in)                :: &
      ...
      pevat                                      ! Number of degree of freedom
    real(rp),    intent(in)                :: &
      elrhs(pevat)                               ! ELEMENT right hand side
    real(rp),    intent(out)               :: &
      elfrx(pdime,pnode)                         ! ELEMENT rection force
    ! ---------------------------------------
    integer(ip)                            :: &
