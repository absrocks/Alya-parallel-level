...
module mod_sld_plastic_model
  ...
  use def_solidz,         only: svegm_sld, parco_sld, lmate_sld,  kfote_sld
...
contains
  ...
  subroutine sld_plastic_model_biso(pgaus,gpgdi,gpidg,gpdet,gpstr,ielem,flagImpl,gpdds)
    ...
    material_points: do igaus = 1,pgaus
       ...
       call SM_tensor_to_voigt_second(ndime,0_ip, gpgre(:,:,igaus), strain1(:))
       !
       ...
       call SM_tensor_to_voigt_fourth(ndime,1_ip,gpdds_aux(:,:,:,:,igaus),vodds)
       call SM_tensor_to_voigt_second(ndime,1_ip,gpcau(:,:,igaus),stress(:,igaus))
       !
