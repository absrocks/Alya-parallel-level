subroutine sld_crapro(&
  ...
  use def_master, only       :  lun_livei,kfl_paral
  use def_domain, only       :  ndime,lelch,nmate,lnods,kfl_elcoh
  use def_solidz, only       :  kfl_xfeme_sld,leenr_sld,parch_sld,eleng_sld,lcrkf_sld
  use def_solidz, only       :  lecoh_sld,cranx_sld,crapx_sld,lmate_sld,kfl_cohes_sld
  use def_solidz, only       :  nopio_sld,ncrak_sld,sgmax_sld,sgult_sld
  implicit none
  ...
  integer(ip)                :: jelem,nrank,knode,pelty,pmate,eltip
  real(rp)                   :: sigma(ndime,ndime),gpinv,xmean,sigma3(3,3)
  real(rp)                   :: eval3(3),evec3(3,3),dummr,frori(3,3),finor(3)
  real(rp)                   :: detf0,sgmax,sgdir(ndime),tcrit,eleng,cgpos(3)
  real(rp)                   :: weigh,dista,fibor,fiber,towei,elref,elmcg(3)
  real(rp)                   :: tempr
