...
module mod_sld_pdn_contact
  ...
  use mod_commdom_driver,  only : commdom_driver_exchange02
...
contains
...
#ifdef COMMDOM
#if COMMDOM==2
    use mod_commdom_driver, only : CNT_SENDRECV
  ...
  subroutine commdom_sld_plugin_pdn()
#ifdef COMMDOM
#if COMMDOM==2
    use def_master,          only : displ, TIME_N, iblok
    use mod_commdom_driver,  only : CNT_SENDRECV, CNT_CPLNG, commdom_driver_exchange02
    use mod_commdom_plepp,   only : PLEPP_CPLNG
    ...
    use def_solidz,          only : kfl_timet_sld, SLD_EXPLICIT_SCHEME
    ...
    use def_solidz,          only : kfl_conta_sld, vect_proje_sld
             ...
             if ( INOTMASTER ) then
                ...
                if ( ndime == 2_ip ) then ! 2-d problem
                   ...
                   do ipoin = 1,nrecv
                      ...
                      !if( any(kfl_codno(:,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld) .and. (0.99_rp < data_recv(8,ipoin)) .and. (data_recv(8,ipoin) < 1.01_rp) ) then

                         jacrot_du_dq_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) =  data_recv(3,ipoin) !nx
                         jacrot_du_dq_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) =  data_recv(4,ipoin) !ny
                         jacrot_du_dq_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = -data_recv(4,ipoin) !tx
                         jacrot_du_dq_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) =  data_recv(3,ipoin) !ty

                         jacrot_dq_du_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))

                         kfl_fixno_sld(1,PLEPP_CPLNG%interior_list_j(ipoin)) = 3_ip

                         displ(1,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(5,ipoin)*data_recv(6,ipoin) + displ(1,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) + tol
                         displ(2,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(5,ipoin)*data_recv(7,ipoin) + displ(2,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) + tol
                   ...
                   do ipoin = 1,nrecv
                      if ( any(kfl_codno(:,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld) ) then
                      !if( any(kfl_codno(:,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld) .and. (0.99_rp < data_recv(11,ipoin)) .and. (data_recv(11,ipoin) < 1.01_rp) ) then

                         jacrot_du_dq_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = data_recv(4,ipoin) !nx
                         jacrot_du_dq_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = data_recv(5,ipoin) !ny
                         jacrot_du_dq_sld(3,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = data_recv(6,ipoin) !nz
                         jacrot_du_dq_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = data_recv(1,ipoin) !t1x
                         jacrot_du_dq_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = data_recv(2,ipoin) !t1y
                         jacrot_du_dq_sld(3,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = data_recv(3,ipoin) !t1z
                         ...
                         jacrot_du_dq_sld(1,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = tmp_tgx/sqrt(tmp_tgx*tmp_tgx + tmp_tgy*tmp_tgy + tmp_tgz*tmp_tgz) !t2x
                         jacrot_du_dq_sld(2,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = tmp_tgy/sqrt(tmp_tgx*tmp_tgx + tmp_tgy*tmp_tgy + tmp_tgz*tmp_tgz) !t2y
                         jacrot_du_dq_sld(3,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = tmp_tgz/sqrt(tmp_tgx*tmp_tgx + tmp_tgy*tmp_tgy + tmp_tgz*tmp_tgz) !t2z

                         jacrot_dq_du_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(1,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(3,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(2,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(3,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(3,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(1,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(3,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(2,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))
                         jacrot_dq_du_sld(3,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) = jacrot_du_dq_sld(3,3,lpoty(PLEPP_CPLNG%interior_list_j(ipoin)))

                         kfl_fixno_sld(1,PLEPP_CPLNG%interior_list_j(ipoin)) = 3_ip

                         displ(1,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(7,ipoin)*data_recv(8,ipoin) + displ(1,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N)
                         displ(2,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(7,ipoin)*data_recv(9,ipoin) + displ(2,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N)
                         displ(3,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N) = data_recv(7,ipoin)*data_recv(10,ipoin) + displ(3,PLEPP_CPLNG%interior_list_j(ipoin),TIME_N)
                ...
                do ipoin = 1,nrecv
                   ...
                   if( ibopo > 0_ip ) then
                      ...
                      call sld_rotsys(2_ip, 1_ip, 1_ip, ndime, ndime, dummy_matrix, resid_tmp(1:ndime), &
                           jacrot_du_dq_sld(1,1,ibopo), jacrot_dq_du_sld(1,1,ibopo))
                   end if
                   ! Save it
  ...
  subroutine commdom_proje_nodes_2d(tangent_proje,normal_proje,normal_proje2,norm_dist,in_contact,e_recta)
    ...
    integer(ip)                                :: nsend,nrecv
    real(rp), pointer                          :: dist_coords_j(:) => null()   !< Coord. nodes body j (A2) which are inside body i (A1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do ipoin = 1, nsend ! for each of the nodes that belong to the other code (those which plepp has located)
       ...
       do iboun = 1, contactbou_total ! for each of all the boundaries defined as "contact boundary"
          ...
          if (cross < 1e-6_rp) then !if they are colinear, check if the projected point is inside the boundary
             ...
             if ((d1 <= d3) .and. (d2 <= d3)) then  !the node is inside the boundary      x------o------x
                ...
                if ( kfl_conta == SLD_PDN_UNILATERAL ) then
                   ...
                   if ( ((y1-y2)*(ax-x1)+(x2-x1)*(ay-y1))*((y1-y2)*(bx-x1)+(x2-x1)*(by-y1)) < 0_rp ) then ! LADO OPUESTO, INVIERTO SIGNOS
                   ...
                   else                                                                                   ! DEL MISMO LADO, DEJO TODO COMO ESTA
                      normal_proje2(1,ipoin) = normal_proje(1,ipoin)
                      normal_proje2(2,ipoin) = normal_proje(2,ipoin)
                      in_contact(ipoin) = 1.0_rp
                   end if
          ...
          if(tmp > 1_ip) call runend("runend in commdom_proje_nodes_2d -- a projection is inside of more than one boundary element")
    ...
    if (tmp2 /= nsend) then
       call runend("ME SOBRAN O ME FALTAN PROYECCIONES DE NODOS... BYE!!!!")
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!--PARALLEL TWEAK - SEND CONTACT BOUNDARY TO ALL SLAVES--!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do ipoin = 1, nsend !for each of the nodes that belong to the other code (those which plepp has located)
       ...
       do iboun = 1, contactbou_total ! For each of all the boundaries defined as "contact boundary"
          ...
          vec3(4) = -(vec3(1)*bocod_total(1,1,iboun) + vec3(2)*bocod_total(2,1,iboun) + vec3(3)*bocod_total(3,1,iboun)) !independent term (d)
          ...
          if ((t2 .eq. 0_rp) .or. (t1 .eq. 0_rp)) call runend("runend in commdom_proje_nodes_3d -- point to project is already on the plane")
          ...
          if ( ifoun == 1_ip ) then !the projected point is inside the triangle or quad
             ...
             if ( kfl_conta == SLD_PDN_UNILATERAL ) then
                ...
                pi_origin_coord = vec3(1)*orig_coord_j(1,ipoin) + vec3(2)*orig_coord_j(2,ipoin) + vec3(3)*orig_coord_j(3,ipoin) + vec3(4)
             ...
             !the point to project is given by coord_j and the plane has a normal given by vec3(:) and passes through coord_proje(:)
             vec3(4) = -(vec3(1)*coord_proje_tmp(1) + vec3(2)*coord_proje_tmp(2) + vec3(3)*coord_proje_tmp(3)) !recalculate the independent term of tangent plane
             norm_dist(ipoin) = abs(vec3(1)*coord_j(1,ipoin) + vec3(2)*coord_j(2,ipoin) + vec3(3)*coord_j(3,ipoin)  + vec3(4))/sqrt(vec3(1)*vec3(1) + vec3(2)*vec3(2) + vec3(3)*vec3(3))
             tmp2 = tmp2 + 1_ip
          end if

          if (tmp > 1_ip) call runend("runend in commdom_proje_nodes_3d -- a projection is inside of more than one boundary element")
  ...
  subroutine commdom_sld_bilateral_neumann()
    ...
    ! Relaxation
    !
    reaction_ant(1:ndime,1:npoin,3_ip) = reaction_ant(1:ndime,1:npoin,2_ip)
    reaction_ant(1:ndime,1:npoin,2_ip) = reaction_ant(1:ndime,1:npoin,1_ip)
    !
    if ( coupling_driver_iteration(iblok) < 3_ip*10000_ip ) then
       ...
       do ipoin = 1,nrecv
          do idime = 1,ndime
             rip1 = reaction_ant(idime,PLEPP_CPLNG%interior_list_j(ipoin),4_ip) - reaction_ant(idime,PLEPP_CPLNG%interior_list_j(ipoin),3_ip)
             rip2 = CNT_CPLNG%var_ji(idime,PLEPP_CPLNG%interior_list_j(ipoin)) - reaction_ant(idime,PLEPP_CPLNG%interior_list_j(ipoin),2_ip)
    ...
    !
    solve(1)%bvnat(1:ndime,1:npoin) = 0.0_rp
    !
    if ( (PLEPP_CPLNG%commij /= -1).or.(PLEPP_CPLNG%commji /= -1) ) then
       if ( INOTMASTER ) then
          solve(1)%bvnat(1:ndime,PLEPP_CPLNG%interior_list_j) = relax_bilateral * CNT_CPLNG%var_ji(1:ndime,PLEPP_CPLNG%interior_list_j) + &
    ...
    if ( INOTMASTER ) then
       do idime = 1,ndime
          res_diff(idime) = dot_product( (solve(1) % bvnat(idime,PLEPP_CPLNG%interior_list_j) - reaction_ant(idime,PLEPP_CPLNG%interior_list_j,1_ip)), &
                                         (solve(1) % bvnat(idime,PLEPP_CPLNG%interior_list_j) - reaction_ant(idime,PLEPP_CPLNG%interior_list_j,1_ip)) )
          res_reac(idime) = dot_product( solve(1) % bvnat(idime,PLEPP_CPLNG%interior_list_j) , solve(1) % bvnat(idime,PLEPP_CPLNG%interior_list_j) )
       end do
    end if
    !
    ! Assign reaction to the other instance
    !
    reaction_ant(1:ndime,1:npoin,1_ip) = solve(1) % bvnat(1:ndime,1:npoin)
    reaction_ant(1:ndime,1:npoin,4_ip) = CNT_CPLNG%var_ji(1:ndime,1:npoin)
    !
    ! Parall
    !
    ...
    ! Live info
    !
    print *,"ITERACION DE ACOPLE -- RESIDUO ACOPLE CONTACTO --> ", coupling_driver_iteration(iblok), " ------ ", res_diff(1:ndime), "---", relax_bilateral
    !
    ! Parall
    !
    call commdom_dynamic_reduce_sum(prop_in, prop_out)
  ...
  !> @author  Matias Rivero
  !> @date    Februaury, 2019
  !> @brief   Subroutine PDN contact begin time step
  !> @details Set number of coupling contact interations for unilateral and
  !>          bilateral methods
  !----------------------------------------------------------------------------

  subroutine sld_pdn_begste()
    ...
    if( kfl_conta_sld == SLD_PDN_BILATERAL ) then
       ...
       !
       ! Unilateral (Matias) and Rigid-Deformable
       !
       coupling_driver_max_iteration(iblok) = 1_ip
    end if

  end subroutine sld_pdn_begste
  ...
  subroutine commdom_sld_calc_reaction()
    ...
    use def_domain,          only : kfl_codno, lpoty, ndime
    use mod_commdom_plepp,   only : PLEPP_CPLNG
    use mod_commdom_driver,  only : CNT_CPLNG
    use def_solidz,          only : frxid_sld, fcont_sld
    use def_solidz,          only : SLD_IMPLICIT_SCHEME,kfl_timet_sld
    use def_solidz,          only : release_nodes, new_to_release, contactbou_sld

    implicit none

    integer(ip) :: ipoin, irecv, nrecv, itott, ibopo
    real(rp)    :: frxid_aux(ndime,npoin), fcont(ndime,npoin)
    ...
    if( CNT_CPLNG%current_code == CNT_CPLNG%code_j ) then
       ...
       if( INOTMASTER ) then
          ...
          nrecv = PLEPP_CPLNG%n_ji
          frxid_aux(1:ndime,1:npoin) = reshape(frxid_sld,shape=(/ndime,npoin/)) ! already exchanged
          !
          ...
          do irecv = 1, nrecv
             ...
             if( kfl_fixno_sld(1,ipoin) == 3_ip ) then
                ...
                ! Node check
                !
                if( kfl_codno(1,ipoin) /= contactbou_sld ) call runend('MOD_SLD_PDN_CONTACT: GET REACTION OF CONTACT NODE OUT OF THE CONTACT SURFACE')
                !
                ! Reaction method
                !
                if( kfl_timet_sld == SLD_IMPLICIT_SCHEME ) then
                   if( algebraic ) then
                      ...
                      fcont(1:ndime,ipoin) = solve(1) % reaction(1:ndime,ipoin)  ! already exchanged by solver
                   else
                      ! Reaction from solidz
                      fcont(1:ndime,ipoin) = frxid_aux(1:ndime,ipoin)            ! already exchanged
                   end if
                else
                   fcont(1:ndime,ipoin) = frxid_aux(1:ndime,ipoin)
                end if
                !
                ! Mark nodes in traction
                !
                if( fcont(1,ipoin) > 0.0_rp .and. release_nodes(irecv) == 0_ip ) then ! not marked
                ...
                !
                call sld_csys_rotuni(2_ip,ndime,ipoin,fcont_sld(itott+1:itott+ndime)) ! Not exchanged yet!
       ...
       if ( INOTMASTER ) then
          ...
          do irecv = 1,nrecv
             ...
             if( kfl_fixno_sld(1,ipoin) == 3_ip ) then
                ...
                ! Node checks
                !
                if( kfl_codno(1,ipoin) /= contactbou_sld ) call runend('MOD_SLD_PDN_CONTACT: RELEASING CONTACT NODE OUT OF THE CONTACT SURFACE')
                !
                ! Set free node
                !
                if ( release_nodes(irecv) == 1_ip ) then
  ...
  subroutine sld_pdn_conta_projection_recv(ndata,nsend,nrecv,data_send,data_recv)
    ...
    if ( INOTMASTER ) then
       ...
       do irecv = 1, nrecv
          ...
          if ( kfl_codno(1,ipoin) == contactbou_sld ) then       ! nodes that belong to the boundary
             !
             ...
             !
             jacrot_du_dq_sld(1:ndime,1,ibopo) = N(1:ndime)
             jacrot_du_dq_sld(1:ndime,2,ibopo) = T1(1:ndime)
             if( ndime == 3_ip ) jacrot_du_dq_sld(1:ndime,3,ibopo) = T2(1:ndime)
             ! Transpose of R
             jacrot_dq_du_sld(:,:,ibopo) = transpose(jacrot_du_dq_sld(:,:,ibopo))
             !
             ! Mark contact node
             !
             ...
             ! Prescribe Dirichlet
             !
             bvess_aux(1:ndime) = data_recv(ndime*2+1,irecv)*data_recv(ndime*2+2:ndime*3+1,irecv) + 1.0e-4_rp
             !
  ...
  subroutine sld_rbo_bilateral_send()
    ...
    if ( INOTMASTER ) then
       ...
       force_dbo(1:ndime) = sum(fcont_aux(1:ndime,PLEPP_CPLNG%interior_list_j), dim=2)
       !
       ! Parall exchange
       !
    ...
    if ( INOTMASTER ) then
       do idime=1,ndime
          CNT_CPLNG%var_ij(idime,1:npoin) = -force_dbo(idime)
       end do
    end if
    !
    ! Exchange data between instancess
    !
    call commdom_driver_exchange02( CNT_CPLNG , debug=.false.) !-> SEND
    !
    ! Live info
    !
    call livinf(-14_ip,'-> SEND '//trim('FORCE')//' AS A SOURCE FOR COUPLING ',1_ip)
    !
  ...
  subroutine sld_rbo_bilateral_recv( )
    ...
    !
    call commdom_driver_exchange02( CNT_CPLNG , debug=.false.) !<- RECV
    !
    ! Compatibility checks
    !
    ...
    ! Live info
    !
    call livinf(-14_ip,'<- RECV '//trim('FORCE')//' AS A TARGET FOR COUPLING ',1_ip)
  ...
  subroutine commdom_sld_impose_friction()
    ...
    character(5)               :: legend
    integer(ip)                :: ievat,jevat,ibopo,dummy_matrix(ndofn_sld,ndofn_sld)
    real(rp)                   :: rhsid_TMP(npoin*ndofn_sld)
             ...
             if( any(kfl_codno(:,PLEPP_CPLNG%interior_list_j(ipoin)) == contactbou_sld) ) then
                if(ndime == 2_ip) then
                   if(rhsid(2*idx) /= 0_rp) then
                      !jacrot_du_dq_sld(1,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) --> nx
                      !jacrot_du_dq_sld(2,1,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) --> ny
                      !jacrot_du_dq_sld(1,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) --> tx
                      !jacrot_du_dq_sld(2,2,lpoty(PLEPP_CPLNG%interior_list_j(ipoin))) --> ty
                      ...
                      !if( (0.8199999_rp < coord(1,idx)) .and. (coord(1,idx) < 0.82010000_rp) ) write(789,*) coord(1,idx), rhsid1st_it(2*idx-1), rhsid1st_it(2*idx), cutim, ittim
                   end if
                else if(ndime == 3_ip) then
                   !rhsid(3*idx-2)
                   !rhsid(3*idx-1)
