...
contains
  ...
  subroutine sld_bclocal_displ(ndofn,ipoin,bvesc)
    ...
    real(rp),    optional   :: bvesc(ndofn*npoin)  !< Dirichlet contact nodes
    ...
    if ( ibopo > 0 ) then
       ...
       else if ( kfl_fixno_sld(1,ipoin) == 3_ip .and. kfl_timet_sld == SLD_EXPLICIT_SCHEME ) then
          !
          ! PDN Contact - Modify nodal displacement for contact nodes
          !
          if ( present(bvesc) ) then
             ! - Global --> Local
             call sld_csys_rotuni(1_ip,ndofn,ipoin,unkno(itott+1:itott+ndofn))
             call sld_csys_rotuni(1_ip,ndofn,ipoin,bvesc(itott+1:itott+ndofn))
             ! - Fixed displacement
             unkno(itott+1) = bvesc(itott+1)  ! normal value
             ! - Global <-- Local
             call sld_csys_rotuni(2_ip,ndofn,ipoin,unkno(itott+1:itott+ndofn))
             call sld_csys_rotuni(2_ip,ndofn,ipoin,bvesc(itott+1:itott+ndofn))
             !
             ! Update displacement vector
             !
             displ(1:ndofn,ipoin,ITER_K) = unkno(itott+1:itott+ndofn)
          end if
  ...
  subroutine sld_bclocal_rhsid_fixity()
    ...
    integer(ip) :: ipoin, ibopo, idime, itott
    ...
    do ipoin = 1,npoin
       ...
       if ( ibopo > 0 ) then
          ...
          if ( kfl_fixrs_sld(ibopo) /= 0_ip ) then
             ...
             !
             do idime = 1, ndime
                if ( kfl_fixno_sld(idime,ipoin) == 1_ip ) rhsid(itott+idime) = 0.0_rp
             end do
             call sld_csys_rotuni(2_ip,ndime,ipoin,rhsid(itott+1:itott+ndime))  ! rotate back

          else if ( kfl_fixno_sld(1,ipoin) == 2_ip .or. kfl_fixno_sld(1,ipoin) == 3_ip ) then
             !
  ...
  subroutine sld_bclocal_roback(ndofn,ipoin)
    ...
    if ( ibopo > 0 ) then
       ...
       if ( kfl_fixno_sld(1,ipoin) == 2_ip .or. &
            kfl_fixno_sld(1,ipoin) == 3_ip .or. kfl_fixrs_sld(ibopo) /= 0_ip ) then
