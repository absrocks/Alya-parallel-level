subroutine sld_dstrain(ielem,pnode,pgaus,pmate,eldis,gpsha,gpcar,gpgi0,gpgdi,deltaE)
  ...
  use def_solidz, only       :  nvgij_inv_sld
  use def_master, only       :  kfl_paral,coupling, ittim,ITER_K, TIME_N, dtime
  ...
  GAUSS_POINTS_LOOP: do igaus = 1,pgaus
    ...
    !
    call SM_polar_decomposition(deltaF(1,1,igaus),dRrot(1,1,igaus),dUdef(1,1,igaus))
    ...
    do kdime=1,ndime
      ...
      call SM_polar_decomposition(gpgi1(1,1,igaus),rstr1(1,1,igaus),udefo(1,1,igaus))
    ...
    !
      ...
      gplep0 = 0.0_rp
      !Get R and U (F=R*U : gpgi1 = rstr1*udefo)
      call SM_polar_decomposition(gpgi0(1,1,igaus),rstr1(1,1,igaus),udefo(1,1,igaus))
