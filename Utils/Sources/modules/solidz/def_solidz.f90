...
module def_solidz
  ...
  integer(ip),   parameter            :: &
       ...
       ncoef_sld                  =  30    ! # coefficient for material laws
  ...
  real(rp),     pointer               :: &
       ...
       preti_sld(:),                     & ! Tiem to pre-stress for cardiac
       ...
       ortk1_sld(:),                     & ! First coefficient for the orthotropic activation
       ortk2_sld(:),                     & ! First coefficient for the orthotropic activation
  ...
  integer(ip)                           :: &
       ...
       kfl_rsfor_sld,                      & ! Calculo RF y U en un set de nodos con dirichlet (postprocess in cvgunk)
       kfl_safet_table_sld,                & !
  ...
  real(rp)                            :: &
       tzero_sld(4),                     & ! Preloading time for the windkessel pressure model
       tpstr_sld(4),                     & ! Initiation of the prestress (make sure that tpstr_sld < tzero_sld)
       pzero_sld(4),                     & ! Initial ventricular pressure for the windkessel pressure model
       pstr0_sld(4),                     & ! Initial value of the prestress during tpstr_sld
       cpres_sld(4),                     & ! C constants for the windkessel pressure model
       rpres_sld(4),                     & ! R constants for the windkessel pressure model
       part0_sld(4),                     & ! Initial arterial pressure for the windkessel model
       ppost_sld(4),                     & ! Pressure to switch to filling phase for the windkessel model
       gfill_sld(4),                     & ! Decay constant of the filling phase for the windkessel model
       prest_sld(4),                     & ! Pre-stress for cardiac
  ...
  integer(ip)                         :: &
       ...
       kfl_csysp_sld,                    & ! Existance of a local coordinate system for post-process
  ...
  real(rp)                            :: &
       ...
       csysp_sld(9,10),                  & ! Parameters local coordinate system for post-process
  ...
  real(rp),     pointer                :: &
       vdiag_sld(:),                      & ! Local timesteps diagonal mass matrix
  ...
  real(rp), pointer                     :: &
       ...
       eleng_sld(:),                       & ! Element characteristic length (at the begining of the run)
  ...
  real(rp)                              :: &
       ...
       resid_sld,                          & ! Residual, external iterations (coupling problem)
       ...
       resforce_sld(3,10),                 & ! Residual force for 10 sets
       resdispl_sld(3,10),                 & ! Displacement for residual force for 10 sets
  ...
  ! ODE vector for Myofilament model
  real(rp)                              :: &
       myofi_sld(12,3)
  !
  ...
  type(r3p), pointer                    :: &
       ...
       svaux_sld(:)                          ! Auxiliar state variables for postprocess purposes

  real(rp), pointer :: aux_sld(:,:,:)
