...
subroutine sld_begste()
  ...
  use mod_sld_pdn_contact, only : sld_pdn_begste
  ...
  if ( kfl_stead_sld /= 1_ip ) then
     ...
     if ( kfl_rigid_sld == 0_ip ) then
        ...
        !
        call sld_updunk(1_ip)     ! Update unknowns (:,ITER_AUX) <= (:,TIME_N)
        call sld_updene(1_ip)     ! Update energies
        !
        ! Update boundary conditions
        !
        call sld_updbcs(1_ip)
        !
        ! Updates XFEM/CZM
        !
        ...
        !
        if ( kfl_conta_sld /= 0_ip ) call sld_pdn_begste()
        ...
        !
        call sld_rbo_updunk(1_ip) ! Update unknowns (:,ITER_AUX) <= (:,TIME_N)
        !
        ! Coupling: PDN contact
        !
        if ( kfl_conta_sld /= 0_ip ) call sld_pdn_begste()
