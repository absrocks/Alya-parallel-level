subroutine sld_elmcla(&
     ...
     gpigd,gptlo,gptmo,nfibe,ielem,elcod,pnode,lnods,gpsha,gpdds,&
...
!-----------------------------------------------------------------------
  ...
  if (kfl_tange_sld == SLD_TANGENT_CHECK .or. kfl_tange_sld == 1_ip) then
  ...
  call sld_builtin_materials(pgaus,pmate,gpvol,temp0,temp1,&
       gpgi0,gpgdi,gpigd,gpcau,gpdet,gprat,gppio,gpstr,gpene,&
       flagt,gptmo,flags,ggsgo,ielem,gptlo,nfibe,elcod,pnode,lnods,gpsha,gpdds,&
       gpigd_eps,gpgdi_eps,gpdet_eps,gppio_eps,&
       gpmof)
  !
