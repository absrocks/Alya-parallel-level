...
subroutine sld_stress_model_134implicit(&
  ...
  use def_solidz
  use def_master, only : cutim,ittim,postp,ittyp,ITASK_ENDRUN,fiber,gpfib,vconc,coupling,         &
   kfl_paral,fisoc,kfl_eccty,dtime, stateland, cell_ca0_ecc
  use def_master, only : kfl_cellmod,CELL_TT2006_EXMEDI,CELL_OHARA_EXMEDI,CELL_FITZHUGH_EXMEDI,   &
   CELL_NOMOD_EXMEDI
  ...
  real(rp), intent(in) :: gpcau(ndime,ndime,pgaus),gpdet(pgaus),gpgdi(ndime,ndime,pgaus),         &
   ...
   gpigd_eps(ndime,ndime,pgaus),gpdet_eps(pgaus),gptlo(pgaus),gpvol(pgaus),elcod(ndime,mnode)   
  ...
  real(rp) :: detf0,nfibe_length,tactf,nfibe(ndime,pgaus),nfibe_2(ndime,pgaus),                   &
   gpcal(ndime,ndime),castr(ndime,ndime),castr_active(ndime,ndime),gpstr_active(ndime,ndime),     &
   a,b,af,bf,as,bs,afs,bfs,scalf,i1,i4f,i4s,i4n,i8fs,term1,gpcai(ndime,ndime),Kct,lamda(3),bidon, &
   dummr,ovlam(3),elfib(3,mnode),elfis(3,mnode),elfin(3,mnode),elcac(mnode),gpcac,tkron(3,3),     &
   statvar(6,2),fporf(ndime,ndime),fporf_0(ndime,ndime),spors(ndime,ndime),spors_0(ndime,ndime),  &
   ...
   fxfxinvc(ndime,ndime,ndime,ndime),invcxfxf(ndime,ndime,ndime,ndime),                           &
   i4fxi4f(ndime,ndime,ndime,ndime),sxsxinvc(ndime,ndime,ndime,ndime),                            &
   invcxsxs(ndime,ndime,ndime,ndime),i4sxi4s(ndime,ndime,ndime,ndime),                            &
   fsxfsxinvc(ndime,ndime,ndime,ndime),invcxfsxfs(ndime,ndime,ndime,ndime),                       &
   i8fsxi8fs(ndime,ndime,ndime,ndime),dev_ipori(ndime,ndime),J,J23,J43,                           &
   ...
   iso_i4s,iso_i8fs,fxfxfxf(ndime,ndime,ndime,ndime),beta,Cal50,cca2p,n,                          &
   tan_opt_act(ndime,ndime,ndime,ndime),tmaxi,calterm,kact,i4fp,i4sp,                             &
   sxsxsxs(ndime,ndime,ndime,ndime),nxnxnxn(ndime,ndime,ndime,ndime),                             &
   nxnxinvc(ndime,ndime,ndime,ndime),h,lambda_aux,Jac_statvar(6,6),param(25),dhdlambda,dtime_aux, &
   dRdlambda(6),dsdlambda(6),S,W,CaTRPN,XB,zeta_s,zeta_w,Ca50,eloterm,ort_act,dtactf,tacts,tactn, &
   sxsxfxf(ndime,ndime,ndime,ndime),nxnxfxf(ndime,ndime,ndime,ndime),nporn_0(ndime,ndime), ca0
  ! nxnxinvc(ndime,ndime,ndime,ndime),sxsxinvc(ndime,ndime,ndime,ndime),                           &
  ! fxfxinvc(ndime,ndime,ndime,ndime)
  ...
  do igaus = 1,pgaus
    ...
    spk_4f = 2.0_rp*J23*af*i4fp*term4f*dev_fporf
    spk_4s = 2.0_rp*J23*as*i4sp*term4s*dev_spors
    ...
    if ((coupling('SOLIDZ','EXMEDI') >= 1) .or. (coupling('EXMEDI','SOLIDZ') >= 1)) then
      ...
      ! Activation for Hunter and Land models
      if ((kfl_eccty(pmate) == 1 .or. kfl_eccty(pmate) == 3 .or. kfl_eccty(pmate)==4) .and. (ikact > 0)) then

        ! Retrieve state variables for the Land model
        if (kfl_eccty(pmate) == 3 .or. kfl_eccty(pmate)==4 ) then

          ! State variables equal to the converged ones of the previous time step
          statvar(1:6,1) = stateland(1:6,ielem,igaus,1) 

        end if

        call sld_eccoup(pgaus,pmate,igaus,gpdet,gptlo,lamda,nfibt,nshtt,normt,statvar,gpigd,      &
         castr_active,gpstr_active,gpcac,tactf,ielem,Jac_statvar,param)             

        if (kfl_eccty(pmate) == 3 .or. kfl_eccty(pmate)==4) then

          ! Non-converged global state variables equal to the result of the Land system of ODEs
          stateland(1:6,ielem,igaus,2) = statvar(1:6,2)
          exm_lambda_sld(ielem,igaus,2) = lamda(1)

        end if

      ! Activation for Rice model
      ...
      castr_active = scalf * castr_active
      gpstr_active = 0.0_rp
      do idime = 1,ndime
        do jdime = 1,ndime
          do kdime = 1,ndime
            do ldime = 1,ndime
              gpstr_active(idime,jdime) = gpstr_active(idime,jdime)+gpdet(igaus)*                 &
               gpigd(idime,kdime,igaus)*castr_active(kdime,ldime)*gpigd(jdime,ldime,igaus)
            end do
          end do
        end do
      end do
    ...
    if (flagt == 1_ip) then
        ...
        do jdime = 1,ndime
          do kdime = 1,ndime
            do ldime = 1,ndime
              ...
              i_invc(idime,jdime,kdime,ldime) = (gpcai(idime,kdime)*gpcai(jdime,ldime)+           &
      ...
      tan_opt_aniso_4f = 4.0_rp*J23*af*i4fp*term4f*(-(1.0_rp/3.0_rp)*fxfxinvc-(1.0_rp/3.0_rp)*    &
       invcxfxf+(1.0_rp/9.0_rp)*i4f*invcxinvc+(1.0_rp/6.0_rp)*i4f*i_invc)+ &
       4.0_rp*J43*af*term4f*i4fxi4f+8.0_rp*J43*af*bf*(i4fp**2.0_rp)*term4f*i4fxi4f

      tan_opt_aniso_4s = 4.0_rp*J23*as*i4sp*term4s*(-(1.0_rp/3.0_rp)*sxsxinvc-(1.0_rp/3.0_rp)*    &
       invcxsxs+(1.0_rp/9.0_rp)*i4s*invcxinvc+(1.0_rp/6.0_rp)*i4s*i_invc)+4.0_rp*J43*as*term4s*   &
       i4sxi4s+8.0_rp*J43*as*bs*(i4sp**2.0_rp)*term4s*i4sxi4s
      ...
      if ((coupling('SOLIDZ','EXMEDI') >= 1) .or. (coupling('EXMEDI','SOLIDZ') >= 1)) then
        ...
        ! Active component of the Hunter model
        if (kfl_eccty(pmate) == 1) then

          !!
          ! Ca0 is not necesarily zero for the EP model.
          ! An aveage of the three cells is done to avoid the
          ! gather operation. This produces a neglible error 
          ! as the three Ca0 are similar.
          !!
          ...
          calterm = (cca2p**n/(cca2p**n+Cal50**n))
          eloterm = 1.0_rp+beta*(lamda(1)-1.0_rp)
          kact = calterm*tmaxi*eloterm

          ! Orthotropic activation
          if( kfl_fiber_sld == 2 .or. kfl_fiber_sld == 3) then
            call runend('SLD_STRESS_134IMPL: ORTHOTROPIC ACTIVATION FOR HUNTER NOT CODED')
          else
            tan_opt_act = (1.0_rp/(lamda(1)**2.0_rp))*(((tmaxi*beta*calterm/lamda(1))-            &
             2.0_rp*(kact/(lamda(1)**2.0_rp)))*fxfxfxf)
          end if

        ! Active component of the Land model
        elseif (kfl_eccty(pmate) == 3 .or. kfl_eccty(pmate)==4) then
          ...
          ! Retrieve the time increment in milliseconds
          dtime_aux = dtime
          ...
          ! Retrieve Ca50
          Ca50 = param(3)+param(16)*(lambda_aux-1.0_rp)

          ! Calculate dRdlambda and dsdlambda
          dRdlambda(1) = 0.0_rp
          dRdlambda(2) = 0.0_rp

          if (lamda(1) < 1.2_rp) then
            dRdlambda(3) = param(16)*param(2)*param(1)*dtime_aux*(1.0_rp-CaTRPN)*                 &
             ((gpcac**param(2))*(Ca50**(-param(2)-1.0_rp)))
          else
            dRdlambda(3) = 0.0_rp
          end if

          dRdlambda(4) = 0.0_rp
          dRdlambda(5) = -param(20)
          dRdlambda(6) = -param(20)

          call invert(Jac_statvar,6,6)

          dsdlambda = -MATMUL(Jac_statvar,dRdlambda)

          ! Definition of consistent tangent operator
          h = MAX(0.0_rp,1.0_rp+param(15)*(lambda_aux+MIN(0.87_rp,lambda_aux)-1.87_rp))
          if (lamda(1) < ((1.87_rp*param(15)-1.0_rp)/(2.0_rp*param(15)))) then
            dhdlambda = 0.0_rp
          elseif (lamda(1) < 0.87_rp) then
            dhdlambda = 2.0_rp*param(15)
          elseif (lamda(1) < 1.2_rp) then
            dhdlambda = param(15)
          else
            dhdlambda = 0.0_rp
          end if

          dtactf = (param(17)/param(10))*(dhdlambda*((zeta_s+1.0_rp)*S+zeta_w*W)+                 &
           h*((dsdlambda(5)*S+zeta_s*dsdlambda(1))+(dsdlambda(6)*W+zeta_w*dsdlambda(2))))

          tan_opt_act = (1.0_rp/(lamda(1)**3.0_rp))*(dtactf-2.0_rp*(tactf/lamda(1)))*fxfxfxf !+              &

          ! Orthotropic activation
          if( kfl_fiber_sld == 2 .or. kfl_fiber_sld == 3) then
            tan_opt_act = tan_opt_act+                                                            &
             param(24)*((dtactf/((lamda(2)**2.0_rp)*lamda(1)))*sxsxfxf-                           &
             (2.0_rp/(lamda(2)**4.0_rp))*tactf*sxsxsxs)+                                          &
             param(25)*((dtactf/((lamda(3)**2.0_rp)*lamda(1)))*nxnxfxf-                           &
             (2.0_rp/(lamda(3)**4.0_rp))*tactf*nxnxnxn) 
          end if
        end if
...
end subroutine sld_stress_model_134implicit
