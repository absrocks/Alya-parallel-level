...
!-----------------------------------------------------------------------
  ...
  integer(ip)                :: ii,jj,kk,ll,idumm,ivoig
 ...
 do igaus=1,pgaus
    ...
    do ii=1,6
       ...
       if (lawst_sld(pmate)==100) then                                         ! isolinear elastic
          call sld_stress_model_100(pgaus,pmate,Fpert,Spert,pertcau,Fpdet,gptlo,Fpinv,ielem,elcod,flagt,pertdds,gpmof)    !with activation
       else if (lawst_sld(pmate)==101) then                                    ! Neo-Hookean belytschko (textbook)
          ...
          call sld_stress_model_134(pgaus,pmate,pertcau,Fpert,Fpinv,Spert,Fpdet,gptlo,nfibe,ielem,elcod,&
               pnode,lnods,gpsha,flagt,pertdds,gpmof)
       end if
