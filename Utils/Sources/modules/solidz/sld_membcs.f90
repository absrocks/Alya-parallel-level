...
!-----------------------------------------------------------------------
  ...
  if (itask == 1_ip) then
     ...
     if( INOTMASTER ) then
        ...
        call memory_alloca( mem_modul(1:2,modul),'KFL_FIXRS_SLD','sld_membcs',kfl_fixrs_sld,nbopo           ) ! Local axes
        ...
        if( kfl_local_sld /= 0_ip .or. kfl_conta_sld /= 0_ip ) then
           call memory_alloca( mem_modul(1:2,modul),'JACROT_DU_DQ_SLD','sld_membcs',jacrot_du_dq_sld,ndime,ndime,nbopo ) ! Rot. matrix
           call memory_alloca( mem_modul(1:2,modul),'JACROT_DQ_DU_SLD','sld_membcs',jacrot_dq_du_sld,ndime,ndime,nbopo ) ! Rot. matrix (transpose)
        end if

     end if

     call memory_alloca_min( kfl_fixno_sld )
     call memory_alloca_min( kfl_fixbo_sld )
     call memory_alloca_min( kfl_fixrs_sld )
     call memory_alloca_min( jacrot_du_dq_sld )
     call memory_alloca_min( jacrot_dq_du_sld )
     call memory_alloca_min( bvess_sld     )
     call memory_alloca_min( bvnat_sld     )
