...
subroutine sld_element_operations(itask,ielem,ielem_min,gpdet_min)
  ...
  use mod_sld_interface_element, only : ELINT_elemental_operations            ! Cohesive element (*AQU*)
  ...
  integer(ip) :: dummi
  integer(ip) :: pelty,pmate,pnode,pgaus,plapl,porde,PROJECT=1
  ...
  if ( pelty > 0 .and. pelty /= SHELL .and. pelty /= BAR3D .and. lawst_sld(lmate(ielem)) /= 200_ip .and. lelch(ielem) /= ELINT) then
     ...
     call sld_elmpre(&
          ...
          gpmof,gprestre,gpdet_min,ielem_min,troponin_temp)
...
!!$        end if
     ...
     call sld_elmcla(&
          ...
          gpdet,gprat,gpigd,gptlo,gptmo,&
          nfibe,ielem,elcod,pnode,lnods(1,ielem),gpsha,gpdds,&
     ...
     if( itask <= 2 ) then
        ...
        !
        ! For pseudo: project dt from elements to nodes and store it in vdiag_sld (locally for each slave)
        !
        call sld_elenod(PROJECT,pnode,pgaus,ielem,gpvol,elmar(pelty)%shape)
        ...
        if ((( coupling('SOLIDZ','EXMEDI') >= 1 ) .or. ( coupling('EXMEDI','SOLIDZ') >= 1 )) &
           ...
           call matrix_assemble_element_RHS(1_ip,1_ip,pnode,lnods(:,ielem),troponin_temp, &
                troponin)
        end if
