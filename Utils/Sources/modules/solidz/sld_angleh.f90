subroutine sld_angleh(&
  ...
  use def_solidz, only       :  lawco_sld,parco_sld,kfl_serei_sld,parsp_sld
  use def_master, only       :  kfl_modul,ID_EXMEDI,cutim,ittim,postp,ittyp,ITASK_ENDRUN
  ...
  real(rp)                   :: xcord,ycord,zcord,vuni1(3),uuni1(3),vuni2(3),bidon

  real(rp)                   :: lenght_pnt,angle1,omega,a_epi,b_epi,yp_epi,zp_epi,lenght_end,l_star,&
       rotma(3,3),nfibe0T(ndime,pgaus),lenght_epi,yp_end,zp_end,a_end,b_end,nore1,rcord,&
       b_qua,d_qua,c_end,a_hyp,nu,mu_end,mu_epi,mu,helix,x_endo,y_endo,z_endo,phi   

  integer(ip)                :: idime,jdime,kdime,nnode,pelty,inode,ipoin
