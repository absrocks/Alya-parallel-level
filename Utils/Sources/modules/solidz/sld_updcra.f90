...
subroutine sld_updcra()
  ...
  use mod_cutele
  implicit none
  integer(ip)       :: nctip,ifacg,ielem,pmate,idime,iface,ielty,pface,ipoin,inode,jdime,pnode
  ...
  if (kfl_xfeme_sld > 0) then
     ...
     do while (nctip > 0_ip) 
        ...
        !
        call parari('MAX',0_ip,1_ip,nctip)
...
!           end do
     ...
     !
     call parari('MAX',0_ip,1_ip,ncrak_sld)
     if (ncrak_sld == 0) then
        if (INOTMASTER) then
           call sld_elmope(6_ip)                     ! get the position and normal of new element crack
        else
        ...
        call pararr('MAX',0_ip,1_ip,sgult_sld)
        if (INOTMASTER) then
           pmate = 1
           if( nmate > 1 ) then
