...
module mod_sld_stress_model_comput
  ...
  !> @details Transport operators for stress measure
  !>
  !>          Pull-back:    {S}_ij     = J F_{ik}^{-1} sigma_{kl} F_{lj}^{-T}
  !>          Push-forward: {sigma}_ij = J^{-1} F_{iI}^{-1} S_{IJ} F_{jJ}^{-T}
  !>
  !> @warning
  !>          In Oliver: {S}_ij = J F_{ik}^{-1} sigma_{kl} F_{lj}^{-T}
  !>          In Beyly:  {S}_IJ = J F_{iI}^{-1} sigma_{ij} F_{jJ}^{-T}
  !>
  !------------------------------------------------------------------------------
    ...
    case(0_ip)
       ...
       do I = 1, ndofn
          do J = 1, ndofn
             do idime = 1, ndofn
                do jdime = 1, ndofn
                   stress(I,J) = stress(I,J) + detF*Finv(idime,I)*sigma(idime,jdime)*Finv(J,jdime)
                end do
             end do
          end do
       end do
    ...
    case(1_ip)
       ...
       do idime = 1, ndofn
          do jdime = 1, ndofn
             do I = 1, ndofn
                do J = 1, ndofn
                   sigma(idime,jdime) = sigma(idime,jdime) + detF*F(idime,I)*stress(I,J)*F(J,jdime)
                end do
             end do
          end do
       end do
  ...
  !>                                                    +-                -+   +-             -+
  !------------------------------------------------------------------------------
  subroutine SM_tensor_to_voigt_second(ndofn, task, tensor, voigt)
  ...
  subroutine SM_rotate_basis_creation(ielem, gpgdi, rotmat)
    ...
    rotrig = 0.0_rp
    auxM1  = 0.0_rp
    !call sld_polar_decomposition(gpgdi(:,:), rotrig(:,:), auxM1(:,:))
    rotrig = 0.0_rp
    do i = 1, 3
      rotrig(i,i) = 1.0_rp
    enddo
    ...
    !
    rotmcs = 0.0_rp
    if ( kfl_fiber_sld == 4_ip .or. &
         kfl_fiber_sld == 5_ip .or. &
         kfl_fiber_sld == 6_ip .or. &
         kfl_fiber_sld == 7_ip ) then
       do i = 1, 3
  ...
  !           the right stretch tensor, U.
  !------------------------------------------------------------------------------
  subroutine SM_polar_decomposition(F, R, U)
    ...
    do i = 1,3
      do j = 1,3
         do k = 1,3
           tensor(i,j) = tensor(i,j) + F(k,i)*F(k,j)
         enddo
      enddo
    enddo
    !
    ...
    !
    call spcdec(tensor, omega(:), eigvec(:,:), NROT, 1_ip,'MOD_SLD_STRESS_MODEL_COMPUT')
    !
    ...
    !
    call maths_invert_matrix(3_ip, U, detU, Uinv)
    !
    ! Calculate R = F*U^{-1}
    !
    do i = 1,3
