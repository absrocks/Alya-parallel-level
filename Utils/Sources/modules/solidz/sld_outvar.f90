...
subroutine sld_outvar(ivari)
  ...
  use def_domain,          only: ndime,nelem,npoin,ngaus,nboun,neset
  ...
  use def_solidz,          only: green_sld, lepsi_sld
  ...
  use mod_sld_commdom,     only: commdom_sld_nominal_stress_dot_n, commdom_sld_trace_sigma, commdom_sld_n_sigma_n !< 2015JUL17
     ...
     if ( INOTMASTER ) then
        ...
        do ipoin = 1,npoin
           if ( lpoty(ipoin) > 0 ) then
              gesca(ipoin) = real(kfl_fixrs_sld(lpoty(ipoin)),rp)
           else
              gesca(ipoin) = 0.0_rp
           end if
        end do
  ...
  case ( 71_ip )
     ...
     if ( INOTMASTER ) then
        ...
        do ipoin = 1, npoin
           ibopo = lpoty(ipoin)
           if ( ibopo > 0 ) then
              gevec(1:ndime,ipoin) = jacrot_du_dq_sld(1:ndime,1,ibopo)
           else
  ...
  case ( 72_ip )
     ...
     if ( INOTMASTER ) then
        ...
        do ipoin = 1, npoin
           ibopo = lpoty(ipoin)
           if ( ibopo > 0 ) then
              gevec(1:ndime,ipoin) = jacrot_du_dq_sld(1:ndime,2,ibopo)
           else
              gevec(1:ndime,ipoin) = 0.0_rp
           end if
        end do
  ...
  case ( 73_ip )
     ...
     if ( INOTMASTER .and. ndime == 3_ip ) then
        ...
        do ipoin = 1, npoin
           ibopo = lpoty(ipoin)
           if ( ibopo > 0 ) then
              gevec(1:ndime,ipoin) = jacrot_du_dq_sld(1:ndime,3,ibopo)
           else
              gevec(1:ndime,ipoin) = 0.0_rp
           end if
        end do
