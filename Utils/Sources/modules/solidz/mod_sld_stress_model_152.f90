...
module mod_sld_stress_model_152
  ...
  subroutine sm152_model_properties( &
       ...
       sLC1, hLC2, sLC2, rLCc, hG1, sG1, h61, s61, a11, a22, b11, b22, dM, length, A, B,        &
       flaVi, eta)
    ...
    !
    real(rp), intent(in)                               :: &
         properties(31),                                  & !< Properties vector
    ...
    real(rp), intent(out)                              :: &
         ...
         hG1, sG1, h61, s61,                              &
         a11, a22, b11, b22, dM,                          & ! Hygrothermal
    ...
    sLT2 = ((1.0_rp + fgT*(fxT - 1.0_rp))*fxT*xT)/(1.0_rp + fgT*(fxT**2.0_rp - 1.0_rp))
    ...
    sLC2 = ((1.0_rp + fgC*(fxC - 1.0_rp))*fxC*xC)/(1.0_rp + fgC*(fxC**2.0_rp - 1.0_rp))
    ...
    rLCc = -((sLC1 - sLC2)*E11 + (hLC1*sLC2 - hLC2*sLC1)*length)/((hLC1 - hLC2)*length*sLC1)*(1.0_rp  &
         /aux2)*(sqrt(aux2**2.0_rp - 4.0_rp*v12*v21*aux2*aux1+ (nTQ + 4.0_rp*v12**2.0_rp)*(v21         &
         *aux1)**2.0_rp) - nT*v21*aux1)
    !
    g1T  = properties(23)
    g2T  = properties(24)
    sG1  = sT
  ...
  subroutine sm152_stiffness_tensor( &
    ...
    stiff(1,1) = (E11**2.0_rp)*(1.0_rp - d1)/auxS1
  ...
  subroutine sm152_loading_functions( &
    ...
    if (s1 > supZer) then
       ...
       ! compression
       phiLT = 0.0_rp
       phiLC = (sqrt(s1**2.0_rp + nTQ*pT**2.0_rp + nSQ*tL**2.0_rp) + nT*pT  + nS*tL)/xC
    else
       !
       phiLT = 0.0_rp
       phiLC = 0.0_rp
    ...
    ! - Transversal
    phiT = sqrt(((yT + yC)/(yT*yC))**2.0_rp*(tT**2.0_rp + aT*pT**2.0_rp)/(1.0_rp + aT)+ (bT*tL/sL) &
         **2.0_rp) + ((yC - yT)*pT)/(yT*yC) + (1.0_rp - bT)*tL/sL
    !
    ! -----------------------------------------------------------------------------------------
  ...
  subroutine sm152_damage_state( &
    ...
    elseif (s1 < infZer .and. rLC > supOne) then
       ...
       aux2 = 2.0_rp*v12*v21 + v23 - 1.0_rp
       aux3 = sLC1/sLC
       aux4 = (nT*v21 + aux1*aux2*aux3*rLC)**2.0_rp - (4.0_rp*(v12**2.0_rp) + nTQ)*(v21**2.0_rp)
       aux5 = 2.0_rp*aux2*(2.0_rp*v12*v21 - (nT*v21 + aux1*aux2*aux3*rLC)*aux3*rLC)
       d1 = min(dlimi, (-aux5 - sqrt((aux5**2.0_rp) - 4.0_rp*aux4*((aux2**2.0_rp)*((aux3*rLC)    &
            **2.0_rp - 1.0_rp))))/(2.0_rp*aux4))
    end if
  ...
  subroutine sm152_tangent_analytical( &
    ...
    tT = 0.5_rp*sqrt((streff(2) - streff(3))**2.0_rp + 4.0_rp*streff(4)**2.0_rp)
    if (abs(tT) < supZer) then
       tT = 0.0_rp
    end if
    tL = sqrt(streff(5)**2.0_rp + streff(6)**2.0_rp)
    ...
    if (flaLT) then
       if (d1t < 1.0_rp) then
          ...
          dddr = (E11*sLT)/(xT*(E11 - hLT*length)*(rLT**2.0_rp))
          ! coef = (s11*dH/dd1)
          coef = strnom(1)/(E11*((1.0_rp - d1t)**2.0_rp))
       ...
       if (d1t < 1.0_rp) then
          ! drds = drL/dseff
          aux1 = sqrt((streff(1)**2.0_rp) + nTQ*(pT**2.0_rp) + nSQ*(tL**2.0_rp))
          drds(:) = 0.0_rp
          if (aux1 > supZer) then
          ...
          aux4 = (nT*v21 + aux1*aux2*aux3*rLC)**2.0_rp - (4.0_rp*v12**2.0_rp + nTQ)*(v21**2.0_rp)
          aux5 = 2.0_rp*aux2*(2.0_rp*v12*v21 - (nT*v21 + aux1*aux2*aux3*rLC)*aux3*rLC)
          aux6 = (aux2**2.0_rp)*((aux3*rLC)**2.0_rp - 1.0_rp)
          aux7 = 2.0_rp*aux1*aux2*aux3*(aux1*aux2*aux3*rLC + nT*v21)
          aux8 = -2.0_rp*aux2*aux3*(2.0_rp*aux1*aux2*aux3*rLC + nT*v21)
          aux9 = 2.0_rp*((aux2*aux3)**2.0_rp)*rLC
          dddr = (aux4*(-aux8 + (2.0_rp*aux6*aux7 - aux5*aux8 + 2.0_rp*aux4*aux9)/sqrt(aux5**2.0_rp &
               - 4.0_rp*aux4*aux6)) + (aux5 + sqrt(aux5**2.0_rp - 4.0_rp*aux4*aux6))*aux7)/(2.0_rp   &
               *aux4**2.0_rp)
          coef = strnom(1)/(E11*((1.0_rp - d1t)**2.0_rp))
          ! M(1,:) = (s11*dH/dd1)*(dd1/drL)*(drL/dseff)*(dseff/de)
          M(1,:) = coef*dddr*drde(:)
    ...
    if (flaT) then
       ! drds = drT/dsef
       aux1 = ((yT + yC)/(yT*yC))**2.0_rp
       aux2 = (yC - yT)/(yT*yC)
       aux3 = sqrt(aux1*((tT**2.0_rp) + aT*(pT**2.0_rp))/(1.0_rp + aT) + (bT*tL &
            /sL)**2.0_rp)
       drds(:) = 0.0_rp
       if (aux3 > supZer) then
          ...
          if (abs(streff(5)) > supZer) then
             drds(5) = ((bT**2.0_rp)/(sL*aux3) + ((1.0_rp - bT)/tL))*(streff(5)/sL)
          end if
          if (abs(streff(6)) > supZer) then
             drds(6) = ((bT**2.0_rp)/(sL*aux3) + ((1.0_rp - bT)/tL))*(streff(6)/sL)
          end if
       end if
       ! drde = (drL/dseff)*(dseff/de)
       drde(:) = matmul(drds(:),stieff(:,:))
       ...
       if (pT > 0.0_rp .and. dGt < 1.0_rp) then
          ...
          hG = hG1
          sG = sG1
          dddr = (EGT*sG)/(sG1*(EGT - hG*length)*(rT**2.0_rp))
          ! coef = (s22 - s33)*(dH/ddG)
          coef = (strnom(2) - strnom(3))/(4.0_rp*EGT*((1.0_rp - dGt)**2.0_rp))
          ! aux1 = coef*ddr
          aux1 = coef*dddr
          !
          ! coef = (s33 + s22)*(dH/ddG)
          coef = (strnom(3) - strnom(2))/(4.0_rp*EGT*((1.0_rp - dGt)**2.0_rp))
          ! aux1 = coef*dddr
          aux2 = coef*dddr
       else
          aux1 = 0.0_rp
       ...
       if (dKt < 1.0_rp) then
          ...
          dddr = ((A + rT)*(A + B*(rT - 1.0_rp) + 1.0_rp)*(EGT*sG)/(sG1*(EGT - hG*length)    &
               *(rT**2.0_rp)) + (A + 1.0_rp)*(B - 1.0_rp)*dGt - (A + 1.0_rp)*(B - 1.0_rp))/((A   &
               + rT)**2.0_rp)
          ! coef = (s22 + s33)*(dH/ddK)
          coef = (strnom(2) + strnom(3))/(4.0_rp*EKT*((1.0_rp - dKt)**2.0_rp))
          ! aux 3 = coef*dddr
          aux3 = coef*dddr
          ! coef = (s22 + s33)*(dH/ddK)
          coef = (strnom(2) + strnom(3))/(4.0_rp*EKT*((1.0_rp - dKt)**2.0_rp))
       ...
       if (dGt < 1.0_rp) then
          ...
          dddr = (EGT*sG)/(sG1*(EGT - hG*length)*(rT**2.0_rp))
          ! coef = (s23*dH/ddG)
          coef = strnom(4)/(EGT*((1.0_rp - dGt)**2.0_rp))
       ...
       if (d6t < 1.0_rp) then
          ...
          h6 = h61
          s6 = s61
          dddr = (G12*s6)/(s61*(G12 - h6*length)*(rT**2.0_rp))
          !
          ! coef = (s12*dH/dd6)
          coef = strnom(5)/(G12*((1.0_rp - d6t)**2.0_rp))
          ! M(5,:) = (s12*dH/dd6)*(dd6/drT)*(drL/dseff)*(dseff/de)
          M(5,:) = coef*dddr*drde(:)
          !
          ! coef = (s13*dH/dd6)
          coef = strnom(6)/(G12*((1.0_rp - d6t)**2.0_rp))
  ...
  subroutine sld_stress_model_152( &
    ...
    if (ittim == 1_ip .and. itinn(modul) == 1_ip ) then
       ...
       eleng_sld(ielem) = length
    endif
    length = eleng_sld(ielem)
    !
    ! Material properties
    !
    ...
    material_points: do igaus = 1, pgaus
       ...
       !
       call SM_tensor_to_voigt_second(ndime, 0_ip, gpgre(:,:,igaus), greGlo(:))
       !
       ...
       tT = 0.5_rp*sqrt((strMatEff(2) - strMatEff(3))**2.0_rp + 4.0_rp*strMatEff(4)**2.0_rp)
       if (abs(tT) < supZer) then
          tT = 0.0_rp
       end if
       tL = sqrt(strMatEff(5)**2.0_rp + strMatEff(6)**2.0_rp)
       if (abs(tL) < supZer) then
          tL = 0.0_rp
       end if
       ...
       auxS3 = 2.0_rp*(EKT*(1.0_rp - dK) - (4.0_rp*(EKT**2.0_rp)*(v12**2.0_rp)*(d1 - 1.0_rp)  &
            *(dK - 1.0_rp)**2.0_rp)/auxS1)
       ...
       !
       auxMA33(:,:) = 0.0_rp
       call SM_tensor_to_voigt_second(ndime, 1_ip, auxMA33(:,:), strGloNom(:))
       !
