...
contains
  ...
  subroutine sm154_model_properties( &
       props, E11, E22, v12, v21, v23, G12, G23, xT, xTO, xC, xCO, yT, yC, sL, sT, nL, nT,  &
    ...
    real(rp),    intent(out)                              :: &
         ...
         xT, xTO, xC, xCO, yT, yC, sL, sT, nL, nT, alpha0,   & ! Strength
  ...
  subroutine sm154_strength_reduction( &
       E11, E22, G12, gxT, gxTO, gxC, gXCO, gyT, gyC, gsL, lenchar, xT, xTO, xC, xCO, yT,   &
       yC, sL)
    ! -----------------------------------------------------------------------------------------
    ! INIT
    ! -----------------------------------------------------------------------------------------
    ...
    if (minGA < minG) then
       lim = -10.0_rp*E11
       xC  = sqrt(-2.0_rp*(gxT + gxTO)*E11*lim/lenchar/(E11 - lim))
  ...
  subroutine sm154_strain_tensor( &
    ...
    if (flagHygro) then
       do idof = 1, 3
          greTotal(idof) = greTotal(idof) - alpha(idof)*incTemp - beta(idof)*incMois ! <AQC> Bessa said that should be only alpha*tempNew
       end do
    end if
    !
    ! PLATIC RESPONSE
  ...
  subroutine sm154_stiffness_tensor( &
       stiff, E11, E22, G12, G23, v12, v21, v23, d1, d2, d3, d4, d5, d6)
    ...
    !
    real(rp),    intent(in)                               :: &
         E11, E22, G12, G23, v12, v21, v23,                   & ! Material properties
  ...
  subroutine sm154_damage_state( &
    ...
    if (rFT > one) then
       ...
       !
       ! Damage variable
      !d1T = min(d1Max, 1.0_rp - mxT/E11 - nxT/(XT*rFT))
       d1T = min(d1Max, max(0.0_rp,1.0_rp - mxT/E11 - nxT/(XT*rFT)))
    end if
    ...
    if (rFC > one) then
       ...
       !
       ! Damage variable
      !d1C = min(d1Max, 1.0_rp - mxC/E11 + nxC*invGreEff)
       d1C = min(d1Max, max(0.0_rp, 1.0_rp - mxC/E11 + nxC*invGreEff))
    ...
    if (rMT > one) then
       ...
       !
       ! Damage variable
      !d2T = min(d2Max, 1.0_rp - myT/E22 - (2.0_rp*a1*nyT)/(E22*(a3 + a2)))
       d2T = min(d2Max, max(0.0_rp, 1.0_rp - myT/E22 - (2.0_rp*a1*nyT)/(E22*(a3 + a2))))
    end if
    !
    ! Compressive
    d2C = 0.0_rp
    if (rMC > one) then
       ...
       !
       ! Damage variable
      !d2C = min(d2Max, 1.0_rp - myC/E22 + nyC*(E22 - myC*v12*v21)/(E22*(AYC + BYC*rMC*sT)))
       d2C = min(d2Max, max(0.0_rp, 1.0_rp - myC/E22 + nyC*(E22 - myC*v12*v21)/(E22*(AYC + BYC*rMC*sT))))
    end if
    !
    ! SHEAR DIRECTION
    !
    ...
    if (rMT > one) then
       ...
       !
       ! Damage variable
      !d66 = min(d6Max, 1.0_rp - msL/G12 - nsL/(sL*rMT))
       d66 = min(d6Max, max(0.0_rp, 1.0_rp - msL/G12 - nsL/(sL*rMT)))
    end if
    !
  ...
  subroutine sm154_precalculus( &
    ...
    !
    call sm154_stiffness_tensor(stiff0_sld(:,:,imate), E11, E22, G12, G23, v12, v21, v23, &
  ...
  subroutine sld_stress_model_154( &
       pgaus, pmate, gpgdi, gpigd, gpdet, gpstr, ielem, flagImpl, gpdds)
    ...
    use def_domain, only                                :  &
         ndime
    use def_solidz, only                                :  &
         stiff0_sld, svegm_sld, parco_sld, eleng_sld,      &
    ...
    real(rp),    intent(in)                             :: &
         ...
         gpigd(ndime,ndime,pgaus),                         & ! Inverse of deformation gradient tensor
    ...
    call sm154_model_properties( &
         ...
         sT, nL, nT, alpha0, gxT, gxTO, gxC, gxCO, gyT, gyC, gsL, gG, flagPlast, sP, kP,     &
         flagVisco, eta, flagHygro, a11, a22, incrTemp, b11, b22, incrMois)
    !
    ! Characteristics element length and strength reduction properties (computed only once)
    !
    if ( ittim == 1_ip .and. itinn(modul) == 1_ip ) then

       call sm154_characteristics_length(ielem, lenchar)
       eleng_sld(ielem) = lenchar

       call sm154_strength_reduction(&
            E11, E22, G12, gxT, gxTO, gxC, gXCO, gyT, gyC, gsL, lenchar, &
            xT, xTO, xC, xCO, yT, yC, sL)

    end if
    lenchar = eleng_sld(ielem)
    !
    ...
    material_points: do igaus = 1, pgaus
       ...
       !
       call SM_tensor_to_voigt_second(3_ip, 0_ip, gpgre(:,:,igaus), greGloTot(:))
       !
       ! Rotate from the global to the material coordinate system
       !
       call SM_rotate_basis_creation(ielem, gpgdi(:,:,igaus), traMa(:,:))
       call SM_rotate_voigt_second(1_ip, traMa(:,:), greGloTot(:), greMatTot(:))
       !
       ...
       ! Stiffness tensor
       !
       call sm154_stiffness_tensor(stiMatNom(:,:), E11, E22, G12, G23, v12, v21, v23, &
            d1, d2, d3, d4, d5, d6)
       !
       ...
       !
       auxMA33(:,:) = 0.0_rp
       call SM_tensor_to_voigt_second(3_ip, 1_ip, auxMA33(:,:), strGloNom(:))
       !
