           ...
           if( ielem > 0 ) then
              ...
              if( postp(1) % npp_witne(1) == 1_ip ) then
                 do inode = 1,pnode
                    ...
                    witne(1,iwitn) = witne(1,iwitn) + shwit(inode,iwitn) *  displ(1,ipoin,1)
                 end do
              end if
              !
              ! DISPY
              ...
              if( postp(1) % npp_witne(2) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(2,iwitn) = witne(2,iwitn) + shwit(inode,iwitn) *  displ(2,ipoin,1)
                 end do
              end if
              !
              ! DISPZ
              ...
              if( postp(1) % npp_witne(3) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(3,iwitn) = witne(3,iwitn) + shwit(inode,iwitn) *  displ(ndime,ipoin,1)
                 end do
              end if
              !
              ! VELOX
              ...
              if( postp(1) % npp_witne(4) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(4,iwitn) = witne(4,iwitn) + shwit(inode,iwitn)*veloc_sld(1,ipoin,1)
                 end do
              end if
              !
              ! VELOY
              ...
              if( postp(1) % npp_witne(5) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(5,iwitn) = witne(5,iwitn) + shwit(inode,iwitn)*veloc_sld(2,ipoin,1)
                 end do
              end if
              !
              ! VELOZ
              ...
              if( postp(1) % npp_witne(6) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(6,iwitn) = witne(6,iwitn) + shwit(inode,iwitn)*veloc_sld(3,ipoin,1)
                 end do
              end if
              !
              ! ACCEX
              ...
              if( postp(1) % npp_witne(7) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(7,iwitn) = witne(7,iwitn) + shwit(inode,iwitn)*accel_sld(1,ipoin,1)
                 end do
              end if
              !
              ! ACCEY
              ...
              if( postp(1) % npp_witne(8) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(8,iwitn) = witne(8,iwitn) + shwit(inode,iwitn)*accel_sld(2,ipoin,1)
                 end do
              end if
              !
              ! ACCEZ
              ...
              if( postp(1) % npp_witne(9) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(9,iwitn) = witne(9,iwitn) + shwit(inode,iwitn)*accel_sld(3,ipoin,1)
                 end do
              end if
              !
              ! FRXIX
              ...
              if( postp(1) % npp_witne(10) == 1_ip ) then
                 do inode = 1,pnode
                    ...
                    witne(10,iwitn) = witne(10,iwitn) + shwit(inode,iwitn)*rfaux
                 end do
              end if
              !
              ! FRXIY
              ...
              if( postp(1) % npp_witne(11) == 1_ip ) then
                 do inode = 1,pnode
                    ...
                    rfaux = -frxid_sld(idofn) ! opposite sign
                    witne(11,iwitn) = witne(11,iwitn) + shwit(inode,iwitn)*rfaux
                 end do
              end if
              !
              ! FRXIZ
              ...
              if( postp(1) % npp_witne(12) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ...
                    rfaux = -frxid_sld(idofn) ! opposite sign
                    witne(12,iwitn) = witne(12,iwitn) + shwit(inode,iwitn)*rfaux
                 end do
              end if
              !
              ! SIGXX
              ...
              if( postp(1) % npp_witne(13) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(13,iwitn) = witne(13,iwitn) + shwit(inode,iwitn)*caust_sld(1,ipoin)
                 end do
              end if
              !
              ! SIGYY
              ...
              if( postp(1) % npp_witne(14) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(14,iwitn) = witne(14,iwitn) + shwit(inode,iwitn)*caust_sld(2,ipoin)
                 end do
              end if
              !
              ! SIGZZ
              ...
              if( postp(1) % npp_witne(15) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(15,iwitn) = witne(15,iwitn) + shwit(inode,iwitn)*caust_sld(3,ipoin)
                 end do
              end if
              !
              ! SIGYZ
              ...
              if( postp(1) % npp_witne(16) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(16,iwitn) = witne(16,iwitn) + shwit(inode,iwitn)*caust_sld(4,ipoin)
                 end do
              end if
              !
              ! SIGXZ
              ...
              if( postp(1) % npp_witne(17) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(17,iwitn) = witne(17,iwitn) + shwit(inode,iwitn)*caust_sld(5,ipoin)
                 end do
              end if
              !
              ! SIGXY
              ...
              if( postp(1) % npp_witne(18) == 1_ip ) then
                 if( ndime == 2_ip ) then
                    do inode = 1,pnode
                       ipoin = lnods(inode,ielem)
                       witne(18,iwitn) = witne(18,iwitn) + shwit(inode,iwitn)*caust_sld(3,ipoin)
                    end do
                 else
                    do inode = 1,pnode
                       ipoin = lnods(inode,ielem)
                       witne(18,iwitn) = witne(18,iwitn) + shwit(inode,iwitn)*caust_sld(6,ipoin)
                    end do
                 end if
              end if
              !
              ...
              if( postp(1) % npp_witne(19) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(19,iwitn) = witne(19,iwitn) + shwit(inode,iwitn)*green_sld(1,ipoin)
                 end do
              end if
              !
              ! EPSYY
              ...
              if( postp(1) % npp_witne(20) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(20,iwitn) = witne(20,iwitn) + shwit(inode,iwitn)*green_sld(2,ipoin)
                 end do
              end if
              !
              ! EPSZZ
              ...
              if( postp(1) % npp_witne(21) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(21,iwitn) = witne(21,iwitn) + shwit(inode,iwitn)*green_sld(3,ipoin)
                 end do
              end if
              !
              ! EPSYZ
              ...
              if( postp(1) % npp_witne(22) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(22,iwitn) = witne(22,iwitn) + shwit(inode,iwitn)*green_sld(4,ipoin)
                 end do
              end if
              !
              ! EPSXZ
              ...
              if( postp(1) % npp_witne(23) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(23,iwitn) = witne(23,iwitn) + shwit(inode,iwitn)*green_sld(5,ipoin)
                 end do
              end if
              !
              ! EPSXY
              ...
              if( postp(1) % npp_witne(24) == 1_ip ) then
                 if (ndime == 2_ip) then
                    do inode = 1,pnode
                       ipoin = lnods(inode,ielem)
                       witne(24,iwitn) = witne(24,iwitn) + shwit(inode,iwitn)*green_sld(3,ipoin)
                    end do
                 else
                    do inode = 1,pnode
                       ipoin = lnods(inode,ielem)
                       witne(24,iwitn) = witne(24,iwitn) + shwit(inode,iwitn)*green_sld(6,ipoin)
                    end do
                 end if
              end if
              !
              ...
              if( postp(1) % npp_witne(25) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(25,iwitn) = witne(25,iwitn) + shwit(inode,iwitn)*lepsi_sld(1,ipoin)
                 end do
              end if
              !
              ! LEPYY
              ...
              if( postp(1) % npp_witne(26) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(26,iwitn) = witne(26,iwitn) + shwit(inode,iwitn)*lepsi_sld(2,ipoin)
                 end do
              end if
              !
              ! LEPZZ
              ...
              if( postp(1) % npp_witne(27) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(27,iwitn) = witne(27,iwitn) + shwit(inode,iwitn)*lepsi_sld(3,ipoin)
                 end do
              end if
              !
              ! LEPYZ
              ...
              if( postp(1) % npp_witne(28) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(28,iwitn) = witne(28,iwitn) + shwit(inode,iwitn)*lepsi_sld(4,ipoin)
                 end do
              end if
              !
              ! LEPXZ
              ...
              if( postp(1) % npp_witne(29) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(29,iwitn) = witne(29,iwitn) + shwit(inode,iwitn)*lepsi_sld(5,ipoin)
                 end do
              end if
              !
              ! LEPXY
              ...
              if( postp(1) % npp_witne(30) == 1_ip ) then
                 if (ndime == 2_ip) then
                    do inode = 1,pnode
                       ipoin = lnods(inode,ielem)
                       witne(30,iwitn) = witne(30,iwitn) + shwit(inode,iwitn)*lepsi_sld(3,ipoin)
                    end do
                 else
                    do inode = 1,pnode
                       ipoin = lnods(inode,ielem)
                       witne(30,iwitn) = witne(30,iwitn) + shwit(inode,iwitn)*lepsi_sld(6,ipoin)
                    end do
                 end if
              end if
              !
              ...
              if( postp(1) % npp_witne(31) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(31,iwitn) = witne(31,iwitn) + shwit(inode,iwitn)*coord(1,ipoin)
                 end do
              end if
              !
              ! COORY
              ...
              if( postp(1) % npp_witne(32) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(32,iwitn) = witne(32,iwitn) + shwit(inode,iwitn)*coord(2,ipoin)
                 end do
              end if
              !
              ! COORZ
              ...
              if( postp(1) % npp_witne(33) == 1_ip .and. ndime == 3_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(33,iwitn) = witne(33,iwitn) + shwit(inode,iwitn)*coord(3,ipoin)
                 end do
              end if
              !
              ! SEQVM
              ...
              if( postp(1) % npp_witne(34) == 1_ip ) then
                 do inode = 1,pnode
                    ipoin = lnods(inode,ielem)
                    witne(34,iwitn) = witne(34,iwitn) + shwit(inode,iwitn)*seqvm_sld(ipoin)
                 end do
              end if
              !
              ! SIGFI
              ...
              if( postp(1) % npp_witne(35) == 1_ip ) then
                 do inode = 1,pnode
                    ...
                    witne(35,iwitn) = witne(35,iwitn) + shwit(inode,iwitn) * stfib
                 end do
              end if
              !
              ! MICRO
