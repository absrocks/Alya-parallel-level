subroutine chm_elmgac_species(&
     itask,ielem,pnode,lnods,elden,elcod,elcon,elco2,elvel,eltem,elDik,elmas,&
     elmol,elmut,elsen)
  !------------------------------------------------------------------------
  !****f* chemic/chm_elmgac_species
  ! NAME 
  !    chm_elmgac_species
  ! DESCRIPTION
  !    Gather operations for the combustion model based on kinetics
  ! USES
  ! USED BY
  !    chm_elmcom
  !***
  ...
  if (itask >= 1_ip) then  ! Everybody calls this, elcon(1), elcod, elmas and eldik
     ...
     do ispec = 1, nspec_chm
        ...
        case (3_ip) ! Uniform Lewis and Prandtl numbers
           call ker_proper('VISCO','PNODE',dummi,ielem,elvis,pnode)
           do inode = 1,pnode
...
end subroutine chm_elmgac_species
