subroutine got_elmsm2(&
     pnode,pgaus,pevat,ndofr,gpgvd,gpvdr,gpst1,gprhs,resim,&
     gpvol,gpcar,elvdr,dtinv_got,chale,elmat)
  !-----------------------------------------------------------------------
  !****f* Gotita/got_elmsm2
  ! NAME
  !   got_elmsm2
  ...
  use def_gotita, only       :  kfl_shocm_got,shock_got,kfl_linea_got,&
       &                        kfl_timem_got,kfl_weigm_got
  implicit none  
  integer(ip), intent(in)    :: pnode,pgaus,pevat,ndofr
  real(rp),    intent(in)    :: gpgvd(ndime,ndime,pgaus)
  real(rp),    intent(in)    :: gpvdr(ndime,pgaus,2)
  real(rp),    intent(in)    :: gpst1(pgaus)
  real(rp),    intent(in)    :: gprhs(ndime,pgaus)
  real(rp),    intent(in)    :: resim(ndofr,pnode,pgaus)
  real(rp),    intent(in)    :: gpvol(pgaus)
  real(rp),    intent(in)    :: gpcar(ndime,mnode,pgaus)
  real(rp),    intent(in)    :: elvdr(ndime,pnode)
  ...
  do igaus=1,pgaus
     ...
     do mdime=1,ndime
        ...
        if(kfl_linea_got==1) then
           do inode=1,pnode
              gpres=gpres-resim(1,inode,igaus)*elvdr(mdime,inode)
           end do
        else
           do inode=1,pnode
              do idime=1,ndime
                 idofn=(mdime-1)*ndime+idime
                 gpres=gpres-resim(idofn,inode,igaus)*elvdr(idime,inode)
              end do
           end do
        end if
        if(kfl_timem_got==1.and.kfl_weigm_got==0)&
             gpres=gpres-dtinv_got*(gpvdr(mdime,igaus,1)-gpvdr(mdime,igaus,2))
        gpres=abs(gpres)
        !
        ...
        if(gpve2>0.0_rp.and.grnor>1.0e-6) then
           ...
           if(kfl_shocm_got==1) then                       ! Isotropic SC:
              ...
              sldif=max(0.0_rp,scdif-gpst1(igaus)*gpve2)   ! sldif = <kiso-k'> = max(0,kiso-tau_1*u^2)
           end if
           ...
           do inode=1,pnode
              do jnode=1,pnode 
                 ...
                 idofn=(inode-1)*ndime+mdime
                 jdofn=(jnode-1)*ndime+mdime
