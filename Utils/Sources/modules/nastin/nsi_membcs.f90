...
subroutine nsi_membcs(itask)
  ...
  case( 1_ip )
     ...
     if( INOTMASTER ) then
        ...
        call memory_alloca( mem_modul(1:2,modul),'KFL_FIXRS_NSI','nsi_membcs',kfl_fixrs_nsi,nbopo           )   ! Local axes
        ...
        if( kfl_conbc_nsi == 1 ) then 
           ...
           call memory_alloca( mem_modul(1:2,modul),'BVNAT_NSI'    ,'nsi_membcs',bvnat_nsi,4_ip,nboun,1_ip  )   ! Natural condition values
           ...
           call memory_alloca( mem_modul(1:2,modul),'BVNAT_NSI'    ,'nsi_membcs',bvnat_nsi,4_ip,nboun,2_ip  )
     ...
     call memory_alloca_min( kfl_fixno_nsi )
     call memory_alloca_min( kfl_fixbo_nsi )
     call memory_alloca_min( kfl_fixrs_nsi )
     call memory_alloca_min( kfl_fixpr_nsi )
     call memory_alloca_min( kfl_fixpp_nsi )
     call memory_alloca_min( kfl_wlawf_nsi )
     call memory_alloca_min( bvess_nsi     )
     call memory_alloca_min( bvnat_nsi     )
     call memory_alloca_min( bpess_nsi     )
