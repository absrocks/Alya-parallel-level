subroutine nsi_matrix()
  ...
  use def_kermod,                      only : wallcoupling_waexl,kfl_waexl_ker,velel_ker
  ...
  if( INOTMASTER ) bupor_nsi = 0.0_rp 
  porfo_nsi = 0.0_rp 
  ...
  if( INOTMASTER ) then
     ...
     if( NSI_MONOLITHIC .or. kfl_grad_div_nsi /= 0 ) then
        if( kfl_tisch_nsi==3 .or. kfl_tisch_nsi==4) then
           rhsid(1:(ndime+1)*npoin) = 0.0_rp                 ! Should be ndime+1  or ndime -- rethink 
        else
     ...
     if(  kfl_predi_nsi == 7 .or. &
        ...
        dt_rho_nsi = 0.0_rp                        ! rho / dt
        mu_rho_nsi = 0.0_rp                        ! mu / rho
        tau_nsi    = 0.0_rp                        ! rho / tau
     end if
  end if
  !
  ...
  !-----------------------------------------------------------------
  !
  ! Matrix, preconditioner, RHS  and projections element assemblies
  !
  !-----------------------------------------------------------------
  ...
  if( kfl_predi_nsi == 7 .or. kfl_predi_nsi == 8 .or. kfl_predi_nsi == 9 ) then
     ...
     call solver_lumped_mass_system(1_ip,mu_rho_nsi)
  ...
  if( kfl_matdi_nsi == NSI_DIRICHLET_MATRIX .and. INOTMASTER ) then
     call nsi_dirichlet_global_system(&
          amatr(poaup_nsi:),amatr(poapu_nsi:),lapla_nsi,rhsid,&
          rhsid(ndbgs_nsi+1:),unkno,unkno(ndbgs_nsi+1:),&
          amatr(poauu_nsi:),amatr(poapp_nsi:))
  else if( kfl_matdi_nsi == NSI_DIRICHLET_ALGORITHM .and. INOTMASTER .and. kfl_grad_div_nsi == 0 ) then
     call nsi_dirichlet_laplacian(lapla_nsi,rhsid)
  end if
  ...
  if( kfl_stabi_nsi == NSI_ALGEBRAIC_SPLIT_OSS .and. INOTMASTER ) &
