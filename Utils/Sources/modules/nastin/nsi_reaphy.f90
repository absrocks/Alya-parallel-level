...
!-----------------------------------------------------------------------
 ...
 !.md<sec>
  ...
  if( INOTSLAVE ) then
     ...
     nbtdt_nsi     = 0.0_rp                                    ! time-step size of the turbulent inlet database
        ...
        if(words(1) == 'PROBL' ) then
           ...
           do while(words(1)/='ENDPR')
              ...
              if(words(1)=='TEMPO') then          
                 ...
                 !--> </inputLine>                         
                 !
                 !.md<2>CONVECTIVE_TERM:      ON | OFF                                                              $ Existence of convective term    
                 ...
                 !-->      <inputLineHelp>Turbulence Model definition. For LES_MODEL, a real number is required. For smagorinsky, From Turbul options,
                 !--->        Nastin uses the turbulent viscosity computed by Turbul.</inputLineHelp>
                 !-->      <inputElement>
                 !-->          <inputElementType>combo</inputElementType>
                 !-->          <item>
                 ...
                 !--></inputLine>              
                 !
                 !.md<2>TURBULENCE_MODEL:     LES_MODEL [SMAGORINSKY | WALE , PARAMETER=real] | FROM_TURBUL         $ Turbulence model
                 !.md<field>TURBULENCE_MODEL
                 !.md<com>Turbulence Model definition.
                 !.md<com>For LES_MODEL, a real number is required. For smagorinsky, 
                 !.md<com>From Turbul options, Nastin uses the turbulent viscosity computed by Turbul.
                 !
                 if(words(2)=='LESMO') then
                    if(exists('SMAGO')) then
                       ...
                       kfl_cotur_nsi=-4
                       turbu_nsi=getrea('PARAM',0.07_rp,'#Coefficient c**2')   ! The default for Vreman is c^**2 = 0.07
                    end if
                 else if(words(2)=='RANSA') then
                    if(exists('MIXIN')) then
                       kfl_cotur_nsi=-10
                       turbu_nsi=getrea('PARAM',26.0_rp,'#Coefficient C L^2')  ! The default for the dumping constant is A = 26
                 ...
                 !--> </inputLine> 
                 !
                 !.md<2>FORCE_TERM:           OFF | MATERIAL                                                        $ Force term
                 !.md<3>MATERIALS=int1, MODEL=WAKE, PARAMETERS=real1,real2,...                                  $ material force model and parameters 
                 !.md<3>...
                 !.md<2>END_FORCE_TERM
                 !.md<field>FORCE_TERM
                 !.md<com>int1 is the material number. char1 is the force model (only WAKE model is available).
                 ...
                 !
                 !.md<1>BOUNDARY_NODES = int                           $ Number of nodes on the boundary
                 !.md<field>BOUNDARY_NODES
                 !.md<com>This number if the number of boundary nodes. It is used for space and time dependent boundary
                 !.md<com>conditions defined by an input file.
                 !
                 kfl_bnods_nsi = 1_ip
                 ...
                 !
                 !.md<1>VALUES_ON_BOUNDARY = int1, real1                $ Number of nodes on the boundary
                 !.md<field>VALUES_ON_BOUNDARY
                 !.md<com>This are the values for the space and time dependent boundary
                 !.md<com>conditions defined by an input file.
                 !
                 if (kfl_bnods_nsi /= 1_ip) call runend('NSI_REAPHY: FOR THE VALUES_ON_BOUNDARY OPTION THE BOUNDARY_NODES ARE REQUIRED AS WELL')
                 nbval_nsi = nbnod_nsi
                 nbtim_nsi = int(param(1_ip),ip)
                 nbtdt_nsi = param(2_ip)
