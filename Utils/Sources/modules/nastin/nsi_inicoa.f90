...
subroutine nsi_solcoa(Auu,Aup,Apu,App,bu,bp)
  ...
  integer(ip)             :: mgrou,ipoin1,ii,ngrop,ibopo,jj,jbopo,ifixx,jfixx
  ...
  call livinf(0_ip,'NASTIN: SOLVE COARSE GRID SYSTEM',0_ip)
  ndofn = ndime + 1
  ngrou = ngrou_dom 
  !
  ! Parallel exchange
  ...
  !
  if( INOTMASTER ) call nsi_rotsch(1_ip,veloc(:,:,nprev_nsi))
  !
  ! Define groups
  !
  if( INOTMASTER ) then
  ...
  call parari('SUM',0_ip,ngrou,gigro)
  jgrou = 0
  do igrou = 1,ngrou
  ...
  call parari('MIN',1_ip,ndofn*ngrou+1,iskyl)
  nskyl    =  1
  iskyl(1) =  1
  !
  ! Skyline: velocity
  ...
  call pararr('SUM',0_ip,nskyl,Ac)
  !
  ! Check matrix can be inverted
  !
  call parari('SUM',0_ip,ngrou*ndofn,kmatr)
  do igrou1 = 1,ngrou*ndofn
  ...
  if( INOTMASTER ) then
     ...
     !
     call nsi_auuvec(1_ip,Auu,veloc(:,:,nprev_nsi),ww)
     call nsi_aupvec(2_ip,Aup,press(:,nprev_nsi),ww)
     do ipoin = 1,npoin
        ii = ( ipoin - 1 ) * ndime
        jj = ( ipoin - 1 ) * ndofn
        do idime = 1,ndime
     ...
     !
     call nsi_apuvec(1_ip,Apu,veloc(:,:,nprev_nsi),ww)
     call nsi_appvec(2_ip,App,press(:,nprev_nsi),ww)
     do ipoin = 1,npoin
        jj     = ipoin * ndofn
  ...
  call pararr('SUM',0_ip,ndofn*ngrou,mu)
  ...
  call parari('SUM',0_ip,1_ip,info)
  ...
  if( INOTMASTER ) then
     ...
     do ipoin = 1,npoin
        ...
        do idime = 1,ndime
           idofn = idofn + 1
           if( kfl_fixno_nsi(idime,ipoin) == 0 ) then
              veloc(idime,ipoin,nprev_nsi) = veloc(idime,ipoin,nprev_nsi) + alpha * rr(idofn)
           end if
        end do
        idofn = idofn + 1
        press(ipoin,nprev_nsi) = press(ipoin,nprev_nsi) + alpha * rr(idofn)
     end do
  end if
  !
  ! Rotate local to global
  !
  if( INOTMASTER ) call nsi_rotsch(2_ip,veloc(:,:,nprev_nsi))
  !
  ! Deallocate memory
  !
  if( INOTMASTER ) then
