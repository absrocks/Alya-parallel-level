...
contains
  ...
  subroutine nsi_assembly_fractional_step(&
    ...
    use def_kintyp, only       :  ip,rp
    use def_domain, only       :  nzdom,ndime,r_dom,c_dom,r_sol
    use def_domain, only       :  r_sym,c_sym,nzsym,c_sol,lezdo
  ...
  !>          MU_RHO_NSI = Projection of mu / rho
  ...
  subroutine nsi_assembly_dt_rho_tau_nu(&
    ...
    real(rp),    intent(inout), pointer :: mu_rho_nsi(:)
...
!-----------------------------------------------------------------------
  ...
  !> @author  Herbert Owen
  !> @date    8/10/2018
  !> @brief   velocity rhs assemble
  !> @details needed to output force made by porosity
  !>
  !-----------------------------------------------------------------------

  subroutine nsi_assembly_vel_rhs(&
       pnode,pevat,list_elements,lnods,elporrbu,bupor_nsi)

    use def_kintyp, only       :  ip,rp
    use def_domain, only       :  ndime
    use def_nastin, only       :  porfo_nsi
    ...
    integer(ip), intent(in)              :: list_elements(VECTOR_SIZE)
    integer(ip), intent(in)              :: lnods(VECTOR_SIZE,pnode)
    real(rp),    intent(inout)           :: elporrbu(VECTOR_SIZE,ndime,pnode)
    real(rp),    intent(inout)           :: bupor_nsi(ndime,*)

    integer(ip)                          :: inode,ielem,ivect
    integer(ip)                          :: idime,ipoin
    
    !
    ! RHS porosity contribution - used to output force due to porosity
    !
    do ivect = 1,VECTOR_SIZE
       ielem = list_elements(ivect)
       if( ielem > 0 ) then
          do inode = 1,pnode
             ...
             do idime = 1,ndime
                ...
                !$OMP ATOMIC
#endif
                bupor_nsi(idime,ipoin) = bupor_nsi(idime,ipoin) + elporrbu(ivect,idime,inode)
                porfo_nsi(idime) = porfo_nsi(idime) +  elporrbu(ivect,idime,inode)
             end do
          end do
       end if
    end do

  end subroutine nsi_assembly_vel_rhs
