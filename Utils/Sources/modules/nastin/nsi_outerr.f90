subroutine nsi_outerr()
  ...
  implicit none
  integer(ip)    :: ierro=0,iwarn=0,iboun,ipoin,idime,ibopo,ipara,iaux,ielty
  integer(ip)    :: dumm1,dumm2,ivara,ivar1,ivar2
  character(200) :: wmess
  !
  ! Set postprocess
  !
  ...
  ! Grad div
  ! 
  !if( kfl_grad_div_nsi /= 0 .and. nperi /= 0 ) then
  !   ierro = ierro + 1
  !   call outfor(2_ip,momod(modul)%lun_outpu,'GRAD DIV FORMULATION DOES NOT WORK WITH PERIODICITY')     
  !end if
  ...
  if( kfl_noslw_ker == 0 ) then
     if( output_postprocess_check_variable_postprocess(101_ip) .or.   &
          output_postprocess_check_variable_postprocess(102_ip) .or.   &
          output_postprocess_check_variable_postprocess(104_ip) .or.   &
          output_postprocess_check_variable_postprocess(105_ip) .or.   &
          output_postprocess_check_variable_postprocess(106_ip) ) then
        ierro = ierro + 1
        call outfor(2_ip,momod(modul)%lun_outpu,'AVNTR, AVGTR, VAFOR, AVVAF, AND FANSW CAN ONLY BE POSTPROCESSED WITH NO SLIP WALL LAW')
     end if
  end if
  !
  ! check that postproces frequency and restart freq are idem if AVVEL is postprocessed
  !
  if( output_postprocess_check_variable_postprocess(21_ip) ) then  ! AVVEL postprocesed
     if (kfl_preli == 1) then   ! preliminary run      
        if (nprit /= postp(1) % npp_stepi(21) ) then
           print*,'nprit, postp(1) % npp_stepi(21)',nprit, postp(1) % npp_stepi(21)
           ierro = ierro + 1
           ! another option would be a warning and set nprit = postp(1) % npp_stepi(21)
           call outfor(2_ip,momod(modul)%lun_outpu,'OUTPUT and RESTART frequency must be equal if AVVEL is postprocesed' )
        end if
     end if
  end if  
  !
  ...
  if( kfl_waexl_ker == 0 .and. kfl_twola_ker == 0 .and. kfl_noslw_ker == 0 ) then
     if( output_postprocess_check_variable_postprocess(103_ip)) then
        ierro = ierro + 1
 ...
 !         'PRESSURE SHOULD BE PRESCRIBED TO ZERO' )
  ...
  !
  ! Fraction steo stabilized with TAU
  !
  ...
  !
  if ( kfl_grad_div_nsi == 0_ip .and. ( kfl_assem_nsi == 4_ip .or.  kfl_assem_nsi == 5_ip ) ) then
     ierro = ierro + 1
     call outfor(1_ip,momod(modul)%lun_outpu,&
          'GPU and GPU2 assemblies only work with GRAD_DIV ')
  end if  
