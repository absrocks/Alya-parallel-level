subroutine nsi_chkbcs()
  ...
  integer(ip)  :: pnode,pgaus,pevat,pelty,iorde
  ...
  if( kfl_rstar == 0 ) then
     ...
     !
     call nsi_rotunk(1_ip,unkno)                           ! Transform from global to local 
     call solver(rhsid,unkno,amatr,pmatr)
     call nsi_rotunk(2_ip,unkno)                           ! Transform from local to global
     !
     ! Postprocess bcs
     !
     do ipoin=1,npoin
