...
!$acc routine(frivel) seq
    ...
    integer(ip)             :: itera,pnode_max
    real(rp)                :: xmuit,fdvfr,devfr
    real(rp)                :: vkinv,diffd,parco,ypl,onovu,yplu2
...
!             include './my_frivel_include.txt'
             call frivel(delta_aux(ivect),roughness,tveno(ivect),vikin(ivect),velfr(ivect)) 
...
!$acc routine(frivel) seq
    ...
    real(rp)                :: elmat(VECTOR_DIM,mnode*ndime,mnode*ndime)     ! Element matrices
    real(rp)                :: elmap(VECTOR_DIM,mnode,mnode)
    real(rp)                :: wmatr(VECTOR_DIM,mnode*ndime,mnode*ndime)
    real(rp)                :: elrhs(VECTOR_DIM,ndime,mnode)
    real(rp)                :: wrhsi(VECTOR_DIM,mnode*ndime)

    real(rp)                :: baloc(VECTOR_DIM,ndime,ndime)                 ! Gather
    real(rp)                :: bovel(VECTOR_DIM,ndime,pnodb)
    real(rp)                :: bovfi(VECTOR_DIM,ndime,pnodb)
    real(rp)                :: bocod(VECTOR_DIM,ndime,pnodb)
    real(rp)                :: bopre(VECTOR_DIM,pnodb)
    real(rp)                :: bomut(VECTOR_DIM,pnodb)
    real(rp)                :: botem(VECTOR_DIM,pnodb)
    real(rp)                :: elmas(VECTOR_DIM,pnodb)
    real(rp)                :: elcod(VECTOR_DIM,ndime,mnode)
    real(rp)                :: elvel(VECTOR_DIM,ndime,mnode)
    real(rp)                :: elpre(VECTOR_DIM,mnode)

    real(rp)                :: gbcar(VECTOR_DIM,ndime,mnode,mgaus)
    real(rp)                :: gpcar(VECTOR_DIM,ndime,mnode,mgaus)
    real(rp)                :: xjaci(VECTOR_DIM,9),xjacm(VECTOR_DIM,9),shapp(VECTOR_DIM,mnode)
    real(rp)                :: gbsur(VECTOR_DIM)
    real(rp)                :: eucta(VECTOR_DIM)                     ! Values at Gauss points

    real(rp)                :: gbden(VECTOR_DIM,pgaub)
    real(rp)                :: gbvis(VECTOR_DIM,pgaub)
    real(rp)                :: gbpor(VECTOR_DIM,pgaub)
    real(rp)                :: gbgvi(VECTOR_DIM,ndime,pgaub)
    real(rp)                :: grvis(VECTOR_DIM,ndime,pgaub)
    real(rp)                :: produ(VECTOR_DIM)

    real(rp)                :: gbden_tmp(pgaub)
    real(rp)                :: gbvis_tmp(pgaub)

    real(rp)                :: gbtem(VECTOR_DIM,pgaub)
    real(rp)                :: gbgve(VECTOR_DIM,ndime,ndime,pgaub)
    real(rp)                :: gbpre(VECTOR_DIM,pgaub)
    real(rp)                :: gbfle(VECTOR_DIM,pgaub)
    real(rp)                :: gpvol(VECTOR_DIM,mgaus)
    real(rp)                :: chale(3),chave(3),dummr
    real(rp)                :: velex(3),veave(3)
    real(rp)                :: gpvis,ustar,tragl(9),hleng(3),kinen
    real(rp)                :: udotn,gbvel(3),roughness
    integer(ip)             :: ielem,ipoin,inode,idime,jnode,jdime     ! Indices and dimensions
    integer(ip)             :: pgaus,iboun,igaub,inodb,kdime,idofn
    integer(ip)             :: pelty,pmate,pblty,kboun,ievab,jevab
    integer(ip)             :: pevat,ievat,jevat,porde
    integer(ip)             :: dummi,iflow,jnodb
    ...
    integer(ip)             :: itera,pnode_max,iboun0(1)
    real(rp)                :: xmuit,fdvfr,devfr
    real(rp)                :: vkinv,diffd,parco,ypl,onovu,yplu2
    real(rp)                :: ypele,expye,expyt,oneoe,firsl,ypel2
    real(rp)                :: pplus,gradp,grpr2,py,sq,inv,ln4,uplus
...
#define DEF_VECT 1:VECTOR_SIZE
    ...
    ldensi = densi_ker % value_const(1)
    lvisco = visco_ker % value_const(1)
...
!!!
    ...
    do ivect = 1,VECTOR_DIM
#endif       
       gbden(DEF_VECT,:) = ldensi !densi_ker % value_const(1)
       gbvis(DEF_VECT,:) = lvisco !visco_ker % value_const(1)
...
!$acc routine(frivel) seq
    ...
    real(rp)                :: eucta,detjm                     ! Values at Gauss points
    ...
    real(rp)                :: gpvol(VECTOR_DIM,mgaus)
    real(rp)                :: chale(3),chave(3),dummr
    real(rp)                :: velex(3),veave(3),baloc_tmp(ndime,ndime)
    real(rp)                :: gpvis,ustar,tragl(9),hleng(3),kinen
    real(rp)                :: udotn,gbvel(3),roughness
    integer(ip)             :: ielem,ipoin,inode,idime,jnode,jdime     ! Indices and dimensions
    integer(ip)             :: pgaus,iboun,igaub,inodb,kdime,idofn
