...
module mod_nsi_element_operations
  ...
  use mod_nsi_assembly_global_system, only : nsi_assembly_vel_rhs
...
contains
  ...
  subroutine nsi_element_operations(&
    ...
    use def_kintyp,            only : ip,rp
    use def_master,            only : amatr,rhsid,cutim,kfl_timco,igene ! CONYO
    ...
    use def_domain,            only : ngaus,lnods,nmate
    ...
    use def_nastin,            only : mu_rho_nsi
    use def_nastin,            only : tau_nsi
    use def_nastin,            only : bupor_nsi
    ...
    use def_domain,            only : npoin   ! para bupor
    ...
    real(rp),    intent(out)         :: resis_nsi(*)
    integer(ip), intent(out)         :: itsta_nsi(*)
    real(rp),    intent(out)         :: resgs_nsi(2)
    real(rp),    intent(out)         :: tamin_nsi                    ! Minimum tau
    real(rp),    intent(out)         :: rmsgs_nsi
    real(rp),    intent(out)         :: tamax_nsi                    ! Maximum tau
    real(rp),    intent(out)         :: dtmax_nsi                    ! Maximum local time step
    ...
    real(rp)    :: elmut(VECTOR_SIZE,pnode)                          ! mut
    ...
    real(rp)    :: gpfli(VECTOR_SIZE,pgaus)                          ! phy_hyd
    ...
    call nsi_element_operations_gather(&
         ...
         elfle,elvep,elprp,elgrp,elmut,eltem,elmsh,elnor,elcur,&
         elwmean,gphyd,gpsgt,gpsgs,elbub,ellag,elavv,elibopo,elnnsw,elywal)
    ...
    if (kfl_cotur_nsi < 0) gpmut = gpden * gpmut                                             ! smago, wale, vreman etc
    ...
    call nsi_element_residual(&
         ...
         gppor,gptem,gpsgs,elvel,elpre,elvep,elprp,elgrp,ellag,    &
    ...
    if( kfl_force_nsi == 1 ) then
       call nsi_element_external_force(&
            pgaus,list_elements,lmate_loc,gpden,gprhs,gpvel)
    end if
    !
    ...
    if( itask == 4 ) then
       ...
       if( kfl_predi_nsi == 7 .or. kfl_predi_nsi == 8 .or. kfl_predi_nsi == 9 ) then
          call nsi_rhodt_rhotau_nu(                                      &
               ...
               chale,dtinv_loc,eldtrho,eltau,elmurho)
       end if
       !
       ...
       !   call nsi_assembly_vel_rhs(&
       !        pnode,pevat,list_elements,lnods,elporrbu,bupor_nsi)
          !          if( INOTMASTER )  call rhsmod(ndime,bupor_nsi)    ! le meto el not master a ver si ahora va
          ! quito el rhsmod y en nsi_bouset quito el if (ipoin <=  npoin_own)    esto deberia ir ok tambien
          ! en algun momento preguntarle a guillaume porque me daba prob el rhsmod
  ...
  subroutine nsi_element_operations_gather_scalar(&
       ...
       elgrp,elmut,eltem,elmsh,elnor,elcur,elwmean,gphyd,&
       gpsgt,gpsgs,elbub,elavv,elibopo,elnnsw,elywal)
    !-----------------------------------------------------------------------
    !****f* Nastin/nsi_elmga3
    ! NAME
    !    nsi_elmga3
    ...
    ! Mesh velocity
    !
    if( kfl_coupl(ID_NASTIN,ID_ALEFOR) /= 0 .or. associated(velom) ) then ! TESTEO
       do inode = 1,pnode
          ipoin = lnods(inode)
  ...
  subroutine nsi_element_operations_gather_vector(&
       ...
       gphyd,gpsgt,gpsgs,elbub,ellag,elavv,elibopo,elnnsw,elywal)
    ...
    real(rp),    intent(out)   :: elvel(VECTOR_SIZE,ndime,pnode,ncomp_nsi)
    ...
    real(rp),    intent(out)   :: elmut(VECTOR_SIZE,pnode)
    ...
    integer(ip)                :: inode,idime,ipoin,itime,ivect,ielem
  ...
  subroutine nsi_element_residual(                              &
    ...
    use def_master, only     :  kfl_coupl,&
         &                      wmean,ID_NASTIN,ID_ALEFOR,ID_TEMPER,   &
         &                      ID_CHEMIC,tesgs,kfl_paral,kfl_lumped,  &
         &                      prthe,dpthe,velom
    use def_kermod, only     :  thicl,gasco,kfl_prope,densi_ker,       &
         &                      kfl_adj_prob
    use def_domain, only     :  ndime,ntens,mnode,lnods,walld,nmate
    use def_nastin, only     :  kfl_cotur_nsi,grnor_nsi,gravi_nsi,     &
         ...
         &                      kfl_timei_nsi,kfl_regim_nsi,           &
         &                      lowtr_nsi,kfl_prthe_nsi,               &
         ...
         &                      kfl_penal_nsi,penal_nsi,kfl_bubbl_nsi, &
         &                      kfl_immer_nsi
    ...
    real(rp)                   :: den_time(1:VECTOR_SIZE)
    real(rp)                   :: w_time(1:VECTOR_SIZE)
    real(rp)                   :: dtinv_res(VECTOR_SIZE)
...
#define DEF_VECT 1:VECTOR_SIZE
    ...
    do ivect = 1,VECTOR_SIZE
       ...
       if( kfl_coupl(ID_NASTIN,ID_TEMPER) /= 0 .or. kfl_cotem_nsi == 1 .or. kfl_regim_nsi==3 ) then
          ...
          do igaus = 1,pgaus
             do itime = 1,nbdfp_nsi
                gptem(DEF_VECT,igaus,itime)   = 0.0_rp
                gpwmean(DEF_VECT,igaus,itime) = 0.0_rp
                do inode = 1,pnode
          ...
          if (kfl_regim_nsi == 3) then   ! loads density in last time steps
             if(associated(tesgs)) then
                ...
                do igaus = 1,pgaus
                   do itime =1,nbdfp_nsi
                      densi(DEF_VECT,igaus, itime) =  prthe(itime) * gpwmean(DEF_VECT,igaus,itime) /gasco / gptem(DEF_VECT,igaus,itime)
                   end do
                end do
             end if
          end if
       ...
       if( kfl_stabi_nsi == NSI_GALERKIN .or. kfl_stabi_nsi == NSI_ALGEBRAIC_SPLIT_OSS ) then
          ...
          !
          do igaus = 1,pgaus
             fact1(DEF_VECT) = dtinv_res(DEF_VECT) * pabdf_nsi(1) * gpden(DEF_VECT,igaus) + gppor(DEF_VECT,igaus)
             do inode = 1,pnode
       ...
       if( kfl_stabi_nsi /= NSI_SPLIT_OSS .and. kfl_stabi_nsi /= NSI_GALERKIN .and. kfl_stabi_nsi /= NSI_ALGEBRAIC_SPLIT_OSS ) then
          do itime = 2, nbdfp_nsi
             do igaus = 1,pgaus
                fact1(DEF_VECT) = gpden(DEF_VECT,igaus) * dtinv_res(DEF_VECT) * pabdf_nsi(itime)
                do idime = 1,ndime
          ...
          do igaus = 1,pgaus
             ...
             do itime = 2,nbdfp_nsi
                if( kfl_coupl(ID_TEMPER,ID_CHEMIC) /= 0 ) then
                   w_time(DEF_VECT) = gpwmean(DEF_VECT,igaus,itime)
                else
                   w_time(DEF_VECT) = 1.0_rp
                endif
                fact0(DEF_VECT) = fact0(DEF_VECT) + pabdf_nsi(itime) * densi(DEF_VECT,igaus,itime)
             end do
             !
...
!!$              end do
  ...
  subroutine nsi_element_stabilization(&
    ...
    real(rp)                   :: adv,dif,rea,h2,gpvno,densi_max
    ...
    if( kfl_stabi_nsi == NSI_GALERKIN ) then
       ...
       do igaus = 1,pgaus
          do ivect = 1,VECTOR_SIZE
             ...
             call tauadr(&
                  kfl_taust_nsi,staco_nsi,adv,dif,rea,&
                  chale(ivect,1),chale(ivect,2),gpst1(ivect,igaus),dtinv_loc(ivect))
                ...
                if( ibopo > 0 ) then
                   ...
                   if( iroty == -1 ) then                                    ! Tangent system
                      call nsi_rotma3(&
                           ...
                           &      elapu(ivect,:,:),elrbu(ivect,:),elcmm(ivect,:,:),exnor(1,1,ibopo),worma)
                   else if( iroty >= 1 ) then                                ! Given system
                      call nsi_rotma3(&
                           &      inode,pnode,pevat,elauu(ivect,:,:),elaup(ivect,:,:),&
                           &      elapu(ivect,:,:),elrbu(ivect,:),elcmm(ivect,:,:),skcos(1,1,iroty),worma)
                   else if( iroty == -2 ) then                               ! Given system
                      call nsi_rotma3(&
                           &      inode,pnode,pevat,elauu(ivect,:,:),elaup(ivect,:,:),&
                           &      elapu(ivect,:,:),elrbu(ivect,:),elcmm(ivect,:,:),skcos_nsi(1,1,ibopo),worma)
                   else if( iroty == -3 ) then                               ! Geometrical normal
                      call nsi_rotma3(&
                           &      inode,pnode,pevat,elauu(ivect,:,:),elaup(ivect,:,:),&
                           &      elapu(ivect,:,:),elrbu(ivect,:),elcmm(ivect,:,:),skcos(1,1,ibopo),worma)
                   end if
                end if
             end do
          end if
  ...
  subroutine nsi_element_schur(&
    ...
    use def_nastin, only     :  nodpr_nsi,lperp_nsi,kfl_perip_nsi,&
         ...
         &                      staco_nsi,kfl_taush_nsi,penal_nsi,&
         &                      kfl_predi_nsi,kfl_advec_nsi,kfl_matdi_nsi,&
         &                      nbdfp_nsi,pabdf_nsi,kfl_regim_nsi,kfl_confi_nsi
    ...
    integer(ip)              :: ipres,iperi,idime,ihang,ivect
  ...
  subroutine nsi_element_time_step(&
    ...
    if( kfl_cotur_nsi /= 0 ) then
       call runend('NSI_TURBUL IS OBSOLETE')
    end if
    !
    ! DTCRI: Critical time step
    !
    adv(1:VECTOR_SIZE)   = gpden_ave(1:VECTOR_SIZE) * gpvno_ave(1:VECTOR_SIZE)                   ! Convective term ... rho*|u|
    dif(1:VECTOR_SIZE)   = gpvis_ave(1:VECTOR_SIZE) ! + gpmut_ave(1:VECTOR_SIZE)                   ! Viscous term ...... mu + mut
  ...
  subroutine nsi_element_external_force(&
    ...
    use def_nastin,          only : xforc_material_nsi, ntabr_nsi, fvela_nsi
    ...
    real(rp),    intent(in)  :: gpvel(VECTOR_SIZE,ndime,pgaus)
    real(rp),    intent(out) :: gprhs(VECTOR_SIZE,ndime,pgaus)
    integer(ip)              :: idime,igaus,ivect,pmate,ielem
    real(rp)                 :: u_ref(3),A_disk,n_disk(3),Ct,rho,uref2,uref3,V_disk
    real(rp)                 :: F_total,Force_normal, d_disk, veinf, Cp, fcori
    real(rp)                 :: gpcod(ndime)
...
!!$              end do
!!$             call get_tendencies_u(coord(3),ctime, gpten_geo, gpten_adv) ! get geostrophic and advection forces
!!$             do igaus = 1,pgaus
...
!!$          
  ...
  subroutine nsi_element_shock_capturing(&
    ...
    real(rp)                   :: additfact1,factt,facta
  ...
  subroutine nsi_rhodt_rhotau_nu_vector(&
    ...
    do igaus = 1,pgaus
       ...
       do ivect = 1,VECTOR_SIZE
          call tauadr(&
               kfl_taust_nsi,staco_nsi,adv(ivect),dif(ivect),rea(ivect),&
               chale(ivect,1),chale(ivect,2),gpsp1(ivect,igaus),dtinv_loc(ivect))
       end do
