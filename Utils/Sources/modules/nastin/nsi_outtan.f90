...
!  end if
  ...
  if( kfl_noslw_ker /= 0 ) then
     ...
     nullify(  visco_nsw )
     allocate( visco_nsw(npoin) )
     visco_nsw = 0.0_rp
     call projec_elements_to_nodes(el_nsw_visc,visco_nsw)
     visco_tmp = visco_tmp + visco_nsw
     !     deallocate( visco_nsw )
     if(1==1) then   ! This acn later be eliminated
        !
        ...
        !
        visco_nsw =0.0_rp
        call ker_proper('TURBU','NPOIN',dummi,dummi,visco_nsw)   ! I reuse visco_nsw   but now it is teh turbulent viscosity
        ...
        !     call ker_proper('TURBU','PGAUS',dummi,ielem,gpmut,pnode,pgaus,gpsha,gpcar)   ! nsi_elmope
        nullify(  densi_tmp )
        allocate( densi_tmp(npoin) )
        call ker_proper('DENSI','NPOIN',dummi,dummi,densi_tmp)
        if (kfl_cotur_nsi < 0) visco_nsw = densi_tmp * visco_nsw
        deallocate( densi_tmp )
        visco_tmp = visco_tmp + visco_nsw
     end if
     deallocate( visco_nsw )  
  end if
