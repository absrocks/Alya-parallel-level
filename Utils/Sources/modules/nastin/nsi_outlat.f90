     ...
     case(1_ip)
        ...
        if(kfl_visco_nsi/=0) then
           if(kfl_advec_nsi==0) then
              ...
              if(kfl_cotur_nsi==1.or.kfl_cotur_nsi<=-10) then
                 equat=trim(equat)//' Reynolds averaged Navier-Stokes equations (RANS)'        
              else if(kfl_cotur_nsi<=-1) then
                 equat=trim(equat)//' Navier-Stokes with LES filtering'
              else
        ...
        if(kfl_visco_nsi/=0) then
           lvisc=''//wback//'mu'
           if(kfl_cotur_nsi/=0) lvisc='('//wback//'mu+'//wback//'mu_t)'
