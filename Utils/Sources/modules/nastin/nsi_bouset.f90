subroutine nsi_bouset(kbsec,kbset)
  ...
  real(rp)                 :: gbvel(ndime,max(mgaus,mgaub)),tauwa,velno,xfact
  ...
  real(rp)                 :: norfo,rho_air,gbvis_aux
  logical(lg)              :: lauxi
           ...
           do inodb = 1,pnodb
              ...
              do idime = 1,ndime
                 ...
                 if (kfl_adj_prob == 0) then
                   ...
                   bovel(idime,inodb) = veloc_forw(idime,ipoin,1)
                 endif
              end do              
...
!               end do
              ...
              gauss_points: do igaub = 1,pgaub
                 ...
                 if( postp(1) % npp_setsb(31) /= 0 ) then
                    sedot = sedot + gbsur * dot_product(gbvel(1:ndime,igaub),baloc(1:ndime,ndime))
                 end if
                 ...
                 if( kfl_force == 1 ) then
                    ...
                    if( kfl_fixbo_nsi(iboun) == 3 ) then
                       ...
                       call nsi_bouwal(&                        
                            ...
                            bovel,bovfi,dummr,gbvis(igaub),gbden(igaub),baloc,ustar,dummr,rough_dom, dummr(1), &
                 ...
                 if( postp(1) % npp_setsb(16) /= 0 ) then
                    do idime = 1,ndime
                       setmv = setmv + gbsur * gbvel(idime,igaub) * baloc(idime,ndime)
                    end do
                 end if
                 ...
                 if( postp(1) % npp_setsb(27) /= 0 ) then
                    ...
                    if( sum(kfl_fixno_nsi(1:ndime,ipoin)) /= ndime .and. kfl_fixbo_nsi(iboun) /= 3 ) then
                       ...
                       !
                       norfo = dot_product(gbvel(1:ndime,igaub),baloc(1:ndime,ndime))
                       tanfo(1:ndime) = gbvel(1:ndime,igaub) - baloc(1:ndime,ndime) * norfo                       
                    else
                       !
  ...
  if( postp(1) % npp_setsb(27) /= 0 ) then
     ...
     !
     call PAR_MAX(taudotmax,'IN MY CODE','EXCLUDE MASTER',rank_max_owner)
...
!        if(associated(bupor_nsi)) then  ! to avoid entereing in the initialization phase where bupor_nsi is not assoc
!           do ipoin = 1,npoin
!              sepor(1:ndime) = sepor(1:ndime) + bupor_nsi(1:ndime,ipoin) !only own nodes to avoid repeating at interfase
!           end do
        !        end if
