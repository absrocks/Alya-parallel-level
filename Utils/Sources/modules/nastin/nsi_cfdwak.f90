...
!  use def_postpr
  ...
  real(rp)                   :: vehub(3), veave(3), Uinft, Cp, Ct, uhubn, uaven, power, yaw
  character(20)              :: messa
  character(7)               :: statu
  character(11)              :: forma
  character(6)               :: posit
 ...
 !             r_hub= min(r_hub, 0.99)
              do while (radiu_nsi(ipoin, pmate).lt.r_hub) 
                 ipoin =ipoin+1
              end do
              ! impose uniform force per unit area until r = r_hub
              r_hub = radiu_nsi(ipoin, pmate)
              if (ipoin.gt.1) then !.and.ipoin.lt.ntabl) then !all points greater than r = r_hub
                 do inode = 1, ipoin -1
            ...
            !     end do
              ...
              integ = 0.0_rp
              inte2 = 0.0_rp
              ! assuming linear behavior until first radial node. 
              delta = radiu_nsi(1, pmate)
              integ = integ + delta*forcn_nsi(1, pmate)  
              ! exact integral considering uniform tang force per unit area, that is, q_t = ar, 
              ! and normalizing q_tdr = ar^2dr
        ...
        if(kfl_rstar==2) then 
          ...
          do pmate = 1,kmate
             if( lforc_material_nsi(pmate) == 1 ) then
                ...
                call iofile(zero,lun_windt_nsi+pmate-2,fil_windt_nsi,'NASTIN TURBINE',statu,forma,posit)
                if (kfl_rstar/=2) write (lun_windt_nsi + pmate -2, 100) 
             end if
          end do
       end if
100    format('# 1:Step  2:Time(s)   3:ThrustCoeff   4:PowerCoeff   5:Vel Infty   6:Vel Hub   7:Vel Averaged   8:Power(kW)   9:Yaw (degrees) ')
  case ( 2_ip )
...
101 format(2x,i4,3x,f11.2,1x,6(f13.4,1x),f6.1)
