...
subroutine nsi_elmope_omp(itask)
  ...
  use def_master,            only : tesgs,current_zone,lumma,vesgs,rgene
  ...
  use def_master, only : fleve,kfl_paral,leinv_loc ! SACAR
  ...
  colors: do icolo = 1,par_omp_num_colors
     ...
     !$OMP                 kfl_kemod_ker, kfl_grvir_nsi, mgaus, cutim,             &
...
!              call ker_proper('VDRTU','PGAUS',dummi,ielem,dgpmut_dvel,pnode,pgaus,gpsha,gpcar)
           ...
           call nsi_turbul(&
                itask,1_ip,pnode,pgaus,1_ip,pgaus,kfl_cotur_nsi,gpsha,gpcar,elvel,&
                gpden,gpvis,gpmut,gpgvi,grvis,gpgve,ielem,kfl_kemod_ker)
              ...
              if( itask == 4 ) then
                 ...
                 if( NSI_FRACTIONAL_STEP ) then
                    call nsi_rhodt_rhotau_nu(                                      &
                         ...
                         chale,dtinv_loc,elrhodt,elrhotau,elmurho)
                 end if
                 !
                 ! Assembly
                 !
                 ...
                 if( NSI_FRACTIONAL_STEP ) then
                    do inode = 1,pnode
                       ...
                       !$OMP ATOMIC
#endif
                       mu_rho_nsi(ipoin) = mu_rho_nsi(ipoin) + elmurho(inode)
