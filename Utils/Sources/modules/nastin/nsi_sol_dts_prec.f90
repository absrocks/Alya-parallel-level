...
subroutine nsi_sol_dts_prec(rhsid,unkno,amatr,pmatr)
  ...
  if( INOTMASTER ) then
     ...
     call memory_alloca_min(amatr_dts)
     call memory_alloca_min(delta_dts)
     call memory_alloca_min(resi)
     call memory_alloca_min(resi_auxi)
