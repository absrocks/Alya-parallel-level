...
subroutine nsi_exacso(&
  ...
  real(rp)                   :: x,y,z,r,freq,diss,ft,dfdt,n,omega,ri,ro,theta,vt
  ...
  if( kfl_exacs_nsi == 1 ) then
     ...
     p       =  sin(pi*x)*sin(pi*x) + sin(pi*y)*sin(pi*y)
     dpdx    =  2.0_rp*pi*cos(pi*x)*sin(pi*x)
     dpdy    =  2.0_rp*pi*cos(pi*y)*sin(pi*y)
  ...
  if( itask == 1 ) then 
     ...
     if( kfl_visco_nsi == 1  ) then
        ...
        if( fvins_nsi > 0.9_rp  ) then
           !
           ! Divergence form:  - mu*d^2uj/dxidxj - dmu/dxj duj/dxi
           !
        ...
        if( fvins_nsi > 1.9_rp  ) then
           !
           ! Complete form: 2/3 dmu/dxi duj/dxj + 2/3 mu d^2uj/dxidxj
           !
