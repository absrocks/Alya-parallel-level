subroutine nsi_updthe(itask)
  ...
  real(rp), target        :: avtem_xxx(1),volum_xxx(1)
  ...
  if(kfl_regim_nsi==3) then
     if(abs(itask)==1) then
        ...
        if(kfl_prthe_nsi==2) then
           ...
           !
           call pararr('SUM',0_ip,1_ip,bmflx)
           call pararr('SUM',0_ip,1_ip,vinvt_nsi(1))
...
!!$                 call runend('NSI_UPDTHE:PLEASE, SET THE INITIAL MASS')      
     ...
     if (lowtr_nsi == 0.0_rp) then
            ...
            do ielem=1,nelem
              ...
              if(pmate/=-1) then
                 ...
                 do igaus=1,pgaus
                    ...
                    avtem_xxx(1)=avtem_xxx(1)+gpvol(igaus)*gptem
                    volum_xxx(1)=volum_xxx(1)+gpvol(igaus)
                 end do
              end if
           end do 
        end if
        ...
        ! Parall: Sum over all subdomains
        ! 
        call pararr('SUM',0_ip,1_ip,avtem_xxx)
        call pararr('SUM',0_ip,1_ip,volum_xxx)
        !
        ! Store average temperature
        !
        if (volum_xxx(1)  /= 0.0_rp ) then 
           lowtr_nsi = avtem_xxx(1) / volum_xxx(1)
        else
           print *,'ZERO VOLUME !!!'
        endif
     endif 
