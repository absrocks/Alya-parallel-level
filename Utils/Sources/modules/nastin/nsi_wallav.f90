subroutine nsi_wallav(itask)
  ...
  !
  !    ITASK = 1 ... Calculates time-averaged velocity at the boundary gauss points
  !            2 ... Calculates time-averaged velocity at the nodes (for postprocessing)
  !            3 ... Calculates initial value of time-averaged velocity
  !
  ...
  !***
  !-----------------------------------------------------------------------
  use def_master,             only : ip,rp,dtime,ittim,solve,veloc,inotmaster,kfl_rstar,kfl_paral
  ...
  use def_kermod,             only : wallcoupling_waexl, kfl_waexl_ker, velel_ker, lexlo_ker, velav_ker, tpeav_ker
...
!  real(rp)                  :: ux,uy
  ...
  if ( itask == 1_ip ) then
     ...
     call nsi_nsw_averaging(1_ip,avwei)     ! obtains lnsw_exch(ielem)%velav
     ...
     if ( kfl_waexl_ker == 1_ip ) then
        call COU_GET_INTERPOLATE_POINTS_VALUES(veloc,velel_ker,wallcoupling_waexl)
     end if
     ...
     if ( INOTMASTER )then
        ...
        call nsi_nsw_averaging(1_ip,avwei)  ! Obtains lnsw_exch(ielem)%velav(1:ndime) -- here avwei is not used due to 1_ip - this must be done with or without restart
...
subroutine nsi_nsw_averaging(kfl_initilization,avwei)
  ...
  real(rp),intent(in)       :: avwei   ! e=dt/T, weighting function for averaging
  ...
  if (kfl_noslw_ker == 1_ip ) then
     ...
     if (kfl_initilization==1_ip) then
        ...
        elements: do ielem = 1,nelem
           if ( lnsw_exch(ielem)%nbogp /= 0_ip ) then
              lnsw_exch(ielem)%velav(1:ndime) = avwei * lnsw_exch(ielem)%fact * lnsw_exch(ielem)%vel_aux(1:ndime) +(1.0_rp-avwei)*lnsw_exch(ielem)%velav(1:ndime)
           end if
        end do elements
     end if
