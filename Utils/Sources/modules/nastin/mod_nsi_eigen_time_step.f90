...
module mod_nsi_eigen_time_step
  ...
  use def_master,              only : INOTMASTER,veloc,mem_modul,modul,kfl_paral
...
contains
  ...
  subroutine nsi_eigen_time_step_all(dt)
    ...
    if(fact1 < 1e-15_rp ) then
       phi = 0.0_rp
    else
       phi = pi*0.5_rp - atan(fact0/fact1)
    endif

    !if(kfl_paral<=0) print*,'a=',phi,fact0,fact1
  ...
  subroutine nsi_eigen_time_step_element_operations(VECTOR_DIM,pnode,pgaus,list_elements)
    ...
    real(rp)                         :: elconv_pos(VECTOR_DIM,ndime,mnode)
    real(rp)                         :: elconv_neg(VECTOR_DIM,ndime,mnode)
    real(rp)                         :: elvisc(VECTOR_DIM,ndime,mnode)
    real(rp)                         :: fact0(VECTOR_DIM)
    ...
    real(rp)                         :: gpder(VECTOR_DIM,ndime,pnode,pgaus)  ! dN/dsi (dummy)
    real(rp)                         :: gpden(VECTOR_DIM,pgaus)
    real(rp)                         :: gpvis(VECTOR_DIM,pgaus)
    real(rp)                         :: gpmut(VECTOR_DIM,pgaus)
    real(rp)                         :: agrau(VECTOR_DIM,pnode)
    real(rp)                         :: gpvel(VECTOR_DIM,ndime)
    integer(ip)                      :: list_elements_p(VECTOR_DIM)          ! List of elements (always positive)
!!$    real(rp),    allocatable          :: elcod(:,:,:)
!!$    real(rp),    allocatable          :: elvel(:,:,:)
!!$    real(rp),    allocatable          :: elconv_pos(:,:,:)
!!$    real(rp),    allocatable          :: elconv_neg(:,:,:)
!!$    real(rp),    allocatable          :: elvisc(:,:,:)
!!$    real(rp),    allocatable          :: fact0(:)
!!$    real(rp),    allocatable          :: fact1(:) 
!!$    real(rp),    allocatable          :: fact2(:) 
!!$    real(rp),    allocatable          :: gpsha(:,:,:)       ! N
!!$    real(rp),    allocatable          :: gpcar(:,:,:,:)     ! dN/dxi
!!$    real(rp),    allocatable          :: gpvol(:,:)         ! w*|J|, |J|
!!$    real(rp),    allocatable          :: gpder(:,:,:,:)     ! dN/dsi (dummy)
!!$    real(rp),    allocatable          :: gpden(:,:)
!!$    real(rp),    allocatable          :: gpvis(:,:)
!!$    real(rp),    allocatable          :: agrau(:,:)
!!$    real(rp),    allocatable          :: gpvel(:,:)
!!$    integer(ip), allocatable          :: list_elements_p(:) ! List of elements (always positive)
...
!!$    allocate(elcod(VECTOR_DIM,ndime,mnode))
!!$    allocate(elvel(VECTOR_DIM,ndime,mnode))
!!$    allocate(elconv_pos(VECTOR_DIM,ndime,mnode))
!!$    allocate(elconv_neg(VECTOR_DIM,ndime,mnode))
!!$    allocate(elvisc(VECTOR_DIM,ndime,mnode))
!!$    allocate(fact0(VECTOR_DIM))
!!$    allocate(fact1(VECTOR_DIM)) 
!!$    allocate(fact2(VECTOR_DIM)) 
!!$    allocate(gpsha(VECTOR_DIM,pnode,pgaus))        ! N
!!$    allocate(gpcar(VECTOR_DIM,ndime,mnode,pgaus))  ! dN/dxi
!!$    allocate(gpvol(VECTOR_DIM,pgaus))              ! w*|J|, |J|
!!$    allocate(gpder(VECTOR_DIM,ndime,pnode,pgaus))  ! dN/dsi (dummy))
!!$    allocate(gpden(VECTOR_DIM,pgaus))
!!$    allocate(gpvis(VECTOR_DIM,pgaus))
!!$    allocate(agrau(VECTOR_DIM,pnode))
!!$    allocate(gpvel(VECTOR_DIM,ndime))
!!$    allocate(list_elements_p(VECTOR_DIM))          ! List of elements (always positive)
    !
    ! Initialization
    !
    elconv_pos = 0.0_rp
    ...
    !
   if( kgaus == 1 ) then
       dummr = elmar(pelty) % weicg   
       !dummr = elmar(pelty) % weigp(1)
       call element_shape_function_derivatives_jacobian_vector_2(&
            pnode,kgaus,plapl,dummr,elmar(pelty) % shacg,&
            elmar(pelty) % dercg,elmar(pelty) % hescg,&
            elcod,gpvol,gpsha,gpder,gpcar,list_elements=list_elements_p)
    else
       call element_shape_function_derivatives_jacobian(&
            pnode,kgaus,plapl,elmar(pelty) % weigp,elmar(pelty) % shape,&
    ...
    !
    if( kgaus == 1 ) then
       call ker_proper('DENSI','COG  ',dummi,list_elements_p,gpden,pnode,kgaus,gpsha,gpcar)
       call ker_proper('VISCO','COG  ',dummi,list_elements_p,gpvis,pnode,kgaus,gpsha,gpcar)
       call ker_proper('TURBU','COG  ',dummi,list_elements_p,gpmut,pnode,kgaus,gpsha,gpcar)
    else
       call ker_proper('DENSI','PGAUS',dummi,list_elements_p,gpden,pnode,kgaus,gpsha,gpcar)  
       call ker_proper('VISCO','PGAUS',dummi,list_elements_p,gpvis,pnode,kgaus,gpsha,gpcar)         
       call ker_proper('TURBU','PGAUS',dummi,list_elements_p,gpmut,pnode,kgaus,gpsha,gpcar)         
    end if
    !
    ! a.grad(Ni)
    !
    do igaus = 1,kgaus
       gpvis(DEF_VECT,igaus) = gpvis(DEF_VECT,igaus) + gpden(DEF_VECT,igaus) * gpmut(DEF_VECT,igaus) 
       ...
       do inode = 1,pnode
          do idime = 1,ndime
             agrau(DEF_VECT,inode) = agrau(DEF_VECT,inode) + gpvel(DEF_VECT,idime) * gpcar(DEF_VECT,idime,inode,igaus)
          end do
       end do
       !
...
!!$    deallocate(elcod)
!!$    deallocate(elvel)
!!$    deallocate(elconv_pos)
!!$    deallocate(elconv_neg)
!!$    deallocate(elvisc)
!!$    deallocate(fact0)
!!$    deallocate(fact1) 
!!$    deallocate(fact2) 
