...
module mod_nsi_schur_complement
  ...
  real(rp),                save, pointer :: xx(:)
  real(rp),                save, pointer :: zz(:)
  real(rp),                save, pointer :: vv(:)
  real(rp),                save, pointer :: rr(:)
  real(rp),                      pointer :: bp(:)
  real(rp),                save, pointer :: bu(:)
  real(rp),                save, pointer :: uu(:)
  real(rp),                save, pointer :: pp(:)
  real(rp),                save, pointer :: Q(:)
...
contains
  ...
  subroutine nsi_schur_complement_solution()
    ...
    if ( kfl_expco_nsi /= 1 .or. itinn(modul) == 1 ) then  ! for the case kfl_expco_nsi==1 do this part only the first iteration, else do it always
       ...
       call nsi_rotsch(1_ip,uu)                                     ! Global to local
    ...
    if( NSI_SCHUR_COMPLEMENT ) then
       ...
       call nsi_rotsch(2_ip,uu)                      ! Local to global
       !
       ! Velocity correction
       !
...
!#endif
  ...
  subroutine nsi_solcor_consistent_mass()
    ...
    call nsi_rotsch(2_ip,vv)                           ! Local to global for delta_u

    uu(1:ndime*npoin) = uu(1:ndime*npoin) + vv(1:ndime*npoin)

    if( kfl_timin == 1 ) call Parall(20_ip)    ! par_barrier
...
!    deallocate(hhh)
    ...
    call cputim(time3)
    cputi_nsi(1) = cputi_nsi(1) + time3 - time2
