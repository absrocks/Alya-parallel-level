...
!> @brief   Single cell run for Initial condition setup for Ohara-Rudy 2011 heterogeneous model plus Land 2017 ECC model
...
!> @} 
!!-----------------------------------------------------------------------
logical function exm_oceola(mat) 
...
!!!! calculate ca current in the junctional sarcoplasmic reticulum jsr
        ...
        !!ALF if (j>=10) then  !it was 10 for saving
        !   write(995,400) vaulo_exm(1:29,1)
!!!ALF     write(996,*) vcolo_exm(1:11,2)  !atim
        !   write(994,300) viclo_exm(1:26,1)
!!!ALF     write(997,*) elmlo_exm(2)
!!!ALF     j = 0
!!!ALF end if
!!!ALF     j = j + 1
  ...
  !!ALFwrite(997,*) "999999"  !separator between cells
...
!! COMMENT ALF ALF ALF BUG FRANCESC SERIAL / PARAL
!  if (INOTMASTER) then
     stateland(1,:,:,:) = S
     stateland(2,:,:,:) = W
     stateland(3,:,:,:) = CaTRPN
