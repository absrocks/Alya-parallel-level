...
subroutine exm_scvent_ionicurrents(ipoin,xioni,dioni)
  ...
  integer(ip) :: kfl_odets_exm,i,iauxi,clmat,n,imate

  real(rp) :: tt, h, j, m, d, f_Ca, f1, f2, r, q, xr1, xr2, xs, xf, g, Ca_i, Ca_SR, Na_i, V
  
  real(rp) ::  func_CaiV, func_CaSRV, func_dV, func_f1V, &
  func_f2V, func_fCaV, func_XsV, func_gV, func_hV, func_jV, func_mV, &
  func_NaiV, func_qV, func_rV, func_VV, func_XfV, func_xr1V, func_xr2V
  ...
  E_f = -0.017
  E_Ca = (0.5*Rcons*Temp/F)*log(Ca_o/vconc(1,ipoin,2))
  Alpha_K1 = 3.91/(1+exp(0.5942*(V*1000.0-E_K*1000.0-200.0)))
  beta_K1 = (-1.509*exp(0.0002*(V*1000.0-E_K*1000.0+100.0))+exp(0.5886*(V*1000.0-E_K*1000.0-10.0)))/ &
            (1.0+exp(0.4547*(V*1000.0-E_K*1000.0)))
  x_K1_inf = Alpha_K1/(Alpha_K1+beta_K1)
    
  vicel_exm(1,ipoin,1) = g_Na*(vauxi_exm(3,ipoin,2)**3.0)*vauxi_exm(1,ipoin,2)*vauxi_exm(2,ipoin,2)*(V-E_Na)
  vicel_exm(2,ipoin,1) = (g_CaL*4.0*V*F**2.0)*(vconc(1,ipoin,2)*exp(2.0*V*F/(Rcons*Temp))-0.341*Ca_o)*vauxi_exm(4,ipoin,2)*vauxi_exm(6,ipoin,2)*vauxi_exm(7,ipoin,2)*f_Ca/ & 
                   (Rcons*Temp*(exp(2.0*V*F/(Rcons*Temp))-1.0))
  vicel_exm(3,ipoin,1) = g_to*vauxi_exm(8,ipoin,2)*vauxi_exm(9,ipoin,2)*(V-E_K)
  vicel_exm(4,ipoin,1) = g_Kr*sqrt(K_o/5.4)*vauxi_exm(10,ipoin,2)*vauxi_exm(11,ipoin,2)*(V-E_K)
  vicel_exm(5,ipoin,1) = g_Ks*vauxi_exm(12,ipoin,2)**2.0*(1.0+0.6/(1.0+(3.8*1e-5/vconc(1,ipoin,2))**1.4))*(V-E_Ks)
  vicel_exm(6,ipoin,1) = g_K1*x_K1_inf*sqrt(K_o/5.4)*(V-E_K)
  vicel_exm(7,ipoin,1) = g_f*vauxi_exm(13,ipoin,2)*(V-E_f)
  vicel_exm(8,ipoin,1) = ((P_NaK*K_o*vconc(3,ipoin,2)/(K_o+K_mk))/(vconc(3,ipoin,2)+K_mNa))/ &
                   (1.0+0.1245*exp(-0.1*V*F/(Rcons*Temp))+0.353*exp(-V*F/(Rcons*Temp)))
  vicel_exm(9,ipoin,1) = K_NaCa*(exp(Gamma*V*F/(Rcons*Temp))*vconc(3,ipoin,2)**3.0*Ca_o-exp((Gamma-1.0)*V*F/(Rcons*Temp)) &
                   *Na_o**3.0*vconc(1,ipoin,2)*Alpha)/((Km_Na_i**3.0+Na_o**3.0)*(Km_Ca+Ca_o)* &
                   (1.0+K_sat*exp((Gamma-1.0)*V*F/(Rcons*Temp))))
  vicel_exm(10,ipoin,1) = ((a_rel*(vconc(2,ipoin,2)**2.0)/(b_rel**2.0+vconc(2,ipoin,2)**2.0))+c_rel)*vauxi_exm(4,ipoin,2)*vauxi_exm(14,ipoin,2)*(0.0556)
  vicel_exm(11,ipoin,1) = Vmax_up/(1.0+K_up**2.0/vconc(1,ipoin,2)**2.0)
  vicel_exm(12,ipoin,1) = V_leak*(vconc(2,ipoin,2)-vconc(1,ipoin,2))
  vicel_exm(13,ipoin,1) = g_pCa*vconc(1,ipoin,2)/(vconc(1,ipoin,2)+K_pCa)
  ...
  !open(unit=1, file='voltage.dat')
    ...
    !! K2
    K2h =  dtime*func_hV(tt+dtime/2.0,  V+K1V/2.0,  h+K1h/2.0)
    K2j =  dtime*func_jV(tt+dtime/2.0,  V+K1V/2.0,  j+K1j/2.0)
    K2m =     dtime*func_mV(tt+dtime/2.0,  V+K1V/2.0,  m+K1m/2.0)
    K2d =     dtime*func_dV(tt+dtime/2.0,  V+K1V/2.0,  d+K1d/2.0)
    K2f_Ca =  dtime*func_fCaV(tt+dtime/2.0,  f_Ca+K1f_Ca/2.0,  Ca_i+K1Ca_i/2.0 ,V+K1V/2.0)
    K2f1 = dtime*func_f1V(tt+dtime/2.0,  V+K1V/2.0,  f1+K1f1/2.0  ,Ca_i+K1f1/2.0 ,df1)
    K2f2 =    dtime*func_f2V(tt+dtime/2.0,  V+K1V/2.0,  f2+K1f2/2.0)
    K2r =     dtime*func_rV(tt+dtime/2.0,  V+K1V/2.0,  r+K1r/2.0)
    K2q =     dtime*func_qV(tt+dtime/2.0,  V+K1V/2.0,  q+K1q/2.0)
    K2xr1 =   dtime*func_xr1V(tt+dtime/2.0,V+K1V/2.0,  xr1+K1xr1/2.0)
    K2xr2 =   dtime*func_xr2V(tt+dtime/2.0,  V+K1V/2.0,  xr2+K1xr2/2.0)
    K2Xs =    dtime*func_XsV(tt+dtime/2.0,  V+K1V/2.0,  xs+K1Xs/2.0)
    K2Xf =    dtime*func_XfV(tt+dtime/2.0,  V+K1V/2.0,  xf+K1Xf/2.0)
    K2g =     dtime*func_gV(tt+dtime/2.0,  Ca_i+K1Ca_i/2.0,  g+K1g/2.0 ,V+K1V/2.0)
    K2Ca_i =  dtime*func_CaiV(tt+dtime/2.0,  Ca_i+K1Ca_i/2.0,  Ca_SR+K1Ca_SR/2.0,  d+K1d/2.0,  g+K1g/2.0,  V+K1V/2.0, Na_i+K1Na_i/2.0,  f1+K1f1/2.0  ,f2+K1f2/2.0  ,f_Ca+K1f_Ca/2.0)
    K2Ca_SR = dtime*func_CaSRV(tt+dtime/2.0,  Ca_i+K1Ca_i/2.0,  Ca_SR+K1Ca_SR/2.0,  d+K1d/2.0,  g+K1g/2.0)
    K2Na_i =  dtime*func_NaiV(tt+dtime/2.0,  m+K1m/2.0,  h+K1h/2.0,  j+K1j/2.0,  V+K1V/2.0,  Na_i+K1Na_i/2.0,  Ca_i+K1Ca_i/2.0)
    K2V =     dtime*func_VV(tt+dtime/2.0,  V+K1V/2.0,  r+K1r/2.0,  q+K1q/2.0,  xr1+K1xr1/2.0,  xr2+K1xr2/2.0,  xs+K1Xs/2.0, &
              Ca_i+K1Ca_i/2.0,  d+K1d/2.0,  f1+K1f1/2.0,  f2+K1f2/2.0,  f_Ca+K1f_Ca/2.0,  Na_i+K1Na_i/2.0,  m+K1m/2.0,  h+K1h/2.0,  j+K1j/2.0  ,xf+K1Xf/2.0)

    !! K3
    K3h = dtime*func_hV(tt+dtime/2.0,  V+K2V/2.0,  h+K2h/2.0)
    K3j = dtime*func_jV(tt+dtime/2.0,  V+K2V/2.0,  j+K2j/2.0)
    K3m =     dtime*func_mV(tt+dtime/2.0,  V+K2V/2.0,  m+K2m/2.0)
    K3d =     dtime*func_dV(tt+dtime/2.0,  V+K2V/2.0,  d+K2d/2.0)
    K3f_Ca =  dtime*func_fCaV(tt+dtime/2.0,  f_Ca+K2f_Ca/2.0,  Ca_i+K2Ca_i/2.0 ,V+K2V/2.0)
    K3f1 = dtime*func_f1V(tt+dtime/2.0,  V+K2V/2.0,  f1+K2f1/2.0, Ca_i+K2f1/2.0,df1)
    K3f2 =    dtime*func_f2V(tt+dtime/2.0,  V+K2V/2.0,  f2+K2f2/2.0)
    K3r =     dtime*func_rV(tt+dtime/2.0,  V+K2V/2.0,  r+K2r/2.0)
    K3q =     dtime*func_qV(tt+dtime/2.0,  V+K2V/2.0,  q+K2q/2.0)
    K3xr1 =   dtime*func_xr1V(tt+dtime/2.0,V+K2V/2.0,  xr1+K2xr1/2.0)
    K3xr2 =  dtime* func_xr2V(tt+dtime/2.0,  V+K2V/2.0,  xr2+K2xr2/2.0)
    K3Xs =    dtime*func_XsV(tt+dtime/2.0,  V+K2V/2.0,  xs+K2Xs/2.0)
    K3Xf =    dtime*func_XfV(tt+dtime/2.0,  V+K2V/2.0,  xf+K2Xf/2.0)
    K3g =     dtime*func_gV(tt+dtime/2.0,  Ca_i+K2Ca_i/2.0,  g+K2g/2.0 ,V+K2V/2.0)
    K3Ca_i =  dtime*func_CaiV(tt+dtime/2.0,  Ca_i+K2Ca_i/2.0,  Ca_SR+K2Ca_SR/2.0,  d+K2d/2.0,  g+K2g/2.0,  V+K2V/2.0,  Na_i+K2Na_i/2.0,  f1+K2f1/2.0  ,f2+K2f2/2.0  ,f_Ca+K2f_Ca/2.0)
    K3Ca_SR = dtime*func_CaSRV(tt+dtime/2.0,  Ca_i+K2Ca_i/2.0,  Ca_SR+K2Ca_SR/2.0,  d+K2d/2.0,  g+K2g/2.0)
    K3Na_i =  dtime*func_NaiV(tt+dtime/2.0,  m+K2m/2.0,  h+K2h/2.0,  j+K2j/2.0,  V+K2V/2.0,  Na_i+K2Na_i/2.0,  Ca_i+K2Ca_i/2.0)
    K3V =     dtime*func_VV(tt+dtime/2.0,  V+K2V/2.0,  r+K2r/2.0,  q+K2q/2.0,  xr1+K2xr1/2.0,  xr2+K2xr2/2.0,  xs+K2Xs/2.0,  Ca_i+K2Ca_i/2.0,  d+K2d/2.0,  f1+K2f1/2.0,  f2+K2f2/2.0,  f_Ca+K2f_Ca/2.0,  Na_i+K2Na_i/2.0,  m+K2m/2.0,  h+K2h/2.0,  j+K2j/2.0  ,xf+K2Xf/2.0)
...
real(rp) function func_CaiV(tt,Ca_i,Ca_SR,d,g,V,Na_i,f1,f2,f_Ca)
  ...
  !! Ca2+ dynamics
  !Ventricular
  I_rel = ((a_rel*Ca_SR**2.0/(b_rel**2.0+Ca_SR**2.0))+c_rel)*d*g*(0.0411)
  ...
  func_CaiV = Ca_ibufc*(I_leak-I_up+I_rel-(I_CaL+I_bCa+I_pCa-2.0*I_NaCa)*C_m/(2.0*V_c*F*1.0e-18));
end function
...
real(rp) function func_dV(tt,V,d)
  ...
  !I_CaL, d gate
  !Ventricular
  d_inf = 1.0/(1.0+exp(-(V*1000.0+9.1)/7.0))
...
real(rp) function  func_f1V(tt,V,f1,Ca_i,df1)
  ...
  !! I_CaL, f1 gate
  !Ventricular
  f1_inf = 1.0/(1.0+exp((V*1000.0+26.0)/3.0))
...
real(rp) function func_f2V(tt,V,f2)
  ...
  !! I_CaL, f2 gate
  !Ventricular
  f2_inf = (0.67/(1.0+exp((V*1000.0+35.0)/4.0)))+0.33
  taw_f2 =  ((600.0*(exp((-(V*1000.0+25.0)**2.0)/170.0))+31.0/(1.0+exp((25.0-V*1000.0)/10.0))+16.0/(1.0+exp((30.0+V*1000.0)/10.0))))/1000.0
...
real(rp) function func_mV(tt,V,m)
  ...
  !! I_Na, m gate
  m_inf =  (1.0+dexp((-34.1-V*1000.0)/5.9))**(-1.0/3.0)
  Alpha_m =  1.0/(1.0+dexp((-60.0-V*1000.0)/5.0))
  beta_m =  0.1/(1.0+dexp((V*1000.0+35.0)/5.0))+0.1/(1.0+dexp((V*1000.0-50.0)/200.0))
...
real(rp) function func_NaiV(tt,m,h,j,V,Na_i,Ca_i)
  ...
  !! sodium dynamics
  func_NaiV = -C_m*(I_Na+I_bNa+3.0*I_NaK+3.0*I_NaCa)/(F*V_c*1e-18)
end function
