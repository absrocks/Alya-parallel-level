...
!!        end if
  ...
  if(ituss_exm==3) then !EPICARDIUM
     !vauin_exm(:,1,1)=vaulo_exm(:,tt)
     !vcoin_exm(:,1,1) = vcolo_exm(:,tt)
     !vminimate_exm(1,1) = elmlo_exm(tt)
     vauin_exm(:,3,1)=vaulo_exm(:,t)
     vcoin_exm(:,3,1) = vcolo_exm(:,t)
     vminimate_exm(3,1) = elmlo_exm(t)
  else if (ituss_exm==1) then !ENDOCARDIUM
     !vauin_exm(:,2,1)=vaulo_exm(:,tt)
     !vcoin_exm(:,2,1) = vcolo_exm(:,tt)
     !vminimate_exm(1,1) = elmlo_exm(tt)
     vauin_exm(:,1,1)=vaulo_exm(:,t)
     vcoin_exm(:,1,1) = vcolo_exm(:,t)
     vminimate_exm(1,1) = elmlo_exm(t)
  else if (ituss_exm==2) then !MIDMYOCARDIUM
     !vauin_exm(:,3,1)=vaulo_exm(:,tt)
     !vcoin_exm(:,3,1) = vcolo_exm(:,tt)
     !vminimate_exm(1,1) = elmlo_exm(tt)
     vauin_exm(:,2,1)=vaulo_exm(:,t)
     vcoin_exm(:,2,1) = vcolo_exm(:,t)
