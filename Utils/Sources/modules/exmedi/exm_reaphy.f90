...
!!!!!
     ...
     kfl_comdi_exm = 0_ip                     ! Conductivity model distribution

     kfl_prdef_exm = 0_ip                     ! Properties global (0), node-wise(1) or element-wise(2) 
     ...
     fract_diff_exm = 1.0_rp        ! Default value of Fractional diffusion is 1.
     
     modfi_exm     = 0_ip                     ! Field: Fiber model
     modac_exm     = 0_ip                     ! Field: Activation potential
                 ...
                 if (words(2)=='ON') then
                    ...
                    do while (words(1)/='ENDPS')
                       ...
                       if (words(1) == 'NUMBE') then
                          ...
                          if (nrootecg_exm > 256) call runend('EXM_REAPHY: TOO MANY ECG ROOTS, MAXVAL IS 256!!')
...
!!!                          frequecg_exm = param(1)
           ...
           do while(words(1)/='ENDPR')
              if( words(1) == 'NODIF' ) then
                    ...
                    if(words(1)=='FRACT') then
                       ...
                       do while(words(1)/='ENDFR')
                            ...
                            if(words(1)=='COEFF')then
                               fract_diff_exm(imate) = getrea('COEFF',1.0_rp,'#Fractional diffusion coefficient')  
                        ...
                        if (fract_diff_exm(imate).gt.1.0_rp .or. fract_diff_exm(imate).le.0.0_rp) then
                 ...
                 if (words(2)=='FITZH') then
                    ...
                    !    ALPHA= 0.1   $ old alpha
                    !    BETA = 0.01  $ old epsilon
                    !    DELTA= 0.5   $ old gamma_membrane
                    !    C1FHN= 10.0  $ old c_membrane_const
                    !    C2FHN= 0.5   $ no estaba
                 ...
                 do while(words(1)/='ENDIN')
                    ...
                    !         kfl_hfmod_exm(imate) = 3_ip                          
                       ...
                       !call ecoute('exm_reaphy')
                    else if(words(1)=='CONDU' .and. kfl_hfmod_exm(imate) >= 1) then
                       call ecoute('exm_reaphy')
                       igrou = igrou+1
                       ivalu = 0
           ...
           if(words(2)=='ACTIV') then
              modac_exm = -getint('FIELD',1_ip,'#Field Number for the activation potential')
           end if
