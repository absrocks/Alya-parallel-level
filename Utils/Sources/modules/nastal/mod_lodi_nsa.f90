...
module mod_lodi_nsa
  ...
  !
  integer(ip)           :: ndofn
  real(rp), pointer     :: gamma_nsa(:)        !npoin
  !
  type LODI
    integer(ip)           :: idofn           =  1 
    integer(ip)           :: ndofn           =  0 
  ...
  public :: CHARAs
  public :: lodi_nsa_allocate
  public :: lodi_nsa_deallocate
  public :: lodi_nsa_get_characteristics
...
contains 
  ...
  !                                                                            !
  !-----------------------------------------------------------------------||---!
  subroutine lodi_nsa_allocate( STRUCT ) !, dof, den, vel, pre )
...
!
  ...
  !                                                                            !
  !-----------------------------------------------------------------------||---!
  subroutine lodi_nsa_deallocate( STRUCT )
    implicit none 
    type(LODI),  intent(inout) :: STRUCT
    !
    if(INOTMASTER) then
  ...
  !                                                                            !
  !-----------------------------------------------------------------------||---!
  subroutine lodi_nsa_get_characteristics( STRUCT )
  implicit none 
  type(LODI), intent(inout) :: STRUCT
  !-----------------------------------------------------------------------||---!
  !
  ...
  do ielem = 1,nelem 
    ...
    do inode = 1,pnode
      ...
      elgamme(inode)       = STRUCT%gamme !gamma_nsa(ipoin) 
      !
      ! PHIt -> [rho, vel, p] 
