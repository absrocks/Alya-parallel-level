subroutine tur_addarr
  ...
  call parari('MAX',0_ip,1_ip,kfl_fixn6_tur)
  call parari('MAX',0_ip,1_ip,kfl_fixn8_tur)
  ...
  if(  TUR_K_EPS_CHIEN         .or.& 
     ...
     kfl_ustar_tur=2
       ...
       TUR_SST_K_OMEGA)        .or.&
       postp(1) % npp_stepi (11)/=0    .or.&
       postp(1) % npp_stepi (12)/=0) then
     kfl_ustar_tur=1
  end if
  ...
  if(  TUR_SPALART_ALLMARAS               .or.&
       ...
       postp(1) % npp_stepi (10)/=0               .or.&
       postp(1) % npp_stepi (11)/=0               .or.&
       postp(1) % npp_stepi (14)/=0) then
     kfl_walld_tur=1
  end if
  ...
  if( abs(kfl_custo) == 2 .or. kfl_custo == 3 ) then
     kfl_walld_tur=1
  end if
  !
  ! grad(phi)
...
!     end if
  ...
  call parari('MAX',0_ip,1_ip,kfl_grono_tur)
  call parari('MAX',0_ip,1_ip,kfl_greps_tur)
  call parari('MAX',0_ip,1_ip,kfl_grk12_tur)
  ...
  call parari('MAX',0_ip,1_ip,dummi)
