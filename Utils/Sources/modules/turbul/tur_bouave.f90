        ...
        if(nboun==0) then
           do ipoin=1,npoin
              if(lpoty(ipoin)/=0) then
                 if(kfl_fixno_tur(1,ipoin,1)==6.or.kfl_fixno_tur(1,ipoin,1)==8) then
                    vevel=0.0_rp
                    do idime=1,ndime
                       vevel=vevel+veloc(idime,ipoin,1)*veloc(idime,ipoin,1)
                    end do
           ...
           boundaries: do iboun=1,nboun
              ...
              if(fixn6==pnodb.or.fixn8==pnodb) then
                 ...
                 bovel(1:ndime,1:pnodb)=veloc(1:ndime,lnodb(1:pnodb,iboun),1)
                       ...
                       do inodb=1,pnodb
                          ...
                          do idime=1,ndime
                             gbvel(idime)=gbvel(idime)&
                                  +elmar(pblty)%shape(inodb,igaub)*veloc(idime,ipoin,1)
                          end do
                       end do
