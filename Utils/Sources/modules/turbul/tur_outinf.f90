  ...
  if(kfl_paral<=0) then
     ...
     case(1_ip)
        ...
        if(TUR_SPALART_ALLMARAS) then
           ...
           if (kfl_kemod==1) write(momod(modul)%lun_outpu,111) 'RNG model'
           if (kfl_kemod==2) write(momod(modul)%lun_outpu,111) 'Realizable model'
           if (kfl_kemod==3) write(momod(modul)%lun_outpu,111) 'ke-f-p model'
