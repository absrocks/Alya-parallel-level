subroutine tur_elmtss(&
  ...
  use def_domain, only       :  ndime,mnode,elmar
  use def_turbul, only       :  nturb_tur,iunkn_tur,kfl_model_tur,&
       &                        param_tur,TUR_SPALART_ALLMARAS,&
       &                        kfl_advec_tur,kfl_taust_tur,staco_tur
  ...
  real(rp)                   :: gpvis(1),gpden(1),gpvno,gpnut,gpfle(1), gptvi(1)
  ...
  !
  if( kfl_prope /= 0 ) then ! kermod in charge of properties
     ...
     gpnut = gptvi(1)/gpden(1)
  else
     call tur_elmpro(&
          pnode,1_ip,lnods,elfle,elmar(pelty)%shacg,hleng,gpfle,&
          gpden,gpvis)
  end if
  !
  ! GPNUT: Model dependent variables
  !
  if(turmu_ker%kfl_exist==0) then ! turbul not from kernel (matias)
     gpnut = 0.0_rp
     if(TUR_SPALART_ALLMARAS) then 
        smole = 1.0_rp/param_tur(3)
        sturb = smole
        do inode=1,pnode        
           gpnut = gpnut+eltur(1,inode)
        end do
        gpnut = rnode*gpnut
     else 
        smole = 1.0_rp
        ...
        do inode=1,pnode        
           gpnut = gpnut+eledd(inode)
        end do
        gpnut = rnode*gpnut/gpden(1)
     end if
  else ! turbul from kernel
     if(TUR_SPALART_ALLMARAS) then 
        smole = 1.0_rp/param_tur(3)
        sturb = smole
     else 
  ...
  rea = 0.0_rp    
  call tauadr(&
       kfl_taust_tur,staco_tur,adv,dif,rea,&
