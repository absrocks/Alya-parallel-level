...
subroutine tur_stdkep(&
  ...
  use def_kermod, only     :  cmu_st,turmu_ker
  ...
  real(rp)                 :: lm, gpepo, gpkio, alpha, onerp, react,reac2, reac_lhs,cmu0, grugr, reac3, canopy
  real(rp)                 :: reguk, regue, drdk, drde,drdko, drdeo, reguko, regueo, difco
  real(rp)                 :: d2rdk, d2rde, rfunc, dfdk, dfde, d2fdk, d2fde, df2dk, df2de, df3de
  ...
  if (kfl_lmaxi_tur==0) then ! constant max mixing length
     inv_lmax =  inv_l_max
  else                       ! mellor yamada
     if (gplmax.gt.1.0_rp) then
        inv_lmax =  1.0_rp/gplmax
     else
        inv_lmax =  1.0_rp/1.0_rp
     end if
     
  end if
!  if (turmu_ker % kfl_exist==0) then
     gpmut(1) = max(cmu*gpkin*gpkin/gpeps*gpden, gpvis)! matias
     gpmut(2) = max(cmu*gpkio*gpkio/gpepo*gpden, gpvis)! matias
!    if (kfl_kemod==2.or.kfl_kemod==3 ) cmu = gpmut(1)*gpeps/(gpden*gpkin*gpkin) 
!  end if
  gppro    = gpmut(2) * gppr2
  if (kfl_regularization) then
     reguk  = regul_k(gpkin)
  ...
  difco = 1.0_rp ! diffusion coefficient
     ...
     if( iunkn_tur == 1 ) then  ! TKE equation
        if (.not.kfl_regularization ) then
           ...
           reac2 = (gppro +  gpden*(epsam))   *df2dk
           gprea = react + reac2
           sreac = react + reac2
           gprhs = (gppro +  gpden*(epsam))/dregularization(gpkin)  &
          !      -  gpden*regue/dregularization(gpkio) &
           ...
           gprea = react 
           sreac = react
           gprhs = gppro  - gpden*gpden*Cmu/gpmut(2)*rfunc &
                + react*gpkin &
...
!                + gpden* dtinv_tur*(drdk*gptur(1,3) )
        ...
        if (.not.kfl_regularization ) then
           ...
           if (.not.kfl_second) then ! 
              ...
              reac2 = (C1p*Cmu*gpden*reguk* gppr2  + Ce2*gpden *epsam*epsam/keyam)*df2de
              canopy= 12.0_rp*(Ce1-Ce2)*sqrt(Cmu)*gpcan
              reac3 = canopy*df3de
              ! reactive term (linearized)
              gprea = react + reac2 + reac3 
              sreac = react + reac2 + reac3
              gprhs = (C1p*Cmu*gpden*reguk* gppr2 + Ce2*gpden*epsam*epsam/keyam)/drde &
              ...
              gprea = react + reac3
              sreac = react + reac3
              gprhs = C1p*Cmu*gpden*reguk* gppr2  - Ce2 * gpden / reguk*rfunc &
                   +  (react + reac3) *gpeps - canopy*regue &
                   + gpden*dtinv_tur*(drde*gpeps + regul_e(gptur(2,3)) - regue)
!                   + gpden*dtinv_tur*(drde*gptur(2,3)) ! + regularization(gptur(2,3)) - regue)
