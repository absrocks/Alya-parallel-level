subroutine tur_updbcs(itask)
  ...
  real(rp)                :: dummy,bvaux, delta, reguk, alpha
  ...
  if(TUR_K_EPS_STD) Cmu = param_tur(6)
  ...
  turbulent_variables: do iunkn=i1,i2
     ...
     if( itask==TUR_BEFORE_TIME_STEP .and. kexist_tran_fiel > 0 ) then   ! needs checkin if done only before time step is enough
        do ipoin=1,npoin
           if( kfl_funno_tur(ipoin,iunkn) > interval_funno ) then 
              ifunc = kfl_funno_tur(ipoin,iunkn) - interval_funno
     ...
     if( itask==TUR_BEFORE_TIME_STEP .and. kfl_exist_fixi7_tur==1 ) then   ! needs checkin if done only before time step is enough
        ...
        call open_close(2,kfl_fixno_tur(1,1,iunkn),dummy,1_ip)   ! The last parameter (1_ip) corresponds to the first dimension of kfl_fixno_tur
     end if
     ...
     nodes: do ipoin=1,npoin
        ...
        type_bc: if(kfl_fixno_tur(1,ipoin,iunkn) == 3.and.itask==BEFIT) then
           ...
           call frivel(y,rough_dom,tveno,nu,ustar)
           ...
           if(TUR_SPALART_ALLMARAS) then
              ...
              !
              if (kfl_logva==0) then
                 if (kfl_ustar==2.and.iunkn==2) then
              ...
              else ! logarithmic variables 
                 if (kfl_ustar==2) then
                    ! ustar ^2 in terms of k
                    ustar2 = exp(untur(1,ipoin,1))*sqrt(Cmu)         ! u*=k^0.5*cmu^0.25      
                    y   = walld(ipoin)
                    ...
                    !                  when velocity increases
                    eps = ustar2*ustar/(kap*(y+rough_dom))     ! eps=U*^3/(kappa*y)=Cmu^{3/4}*k^{3/2}/(kap*y)                      
!                    bvess_tur(1,ipoin,2) = log(eps) ! max(eps,xsmal)
                    bvess_tur(1,ipoin,2) = log (Cmu**(0.75_rp)/(kap*(y+rough_dom))) + 1.5_rp*untur(1,ipoin,1)
                 else                    
                    y   = walld(ipoin)
                    k   = ustar*ustar/sqrt(Cmu)                           ! k=U*^2/sqrt(Cmu)
                    eps = ustar*ustar*ustar/(kap*(y+rough_dom))           ! eps=U*^3/(kappa*y)=Cmu^{3/4}*k^{3/2}/(kap*y) 
                    if(iunkn==1) bvess_tur(1,ipoin,1) = log(k) 
                    if(iunkn==2) bvess_tur(1,ipoin,2) = log(eps)
                 end if
              end if
              ...
              !
              y   = walld(ipoin)
              k = ustar*ustar/sqrt(bs)                              ! k=U*^2/sqrt(B*)
              w = ustar/(sqrt(bs)*kap*y)                            ! w=U*/(sqrt(beta*)*kap*y)
           ...
           if( kfl_funno_tur(ipoin,iunkn) < 0 ) then 
           ...
           !
           ! Open of close point depending on angle between normal and VELOC
           !
