subroutine tur_frivel()
  ...
  if( kfl_ustar_tur/=0 .and. INOTMASTER ) then
     ...
     call gradie(veloc(1:ndime,1:npoin,1),gevec)
        ...
        do ipoin=1,npoin
           ...
           if(ibopo>=1) then
              ...
              if(kfl_fixno_tur(1,ipoin,1)==3) then
                 ...
                 !
                 nu = mu(1)/rho(1)
                 call vecnor(veloc(:,ipoin,1),ndime,tveno,2_ip)
                 if( kfl_rough > 0 ) rough_dom = rough(ipoin) 
                 if (kfl_delta == 1 ) then
                    call frivel(ywalp(ibopo),rough_dom,tveno,nu,ustar_tur(ibopo))
                 else
                    call frivel(delta_tur,rough_dom,tveno,nu,ustar_tur(ibopo))
                 end if
              else
                 !
