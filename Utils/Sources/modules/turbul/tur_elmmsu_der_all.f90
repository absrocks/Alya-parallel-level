subroutine tur_elmmsu_der_all(&
  ...
  use def_domain, only       :  ndime,ntens,mnode,lnods
  use def_turbul, only       :  nturb_tur,iunkn_tur,kfl_timei_tur,&
       &                        kfl_weigh_tur,dtinv_tur,kfl_advec_tur, kfl_taust_tur, staco_tur,param_tur,Rhsadjtur_tur,&
                                TUR_SPALART_ALLMARAS, kfl_fixno_tur
  ...
  integer(ip)                :: inode,jnode,idime,igaus,jdime,ipoin,jpoin,iturb_tur
  real(rp)                   :: fact1,fact2,fact3, resid(pnode), resid_sou(pnode), gpper(pnode) , uscoe
  real(rp)                   :: xmuit, tau, gpadv(pnode), gpnve, grvgr, resi2(pnode)
  real(rp)                   :: rhnve, gpad1(pnode), rresi, ugrau, grtur(ndime), grnor,  xmui3
  real(rp)                   :: rhnvv, auxve(ndime)
