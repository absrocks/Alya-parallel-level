subroutine tur_iniunk()
  ...
  implicit none
  integer(ip)          :: ipoin,iturb,kfl_value,dummi,ifunc,idime
...
!  avtvi_tur = 0.0_rp
  ...
  if( kfl_rstar == 0 ) then
        ...
        if( TUR_SPALART_ALLMARAS ) then
           ...
           if( INOTMASTER ) then 
              do ipoin = 1,npoin
                 if(  kfl_fixno_tur(1,ipoin,1) <= 0 .or.&
                    ...
                    untur(1,ipoin,nprev_tur) = nup
                 else
                    untur(1,ipoin,nprev_tur) = bvess_tur(1,ipoin,1)
                 end if
              end do
           end if
           ...
           if( INOTMASTER ) then
              ...
              if( kfl_inifi_tur(1) > 0 ) then
                 ...
                 do iturb = 1,nturb_tur
                    do ipoin = 1,npoin
                       untur(iturb,ipoin,nprev_tur) = xfiel(-nfiel_tur(iturb))%a(1,ipoin,1)
                    end do
                 end do
                 ...
                 do ipoin = 1,npoin
                    do iturb = 1,nturb_tur
                       untur(iturb,ipoin,nprev_tur) = bvess_tur(1,ipoin,iturb)
                    end do
                 end do 
                 ...
                 do iturb = 1,nturb_tur
                    ...
                    do ipoin = 1,npoin
                       untur(iturb,ipoin,nprev_tur) = xfiel(kfl_value) % a(1,ipoin,1)
                    end do
                 end do
                   ...
                   if( ifunc > 0 ) then
                      do ipoin = 1,npoin
                         call ker_space_time_function(&
                              ifunc,coord(1,ipoin),coord(2,ipoin),coord(ndime,ipoin),cutim, untur(iturb,ipoin,nprev_tur))
                      end do
                   end if
                end do
                 ...
                 do ipoin = 1,npoin
                    do iturb = 1,nturb_tur
                       if(  kfl_fixno_tur(1,ipoin,iturb) <= 0 .or.&
                          ...
                          untur(iturb,ipoin,nprev_tur) = turbm(iturb)
                       else 
                          untur(iturb,ipoin,nprev_tur) = bvess_tur(1,ipoin,iturb)
                       end if
                    end do
        ...
        if( INOTMASTER ) then
           ...
           if (turmu_ker%kfl_exist==0) then !(matias)
              call tur_updedd()    
!           else
!              call ker_updpro()
           end if
...
!     if( INOTMASTER ) call tur_memarr(1_ip)  esta linea lo jodia todo 
  ...
  if (INOTMASTER.and. kfl_rstar == 0) then
     ...
     if (kfl_logva==1) then
        untur(1:nturb_tur,1:npoin,nprev_tur) = log (untur(1:nturb_tur,1:npoin,nprev_tur))
        call tur_updunk(10_ip)
...
!        do iturb = 1, nturb_tur
           do ipoin = 1, npoin 
              untur(1,ipoin,nprev_tur) = inv_regul_k(untur(1,ipoin,nprev_tur))
              untur(2,ipoin,nprev_tur) = inv_regul_e(untur(2,ipoin,nprev_tur))
           end do
!        end do
