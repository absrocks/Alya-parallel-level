subroutine tur_elmgat(&
  ...
  use def_master, only     :  untur,turmu,tempe,advec,fleve,velom,kfl_coupl,&
       &                      ID_TURBUL,ID_ALEFOR,veloc_forw,untur_forw, TUR_K_EPS_STD
  use def_domain, only     :  ndime,ntens,mnode,coord,walld
  use def_turbul, only     :  kfl_ustar_tur,kfl_walld_tur,kfl_timei_tur,&
       ...
       &                      unpgr_tur,nbdfp_tur, tur_max_mixlen
  use def_kermod, only     :  kfl_adj_prob
  implicit none
  integer(ip), intent(in)  :: pnode
  integer(ip), intent(in)  :: lnods(pnode)
  real(rp),    intent(out) :: eltur(nturb_tur,pnode,3)
