subroutine tur_elmco2(&
  ...
  use def_master, only     :  kfl_coupl,ID_TURBUL,ID_ALEFOR,TUR_K_EPS_STD
  ...
  use def_turbul, only     :  nturb_tur,iunkn_tur,kfl_timei_tur,&
       &                      kfl_cotem_tur,kfl_grve2_tur,kfl_model_tur,&
       ...
       &                      TUR_K_EPS_PHI_F,TUR_SST_K_OMEGA, kfl_kemod, &
       &                      kfl_discd_tur, zetur, &
  ...
  real(rp)                 :: gpust,gphev,gppr2,gpsqk,gpgrp(3),gplap, eta, seci4, A0, W, divve, simgr(3,3), grunk(ndime, 2), grtur(nturb_tur, ndime, 2)
  real(rp)                 :: a1, F2, Cr, f0, f02, fact,  Wsqr6, turvi(2)
  ...
  if( turmu_ker % kfl_exist == 0 ) then
     gpmut = 0.0_rp
     do inode = 1,pnode
        gpmut(1) = gpmut(1) + gpsha(inode) * eledd(inode)
     end do
     gpmut(2) = gpmut(1)  ! to correct error introduced in 2046 that left gpmut(2) undefined but used for all models /= k-eps
  end if
  !
...
!  end do
  ...
  if( TUR_K_EPS_STD) then
     !.and.turmu_ker % kfl_exist == 0) then ! only if properties are not loaded from kernel.
     if (kfl_kemod==2.or.kfl_kemod==3) then ! Realizable or kefp model
        A0 = 4.04_rp 
        ...
        if (kfl_kemod==2) then      ! realizable
           cmu =1.0_rp/(A0+As*gptur(1,2)/gptur(2,2)*uaste)
        else if (kfl_kemod==3) then ! kefp model
           Cr = 4.5_rp
           f0 = Cr/(Cr-1.0_rp)
...
!                    +  gpgrv(jdime, idime))
  ...
  else  if (turmu_ker % kfl_exist==1) then ! properties from ker_proper                                
     gpmut(1) = gptvi
     gpmut(2) = gptvi
...
!!$     end do
  ...
  if( .not. TUR_SPALART_ALLMARAS ) then
     if( kfl_produ_tur == 0 ) then
        ...
        gppro = gpmut(2) * gppr2
        eta = sqrt(gppr2)*gptur(1,2)/gptur(2,2)
     else
        !
        ! GPPRO = 2*mut*Sij*Sij
        !
  ...
  ! Orthogonal SGS
  !
  if( kfl_ortho_tur == 1 ) then ! FULL OSS
     gpprj = 0.0_rp
     do inode = 1,pnode
  ...
  if( TUR_SPALART_ALLMARAS ) then
     ...
     call tur_stdkep(& 
          ...
          gpgrd, kfl_kemod, eta, cmu, gpcan, sreac, &
          pmate, grunk, conve, gplmax)

     gpvel(1:ndime)= conve(1:ndime) ! stabilizes convective terms
