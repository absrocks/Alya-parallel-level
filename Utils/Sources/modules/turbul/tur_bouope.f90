subroutine tur_bouope()
  ...
  use mod_ker_proper 
  use def_domain
  use def_turbul, only : kfl_fixbo_tur, kfl_advec_tur, iunkn_tur, bvnat_tur, &
...
!  real(rp)    :: bovel(ndime,mnodb)
  ...
  boundaries: do iboun=1,nboun
     Neumann_or_Robin: if(kfl_fixbo_tur(iboun) == 20) then  ! 
...
!!$           end if
           ...
           gauss_points: do igaub=1,ngaus(pblty)
              ...
              if( kfl_fixbo_tur(iboun) == 20 ) then
                 !
...
!!$                    end if
                    if (kfl_geono(lpoty(ipoin))==20) then ! outflow bc
                       if(udotn.lt.0.0) then ! incoming flow
                          fixbc(ipoin) =1
                          kfl_fixno_tur(1,ipoin, iunkn_tur) =1
