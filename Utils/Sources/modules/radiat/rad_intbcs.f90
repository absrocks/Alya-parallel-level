...
!!$        allocate(vbmas(nbopo),stat=istat)
!!$        call memchk(zero,istat,mem_modul(1:2,modul),'VBMAS','rad_intbcs',vbmas)
!!$        allocate(rtbck(nboun),stat=istat)
!!$        call memchk(zero,istat,mem_modul(1:2,modul),'RTBCK','rad_intbcs',rtbck)
!!$        allocate(itbck(nboun),stat=istat)
!!$        call memchk(zero,istat,mem_modul(1:2,modul),'ITBCK','rad_intbcs',itbck)
!!$        !
!!$        ! Reads boundary conditions from file
!!$        ! 
!!$        lispa = 0
!!$        lisda = lun_intbc_rad  ! Reading file 
!!$        lisre = momod(modul)%lun_outpu  ! Writing file
!!$        words(1)=''
!!$        param(1)=-999999.0_rp
!!$
!!$        do while(words(1)/='TIME ')
!!$           call ecoute('rad_intbcs')
!!$        end do
...
!!$        !end do
!!$        call ecoute('rad_intbcs')
!!$        if(iknbo_rad==1) then                   ! Unknown boundary
...
!!$                 call bouder(&
!!$                      pnodb,ndime,ndimb,elmar(pblty)%deric(1,1,inodb),&
!!$                      bocod,baloc,eucta)
...
!!$        !
!!$        do ipoin=1,npoin
!!$           if(kfl_fixno_rad(1,ipoin)==5) then
!!$              ibopo=lpoty(ipoin)
!!$              if(ibopo/=0) then
...
!!$                 if(kfl_timei_rad/=0.and.kfl_tiacc_rad==2.and.kfl_tisch_rad==1) then
!!$                    teold=tempr(ipoin,3)
!!$                    bvess_rad(ipoin,1)=0.50_rp*tenew+0.50_rp*teold
!!$                 else
!!$                    bvess_rad(ipoin,1)=tenew                 
!!$                 end if
!!$              end if
!!$              end if
!!$           end if
...
!!$        call memchk(two,istat,mem_modul(1:2,modul),'VBMAS','rad_intbcs',vbmas)
!!$        deallocate(vbmas,stat=istat)
!!$        if(istat/=0) call memerr(two,'VBMAS','rad_intbcs',0_ip)     
!!$        call memchk(two,istat,mem_modul(1:2,modul),'RTBCK','rad_intbcs',rtbck)
!!$        deallocate(rtbck,stat=istat)
!!$        if(istat/=0) call memerr(two,'RTBCK','rad_intbcs',0_ip)     
!!$        call memchk(two,istat,mem_modul(1:2,modul),'ITBCK','rad_intbcs',itbck)
!!$        deallocate(itbck,stat=istat)
!!$        if(istat/=0) call memerr(two,'ITBCK','rad_intbcs',0_ip)     
!!$
!!$     end select
!!$
!!$  end if

end subroutine rad_intbcs
