subroutine lev_reanut
  ...
  if( INOTSLAVE ) then
     ...
     sstol_lev     = 1.0e-8                           ! Steady state tolerance
     cotol_lev     = 0.1_rp                           ! Internal tolerance
     cpuit_lev     = 0_rp                             ! CPU time per iteration
     supgp_lev     = 1_rp                             ! SUPG stabililization 
     ...
     !
     if( iredi == 1 .and. solve_sol(2)%kfl_algso==-999 ) then
        solve_sol => solve(1:)
     ...
     !    
     if(solve_sol(3)%kfl_algso==-999) then  
        solve_sol => solve(1:)
