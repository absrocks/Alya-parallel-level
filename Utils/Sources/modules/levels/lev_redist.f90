subroutine lev_redist
  ...
  call livinf(81_ip,'GEOMETRICAL REDISTANTIATION',0_ip)
  ...
  if( kfl_timin == 1 ) call Parall(20_ip)
  call cputim(timr1)
  if( INOTSLAVE .and.  kfl_timin == 1 ) then
  ...
  call parari('MIN',0_ip,1_ip,prfin)

  if( INOTSLAVE ) then
     if(prfin(1)==1_ip) then
        if(ipass==0) then
           if(kfl_rstar/=2) then
              ipass=1
...
subroutine lev_cristo()
  ...
  if( INOTMASTER ) then
     do ipoin = 1,npoin
        ...
        !call dpopar(&
        !     1_ip,xcoor,npoin_lev,mnodb_lev,nboun_lev,chkdi,ltypb_lev,&
