...
use def_master, only:kfl_paral
  ...
  subroutine pts_transport_single_particle(ilagr,nlagr_non_migrating_pts,nlagr_migrating)
    ...
    real(rp)                  :: visfl,denfl,diame,Du,Cd                  ! Drag
    real(rp)                  :: spher
    ...
    Cc                = 1.0_rp                      ! No Cunningham correction
    ...
    ! 2. Particle go out of computational domain (depodited or outflow)
...
20     continue
       ...
       if( ( dt_k < 0.1_rp * dtmin_pts .or. itint > itmax ) .and. iwall /= 1 ) then
          ...
          if( .not. newmark_converged ) then                 
             print *, 'Newmark did not converge after all iteraitons. Particle id=', lagrtyp(ilagr) % ilagr,'; coords=',lagrtyp(ilagr) % coord(1:ndime)
          end if
       end if

       newmark_converged = .true.
  ...
  subroutine pts_transport_finalize()
    ...
    integer(ip)                  :: ilagr,ielem,iboun,klagr,dummi(4)
    ...
    call PAR_SUM(4_ip,dummi,'IN MY ZONE')
