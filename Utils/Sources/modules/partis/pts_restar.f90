...
subroutine pts_restar(itask)
  ...
  integer(ip)             :: kfl_gores,ipart,ilagr,ipars,isize
     ...
     if( kfl_rstar == 2 ) then
        ...
        if( IMASTER ) then
           ...
           do ipart = 1,npart              
              ...
              call PAR_SEND(cutla_pts,'IN MY CODE',ipart)
           ...
           call PAR_RECEIVE(cutla_pts,'IN MY CODE',ipart)
  ...
  case ( WRITE_RESTART_FILE )
     ...
     if( IPARALL ) then
        num_particles4 = 0_4
        if( IMASTER ) then
           call memory_alloca(mem_modul(1:2,modul),'NUM_PARTICLES_GAT4','pts_output',num_particles_gat4,int(npart+1_ip,4),LBOUN=0_4)
        else if( ISLAVE ) then
           num_particles4 = count( lagrtyp(1:mlagr) % kfl_exist == -1 ) * nvar2_4
           call memory_alloca(mem_modul(1:2,modul),'PARTICLES_SEND','pts_output',particles_send,int(num_particles4,ip))
           ipars = 0
           do ilagr = 1,mlagr
        ...
        if( IMASTER ) then
           ...
           call memory_alloca(mem_modul(1:2,modul),'PARTICLES_GAT','pts_output',particles_gat,num_particles4)
        end if
        !
        ! MPI_Gatherv  
        !
