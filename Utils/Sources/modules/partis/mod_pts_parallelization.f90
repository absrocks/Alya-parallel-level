...
module mod_pts_parallelization
  ...
  use def_partis,         only : kfl_posla_pts
...
use def_master, only: kfl_paral
    type(comm_data_par), pointer, intent(in)    :: commu               !< Communicator
    integer(ip),                  intent(inout) :: nlagr_migrating(:)  !< Number of particles to send
    integer(ip),                  intent(inout) :: particles_sent      !< Number of particles sent
    integer(ip),                  intent(inout) :: particles_recv      !< Number of particle received
    ...
    !
    do ineig = 1,nneig
       particles_sent = particles_sent + nlagr_migrating(ineig) 
       particles_recv = particles_recv + nlagr_receiving(ineig)
    end do
