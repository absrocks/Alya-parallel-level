...
!  use mod_random
  ...
  real(rp)             :: t,z,r,radius,rad,xe,ye,ze,rr,hh,alpha
  integer(ip)          :: nside,nlagr_new,nlagr_pos,itype
  integer(ip)          :: number_injected_pts,num_types_injected
  ...
  do iinj = 1,size(kfl_injla_pts)
     ...
     if( kfl_injla_pts(iinj) == 1 ) then
       ...
       !! print*,'holaaaaaaa-semi-esfere-num_inject',  nside ,ndime ,num_types_injected,2 * nside ** ndime * num_types_injected
        ...
        ! Pointwise injector
        !
        number_injected_pts = number_injected_pts + 1 * num_types_injected
       !! print*,'number_injected_pts',number_injected_pts
        ...
        !
        nside = int(parla_pts(iinj,7),ip)
        number_injected_pts = nside  * num_types_injected      
     end if
...
!!print*,'PASO 1------------------------------bucle iinj',size(kfl_injla_pts)
     do iinj = 1,size(kfl_injla_pts)
        ...
        if( kfl_injla_pts(iinj) == 1 ) then
           ...
           if( kfl_injty_pts(iinj) > 0 ) then
              do ii = 1,nside
                 nlagr_pos                                  = nlagr_pos + 1
                 particle_position(1,nlagr_pos)             = xmini + real(ii,rp)/real(nside,rp)*(xmaxi-xmini)
                 particle_position(2,nlagr_pos)             = ymini + real(ii,rp)/real(nside,rp)*(ymaxi-ymini)
                 if(ndime==3)particle_position(3,nlagr_pos) = zmini + real(ii,rp)/real(nside,rp)*(zmaxi-zmini)             
                 particle_injector(nlagr_pos)               = iinj
                 particle_type(nlagr_pos)                   = kfl_injty_pts(iinj)
              end do
              ...
              do itype = 1,ntyla_pts
                 if( parttyp(itype) % kfl_exist /= 0 ) then
                    do ii = 1,nside
                       nlagr_pos                                  = nlagr_pos + 1
                       particle_position(1,nlagr_pos)             = xmini + ii/nside*(xmaxi-xmini)
                       particle_position(2,nlagr_pos)             = ymini + ii/nside*(ymaxi-ymini)
                       if(ndime==3)particle_position(3,nlagr_pos) = zmini + ii/nside*(zmaxi-zmini)             
                       particle_injector(nlagr_pos)               = iinj
                       particle_type(nlagr_pos)                   = itype
                    end do
                 end if
