...
subroutine pts_output()
  ...
  if( kfl_ifpos == 1 .or. kfl_ifdep == 1 ) then
     ...
     if( IPARALL ) then
        ...
        call memory_alloca(mem_modul(1:2,modul),'PERM_POSTPROCESS','pts_inivar',perm_deposition, nvard_pts)
  ...
  call memory_deallo(mem_modul(1:2,modul),'PERM_POSTPROCESS'  ,'pts_output',perm_deposition)
...
subroutine pts_output_header()
  ...
  if( nvarp_pts > 0 ) then
     if (kfl_ptsres_binary==0) then
        ...
        write(lun_resul_pts,1,ADVANCE='NO') (adjustr(trim(postprocess_name_pts(postprocess_list_pts(ivarp)))),ivarp=1,nvarp_pts)
