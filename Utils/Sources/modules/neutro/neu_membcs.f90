...
subroutine neu_membcs(itask)
  ...
  use def_neutro, only : kfl_fixbo_neu
     ...
     if( INOTMASTER ) then
        ...
        do idirection = 1,num_directions_neu
           do ienergy = 1,num_energies_neu
              call memory_alloca( mem_modul(1:2,modul),'KFL_FIXNO_NEU','neu_membcs',kfl_fixno_neu(ienergy,idirection) % l,1_ip,npoin )  
              call memory_alloca( mem_modul(1:2,modul),'KFL_FIXBO_NEU','neu_membcs',kfl_fixbo_neu(ienergy,idirection) % l,nboun      )  
           end do
        end do
        !
        ! Values
        ...
        do idirection = 1,num_directions_neu
           do ienergy = 1,num_energies_neu
              call memory_alloca( mem_modul(1:2,modul),'BVESS_NEU','neu_membcs',bvess_neu(ienergy,idirection) % a,1_ip,npoin,1_ip) 
              call memory_alloca( mem_modul(1:2,modul),'BVNAT_NEU','neu_membcs',bvnat_neu(ienergy,idirection) % a,1_ip,nboun,1_ip) 
           end do
        end do
