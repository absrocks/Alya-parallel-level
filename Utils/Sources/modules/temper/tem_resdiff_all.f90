subroutine tem_resdiff_all(&
  ...
  use def_kermod, only       :  gasco,kfl_dvar_type
  ...
  integer(ip) :: inode,igaus,idesvar
...
subroutine tem_elmmuldiff(&
  ...
  use def_master, only       :  prthe,design_vars
...
!   use def_temper, only       :  kfl_regim_tem
  ...
  real(rp)                   :: fact1,fact2,fact3, resid(pnode, pgaus), gpper(pnode), uscoe
  real(rp)                   :: xmuit, gpadv, gpnve, grvgr, resi2(pnode), sreac(pgaus),grtem(ndime)
  real(rp)                   :: rhnve, gpad1(pnode), rresi, grnor,  xmui3, gplap
  integer(ip)                :: itime, itsgs
  real(rp)                   :: Dsupg,umbra, CD, SD, factt            ! Shock capturring variables
  real(rp)                   :: gpsta(2), gpstt
  real(rp)                   :: lapla, dtite, ugrat, grkgr, rhsgs
  real(rp)                   :: error, densi(4), tesgs, tempt, tesin
  real(rp)                   :: gprhs_sgs(pgaus), elmatdiff(pnode,pnode),elrhsdiff(pnode),gprhsdiff(pgaus)
  !
  ! Initialization
  !
...
subroutine tem_elmmuldiff_cond(&
  ...
  use def_temper, only       :  kfl_regim_tem,kfl_timei_tem
  use mod_tauadr, only       :  tauadr
  ...
  real(rp),    intent(in)    :: eltem_forw(pnode)
  real(rp),    intent(out)   :: elresdiff(pnode)
  integer(ip)                :: inode,jnode,idime,igaus,ipoin
  real(rp)                   :: fact1,fact2,fact3, resid(pnode, pgaus), gpper(pnode), uscoe
  real(rp)                   :: xmuit, gpadv, gpnve, grvgr, resi2(pnode), sreac(pgaus),grtem(ndime)
  real(rp)                   :: rhnve, gpad1(pnode), rresi, grnor,  xmui3, gplap
  integer(ip)                :: itime, itsgs
