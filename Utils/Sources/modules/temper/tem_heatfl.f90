subroutine tem_heatfl(gradt)
  ...
  real(rp)                         :: xjaci(9),xjacm(9),gpdif,gpcon(1),gpsph(1),dummr(mnode*ndime)
  real(rp)                         :: gptem(mnode),fact1,fact2
  real(rp)                         :: gptur(2),gpden(2)
  ...
  if(kfl_paral/=0) then
     ...
     elements: do ielem = 1,nelem
        ...
        gauss_points: do inode = 1,pnode
           !
           !  Properties: GPCON (k) and GPSPH (Cp)
           ! 
           ...
           call ker_proper('SPHEA','IGAUS',1_ip,ielem,gpsph,pnode,1_ip,elmar(pelty)%shapc(:,inode))
           call ker_proper('TURBU','IGAUS',1_ip,ielem,gptur,pnode,1_ip,elmar(pelty)%shapc(:,inode))

           call tem_turbul(&
                ielem,pnode,1_ip,1_ip,1_ip,elmar(pelty)%shapc(1,inode),gpcon,gpsph,gpdif,dummr,gpden,gptur)
