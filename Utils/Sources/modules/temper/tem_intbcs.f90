subroutine tem_intbcs()
  ...
  if(kfl_paral==-1) then
     ...
     case(2)
        ...
        call memchk(zero,istat,mem_modul(1:2,modul),'VBMAS','tem_intbcs',vbmas)
        allocate(rtbck(nboun),stat=istat)
        call memchk(zero,istat,mem_modul(1:2,modul),'RTBCK','tem_intbcs',rtbck)
        allocate(itbck(nboun),stat=istat)
        call memchk(zero,istat,mem_modul(1:2,modul),'ITBCK','tem_intbcs',itbck)
        !
        ! Reads boundary conditions from file
        ! 
        lispa = 0
        lisda = lun_intbc_tem  ! Reading file 
        ...
        call memchk(two,istat,mem_modul(1:2,modul),'VBMAS','tem_intbcs',vbmas)
        deallocate(vbmas,stat=istat)
        if(istat/=0) call memerr(two,'VBMAS','tem_intbcs',0_ip)     
        call memchk(two,istat,mem_modul(1:2,modul),'RTBCK','tem_intbcs',rtbck)
        deallocate(rtbck,stat=istat)
        if(istat/=0) call memerr(two,'RTBCK','tem_intbcs',0_ip)     
        call memchk(two,istat,mem_modul(1:2,modul),'ITBCK','tem_intbcs',itbck)
        deallocate(itbck,stat=istat)
        if(istat/=0) call memerr(two,'ITBCK','tem_intbcs',0_ip)     
