subroutine tem_updtss()
  ...
  real(rp)         :: eledd(ndime,mnode),elDik(ndime,mnode)
  real(rp)         :: eltem(mnode,ADR_tem % ntime)
  real(rp)         :: elmsh(ndime,mnode)
  real(rp)         :: gpden(2),gpcon(2),gpdif(2)
  real(rp)         :: gpsph(2),gptur(2),gprea(2)
  real(rp)         :: gpvel(3),gpvol(2),gpgrd(3)
...
#endif
     call tem_updunk(7_ip) ! Compute density for compressible flow
     ...
     if( INOTMASTER ) then
        ...
        do ielem = 1,nelem
           ...
           if( pelty > 0 ) then
              ...
              !
              call tem_elmgat(&
                   ielem,pnode,lnods(:,ielem),eltem,elvel,elcod,eledd,elDik,elmsh)
              !
              ...
              !
              call tem_turbul(& 
                   ielem,pnode,1_ip,1_ip,1_ip,elmar(pelty)%shacg,gpcon,gpsph,gpdif,gpgrd,gpden,gptur)
              !
              ! Reaction
              !
