subroutine tem_elmtss(&
  ...
  use def_domain, only       :  mnode,ndime,elmar,lnods
  use def_master, only       :  wmean,kfl_coupl,chemical_heat,div_enthalpy_transport,&
       &                        ID_TEMPER,ID_CHEMIC
  use def_temper, only       :  kfl_cotur_tem,kfl_advec_tem,&
       &                        kfl_sgsno_tem,prtur_tem,&
       &                        kfl_taust_tem,staco_tem,kfl_regim_tem,&
       &                        safet_tem,source_safet_tem
  use def_kermod
  use mod_ker_proper
  ...
  !
  if(kfl_cotur_tem > 0) then   ! RANS modelling
     do inode=1,pnode
        gptur(1) = gptur(1) + eledd(inode)
     end do
     gptur(1) = rnode*gptur(1)
  else                         ! LES modelling
  ...
  call tauadr(&
       kfl_taust_tem,staco_tem,adv,dif,rea,&
  ...
  if(kfl_coupl(ID_TEMPER,ID_CHEMIC)==1) then
     ...
     if (div_enthalpy_transport(ielem)%a(1) + chemical_heat(ielem)%a(1) /= 0.0_rp) then
        chtim = source_safet_tem*rocpT/(abs(div_enthalpy_transport(ielem)%a(1)+ chemical_heat(ielem)%a(1)))
     else
        chtim=1.0e12_rp
     endif
