...
!
  if(kfl_outfo==1) then
     ...
     call iofile(zero,lun_rares_tem,fil_rares_tem,'POST-PROCESS')     
      ...
      CALL GID_OPENPOSTRESULTFILE(fil_rares_tem,0)
  else if(kfl_outfo==4) then
      CALL GID_OPENPOSTRESULTFILE(fil_rares_tem,2)
  end if
     ...
     do iview=1,10
        ...
        if(iboun>=1) then
           ...
           if(kfl_outfo==1) then
              ...
              flush(lun_rares_tem)
              !else if(kfl_outfo==2) then
              !   write(lun_rares_tem,710) 100,'C','Step  ',ttime,1,ittim
              !   write(lun_rares_tem,720) -4,wopos,1,1,0
              !   write(lun_rares_tem,730) -5,wopos,1,1
              !   do ipoin=1,npoin
              !      write(lun_rares_tem,740) -1,ipoin,bridge(ipoin)
              !   end do
              !   write(lun_rares_tem,750) -3
              !   flush(lun_rares_tem)
              !else if(kfl_outfo==3.or.kfl_outfo==4) then
              !   call GID_BEGINSCALARRESULT(wopos,state,ttime,0,NULL,NULL,NULL)
              !   do ipoin=1,npoin
              !      CALL GID_WRITESCALAR(ipoin,bridge(ipoin))
              !   end do
              !   CALL GID_ENDRESULT
              !end if
           end if
