...
**************************************************************************/
void Balance2Way(CtrlType *ctrl, GraphType *graph, my_int *tpwgts, float ubfactor)
{
  my_int i, j, nvtxs, from, imax, gain, mindiff;
  idxtype *id, *ed;
  ...
  if (mindiff < 3*(graph->pwgts[0]+graph->pwgts[1])/graph->nvtxs)
    return;
  if (graph->pwgts[0] > tpwgts[0] && graph->pwgts[0] < (my_int)(ubfactor*tpwgts[0]))
    return;
  if (graph->pwgts[1] > tpwgts[1] && graph->pwgts[1] < (my_int)(ubfactor*tpwgts[1]))
    return;
...
**************************************************************************/
void Bnd2WayBalance(CtrlType *ctrl, GraphType *graph, my_int *tpwgts)
{
  my_int i, ii, j, k, kwgt, nvtxs, nbnd, nswaps, from, to, pass, me, tmp;
  idxtype *xadj, *vwgt, *adjncy, *adjwgt, *where, *id, *ed, *bndptr, *bndind, *pwgts;
  idxtype *moved, *perm;
  PQueueType parts;
  my_int higain, oldgain, mincut, mindiff;
...
* is underweight.
**************************************************************************/
void General2WayBalance(CtrlType *ctrl, GraphType *graph, my_int *tpwgts)
{
  my_int i, ii, j, k, kwgt, nvtxs, nbnd, nswaps, from, to, pass, me, tmp;
  idxtype *xadj, *vwgt, *adjncy, *adjwgt, *where, *id, *ed, *bndptr, *bndind, *pwgts;
  idxtype *moved, *perm;
