...
#ifndef NVIDIA_INDEX_BIN_COMMON_SHORT_VALUE_RAW_VOLUME_DATA_IMPORTER_H
#define NVIDIA_INDEX_BIN_COMMON_SHORT_VALUE_RAW_VOLUME_DATA_IMPORTER_H
...
///
class Short_value_raw_volume_data_importer :
    public nv::index::Distributed_discrete_data_import_callback<0xd2fe1e1b,0xc9ef,0x1e49,0xbc,0x51,0x0b,0x23,0x0f,0x81,0xc4,0x82>
{
public:
    /// The raw volume data importer reads 16-bit volume brick data from file.
    ...
    ///
    Short_value_raw_volume_data_importer(
    ...
    Short_value_raw_volume_data_importer();
    virtual ~Short_value_raw_volume_data_importer();
    ...
    virtual mi::base::Uuid subset_id() const { return nv::index::IRegular_volume_brick_uint16::IID(); }
    // -------------------------------------------------------------------------

    /// Normalize the voxel values to the range [0,1]
    virtual void set_range(const mi::math::Vector<mi::Uint16, 2>& range) { m_range = range; }
...
private:
    ...
    mi::math::Vector<mi::Uint16, 2>         m_range;
...
#endif // NVIDIA_INDEX_BIN_COMMON_SHORT_VALUE_RAW_VOLUME_DATA_IMPORTER_H
