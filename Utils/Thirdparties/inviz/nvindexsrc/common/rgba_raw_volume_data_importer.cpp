...
#include "rgba_raw_volume_data_importer.h"

namespace nv {
namespace index_common {

mi::base::Lock Rgba_raw_volume_data_importer::s_file_access_lock;
mi::Uint64     Rgba_raw_volume_data_importer::s_total_read_bytes = 0;
mi::Float64    Rgba_raw_volume_data_importer::s_total_read_time  = 0.0;

//----------------------------------------------------------------------
Rgba_raw_volume_data_importer::Rgba_raw_volume_data_importer(
...
struct B_frame_storages
{
        ...
        const mi::math::Bbox_struct<mi::Sint32, 3>& box)
    {
        ...
        std::map<mi::Uint64, mi::Uint32*>::iterator find = m_b_frames.find(code);
    ...
    void set_b_frame(
        const mi::math::Bbox_struct<mi::Sint32, 3>& box,
        mi::Sint64                                 brick_voxels,
        mi::Uint32*                                brick_data)
    {
        mi::Uint32* b_frame = new mi::Uint32[brick_voxels];
        memcpy(&b_frame[0], &brick_data[0], brick_voxels*sizeof(mi::Uint32));
    ...
    std::map<mi::Uint64, mi::Uint32*>   m_b_frames;
    mi::base::Lock                      m_lock;
};
...
void Rgba_raw_volume_data_importer::add_time_step(const std::string& file)
{
    m_time_steps.push_back(file);
}

mi::Size Rgba_raw_volume_data_importer::estimate(
    const mi::math::Bbox_struct<mi::Sint32, 3>&     bounding_box,
    mi::neuraylib::IDice_transaction*               dice_transaction) const
{
    // This is an 32-bit packed RGBA volume data importer:
    ...
    const mi::Size volume_brick_size = dx * dy * dz * sizeof(mi::Uint32);
    return volume_brick_size;
}

nv::index::IDistributed_data_subset* Rgba_raw_volume_data_importer::create(
    const mi::math::Bbox_struct<mi::Sint32, 3>&     bounding_box,
    mi::Uint32                                      time_step,
    ...
    mi::neuraylib::IDice_transaction*               dice_transaction) const
{
    ...
    mi::Sint64 volume_brick_voxels = dx * dy * dz;
    mi::Size volume_brick_size = volume_brick_voxels * sizeof(mi::Uint32);

    INFO_LOG << "Scalar volume data importer loads '" << src_filename << "', bounds: " << bounds;

    mi::base::Handle<nv::index::IRegular_volume_brick_rgba8ui> volume_brick(factory->create<nv::index::IRegular_volume_brick_rgba8ui>());
    ...
    mi::Uint32* voxel_data_storage = reinterpret_cast<mi::Uint32*>(volume_brick->generate_voxel_storage(bounding_box));
    if (voxel_data_storage == 0)
    {
        ERROR_LOG << "Cannot generate a RGBA typed voxel storage.";
        return 0;
    }
    ...
    {    
        std::ostringstream cache_file_name_str;
        {
            cache_file_name_str << m_dst_folder << "/" << file_name << "_"
                                ...
                                << "_rgba_cache" << ( (!is_b_frame && m_use_difference_encoding) ? "_diff" : "" ) << ".tmp";
        }
    ...
    else
    {
        INFO_LOG << "Using cache file '" << cache_file_name << "'";
        {
            ...
            {
                ERROR_LOG << "Rgba_raw_volume_data_importer::create(): "
        ...
        if(m_use_difference_encoding)
        {
            if(is_b_frame)
            {
                g_b_frames.set_b_frame(bounds, volume_brick_voxels, &voxel_data_storage[0]);
            }
            else
            {
                mi::Uint32* BFRM = g_b_frames.get_b_frame(bounds);
                mi::Uint32* ORIG = &voxel_data_storage[0];
                mi::Uint64 counter = 0;
                for(mi::Sint64 i=0; i<volume_brick_voxels; ++i)
                {
                    // mi::Float32 value = (*BFRM)-(*ORIG);
                    mi::Uint32 value = (*ORIG);
                ...
                WARN_LOG << "Brick cache: " << bounds << " has " << counter << " of " << volume_brick_voxels << " zero differences";
            }
        }
    ...
    if (!in_file)
    {
        ...
        // only contains invalid values mi::Uint32(0).
        ERROR_LOG << "Cannot open volume data file '"  << src_filename << "'. "
                  << "Will create a database element with invalid value '0' (type: RGBA).";
        return NULL;
    }
    ...
    for (mi::Sint32 x = bounds.min.x; x < bounds.max.x; ++x)
    {
        ...
        for (mi::Sint32 y = bounds.min.y; y < bounds.max.y; ++y)
        {
            ...
            mi::Uint64 file_offset= (mi::Uint64)m_size.z * m_size.y * x + m_size.z * y + bounds.min.z;
            ...
            bytes_to_read *= sizeof(mi::Uint32);
            file_offset *= sizeof(mi::Uint32);
    ...
    {
        g_b_frames.set_b_frame(bounds, volume_brick_voxels, &voxel_data_storage[0]);
        // memcpy(&m_previous_b_frame[0], &voxel_data_storage[0], volume_brick_voxels*4);
    }
    ...
    if (m_use_cache)
    {
        ...
        else
        {
            if(!is_b_frame && m_use_difference_encoding)
            {
                ...
                mi::Uint32* DST_ptr  = new mi::Uint32[volume_brick_voxels];
                mi::Uint32* DST  = &DST_ptr[0];
                mi::Uint32* BFRM = g_b_frames.get_b_frame(bounds);
                mi::Uint32* ORIG = &voxel_data_storage[0];
                mi::Uint64 counter = 0;
                for(mi::Sint64 i=0; i<volume_brick_voxels; ++i)
                {
                    mi::Uint32 value = (*BFRM)-(*ORIG);
                    if(value==0u)
                    {
                        counter++;
                    }
                ...
                WARN_LOG << "Brick: " << bounds << " has " << counter << " of " << volume_brick_voxels << " zero differences";

                if (cache_file.write(DST_ptr, 0ll, volume_brick_size) != static_cast<mi::Uint64>(volume_brick_size))
                {
                    ERROR_LOG << "Rgba_raw_volume_data_importer::create(): "
...
//----------------------------------------------------------------------
void Rgba_raw_volume_data_importer::serialize(
...
//----------------------------------------------------------------------
void Rgba_raw_volume_data_importer::deserialize(
    mi::neuraylib::IDeserializer* deserializer)
{
...
//----------------------------------------------------------------------
const char* Rgba_raw_volume_data_importer::get_configuration() const
{
