...
#include "raw_volume_data_sequence_importer.h"
...
mi::base::Lock Raw_volume_data_sequence_importer::s_file_access_lock;
mi::Uint64     Raw_volume_data_sequence_importer::s_total_read_bytes = 0;
mi::Float64    Raw_volume_data_sequence_importer::s_total_read_time  = 0.0;

//----------------------------------------------------------------------
Raw_volume_data_sequence_importer::Raw_volume_data_sequence_importer(
...
mi::Size Raw_volume_data_sequence_importer::estimate(
...
nv::index::IDistributed_data_subset* Raw_volume_data_sequence_importer::create(
    const mi::math::Bbox_struct<mi::Sint32, 3>&     bounding_box,
    mi::Uint32                                      time_step,
    ...
    mi::neuraylib::IDice_transaction*               dice_transaction) const
{
    std::string file_name;
    {
        ...
        std::ostringstream time_step_str;
            time_step_str << m_file_name << file_sequence << ".raw";
        file_name = time_step_str.str(); 
    }
    ...
    mi::neuraylib::IDice_transaction*               dice_transaction) const
{
    ...
    {
        std::ostringstream time_step_str;
            time_step_str << m_file_name << m_start_time << ".raw";
        file_name = time_step_str.str(); 
    }
    ...
    mi::neuraylib::IDice_transaction*               dice_transaction) const
{
    ...
    INFO_LOG << "Raw volume data sequence importer loads '" << file_name << "', bounds: " << bounds;
    ...
    mi::Uint8* demo_scale_src_data = 0;
    ...
    {
        demo_scale_src_data = new mi::Uint8[volume_brick_size / demo_scale_memory]; // we only read an 8th of the size
            ...
            for (mi::Uint32 x = 0; x < dst_size.x; ++x)
            {
                for (mi::Uint32 y = 0; y < dst_size.y; ++y)
                {
                    for (mi::Uint32 z = 0; z < dst_size.z; ++z)
                    {
                        ...
                        voxel_data[dst_o] = oval;
                    }
                }
            }
            delete [] demo_scale_src_data;
            demo_scale_src_data = 0;
        }
        else
        {
            // Fetch data for the entire brick at once
    ...
    io::File in_file(file_name.c_str(), std::ios_base::in);
    if (!in_file)
    {
    ...
    for (mi::Sint32 x = bounds.min.x; x < bounds.max.x; ++x)
    {
        ...
        for (mi::Sint32 y = bounds.min.y; y < bounds.max.y; ++y)
        {
            ...
            if (bytes_to_read != in_file.read(&voxel_data[memory_offset], file_offset, bytes_to_read))
            {
                ERROR_LOG << "Raw_volume_importer::create(): "
...
//----------------------------------------------------------------------
mi::base::Uuid Raw_volume_data_sequence_importer::subset_id() const
{
...
//----------------------------------------------------------------------
const char* Raw_volume_data_sequence_importer::get_configuration() const
{
    return m_configuration.c_str();
}

//----------------------------------------------------------------------
void Raw_volume_data_sequence_importer::serialize(
...
//----------------------------------------------------------------------
void Raw_volume_data_sequence_importer::deserialize(
