...
/*                                                                 */
/*  Id : $Id: XdmfElement.cxx,v 1.34 2010-04-01 16:58:35 kwleiter Exp $  */
/*  Date : $Date: 2010-04-01 16:58:35 $ */
...
XdmfXmlNode
XdmfElement::FollowReference(XdmfXmlNode anElement, XdmfXmlNode refElement){
    XdmfConstString Value;
    ...
    /* Verify Reference in Read mode, anElement is the head XML node */
    if (!refElement) {
    ...
    /* Check Reference in Write mode, refElement is the Referenced object, does it exist? */
    else {
      if (this->DOM->IsChild( refElement, this->DOM->GetTree() ) == XDMF_SUCCESS)
        ref = refElement;
      else {
        XdmfErrorMessage("Can't Find XdmfXmlNode " << refElement << " in DOM tree");
        return((XdmfXmlNode)XDMF_ERROR_REFERENCE);
      }
    }
    return(ref);
...
XdmfInt32
XdmfElement::GetNumberOfParents(XdmfXmlNode anElement) {
   int nParents = 0;
   while (anElement != NULL) {
     anElement = anElement->parent;
     nParents++;
   }
   XdmfDebug("Number of parents for XdmfXmlNode: " << nParents);
   return(nParents);
}

XdmfXmlNode
XdmfElement::CheckForReference(XdmfXmlNode anElement, XdmfXmlNode refElement){
    ...
    node = this->FollowReference(anElement, refElement);
    ...
    if(node){
        ...
        XdmfErrorMessage("This is not a Reference node");
        return((XdmfXmlNode)XDMF_EMPTY_REFERENCE);
    }
    XdmfDebug("Setting ReferenceElement");
    if (refElement) this->ReferenceElement = refElement; // In Write mode this is the reference
    else            this->ReferenceElement = anElement;  // TODO: Check why this is the reference in Read mode???
...
XdmfInt32
XdmfElement::BuildFromDataXml(XdmfInt32 AllowCData, XdmfInt32 force){
    if(this->DataXml){
        if(AllowCData){
            char        first = 0;
            XdmfInt64   i = 0;
        ...
        if(this->DOM){
            // If not force to append Data Xml check if already done
            if((!force) && (this->InsertedDataXml == this->DataXml)){
                // Already done
                return(XDMF_SUCCESS);
            }
            ...
            XdmfErrorMessage("Can't insert raw XML since DOM is not set");
            return(XDMF_FAIL);
        }
    }
    return(XDMF_FAIL);
...
XdmfInt32 XdmfElement::Build(){
    ...
    /* Unset all attributes in case of a Reference */
    if (this->IsReference)
      this->GetDOM()->UnsetAll( this->Element );

    name = this->GetName();
    if(name && (strlen(name) > 0)){
        this->Set("Name", name);
    }
    /* If this is a Reference element, free all previously added
     * attributes, add Reference attribute and return
     * to avoid adding other attributes to the XdmfXmlNode */
    if (this->IsReference) {
      XdmfConstString XPath;
      XPath = this->DOM->GetPath(ReferenceElement);
      this->Set("Reference", XPath);
      return(XDMF_SUCCESS);
    }
    if(this->DOM){
