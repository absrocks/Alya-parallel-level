...
/*                                                                 */
/*  Id : $Id: XdmfHDF.cxx,v 1.15 2009-11-27 10:03:29 biddisco Exp $  */
/*  Date : $Date: 2009-11-27 10:03:29 $ */
...
//    XdmfErrorMessage("Target = " << XDMF_64BIT_CAST(dest_npts) << " items");
  if( src_npts != dest_npts ) {
    ...
    // for (int i= 0; i< src_npts; i++) printf("%lf ", ((double*)Array->GetDataPointer())[i]);
  }

// Added by Raul
  //int ndims;
  //hsize_t dims[2], maxdims[2];
  //H5Sget_simple_extent_dims( Array->GetDataSpace(), &dims[0], &maxdims[0] );
  //ndims = H5Sget_simple_extent_ndims( Array->GetDataSpace());
  //printf("Array->get_simple_extent_dims: %d, %d, ndims: %d\n", dims[0], maxdims[0], ndims );
  //H5Sget_simple_extent_dims( this->GetDataSpace(), &dims[0], &maxdims[0] );
  //ndims = H5Sget_simple_extent_ndims( this->GetDataSpace());
  //printf("this->get_simple_extent_dims: %d, %d, ndims: %d\n", dims[0], maxdims[0], ndims );
// Added by Raul
...
XdmfHDF::DoOpen( XdmfConstString DataSetName, XdmfConstString access ) {
  ...
  XdmfInt32  AllowOpen = 0, AllowCreate = 0;
  ostrstream FullFileName;

  XdmfDebug("DataSetName: " << DataSetName << " access: " << access );
  if( DataSetName != NULL ) {
    ...
    if( ( lastcolon == NULL ) && ( firstcolon == NULL ) ){
      ...
      *firstcolon = '\0';
      ...
      XdmfDebug("Two Colons -  Full HDF Filename Domain : " <<
        this->Domain << " File " << this->FileName <<
        " Path " << this->Path);
    } else {
      // One :
      //  
