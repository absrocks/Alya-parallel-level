...
// Organizations
#define XDMF_ITEM_UNIFORM        0x00 // Single Array
#define XDMF_ITEM_HYPERSLAB      0x01 // Multiple arrays in several ways (see Xdmf std)
...
//! Update the DOM (OUTPUT)
    ...
    */
    ...
    //! Get if ItemType evaluates to a Single or Multiple XdmfArray. Only Uniform is Single (Collection and tree are multiple).
    //XdmfInt32   GetIsMultiple() { return((this->ItemType & XDMF_ITEM_MASK) ? 1: 0); };
    XdmfInt32   GetIsMultiple() { return(this->ItemType != XDMF_ITEM_UNIFORM); };
...
protected:
    ...
    XdmfInt32       ItemType;
    XdmfDataDesc    *DataDesc;  // It should describe TYPE, SHAPE and SELECTION for file space
    XdmfArray       *Array;     // It should describe TYPE, SHAPE and SELECTION for memory space
    XdmfValues      *Values;
