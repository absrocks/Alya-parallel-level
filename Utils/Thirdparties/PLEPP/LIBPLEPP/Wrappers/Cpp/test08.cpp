/*
  ...
  Migue Zavala
  FROM:
      z2019_1/REPOSITORY/HEXAS_PLEPP/LIBPLEPP/Wrappers/Cpp
*/
...
void get_baricentric_coords_j(double*  coords, int* vertices, int n_dist_j, int* elemts_i, double*      coords_j, double* tetracoords_j);
...
//  string  name_argv = "";
  ...
  if(commij != MPI_COMM_NULL)
  {
    ...
    { // for2
      ...
      vector<double>  props_i(n_vertices_i, 0.0);
      for(int i=0,k=0; i<n_vertices_i; i++) for(int j=0; j<DIM; j++,k++) props_i[i] += points[k] * points[k] ;

      int id_max = std::distance( props_i.begin(), std::max_element(props_i.begin(), props_i.end()) ) ;
      int id_min = std::distance( props_i.begin(), std::min_element(props_i.begin(), props_i.end()) ) ;
      for(int i=0,k=0; i<n_vertices_i; i++) props_i[i] = (props_i[i] -props_i[id_min])/(props_i[id_max]-props_i[id_min]);

      // Interpolation of 'local properties'(props_i) to 'remote points'(var_ij)
      int n_send = CD.get_n_dist_points();
      vector<int>    dist_locations_i(n_send,                 -1);
      vector<double>    dist_coords_j(n_send*DIM,       HUGE_VAL);
      CD.__locator_get_dist_locations__( dist_locations_i.data() ); // local cells  with
      CD.__locator_get_dist_coords__(       dist_coords_j.data() ); // remote points

      vector<double>   shapef_j( n_node*n_send, HUGE_VAL);
      get_baricentric_coords_j(vertex_coords_i_ptr, vertex_num_i_ptr,
                               n_send, dist_locations_i.data(), dist_coords_j.data(), shapef_j.data() );
/*
if( name_argv == NAMEj )
for(int i=0, k=0; i<n_send; i++)
{
  for(int j=0; j<n_node; j++, k++) cout<< shapef_j[k] <<" ";
  cout<<"\n";
}
*/
      vector<double> var_ij(n_send, HUGE_VAL);
      propi2propj(props_i.data(), vertex_num_i_ptr, var_ij.size(), dist_locations_i.data(), shapef_j.data(), var_ij.data());
/*
if( name_argv == NAMEj )
for(int i=0; i<var_ij.size(); i++)
{
cout<< var_ij[i] <<" ";
cout<<"\n";
}
*/
      ...
      vector<double> var_ji(n_recv, HUGE_VAL);
      ...
      cout<<"["<< name_argv<<"] "
          << std::accumulate(var_ij.begin(), var_ij.end(), 0.0) / var_ij.size() <<" "
          << std::accumulate(var_ji.begin(), var_ji.end(), 0.0) / var_ji.size() <<" "
          <<" \n";
...
propi2propj(double* props_i, int* vertices, int n_dist_j, int* elemts_i, double* tetracoords_j, double* props_j )
{
  ...
  if(n_dist_j > 0)
  {
    ...
    {
      int ielem = elemts_i[ii]-1; //  <- Fortran style ??
...
{
cout<<" ";
for(int jj=0; jj<n_node; jj++) cout<< prop_j[jj] <<" ";
cout<<": "<< props_j[ii] <<" ";
cout<<" \n";
}
*/
...
void
get_baricentric_coords_j(double* coords, int* vertices, int n_dist_j, int* elemts_i, double* coords_j, double* tetracoords_j)
{
  if(n_dist_j > 0)
  {
    for(int ii=0; ii<n_dist_j; ii++ )
    {
      vector<double>  point_j(DIM, HUGE_VAL);
      for(int jj=0; jj<DIM;     jj++)   point_j[jj] = coords_j[DIM*ii + jj];

      int ielem = elemts_i[ii]-1; //  <- Fortran style ??
      vector<int>  vertices_i(n_node, -1);
      for(int jj=0; jj<n_node; jj++) vertices_i[jj] = vertices[n_node*ielem + jj]; // <- Fortran style, yes!! (ple rest one)

      vector<double> vol_coords_j(n_node, HUGE_VAL);
      CD.__simple_interpolation__( coords, vertices_i.data(), point_j.data(), vol_coords_j.data() );

      for(int jj=0; jj<n_node; jj++) tetracoords_j[n_node*ii + jj] = vol_coords_j[jj];
/*
if( name_argv == NAMEj ){
      for(int jj=0; jj<n_node; jj++) cout<< vol_coords_j[jj] <<" ";
//      for(int jj=0; jj<n_node; jj++) cout<< tetracoords_j[n_node*ii + jj] <<" ";
cout<<"\n\n";
}
*/
