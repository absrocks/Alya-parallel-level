...
python getRealBoundaries.py $name 3

# Comment out non-periodic dimensions
...
rm $per 
touch $per
./periodic_extractor/build/per.x $name $xmin $xmax 0 2  #arguments: name of case, lower limit of coordinate, upper limit of coordinate, index of coordinate: x=0 y=1 z=2, dimension of problem
cat $name".x.per" >> $per
rm $name".x.per"
